...
- [Go: сборка и развертывание](#go-сборка-и-развертывание)
- [Введение в сборку](#введение-в-сборку)
  - [Основные команды](#основные-команды)
- [go build](#go-build)
  - [Базовая сборка](#базовая-сборка)
- [Сборка текущего пакета](#сборка-текущего-пакета)
- [Сборка с указанием выходного файла](#сборка-с-указанием-выходного-файла)
- [Сборка пакета](#сборка-пакета)
  - [Флаги сборки](#флаги-сборки)
- [Сборка с отладочной информацией](#сборка-с-отладочной-информацией)
- [Сборка с оптимизацией](#сборка-с-оптимизацией)
- [Сборка статического бинарника](#сборка-статического-бинарника)
  - [Сборка для production](#сборка-для-production)
- [Оптимизированная сборка](#оптимизированная-сборка)
- [Сборка с версией](#сборка-с-версией)
- [Cross-compilation](#cross-compilation)
  - [Компиляция для других платформ](#компиляция-для-других-платформ)
- [Linux](#linux)
- [Windows](#windows)
- [macOS](#macos)
- [ARM](#arm)
  - [Поддерживаемые платформы](#поддерживаемые-платформы)
- [Список поддерживаемых платформ](#список-поддерживаемых-платформ)
- [Docker](#docker)
  - [Базовый Dockerfile](#базовый-dockerfile)
- [Многоэтапная сборка](#многоэтапная-сборка)
  - [Оптимизированный Dockerfile](#оптимизированный-dockerfile)
- [CI/CD](#cicd)
  - [GitHub Actions](#github-actions)
  - [GitLab CI](#gitlab-ci)
  - [Детальные флаги сборки](#детальные-флаги-сборки)
- [Отключение оптимизаций компилятора (для отладки)](#отключение-оптимизаций-компилятора-для-отладки)
- [Включение всех оптимизаций](#включение-всех-оптимизаций)
- [Отключение инлайнинга](#отключение-инлайнинга)
- [Включение race detector](#включение-race-detector)
- [Сборка с информацией о версии](#сборка-с-информацией-о-версии)
  - [Build tags и constraints](#build-tags-и-constraints)
  - [Использование build tags](#использование-build-tags)
- [Сборка с определенным тегом](#сборка-с-определенным-тегом)
- [Сборка с несколькими тегами](#сборка-с-несколькими-тегами)
- [Сборка без тегов](#сборка-без-тегов)
  - [Build constraints примеры](#build-constraints-примеры)
  - [CGO интеграция](#cgo-интеграция)
- [Сборка с CGO](#сборка-с-cgo)
- [Сборка без CGO (статический бинарник)](#сборка-без-cgo-статический-бинарник)
...
- [Удаление символов отладки и таблиц](#удаление-символов-отладки-и-таблиц)
- [Сравнение размеров](#сравнение-размеров)
  - [Встраивание версии в бинарник](#встраивание-версии-в-бинарник)
  - [Практические примеры: Makefile для сборки](#практические-примеры-makefile-для-сборки)
  - [Практические примеры: Улучшенный Dockerfile](#практические-примеры-улучшенный-dockerfile)
- [Build stage](#build-stage)
- [Установка зависимостей для сборки](#установка-зависимостей-для-сборки)
- [Копирование go.mod и go.sum для кэширования зависимостей](#копирование-gomod-и-gosum-для-кэширования-зависимостей)
- [Копирование исходного кода](#копирование-исходного-кода)
- [Сборка приложения](#сборка-приложения)
- [Runtime stage](#runtime-stage)
- [Установка CA сертификатов для HTTPS](#установка-ca-сертификатов-для-https)
- [Создание непривилегированного пользователя](#создание-непривилегированного-пользователя)
- [Копирование бинарника](#копирование-бинарника)
- [Установка прав](#установка-прав)
- [Переключение на непривилегированного пользователя](#переключение-на-непривилегированного-пользователя)
- [Health check](#health-check)
  - [Практические примеры: Dockerfile с кэшированием](#практические-примеры-dockerfile-с-кэшированием)
- [Копирование только файлов зависимостей](#копирование-только-файлов-зависимостей)
- [Загрузка зависимостей (кэшируется если go.mod/go.sum не изменились)](#загрузка-зависимостей-кэшируется-если-gomodgosum-не-изменились)
- [Сборка](#сборка)
  - [Практические примеры: .dockerignore](#практические-примеры-dockerignore)
- [.dockerignore](#dockerignore)
  - [Практические примеры: GitHub Actions с кэшированием](#практические-примеры-github-actions-с-кэшированием)
  - [Практические примеры: GitLab CI с артефактами](#практические-примеры-gitlab-ci-с-артефактами)
  - [Практические примеры: Jenkins Pipeline](#практические-примеры-jenkins-pipeline)
  - [Практические примеры: Docker Compose для разработки](#практические-примеры-docker-compose-для-разработки)
  - [Практические примеры: Multi-stage build с тестированием](#практические-примеры-multi-stage-build-с-тестированием)
- [Test stage](#test-stage)
  - [Практические примеры: Сборка для разных архитектур](#практические-примеры-сборка-для-разных-архитектур)
- [Скрипт для сборки для всех платформ](#скрипт-для-сборки-для-всех-платформ)
  - [Практические примеры: Build с версионированием](#практические-примеры-build-с-версионированием)
  - [Практические примеры: Dockerfile с multi-stage build](#практические-примеры-dockerfile-с-multi-stage-build)
- [Stage 1: Build](#stage-1-build)
- [Кэширование зависимостей](#кэширование-зависимостей)
- [Копирование исходников](#копирование-исходников)
- [Stage 2: Runtime](#stage-2-runtime)
  - [Практические примеры: Build tags для разных окружений](#практические-примеры-build-tags-для-разных-окружений)
  - [Практические примеры: GitHub Actions CI/CD](#практические-примеры-github-actions-cicd)
  - [Практические примеры: GitLab CI/CD](#практические-примеры-gitlab-cicd)
  - [Практические примеры: Сборка с CGO](#практические-примеры-сборка-с-cgo)
- [Сборка с указанием C компилятора](#сборка-с-указанием-c-компилятора)
  - [Практические примеры: Оптимизация размера бинарника](#практические-примеры-оптимизация-размера-бинарника)
- [Удаление символов отладки](#удаление-символов-отладки)
- [Минимизация размера с UPX](#минимизация-размера-с-upx)
- [Анализ размера бинарника](#анализ-размера-бинарника)
  - [Практические примеры: Build с race detector](#практические-примеры-build-с-race-detector)
- [Сборка с race detector](#сборка-с-race-detector)
- [Запуск с race detector](#запуск-с-race-detector)
- [Тесты с race detector](#тесты-с-race-detector)
