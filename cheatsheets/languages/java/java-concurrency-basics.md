---
title: "Java Concurrency: основы"
description: "Материал по теме Java Concurrency: основы в разделе cheatsheets."
tags:
  - languages
  - java
...
- [Обзор java.util.concurrent](#обзор-javautilconcurrent)
  - [Executor](#executor)
  ...
  - [BlockingQueue и DelayQueue](#blockingqueue-и-delayqueue)
  - [Locks](#locks)
  - [Phaser](#phaser)
- [Руководство по synchronized](#руководство-по-synchronized)
  - [Синхронизированные методы](#синхронизированные-методы)
  - [Синхронизированные статические методы](#синхронизированные-статические-методы)
  - [Синхронизированные блоки](#синхронизированные-блоки)
  - [Повторно используемая блокировка](#повторно-используемая-блокировка)
- [Руководство по volatile](#руководство-по-volatile)
  - [Модификатор volatile](#модификатор-volatile)
  - [Эффекты видимости памяти](#эффекты-видимости-памяти)
- [Руководство по Future](#руководство-по-future)
  - [ForkJoinTask](#forkjointask)
- [Руководство по ThreadLocal](#руководство-по-threadlocal)
  - [Пример использования ThreadLocal](#пример-использования-threadlocal)
  - [ThreadLocal и пулы потоков](#threadlocal-и-пулы-потоков)
...
- [Как убить поток?](#как-убить-поток)
- [Руководство по пулам потоков](#руководство-по-пулам-потоков)
  - [ThreadPoolExecutor](#threadpoolexecutor)
  - [ScheduledThreadPoolExecutor](#scheduledthreadpoolexecutor)
  - [ForkJoinPool](#forkjoinpool)
- [Реализация Runnable и расширение потока](#реализация-runnable-и-расширение-потока)
...
- [Руководство по метод Thread.join()](#руководство-по-метод-threadjoin)
- [Использование объекта Mutex](#использование-объекта-mutex)
- [ThreadPoolTaskExecutor corePoolSize против maxPoolSize](#threadpooltaskexecutor-corepoolsize-против-maxpoolsize)
- [Асинхронное программирование](#асинхронное-программирование)
  - [Использование Thread](#использование-thread)
  - [Использование ExecutorService и Future](#использование-executorservice-и-future)
  - [Использование CompletableFuture](#использование-completablefuture)
- [Почему локальные переменные потокобезопасны?](#почему-локальные-переменные-потокобезопасны)
- [Как получить количество потоков в процессе?](#как-получить-количество-потоков-в-процессе)
  - [Использование Thread.getAllStackTraces()](#использование-threadgetallstacktraces)
  - [Использование ThreadGroup](#использование-threadgroup)
...
Вызов **cancel()** может завершиться ошибкой — тогда возвращается **false**. Метод **cancel()** принимает логическое значение. Это определяет, должен ли поток, выполняющий задачу, быть прерван или нет.
...
**Теперь мы создадим **ExecutorService**, отправим задачу, а затем используем возвращенный метод **get** из **Future**, чтобы дождаться завершения отправленной задачи и возврата значения:**
...
**Метод **call()** из **Callable** содержит предложение **"throws `Exception`"**, поэтому мы можем легко распространять проверенные исключения дальше:**
...
- **Не расширять `Thread`:** предпочитать реализацию `Runnable` или `Callable` и передачу в `ExecutorService`; пулы потоков управляют жизненным циклом и переиспользованием потоков.
- **Синхронизация:** минимизировать область блокировки; предпочитать блокировки на финальных объектах; использовать `synchronized` на приватных полях или явные `Lock` при необходимости таймаутов/условий.
- **volatile:** использовать для флагов и **visibility** без составных инвариантов; для счётчиков и сложной логики — `Atomic` или блокировки.
- **wait/notify:** всегда вызывать в цикле `while (condition) { wait(); }`; использовать в рамках одного монитора; предпочитать `java.util.concurrent` (Condition, BlockingQueue) при возможности.
- **Потоки vs `ExecutorService`:** для задач и расписаний — `ExecutorService`, `ScheduledExecutorService`; не создавать множество «голых» потоков для коротких задач.
- **Пул потоков:** настраивать `corePoolSize` и `maxPoolSize` под нагрузку и тип задач; использовать ограниченные очереди и политику отказа при перегрузке; закрывать пул через **shutdown()** / **awaitTermination()**.
- **ThreadLocal:** очищать в **finally** или при снятии контекста (например, в веб-фильтрах), чтобы избежать утечек в пулах потоков.
