...
**Нюанс про Grafana.** С Linkerd 2.12 Grafana исключена из бандла Viz из-за лицензии AGPL: её ставят отдельно (например, Helm-чартом) и подключают к ней официальные дашборды Linkerd. Distributed tracing — тоже не часть Viz, а отдельное расширение `linkerd-jaeger`. Так что формула «Viz = Prometheus + Grafana + Jaeger в одном пакете» устарела — в собеседовании это стоит проговорить явно.
