...
**Иерархия сертификатов** — три уровня:
- **Trust anchor** — корневой CA, общий для всего mesh (а в multi-cluster — для всех кластеров);
- **Issuer** — промежуточный сертификат, которым сервис identity подписывает сертификаты подов;
- **Leaf-сертификаты подов** — самый нижний уровень, живут 24 часа и ротируются автоматически.

**Подводный камень — истечение issuer и trust anchor.** Автоматически ротируются только leaf-сертификаты; issuer и trust anchor сами не обновляются. Issuer, сгенерированный CLI при установке, по умолчанию живёт год, и его истечение — классический production-инцидент: identity перестаёт выдавать сертификаты, прокси не могут установить mTLS, и mesh фактически ложится. Защита: следить за сроками через `linkerd check` (он предупреждает об истекающих сертификатах) и автоматизировать ротацию issuer через cert-manager; ротация trust anchor — отдельная процедура, которую стоит отрепетировать заранее.
...
**mTLS — ещё и фундамент авторизации.** Поверх криптографической идентичности работают политики доступа: с Linkerd 2.11 — CRD `Server` и `ServerAuthorization`, с 2.12 — `AuthorizationPolicy` и `MeshTLSAuthentication`. Они ограничивают, какие mesh-идентичности (ServiceAccount) могут обращаться к конкретному порту или маршруту, вплоть до режима default-deny для всего кластера.

## Q10. Как Linkerd делает traffic split для canary и blue-green?

**Современный механизм — Gateway API: `HTTPRoute` с весами в `backendRefs` (полноценная поддержка с Linkerd 2.14); классический — SMI `TrafficSplit`, который с Linkerd 2.12 вынесен из ядра в отдельное расширение `linkerd-smi` и считается legacy.** Идея в обоих случаях одна: задаёшь веса между версиями сервиса на уровне mesh, не трогая код приложения, и постепенным сдвигом весов реализуешь canary или blue-green выкатку.

**TrafficSplit** (спецификация SMI, legacy-путь):
```yaml
...
**Нюанс.** Пример выше — SMI: в свежих установках он потребует расширения `linkerd-smi`, и развивать этот путь больше не будут. Современный эквивалент — `HTTPRoute` из Gateway API, где те же веса задаются полем `weight` у `backendRefs`; семантика распределения 90/10 идентична, но вдобавок доступен матчинг по пути/заголовкам и единый стандартный API вместо вендорной спецификации.
...
**В связке с Flagger** (CNCF) получается автоматизированный progressive delivery: canary с продвижением на основе метрик. Flagger откатит выкатку автоматически, если success rate или latency деградируют; без него сдвиг весов остаётся ручным редактированием весов. Flagger умеет работать и с SMI `TrafficSplit`, и с Gateway API `HTTPRoute`.

## Q11. Как Linkerd настраивает retries и timeouts?

**Retries и timeouts задаются per-route, а не глобально на весь mesh. Классический механизм — `ServiceProfile`; современный (с Linkerd 2.16) — Gateway API: аннотации `retry.linkerd.io/*` и `timeout.linkerd.io/*` на `HTTPRoute`/`GRPCRoute` или на Service.** В обоих случаях один и тот же сервис может иметь retryable GET и non-retryable POST — чтобы прокси не повторял неидемпотентные операции и не плодил дубликаты.

**Service profiles (классический путь):**
```yaml
...
**Современный путь — Gateway API.** С Linkerd 2.16 ретраи и таймауты вешаются аннотациями `retry.linkerd.io/*` (какие классы ответов повторять, лимит попыток) и `timeout.linkerd.io/*` прямо на `HTTPRoute`, `GRPCRoute` или Service — это закрывает старый пробел ServiceProfile, где тонкой настройки по кодам ответа не было. ServiceProfile продолжает работать, но это уходящий механизм: новые возможности развиваются вокруг Gateway API.

**Нюанс.** Retry budget в Linkerd ограничивает долю повторов, чтобы ретраи не устроили retry storm. **Гибкости всё равно меньше, чем в Istio**, но ~80% сценариев покрыто.
...
**`Linkerd Viz` — опциональное расширение с веб-дашбордом и собственным Prometheus.** Это «лицо» observability: метрики Linkerd собирает в любом случае, а Viz делает их наглядными. Основной mesh работает и без него.
...
**Нюанс про Grafana.** С Linkerd 2.12 Grafana исключена из бандла Viz из-за лицензии AGPL: её ставят отдельно (например, Helm-чартом) и подключают к ней официальные дашборды Linkerd. Distributed tracing — тоже не часть Viz: исторически его добавляло расширение `linkerd-jaeger`, но оно объявлено устаревшим, и современный путь — экспортировать трейсы по `OpenTelemetry`/`OTLP` в собственный коллектор (Tempo, Jaeger и т.п.). Так что формула «Viz = Prometheus + Grafana + Jaeger в одном пакете» устарела — в собеседовании это стоит проговорить явно.
...
- нужны **только базовые возможности mesh** — mTLS, observability, traffic split, политики авторизации по mTLS-идентичности (`Server`/`AuthorizationPolicy`);
- развёртывание **только в Kubernetes**.

**Не выбирай, когда:**
- нужны **продвинутые фичи** Istio (например, валидация end-user JWT: авторизация в Linkerd строится на mTLS-идентичности сервисов, а не на пользовательских токенах);
...
6. **Нет валидации end-user JWT** (аналога Istio RequestAuthentication) — здесь Istio сильнее. Важно не преувеличивать: авторизация в Linkerd есть — с 2.11 это `Server`/`ServerAuthorization`, с 2.12 — `AuthorizationPolicy` на базе mTLS-идентичности; слабое место именно политика по пользовательским токенам, а не авторизация вообще.
...
**В феврале 2024 Buoyant (создатель) прекратила публиковать бесплатные стабильные (stable) релизы Linkerd.** Сам код остался open source под Apache 2.0, а разработка продолжается в виде еженедельных edge-релизов; готовые стабильные/LTS-сборки и поддержку теперь даёт коммерческий Buoyant Enterprise for Linkerd (BEL).

Что получилось на практике:
- **Linkerd Open Source** — та же Apache 2.0, бесплатно; поставляется как edge-релизы, которые Buoyant позиционирует как production-ready (а не «сырые»);
- **Buoyant Enterprise (BEL)** — платно: собранные stable/LTS-сборки, продвинутые фичи и поддержка;
- **готовых бесплатных «stable»-артефактов в свободном доступе больше нет** — стабильные сборки идут через BEL.

**Эффект:**
- **хобби и OSS**-пользователи — по-прежнему бесплатно, на edge-релизах;
- кому нужна **готовая стабильная сборка с поддержкой** — берут BEL;
- **в сообществе это вызвало споры**: часть пользователей считает потерю бесплатного stable-канала фактическим ухудшением, тогда как Buoyant настаивает, что edge production-ready.
