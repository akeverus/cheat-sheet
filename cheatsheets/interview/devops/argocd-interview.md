...
`ArgoCD` хранит историю деплоев (по умолчанию 10 последних ревизий, глубина настраивается через `app.kubernetes.io/revision`), поэтому откатиться можно несколькими способами. Но важно понимать разницу между откатом *в ArgoCD* и откатом *в Git* — это ключевой нюанс вопроса (см. ниже).
