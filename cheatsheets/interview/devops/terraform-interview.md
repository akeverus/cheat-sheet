...
- `state rm` — «забыть» о ресурсе, оставив его живым в облаке (например, передать управление в другой проект).
...
Это одна и та же платформа в двух вариантах поставки. Terraform Cloud — SaaS, который хостит HashiCorp; Terraform Enterprise — та же функциональность, но разворачивается у тебя (self-hosted). Различие сводится к тому, *где* живут данные и насколько строгие требования к compliance.
...
**5. Terraform 1.7+ State Encryption** — нативное шифрование самого state-файла внутри Terraform, не полагаясь на backend:
```hcl
terraform {
  encryption {
