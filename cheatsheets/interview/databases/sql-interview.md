...
- [Q3. (!) В чём разница `PRIMARY KEY` и `UNIQUE`?](#q3--в-чём-разница-primary-key-и-unique)
...
`SQL` (`Structured Query Language`) — декларативный язык для работы с реляционными БД: вы описываете *какой* результат нужен, а оптимизатор сам решает *как* его получить. Язык делят на четыре подмножества по типу задачи — это деление важно, потому что для разных подмножеств действуют разные правила транзакционности (например, `DDL` во многих СУБД нельзя откатить).
...
Ограничения (`constraints`) — это правила, которые СУБД проверяет на уровне самой базы, поэтому данные остаются корректными независимо от того, какое приложение их пишет. Это надёжнее, чем валидация в коде: ни баг в сервисе, ни прямой `INSERT` из консоли не смогут нарушить инвариант. Основные ограничения:
...
**Что происходит при удалении/обновлении родительской строки** — задаётся через `ON DELETE` / `ON UPDATE`:

- `CASCADE` — удалить/обновить и дочерние строки вслед за родительской
- `SET NULL` — обнулить ссылку в дочерних строках
- `SET DEFAULT` — подставить значение по умолчанию
- `RESTRICT` / `NO ACTION` — запретить операцию, если есть зависимые строки (`RESTRICT` проверяет сразу, `NO ACTION` — в конце оператора, что важно для отложенных ограничений)

## Q3. (!) В чём разница `PRIMARY KEY` и `UNIQUE`?

Оба обеспечивают уникальность значений, но `PRIMARY KEY` — это «главный» идентификатор строки (один на таблицу, без `NULL`), а `UNIQUE` — дополнительное ограничение бизнес-уникальности, которых может быть много и которые допускают `NULL`. По сути `PRIMARY KEY` = `UNIQUE` + `NOT NULL` + роль «по умолчанию для `FOREIGN KEY`».
...
`NULL` — это маркер «значение неизвестно или отсутствует», а не ноль и не пустая строка. Ключевая идея: `NULL` нельзя сравнивать обычными операторами, потому что «неизвестное» не равно и не не-равно чему-либо — отсюда трёхзначная логика. Основные правила:

- Любое сравнение с `NULL` даёт `UNKNOWN` (не `TRUE` и не `FALSE`) — поэтому `WHERE col = NULL` никогда не вернёт строк
- Для проверки используются `IS NULL` / `IS NOT NULL`
- Арифметика с `NULL` даёт `NULL`: `5 + NULL = NULL`
- Агрегатные функции (`SUM`, `AVG`, `COUNT(column)`) игнорируют `NULL` — это часто источник сюрпризов: `AVG` считает среднее только по непустым значениям
...
Главное различие — отношение к часовым поясам: `DATETIME` хранит «настенное» время как есть, а `TIMESTAMP` приводит его к UTC при записи и обратно при чтении (поэтому одно и то же значение «выглядит» по-разному в разных зонах). Плюс у `TIMESTAMP` в MySQL узкий диапазон (до 2038 года — наследие 32-битного хранения).
...
**Нормализация** — разбиение данных на таблицы так, чтобы каждый факт хранился ровно в одном месте. Это убирает избыточность и три класса аномалий: при **вставке** (нельзя добавить факт без не относящихся к нему данных), **обновлении** (один факт продублирован — придётся менять во многих строках, легко рассинхронизировать) и **удалении** (удаляя строку, теряешь заодно нужный факт). Нормальные формы — это лестница: каждая следующая форма включает требования предыдущей и устраняет ещё один тип зависимости.

Лестница нормальных форм (каждая следующая включает требования предыдущей):

- Ненормализованная форма (`UNF`) →
- `1NF`: атомарные значения →
- `2NF`: нет частичных зависимостей →
- `3NF`: нет транзитивных зависимостей →
- `BCNF`: каждый детерминант — кандидат-ключ →
- `4NF`: нет многозначных зависимостей →
- `5NF`: нет зависимостей соединения

Та же лестница как поток сверху вниз -- каждая стрелка добавляет одно требование:
...
**Денормализация** — осознанный возврат избыточности ради скорости чтения: данные дублируются, чтобы избавиться от дорогих `JOIN` и агрегаций на горячем пути. Это обратная сторона нормализации — вы платите усложнением записи за дешёвое чтение. Типичные приёмы:
...
**Когда оправдана:**
- В **OLAP/DWH** (аналитические хранилища, схемы «звезда» и «снежинка»), где пишут пакетами, а читают много и сложно
- Когда `JOIN` по большому числу таблиц стал доказанным узким местом (подтверждено профилированием, а не «на всякий случай»)
- При необходимости крайне низкой задержки на чтение

**Цена:** при каждой записи нужно обновлять все копии факта, иначе данные разойдутся. Поэтому денормализацию обычно дополняют триггерами, фоновыми пересчётами или материализованными представлениями, чтобы синхронизацию не приходилось держать в голове у разработчика.

## Q9. Что такое `View` и `Materialized View`?

**View** (представление) — это сохранённый запрос под именем таблицы. Физических данных за ним нет: при каждом обращении запрос выполняется заново, поэтому `View` всегда показывает актуальные данные, но стоит ровно столько же, сколько лежащий в основе запрос. Удобен, чтобы спрятать сложную логику и переиспользовать её.
...
**Materialized View** — представление, результат которого один раз посчитан и физически сохранён. Чтение мгновенное (данные уже готовы), но за это платишь устареванием: чтобы освежить данные, нужен явный `REFRESH`. По сути это кэш тяжёлого запроса.
...
И то, и другое — код, который хранится и выполняется на сервере БД (логика рядом с данными, без сетевых round-trip'ов). Разница в роли: **Function** обязана вернуть значение и вызывается *внутри* запроса (`SELECT get_x(...)`), поэтому ей нельзя управлять транзакцией. **Stored Procedure** вызывается отдельно через `CALL`, может ничего не возвращать и (в PostgreSQL 11+) умеет делать `COMMIT`/`ROLLBACK` — это нужно для пакетной обработки, где результат фиксируют порциями.
...
**Триггер** — код, который СУБД сама вызывает в ответ на `INSERT`, `UPDATE` или `DELETE`. Главное достоинство — гарантия: логика отработает при *любом* изменении таблицы, даже если кто-то правит данные в обход приложения. Поэтому триггеры хороши для аудита, поддержания инвариантов и заполнения служебных полей.
...
**Виды триггеров** (две независимые оси):
- по моменту: `BEFORE` (можно изменить или отклонить строку до записи), `AFTER` (данные уже записаны — удобно для аудита), `INSTEAD OF` (подмена операции, нужна для записи через `View`)
- по охвату: `FOR EACH ROW` (срабатывает на каждую затронутую строку) или `FOR EACH STATEMENT` (один раз на весь оператор)
...
**Временная таблица** видна только создавшей её сессии и автоматически исчезает при её завершении (или при `COMMIT`, если создана с `ON COMMIT DROP`). Нужна как «черновик» для промежуточных результатов сложной обработки: разные сессии не мешают друг другу, а чистить за собой не приходится.
...
**Sequence** — отдельный объект-генератор уникальных чисел, не привязанный к конкретной таблице (в отличие от «свёрнутых» вокруг него `SERIAL`, `IDENTITY`, `AUTO_INCREMENT`). Один `sequence` можно делить между несколькими таблицами, а значение получать заранее, до `INSERT`.
...
**Важный нюанс:** `nextval` не откатывается при `ROLLBACK`. Это сделано намеренно — иначе параллельные транзакции выстраивались бы в очередь за номером. Платой становятся «дырки» в нумерации (если транзакция откатилась, её номер пропадает), поэтому на значения `sequence` нельзя полагаться как на непрерывный счётчик «без пропусков».

## Q14. Порядок выполнения `SELECT`-запроса

`SELECT` пишется не в том порядке, в котором СУБД его *логически* обрабатывает. Понимание реального порядка снимает половину «странностей» SQL — почему алиас не виден в `WHERE`, почему `WHERE` нельзя фильтровать по агрегату, почему `LIMIT` применяется последним. Логический порядок:

Логический порядок выполнения (каждый этап получает результат предыдущего):
...
Конвейер этапов сверху вниз -- результат каждого блока питает следующий:
...
Все три удаляют данные, но на разном уровне: `DELETE` (это `DML`) убирает выбранные строки построчно и логируется, `TRUNCATE` (это `DDL`) мгновенно сбрасывает всё содержимое таблицы целиком, оставляя её структуру, а `DROP` (`DDL`) удаляет саму таблицу вместе с данными. Ключевой практический критерий выбора — нужен ли `WHERE`, должны ли сработать триггеры и можно ли откатить.
...
`MERGE` (UPSERT) решает частую задачу «вставь, а если такая запись уже есть — обнови». Без него пришлось бы делать `SELECT`, потом `INSERT` или `UPDATE` — а между ними другой клиент мог вставить ту же строку (race condition). `MERGE` выполняет проверку и запись атомарно, в одном операторе. В `PostgreSQL` исторически использовался идиоматичный синтаксис `INSERT ... ON CONFLICT` (полноценный `MERGE` появился в PostgreSQL 15):
...
Все три отвечают на вопрос «оставить строки, для которых есть связь в другом наборе», но по-разному. `IN` сравнивает значение со списком, `EXISTS` проверяет сам факт наличия хотя бы одной подходящей строки (и может остановиться на первой же), а `JOIN` физически соединяет наборы. Главное практическое отличие: `IN` опасен с `NULL`, `EXISTS` от него защищён, а `JOIN` может размножить строки при связи 1:N.
...
`CASE` — условное выражение, аналог `if-else`, который возвращает значение прямо внутри `SELECT`, `ORDER BY` или агрегата. Возвращает первую ветку, чьё условие истинно (порядок важен). Есть две формы: **поисковая** (`CASE WHEN <условие> THEN ...`) для произвольных условий и **простая** (`CASE <выражение> WHEN <значение> THEN ...`) для сравнения одного выражения с набором значений.
...
`CTE` — именованный подзапрос, объявленный через `WITH` и доступный по имени в основном запросе. По сути это способ разбить громоздкий запрос на читаемые «шаги», как локальные переменные в коде. Два главных применения: сделать сложный запрос понятным сверху вниз и реализовать рекурсию (`WITH RECURSIVE`), невозможную в обычном подзапросе.
...
**Нюанс:** в старых версиях PostgreSQL (до 12) `CTE` всегда материализовался как барьер оптимизации — оптимизатор не «протаскивал» условия внутрь. С PostgreSQL 12 простые `CTE` инлайнятся по умолчанию, а явно управлять этим можно через `MATERIALIZED` / `NOT MATERIALIZED`.

## Q20. Что такое `RETURNING`?

`RETURNING` отдаёт значения строк, которые только что затронул `INSERT`/`UPDATE`/`DELETE`, прямо в результате той же команды. Это экономит лишний `SELECT` и, что важнее, избавляет от гонки: например, сгенерированный `id` или новое значение счётчика вы получаете атомарно вместе с вставкой, без отдельного запроса, который мог бы вернуть уже изменённое другим клиентом значение.
...
`JOIN` соединяет строки двух таблиц по условию. Тип `JOIN` отвечает на один вопрос: **что делать со строками, у которых нет пары?** `INNER` оставляет только совпадения; `LEFT`/`RIGHT` дополняют их всеми строками одной из таблиц (несовпавшим столбцам подставляется `NULL`); `FULL OUTER` берёт всё из обеих; `CROSS` вообще не проверяет условие и даёт декартово произведение.

Какую часть множеств A и B берёт каждый тип (закрашенная область -- что попадает в результат):

```
INNER JOIN                 LEFT JOIN
только пересечение         вся A + пересечение
   ┌───────┐                  ┌───────┐
   │ A ∩ B │                  │   A   │───[ A ∩ B ]
   └───────┘                  └───────┘

RIGHT JOIN                 FULL OUTER JOIN
пересечение + вся B        вся A + пересечение + вся B
   [ A ∩ B ]───┌───────┐      ┌───────┐               ┌───────┐
               │   B   │      │   A   │───[ A ∩ B ]───│   B   │
               └───────┘      └───────┘               └───────┘
```
...
Разница в моменте фильтрации: `ON` отбирает пары **во время** соединения, а `WHERE` — **после**, по уже готовому результату. Для `INNER JOIN` это незаметно (оптимизатор объединяет условия). Но для `LEFT`/`RIGHT JOIN` различие принципиально: условие в `WHERE` на столбец правой таблицы отбрасывает строки с `NULL` и тем самым «превращает» внешний `JOIN` в `INNER` — это классическая ловушка.
...
Короткий ответ: однозначного победителя нет — для современных оптимизаторов это часто эквивалентные записи одного плана. Выбирайте по читаемости. Единственное реальное исключение по производительности — **коррелированный подзапрос**: он зависит от каждой внешней строки и потому рискует выполняться по разу на строку. Его почти всегда стоит переписать в `JOIN`.
...
Оба «склеивают» результаты нескольких `SELECT` в один набор (по вертикали, в отличие от `JOIN`). Разница одна, но важная для производительности: `UNION` дополнительно удаляет дубликаты, а для этого вынужден отсортировать или захешировать весь результат. `UNION ALL` просто конкатенирует наборы и потому заметно быстрее.
...
`CROSS JOIN` соединяет таблицы без условия: каждая строка A комбинируется с каждой строкой B, давая декартово произведение (|A| × |B| строк). Это единственный `JOIN`, который намеренно «размножает» строки — поэтому случайный `CROSS JOIN` (забытое `ON`) обычно баг, но как осознанный приём он удобен для генерации всех комбинаций.
...
`SELF JOIN` — это обычный `JOIN` таблицы с самой собой; «особого» синтаксиса нет, нужны лишь два разных алиаса, чтобы различать «копии». Применяется, когда строки нужно сопоставлять с другими строками той же таблицы: разворачивать иерархию (`manager_id` → строка-руководитель) или искать пары по условию (одинаковая зарплата, дубликаты).
...
`GROUP BY` схлопывает строки в группы по значениям указанных столбцов, а агрегаты (`COUNT`, `SUM`, `AVG`...) считают по одной величине на группу. `WHERE` и `HAVING` фильтруют на разных этапах: `WHERE` отсеивает строки **до** группировки (и не видит агрегатов), `HAVING` отсеивает уже готовые группы **после** неё (и работает по агрегатам). Это прямое следствие порядка выполнения из Q14.
...
Для простого устранения дубликатов они эквивалентны (и обычно дают одинаковый план). Разница в назначении: `DISTINCT` умеет только убрать повторы, а `GROUP BY` — это полноценная группировка, которая вдобавок позволяет считать агрегаты по каждой группе. Правило: нужны просто уникальные значения — `DISTINCT` выразительнее; нужны агрегаты — `GROUP BY`.
...
Все три считают строки, но по-разному обходятся с `NULL` и дубликатами:

- `COUNT(*)` — считает **все** строки, включая те, где значения `NULL`
- `COUNT(column)` — считает строки, где `column` **не** `NULL` (агрегаты игнорируют `NULL`, см. Q5)
- `COUNT(DISTINCT column)` — считает **уникальные** не-`NULL` значения

На практике это частый источник расхождений в отчётах: `COUNT(*)` и `COUNT(col)` дадут разные числа, если в столбце есть пропуски.
...
Оконная функция считает агрегат или ранг по «окну» соседних строк, но при этом **не схлопывает** строки, как `GROUP BY` — каждая исходная строка остаётся в результате и получает дополнительное вычисленное значение. Это решает задачи, недоступные обычной агрегации: показать строку рядом со средним по её группе, нарастающий итог, ранг, сравнение с соседом. Окно задаётся в `OVER(...)` тремя частями: `PARTITION BY` (на какие группы делить), `ORDER BY` (порядок внутри окна) и рамка `ROWS`/`RANGE` (сколько соседних строк брать).

Три составляющие `OVER()`:
...
```
     ...
     ▼           ▼                        ▼
┌──────────┐ ┌──────────┐          ┌────────────┐
│PARTITION │ │ ORDER BY │          │ ROWS/RANGE │
│   BY     │ │          │          │            │
│ Деление  │ │ Порядок  │          │ Рамка окна │
│ на группы│ │ в окне   │          │            │
└──────────┘ └──────────┘          └────────────┘
```
...
Все три нумеруют строки по `ORDER BY`, но по-разному обрабатывают **равные значения** (ties): `ROW_NUMBER` всегда даёт уникальный номер (равным значениям — произвольный порядок), `RANK` даёт одинаковый ранг и **пропускает** следующие номера (1, 1, 3), а `DENSE_RANK` тоже даёт одинаковый ранг, но **без пропусков** (1, 1, 2). Запомнить легко по примеру с зарплатами 100, 100, 90, 80:
...
Навигационные оконные функции достают значение из **другой строки** окна, не выходя из текущей. Это снимает необходимость в `SELF JOIN` для сравнения «строка с соседом». Главные:

- `LAG(col, n)` — значение из строки на `n` позиций **назад** (предыдущий период)
- `LEAD(col, n)` — на `n` позиций **вперёд** (следующий период)
- `FIRST_VALUE` / `LAST_VALUE` — первое/последнее значение окна
- `NTH_VALUE(col, n)` — `n`-е значение окна

Классический сценарий — расчёт изменения относительно предыдущего дня/месяца (рост, отклонение):
...
Потому что оконные функции вычисляются на этапе `SELECT`, а `WHERE` отрабатывает раньше (см. порядок выполнения в Q14) — на момент фильтрации ранги и нумерация ещё не посчитаны, фильтровать просто нечего. По той же причине нельзя ссылаться на оконную функцию в `GROUP BY` или `HAVING`. Решение стандартное: посчитать окно во вложенном запросе или `CTE`, а фильтровать уже снаружи:
...
**Индекс** — вспомогательная структура, которая позволяет находить строки, не сканируя всю таблицу (как алфавитный указатель в книге). Это превращает поиск из O(n) в O(log n). Стандартная реализация — **B+tree**: сбалансированное дерево, где все реальные ключи лежат в листьях, а листья связаны в список — поэтому B+tree одинаково хорош и для точного поиска (`=`), и для диапазонов (`BETWEEN`, `>`), и для `ORDER BY` (данные в индексе уже отсортированы).

Структура B+tree (корень → промежуточные узлы → листья со ссылками на строки):
...
Тот же индекс как дерево с ветвлением -- корень делит диапазон, листья указывают на строки таблицы:
...
```
       ...
       └───────────┘      └───────────┘      └────────────────┘
          │    │             │    │            │     │     │
          ▼    ▼             ▼    ▼            ▼     ▼     ▼
       строки строки      строки строки     строки строки строки
       1-10  11-20        31-40 41-50       61-70 71-80 81-90
```
...
**Правило «левого префикса»:** составной индекс `(A, B, C)` отсортирован сначала по `A`, внутри равных `A` — по `B`, и так далее (как сортировка по фамилии, потом имени). Поэтому он работает для `WHERE A`, `WHERE A AND B`, `WHERE A AND B AND C`, но **не** для `WHERE B` или `WHERE C` в одиночку — без значения `A` неизвестно, в какой части дерева искать. Отсюда практический вывод: порядок столбцов в составном индексе нужно подбирать под запросы.
...
Главное различие — где лежат сами данные строк. **Clustered** индекс задаёт физический порядок строк на диске: данные таблицы *и есть* листья этого дерева. Поэтому он один на таблицу и делает диапазонные запросы очень быстрыми (нужные строки лежат рядом). **Non-clustered** индекс хранит отдельно от данных только ключ и указатель на строку — их может быть много, но за фактическими данными приходится делать дополнительный «прыжок» к таблице.
...
**Covering Index** (покрывающий) — индекс, в котором есть все столбцы, нужные запросу (и в условии, и в `SELECT`). Тогда СУБД отвечает прямо из индекса, не делая дорогой «прыжок» к таблице за остальными полями — это режим **Index-Only Scan**, заметно быстрее обычного `Index Scan`:
...
**Partial Index** (частичный) — индексирует только строки, подходящие под `WHERE`-условие. Смысл: если запросы всегда касаются маленькой «горячей» части таблицы (активные заказы, неудалённые записи), нет смысла индексировать остальное — частичный индекс получается компактнее, а значит и быстрее в поиске, и дешевле в обновлении:
...
Индекс — не «волшебная кнопка скорости»: оптимизатор сознательно его игнорирует, когда полный скан дешевле. Общий принцип: индекс выигрывает, только если он отсекает большую часть таблицы и обращение к индексу + переходы к строкам в сумме дешевле линейного чтения. Типичные случаи, когда это не так:
...
`ACID` — четыре гарантии, благодаря которым на транзакцию можно полагаться даже при сбоях и конкурентном доступе. Идея: группа операций ведёт себя как одна неделимая «атомарная» единица, после успешного `COMMIT` её результат не теряется, а параллельные транзакции не мешают друг другу.
...
Уровень изоляции — это компромисс между корректностью и параллелизмом: чем строже изоляция, тем меньше странных эффектов от конкурентных транзакций, но тем больше блокировок и ниже пропускная способность. Стандарт SQL определяет четыре уровня по тому, какие три аномалии они допускают (от самого слабого к самому строгому):
...
**Аномалии** (каждый следующий уровень устраняет ещё одну):
- **Dirty Read** — видишь незакоммиченные данные другой транзакции, которые потом могут откатиться
- **Non-repeatable Read** — повторное чтение **той же строки** даёт другой результат (кто-то её изменил и закоммитил между чтениями)
- **Phantom Read** — повторный запрос **по тому же условию** возвращает новые строки (кто-то их вставил)
...
**Deadlock** — ситуация, когда две (или больше) транзакции зашли в тупик: каждая держит ресурс, нужный другой, и ждёт встречного освобождения, которого не будет. Сами по себе они не разрешатся, поэтому СУБД обнаруживает цикл ожидания и принудительно откатывает одну из транзакций («жертву»), чтобы разорвать его.

Цикл взаимного ожидания -- стрелки «ждёт» замыкаются в кольцо, которого никто не разорвёт сам:

```
┌──────────────────┐    ждёт B    ┌──────────────────┐
│  Транзакция 1    │ ───────────▶ │  Транзакция 2    │
│  Захватила A     │              │  Захватила B     │
│  Ждёт B          │ ◀─────────── │  Ждёт A          │
└──────────────────┘    ждёт A    └──────────────────┘
```
...
Разница в том, *когда* проверяется конфликт. **Пессимистичная** блокировка исходит из «конфликт вероятен»: захватывает строку заранее (`SELECT ... FOR UPDATE`), и остальные ждут. **Оптимистичная** исходит из «конфликт маловероятен»: ничего не блокирует, а при сохранении проверяет, не изменил ли данные кто-то ещё (через столбец `version`) — и если изменил, отклоняет операцию. Выбор зависит от того, насколько часто реально дерутся за одни и те же данные.
...
`EXPLAIN` — главный инструмент диагностики медленных запросов: он показывает **план выполнения**, то есть какие операции (сканы, соединения, сортировки) и в каком порядке выберет оптимизатор. `EXPLAIN` даёт лишь оценки планировщика; `EXPLAIN ANALYZE` дополнительно *выполняет* запрос и показывает реальное время и число строк — сравнение оценки с фактом сразу выявляет, где планировщик ошибся (обычно из-за устаревшей статистики).
...
Базовое правило оптимизации — сначала измерить (`EXPLAIN ANALYZE`), потом чинить узкое место, а не угадывать. Цель почти всегда одна: заставить СУБД трогать как можно меньше данных. Основные рычаги:
...
`N+1` — антипаттерн ORM: вы делаете 1 запрос за список из N записей, а затем по одному запросу на связанную сущность каждой из них — итого N+1 обращений к БД вместо одного `JOIN`. Чаще всего возникает незаметно при ленивой загрузке (`LAZY`): код в цикле дёргает связь, и каждый такой вызов превращается в отдельный поход в базу. На малых данных это незаметно, но под нагрузкой обрушивает производительность.
...
`JDBC` — низкоуровневый стандарт доступа к БД из Java: вы сами пишете SQL и вручную раскладываете `ResultSet` по объектам. `JPA` (с реализацией вроде Hibernate) — это ORM поверх `JDBC`: он сам генерирует SQL и маппит строки в сущности, добавляя кэширование и управление состоянием. Компромисс прямой: `JDBC` даёт полный контроль и предсказуемую производительность ценой рутины, `JPA` экономит код, но прячет SQL — отсюда сюрпризы вроде N+1 и неожиданных запросов.
...
Индексы объявляются в атрибуте `indexes` аннотации `@Table` — отдельными `@Index` с именем и списком столбцов (для составного индекса столбцы перечисляют через запятую, порядок важен):
...
Spring предлагает два подхода. **Декларативный** (`@Transactional`) — основной: вы помечаете метод аннотацией, а Spring через AOP-proxy сам открывает транзакцию до вызова и коммитит/откатывает после. **Программный** (`TransactionTemplate`) нужен, когда границы транзакции должны быть уже метода или зависят от логики во время выполнения.
...
Рекурсивный `CTE` (`WITH RECURSIVE`) — единственный штатный способ обойти структуру неизвестной заранее глубины (дерево, граф) одним SQL-запросом. Он работает как цикл: считает начальный набор строк (якорь), затем повторно применяет рекурсивную часть к результату предыдущего шага, пока та не перестанет возвращать новые строки. `UNION ALL` накапливает все шаги, `UNION` дополнительно отсекает дубликаты.
...
План читается **снизу вверх и изнутри наружу**: вложенные узлы (с отступом) выполняются первыми, а их результаты «поднимаются» к родительским операциям. Каждый узел — это одна операция (скан, соединение, сортировка) со своими оценками и, в режиме `ANALYZE`, фактическими цифрами. Задача при чтении — найти узел, который читает слишком много строк или тратит больше всего времени, и понять почему.
...
**Партиционирование** — разбиение одной большой таблицы на физически отдельные куски (партиции) по значению ключа, при том что для приложения это по-прежнему одна таблица. Выигрыш в том, что СУБД, зная границы партиций, может обращаться только к нужным из них (**partition pruning**), а старые данные удаляются мгновенным `DROP` партиции вместо медленного `DELETE`. PostgreSQL поддерживает декларативное партиционирование с версии 10.
...
У PostgreSQL не один индекс «на всё», а несколько типов с разными внутренними структурами — потому что разные операции (равенство, диапазоны, вхождение в JSONB, пересечение геометрий) требуют разных алгоритмов. Знать их полезно, чтобы под нестандартный запрос выбрать подходящий тип, а не упираться в `Seq Scan` с дефолтным B-tree.
...
Это две служебные команды, без которых PostgreSQL со временем деградирует. Кратко: `VACUUM` убирает мусор, который порождает MVCC, а `ANALYZE` обновляет статистику, по которой планировщик строит хорошие планы. Обычно их выполняет автоматический `autovacuum`, но понимать, что и зачем они делают, нужно для диагностики.
...
**MVCC** решает проблему конкурентного доступа так, чтобы чтение и запись не мешали друг другу. Идея: вместо того чтобы изменять строку «на месте» и блокировать читателей, СУБД хранит несколько версий строки, и каждая транзакция видит ту версию, что была актуальна на момент её снимка (snapshot). Поэтому **читатели не блокируют писателей, а писатели — читателей** — отсюда высокий параллелизм. Расплата — мёртвые версии строк, которые потом убирает `VACUUM` (см. Q52).
