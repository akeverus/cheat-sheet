...
> [!mcq]
> - [ ] `SELECT`, `INSERT`, `UPDATE`, `DELETE` относятся к подмножеству `DDL` | Неверно — это операторы `DML`. ❌ ПОСЛЕДСТВИЕ: миграция называется `flyway/V12__ddl.sql`, но содержит `INSERT` — DBA отвергает PR, потому что DDL и DML смешаны в одном rollback-неспособном файле.
> - [ ] `GRANT` и `REVOKE` относятся к подмножеству `TCL` | Неверно — это `DCL`. ❌ ПОСЛЕДСТВИЕ: junior пишет в `application.yml` `transactionRollback: GRANT` думая что это transaction control — Spring молча игнорирует, права не выдаются, prod-инцидент с 403 после деплоя.
> - [x] `CREATE`, `ALTER`, `DROP` относятся к подмножеству `DDL` | Верно — `DDL` отвечает за определение структуры объектов БД, `CREATE`/`ALTER`/`DROP` влияют на схему, а не на данные. ✓ ПРИМЕНЯТЬ: Flyway/Liquibase разделяет миграции на `V*__ddl.sql` (DDL) и `V*__data.sql` (DML), у DDL в PostgreSQL transactional (можно ROLLBACK), в MySQL — auto-commit. 📋 ПРАВИЛО: «DDL — структура, DML — данные, DCL — права, TCL — транзакции». 🔗 См. Q11, Q40.
> - [ ] `COMMIT` и `ROLLBACK` относятся к подмножеству `DCL` | Неверно — это `TCL`, `DCL` включает только `GRANT`/`REVOKE`. ❌ ПОСЛЕДСТВИЕ: на code review кандидат говорит «`COMMIT` это команда DCL для контроля доступа» — собеседование завершается, потому что путать управление транзакциями и привилегиями — fundamental gap.
...
> [!mcq]
> - [ ] Ограничение `UNIQUE` не допускает `NULL`-значений, как и `PRIMARY KEY` | Неверно — `UNIQUE` в большинстве СУБД допускает `NULL` (в PostgreSQL даже несколько, в SQL Server — один). ❌ ПОСЛЕДСТВИЕ: разработчик ставит `UNIQUE` на nullable `email_secondary`, ожидая `NOT NULL`-семантику — после миграции 12k строк с `NULL` проходят, business validation падает на стороне приложения.
> - [ ] Ограничение `CHECK` позволяет задать уникальность значения в столбце | Неверно — `CHECK` задаёт произвольное логическое условие на ОДНУ строку, не видит других строк. ❌ ПОСЛЕДСТВИЕ: миграция `CHECK (count(*)... ) = 1)` не компилируется в PostgreSQL — выдаёт `aggregate functions are not allowed in check constraints`, релиз блокируется.
> - [x] Ограничение `EXCLUDE` (PostgreSQL) запрещает пересечение диапазонов, используя операторы GiST | Верно — `EXCLUDE USING GIST (valid_from WITH &&)` запрещает пересекающиеся временны́е интервалы или геометрические объекты, чего нет в стандарте SQL. ✓ ПРИМЕНЯТЬ: бронирование переговорок в офисных системах (Booking.com room schedules) — `EXCLUDE` гарантирует, что два рендж-объекта `tsrange` не перекрываются на уровне БД, без race condition в приложении. 📋 ПРАВИЛО: «EXCLUDE — UNIQUE для диапазонов, не только для скаляров». 🔗 См. Q3, Q4.
> - [ ] Ограничение `FOREIGN KEY` гарантирует уникальность значений в столбце | Неверно — `FOREIGN KEY` обеспечивает ссылочную целостность, не уникальность. ❌ ПОСЛЕДСТВИЕ: связь `order.customer_id → customer.id` корректно создана, но 50k заказов одного клиента валидны (1:N) — junior удивляется, что email клиента дублируется в `orders`, добавляет лишний `UNIQUE`, ломает бизнес-логику повторных покупок.
...
> [!mcq]
> - [ ] `PRIMARY KEY` допускает несколько `NULL`-значений, а `UNIQUE` — нет | Неверно — всё наоборот, `PRIMARY KEY` НЕ допускает `NULL`, `UNIQUE` допускает. ❌ ПОСЛЕДСТВИЕ: senior пишет `CREATE TABLE users (id INT PRIMARY KEY)` ожидая авто-присваивание для `NULL` — `INSERT (id) VALUES (NULL)` падает с `null value in column "id" violates not-null constraint`, миграция блокируется.
> - [ ] На одну таблицу можно задать несколько `PRIMARY KEY`, но только один `UNIQUE` | Неверно — `PRIMARY KEY` ровно один (можно составной), `UNIQUE` неограничено. ❌ ПОСЛЕДСТВИЕ: попытка `ALTER TABLE ADD PRIMARY KEY (email)` после существующего `PRIMARY KEY (id)` падает с `multiple primary keys for table "users" are not allowed` — DBA откатывает migration prod-deploy.
> - [ ] Столбец с ограничением `UNIQUE` не может быть целью `FOREIGN KEY`, а `PRIMARY KEY` может | Неверно — `FOREIGN KEY` может ссылаться и на `UNIQUE`, главное чтобы столбец был уникально идентифицирующим. ❌ ПОСЛЕДСТВИЕ: разработчик создаёт лишнюю «суррогатную» PK таблицу `country_codes_pk` вместо `FOREIGN KEY (country_iso) REFERENCES countries(iso UNIQUE)` — схема раздувается на 30%, JOIN'ы становятся 3-уровневыми.
> - [x] `PRIMARY KEY` не допускает `NULL` и автоматически создаёт уникальный индекс, тогда как `UNIQUE` допускает `NULL` | Верно — `PRIMARY KEY` = `NOT NULL` + `UNIQUE`, в PostgreSQL `UNIQUE` допускает несколько `NULL` (они считаются различными). ✓ ПРИМЕНЯТЬ: e-commerce схема Wolt — `id BIGSERIAL PRIMARY KEY` для суррогатного ключа + `UNIQUE(email)` где `email` может быть `NULL` для гостевых заказов. 📋 ПРАВИЛО: «PK — обязательная личность строки; UNIQUE — необязательная бизнес-уникальность». 🔗 См. Q2, Q4.
...
> [!mcq]
> - [ ] Нет, `FOREIGN KEY` всегда требует валидной ссылки на родительскую таблицу и не может быть `NULL` | Неверно — СУБД применяет проверку FK только к ненулевым значениям. ❌ ПОСЛЕДСТВИЕ: миграция таблицы `employees(manager_id INT NOT NULL REFERENCES employees(id))` падает с `null value violates not-null constraint` на строке CEO — production rollout откатывается, переделывают схему «иерархии» с искусственным «root» сотрудником.
> - [x] Да, `FOREIGN KEY` может быть `NULL`, если на столбце не задано `NOT NULL` — это означает «связь не установлена» | Верно — `NULL` в FK = «связи нет», ссылочная целостность не нарушается, СУБД проверяет FK только для ненулевых значений. ✓ ПРИМЕНЯТЬ: иерархия сотрудников Yandex Lavka — `manager_id INT REFERENCES employees(id)` без `NOT NULL`, CEO имеет `manager_id IS NULL`, остальные ссылаются на руководителя. 📋 ПРАВИЛО: «FK — это про корректность ссылки, NULL — про её наличие; не путай». 🔗 См. Q2, Q3, Q5.
> - [ ] `FOREIGN KEY` может быть `NULL` только если родительская таблица допускает `NULL` в `PRIMARY KEY` | Неверно — `PRIMARY KEY` вообще не допускает `NULL`, и состояние FK не зависит от родителя. ❌ ПОСЛЕДСТВИЕ: разработчик добавляет лишний trigger «синхронизировать NULL между PK и FK» — trigger зацикливается на `INSERT` через `RAISE` recursion, deadlock при rolling deploy.
> - [ ] `FOREIGN KEY` может быть `NULL` только если указана стратегия `ON DELETE SET NULL` | Неверно — `ON DELETE SET NULL` определяет поведение при удалении родителя, не возможность хранить `NULL` при `INSERT`. ❌ ПОСЛЕДСТВИЕ: команда добавляет `ON DELETE SET NULL` на каждый FK «на всякий случай» — внезапно при удалении родителя 200k orphan-строк теряют связь, аналитика по revenue ломается без error log.
...
> [!mcq]
> - [ ] Выражение `5 + NULL` вернёт `5`, так как `NULL` интерпретируется как `0` | Неверно — любая арифметика с `NULL` возвращает `NULL`. ❌ ПОСЛЕДСТВИЕ: запрос `total_revenue = price * quantity + discount` где `discount IS NULL` возвращает `total_revenue = NULL` для 30% строк — отчёт показывает «нет выручки», CFO эскалирует в panic-mode на квартальном ревью.
> - [ ] Для проверки на отсутствие значения следует использовать `WHERE email = NULL` | Неверно — `= NULL` всегда `UNKNOWN`, нужно `IS NULL`. ❌ ПОСЛЕДСТВИЕ: cron-job рассылки писем `WHERE email_verified = NULL` тихо ничего не отправляет неделями — incident discovered только когда отдел маркетинга жалуется на 0 open rate, бизнес теряет $50k engagement.
> - [ ] Агрегатная функция `COUNT(*)` игнорирует строки, где указанный столбец равен `NULL` | Неверно — `COUNT(*)` считает ВСЕ строки, `NULL` игнорирует только `COUNT(column)`. ❌ ПОСЛЕДСТВИЕ: dashboard показывает «100 заказов» (`COUNT(*)`), а финансы показывают «72 заказа» (`COUNT(payment_id)`) — расхождение 28% воспринимается как data corruption, тратят 2 дня на разбор вместо документирования NULL-семантики.
> - [x] `COALESCE(phone, 'не указан')` вернёт `'не указан'`, если `phone` равен `NULL` | Верно — `COALESCE` возвращает первый non-NULL аргумент слева направо, стандартный способ подстановки default. ✓ ПРИМЕНЯТЬ: сериализация в JSON API ответа DM/Wolt — `COALESCE(user.phone, user.email, 'no-contact')` гарантирует non-null поле в response, упрощает client-side обработку. 📋 ПРАВИЛО: «COALESCE — каскад значений: first non-NULL wins». 🔗 См. Q2, Q4.
...
> [!mcq]
> - [ ] `DATETIME` в MySQL хранит время в UTC и автоматически конвертирует при чтении с учётом часового пояса клиента | Неверно — `DATETIME` хранит как есть, в UTC конвертирует только `TIMESTAMP`. ❌ ПОСЛЕДСТВИЕ: миграция продуктовой БД из MySQL EU-region в US-region: `created_at DATETIME` остаётся в local time исходного сервера, появляются заказы «на 8 часов раньше создания клиента» в логах, пользователи получают чек с прошлым временем.
> - [ ] `TIMESTAMP` в MySQL покрывает диапазон от 1000 до 9999 года, что делает его предпочтительным для хранения исторических дат | Неверно — `TIMESTAMP` MySQL ограничен 1970–2038 (Unix epoch 32-bit). ❌ ПОСЛЕДСТВИЕ: банковская система хранит даты рождения клиентов в `TIMESTAMP` — все клиенты до 1970 (пенсионеры) получают `'0000-00-00'` или ошибку, KYC-процесс ломается, регулятор фиксирует non-compliance.
> - [x] `TIMESTAMPTZ` в PostgreSQL хранит момент времени в UTC и конвертирует в часовой пояс сессии при чтении | Верно — `TIMESTAMPTZ` сохраняет в UTC, при чтении применяет `TimeZone` сессии, оба типа (`TIMESTAMP` и `TIMESTAMPTZ`) занимают 8 байт. ✓ ПРИМЕНЯТЬ: международные сервисы Booking.com/Wolt — `TIMESTAMPTZ` для всех временных меток + `SET TIME ZONE 'Europe/Moscow'` per-session, JVM `Instant.now()` маппится в UTC, фронт получает дату в локальной зоне. 📋 ПРАВИЛО: «TIMESTAMPTZ хранит абсолютный момент, TIMESTAMP — wall clock без зоны». 🔗 См. Q5, Q7.
> - [ ] В PostgreSQL тип `TIMESTAMP` и `TIMESTAMPTZ` занимают разный объём: `TIMESTAMPTZ` требует 16 байт для хранения зоны | Неверно — оба занимают 8 байт, зона не хранится в поле. ❌ ПОСЛЕДСТВИЕ: архитектор отказывается от `TIMESTAMPTZ` в high-volume таблице IoT-телеметрии (5B строк) ради «экономии 40 GB» — реальной экономии нет, зато все аналитики ломают голову над часовыми поясами при cross-region отчётах.
...
> [!mcq]
> - [ ] **1NF** требует отсутствия транзитивных зависимостей между неключевыми атрибутами | Неверно — это требование **3NF**, не 1NF. **1NF** касается только структуры: каждая ячейка должна содержать только атомарное значение (не список, не множество). Например, `phones = '111,222'` нарушает 1NF — нужно распределить телефоны в отдельную таблицу `employee_phones`.
> - [ ] **2NF** требует, чтобы каждый детерминант был кандидат-ключом | Неверно — это требование **BCNF**, которое ужесточает 3NF. **2NF** решает другую проблему: при составном PK неключевой атрибут должен зависеть от ВСЕГО ключа, не только от его части. Пример нарушения: таблица `(student_id, course_id, professor_name)` где `professor_name` зависит только от `course_id`, а не от студента.
> - [x] **3NF** требует отсутствия транзитивных зависимостей: неключевые атрибуты должны зависеть только от первичного ключа, не друг от друга | Верно — в **3NF** исключаются цепочки зависимостей типа `street → city → state → country`. Классический пример: таблица `(customer_id, name, city, state)` — если `state` зависит от `city`, это нарушение 3NF. Решение: отдельная таблица `cities(city_id, name, state)`.
> - [ ] **BCNF** требует отсутствия многозначных зависимостей между атрибутами | Неверно — отсутствие многозначных зависимостей — это требование **4NF** (редко встречается в practice). **BCNF** — это уже очень сильное требование: любой детерминант (атрибут, от которого что-то зависит) должен быть кандидат-ключом. На практике 3NF достаточно для 99% OLTP систем.
...
> [!mcq]
> - [ ] Денормализация всегда ухудшает производительность, так как увеличивает объём данных | Неверно — денормализация улучшает производительность ЧТЕНИЯ за счёт избыточности: например, дублируя `customer_name` в таблице `orders` избегаем дорогого JOIN. Плата за это — медленнее ЗАПИСЬ (обновить имя нужно везде) и риск рассинхронизации (забыли обновить в одной таблице).
> - [x] Денормализация оправдана в OLAP/DWH когда запросы аналитические (mostly read, few writes), но вредна в OLTP | Верно — хранилища данных используют схемы «звезда» (fact table + dimension tables) специально для быстрых аналитических запросов. В OLTP (e.commerce, banking) денормализация созда́ёт кошмар синхронизации: обновил цену в одной таблице, забыл в другой — данные рассинхронизированы. Amazon и Netflix денормализуют только в Analytics, OLTP остаётся нормализованным.
> - [ ] Денормализация нужна когда много таблиц (>10), независимо от pattern доступа | Неверно — количество таблиц не главное. Главное: есть ли узкое место на JOIN? Если 90% запросов это `SELECT * FROM orders o JOIN customers c WHERE o.id=?` — денормализуйте, добавьте `customer_name` в `orders`. Если JOIN редко используется, нормализация лучше.
> - [ ] Денормализация предотвращает аномалии обновления (update anomalies) | Неверно — денормализация их создаёт, не устраняет. Классический пример: таблица `employees(id, name, department, dept_manager)`. Если изменился менеджер отдела, обновить нужно У ВСЕХ сотрудников этого отдела — одна ошибка и данные рассинхронизированы. Нормализация устраняет эту проблему через таблицу `departments(id, name, manager_id)`.
...
> [!mcq]
> - [ ] `View` хранит данные и синхронизируется с базовыми таблицами автоматически | Неверно — `View` не хранит ничего физически, это просто сохранённый SQL-запрос. Каждый `SELECT FROM view_name` выполняет запрос заново и может быть медленным при сложных JOIN. Используйте `View` для абстракции, переиспользования логики или скрытия столбцов (security).
> - [x] `Materialized View` — это физически хранимый результат запроса, поддерживает индексы и быстрый доступ, но требует `REFRESH` для обновления | Верно — `Materialized View` это кеш. В PostgreSQL `REFRESH CONCURRENTLY monthly_sales` обновляет асинхронно без блокировки чтения. Хорошее решение для аналитики: агрегаты по месяцам/годам нужны часто, а запрос по биллионам строк медленный — кешируйте в materialized view.
> - [ ] `Materialized View` в PostgreSQL автоматически обновляется при изменении базовых таблиц | Неверно — нет встроенной синхронизации. Нужен явный `REFRESH` или использовать триггеры + `pg_cron`. Пример: ежедневно в 2 AM запускается `SELECT refresh_materialized_view()`. Это трейд-офф: актуальность данных vs производительность.
> - [ ] Обычный `View` можно индексировать для ускорения | Неверно — индексы работают на физических данных, a `View` это virtual table (только запрос). Для индексирования нужен `Materialized View` или фактическая таблица. Если часто на `View` делаете `WHERE department = 'IT'`, создайте materialized version и индексируйте `department`.
...
> [!mcq]
> - [ ] `Function` может делать `COMMIT` и `ROLLBACK` внутри, процедура не может | Неверно — наоборот! Процедура (добавлена в PostgreSQL 11) может делать `COMMIT` и управлять транзакциями. Функция выполняется в контексте вызывающей транзакции и не может разрывать её. Типичное использование: процедура для бизнес-логики с несколькими таблицами, функция для чистых вычислений (преобразование данных).
> - [ ] `Procedure` обязана возвращать значение и может использоваться в `SELECT` | Неверно — это `Function`. Procedure вызывается через `CALL transfer_employee(emp_id, dept_id)` и не возвращает значение в SQL (может устанавливать OUT параметры). Используйте procedure для действий (UPDATE, INSERT, DELETE с логикой), function для вычислений.
> - [x] `Function` встраивается в запросы (`SELECT get_salary_total(dept_id)`), procedure — это отдельная команда (`CALL transfer(emp_id, new_dept)`) | Верно — функция это операция над данными (вычисления, преобразования), процедура это действие (бизнес-логика с побочными эффектами). В production: используйте функции для вычисляемых полей в SELECT, procedures для ETL/batch операций.
> - [ ] `Function` и `Procedure` полностью идентичны, различаются только способом вызова | Неверно — существуют важные различия: функция не может управлять транзакциями, не может использовать `OUT` параметры для возврата множества значений (нужен RECORD), procedure более гибкая для сложной бизнес-логики. В старых БД (MySQL) различий не было, в современных (PostgreSQL 11+) — есть.
...
> [!mcq]
> - [ ] `INSTEAD OF` триггер выполняется вместо операции на обычной таблице | Неверно — `INSTEAD OF` специально для VIEW, чтобы сделать их обновляемыми. Пример: представление `view_active_employees` читается, пишется через `INSTEAD OF UPDATE` триггер, а реально обновляется таблица `employees`. На обычных таблицах используются `BEFORE`/`AFTER`.
> - [x] `FOR EACH ROW` триггер срабатывает для каждой строки, `FOR EACH STATEMENT` — один раз для операции целиком | Верно — критично для performance. `UPDATE table SET salary = salary * 1.1` на 10M строк: `FOR EACH ROW` срабатывает 10M раз (может быть медленным), `FOR EACH STATEMENT` — один раз. Используйте `FOR EACH ROW` когда нужны `OLD`/`NEW` значения, `FOR EACH STATEMENT` для глобальных действий (логирование операции).
> - [ ] `BEFORE` триггер не подходит для аудита, только `AFTER` | Неверно — `BEFORE` отлично подходит для аудита И изменения данных перед записью. Пример: `BEFORE UPDATE` триггер может: 1) добавить audit record, 2) изменить `NEW.updated_at = NOW()`. `AFTER` триггер используйте когда нужны побочные эффекты после записи (отправить уведомление, обновить счётчик в другой таблице).
> - [ ] Триггер в PostgreSQL создаётся одной командой `CREATE TRIGGER` | Неверно — нужно две команды: сначала `CREATE FUNCTION trigger_func() RETURNS TRIGGER` (определить логику), потом `CREATE TRIGGER salary_audit BEFORE UPDATE ON employees EXECUTE FUNCTION trigger_func()`. Это inconvenient но даёт гибкость переиспользовать функцию в разных триггерах.
...
> - [x] `INSERT ... ON CONFLICT DO UPDATE` в PostgreSQL — это PostgreSQL-специфичная реализация операции UPSERT | Верно — синтаксис `ON CONFLICT` позволяет при конфликте уникального ключа выполнить `UPDATE` вместо ошибки. В `EXCLUDED` доступны значения, которые пытались вставить. INSERT OR REPLACE (SQLite), ON CONFLICT (PostgreSQL), ON DUPLICATE KEY UPDATE (MySQL).
...
> [!mcq]
> - [ ] `NOT IN` с подзапросом безопасен для `NULL`-значений, так как SQL автоматически исключает `NULL` из сравнения | Неверно — `NOT IN` с одним `NULL` в подзапросе ВСЕГДА возвращает пустой результат через трёхзначную логику. ❌ ПОСЛЕДСТВИЕ: WHERE NOT IN с NULL → 0 строк, classic gotcha — обнаруживается only after migration, когда DBA добавляет `email IS NULL` в продовую таблицу и ночной батч `WHERE id NOT IN (SELECT user_id FROM blacklist)` начинает молча возвращать 0 заблокированных.
> - [ ] `IN` с большим подзапросом всегда быстрее `EXISTS`, так как подзапрос выполняется один раз | Неверно — оптимизаторы часто преобразуют `IN` в `EXISTS`, коррелированный `EXISTS` с ранним выходом может быть быстрее. ❌ ПОСЛЕДСТВИЕ: разработчик переписывает `EXISTS` в `IN` ради «оптимизации» в hot-path checkout — на 10M строк план меняется на full hash, p99 latency растёт с 30ms до 800ms.
> - [x] `NOT EXISTS` безопаснее `NOT IN` при наличии `NULL`-значений в подзапросе, так как не имеет проблем с трёхзначной логикой | Верно — `NOT EXISTS` проверяет факт существования строк, корректно обрабатывает `NULL`, возвращает `FALSE` только при совпадении. ✓ ПРИМЕНЯТЬ: anti-join паттерн в системах рекомендаций Netflix/Yandex — `WHERE NOT EXISTS (SELECT 1 FROM viewed WHERE user_id = u.id AND movie_id = m.id)` для исключения уже просмотренных фильмов, безопасно к `NULL`. 📋 ПРАВИЛО: «NOT IN — мина с NULL; NOT EXISTS — броненосец». 🔗 См. Q5, Q18.
> - [ ] `JOIN` вместо `IN` всегда возвращает меньше строк, так как устраняет дубликаты | Неверно — `JOIN` при 1:N может возвращать БОЛЬШЕ строк (дубликаты левой таблицы). ❌ ПОСЛЕДСТВИЕ: рефакторинг `SELECT users WHERE id IN (SELECT user_id FROM orders)` в `JOIN` в отчёте «активные клиенты» — клиент с 50 заказами появляется 50 раз вместо 1, dashboard «уникальных клиентов» вырастает в 10 раз без объявления, бизнес принимает решения на ложных данных.
...
> - [x] `CASE`-выражение возвращает `NULL`, если ни одна ветка `WHEN` не совпала и нет `ELSE` | Верно — без `ELSE` несовпавший `CASE` возвращает `NULL`. Это стандартное поведение SQL. Рекомендуется всегда указывать `ELSE`, чтобы явно контролировать значение по умолчанию. NULL != 0 != ""; используйте IS NULL/IS NOT NULL, COALESCE для подстановки значений.
...
> [!mcq]
> - [ ] `LEFT JOIN` возвращает только совпадающие строки | Неверно — `LEFT JOIN` возвращает ВСЕ строки из левой таблицы. Для строк без совпадения в правой — столбцы правой таблицы будут `NULL`. Практический пример: `SELECT c.*, o.* FROM customers c LEFT JOIN orders o ...` вернёт и покупателей БЕЗ заказов (с `NULL` в `o.*`). Это позволяет найти потенциальные проблемы: покупатели, никогда не заказывавшие.
> - [ ] `FULL OUTER JOIN` возвращает только пересечение | Неверно — `FULL OUTER JOIN` возвращает ВСЕ строки из ОБЕИХ таблиц. Когда совпадения нет, заполняются `NULL`. Пример: `SELECT * FROM employees FULL OUTER JOIN departments` вернёт orphaned сотрудников (dept_id = NULL, они есть в employees но не в departments) И orphaned отделы (есть в departments но без сотрудников).
> - [x] `INNER JOIN` возвращает только строки, которые совпадают в обеих таблицах одновременно | Верно — это пересечение: только matching rows, orphaned записи отбрасываются. Это 90% всех JOIN в production. Оптимизатор выбирает между: nested loop (для маленьких таблиц), hash join (для больших, требует больше памяти), merge join (если обе отсортированы).
> - [ ] `RIGHT JOIN` практически не используется и его можно заменить на `LEFT JOIN` | Верно, что семантически эквивалентно `LEFT JOIN` с переставленными таблицами, но некорректно, что не используется. На практике используют когда читаемость требует: например в отчёте "Все отделы и их сотрудников" пишут `FROM departments RIGHT JOIN employees` чтобы отделы были слева. Или в UNION для симметрии.
...
> [!mcq]
> - [x] В `LEFT JOIN` условие в `ON` применяется при соединении (сохраняя `NULL`), а `WHERE` применяется после (исключая `NULL`-строки), фактически превращая `LEFT JOIN` в `INNER` | Верно — это частая ошибка в production. `LEFT JOIN departments d ON d.id = e.dept_id AND d.budget > 100000` вернёт сотрудников БЕЗ отдела (d.* = NULL). А вот `LEFT JOIN ... WHERE d.budget > 100000` исключит их (NULL > 100000 = NULL, фильтруется). Проверить: `EXPLAIN` покажет разницу в плане.
> - [ ] Для `INNER JOIN` разница между `ON` и `WHERE` большая, `ON` выполняется раньше | Неверно — для `INNER JOIN` оптимизатор объединяет условия `ON` и `WHERE`, результат одинаков. PostgreSQL выполнит оба одинаково быстро с индексом. Важно: разница появляется ТОЛЬКО для LEFT/RIGHT/FULL JOIN.
> - [ ] `ON` может содержать только `=`, другие операторы запрещены в SQL | Неверно — `ON` может содержать любые условия: `ON o.user_id = u.id AND o.amount > 1000 AND o.status IN ('paid', 'pending')`. Нет ограничений. Часто используют для "fuzzy join": `ON users.city = orders.city AND ABS(users.age - orders.customer_age) < 5`.
> - [ ] `WHERE` выполняется раньше `ON`, поэтому там быстрее фильтровать | Неверно логически (WHERE применяется после JOIN) и неверно практически (оптимизатор переупорядочивает). Главное: для OUTER JOIN условие в `ON` сохраняет строки, `WHERE` исключает их. Для INNER — результат одинаков.
...
> [!mcq]
> - [ ] Коррелированный подзапрос выполняется один раз и быстрее `JOIN` | Неверно — коррелированный подзапрос зависит от внешней строки и выполняется ЗАНОВО для каждой её строки. `SELECT (SELECT COUNT(*) FROM orders o WHERE o.user_id = u.id) FROM users u` при 100K пользователей выполнит inner SELECT 100K раз — O(n²) сложность! Замените на `JOIN` с `GROUP BY` или `LEFT JOIN` с подсчётом.
> - [x] Коррелированный подзапрос — это узкое место: выполняется для каждой внешней строки, что даёт O(n²) | Верно — главный способ оптимизации: переписать как JOIN. Пример: вместо `WHERE EXISTS (SELECT 1 FROM orders WHERE ...)` часто используют `WHERE IN (SELECT id ...)` (хотя `IN` может быть O(n²) на больших наборах, есть его оптимизация). PostgreSQL обычно раскручивает EXISTS в полулу JOIN.
> - [ ] `JOIN` всегда быстрее подзапроса | Неверно — некоррелированный подзапрос может быть хорош: `WHERE EXISTS (SELECT 1 FROM table WHERE ...)` может быть быстрее `LEFT JOIN` если в EXISTS есть ранний выход (оптимизатор знает, нужна только одна строка). Но коррелированный подзапрос — это всегда плохо.
> - [ ] `JOIN` 1:N создаёт дубликаты левой таблицы | Верно — если родитель имеет 3 потомков, строка родителя появится в результате 3 раза. Подзапрос этого не создаёт: `WHERE EXISTS` или `IN` просто проверяет наличие. Если нужно считать, используйте `GROUP BY` с `COUNT(DISTINCT ...)` после `JOIN`.
...
> [!mcq]
> - [ ] `UNION` и `UNION ALL` имеют одинаковую производительность | Неверно — `UNION` требует дополнительный шаг дедупликации (внутри это `DISTINCT`), что на больших наборах может быть O(n log n) или O(n) с хешированием. `UNION ALL` просто конкатенирует, O(n). На 1M строк разница в 10-100x может быть.
> - [x] `UNION ALL` быстрее `UNION`, так как пропускает дедупликацию и сохраняет дубликаты | Верно — если вы уверены, что дубликатов нет (две таблицы с разными ID, например), или дубликаты не важны — всегда используйте `UNION ALL`. Правило: начните с ALL, добавьте `UNION` только если понадобится дедупликация.
...
> [!mcq]
> - [ ] `CROSS JOIN` возвращает только строки, у которых есть совпадение в обеих таблицах | Неверно — это `INNER JOIN`. ❌ ПОСЛЕДСТВИЕ: junior описывает `CROSS JOIN` на собеседовании как «inner join без условия» и пишет `FROM a, b WHERE a.id = b.a_id` думая что это CROSS — пройдёт code review только потому что результат корректен, но на review архитектор отклоняет за непонимание реляционной алгебры.
> - [x] `CROSS JOIN` возвращает декартово произведение таблиц: количество строк в результате равно произведению строк каждой таблицы | Верно — A(M строк) × B(N строк) = M×N строк, без условия соединения, для генерации комбинаций (размеры × цвета, даты × часы). ✓ ПРИМЕНЯТЬ: генерация календарной матрицы в BI-инструментах Metabase/Superset — `generate_series(...)` × `hours` × `regions` создаёт «решётку» для join с фактами и заполнения пропусков нулями. 📋 ПРАВИЛО: «CROSS JOIN — декартов взрыв: M×N всегда, не M+N». 🔗 См. Q21, Q26.
> - [ ] `CROSS JOIN` в SQL требует явного условия `ON` или `USING` для работы | Неверно — `CROSS JOIN` не имеет и не требует условия, иначе это `INNER JOIN`. ❌ ПОСЛЕДСТВИЕ: разработчик пишет `FROM products CROSS JOIN colors ON true` (синтаксически legacy) — PostgreSQL парсит, MySQL отвергает; cross-DB код ломается при миграции, regression tests на CI зелёные только на одной БД.
> - [ ] `CROSS JOIN` трёх таблиц (A, B, C) вернёт A×B + B×C строк | Неверно — даёт A×B×C, при A=10, B=5, C=3 это 150, не 65. ❌ ПОСЛЕДСТВИЕ: разработчик оценивает план запроса на 3 таблицах размером 1000, 10000, 100 как «1.1M строк» (сумма произведений) — реальный результат 1B строк, OOM на planner стадии, transaction зависает 10 минут до query timeout.
...
> [!mcq]
> - [ ] `SELF JOIN` требует специального синтаксиса `SELF JOIN` вместо обычного `JOIN` | Неверно — `SELF JOIN` это концепция, реализуется обычным `JOIN`/`LEFT JOIN` с двумя псевдонимами. ❌ ПОСЛЕДСТВИЕ: разработчик ищет в документации `SELF JOIN` keyword, не находит, пишет issue в Postgres mailing list — community отвечает «just use alias» с пометкой newbie, репутация команды страдает.
> - [x] `SELF JOIN` — это соединение таблицы с самой собой, где каждая копия получает отдельный псевдоним | Верно — два псевдонима позволяют обращаться к одной таблице как к двум разным источникам, реализуется обычным `JOIN` (нет ключевого слова `SELF JOIN`). ✓ ПРИМЕНЯТЬ: иерархия категорий в e-commerce каталоге Wildberries/Ozon — `categories LEFT JOIN categories parent ON c.parent_id = parent.id` для построения breadcrumbs и tree-view товаров. 📋 ПРАВИЛО: «SELF JOIN — два псевдонима одной таблицы; никакого SELF keyword». 🔗 См. Q21, Q25.
> - [ ] `SELF JOIN` можно выполнить только если таблица содержит `FOREIGN KEY` на саму себя | Неверно — соединение возможно по любому условию, не требует самореференсного FK. ❌ ПОСЛЕДСТВИЕ: тимлид требует добавить `FK manager_id REFERENCES employees(id)` в legacy-таблицу с 50M строк только чтобы запрос «сравнить зарплаты сотрудников одного отдела» работал — миграция блокирует таблицу 4 часа, deploy откладывается.
> - [ ] При `SELF JOIN` каждая строка обязательно объединяется с самой собой, что всегда создаёт дубликаты | Неверно — соединение по условию, дубликаты исключаются через `a.id < b.id` или `a.id != b.id`. ❌ ПОСЛЕДСТВИЕ: junior пишет `JOIN employees b ON a.salary = b.salary` без `a.id < b.id` — каждая пара появляется дважды + строки пара-сама-с-собой, аналитика «дубликатов зарплат» завышена в 2 раза, увольняют невинного.
...
> [!mcq]
> - [ ] `HAVING` выполняется до `GROUP BY` и фильтрует строки до группировки | Неверно — `HAVING` после `GROUP BY`, до — `WHERE`. ❌ ПОСЛЕДСТВИЕ: junior пишет `HAVING status = 'ACTIVE'` вместо `WHERE` на 50M строк — все строки группируются (включая INACTIVE), потом фильтрация на агрегате, p99 query latency растёт с 200ms до 12s.
> - [x] `HAVING` применяется после `GROUP BY` и позволяет фильтровать группы по результатам агрегатных функций | Верно — `HAVING COUNT(*) > 5` отбирает группы по результатам агрегатов, в отличие от `WHERE`, который не видит агрегатных функций. ✓ ПРИМЕНЯТЬ: BI-отчёты в Tableau/Metabase «отделы с >100 сотрудников» — `GROUP BY department HAVING COUNT(*) > 100`, оптимизатор PostgreSQL применяет filter после aggregate node в EXPLAIN. 📋 ПРАВИЛО: «WHERE — до группировки, HAVING — после; нет смешивания». 🔗 См. Q28, Q33.
> - [ ] В `SELECT` с `GROUP BY` допустимо обращаться к любым столбцам таблицы, не только к группировочным | Неверно — только столбцы из `GROUP BY` или агрегаты, иначе ошибка. ❌ ПОСЛЕДСТВИЕ: GROUP BY без ALL columns в SELECT — Oracle accepts (выбирает random row), PostgreSQL rejects (`column must appear in GROUP BY`), MySQL silent (с `ONLY_FULL_GROUP_BY=OFF` берёт случайную строку) — миграция Oracle → PostgreSQL ломает 200 отчётов в один день.
> - [ ] `GROUP BY` не обрабатывает `NULL`-значения — строки с `NULL` в группировочном столбце исключаются | Неверно — все `NULL` объединяются в ОДНУ отдельную группу. ❌ ПОСЛЕДСТВИЕ: отчёт «продажи по category_id» молча теряет 15% revenue из строк с `category_id IS NULL` — финансы видят падение продаж, тревога в Slack, через неделю обнаруживается что строки не теряются, а сваливаются в группу `NULL` и игнорируются filter в pipeline.
...
> [!mcq]
> - [ ] `COUNT(*)` считает только строки, где все столбцы имеют ненулевые значения | Неверно — `COUNT(*)` считает АБСОЛЮТНО все строки, включая с `NULL` в любых столбцах. ❌ ПОСЛЕДСТВИЕ: BI-аналитик объясняет CFO «у нас 1.2M клиентов» (`COUNT(*)`), потом продакт говорит «но в email-рассылке 800K» (`COUNT(email)`) — paranoia о data corruption на 2 недели расследования вместо понимания семантики.
> - [x] `COUNT(column)` считает строки, где указанный столбец не равен `NULL`, игнорируя строки с `NULL` | Верно — `COUNT(column)` подсчитывает только ненулевые значения в конкретном столбце. Если в столбце 3 значения из 4 не `NULL`, результат будет 3, тогда как `COUNT(*)` вернёт 4. NULL != 0 != ""; используйте IS NULL/IS NOT NULL, COALESCE для подстановки значений.
...
> [!mcq]
> - [ ] Оконные функции вычисляются до `WHERE`, поэтому их результат можно использовать в условии фильтрации | Неверно — `WHERE` (шаг 2) выполняется ДО оконных функций (шаг 5). ❌ ПОСЛЕДСТВИЕ: разработчик пишет `WHERE ROW_NUMBER() OVER (...) <= 10` — PostgreSQL отвечает `window functions are not allowed in WHERE`, deploy блокируется на CI, тимлид теряет час на объяснение порядка фаз SELECT.
> - [x] Оконные функции нельзя использовать в `WHERE`, так как `WHERE` выполняется до вычисления оконных функций; решение — CTE или подзапрос | Верно — оборачивание в `WITH ranked AS (... ROW_NUMBER() ...)` + внешний `WHERE rn <= 10` обходит ограничение порядка выполнения. ✓ ПРИМЕНЯТЬ: top-N запросы в e-commerce аналитике Avito/Wildberries — `WITH ranked AS (SELECT *, ROW_NUMBER() OVER (PARTITION BY category ORDER BY revenue DESC) rn FROM products) SELECT * FROM ranked WHERE rn <= 5` для top-5 товаров в каждой категории. 📋 ПРАВИЛО: «оконка в WHERE — нельзя; CTE спасает порядок выполнения». 🔗 См. Q27, Q32.
> - [ ] Оконные функции нельзя использовать в `HAVING`, но можно в `WHERE` при наличии `GROUP BY` | Неверно — нельзя ни в `WHERE`, ни в `HAVING`, оба выполняются раньше `SELECT`. ❌ ПОСЛЕДСТВИЕ: разработчик добавляет «временный» `GROUP BY id` думая что это разблокирует оконку в `WHERE` — план запроса меняется на full sort + group, query на 1M строк работает 8 секунд вместо 100ms.
> - [ ] Ограничение на использование оконных функций в `WHERE` существует только в PostgreSQL, в MySQL его нет | Неверно — это стандарт SQL, соблюдается всеми СУБД (PostgreSQL/MySQL/SQL Server/Oracle). ❌ ПОСЛЕДСТВИЕ: разработчик прочитал «MySQL разрешает», переходит с PostgreSQL на MySQL для production-сервиса — обнаруживает ту же ошибку через неделю при первом же top-N запросе, downgrade пока перепишут CTE.
...
> [!mcq]
> - [ ] Составной индекс `(A, B, C)` одинаково эффективен для запросов `WHERE B = x` и `WHERE A = x` | Неверно — правило «левого префикса»: без `A` индекс неприменим. ❌ ПОСЛЕДСТВИЕ: разработчик добавляет 7 «универсальных» составных индексов на orders, ожидая ускорения любых запросов — DB растёт на 30 GB, INSERT throughput падает с 5K/s до 1K/s из-за обновления всех индексов, search latency не меняется потому что префикс не используется.
> - [x] Составной индекс `(A, B, C)` поддерживает запросы с `WHERE A`, `WHERE A AND B`, но не `WHERE B` или `WHERE C` — правило левого префикса | Верно — B-tree сортирует по первому ключу, затем по второму; пропуск первого делает индекс неприменимым (PostgreSQL может использовать `Bitmap Index Scan` ограниченно). ✓ ПРИМЕНЯТЬ: индекс `(customer_id, status, created_at)` в Wolt orders покрывает запросы «активные заказы клиента за период» — PG planner использует full prefix, EXPLAIN показывает `Index Cond` на все три. 📋 ПРАВИЛО: «составной индекс — телефонная книга: ищи по фамилии, потом по имени». 🔗 См. Q35, Q36.
> - [ ] Каждый индекс в PostgreSQL по умолчанию является `GIN`-индексом | Неверно — по умолчанию `B-tree`, `GIN` создаётся явно через `USING GIN` для JSONB/массивов/full-text. ❌ ПОСЛЕДСТВИЕ: разработчик создаёт `CREATE INDEX ON users(email)` ожидая GIN-семантику для substring поиска — `LIKE '%domain%'` всё равно делает Seq Scan, обнаруживается на проде когда taobao migration upgrade выявляет 90% запросов медленнее.
> - [ ] Уникальный индекс не может быть составным — он допустим только на один столбец | Неверно — `UNIQUE(email, tenant_id)` стандарт для мультитенантных систем. ❌ ПОСЛЕДСТВИЕ: SaaS-стартап ставит `UNIQUE(email)` глобально — два разных tenant'а не могут зарегистрировать клиентов с одинаковым email, B2B-клиент жалуется на «account already exists» при первом регистрейшене, потеря enterprise-deal на $200k.
...
> [!mcq]
> - [ ] Покрывающий индекс (Covering Index) хранит только ключевые столбцы и ссылку на строку таблицы | Неверно — это обычный Non-clustered, covering хранит ВСЕ нужные запросу столбцы (через `INCLUDE` или в ключе). ❌ ПОСЛЕДСТВИЕ: разработчик пытается «оптимизировать» горячий запрос, оставляя только индекс по ключу — EXPLAIN показывает `Index Scan + Heap Fetch`, p99 latency 200ms вместо ожидаемых 5ms (Index Only Scan), нагрузка на disk I/O в 10 раз выше.
> - [x] Покрывающий индекс позволяет выполнить `Index Only Scan`, если все нужные запросу столбцы содержатся в индексе | Верно — при `Index Only Scan` СУБД не обращается к heap (таблице), `INCLUDE (col1, col2)` добавляет non-key столбцы в листовые узлы без участия в сортировке. ✓ ПРИМЕНЯТЬ: PostgreSQL 11+ для read-heavy дашбордов — `CREATE INDEX ON orders (customer_id, status) INCLUDE (total, created_at)` даёт Index Only Scan для топ-запросов карточки клиента, latency p99 30ms → 3ms. 📋 ПРАВИЛО: «covering — индекс знает ответ; heap fetch не нужен». 🔗 См. Q34, Q35.
> - [ ] Partial Index индексирует все строки таблицы, но только часть столбцов | Неверно — Partial индексирует ВСЕ указанные столбцы, но только подмножество строк по `WHERE`-условию. ❌ ПОСЛЕДСТВИЕ: разработчик ожидает что `CREATE INDEX ... WHERE status = 'ACTIVE'` сэкономит на широте таблицы (50 столбцов) — экономия только на высоте (количестве строк), explain выдаёт `Bitmap Index Scan` только для запросов с тем же предикатом, остальные запросы делают Seq Scan.
> - [ ] `INCLUDE` в PostgreSQL добавляет столбцы в ключевую часть индекса, влияя на порядок сортировки | Неверно — `INCLUDE` добавляет non-key столбцы в leaves без включения в сортировку. ❌ ПОСЛЕДСТВИЕ: разработчик пишет `INCLUDE (created_at)` ожидая что `ORDER BY created_at` будет использовать индекс — план игнорирует sort, добавляет `Sort` node с 50MB work_mem, p99 latency растёт в 8 раз, work_mem swap на диск.
...
> [!mcq]
> - [ ] Deadlock возникает, когда одна транзакция ждёт снятия блокировки более 30 секунд | Неверно — deadlock это взаимная блокировка двух (или более) транзакций: каждая ждёт ресурс, удерживаемый другой. Логическое состояние, не зависящее от таймаута. ❌ ПОСЛЕДСТВИЕ: путаница `lock_timeout` и deadlock — приложение поднимает таймаут до 5 минут, реальные циклы висят и копят backend-процессы до `max_connections`.
> - [ ] СУБД не может обнаружить deadlock автоматически — разработчик должен явно проверять наличие цикла зависимостей | Неверно — все основные СУБД имеют встроенный детектор. `PostgreSQL` через `deadlock_timeout` (по умолчанию 1s) запускает поиск цикла и откатывает одну транзакцию с `ERROR: deadlock detected`. ❌ ПОСЛЕДСТВИЕ: ручная проверка циклов в приложении — race condition + ложные rollback на легитимных long-running транзакциях.
> - [x] Deadlock — взаимная блокировка, когда `Tx A` ждёт ресурс `Tx B`, а `Tx B` ждёт ресурс `Tx A` | Верно — СУБД обнаруживает цикл в графе ожидания и откатывает «жертву» (обычно youngest). Приложение ловит `40P01` (Postgres) / `1213` (MySQL) и делает retry с jitter. ✓ ПРИМЕНЯТЬ: Spring `@Retryable(retryFor = CannotAcquireLockException.class)` поверх `JdbcTemplate`/`JPA` для idempotent UPDATE. 📋 ПРАВИЛО: «один порядок блокировок + retry на 40P01». 🔗 См. Q38, Q41.
> - [ ] `SELECT ... FOR UPDATE NOWAIT` предотвращает deadlock, делая блокировку более долгой | Неверно — `NOWAIT` мгновенно завершает запрос ошибкой `55P03 lock_not_available`, если ресурс занят. Это сокращает, а не увеличивает время ожидания, и устраняет цикл за счёт fail-fast. ❌ ПОСЛЕДСТВИЕ: интерпретация наоборот — разработчик ставит `NOWAIT` «для надёжности» и получает спам ошибок на горячей строке вместо ожидания.
...
> [!mcq]
> - [x] `N+1` — антипаттерн, когда для загрузки N дочерних сущностей выполняется N отдельных запросов вместо одного `JOIN FETCH` | Верно — `findAll()` возвращает N заказов, затем для каждого лениво подгружается клиент — ещё N запросов. Итого N+1. Решение: `JOIN FETCH`, `@EntityGraph` или `@BatchSize`. Hash join для больших таблиц, nested loop для малых, merge для отсортированных; профилируйте EXPLAIN.
...
> - [x] При self-invocation (вызов `@Transactional`-метода из того же класса) транзакция не создаётся, так как Spring AOP работает через proxy | Верно — это частая ловушка. `this.methodWithTransaction()` обходит proxy и вызывает метод напрямую. Решение: вынести метод в отдельный бин или использовать `TransactionTemplate`. ACID гарантии; изоляция влияет на performance: READ_UNCOMMITTED быстро, SERIALIZABLE безопасно.
...
> [!mcq]
> - [ ] Большое расхождение между `rows` (оценка) и `actual rows` (факт) в `EXPLAIN ANALYZE` означает неэффективный индекс | Неверно — расхождение оценки и факта свидетельствует об устаревшей статистике (`pg_statistic`), решается `ANALYZE table_name`. Неэффективный индекс виден иначе — `Seq Scan` вместо ожидаемого `Index Scan`. ❌ ПОСЛЕДСТВИЕ: DBA ребилдит индекс вместо `ANALYZE` — план не меняется, latency p99 5s держится сутки до следующего autovacuum.
> - [x] `EXPLAIN ANALYZE` сравнивает оценки планировщика (`rows`) с реальностью (`actual rows`) и подсвечивает устаревшую статистику | Верно — если оценка 1000, а факт 1 (или наоборот), планировщик выбрал неоптимальный join order/метод доступа из-за гнилой статистики. Лечится `ANALYZE table` или `default_statistics_target`. ✓ ПРИМЕНЯТЬ: explain.depesz.com + автоматический `ANALYZE` после bulk-load в DWH-пайплайнах (`Airflow` + `Postgres`). 📋 ПРАВИЛО: «оценка ≠ факт → ANALYZE, не REINDEX». 🔗 См. Q42, Q43.
> - [ ] `EXPLAIN (FORMAT JSON)` выполняет запрос и возвращает результат в JSON вместо табличного вывода | Неверно — `FORMAT JSON` меняет формат вывода плана, а не данных. Сам запрос не выполняется без `ANALYZE`. JSON удобен для парсинга в `pgBadger`/`pganalyze`. ❌ ПОСЛЕДСТВИЕ: разработчик пишет `EXPLAIN (FORMAT JSON) DELETE …` думая «увижу JSON», получает план без выполнения и удивляется, что данные не удалились.
> - [ ] Операция `Nested Loop` в плане всегда медленнее `Hash Join` и должна быть устранена | Неверно — `Nested Loop` оптимален когда внешний набор мал (1–10 строк) или есть индекс на inner. Для маленьких таблиц он обгоняет `Hash Join`, который тратит память на построение хеш-таблицы. ❌ ПОСЛЕДСТВИЕ: автоматическое `SET enable_nestloop = off` «для надёжности» — на OLTP-запросах latency p99 растёт с 2ms до 200ms из-за лишних hash-сборок.
