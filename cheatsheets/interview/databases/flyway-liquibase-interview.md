...
**Миграция базы данных** — это версионируемое, воспроизводимое изменение схемы или данных БД, оформленное в виде скрипта и применяемое автоматически. Главная идея: схема БД развивается так же, как код, — через проверяемые в Git изменения, а не через ручные правки на сервере.

Что это даёт:

- **Версионирование схемы** — каждое изменение схемы лежит в репозитории рядом с кодом, который от него зависит, и проходит обычный code review
- **Воспроизводимость** — любой разработчик поднимает базу с нуля, последовательно применив все миграции; результат одинаков на любой машине
- **Консистентность окружений** — dev, staging и production гарантированно находятся в одной версии схемы, потому что прогоняют один и тот же набор миграций
...
```
 [зелёный]      [зелёный]      [зелёный]       [жёлтый]        [красный]
┌───────────┐  ┌───────────┐  ┌───────────┐  ┌────────────┐  ┌────────────┐
│ V1:       │  │ V2:       │  │ V3:       │  │ V4:        │  │ V5:        │
│ Create    │─▶│ Add       │─▶│ Add       │─▶│ Migrate    │─▶│ Drop       │
│ tables    │  │ indexes   │  │ column    │  │ data       │  │ old column │
└───────────┘  └───────────┘  └───────────┘  └────────────┘  └────────────┘
  безопасно      безопасно      безопасно      осторожно       потеря данных
```

Без инструмента миграций команды скатываются к ручным SQL-скриптам, пересылке дампов и устным договорённостям. Итог предсказуем: окружения расходятся, и появляются баги, которые невозможно воспроизвести, — на одном стенде колонка есть, на другом её нет.

## Q2. Какие проблемы решает инструмент миграций?

Инструмент миграций (`Flyway`, `Liquibase`) превращает изменение схемы в управляемый процесс. Конкретно он закрывает такие боли:
...
Оба инструмента написаны на Java и встраиваются в проект несколькими способами: CLI для ручного запуска и CI, плагины Maven/Gradle, программный API и автоконфигурация в `Spring Boot` (миграции прогоняются при старте приложения).

## Q3. (!) Что такое Flyway и как он работает?

**`Flyway`** — инструмент миграции баз данных, основанный на принципе **версионных SQL-скриптов**: каждое изменение схемы — это пронумерованный файл, который применяется ровно один раз. Развивается компанией Redgate.

Как работает migrate по шагам:
...
**Философия Flyway — простота.** Ты пишешь обычный SQL, который и так знаешь по своей СУБД; учить отдельный DSL не нужно. Платой за это становится привязка к конкретной СУБД (SQL не переносим) и отсутствие бесплатного rollback.
...
Основной тип, который вы будете использовать в 90% случаев. Применяются **ровно один раз** и в порядке возрастания версии — это и есть последовательная эволюция схемы. Подходят для любого необратимого изменения: создание таблиц, добавление колонок, индексов.
...
Не имеют версии и применяются заново **каждый раз, когда меняется их checksum** (то есть когда вы отредактировали файл). Выполняются после всех версионных миграций. Это идеально для объектов, которые проще пересоздавать целиком, чем менять по шагам: представления, функции, хранимые процедуры. Вместо `V1`, `V2`, `V3` с правками вьюхи держите один `R__`-файл и редактируете его.
...
Имя файла для `Flyway` — не косметика, а часть протокола: из него инструмент определяет тип миграции, её версию и описание. Поэтому соглашение строгое:
...
> **Рекомендация для команд**: используйте timestamp-версии (`V20260412153000__...`) вместо порядковых номеров. При параллельной разработке двое легко создадут две разные `V5__`-миграции в разных ветках — при merge получится конфликт версий. Timestamp практически уникален, поэтому такой коллизии не возникает.

## Q6. Что такое таблица flyway_schema_history?

`flyway_schema_history` — служебная таблица, которую `Flyway` создаёт автоматически в целевой БД. Это «журнал» применённых миграций: именно по нему `Flyway` понимает, что уже накатано, а что предстоит, и проверяет, не менялись ли уже применённые скрипты.
...
Колонка `checksum` — защита от незаметного изменения истории. `Flyway` хранит хеш каждого применённого файла, и если кто-то отредактировал уже накатанную миграцию, `validate` сразу падает с ошибкой. Это не даёт случайно поменять прошлое: история должна быть неизменной, новое изменение — всегда новая миграция.
...
У `Flyway` 7 основных команд. На практике в CI/CD рабочая лошадка — `migrate` (накатить) и `validate` (проверить целостность); `info` нужен для диагностики, `repair` и `baseline` — для нештатных ситуаций, а `clean` — опасный инструмент для dev.
...
**Callbacks** — это хуки, которые `Flyway` вызывает на определённых этапах жизненного цикла миграции (до/после `migrate`, до/после каждого файла, при ошибке и т.д.). Они нужны для логики, которую неудобно или неправильно держать в самих миграциях, — сквозных задач вроде аудита или подготовки окружения.
...
**Сценарии применения:** логирование и метрики прогона, проверка предусловий до старта (нужное расширение, права), загрузка справочных данных и выдача прав доступа после накатывания. Ключевое отличие от миграций: callbacks не версионируются и выполняются при каждом запуске команды.

## Q9. Что такое placeholders в Flyway?

**Placeholders** — это переменные в SQL-скриптах, которые `Flyway` подставляет на этапе выполнения (синтаксис по умолчанию — `${name}`). Нужны, чтобы один и тот же скрипт работал в разных окружениях без копипасты: имя схемы, признак среды, права для конкретного пользователя задаются снаружи, а не зашиваются в файл.
...
**Baseline** — это способ «подружить» `Flyway` с **уже существующей базой**, которую раньше вели без инструмента миграций. Команда фиксирует текущее состояние схемы как стартовую точку, с которой начнётся отсчёт миграций.

Проблема, которую он решает: у вас есть production-база с десятками таблиц, но без `flyway_schema_history`. Просто запустить `migrate` нельзя — `Flyway` сочтёт базу пустой и попробует создать все таблицы заново, что сразу упадёт.
...
`repair` чинит таблицу `flyway_schema_history`, когда её состояние перестаёт совпадать с реальностью. Важно понимать: `repair` правит только журнал, а не саму схему — он не выполняет и не откатывает SQL. Применяется в двух ситуациях:
...
`clean` **полностью сносит схему**: удаляет все таблицы, представления, процедуры, функции, триггеры, типы — и саму `flyway_schema_history`. По сути это «вернуть базу к нулю». Данные при этом теряются безвозвратно.
...
**`Liquibase`** — инструмент миграции, построенный на **абстрактном описании изменений** (change types) вместо прямого SQL. Вы пишете не `ALTER TABLE`, а декларативное `addColumn`, а `Liquibase` сам генерирует SQL под конкретную СУБД. Описывать изменения можно в XML, YAML, JSON или SQL.

Главное следствие этой абстракции: одни и те же миграции переносимы между СУБД, есть встроенный rollback, preconditions и diff — то, чего у `Flyway` либо нет, либо это платно. Платой становится более высокий порог входа (свой DSL) по сравнению с «просто SQL».
...
```
        Flyway                              Liquibase
   (линейная цепочка)                  (дерево changelog-а)

┌───────────────────┐                 ┌──────────────────────┐
│ V1__create.sql    │                 │ master-changelog.xml │
└─────────┬─────────┘                 └──────────┬───────────┘
          │                                      │
          ▼                       ┌──────────────┼──────────────┐
┌───────────────────┐             ▼              ▼              ▼
...
**Changelog** — корневой файл (или дерево файлов), который задаёт **упорядоченный список** всех изменений базы. Это оглавление миграций: в нём через `include`/`includeAll` подключаются отдельные файлы, а порядок применения определяется порядком записей в changelog (а не сортировкой имён, как у `Flyway`).
...
**Changeset** — атомарная единица изменения, аналог одной миграции в `Flyway`. Уникален по тройке `id` + `author` + путь к файлу — именно по ней `Liquibase` решает, выполнялся ли changeset. Поэтому менять `id` или автора у уже накатанного changeset нельзя: он будет считаться новым и применится повторно.
...
`Liquibase` поддерживает 4 формата описания миграций — выбор влияет только на синтаксис, набор возможностей одинаков. Краткий ориентир: XML — самый полный и проверяемый по схеме (валидация в IDE); YAML/JSON удобны командам, которые и так живут в этих форматах; formatted SQL — компромисс для тех, кто хочет писать обычный SQL, но при этом получить rollback и трекинг changeset-ов от `Liquibase`.
...
`Liquibase` создаёт две служебные таблицы: одну для истории, другую для блокировки. Это аналог `flyway_schema_history`, только разнесённый на две сущности.

### DATABASECHANGELOG

Журнал применённых changeset-ов — по нему `Liquibase` понимает, что уже выполнено. Аналог `flyway_schema_history`, только хеш считается по содержимому changeset, а не файла целиком:
...
Таблица-блокировка: один инстанс захватывает её перед миграцией, остальные ждут. Так предотвращается одновременный запуск `Liquibase` с нескольких реплик приложения, который иначе привёл бы к гонке и порче схемы.
...
Сила декларативного подхода в том, что для обратимых change types `Liquibase` сам знает обратную операцию и генерирует rollback без вашего участия:
...
Для произвольного SQL `Liquibase` не может угадать, как откатить изменение, — обратную операцию нужно задать самому в блоке `<rollback>`:
...
**Preconditions** — условия, которые `Liquibase` проверяет перед выполнением changeset. Они делают миграцию умной: changeset срабатывает только если БД в нужном состоянии. Типичные применения — идемпотентность (не создавать таблицу, если уже есть), привязка к конкретной СУБД и проверка инвариантов данных перед опасной операцией.
...
Что произойдёт, если условие не выполнилось, задаёт атрибут `onFail`. Разница между `CONTINUE` и `MARK_RAN` тонкая, но важная: `MARK_RAN` помечает changeset как выполненный и больше к нему не вернётся, а `CONTINUE` просто пропускает его сейчас и повторит проверку при следующем запуске.
...
**Contexts** позволяют включать или выключать changeset-ы в зависимости от окружения. Помечаете changeset контекстом (`test`, `dev or staging`), а при запуске указываете активные контексты — выполнятся только подходящие. Классический сценарий: тестовые данные накатываются на dev/test, но не на production.
...
**Labels** — второй механизм фильтрации, технически похожий на contexts. Принципиальная разница в том, где живёт логика выражения: у contexts логическое условие (`dev or staging`) пишется на самом changeset, а у labels changeset просто помечается тегами (`feature-123`), и логика выбора задаётся в фильтре при запуске (`--labelFilter`). На практике contexts удобны для окружений, labels — для фич, спринтов и релизов.
...
`diff` сравнивает **две базы** (или базу с офлайн-снимком) и показывает, чем они отличаются. Главное применение — поймать «дрифт»: ручные изменения, которые кто-то внёс на сервере в обход миграций и из-за которых окружения разъехались.
...
Если свести всё к одной фразе: `Flyway` ставит на **простоту** (пишешь привычный SQL, но привязан к СУБД и без бесплатного rollback), `Liquibase` — на **гибкость и переносимость** (абстрактный DSL, rollback и diff из коробки, но выше порог входа). Развёрнуто:
...
`Spring Boot` настраивает миграции автоматически: достаточно добавить зависимость в classpath — и при старте приложения миграции прогонятся сами, до инициализации Hibernate/JPA. Никакого ручного кода обычно не требуется, всё конфигурируется свойствами `spring.flyway.*` / `spring.liquibase.*`.
            ...
            └───────────┬───────────┘
              ...
              └────┬──────────┬────┘
                Да │          │ Нет
                   ▼          ▼
   ┌────────────────────┐  ┌──────────────────────┐
   │FlywayAutoConfigura-│  │ Liquibase в classpath?│
   │tion → flyway.migrate│ └────┬───────────┬─────┘
   └─────────┬──────────┘    Да │           │ Нет
             │                  ▼           │
             │     ┌────────────────────────┐
             │     │ LiquibaseAutoConfigura- │
             │     │ tion → liquibase.update │
             │     └───────────┬────────────┘
             │                 │             │
             └────────┬────────┴─────────────┘
                      ▼
           ┌────────────────────────┐
           │ Hibernate/JPA инициализ.│
           └───────────┬────────────┘
                       ▼
              ┌──────────────────┐
              │ Приложение готово │
...
Автоконфигурация `Spring Boot` рассчитана на один `DataSource`, поэтому при нескольких базах её не хватает — нужно вручную создать по бину `Flyway`/`SpringLiquibase` на каждую БД, со своим набором миграций и своей таблицей истории.
...
**Expand-contract** (расширение-сжатие) — паттерн изменения схемы без остановки приложения. Корень проблемы: при rolling-деплое какое-то время одновременно работают старая и новая версии кода, и схема должна устраивать обе. Поэтому любое разрушающее изменение разбивается на несколько обратно-совместимых шагов: сначала только добавляем (expand), потом переносим данные, и лишь когда старый код полностью выведен — удаляем лишнее (contract).
...
┌─────────────┐  ┌─────────────────┐  ┌──────────────┐  ┌─────────────┐
│ 1. Expand:  │  │ 2. Migrate data:│  │ 3. Deploy    │  │ 4. Contract:│
│ ADD         │─▶│ COPY            │─▶│ code:        │─▶│ DROP        │
│ full_name   │  │ name → full_name│  │ read full_name│ │ name        │
└─────────────┘  └─────────────────┘  └──────────────┘  └─────────────┘
  добавить         перенести данные     читать новую      удалить старую
```
...
Обратно-совместимая (backward-compatible) миграция — та, после которой **старая версия приложения продолжает работать**. Это обязательное условие zero-downtime деплоя: пока новая версия раскатывается по репликам, схема не должна ломать ещё живущий старый код. Практически правило сводится к «только добавляй, ничего не ломай в текущем релизе».
...
Тестировать миграции стоит на нескольких уровнях — от прогона на реальной СУБД до проверки обратной совместимости.

### 1. Testcontainers — золотой стандарт

Поднимаем реальную целевую СУБД в Docker и прогоняем на ней все миграции. Это ловит то, что не поймает H2: специфичный синтаксис, типы и поведение конкретной базы. Если миграции прошли на той же СУБД, что и в проде, — им можно доверять.
...
В multi-tenant приложении одну и ту же миграцию нужно применить ко множеству изолированных схем или баз — по одной на арендатора. Это меняет задачу: вместо «накатить миграцию на одну БД» получается «накатить её на N баз надёжно, быстро и с обработкой частичных сбоев». Базовая идея во всех подходах одна — в цикле прогоняем `Flyway`/`Liquibase` по каждому tenant.
...
Когда модули (`core`, `billing`, `notifications`) ведут свои миграции к одной базе, главная проблема — не дать им столкнуться по порядку и версиям. `Flyway` и `Liquibase` решают это по-разному: `Flyway` собирает файлы из нескольких locations и сортирует их глобально по версии, поэтому нужна дисциплина версионирования; `Liquibase` подключает модульные changelog-и через `include`, и порядок становится явным.
...
Главный вопрос при сбое — поддерживает ли СУБД **транзакционный DDL**, потому что от этого зависит, осталась ли база в консистентном состоянии. На PostgreSQL/SQL Server упавшая миграция откатывается целиком, и восстановление тривиально. На MySQL/Oracle DDL не откатывается — часть команд могла примениться, и базу приходится чинить вручную.
...
**Flyway Teams** (ранее Flyway Pro/Enterprise) — платная редакция от Redgate. Она добавляет к Community то, чего там принципиально нет: откат (`undo`/`U__`), пробный прогон без выполнения (dry run), выборочное применение миграций (cherry-pick) и расширенную поддержку enterprise-СУБД.

### Что даёт Teams сверх Community:
...
**Теги** — это именованные закладки в истории миграций. Вы помечаете тегом текущее состояние БД (обычно перед релизом), а потом одной командой откатываете базу ровно к этой точке. Это превращает rollback из «посчитай, сколько changeset-ов отменить» в «верни базу к состоянию before-v3.0.0».
...
Цель хорошей структуры миграций — чтобы их было легко ревьюить, безопасно откатывать и невозможно случайно сломать при параллельной работе. Отсюда две главные идеи: уникальные неконфликтующие версии (timestamp лучше порядковых номеров) и маленькие миграции с одной целью.
...
**Идемпотентность** — повторный запуск миграции не ломается и не меняет результат. Это страховка на случай, когда миграция могла частично примениться (сбой на non-transactional СУБД) или прогоняется повторно в CI. Достигается через `IF EXISTS`/`IF NOT EXISTS` в SQL или preconditions в `Liquibase`.
...
**Атомарность** — либо вся миграция применяется целиком, либо не применяется ничего. На транзакционных СУБД это даёт сама транзакция; задача инженера — сгруппировать в одну миграцию только то, что действительно должно быть неделимым:
...
**Data migration** — изменение самих данных (заполнение новых колонок, трансформация, очистка), а не структуры. Опасность в объёме: один большой `UPDATE` на миллионы строк держит долгую транзакцию и блокировки, подвешивая прод. Поэтому главные приёмы — отделять изменение данных от изменения схемы и обновлять данные батчами с паузами.
...
В Kubernetes ключевое требование — прогнать миграции **до** старта основного приложения, иначе под поднимется на ещё не обновлённой схеме. Два подхода: init container (просто, но запускается в каждом поде при `replicas > 1`) и отдельный Job, который выполняется один раз перед деплоем (рекомендуется для production).
...
**Liquibase Hub** (устаревшее название) / **Liquibase Pro** — коммерческие расширения open-source ядра. В отличие от Flyway, базовый rollback есть уже в OSS; Pro добавляет вещи enterprise-уровня: автоматический rollback, детект дрифта, политики качества SQL (Checks Framework), оркестрацию команд (Flow Files) и расширенную поддержку Oracle/DB2/MongoDB.

**Что даёт Pro сверх OSS:**
...
**Flyway Callbacks** выполняют SQL или Java-код на определённых этапах прогона миграций. Удобно понимать их как набор именованных точек: SQL-callback — это просто файл, чьё имя совпадает с событием (`beforeMigrate.sql`, `afterMigrate.sql`), а Java-callback реализует `supports()` для выбора нужного события. Применяют их для сквозных задач: аудита, управления соединением и сессией, подготовки seed-данных.
...
**Expand-contract** (он же **parallel change**) реализуется как цепочка из трёх деплоев: каждая фаза совместима с предыдущей версией кода, поэтому в любой момент rolling-обновления старые и новые поды работают с одной и той же схемой без конфликтов. Ключ — синхронизировать шаги схемы с поведением приложения: на фазе expand код пишет в обе колонки, на фазе migrate переключает чтение на новую, и только на фазе contract удаляется старое.
...
Главное различие здесь принципиальное: у `Liquibase` rollback встроен в OSS, а у `Flyway` бесплатной версии его нет вовсе — откат реализуется обходными путями.
...
**Testcontainers** поднимает реальную СУБД в Docker прямо во время тестов. Это снимает главный риск тестирования миграций на встроенной H2: H2 прощает то, что отвергнет PostgreSQL (другой синтаксис, типы, поведение), и зелёные тесты на H2 ничего не гарантируют для прода. Тестируя на той же СУБД, что в проде, вы проверяете миграции в боевых условиях.
...
На больших таблицах (>10M строк) опасны не сами изменения, а **блокировки**: `ALTER TABLE` берёт `AccessExclusiveLock` и на время выполнения подвешивает все чтения и записи — это прямой простой прода. Поэтому стратегия одна: избегать долгих эксклюзивных блокировок. Добавляйте колонки без долгого сканирования, заполняйте данные батчами, стройте индексы через `CONCURRENTLY`, а для перестройки таблиц используйте `pg_repack`.
