...
Те же три стратегии в виде схемы распределения (три отдельных блока):

```
 ── Range sharding ─────────────────────────────────────
   ┌──────────────┐  ┌──────────────┐  ┌──────────────────┐
   │ shard A      │  │ shard B      │  │ shard C          │
   │ id 0..1M     │  │ id 1M..2M    │  │ id 2M..∞         │
   │              │  │              │  │ HOT - все новые  │
   └──────────────┘  └──────────────┘  └──────────────────┘

 ── Hash sharding (hash mod N, N=4) ────────────────────
   ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌────────────┐
   │ shard A    │ │ shard B    │ │ shard C    │ │ shard D    │
   │ mod N = 0  │ │ mod N = 1  │ │ mod N = 2  │ │ mod N = 3  │
   │ ~25%       │ │ ~25%       │ │ ~25%       │ │ ~25%       │
   └────────────┘ └────────────┘ └────────────┘ └────────────┘

 ── Consistent hashing (узлы на кольце, каждый владеет дугой) ──
   ┌────────────┐ ┌────────────┐ ┌────────────┐ ┌────────────┐
   │ node A     │ │ node B     │ │ node C     │ │ node D     │
   │ arc 0..90  │ │ arc 90..180│ │ arc 180..270│ │ arc 270..360│
   └─────┬──────┘ └────────────┘ └────────────┘ └────────────┘
         │ + node E забирает только arc 45..90 у A
         ▼  (остальные дуги не двигаются)
   ┌────────────┐
   │ + node E   │
   │ arc 45..90 │
   └────────────┘
...
То же кольцо vnodes в виде цепочки токенов по часовой стрелке (замыкается на `token 0`):

```
 Хэш-кольцо 0 .. 2^64 (концы сомкнуты), по часовой стрелке:

   ┌─▶ A:vn1 ─▶ B:vn1 ─▶ C:vn1 ─▶ A:vn2 ─▶ B:vn2 ─▶ ...─┐
   │                                                    │
   └──────────────────── (замыкание на 0) ◀─────────────┘

   порядок vnode повторяется: A:vn1, B:vn1, C:vn1, A:vn2, ...

 key K: hash попадает между B:vn1 и C:vn1
        ──▶ достаётся C:vn1 (первый узел по часовой стрелке)
```
