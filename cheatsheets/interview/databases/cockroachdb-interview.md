...
- **Открытый код и self-host** — Spanner доступен только как managed-сервис в GCP, CockroachDB можно развернуть где угодно (с конца 2024 лицензия проприетарная — CockroachDB Software License, — но код открыт; см. Q23).

Итог: к **2025** CockroachDB — основная распределённая SQL-база с открытым кодом (source-available), делающая идеи Spanner доступными вне Google.

## Q4. (!) Как устроена архитектура: ranges, replicas, leases?

Слои укладываются стопкой, каждый опирается на нижний:

- `SQL Layer` → `KV Layer` → `Ranges` → `Replicas` → `Pebble Storage Engine`.

Сверху вниз запрос проходит по этим слоям:
...
То есть SQL-слой транслирует запросы в KV-слой, KV-слой оперирует range-ами, каждый range разложен на реплики, а реплики физически хранятся движком `Pebble`.
...
**Как это работает:**
- все строки живут в едином отсортированном пространстве ключей, и оно нарезается на range-ы **по диапазонам первичного ключа**: каждый range — непрерывный отрезок строк, упорядоченных по PK;
- по умолчанию это именно **range-партиционирование, а не хеширование** — hash-распределение нужно включать явно (`USING HASH`);
...
Идеи общие (обе — распределённый SQL по мотивам одной архитектуры), но Spanner — закрытый managed-сервис Google с атомными часами, а CockroachDB переносит ту же модель на обычное железо: код открыт, развернуть можно где угодно.

| Критерий | CockroachDB | Spanner |
|-----------|-------------|---------|
| Хостинг | Self-host или CockroachCloud | Только managed в GCP |
| Лицензия | CockroachDB Software License (до v24.3 — BSL); код открыт, бесплатно при выручке < $10M | Нет (закрытый сервис) |
...
**Вывод:** Spanner — золотой стандарт распределённого SQL и даёт более сильную гарантию (external consistency за счёт атомных часов), но живёт только в GCP и стоит дорого. CockroachDB делает те же концепции доступными — открытый код, любое облако или своё железо, дешевле, ценой чуть более слабой модели согласованности.
...
| Лицензия | CockroachDB Software License (до v24.3 — BSL), проприетарная | Apache 2.0 (настоящий open-source) |
| Производительность | — | Часто быстрее |

**Ключевое различие:** YugabyteDB переиспользует код самого PostgreSQL (форк его SQL-слоя), поэтому поддерживает больше PG-фич «из коробки» — совместимость теснее. Плюс у него есть Cassandra-совместимый интерфейс (YCQL), которого нет у CockroachDB. Разрыв по лицензиям в конце 2024 только вырос: CockroachDB ушла с BSL на проприетарную CockroachDB Software License (бесплатно лишь при выручке < $10M), а YugabyteDB остаётся под свободной Apache 2.0.
...
| Лицензия | CockroachDB Software License (до v24.3 — BSL) | Apache 2.0 |
...
- нужен **self-host и открытый код** (в противовес закрытому GCP-only Spanner) — с оговоркой: с конца 2024 лицензия CockroachDB проприетарная (source-available, бесплатно при выручке < $10M); настоящий open-source среди распределённых SQL — YugabyteDB/TiDB.
...
Короткий ответ: **Business Source License (BSL)** — «почти open-source» лицензия, под которой CockroachDB распространялась **с 2019 по 2024 год**. Сегодня это уже история: **с версии v24.3 (ноябрь 2024) Cockroach Labs перешла на единую проприетарную CockroachDB Software License**. На собеседовании стоит знать обе.

**Как работала BSL (2019–2024):** она открывала код для большинства применений, но закрывала ровно один сценарий — перепродажу продукта как managed-сервиса:
- **запрещено** было предлагать CockroachDB **как сервис** третьим лицам без коммерческой лицензии;
- **разрешено** всё остальное — self-host, изменение кода, использование в своих продуктах — бесплатно;
- **временной механизм:** через 3 года каждая версия автоматически становилась **Apache 2.0**, то есть старые BSL-релизы со временем превращаются в полноценный open-source.

**Зачем вводили BSL:** не дать AWS/Google запустить «CockroachDB as a Service» и тем самым подорвать бизнес Cockroach Labs, которая сама продаёт managed-сервис CockroachCloud. По духу это похоже на путь Elastic (которая ушла с Apache на SSPL по той же причине).

**Что действует сейчас — CockroachDB Software License (с v24.3, ноябрь 2024):**
- единая лицензия вместо прежней пары «BSL-ядро + проприетарные enterprise-фичи»: все возможности продукта теперь под одной лицензией;
- **бесплатно** для физических лиц и компаний с годовой выручкой **меньше $10M** (бесплатная лицензия требует включённой телеметрии);
- компании крупнее платят за коммерческую (Enterprise) лицензию;
- код остаётся открытым для чтения и сборки (**source-available**), но это **не open-source** по определению OSI, и автоматического перехода новых версий в Apache 2.0 больше нет.

**Почему это важно для выбора:** если для проекта принципиальна именно свободная лицензия, CockroachDB после 2024 года ей не отвечает — из распределённых SQL под Apache 2.0 остаются YugabyteDB и TiDB.
