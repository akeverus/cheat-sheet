...
> - [ ] `Elasticsearch` — это очередь сообщений, обеспечивающая доставку событий между микросервисами. | `ES` не гарантирует упорядоченную доставку: при `refresh_interval=1s` события появляются в произвольном порядке внутри секунды. ❌ ПОСЛЕДСТВИЕ: команда поставила `ES` вместо `Kafka` для order events — потерялись 12% сообщений при rolling restart кластера, дубликаты при retry без идемпотентности.
...
> [!mcq]
> - [ ] Шард в `Elasticsearch` — это логическая единица хранения, аналогичная таблице в реляционной БД, и может находиться только на одном узле. | Шард — физический фрагмент индекса (Lucene-instance), не аналог таблицы. Аналог таблицы — `Index`. ❌ ПОСЛЕДСТВИЕ: команда вручную «зафиксировала» шарды на одном узле через `cluster.routing.allocation.include` — при падении узла потеряли 100% данных индекса вместо 33% при нормальной репликации.
> - [x] Шард в `Elasticsearch` — это полноценный экземпляр `Apache Lucene`, являющийся физическим фрагментом индекса, распределяемым по узлам кластера. | Каждый шард — самостоятельный индекс `Lucene` со своим инвертированным индексом, translog и сегментами. ✓ ПРИМЕНЯТЬ: Wikipedia делит index `enwiki` на 24 primary shards × 4 nodes для распределения 50TB по кластеру. 📋 ПРАВИЛО: «шард = Lucene-инстанс на узле, индекс = логическая коллекция шардов». 🔗 См. Q16, Q17, Q19.
> - [ ] Реплика в `Elasticsearch` — это резервная копия всего кластера, создаваемая для восстановления после сбоя. | Реплика — копия конкретного primary-шарда, а не всего кластера; снапшот кластера делается через `_snapshot` API. ❌ ПОСЛЕДСТВИЕ: при попытке восстановить кластер после сбоя из «реплики» обнаружили, что копии нет — `number_of_replicas: 1` создаёт только per-shard копии, не cluster-wide backup, потеряли неделю логов.
> - [ ] Маппинг в `Elasticsearch` создаётся только автоматически при первой индексации документа и не может быть задан вручную. | Маппинг задаётся вручную (explicit mapping) через `PUT /index { "mappings": ... }` до индексации; dynamic mapping — лишь дефолтный fallback. ❌ ПОСЛЕДСТВИЕ: положились на dynamic mapping — поле `created_at` в первом документе пришло строкой → распознано как `text` → последующие range-запросы по дате падают с `cannot be cast to date`.
...
> [!mcq]
> - [ ] `Elasticsearch` поддерживает полноценные ACID-транзакции, поэтому его можно использовать как основное хранилище вместо `PostgreSQL`. | `ES` не имеет атомарных multi-document операций и rollback — между bulk-операциями возможны частичные сбои без отката. ❌ ПОСЛЕДСТВИЕ: команда мигрировала order processing с `PostgreSQL` на `ES` — при сбое Bulk API половина заказов обновилась, половина нет, восстанавливать пришлось из логов вручную 6 часов.
...
> - [ ] `Elasticsearch` использует SQL как язык запросов, поэтому миграция с `PostgreSQL` не требует изменения запросов. | `ES` использует JSON-based `Query DSL`; `Elasticsearch SQL` поддерживает лишь подмножество (нет JOIN, оконных функций, CTE). ❌ ПОСЛЕДСТВИЕ: миграция приложения «1-в-1 через ES SQL» — JOIN-ы превратились в N+1 запросов, p95 latency вырос с 80ms до 4s, пришлось переписывать на Query DSL с денормализацией.
...
> [!mcq]
> - [ ] `Apache Lucene` — это распределённая система с REST API, которую `Elasticsearch` использует как внешний сервис. | `Lucene` — встраиваемая Java-библиотека (JAR) без сети и REST; `ES` запускает её inproc внутри JVM каждого узла. ❌ ПОСЛЕДСТВИЕ: разработчик попытался поднять «отдельный Lucene-сервер» через docker-compose — никакого образа нет, проект stalled на 2 дня пока не разобрались, что Lucene = библиотека.
> - [ ] `Elasticsearch` и `Apache Lucene` — полностью независимые проекты с разными алгоритмами индексирования. | `ES` напрямую использует Lucene-классы (`IndexWriter`, `IndexSearcher`) — каждый шард это экземпляр Lucene-индекса с теми же алгоритмами BM25, FST, posting lists. ❌ ПОСЛЕДСТВИЕ: команда «оптимизировала Lucene параметры» через `IndexWriterConfig` отдельно от `ES` — настройки игнорировались, потому что `ES` пересоздаёт writer при каждом refresh.
> - [ ] `Apache Lucene` обеспечивает шардирование и репликацию, а `Elasticsearch` добавляет только REST API поверх него. | `Lucene` ничего не знает о сети и распределённости — шардирование, репликация, master-election это слой `ES` (Zen Discovery, cluster state). ❌ ПОСЛЕДСТВИЕ: проект «облегчили» убрав ES-обвязку и взяв чистый Lucene для якобы простоты — пришлось писать свой replication-protocol, через 3 месяца отказались, вернулись к `ES`.
...
> [!mcq]
> - [ ] В стеке ELK компонент `Kibana` отвечает за сбор логов с серверов и их отправку в `Elasticsearch`. | `Kibana` — UI для визуализации (Discover, Lens, дашборды); сбор делают `Beats` (Filebeat) и `Logstash`. ❌ ПОСЛЕДСТВИЕ: архитектор поставил `Kibana` на app-серверы как «log collector» — `Kibana` не имеет input-плагинов, логи не попали в `ES`, инцидент 4 часа без observability.
...
> - [ ] В стеке ELK `Filebeat` выполняет парсинг и обогащение логов с помощью grok-паттернов перед отправкой в `Elasticsearch`. | `Filebeat` — лёгкий shipper на Go, умеет только tail-ить файлы и слать в output; grok-парсинг живёт в `Logstash` filter или Ingest Pipeline. ❌ ПОСЛЕДСТВИЕ: задача grok-парсинга закинута на `Filebeat` через несуществующий конфиг — логи приехали в `ES` raw, поле `level` не извлечено, алерты по ERROR не работают.
> - [ ] Компонент `Beats` в стеке ELK обязателен — без него `Filebeat` не может отправлять данные напрямую в `Elasticsearch`. | `Filebeat` сам является частью семейства `Beats`; `output.elasticsearch` отправляет данные напрямую в `ES`, минуя `Logstash`. ❌ ПОСЛЕДСТВИЕ: команда «искала Beats как отдельный сервис» 2 дня, перепутав название семейства с именем компонента — релиз стека логирования съехал на спринт.
...
> [!mcq]
> - [ ] Главное ограничение `Elasticsearch` — отсутствие REST API, из-за чего интеграция с Java требует специализированных библиотек. | REST API на порту 9200 — основной интерфейс `ES`; HTTP/JSON работает из любого языка. ❌ ПОСЛЕДСТВИЕ: разработчик отказался от `ES` в пользу самописного решения «потому что нет REST» — потерял 3 месяца на велосипед, который повторил 5% возможностей `ES`.
...
> - [ ] Основное ограничение `Elasticsearch` — он не поддерживает горизонтальное масштабирование и работает только на одном узле. | `ES` спроектирован для горизонтального scale-out: shards распределяются по узлам, rebalancing автоматический при join/leave. ❌ ПОСЛЕДСТВИЕ: команда вертикально масштабировала single-node ES до 256GB RAM «потому что нельзя горизонтально» — упёрлись в 32GB heap-ceiling, OS cache не помещал индекс, p99 поиска вырос в 8 раз.
...
> [!mcq]
> - [ ] Для реализации синонимов в `Elasticsearch` достаточно настроить tokenizer с типом synonym — token filters для этого не нужны. | Tokenizer только разбивает текст на токены (тип `standard`, `whitespace`); расширение синонимами — задача token filter `synonym` после токенизации. ❌ ПОСЛЕДСТВИЕ: попытка задать `tokenizer: synonym` в конфиге индекса упала с `unknown tokenizer type [synonym]`, индекс не создался, deploy откатили.
...
> - [ ] Анализатор `russian`, встроенный в `Elasticsearch`, требует отдельной установки языкового пакета перед использованием. | `russian` входит в core distribution `ES` (lang analyzers): standard tokenizer + lowercase + russian_stop + russian_stemmer, без plugin install. ❌ ПОСЛЕДСТВИЕ: DevOps три дня искал «языковой плагин» в `bin/elasticsearch-plugin list`, тормозил релиз поиска по русскоязычному каталогу — анализатор был доступен с самого начала.
...
> - [ ] Операция `flush` в `Elasticsearch` выполняется каждую секунду и делает документы доступными для поиска через обновление in-memory индекса. | `flush` — fsync сегментов и очистка translog (раз в 30 минут или при `index.translog.flush_threshold_size`); видимость для поиска даёт `refresh`. ❌ ПОСЛЕДСТВИЕ: команда выставила `flush_interval: 1s` для «быстрого поиска» — fsync на каждую секунду уронил write throughput с 50K/s до 3K/s, alerts по indexing lag не умолкали.
...
> - [ ] Операцию `refresh` нельзя вызвать вручную — она выполняется только автоматически по таймеру. | `POST /index/_refresh` или `?refresh=true` в индексирующем запросе вызывают refresh принудительно — нужно после batch-загрузки или в integration-тестах. ❌ ПОСЛЕДСТВИЕ: интеграционный тест ждал `Thread.sleep(2000)` после bulk-загрузки «потому что нельзя руками refresh» — flaky-тест ронял CI каждые 3 запуска, debug занял неделю.
...
> [!mcq]
> - [ ] Масштабирование кластера `Elasticsearch` для увеличения скорости записи достигается увеличением числа replica shards существующего индекса. | Каждая реплика дублирует write — больше реплик замедляет запись, не ускоряет; параллелизм записи задают primary shards при создании индекса. ❌ ПОСЛЕДСТВИЕ: при росте нагрузки логов выкрутили `number_of_replicas` с 1 до 3 «для масштабирования» — write throughput упал на 40%, indexing lag вырос до 15 минут, alerts ослепли.
...
> [!mcq]
> - [ ] Coordinating-узел в `Elasticsearch` хранит данные и выполняет поисковые запросы, а также дополнительно маршрутизирует запросы клиентов. | Dedicated coordinating (роли `data: false`, `master: false`) только парсит запрос, рассылает по data-узлам и агрегирует — данных у него нет. ❌ ПОСЛЕДСТВИЕ: на coordinating-узлы случайно включили роль `data` — большие aggregation-запросы пытались выполняться локально, OOM на heap 8GB при индексе 200GB, узлы выпадали из кластера.
...
> [!mcq]
> - [ ] `Query DSL` в `Elasticsearch` использует XML-формат для описания запросов, что обеспечивает совместимость с корпоративными стандартами. | `Query DSL` строго JSON в body HTTP-запроса; XML никогда не поддерживался ни в одной версии `ES`. ❌ ПОСЛЕДСТВИЕ: команда писала XML-обёртку для запросов «из corporate-стандартов» — endpoint вернул `parse_exception: not a JSON`, неделя интеграции в мусор, переписали на Jackson.
> - [ ] Leaf queries в `Elasticsearch` могут комбинировать несколько условий в одном запросе, что делает compound queries избыточными. | Leaf queries (`term`, `match`, `range`) работают с одним полем/условием; комбинации `AND`/`OR`/`NOT` строятся через `bool` compound. ❌ ПОСЛЕДСТВИЕ: разработчик впихнул в `match` две клаузы через массив — `ES` принял первую, проигнорировал вторую, поиск нашёл лишние товары, негативные отзывы клиентов.
> - [ ] Параметр `from` в запросе `Elasticsearch` определяет номер страницы результатов, начиная с 1, а `size` — размер страницы. | `from` — offset от 0 (число пропускаемых документов), не номер страницы; страница N с size=10 это `from=(N-1)*10`. ❌ ПОСЛЕДСТВИЕ: фронт передал `from=1` для первой страницы — пользователь увидел 9 товаров вместо 10, первый товар «исчез» из выдачи, баг приехал в production.
...
> - [ ] Запрос `wildcard` в `Elasticsearch` работает быстрее, чем `match`, и рекомендуется для поиска по частичному совпадению строк. | `wildcard` сканирует все термы инвертированного индекса (особенно с leading `*`), `match` использует прямой lookup; для partial search применяют `edge_ngram` или `search_as_you_type`. ❌ ПОСЛЕДСТВИЕ: поиск по `wildcard: "*phone*"` на индексе из 50M документов занимал 12 секунд, координатор узла словил circuit breaker, кластер ушёл в yellow.
...
> - [ ] Для отладки релевантности в `Elasticsearch` нужно добавить параметр `debug: true` к запросу, который вернёт пошаговый расчёт score. | Параметр отладки — `explain: true` или endpoint `GET /index/_explain/<id>`; `debug: true` `ES` молча игнорирует. ❌ ПОСЛЕДСТВИЕ: SEO-команда дебажила «почему ранжирование странное» с `debug: true` — ничего не возвращалось, написали тикет на разработку, две недели гадали пока не нашли в доке `explain`.
...
> [!mcq]
> - [ ] Metric-агрегации в `Elasticsearch` группируют документы по значениям поля и возвращают количество документов в каждой группе. | Группировка — задача Bucket-агрегаций (`terms`, `histogram`); Metric (`sum`, `avg`, `cardinality`) вычисляет одно число над набором. ❌ ПОСЛЕДСТВИЕ: аналитик запросил `avg` на топ-уровне для группировки по категории — получил среднее по всему индексу, отчёт по выручке категорий показал одинаковые цифры везде, бизнес принял ошибочное решение по ассортименту.
...
> - [ ] Агрегация `date_histogram` с `calendar_interval: month` создаёт bucket для каждого уникального дня в данных, группируя их по месяцу. | `calendar_interval: month` создаёт ровно один bucket на календарный месяц; для дневной гранулярности нужен `calendar_interval: day` или `fixed_interval: 1d`. ❌ ПОСЛЕДСТВИЕ: дашборд продаж показывал 12 точек в год вместо 365 — менеджер не видел спайков по выходным дням, упустил тренд по уикэндам, миллион упущенной выручки.
...
> - [ ] Оптимистическая блокировка в `Elasticsearch` реализуется через `_version` и работает так же, как строка VERSION в реляционных БД. | Внешний `_version` deprecated с ES 6.7: при failover primary-шарда счётчик может откатиться, что ломает ordering. Корректный механизм — пара `_seq_no` + `_primary_term`. ❌ ПОСЛЕДСТВИЕ: проект использовал `version_type=external` — после внеплановой смены primary при rolling restart часть update пропустилась без 409, в БД появились старые цены товаров.
...
> [!mcq]
> - [ ] `ElasticsearchOperations` в `Spring Data ES` используется только для агрегаций, а для обычного поиска нужен `ElasticsearchRepository`. | `ElasticsearchOperations` — низкоуровневый универсальный интерфейс для CRUD, поиска, агрегаций, IndexOps; Repository построен поверх него. ❌ ПОСЛЕДСТВИЕ: команда дублировала зависимости и инжектила `Repository` в каждый сервис «потому что Operations только для аггов» — лишние beans, тесты сломались на циклической зависимости после рефакторинга.
...
> - [ ] Аннотация `@Query` в `Spring Data Elasticsearch` принимает JPQL-запросы, которые автоматически конвертируются в `Query DSL`. | `@Query` ожидает строку Query DSL (JSON) с параметрами `?0`, `?1`; JPQL — это язык JPA для реляционных СУБД, никакой конвертации нет. ❌ ПОСЛЕДСТВИЕ: разработчик скопировал `@Query("SELECT p FROM Product p WHERE p.name = ?0")` из JPA-кода — Spring Data ES при старте упал с `ParsingException`, application context не поднялся.
...
> - [ ] Метод `repository.saveAll(List<Product>)` в `Spring Data Elasticsearch` выполняет поштучную индексацию через несколько отдельных HTTP-запросов. | `saveAll` под капотом группирует записи и отправляет их через Bulk API — один HTTP-вызов на пакет. ❌ ПОСЛЕДСТВИЕ: команда обернула `saveAll` в Spring `@Async` цикл по элементам «чтобы было параллельно» — на 100K документов получили 100K HTTP-запросов, latency загрузки выросла с 8 секунд до 12 минут.
...
> - [ ] Политика ILM применяется только к существующим индексам и не распространяется на индексы, созданные в будущем по index template. | ILM-политика прикрепляется к index template через `index.lifecycle.name`; все будущие индексы (включая rollover) наследуют её автоматически. ❌ ПОСЛЕДСТВИЕ: команда вручную привязывала ILM к каждому новому `logs-2026-*` через скрипт — после отпуска DevOps скрипт перестал запускаться, индексы росли неограниченно, диск 80% забит, кластер встал.
...
> [!mcq]
> - [ ] Отправка логов напрямую из приложения в `Elasticsearch` рекомендуется как основной production-паттерн, так как это исключает лишние компоненты в цепочке. | Прямой `LogstashTcpAppender` или HTTP-output блокирует приложение при недоступности `ES`, логи теряются без буфера; production-паттерн — `Filebeat` + `Kafka`. ❌ ПОСЛЕДСТВИЕ: при downtime `ES` 15 минут все JVM-приложения с прямым appender словили pinned threads на сетевые блокировки, request latency p99 вырос с 200ms до 8s, основной бизнес-функционал лёг.
