...
> - [ ] Маппинг индекса в Elasticsearch создаётся заново при каждом запуске и не сохраняется между перезапусками кластера. | Маппинг сохраняется в состоянии кластера (cluster state) и персистируется на диск. Он переживает перезапуск кластера и доступен сразу после восстановления. Это антипаттерн или неправильный выбор в production.
...
> [!mcq]
> - [ ] Поле _id документа всегда генерируется автоматически и не может быть задано пользователем при индексации. | _id можно задать явно при создании документа: PUT /index/_doc/my-custom-id. Если не указывать — Elasticsearch сгенерирует UUID автоматически. Это антипаттерн или неправильный выбор в production.
> - [ ] Поле _source в Elasticsearch хранит только те поля, которые указаны в маппинге, и отбрасывает незадекларированные поля. | _source хранит оригинальный JSON документа полностью, включая незадекларированные поля (если dynamic mapping не установлен в strict). Маппинг определяет индексацию, а не хранение. Это антипаттерн или неправильный выбор в production.
...
> - [ ] Обновление документа в Elasticsearch изменяет его на месте, перезаписывая только изменённые поля в существующем сегменте Lucene. | Lucene-сегменты иммутабельны: обновление документа — это фактически удаление старого (пометка как deleted) и создание нового. Физическое удаление происходит при merge сегментов. Это антипаттерн или неправильный выбор в production.
...
> [!mcq]
> - [ ] Elasticsearch определяет целевой шард документа на основе значения поля _type, поэтому документы одного типа всегда попадают в один шард. | Тип _type удалён в ES 7.0. Шард определяется формулой: shard = hash(_routing) % number_of_primary_shards. По умолчанию _routing равен _id документа. Это антипаттерн или неправильный выбор в production.
...
> - [ ] Oversharding (слишком много мелких шардов) увеличивает производительность поиска, так как запрос выполняется параллельно на большем числе шардов. | Oversharding снижает производительность: каждый шард требует ресурсов JVM и OS, а координация большого числа шардов создаёт overhead. Оптимальный размер шарда — 10-50 ГБ. Это антипаттерн или неправильный выбор в production.

> [!mcq]
> - [ ] Для изменения маршрутизации документов по кастомному полю вместо _id нужно пересоздать индекс с другим числом шардов. | Для кастомной маршрутизации достаточно передать параметр _routing при индексации: PUT /index/_doc/1?routing=user_123. Число шардов при этом не меняется. Это антипаттерн или неправильный выбор в production.
...
> - [ ] Реплика в Elasticsearch используется только для отказоустойчивости и не участвует в обслуживании поисковых запросов. | Реплики полноценно участвуют в обслуживании поисковых запросов. Координатор распределяет запросы между primary и replica shards для балансировки нагрузки чтения (read scaling). Это антипаттерн или неправильный выбор в production.
