...
`REST Client` обращается к кластеру по `HTTP/HTTPS` — запрос можно отправить на любой узел (на схеме — на Узел 1 или Узел 2). Обратите внимание: primary-шард и его реплика всегда лежат на разных узлах (например, `Shard 1 primary` на Узле 1, а его реплика — на Узле 2).
