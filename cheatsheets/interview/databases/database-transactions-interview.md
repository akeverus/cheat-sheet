...
Мнемоника -- четыре свойства как ветви одной транзакции:
...
```
 ...
 Транзакция ──┤
              ...
              └─ Durability  -- сохранность после коммита
```
...
По возрастанию строгости (каждый следующий уровень снимает ещё одну аномалию):
...
Как это выглядит на двух параллельных транзакциях -- `Транзакция 1` (`xid=100`) и `Транзакция 2` (`xid=101`), обе работают через `PostgreSQL`:

1. `Транзакция 1`: `BEGIN` -- снимок включает `xid<=100`.
2. `Транзакция 2`: `BEGIN` -- снимок включает `xid<=101`.
3. `Транзакция 2`: `UPDATE row SET value=42`. В БД создана новая версия строки (`xid=101`).
4. `Транзакция 1`: `SELECT value FROM row` -- видит старую версию (`xid<=100`), то есть значение до апдейта.
5. `Транзакция 2`: `COMMIT`.
6. `Транзакция 1`: повторный `SELECT value FROM row`. Что увидит, зависит от уровня изоляции: на `READ COMMITTED` -- уже `42`; на `REPEATABLE READ` -- по-прежнему старое значение (снимок зафиксирован на старте транзакции).
...
Схема цикла ожидания:

- `Транзакция 1` держит блокировку `A`, ждёт блокировку `B` -- то есть ждёт `Транзакцию 2`.
- `Транзакция 2` держит блокировку `B`, ждёт блокировку `A` -- то есть ждёт `Транзакцию 1`.

Получается замкнутый круг: 1 ждёт 2, а 2 ждёт 1.
...
Поток вызова через прокси (участники: `Caller`, `AOP Proxy`, `Service`, `TransactionManager`):

1. `Caller` вызывает метод -- обращение идёт к `AOP Proxy`, а не напрямую к `Service`.
2. `AOP Proxy` → `TransactionManager`: `beginTransaction()`.
3. `AOP Proxy` → `Service`: вызов реального метода.
4. Дальше -- две ветки:
   - **Успех:** `Service` возвращает результат (`return`) → `AOP Proxy` → `TransactionManager`: `commit()`.
   - **Exception:** `Service` бросает исключение (`throw`) → `AOP Proxy` → `TransactionManager`: `rollback()`.
5. `AOP Proxy` отдаёт `Caller` результат (`return`) либо пробрасывает исключение (`rethrow`).
...
Поток записи (участники: `Приложение`, `Shared Buffers`, `WAL Buffer`, `Диск (WAL)`, `Диск (Данные)`):

1. `Приложение` → `Shared Buffers`: `UPDATE row` -- изменение применяется к странице в памяти.
2. `Shared Buffers` → `WAL Buffer`: формируется WAL-запись об изменении.
3. `Приложение`: `COMMIT`.
4. `WAL Buffer` → `Диск (WAL)`: `fsync` сбрасывает WAL на диск. После этого коммит считается подтверждённым -- даже если страницы данных ещё не записаны.
5. Данные из `Shared Buffers` попадают на `Диск (Данные)` позже, периодическим `checkpoint`, а не в момент коммита.
...
Поток сообщений (участники: `Координатор`, `Участник 1 (БД)`, `Участник 2 (Очередь)`):

1. **Фаза 1: Prepare (голосование).** `Координатор` шлёт `PREPARE` обоим участникам -- `Участнику 1 (БД)` и `Участнику 2 (Очередь)`. Каждый отвечает `READY` («да, могу закоммитить»).
2. **Фаза 2: Commit.** Получив `READY` от всех, `Координатор` шлёт `COMMIT` обоим участникам. Каждый подтверждает выполнение через `ACK`.
...
Прямой ход (цепочка локальных транзакций):

- Создать заказ → Зарезервировать товар → Списать деньги → Подтвердить заказ.

Откат при ошибке (компенсации в обратном порядке). Если, например, шаг «Списать деньги» падает, запускаются компенсирующие операции для уже выполненных шагов:

- Вернуть деньги → Отменить резерв → Отменить заказ.
