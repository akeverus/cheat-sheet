...
- `TTL`-индекс обязан быть single-field: составной (compound) индекс TTL не поддерживает. При этом в одной коллекции можно держать несколько TTL-индексов по разным полям.
...
```
       ...
       ┌──────────┼──────────┐   ┌──────────┼──────────┐
       ▼          ▼          ▼   ▼          ▼          ▼
   ┌───────┐  ┌───────┐  ┌───────┐  (оба роутера шлют на все 3 шарда)
   │Shard 1│  │Shard 2│  │Shard 3│
   │(RepSet)  │(RepSet)  │(RepSet)
   └───────┘  └───────┘  └───────┘
                  ▲                         ▲
                  └───── метаданные ────────┘  (пунктир: справочные)
                  ┌───────────────────────────┐
                  │  Config Servers (RepSet)   │
                  └───────────────────────────┘
```
...
```
          ...
          ▼                                    ▼
   ┌──────────────┐              ┌──────────────────────────────┐
   │ Range sharding│             │ Есть требования к локализации │
   └──────┬───────┘              │ данных?                       │
          ▼                      └───────────────┬──────────────┘
   ┌──────────────┐                   Да         │        Нет
   │Монотонный    │            ┌────────────────┘ └───────────────┐
   │ключ?         │            ▼                                   ▼
   └──────┬───────┘     ┌──────────────┐                  ┌────────────────┐
    Да    │    Нет      │ Zone sharding│                  │ Hashed sharding│
   ┌─────┘ └──────┐     └──────────────┘                  └────────────────┘
   ▼              ▼
┌────────────────────────┐  ┌──────────┐
│ Риск горячего шарда     │  │ Оптимально│
