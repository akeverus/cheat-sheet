...
3. **Rejection sampling**: если `target_prob[i] ≥ draft_prob[i]`, токен принимается безусловно; иначе он принимается лишь с вероятностью `target_prob[i] / draft_prob[i]`, а при отказе правильный токен пересэмплируется из нормализованной разности распределений `(target − draft)⁺`. Именно эта вероятностная схема (а не жёсткий порог) и гарантирует эквивалентность.
...
- **DeepSeek-V3:** 671B всего, активны лишь 37B. 256 маршрутизируемых экспертов + 1 общий; на каждый токен выбирается top-8 маршрутизируемых + 1 общий (итого 9 активных экспертов).
...
**Self-host Llama-3-70B на 1× H100 (~$12/час — доля узла AWS p5.48xlarge: 8× H100 ≈ $98/час):**
- Throughput vLLM FP8: ~2000 tokens/sec суммарно (output)
- За час: 2000 × 3600 = 7.2M токенов
- **$12 / 7.2M ≈ $1.7 / 1M output-токенов**

При реалистичной утилизации 50% это уже ~$3.3 / 1M — плюс инженерные и операционные расходы, которых у API нет. И почти вся стоимость приходится на output — input на фоне него дёшев.
