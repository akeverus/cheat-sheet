...
Как выглядит многоуровневый граф (сверху вниз, от разреженного к плотному):

- **Level 2 (sparse, long edges)** — мало узлов, длинные «прыжки»: `A2 — B2 — C2`.
- **Level 1** — промежуточная плотность: `A1 — B1 — C1`, и дополнительно `B1 — D1`.
- **Level 0 (dense, all nodes)** — все векторы и плотные связи: `A0 — B0 — C0`, а также `B0 — D0 — E0`.
