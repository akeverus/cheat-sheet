...
| Пропускная способность | **Неограниченная** | 300 msg/sec (3000 с батчингом); до ~70 000 в High Throughput Mode |
...
**Message groups дают параллелизм обработки, а не рост общего лимита:** в обычном режиме потолок 300 msg/sec (3000 с батчингом) считается **на очередь**, а не на группу. Разные group ID обрабатываются независимо и параллельно (каждая — своя упорядоченная последовательность), но суммарную пропускную способность отправки выше 300/3000 TPS они не поднимают. Чтобы реально превысить этот потолок, включают **High Throughput Mode for FIFO** (GA с 2021): сообщения распределяются по партициям по group ID, и очередь тянет тысячи msg/sec (в крупных регионах — до ~70 000 с батчингом). Один общий group ID для всего — частая ошибка: он сериализует всю очередь в один поток.
