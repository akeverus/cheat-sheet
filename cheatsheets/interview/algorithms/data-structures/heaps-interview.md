...
> [!mcq]
>
> - [x] **Полное бинарное дерево с heap-property: значение каждого узла упорядочено относительно его детей (min или max в корне)**
>
>   Heap — это complete binary tree (все уровни заполнены, последний — слева направо), где для каждого узла его значение ≤ (min-heap) или ≥ (max-heap) значений детей.
>
>   Благодаря compactness хранится в массиве без указателей: `parent = (i-1)/2`, `left = 2i+1`, `right = 2i+2`. Используется в priority queues (Dijkstra, A*), top-K задачах, heap sort, median maintenance.
>
>   `peek()` за `O(1)`, `insert`/`extract` за `O(log n)`.
>
> - [ ] Двоичное дерево поиска (BST), где левое поддерево меньше корня, а правое — больше
>
>   Это определение **BST**, а не heap. В BST действует **глобальный** порядок (inorder обход даёт отсортированную последовательность), а в heap — только локальный parent ↔ child. Поиск произвольного элемента в BST — `O(log n)`, в heap — `O(n)`.
>
>   Путаница ведёт к выбору неподходящей структуры: для top-K BST переусложнён, для range queries heap бесполезен.
>
> - [ ] Сбалансированное дерево, где разница высот поддеревьев не больше единицы
>
>   Это определение **AVL-дерева** (или близких сбалансированных BST). Heap тоже сбалансирован по высоте (как complete tree), но это **следствие**, а не определяющее свойство. Heap-property — про порядок значений, а не про балансировку.
>
> - [ ] Структура для быстрого поиска по ключу с амортизированной сложностью O(1)
>
>   Это описание **hash table**. Heap **не предназначен** для поиска по произвольному ключу — это `O(n)`. Heap оптимизирован только под доступ к min/max за `O(1)` и их извлечение за `O(log n)`.
...
> [!mcq]
>
> - [ ] Структурой хранения: min-heap хранится в массиве, max-heap — в связном списке
>
>   Структура хранения **идентична** — оба обычно реализуются как массив (binary heap). Тип heap определяется только семантикой сравнения, не способом хранения. В Java оба варианта используют `PriorityQueue`, отличаясь лишь `Comparator`.
>
> - [x] **Направлением heap-property: в min-heap корень — минимум (parent ≤ children), в max-heap корень — максимум (parent ≥ children)**
>
>   Это **единственная** разница. Структурно оба — complete binary tree в массиве, оба имеют те же асимптотики (`O(log n)` insert/extract, `O(1)` peek).
>
>   В Java: `new PriorityQueue<>()` — min-heap по умолчанию, `new PriorityQueue<>(Comparator.reverseOrder())` — max-heap. Любой min-heap можно превратить в max-heap инверсией компаратора или хранением `-x` вместо `x`.
>
> - [ ] Сложностью операций: min-heap работает за O(log n), max-heap — за O(n)
>
>   Асимптотики **идентичны**: обе разновидности — это одна и та же структура с инвертированным сравнением. `insert`, `extractMin`/`extractMax` — `O(log n)`, `peek` — `O(1)`.
>
>   Разница в производительности — миф; выбор между min/max диктуется задачей (Dijkstra → min, top-K largest → max), а не скоростью.
>
> - [ ] Применением: min-heap для приоритетных очередей, max-heap только для сортировки
>
>   Оба применяются и там и там. Heap Sort использует **max-heap** для сортировки по возрастанию (или min-heap для убывания). Priority queue может быть любого типа — зависит от того, что важнее (наименьшая дистанция в Dijkstra — min-heap; наибольший приоритет задачи — max-heap).
...
> [!mcq]
>
> - [ ] Heap — это разновидность BST с улучшенной балансировкой и теми же операциями
>
>   Heap **не является** BST. BST поддерживает глобальный порядок (left < root < right), heap — только локальный parent ↔ child. Поиск произвольного элемента в BST — `O(log n)`, в heap — `O(n)`. Inorder обход BST отсортирован, heap — нет.
>
>   Это разные структуры с разными гарантиями и сценариями применения.
>
> - [ ] BST быстрее во всех операциях, поэтому heap используется только из-за простоты реализации
>
>   Это неверно. Heap **быстрее** BST для своих ключевых операций: `findMin/Max` за `O(1)` vs `O(log n)` в BST, `buildHeap` за `O(n)` vs `O(n log n)` для построения BST. Heap также компактнее по памяти (массив без указателей) и имеет лучшую локальность кеша.
>
>   BST превосходит heap в поиске произвольного элемента, range queries и упорядоченном обходе — это разные ниши.
>
> - [x] **Heap гарантирует только parent ↔ child порядок (быстрый min/max за O(1)), BST — глобальный порядок (быстрый поиск, range queries, сортированный обход)**
>
>   Ключевое различие — **сила порядка**:
>
>   - **Heap:** локальный инвариант (parent ⪯ children). `findMin/Max` — `O(1)`, `insert`/`extract` — `O(log n)`, поиск произвольного — `O(n)`. Inorder — мусор.
>   - **BST:** глобальный порядок (left < root < right). Поиск, insert, delete — `O(log n)` для сбалансированного. Inorder — отсортирован, поддерживает range queries и successor/predecessor.
>
>   Выбор по задаче: priority queue / top-K / heap sort → heap; lookup / range / sorted iteration → BST (TreeMap).
>
> - [ ] Heap и BST идентичны по функциональности, различаются только синтаксисом API
>
>   Это грубая ошибка. У них **разные** асимптотические гарантии и **разные** сценарии. Heap не умеет в `O(log n)` найти "есть ли элемент X" — это `O(n)`. BST не даёт `O(1)` доступ к минимуму без дополнительного указателя.
>
>   Использование heap там, где нужен BST (или наоборот) приводит к деградации производительности на порядок.
...
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q5. (!) Как хранить heap в массиве? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q6. (!) Что такое siftUp и siftDown? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q7. (!) Как строится heap из массива (buildHeap)? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q8. (!) Почему buildHeap — это O(n), а не O(n log n)? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q9. (!) Сложности операций heap? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q10. Как удалить произвольный элемент из heap? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q11. Decrease-key и increase-key — что это? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q12. Heap merge — слияние двух куч? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q13. (!) Как создать min-heap и max-heap в Java? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q14. (!) Что внутри Java PriorityQueue? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q15. Какие сложности у методов PriorityQueue? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q16. Является ли итерация по PriorityQueue упорядоченной? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q17. (!) PriorityQueue thread-safe? Какие альтернативы? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q18. (!) Алгоритм Heap Sort? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q19. Почему Heap Sort O(n log n) во всех случаях? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q20. (!) K-ый наибольший элемент в массиве? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q21. (!) Top K Frequent Elements? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q22. (!) Median from Data Stream — два heap'а? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q23. (!) Слияние K отсортированных списков? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q24. K Closest Points to Origin? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q25. Task Scheduler — минимальное время с cooldown? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q26. Reorganize String — без двух одинаковых подряд? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q27. Что такое Fibonacci heap? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q28. Что такое pairing heap? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q29. Когда heap проигрывает другим структурам? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
