...
>
> - [x] **Doubly хранит `prev` + `next`, что даёт `O(1)` удаление узла по ссылке и двунаправленный обход**
>
>     В singly-linked каждый узел содержит только `next`, поэтому чтобы удалить узел `x`, нужно найти его предшественника за `O(n)` (пройти от head). В doubly-linked есть `x.prev` — удаление становится `O(1)`: `x.prev.next = x.next; x.next.prev = x.prev`.
>
>     **Механизм:** именно поэтому Java `LinkedList` сделан doubly — он реализует `Deque` и позволяет `descendingIterator()`, а `ListIterator.remove()` работает за `O(1)`.
>
>     **Use-case:** LRU-cache (`LinkedHashMap` под капотом), browser history (back/forward), MRU-списки — везде, где нужно удалять узел, имея на него ссылку, без повторного поиска.
>
>     **Цена:** +1 ссылка на узел (на 64-битной JVM это +8 байт `prev` ≈ +25% к размеру узла).
>
> - [ ] Singly быстрее работает с памятью, потому что хранит данные подряд (cache-friendly)
>
>     Это путаница с массивами. И singly, и doubly разбросаны по куче — узлы аллоцируются отдельно, ссылки `next`/`prev` указывают на произвольные адреса. Cache locality плохая у обоих.
>
>     ❌ ПОСЛЕДСТВИЕ: разработчик выбирает LinkedList для «быстрого обхода», получает кэш-промахи на каждом `next`, и линейный обход оказывается в 3-10 раз медленнее, чем по `ArrayList`.
>
> - [ ] Doubly умеет работать как circular, а singly — нет
>
>     Circular — отдельное измерение: `tail.next = head`. И singly, и doubly могут быть circular. Circular doubly (как в Linux kernel `list_head`) — самый распространённый вариант, но это не отличие singly от doubly.
>
>     ❌ ПОСЛЕДСТВИЕ: на собеседовании путают ортогональные свойства структуры и теряют баллы за неточные определения.
>
> - [ ] В doubly нельзя реализовать стек, нужен singly
>
>     Наоборот: на doubly можно делать всё, что на singly, плюс больше. Java `Deque` (через `LinkedList`) работает и как стек (`push`/`pop`), и как очередь. Singly достаточно для стека, но не «обязательно».
>
>     ❌ ПОСЛЕДСТВИЕ: ложное ограничение приводит к over-engineering — пишут две разные реализации там, где хватило бы одной doubly.
...
>
> - [ ] LinkedList медленнее на `add()` в конец, чем ArrayList
>
>     Наоборот: `LinkedList.add(e)` стабильно `O(1)` (просто привязка к `tail`), а `ArrayList.add(e)` амортизированно `O(1)`, но иногда требует ресайза массива (`O(n)` копирование). На вставке в конец LinkedList в среднем не хуже.
>
>     ❌ ПОСЛЕДСТВИЕ: ложный аргумент в пользу ArrayList — выбор делается «по слухам», команда не понимает реальной разницы и не сможет защитить решение на code review.
>
> - [x] **`ArrayList.get(i)` работает за `O(1)` через арифметику адресов, а `LinkedList.get(i)` обходит цепочку за `O(n)` с плохой cache locality**
>
>     ArrayList хранит элементы в одном `Object[]`, поэтому `get(i)` — это `array[i]`: одна арифметическая операция плюс одно чтение из памяти, скорее всего из L1-кэша при последовательном доступе.
>
>     **LinkedList:** каждый узел — отдельный `Node` объект в куче, ссылки `prev`/`next` указывают на произвольные адреса. `get(i)` идёт по цепочке (Java оптимизирует до `O(min(i, n-i))`, начиная с ближайшего конца), но каждый прыжок — это cache miss и dereference указателя. На 100k элементах разница 50-100×.
>
>     **Цифры из реального бенчмарка:** ArrayList.get на 100k индексах ≈ 1-2 мс, LinkedList.get тех же индексов ≈ 100-200 мс из-за random memory access.
>
>     **Память:** LinkedList тратит ~40 байт на узел (header + 3 ссылки) vs ~8 байт на элемент в ArrayList — в 5 раз больше.
>
> - [ ] LinkedList не поддерживает `Iterable`, поэтому нельзя использовать в `for-each`
>
>     Оба класса реализуют `List` → `Collection` → `Iterable`. `for-each` работает для обоих. Это техническая ошибка, которая разоблачит кандидата мгновенно.
>
>     ❌ ПОСЛЕДСТВИЕ: ответ показывает, что человек не открывал исходники Java Collections и не понимает иерархию интерфейсов.
>
> - [ ] LinkedList в Java — это singly-linked, поэтому он медленнее doubly-linked ArrayList
>
>     Двойная ошибка: `LinkedList` в Java — doubly-linked (хранит `prev` и `next`). `ArrayList` вообще не «linked», это динамический массив, у него нет ссылок между элементами.
>
>     ❌ ПОСЛЕДСТВИЕ: путаница базовых терминов делает невозможным дальнейший разговор о структурах данных; такие ответы стоят оффера.
...
>
> - [ ] Dummy node ускоряет операции с `O(n)` до `O(1)` за счёт кеширования head
>
>     Сложность не меняется — dummy не кэширует head, это просто дополнительный узел в памяти. Алгоритм всё ещё проходит список за `O(n)`, но без отдельной ветки `if head == val`.
>
>     ❌ ПОСЛЕДСТВИЕ: на собеседовании кандидат путает «упрощение кода» с «оптимизацией сложности» — интервьюер делает пометку «не понимает Big-O».
>
> - [ ] Dummy node нужен только в circular списках, чтобы tail указывал на dummy вместо null
>
>     В circular списках dummy используется иначе — как маркер «конец круга». А в задачах вроде `removeElements`, `mergeTwoLists`, `partition` dummy ставится в обычные singly-linked списки именно для упрощения работы с head.
>
>     ❌ ПОСЛЕДСТВИЕ: ограничение применимости приёма — разработчик не использует dummy в линейных списках, плодит ветвления, баги на «удаление первого элемента» проскакивают в production.
>
> - [x] **Dummy убирает edge case «удаление/изменение head» — алгоритм единообразно работает с `curr.next` и в конце возвращает `dummy.next` как новый head**
>
>     Без dummy при удалении узлов нужно отдельно обрабатывать `if (head.val == val) head = head.next` — иначе при удалении первого элемента ссылка `head` не обновится. С dummy все узлы становятся «не-head», и условие удаления `curr.next = curr.next.next` работает для всех позиций одинаково.
>
>     **Механизм:** `dummy.next` всегда указывает на актуальный «настоящий head», даже если оригинальный head удалили. В конце `return dummy.next` возвращает корректный начало списка.
>
>     **Где работает:** `removeElements` (LeetCode 203), `mergeTwoLists` (21), `partition` (86), `swapPairs` (24), `oddEvenList` (328) — везде, где head может измениться. На реальном code review это снимает 30-50% багов с null-head.
>
>     **Память:** один лишний узел на стеке/куче — мизерная цена за исчезновение целого класса ошибок.
>
> - [ ] Dummy защищает от `NullPointerException` при пустом списке (`head == null`)
>
>     Частично правда, но это не основная функция. Защита от NPE достигается проверкой `while (curr.next != null)`. Главное — единообразие обработки head, а не работа с пустым списком.
>
>     ❌ ПОСЛЕДСТВИЕ: неполное понимание приёма — кандидат добавляет dummy «на всякий случай» и не может объяснить, какую именно проблему он решает; на code review такой код просят упростить.
