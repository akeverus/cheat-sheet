...
> [!mcq]
>
> **Вопрос:** Какая конкретная задача из реальной разработки требует **топологической сортировки** в графе?
>
> ---
>
> #### A) Поиск кратчайшего маршрута между офисами компании на карте — ❌ Неверно
>
> **Что на самом деле:** маршрутизация по карте — это задача про взвешенный граф с весами=расстояние/время. Решается Dijkstra (или A* для оптимизации), не топологической сортировкой. Топологическая сортировка применима только к **DAG** (Directed Acyclic Graph) и упорядочивает узлы так, что все рёбра идут «вперёд» по порядку.
>
> **Откуда путаница:** «маршрут от A до B» интуитивно звучит как «порядок шагов» — но это два разных понятия. Маршрут — последовательность вершин по рёбрам, топологический порядок — глобальное расписание вершин без зависимостей.
>
> **Если бы это было правдой:** Google Maps выдавал бы маршрут с противоречиями («поверните налево, но сначала направо») — топосорт не учитывает физические расстояния и циклы (а в дорожной сети есть круговые движения, развязки).
>
> ---
>
> #### B) Поиск кратчайшей цепочки знакомств в социальной сети («Six degrees of Kevin Bacon») — ❌ Неверно
>
> **Что на самом деле:** это задача BFS на невзвешенном графе друзей. Социальные сети — **неориентированные** (если A друг B, то B друг A) и обычно **с циклами** (треугольники друзей). Топологическая сортировка требует DAG — а социальный граф им не является.
>
> **Откуда путаница:** «цепочка знакомств» звучит как «упорядочить людей по связям». Но в социальном графе нет иерархии (X не «выше» Y), и BFS даёт кратчайший путь без необходимости упорядочивать всех.
>
> **Если бы это было правдой:** LinkedIn'у пришлось бы отказаться от feature «как вы знаете этого человека» при наличии общих друзей в треугольниках — топосорт упал бы на циклах. По факту LinkedIn использует BFS глубиной 1-2-3 («1st/2nd/3rd connection»).
>
> ---
>
> #### C) Spring Boot определяет порядок инициализации beans, учитывая `@DependsOn` и конструкторные зависимости — ✓ Верно
>
> **Развёрнутое объяснение:**
>
> Spring контейнер строит **граф зависимостей beans**: ребро из B в A означает «B зависит от A» (A должен быть создан раньше). Этот граф **должен быть ациклическим** (DAG) — Spring отказывается стартовать с `BeanCurrentlyInCreationException` при циклических зависимостях между конструкторами.
>
> Spring выполняет топологическую сортировку этого графа и инициализирует beans в порядке зависимостей: сначала те, у кого нет зависимостей (`@Repository` обычно), потом те, кто зависит от них (`@Service`), потом верхние слои (`@Controller`).
>
> **Пример:**
> ```java
> @Configuration
> public class AppConfig {
>     @Bean DataSource dataSource() { ... }           // нет зависимостей → создаётся первым
>     @Bean
>     UserRepository userRepo(DataSource ds) { ... }  // зависит от DataSource → создаётся вторым
>     @Bean
>     UserService userService(UserRepository repo) { ... }  // третьим
>     @Bean
>     UserController userCtrl(UserService svc) { ... }      // четвёртым
> }
> ```
>
> Граф зависимостей здесь линейный: `DataSource → UserRepository → UserService → UserController`. Spring запускает Kahn's algorithm (BFS-вариант топосорта) и стартует beans в этом порядке.
>
> **Когда применять:**
> - **Build systems** (Maven/Gradle) — топосорт модулей `<dependency>` перед сборкой. Если cycle — build fails.
> - **Apache Airflow DAGs** — порядок выполнения tasks. Само название «DAG» подчёркивает это.
> - **Database migrations** — Flyway/Liquibase упорядочивают миграции, учитывая зависимости (V1 → V2 → V3).
> - **Make / Bazel / Buck** — топосорт целей сборки по зависимостям.
> - **NPM/Yarn package install** — порядок установки пакетов с учётом dependencies.
>
> **Подводные камни:**
> - **Циклы — фатальны**. В Spring цикл между конструкторами = `BeanCurrentlyInCreationException`. Workaround: setter injection с `@Lazy` (отложенное создание), но это код-smell.
> - **Несколько валидных порядков**: если beans независимы — Spring выбирает любой допустимый. На воспроизводимость это влияет редко, но при отладке гонок может удивить.
> - **`@DependsOn`** — явный override автоматического разрешения. Используется когда зависимость не выражается через конструктор (например, side-effect от инициализации другого bean).
>
> **Связанные вопросы:** [[Q24]] — топологическая сортировка определение и use-cases; [[Q25]] — алгоритм Кана (BFS-подход) с in-degree; [[Q26]] — DFS-вариант через post-order.
>
> ---
>
> #### D) Recommendation engine рекомендует пользователю фильмы на основе предпочтений похожих пользователей — ❌ Неверно
>
> **Что на самом деле:** Recommendation engines используют **collaborative filtering** на bipartite графе (users ↔ items) и матричную факторизацию (SVD, ALS), не топологическую сортировку. Граф user-item не ациклический в обычном понимании и не имеет порядка обхода — нужна оценка similarity.
>
> **Откуда путаница:** «граф похожих» звучит как «упорядочить по похожести». Но это сортировка по числовой метрике (cosine similarity), не топосорт по структурным зависимостям.
>
> **Если бы это было правдой:** Netflix не показывал бы рекомендации в персонализированном порядке — только в общем «топосортированном» (что бессмысленно для разных пользователей). На практике Netflix использует matrix factorization + reinforcement learning, не topo sort.
...
>
> **Связанные вопросы:** [[Q1]] — типы графов и базовая терминология; [[Q32]] — где графы встречаются в реальной разработке; [[Q34]] — PageRank как пример graph algorithm в production.
>
> ---
>
...
>
> **Связанные вопросы:** [[Q1]] — определение графа (PageRank — directed, weighted); [[Q33]] — graph DB (Neo4j имеет встроенный PR procedure `gds.pageRank`); [[Q32]] — реальные применения графов в продукте.
>
> ---
>
