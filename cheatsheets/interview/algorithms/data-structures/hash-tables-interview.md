...
> [!mcq]
>
> **Вопрос:** Кластер из 100 шардов решает использовать `hash(key) % N` для шардирования. Что произойдёт, когда добавят 101-й шард?
>
> ---
>
> #### A) Перераспределится только 1% ключей — самый загруженный шард будет разгружен — ❌ Неверно
>
> **Что на самом деле:** при modulo-hashing смена N со 100 на 101 меняет результат `hash % N` практически для **всех** ключей. Только случайно (~1/N) ключ останется на том же шарде. Это вынудит перенести ~99% данных между шардами.
>
> **Откуда путаница:** интуитивно «добавили 1 из 100 — изменилось 1%». На деле modulo — глобальная операция, не локальная.
>
> **Если бы это было правдой:** добавление шардов было бы дешёвой операцией, и consistent hashing не было бы нужно.
>
> ---
>
> #### B) Cluster продолжит работать без изменений — старые ключи будут найдены по старому algorithm — ❌ Неверно
>
> **Что на самом деле:** при `hash % N`, изменив N, мы используем **новый** алгоритм для всех queries. Ключ, который раньше указывал на shard 5, теперь укажет на shard 7 — данные «потеряются» (физически они на shard 5, но запрос пойдёт на shard 7 и получит miss).
>
> **Откуда путаница:** ожидание «backward compatibility» без явного rebalancing.
>
> **Если бы это было правдой:** не было бы downtime/rebalancing проблем при scale-out — но именно это и есть главный вызов distributed systems.
>
> ---
>
> #### C) Перераспределится ~1/N (≈1%) ключей — это и есть consistent hashing с ключами и узлами на одном hash ring; key принадлежит первому узлу по часовой стрелке; virtual nodes улучшают балансировку — ✓ Верно
>
> **Развёрнутое объяснение:**
>
> **Consistent hashing** решает проблему massive rebalancing при `hash % N`:
> 1. **Hash ring**: и узлы, и ключи маппятся на одно круговое пространство (обычно `[0, 2³²)`).
> 2. **Принадлежность**: ключ принадлежит первому узлу по часовой стрелке (тому, чей hash ≥ hash ключа).
> 3. **Добавление узла**: новый узел занимает позицию на ринге и «отнимает» ключи только у одного соседнего узла (тот сегмент ринга между предыдущим узлом и новым). Перераспределяется ~K/N ключей.
> 4. **Удаление узла**: его ключи переходят следующему по часовой стрелке. Тоже только K/N перемещений.
> 5. **Virtual nodes (vnodes)**: каждый физический узел маппится на много точек ринга (50-200 vnodes). Без этого распределение неравномерно (узел может «выпасть» в редкой зоне).
>
> **Пример:**
> ```mermaid
> graph TD
>     subgraph "Hash ring (2³² пространство)"
>         N1[Node A: 100]
>         N2[Node B: 200]
>         N3[Node C: 300]
>         N4[Node D added at 250]
>         K1[Key X hash=150 → A→B]
>         K2[Key Y hash=220 → B→D]
>         K3[Key Z hash=280 → C unchanged]
>     end
> ```
>
> При добавлении Node D: только ключи Y (hash 200-250) переезжают с B на D; всё остальное на месте.
>
> **Когда применять:**
> - **Distributed caches**: Memcached, Redis Cluster (slot-based, упрощённая форма consistent hashing).
> - **Distributed databases**: Cassandra (token ring + vnodes), DynamoDB (партиции по hash range).
> - **CDN**: edge-серверы выбираются по consistent hashing на content URL.
> - **Load balancers с session affinity** — клиент попадает на тот же бэкенд после rebalance.
>
> **Подводные камни:**
> - **Без virtual nodes** распределение крайне неравномерное — один узел может получить 50% данных при 4 узлах. Современные системы (Cassandra) используют 256 vnodes по умолчанию.
> - **Hot keys** не решаются consistent hashing — нужен дополнительный slicing (hot-key replication, chunked queues).
> - **Token ring metadata** должен быть согласован между узлами — gossip protocol (Cassandra) или централизованный coordinator (Zookeeper).
> - **Replica placement**: для replicated cluster (factor=3) ключ идёт на 3 последовательных узла на ринге — но при vnodes это могут оказаться vnodes одного физического узла; нужен **rack-aware placement**.
>
> ---
>
> #### D) Consistent hashing использует TreeMap для хранения узлов — это даёт O(log N) на каждый routing — ❌ Неверно (как ответ на вопрос «что произойдёт»)
>
> **Что на самом деле:** утверждение про TreeMap правда (Java-реализации часто используют `TreeMap<Long, Node>` для floor/ceiling lookup) — но это не ответ на вопрос «что произойдёт при добавлении 101-го шарда». Вопрос про rebalancing impact, не про data structure.
>
> **Откуда путаница:** ассоциация «consistent hashing = TreeMap» правильная, но не отвечает на вопрос про процент перемещения.
>
> **Если бы это было правдой:** это был бы корректный ответ только на вопрос «как реализуют ring lookup» — здесь же вопрос про blast radius rebalance.
>
> ---
>
> **Связанные вопросы:** [[Q28]] — rendezvous hashing (альтернатива); [[Q3]] — коллизии (основа); [[Q31]] — HashDoS.
...
> [!mcq]
>
> **Вопрос:** В чём ключевое отличие rendezvous hashing (HRW) от consistent hashing, и когда HRW выгоднее?
>
> ---
>
> #### A) HRW требует glibal coordinator и менее distributed-friendly — ❌ Неверно
>
> **Что на самом деле:** наоборот, HRW проще — каждый узел независимо вычисляет, какому узлу принадлежит ключ. Никакого ring metadata, gossip protocol, координатора. Алгоритм: для ключа `k` вычисляем `hash(k, n)` для каждого узла n; выбираем узел с максимальным hash. Полностью stateless.
>
> **Откуда путаница:** «новее = сложнее» — типичная неверная эвристика. HRW старше consistent hashing (1996 vs 1997), но реже упоминается.
>
> **Если бы это было правдой:** GitHub, Microsoft, Google не использовали бы HRW в своих CDN/edge-системах; реально HRW популярен.
>
> ---
>
> #### B) Для каждого ключа вычисляется `hash(key, node)` для всех узлов; выбирается узел с максимальным hash; даёт лучшую балансировку и проще реализуется, но O(N) на lookup — ✓ Верно
>
> **Развёрнутое объяснение:**
>
> **Rendezvous hashing (Highest Random Weight)** — альтернатива consistent hashing:
> 1. **Алгоритм**: `node = argmax over nodes of hash(key, node)`. Хеш-функция должна равномерно распределять — обычно `hash(key + "_" + node_id)` или `MurmurHash3` от конкатенации.
> 2. **Принадлежность**: ключ принадлежит узлу, для которого `hash(key, node)` максимален. Тае-брейки решает определённость хеша.
> 3. **Перемещение при добавлении узла**: новый узел «отнимает» только те ключи, для которых его hash оказался максимальным — статистически ~K/N ключей (как в consistent hashing).
> 4. **Удаление узла**: ключи переезжают на узел с **вторым по величине** hash — никакого ring shift, всё локально.
> 5. **Сложность lookup**: O(N) — нужно вычислить hash для всех узлов. Для consistent hashing — O(log N) через TreeMap floor lookup.
>
> **Пример:**
> ```java
> String pickNode(String key, List<String> nodes) {
>     return nodes.stream()
>                 .max(Comparator.comparingLong(n -> MurmurHash3.hash(key + ":" + n)))
>                 .orElseThrow();
> }
>
> // Топ-K replicas — берём top-K по hash:
> List<String> pickReplicas(String key, List<String> nodes, int k) {
>     return nodes.stream()
>                 .sorted(Comparator.comparingLong(
>                     (String n) -> MurmurHash3.hash(key + ":" + n)).reversed())
>                 .limit(k)
>                 .toList();
> }
> ```
>
> **Когда применять:**
> - **CDN edge selection** — Microsoft Cache Array Routing Protocol (CARP) использует HRW для выбора cache-сервера.
> - **GitHub Pages routing** — выбор бэкенда без shared state.
> - **Multi-replica placement** — top-K по hash даёт K реплик ключа, и эти реплики стабильны при изменениях кластера.
> - **Малое N (≤100 узлов)** — O(N) lookup приемлем, проще чем поддерживать ring metadata.
>
> **Подводные камни:**
> - **O(N) на каждый lookup** — для тысяч узлов медленнее consistent hashing (O(log N)). Решение — multi-level: HRW для дата-центров, consistent hashing внутри.
> - **Несбалансированность при N≤10** — статистические отклонения заметны при малом числе узлов.
> - **Хеш-функция должна быть качественной** (MurmurHash, xxHash, FNV) — слабый hash даёт неравномерное распределение.
> - **Performance overhead** — для каждого запроса N хеш-вычислений; при высоком RPS и N=1000 это уже 1ns × 1000 = 1μs только на routing.
>
> ---
>
> #### C) HRW использует ring data structure, как consistent hashing — это синонимы — ❌ Неверно
>
> **Что на самом деле:** HRW не использует ring. У него нет «позиции узла на пространстве». Алгоритм — direct comparison хешей `hash(key, node)` для всех узлов. Это его главное отличие от consistent hashing.
>
> **Откуда путаница:** оба алгоритма решают одну задачу, поэтому смешивают.
>
> **Если бы это было правдой:** не было бы причин использовать HRW — он был бы худшей версией consistent hashing.
>
> ---
>
> #### D) HRW работает только для ровно 2 узлов — в больших кластерах не масштабируется — ❌ Неверно
>
> **Что на самом деле:** HRW работает для любого N. CARP протокол Microsoft использует HRW в больших CDN-сетях. Ограничение — O(N) lookup, но не количество узлов.
>
> **Откуда путаница:** «highest random weight» воспринимается как «два игрока». На деле — argmax по всем кандидатам.
>
...
> ---
>
> **Связанные вопросы:** [[Q27]] — consistent hashing (основной конкурент); [[Q3]] — коллизии (общий фон); [[Q2]] — хеш-функции.
...
> [!mcq]
>
> **Вопрос:** Cassandra проверяет Bloom filter перед чтением SSTable. Какой ответ Bloom filter гарантирует, что ключа точно нет в SSTable?
>
> ---
>
> #### A) `mightContain() == true` — означает, что ключ точно есть в SSTable — ✓ Верно (А — корректный ответ; ключ-вопрос о false negatives)
>
> **Развёрнутое объяснение:**
>
> Bloom filter — вероятностная структура с двумя возможными ответами на membership query:
> - **`mightContain() == false`** — ключа **гарантированно нет** в наборе. Нет false negatives.
> - **`mightContain() == true`** — ключ **возможно есть** (но возможен false positive). Нужна проверка реальных данных.
>
> Cassandra использует это так: для каждого SSTable хранится Bloom filter всех ключей. При read query — сначала проверяется фильтр:
> - false → пропускаем SSTable (экономим disk I/O).
> - true → читаем SSTable (может оказаться false positive, потерянное чтение).
>
> Структура: bit array `m` бит + `k` независимых хеш-функций. Вставка: устанавливает `k` бит. Запрос: проверяет все `k` бит — если все = 1, returns true.
>
> **Пример:**
> ```java
> // Guava Bloom filter
...
> filter.put("user:42");
>
> if (!filter.mightContain("user:99")) {
>     // ТОЧНО нет — пропускаем дорогой disk read
> } else {
>     // Может быть — нужна реальная проверка
>     readFromSSTable("user:99");
> }
> ```
>
> **Когда применять:**
> - **Cassandra, RocksDB, LevelDB**: bloom filter перед SSTable read — экономия 90%+ disk reads.
> - **Distributed caches**: «может ли ключ быть в L1 cache?» — если нет, идём на L2 без cache miss penalty.
> - **Web crawlers**: проверка «посещали ли уже URL?» без хранения миллионов URL в памяти.
> - **Spell checkers**: «знакомо ли слово словарю?» — false positive окей (просто пропустим warning), false negative недопустим.
> - **Database joins**: probe-фильтр в HashJoin (Spark, BigQuery).
>
> **Подводные камни:**
> - **Нельзя удалить элемент** (стандартный Bloom). Удаление одного set бита поломает другие записи. Для удалений нужен **Counting Bloom** или **Cuckoo Filter**.
> - **False positive rate растёт с числом insertions**: `(1 − e^(−kn/m))^k`. Нужно правильно подобрать `m` и `k` под expected n.
> - **Не возвращает значение**, только membership. Для key-value lookup всё равно нужен primary index.
> - **Hash quality критичен** — слабые хеши коррелируют, эффективное число «независимых» хешей меньше k.
> - **Размер фильтра в памяти**: 1B ключей при 1% FP rate — ~10GB bit array; сравните с full hash set ~50GB+.
>
> ---
>
> #### B) `mightContain() == false` — означает «возможно нет», возможен false negative — ❌ Неверно
>
> **Что на самом деле:** Bloom filter **не имеет false negatives**. Если все k бит установлены = 1, мы возвращаем true. Если хотя бы один = 0, ключ точно не вставлялся (иначе бы все его биты были = 1). false означает «гарантированно нет».
>
> **Откуда путаница:** разработчик путает направление вероятностной ошибки. false negative было бы катастрофой — фильтр пропустил бы существующий ключ, и read из SSTable не происходил бы.
>
> **Если бы это было правдой:** Cassandra теряла бы данные при чтении (фильтр ошибочно говорил «нет ключа», и SSTable не читался); система была бы непригодна для production.
>
> ---
>
> #### C) Оба ответа гарантированные — Bloom filter точная структура — ❌ Неверно
>
> **Что на самом деле:** Bloom filter **вероятностная** структура. Точный ответ только для false. Для true возможен false positive (другие ключи установили те же биты «случайно»).
>
> **Откуда путаница:** «filter» = «проверка» = «точный да/нет» — неверная интуиция. Bloom — это compromised точности за память.
>
> **Если бы это было правдой:** Bloom filter был бы заменой full hash set, но не было бы выигрыша по памяти.
>
> ---
>
> #### D) Bloom filter возвращает только true — никогда false — ❌ Неверно
>
> **Что на самом деле:** Bloom filter возвращает false, когда хотя бы один из k бит для ключа = 0. Это значит, что ключ точно не вставлялся. Если бы всегда возвращался true — фильтр был бы бесполезен (всегда нужно было бы читать с диска).
>
...
> ---
>
> **Связанные вопросы:** [[Q30]] — Cuckoo hashing/filter (с удалениями); [[Q27]] — consistent hashing (другая distributed техника); [[Q2]] — хеш-функции.
...
> [!mcq]
>
> **Вопрос:** В чём преимущество Cuckoo hashing над обычным linear probing, и какой главный риск?
>
> ---
>
> #### A) Cuckoo hashing использует только одну хеш-функцию — это даёт O(1) гарантированный lookup — ❌ Неверно
>
> **Что на самом деле:** Cuckoo hashing использует **две (или более) хеш-функций**. Каждый элемент может быть в одной из двух позиций — `h1(key)` или `h2(key)`. Lookup всегда проверяет ровно две ячейки → O(1) worst-case.
>
> **Откуда путаница:** «cuckoo» — кукушка, которая «занимает чужие гнёзда» — деталь алгоритма теряется в названии.
>
> **Если бы это было правдой:** Cuckoo hashing был бы тождественен linear probing с одной хеш-функцией; не было бы выигрыша.
>
> ---
>
> #### B) Использует две хеш-функции; при insert если обе позиции заняты — выкидывается существующий элемент в его альтернативную позицию (рекурсивно); O(1) worst-case lookup, но риск infinite loop при insert и высокой загрузке — ✓ Верно
>
> **Развёрнутое объяснение:**
>
> **Cuckoo hashing** — open addressing с **гарантированным** O(1) lookup за счёт двух хешей:
> 1. **Каждый ключ** имеет ровно две candidate-позиции: `h1(key)` и `h2(key)`.
> 2. **Lookup**: проверить обе позиции — итого 2 чтения = O(1). Без zonal scan, без tombstones.
> 3. **Insert**: положить в `h1(k)`. Если занят — вытолкнуть старый элемент на его альтернативную позицию (`h2(old)` или `h1(old)`). Если и там занят — рекурсивно выселить следующего. Если цепочка превышает порог (обычно `log N`) или цикл — **rebuild** с новыми хеш-функциями.
> 4. **Delete**: тривиальное — просто очистить ячейку, никаких tombstones (в отличие от linear probing).
>
> Вариация — **d-ary cuckoo** (d>2 хеш-функций) и **bucketized cuckoo** (k слотов на bucket) — повышают максимальную загрузку до 95%+.
>
> **Пример:**
> ```java
> class CuckooHashTable<K, V> {
>     private Entry[] table1, table2;
>     private HashFn h1, h2;
>     private int maxLoop = 32;
>
>     public boolean put(K key, V value) {
>         Entry e = new Entry(key, value);
>         for (int i = 0; i < maxLoop; i++) {
>             int idx1 = h1.hash(e.key) % table1.length;
>             if (table1[idx1] == null) { table1[idx1] = e; return true; }
>             Entry swap = table1[idx1]; table1[idx1] = e; e = swap;
>
>             int idx2 = h2.hash(e.key) % table2.length;
>             if (table2[idx2] == null) { table2[idx2] = e; return true; }
>             swap = table2[idx2]; table2[idx2] = e; e = swap;
>         }
>         rehashWithNewFunctions(); // цикл — пересоздаём таблицу
...
>
> **Когда применять:**
> - **Cuckoo filter** — улучшение Bloom filter с поддержкой удаления (RocksDB, modern caches).
> - **Network switches / TLB** — гарантия O(1) lookup критична для wire-speed forwarding.
> - **MemC3** (улучшенный Memcached) — Cuckoo hashing для concurrent throughput.
> - **Алгоритмы с жёсткими real-time гарантиями** — нет амортизированного O(1) с риском rehash во время критической операции.
>
> **Подводные камни:**
> - **Insert может зациклиться** при load factor > 0.5 (для basic 2-table cuckoo). Решение — limit на eviction chain + полный rebuild со сменой хеш-функций.
> - **Качество хеш-функций критично** — `h1` и `h2` должны быть независимыми, иначе много циклов.
> - **Memory overhead**: load factor ограничен 50% для 2-table версии — двойная память по сравнению с chaining. Bucketized cuckoo решает это (LF до 95%).
> - **Сложнее реализовать корректно** — особенно concurrent version (рекурсивные swaps плохо параллелизуются).
>
> ---
>
> #### C) Cuckoo hashing не использует хеш-функции — работает через прямое сравнение ключей — ❌ Неверно
>
> **Что на самом деле:** Cuckoo — это семейство hash table алгоритмов. Без хеш-функций это была бы plain associative array с O(n) lookup.
>
...
> ---
>
> #### D) Cuckoo hashing подходит только для небольших key-spaces (≤ 1000 элементов) — ❌ Неверно
>
> **Что на самом деле:** Cuckoo hashing успешно применяется в системах с миллионами ключей (MemC3, network switches). Ограничение — load factor (≤ 50% для basic version, до 95% для bucketized).
>
> **Откуда путаница:** ассоциация «гарантированный O(1) = только для маленьких структур».
>
> **Если бы это было правдой:** Cuckoo filter не использовался бы в RocksDB и других продакшен-системах с миллиардами keys.
>
> ---
>
> **Связанные вопросы:** [[Q29]] — Bloom filter (Cuckoo filter — его улучшение); [[Q16]] — open addressing (Cuckoo — частный случай); [[Q17]] — double hashing.
