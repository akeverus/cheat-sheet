...
Как операции связаны с концами структуры:
- **Stack (LIFO)** — оба действия трогают один конец `top`: `push` кладёт элемент на `top`, `pop` снимает с `top`. Поэтому добавление и извлечение происходят на одной и той же вершине.
- **Queue (FIFO)** — действия на разных концах: `enqueue` добавляет в хвост (`tail`), элемент проходит до головы (`head`), и `dequeue` извлекает из `head`.
