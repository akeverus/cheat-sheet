...
Например, BST с корнем `15`:
- `15` → левый ребёнок `10`, правый ребёнок `20`
- `10` → левый ребёнок `5`, правый ребёнок `12`
- `20` → левый ребёнок `17`, правый ребёнок `25`

Каждый узел слева меньше родителя, справа — больше; inorder-обход даст `5, 10, 12, 15, 17, 20, 25`.
...
**Perfect** ⊂ **Complete** ⊂ **Full** — каждое следующее свойство строже предыдущего. Heap всегда **complete**: за счёт отсутствия дыр он компактно хранится в массиве без пропусков (см. Q5).
...
Пример **LL — правый поворот**. До поворота — перекошенная влево цепочка: `3` → `2` → `1` (каждый следующий узел левый ребёнок предыдущего). После правого поворота вершиной становится `2`: `2` → левый ребёнок `1`, правый ребёнок `3`. Дерево выровнялось, BST-инвариант сохранён.
...
Пример раскраски такого дерева (структура и цвета узлов):
- `10` — чёрный, корень → левый ребёнок `5`, правый ребёнок `20`
- `5` — чёрный → левый ребёнок `3`, правый ребёнок `7`
- `20` — красный → левый ребёнок `15`, правый ребёнок `25`
- листовой уровень: `3` — чёрный, `7` — чёрный, `15` — красный, `25` — красный
...
Пример B+Tree (упрощённо). Внутренний узел-разделитель `[10, 20]` указывает на три листа:
- `[1, 5, 10]`
- `[10, 15, 20]`
- `[20, 25, 30]`

Листья дополнительно связаны в список слева направо: `[1, 5, 10]` → `[10, 15, 20]` → `[20, 25, 30]` (пунктирные sibling-указатели), что и делает range-обход последовательным.
