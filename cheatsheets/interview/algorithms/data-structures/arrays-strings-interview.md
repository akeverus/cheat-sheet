...
>     - **Развёрнутое объяснение:** `int[]` — это reified-структура JVM, элементы лежат в непрерывном блоке памяти как 4-байтовые `int`. `ArrayList<Integer>` внутри держит `Object[] elementData`, каждый элемент — ссылка на `Integer` (16+ байт overhead на объект). Размер массива фиксируется при создании, а `ArrayList` расширяет внутренний буфер в 1.5 раза при переполнении (`Arrays.copyOf`).
>     - **Пример:** `int[] arr = new int[1_000_000]` ≈ 4 МБ; `ArrayList<Integer>` с миллионом элементов ≈ 20+ МБ из-за boxing и заголовков объектов.
>     - **Когда применять:** массивы примитивов — для алгоритмов, hot-path кода, чтения больших файлов; `ArrayList` — когда нужны дженерики, динамический размер, методы `add/remove/contains`.
>     - **Подводные камни:** `Arrays.asList(intArr)` вернёт `List<int[]>` размером 1, а не `List<Integer>`. Для boxing используй `Arrays.stream(arr).boxed().toList()`. Массивы дженерик-типов создать нельзя: `new T[10]` не скомпилируется.
>     - **Связанные вопросы:** [[Q4]], [[Q35]]
> - [ ] **B) Оба используют динамический рост, но `ArrayList` быстрее на чтении из-за внутренней индексации**
>     - **Что на самом деле:** скорость `O(1)` random access одинакова — `ArrayList.get(i)` делает `elementData[i]` под капотом. `int[]` массивы НЕ растут динамически — для расширения нужно явно вызывать `Arrays.copyOf`.
>     - **Откуда путаница:** оба дают `O(1)` доступ по индексу, поэтому кажется, что они эквивалентны по производительности. Но `ArrayList<Integer>.get(i)` ещё делает unboxing — это лишний indirection через ссылку.
>     - **Если бы это было правдой:** массивы примитивов потеряли бы смысл существования, и JIT не имел бы причин их специально оптимизировать (а он оптимизирует через bounds-check elimination и SIMD).
> - [ ] **C) `ArrayList` поддерживает дженерики через reification, а массивы используют type erasure**
>     - **Что на самом деле:** наоборот — **массивы** в Java reified (знают свой компонентный тип в runtime, бросают `ArrayStoreException`), а **дженерики** в `ArrayList<T>` стираются (в runtime это просто `ArrayList`).
>     - **Откуда путаница:** студенты часто путают направление erasure. Reified vs erased — на разных сторонах языка.
>     - **Если бы это было правдой:** `new T[10]` компилировался бы без проблем, а `Object[] arr = new Integer[5]; arr[0] = "x"` падал бы только в `ArrayList`, но не в массиве.
> - [ ] **D) Массивы и ArrayList идентичны по семантике, разница только в синтаксисе `arr[i]` vs `list.get(i)`**
>     - **Что на самом деле:** они различаются в памяти (примитивы vs объекты), росте (фиксированный vs динамический), API (без методов vs `add/remove/contains`), дженериках (нет vs erasure) и контравариантности (массивы ковариантны, generics инвариантны).
>     - **Откуда путаница:** оба «индексируемые контейнеры», и в туториалах часто показывают взаимозаменяемые примеры.
>     - **Если бы это было правдой:** не было бы причин держать в JDK обе абстракции, и `Collections.toArray()` не был бы нужен.
...
>     - **Что на самом деле:** для `int[]` это НЕ сработает — `Arrays.asList(intArr)` вернёт `List<int[]>` из одного элемента (сам массив), и `Collections.reverse` развернёт список из одного элемента, оставив исходные данные нетронутыми. Работает только для `Integer[]` (массива обёрток).
>     - **Откуда путаница:** для object-массивов (`String[]`, `Integer[]`) рецепт действительно одна строка, и его часто механически переносят на примитивы.
>     - **Если бы это было правдой:** в JDK не появились бы постоянные просьбы добавить `Arrays.reverse(int[])`, и StackOverflow не имел бы тысяч вопросов «почему мой массив не реверснулся».
> - [x] **B) Two pointers in-place: `left=0`, `right=n-1`, swap и двигаемся навстречу — `O(n)` время, `O(1)` память**
>     - **Развёрнутое объяснение:** два индекса идут навстречу друг другу, на каждом шаге обмениваем `arr[left]` и `arr[right]`, инкрементируем `left` и декрементируем `right`. Цикл завершается, когда `left >= right`. Делается `n/2` обменов, никаких дополнительных аллокаций — работаем прямо в исходном массиве.
>     - **Пример:** `[1,2,3,4,5]` → swap(0,4) → `[5,2,3,4,1]` → swap(1,3) → `[5,4,3,2,1]` → left=2, right=2 → стоп. Готово за 2 итерации.
>     - **Когда применять:** любой реверс массива/строки в виде `char[]`, разворот сегмента (`reverse(arr, l, r)`), часть алгоритма rotate через тройной реверс (Q3).
>     - **Подводные камни:** условие `left < right` (НЕ `<=` — иначе средний элемент свопнется сам с собой и потеряет смысл, хотя останется корректным). Для `n=0` цикл просто не выполнится — OK. При swap через XOR (`a^=b; b^=a; a^=b`) не забыть проверить `left != right`, иначе элемент обнулится.
>     - **Связанные вопросы:** [[Q3]], [[Q21]]
> - [ ] **C) Создать новый массив того же размера и копировать `arr[n-1-i]` в `result[i]` — самый понятный способ**
>     - **Что на самом деле:** работает корректно, но тратит `O(n)` дополнительной памяти, тогда как in-place two-pointers даёт `O(1)`. Для миллиона int-ов это 4 МБ лишних аллокаций и нагрузка на GC.
>     - **Откуда путаница:** на собеседовании этот вариант проще написать без ошибок, и для маленьких массивов разница в памяти неощутима.
>     - **Если бы это было правдой:** интервьюер не задавал бы follow-up «а можно ли без дополнительной памяти», и in-place реверс не был бы каноничной техникой two-pointers.
> - [ ] **D) `IntStream.range(0, n).map(i -> arr[n-1-i]).toArray()` — функциональный одностроечник, оптимальный на современных JVM**
>     - **Что на самом деле:** Stream создаёт новый массив (`O(n)` память), boxing/unboxing не нужен только потому что это `IntStream`, но overhead на создание pipeline и lambda capture измерим. JIT не сворачивает это в SIMD-цикл.
>     - **Откуда путаница:** «современный код = stream» — модный карго-культ. На бенчмарках для горячего пути обычный цикл стабильно быстрее.
>     - **Если бы это было правдой:** библиотеки типа Apache Commons и Guava не предоставляли бы императивные хелперы, и JMH-бенчмарки не показывали бы разницу в 2-5x на массивных операциях.
...
>     - **Что на самом деле:** работает корректно и тоже занимает `O(n)` время, но требует `O(n)` дополнительной памяти. Для большого массива (миллион элементов) — 4 МБ лишних аллокаций.
>     - **Откуда путаница:** этот вариант проще объяснить и реализовать без off-by-one ошибок, поэтому его часто предлагают первым.
>     - **Если бы это было правдой:** не существовало бы фольклорной задачи «rotate in-place» как одной из топ-50 интервью-вопросов LeetCode.
> - [ ] **B) Использовать `Collections.rotate(Arrays.asList(arr), k)` — стандартное решение из JDK**
>     - **Что на самом деле:** для `int[]` опять же не сработает (`Arrays.asList(intArr)` создаёт `List<int[]>` размера 1). А для `Integer[]` `Collections.rotate` внутри использует именно тройной реверс — то есть это обёртка над правильным решением, а не альтернатива ему.
...
>     - **Если бы это было правдой:** интервьюер был бы удовлетворён ответом «есть в JDK», но он спрашивает именно про реализацию.
> - [x] **C) Тройной реверс: `reverse(arr, 0, n-1)` → `reverse(arr, 0, k-1)` → `reverse(arr, k, n-1)`**
>     - **Развёрнутое объяснение:** трюк основан на наблюдении: если развернуть весь массив, последние `k` элементов окажутся в начале (но в обратном порядке), а первые `n-k` — в конце (тоже в обратном). Два дополнительных реверса восстанавливают правильный порядок в каждой из двух частей. Всего три прохода, каждый `O(n)`, итого `O(n)` времени и `O(1)` памяти — никаких аллокаций.
>     - **Пример:** `arr=[1,2,3,4,5]`, `k=2`. Шаг 1: reverse(0,4) → `[5,4,3,2,1]`. Шаг 2: reverse(0,1) → `[4,5,3,2,1]`. Шаг 3: reverse(2,4) → `[4,5,1,2,3]`. Получили сдвиг вправо на 2 позиции.
>     - **Когда применять:** rotate в LeetCode 189, циклические сдвиги в задачах со строками (rotation check), реализация ring-buffer операций, перестановка сегментов в редакторах текста.
>     - **Подводные камни:** обязательно `k = k % n` до реверсов, иначе для `k >= n` сегменты выйдут за границы. Для пустого массива `n=0` — деление на ноль в `%`, нужен ранний return. Для сдвига влево — порядок реверсов тот же, но третий вызывается с `n-k` как границей, а не `k`.
>     - **Связанные вопросы:** [[Q2]], [[Q20]]
> - [ ] **D) Перемещать элементы по одному `k` раз: `for k раз { last = arr[n-1]; for i = n-1..1 { arr[i] = arr[i-1] }; arr[0] = last }`**
>     - **Что на самом деле:** это `O(n·k)` — для `k = n/2` получаем `O(n²)`. На массиве в миллион элементов со сдвигом 500k это триллион операций — несколько часов работы вместо миллисекунд.
>     - **Откуда путаница:** это самый интуитивный способ, который приходит в голову первым, и на маленьких массивах он работает за приемлемое время.
>     - **Если бы это было правдой:** оптимизация через тройной реверс не давала бы измеримой разницы, и интервьюеры не использовали бы этот вопрос для проверки понимания сложности алгоритмов.
