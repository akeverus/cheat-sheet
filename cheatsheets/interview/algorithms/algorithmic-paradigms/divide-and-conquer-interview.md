...
Любой D&C-алгоритм укладывается в три шага: **divide** (разбить), **conquer** (рекурсивно решить) и **combine** (собрать ответ). Задача размера `n` делится на две подзадачи `n/2`, каждая решается рекурсией, а результаты сливаются в общий ответ. По шагам:

- **Задача размера `n`** → разбивается на две подзадачи (`Divide`): первая `n/2` и вторая `n/2`.
- Подзадача `n/2` → `Conquer: рекурсия` (для каждой из двух половин).
- Оба результата `Conquer` → `Combine` (объединение).
- `Combine` → **Решение** исходной задачи.
