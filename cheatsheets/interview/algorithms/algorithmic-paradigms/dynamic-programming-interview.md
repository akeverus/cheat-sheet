...
**Решение** — запоминать результат каждой подзадачи (мемоизация). Тогда `fib(k)` для каждого `k` вычисляется ровно один раз, и `O(2ⁿ)` схлопывается в `O(n)`. В дереве вызовов повторно вычисляемые узлы — это те самые `fib(3)` и `fib(2)`, которые мемоизация делает один раз:

- `fib(5)`
  - `fib(4)`
    - `fib(3)` ← повторный вызов
    - `fib(2)` ← повторный вызов
  - `fib(3)` ← повторный вызов
    - `fib(2)` ← повторный вызов
- (а внутри второго `fib(3)` снова появляется `fib(2)` ← повторный вызов)

Здесь `fib(3)` встречается в двух разных ветках, а `fib(2)` — в трёх; именно эти повторы и устраняет кеш.
