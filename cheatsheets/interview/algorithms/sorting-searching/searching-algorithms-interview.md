...
> [!mcq]
> - [ ] Алгоритм, который всегда работает за O(log n) | ❌ ПОСЛЕДСТВИЕ: Linear Search O(n) и BFS O(V+E) — не O(log n); неверная классификация мешает выбирать нужный алгоритм
> - [ ] Алгоритм для поиска только в отсортированных структурах | ❌ ПОСЛЕДСТВИЕ: Linear Search и BFS работают без сортировки — такое ограничение исключает половину алгоритмов
> - [x] Алгоритм нахождения элемента или решения в структуре данных; делится на точечный (Binary Search), range, граничный, граф (BFS/DFS) | ✓ ПРИМЕНЯТЬ: для классификации алгоритмов на собеседовании 📋 ПРАВИЛО: поиск ≠ только Linear/Binary — BFS/DFS тоже алгоритмы поиска 🔗 См. Q22
> - [ ] Алгоритм, не изменяющий входные данные | ❌ ПОСЛЕДСТВИЕ: Sentinel Search временно модифицирует массив — определение по side-effect не разграничивает поиск и обход
...
> [!mcq]
> - [ ] Interpolation Search всегда быстрее Binary Search | ❌ ПОСЛЕДСТВИЕ: на неравномерных данных Interpolation деградирует до O(n) — выбор его для скошенного распределения сломает SLA
> - [x] Binary Search требует отсортированного массива и работает за O(log n); Interpolation — O(log log n) на равномерных; Jump — O(√n); HashMap — O(1) среднее | ✓ ПРИМЕНЯТЬ: выбор алгоритма по типу структуры данных 📋 ПРАВИЛО: тип данных → алгоритм → сложность 🔗 См. Q5
> - [ ] Jump Search предпочтительнее Binary Search на отсортированных массивах | ❌ ПОСЛЕДСТВИЕ: Jump O(√n) >> Binary O(log n) при n > 100 — для RAM массивов Binary Search всегда быстрее
> - [ ] HashMap lookup гарантированно O(1) для любого hashCode | ❌ ПОСЛЕДСТВИЕ: при коллизиях деградирует до O(log n) (Java 8+) или O(n) (до Java 8) — без хорошего hashCode гарантии нет
...
> [!mcq]
> - [ ] Linear Search быстрее Binary Search для массивов 1000+ элементов | ❌ ПОСЛЕДСТВИЕ: O(n) vs O(log n) — на 1000 элементах Binary делает ~10 итераций, Linear до 1000; выбор Linear замедлит поиск в 100x
> - [x] O(n) в среднем и худшем, O(1) в лучшем; подходит для малых массивов (<50-100) и несортированных данных; cache-friendly | ✓ ПРИМЕНЯТЬ: когда данные не отсортированы или массив мал 📋 ПРАВИЛО: Linear → когда нельзя/не нужно сортировать 🔗 См. Q4
> - [ ] Linear Search требует отсортированного массива | ❌ ПОСЛЕДСТВИЕ: передача несортированных данных в Binary Search вместо Linear даёт неверные результаты — неправильная предпосылка ведёт к неверному алгоритму
> - [ ] Linear Search возвращает все вхождения автоматически | ❌ ПОСЛЕДСТВИЕ: стандартная реализация возвращает только первый найденный индекс — ожидание всех вхождений без дополнительного кода даст только первый элемент
...
> [!mcq]
> - [ ] Sentinel сортирует массив перед поиском | ❌ ПОСЛЕДСТВИЕ: сортировка O(n log n) дороже самого поиска O(n) — суммарно алгоритм стал бы значительно медленнее
> - [ ] Sentinel гарантированно ускоряет поиск на современных CPU | ❌ ПОСЛЕДСТВИЕ: branch predictor CPU предсказывает паттерн итераций — практический выигрыш нередко нулевой или отрицательный из-за restore последнего элемента
> - [ ] Sentinel позволяет не знать длину массива | ❌ ПОСЛЕДСТВИЕ: нужно знать позицию последнего элемента для записи sentinel и последующего restore — sentinel не устраняет нужды знать bounds
> - [x] Добавляет искомый элемент в конец как sentinel — убирает проверку границы массива из условия цикла, уменьшая число сравнений вдвое | ✓ ПРИМЕНЯТЬ: микрооптимизация Linear Search в tight hot path 📋 ПРАВИЛО: sentinel устраняет boundary check ценой restore 🔗 См. Q3
...
> [!mcq]
> - [ ] `(low + high) / 2` — стандартный безопасный способ вычисления mid | ❌ ПОСЛЕДСТВИЕ: при low = Integer.MAX_VALUE - 100, high = Integer.MAX_VALUE - 50 сумма overflow → отрицательный mid → ArrayIndexOutOfBoundsException
> - [x] `low + (high - low) / 2` защищает от integer overflow; `low <= high` включает проверку последнего элемента; O(log n) время, O(1) память | ✓ ПРИМЕНЯТЬ: поиск в любом отсортированном массиве 📋 ПРАВИЛО: overflow protection = low + range/2, not (low+high)/2 🔗 См. Q29
> - [ ] `while (low < high)` включает все элементы для проверки | ❌ ПОСЛЕДСТВИЕ: при однодлинном поиске когда low == high последний элемент не проверяется → возврат -1 вместо найденного индекса
> - [ ] Binary Search работает на любом массиве включая несортированный | ❌ ПОСЛЕДСТВИЕ: несортированный массив → неверное определение какую половину отсекать → пропуск существующих элементов, ложные результаты
...
> [!mcq]
> - [ ] Рекурсивный Binary Search быстрее итеративного в Java | ❌ ПОСЛЕДСТВИЕ: лишние stack frames и отсутствие TCO в JVM дают overhead — на больших массивах рекурсивный работает медленнее
> - [x] O(log n) время, O(log n) стек — Java не оптимизирует tail calls; итеративный предпочтительнее в production из-за экономии стека | ✓ ПРИМЕНЯТЬ: когда читаемость важнее; итеративный — для production 📋 ПРАВИЛО: рекурсия ≠ итерация в Java — нет TCO → лишний стек O(log n) 🔗 См. Q5
> - [ ] Рекурсивный вариант не требует дополнительной памяти | ❌ ПОСЛЕДСТВИЕ: каждый рекурсивный вызов создаёт stack frame — при массиве 2^30 элементов → 30 frames на стеке → O(log n) памяти
> - [ ] Рекурсивный вариант проще написать корректно | ❌ ПОСЛЕДСТВИЕ: base case `if (low > high)` легко забыть → бесконечная рекурсия → StackOverflowError; итеративный с `while (low <= high)` явнее
...
> [!mcq]
> - [x] Шесть ловушек: overflow (low+high), infinite loop (забыл mid+1), `<` vs `<=`, off-by-one, дубликаты (любое вхождение), empty array | ✓ ПРИМЕНЯТЬ: чеклист перед каждой реализацией Binary Search 📋 ПРАВИЛО: overflow+loop+equals+bounds+dups+empty — 6 точек отказа 🔗 См. Q29
> - [ ] Binary Search с дубликатами всегда возвращает первое вхождение | ❌ ПОСЛЕДСТВИЕ: стандартная реализация возвращает произвольное вхождение — для first/last нужна специальная версия с продолжением поиска
> - [ ] `while (low < high)` безопаснее чем `while (low <= high)` | ❌ ПОСЛЕДСТВИЕ: при `low < high` элемент когда low == high не проверяется → ложный -1 при поиске последнего элемента диапазона
> - [ ] `high = arr.length` (без -1) безопасно в любой реализации | ❌ ПОСЛЕДСТВИЕ: `arr[mid]` когда mid = arr.length → ArrayIndexOutOfBoundsException; нужен консистентный инвариант — high включительный
...
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q9. (!) Search in Rotated Sorted Array? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q10. (!) Search Insert Position? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q11. (!) Find Peak Element? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q12. (!) Median of Two Sorted Arrays? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q13. (!) Можно ли применять Binary Search к LinkedList? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q14. Exponential Search? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q15. Interpolation Search? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q16. Jump Search? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q17. Ternary Search? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q18. (!) Что значит «binary search на ответе»? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q19. (!) Koko Eating Bananas? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q20. Capacity to Ship Packages within D Days? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q21. Split Array Largest Sum? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q22. (!) BFS как алгоритм поиска? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q23. (!) DFS как алгоритм поиска? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q24. Поиск в HashMap — действительно O(1)? ❌ ПОСЛЕДСТВИЕ: антипаттерн деградирует SLA при росте нагрузки или зависимостей.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q25. Поиск в TreeMap — O(log n)? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q26. (!) Arrays.binarySearch и Collections.binarySearch? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q27. (!) Что возвращает binarySearch при отсутствии элемента? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q28. List.indexOf — какая сложность? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q29. (!) Почему mid = (low + high) / 2 опасно? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q30. Когда Binary Search работает неправильно? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q31. (!) Как протестировать корректность Binary Search? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление- [Алгоритмы сортировки](sorting-algorithms-interview.md) ❌ ПОСЛЕДСТВИЕ: антипаттерн деградирует SLA при росте нагрузки или зависимостей.
