...
**Лидерство в команде** — ключевая компетенция для тимлида и техлида. На собеседованиях оценивают не только знание теории, но и умение применять практики в реальных ситуациях: как строить команду, решать конфликты, делегировать и развивать людей. Этот файл охватывает все основные темы с практическими сценариями и примерами.
...
**Четыре «шляпы» тимлида:**
- **Технический лидер** — архитектура, код-ревью, стандарты.
- **Координатор** — планирование, приоритеты, синхронизация с продуктом.
- **Наставник** — развитие людей, обратная связь, 1:1.
- **Защитник команды** — буфер от перегрузки, эскалация ресурсов.
...
**Фреймворк 1:1 встречи (30-60 минут) — четыре блока по порядку:**
1. **Чек-ин (5 мин)** — как дела, настроение.
2. **Блокеры (10 мин)** — что мешает, чем помочь.
3. **Развитие (10 мин)** — карьера, цели, обратная связь.
4. **Действия (5 мин)** — что делаем, кто и когда.
...
- **Stream-aligned** (поточная команда) — основной тип, отвечает за поток бизнес-ценности.
- **Enabling** (помогающая команда) — помогает другим командам осваивать новые практики.
- **Complicated-subsystem** (команда сложной подсистемы) — глубокая экспертиза в узкой области.
- **Platform** (платформенная команда) — предоставляет self-service инфраструктуру.

Все три вспомогательных типа работают на stream-aligned команду, но через разные типы взаимодействия:
- **Platform → Stream-aligned** — через `X-as-a-Service`.
- **Enabling → Stream-aligned** — через `Facilitating`.
- **Complicated-subsystem → Stream-aligned** — через `Collaboration`.
...
**Четыре шага модели по порядку:**
1. **Goal (Цель)** — чего хочешь достичь?
2. **Reality (Реальность)** — где ты сейчас? что уже пробовал?
3. **Options (Варианты)** — какие есть пути?
4. **Will (Намерение)** — что конкретно сделаешь? когда?
...
Матрица — это таблица «область решения × уровень делегирования». Колонки уровней (слева направо, от полного контроля лидера к полному доверию): `Tell` → `Sell` → `Consult` → `Agree` → `Advise` → `Delegate`. В каждой строке для области решения отмечается один уровень.

**Пример заполнения:**
- Архитектура сервиса → `Consult`
- Выбор библиотеки → `Advise`
- Стиль кода → `Agree`
- Отпуска → `Delegate`
- Найм → `Sell`
- Техдолг в спринте → `Agree`
...
**Поток решения по шагам:**
1. Конфликт: два подхода.
2. Отдельные беседы с каждым.
3. Зафиксировать критерии оценки.
4. `Structured debate`.
5. Развилка «консенсус найден?»:
   - **Да** → сразу зафиксировать решение в `ADR`.
   - **Нет** → тимлид принимает решение, после чего оно фиксируется в `ADR`.
6. Итог в обоих случаях — обе стороны поддерживают решение.
...
Четыре метрики разбиты на две группы:
- **Скорость** — `Deployment Frequency` и `Lead Time for Changes`.
- **Стабильность** — `Change Failure Rate` и `Time to Restore`.

Между группами есть баланс: `Lead Time for Changes` (скорость) уравновешивается `Change Failure Rate` (стабильность) — нельзя разгонять одно в ущерб другому.
...
| Lead Time for Changes | < 1 часа | < 1 недели | < 1 месяца | > 1 месяца |
| Change Failure Rate | 0–15% | 16–30% | 31–45% | > 45% |
| Time to Restore | < 1 часа | < 1 дня | < 1 недели | > 1 недели |
