...
>
>   **Связанные вопросы:** [[http-rest-interview#Q3]] выбор HTTP-методов, [[http-rest-interview#Q5]] PUT vs PATCH vs POST, [[http-rest-interview#Q9]] HTTP status codes.
...
>
>   **Связанные вопросы:** [[http-rest-interview#Q3]] выбор HTTP-методов, [[http-rest-interview#Q4]] safe и идемпотентность, [[http-rest-interview#Q6]] HTTP-заголовки.
>
...
>
>   **Связанные вопросы:** [[http-rest-interview#Q4]] safe и идемпотентность, [[http-rest-interview#Q5]] PUT vs PATCH, [[http-rest-interview#Q9]] HTTP status codes (304, 412).
>
...
> [!mcq]
>
> **Вопрос:** Какую схему HTTP-аутентификации выбрать для server-to-server взаимодействия в zero-trust окружении, где важна максимальная безопасность канала и взаимная идентификация?
>
> - [ ] **A)** `Basic` через HTTPS с длинными паролями и ротацией раз в год
>
>     Basic шлёт `login:password` (base64) в каждом запросе — это не шифрование, а кодирование. Даже через HTTPS компрометация одного клиента раскрывает пароль навсегда, а ротация «раз в год» противоречит zero-trust (минимизация blast radius). Schema подходит лишь для legacy-интеграций.
>
> - [ ] **B)** `Bearer` с JWT, подписанным `HS256` и сроком жизни 24 часа
>
>     Симметричный `HS256` требует общего secret на всех сервисах — компрометация одного сервиса раскрывает ключ всего кластера. 24-часовой TTL без revocation даёт атакующему почти сутки на эксплуатацию украденного токена. Это анти-паттерн именно для distributed/zero-trust систем.
>
> - [x] **C)** `mTLS` — взаимная аутентификация по сертификатам с короткими TTL и автоматической ротацией через service mesh
>
>     mTLS обеспечивает криптографическую идентификацию обеих сторон на уровне TLS-handshake: клиент и сервер предъявляют сертификаты, выданные доверенным CA. Service mesh (Istio, Linkerd) автоматизирует выдачу/ротацию (SPIFFE/SPIRE), что устраняет «человеческий» промах с просроченными ключами. Это канонический выбор для zero-trust S2S.
>
>     **Почему именно так:** zero-trust требует «никогда не доверять, всегда проверять» — mTLS делает проверку обязательной на каждом hop, без зависимости от network perimeter.
>
>     **Когда применять:** все internal microservice-вызовы в production, где сеть рассматривается как hostile (Kubernetes/service mesh).
>
> - [ ] **D)** `X-API-Key` в заголовке с UUID v4, общим для всех инстансов сервиса
>
>     API Key — это «общий пароль»: один ключ на множество инстансов означает отсутствие per-instance идентификации. При компрометации любого инстанса (утечка из логов, образа, env) ключ нужно менять везде. Schema приемлема для публичных API сторонних интеграторов, но не для internal zero-trust.
...
> [!mcq]
>
> **Вопрос:** В микросервисной системе с несколькими auth- и resource-сервисами вы выбираете алгоритм подписи JWT. Какой выбор корректен и почему?
>
> - [ ] **A)** `HS256` (HMAC-SHA256) — общий секрет дублируется на все resource-сервисы, секрет генерируется один раз при деплое
>
>     `HS256` симметричный: тот же ключ нужен и для подписи, и для верификации. В микросервисах это означает раздачу secret всем resource-сервисам — компрометация любого из них позволяет атакующему выпускать валидные токены от имени auth-сервера. «Один раз при деплое» = отсутствие ротации, что усугубляет risk.
>
> - [ ] **B)** `none` (без подписи) — для внутренних сетей это допустимо, поскольку периметр уже защищён
>
>     `alg: none` — известная уязвимость (CVE-2015-9235 и десятки производных): любой может подделать токен, просто убрав подпись. «Защищённый периметр» — миф (zero-trust), и любая SSRF/RCE внутри сети превращается в полную компрометацию. Алгоритм `none` должен быть явно отключён в библиотеке валидации.
>
> - [ ] **C)** `RS256` с одним общим private key, который раздаётся всем сервисам для верификации
>
>     Ошибка в назначении ключей: при `RS256` **private key** подписывает (только auth-сервер), а **public key** верифицирует (все resource-сервера). Если раздать private key всем — теряется главное преимущество асимметрии: любой сервис сможет выпускать токены, и компрометация одного резолвится в компрометацию системы.
>
> - [x] **D)** `RS256` (или `ES256`): auth-сервер хранит private key и подписывает токены; resource-сервера получают public key через JWKS-endpoint и только верифицируют
>
>     Это канонический pattern для distributed JWT. Асимметричная подпись разделяет роли: только auth-сервер может выпускать токены (private key изолирован), resource-сервера лишь верифицируют через публичный ключ. JWKS-endpoint (`/.well-known/jwks.json`) позволяет автоматическую ротацию ключей по `kid`-полю в header.
>
>     **Почему именно так:** компрометация resource-сервера не позволяет выпускать новые токены — атакующий получит лишь public key, который и так публичен. `ES256` (ECDSA) даёт меньший размер подписи при той же стойкости, что выгодно при частых запросах.
>
>     **Когда применять:** любые multi-service системы, где JWT валидируется более чем одним сервисом; обязательно для OAuth2/OIDC.
...
> [!mcq]
>
> **Вопрос:** Команда выводит новый REST API в production. Какой набор мер обязателен как минимальный security baseline, а не «приятное дополнение»?
>
> - [x] **A)** TLS с современными cipher-suite, `HSTS`, server-side input validation, rate limiting с `429 + Retry-After`, секреты в vault с ротацией, аудит-логи без PII
>
>     Это и есть production baseline по OWASP API Top 10 и SRE-практикам: TLS+HSTS закрывает transport, валидация — injection-атаки, rate limiting — DoS/credential stuffing, vault — утечки секретов из репо/env, аудит без PII — compliance (GDPR, PCI-DSS). Каждый пункт закрывает конкретный класс атак, опустить любой = известная уязвимость.
>
>     **Почему именно так:** security — это defense in depth, а не один контроль. Пропустить rate limiting = открыть credential stuffing; пропустить input validation = SQL injection даже при идеальной аутентификации.
>
>     **Когда применять:** day-1 любого публичного или internal-but-untrusted API; не «когда дойдут руки», а как gate для production-релиза.
>
> - [ ] **B)** Достаточно HTTPS и `Authorization: Bearer` — остальное добавим, когда появятся реальные атаки
>
>     Reactive-подход к security катастрофичен: к моменту «реальной атаки» утечка уже произошла, и стоимость инцидента (репутация, штрафы GDPR, downtime) на порядки выше превентивных мер. Отсутствие rate limiting позволяет brute-force за минуты; отсутствие input validation — SQL injection с первого запроса автоматизированного сканера.
>
> - [ ] **C)** WAF перед сервисом снимает необходимость в server-side валидации и rate limiting — это «security as a service»
>
>     WAF — дополнительный слой, но не замена. Он работает по сигнатурам/паттернам и обходится через encoding, fragmentation, business-logic атаки (BOLA, mass assignment), которые WAF не понимает. Server-side validation — единственное место, где известна business semantics запроса. Доверять только WAF — known anti-pattern.
>
> - [ ] **D)** Достаточно скрыть API за VPN или private network — internal-сервисы можно не защищать
>
>     «Security by perimeter» противоречит zero-trust и проваливается при первой SSRF/RCE/insider-атаке. Большинство громких breach-ов последних лет — это lateral movement после прорыва периметра. Internal API обязан иметь те же контроли (authN/Z, validation, rate limiting), что и публичный.
...
> [!mcq]
> - [ ] **A.** Один длинный JWT на 30 дней с широкими scopes — клиенту удобно, не нужно обновлять токен.
>
>     Долгоживущий токен с правами «на всё» — это сразу два антипаттерна: украденный токен действует месяцами, и любой компромисс эскалируется до admin-уровня.
>
>     ❌ ПОСЛЕДСТВИЕ: утечка одного токена даёт атакующему месяцы full-access, отозвать без перевыпуска всех клиентов нельзя.
>
> - [x] **B.** Короткий access_token (15 мин) + refresh_token с ротацией, least-privilege scopes, rate limiting на login и `429` с `Retry-After`.
>
>     Это сумма контрмер из таблицы: ограничение TTL, разделение прав, защита auth-эндпоинтов от brute-force, корректная сигнализация клиенту о backpressure.
>
>     Когда атакующий перехватит access_token, у него окно 15 минут вместо месяцев; refresh_token при ротации инвалидирует прошлую цепочку при подозрении на утечку.
>
>     В production это базовый минимум для любого публичного API: OAuth2 BCP (RFC 9700) и OWASP API Security Top 10 явно требуют именно такой набор.
>
>     ✅ ВЫГОДА: ущерб от утечки токена ограничен минутами, brute-force на login упирается в `429`, scope-эскалация невозможна.
>
> - [ ] **C.** Логировать полное тело запроса и ответа (включая `Authorization` и PII) — это поможет на инцидентах.
>
>     Verbose-логирование секретов и PII — классический способ превратить сам логгинг-стек в источник утечки: ELK, S3-бэкапы, чужие dashboards.
>
>     ❌ ПОСЛЕДСТВИЕ: при компрометации Kibana или S3-bucket с логами атакующий получает живые токены и персональные данные пользователей.
>
> - [ ] **D.** `Access-Control-Allow-Origin: *` вместе с `Allow-Credentials: true` — чтобы любые фронтенды могли ходить в API.
>
>     Этот комбинация запрещена спецификацией CORS, но даже если форсировать — это открывает CSRF-подобные атаки: чужой origin со cookies жертвы будет легитимно дёргать API.
>
>     ❌ ПОСЛЕДСТВИЕ: любой вредоносный сайт, на который зашёл аутентифицированный пользователь, выполнит действия от его имени.
...
> [!mcq]
> - [ ] **A.** Поставить Fixed Window 1000 запросов/мин в одном инстансе приложения, считать в `ConcurrentHashMap` в памяти.
>
>     In-memory счётчик не переживёт перезапуск пода и не работает в кластере: на 3 инстансах клиент получит 3000 запросов/мин фактически.
>
>     ❌ ПОСЛЕДСТВИЕ: rate limit «протекает» пропорционально числу инстансов, при rolling update счётчики обнуляются и атакующий получает бесплатное окно.
>
> - [ ] **B.** При превышении лимита возвращать `403 Forbidden` без специальных заголовков — клиент сам разберётся.
>
>     `403` означает «у вас нет прав», а не «слишком часто»; без `Retry-After` и `X-RateLimit-*` клиент не знает, когда повторить, и начнёт hammering с экспонентой ретраев.
>
>     ❌ ПОСЛЕДСТВИЕ: клиент трактует ошибку как permanent, либо ретраит сразу же — DDoS-эффект на собственный API.
>
> - [x] **C.** Token Bucket в Redis на уровне API Gateway, возврат `429 Too Many Requests` с `Retry-After` и `X-RateLimit-Remaining`.
>
>     Token Bucket в распределённом storage (Redis) даёт единый счётчик на кластер и разрешает контролируемые burst-всплески, что соответствует реальным паттернам клиентов.
>
>     `429` с `Retry-After` — стандартный сигнал IETF: клиент знает, через сколько секунд можно повторить, экспоненциальный backoff не нужен.
>
>     Вынос лимита на Gateway (Kong/Envoy/nginx) защищает backend ещё до того, как запрос дойдёт до приложения, экономя CPU и DB-коннекты.
>
>     ✅ ВЫГОДА: единый лимит на весь кластер, корректное поведение клиентов при backoff, защита backend на границе системы.
>
> - [ ] **D.** Лимитить только по IP-адресу и игнорировать API key — анонимные клиенты опаснее всего.
>
>     IP-only ломается на NAT (один корпоративный IP = тысячи легитимных юзеров) и легко обходится через ботнеты и резидентные прокси.
>
>     ❌ ПОСЛЕДСТВИЕ: легитимные клиенты за NAT блокируются массово, а атакующие с пулом IP проходят limit без проблем.
...
> [!mcq]
> - [ ] **A.** Перейти с stateless на server-side sessions с sticky-балансировкой — это упростит реализацию и ускорит запросы.
>
>     Sticky sessions ломают горизонтальное масштабирование: при падении инстанса все его сессии теряются, а нагрузка между подами становится неравномерной.
>
>     ❌ ПОСЛЕДСТВИЕ: масштабирование требует общего session store или приклеенных клиентов; rolling update вызывает массовые logout и потерю состояния корзины.
>
> - [ ] **B.** Возвращать полные коллекции без пагинации (`GET /orders` возвращает все 10M записей) — клиент сам разберётся.
>
>     Unbounded responses съедают heap сервиса и сеть; один запрос может выбить под через OOM, а latency p99 уйдёт в минуты.
>
>     ❌ ПОСЛЕДСТВИЕ: первый же популярный endpoint становится точкой деградации всего сервиса, добавление DB-read-реплик не помогает.
>
> - [ ] **C.** Синхронно обрабатывать тяжёлые операции (генерация отчёта, видео-транскод) в HTTP-запросе — это проще для клиента.
>
>     Длинные синхронные запросы держат worker-потоки и TCP-коннекты, забивают thread pool и приводят к каскадным таймаутам у вызывающих сервисов.
>
>     ❌ ПОСЛЕДСТВИЕ: при росте нагрузки thread pool исчерпывается, gateway возвращает `504` всем клиентам, retry-шторм добивает backend.
>
> - [x] **D.** Stateless-инстансы + Cache-Control/ETag + пагинация + idempotency-keys для retry + `202 Accepted` с polling/webhook для long-running operations.
>
>     Stateless даёт неограниченное горизонтальное масштабирование: любой под обработает любой запрос, autoscaler работает по CPU/RPS без оглядки на affinity.
>
>     `Cache-Control`+`ETag` снимают нагрузку через CDN и conditional requests; пагинация ограничивает размер ответа и DB-сканирование.
>
>     Idempotency-keys позволяют клиентам безопасно ретраить при сетевых ошибках; `202 Accepted` развязывает длинные операции от HTTP-цикла.
>
>     ✅ ВЫГОДА: линейное масштабирование инстансов, p99-latency предсказуем под нагрузкой, тяжёлые операции не блокируют request-pool.
...
> [!mcq] Главное преимущество HTTP/2 над HTTP/1.1 для REST API
>
> - [ ] **HTTP/2 использует UDP вместо TCP, что снижает latency**
>   - Это описание HTTP/3 / QUIC. HTTP/2 всё ещё работает поверх TCP — именно поэтому HTTP/3 и появился, чтобы решить TCP head-of-line blocking
>   - ПОСЛЕДСТВИЕ: ошибка в архитектурном собеседовании — путаница уровней транспорта
>
> - [x] **Мультиплексирование позволяет отправлять много запросов в одном TCP-соединении без head-of-line blocking на уровне HTTP**
>   - HTTP/1.1 требует отдельное соединение на запрос (или pipelining с HOL blocking), HTTP/2 разделяет один TCP-канал на множество параллельных потоков (streams). Плюс HPACK сжимает повторяющиеся заголовки (`Authorization`, `Content-Type`), что экономит трафик на чатти-API
>   - МЕХАНИЗМ: бинарные frames + stream multiplexing внутри одного соединения
>   - КОГДА: фронтенд делает десятки параллельных запросов к API, mobile с дорогим радио
>   - ВЫГОДА: меньше TCP-соединений, ниже p95 latency, экономия CPU на TLS handshake
>
> - [ ] **HTTP/2 обязательно требует TLS по спецификации**
>   - Спецификация HTTP/2 (RFC 7540) допускает h2c (cleartext), но все мажорные браузеры реализуют только h2 поверх TLS. Это де-факто, не de-jure требование, и неточно как "главное преимущество"
>   - ПОСЛЕДСТВИЕ: формально неверное утверждение про стандарт, путает с операционной практикой браузеров
>
> - [ ] **Server Push автоматически отправляет клиенту все нужные ресурсы заранее**
>   - Server Push существует в HTTP/2, но в 2022 Chrome его deprecated из-за плохой эвристики и проблем с кэшем. Это не главное и не работает на практике для REST API
>   - ПОСЛЕДСТВИЕ: вы предложите устаревшую фичу, которая не даст обещанного выигрыша
...
> [!mcq] В каком сценарии SOAP действительно оправдан вместо REST
>
> - [ ] **Для публичного web/mobile API с тысячами клиентских интеграций**
>   - Это явная зона REST: JSON, низкий порог входа, не нужен XML-парсер у клиента, простая отладка curl-ом. SOAP здесь добавит сложности без выигрыша
>   - ПОСЛЕДСТВИЕ: уроните DX интеграторов и время вывода API на рынок
>
> - [ ] **Когда важна высокая производительность service-to-service вызовов**
>   - SOAP по производительности проигрывает REST/JSON и тем более gRPC — XML-сериализация дороже и многословнее. Для перформанса выбирают gRPC, не SOAP
>   - ПОСЛЕДСТВИЕ: уменьшение throughput на нагрузочном тесте при том же hardware
>
> - [x] **Enterprise/банковская интеграция с требованием WS-Security, WS-AtomicTransaction и формального WSDL-контракта**
>   - SOAP оправдан, когда команда явно нуждается в WS-* стеке: распределённые транзакции (WS-AtomicTransaction), сообщение-уровневые подписи и шифрование (WS-Security), строгий WSDL для генерации клиентов в legacy-системах, поддержка не-HTTP транспорта (JMS, SMTP)
>   - МЕХАНИЗМ: формальный контракт WSDL + расширения WS-* поверх envelope
>   - КОГДА: интеграция с банковскими ESB, страховые B2B-обмены, legacy-партнёры на SOAP
>   - КРИТЕРИЙ ВЫБОРА: если ни одна WS-* фича не требуется — берите REST
>
> - [ ] **Когда нужен гибкий выбор полей в ответе**
>   - Это сценарий GraphQL (over/under-fetching), не SOAP. SOAP возвращает целые типизированные объекты по WSDL без гибкого projection
>   - ПОСЛЕДСТВИЕ: вы решаете не ту проблему — добавляете SOAP, но over-fetching остаётся
...
> [!mcq] Мобильное приложение для трейдинга показывает live-котировки тысячам пользователей. Какой протокол выбрать?
>
> - [ ] **REST с polling каждые 100 ms через `GET /quotes/AAPL`**
>   - Polling 10 раз в секунду создаёт огромный overhead: каждый запрос — HTTP-заголовки, TLS-handshake (если без keep-alive), нагрузка на сервер растёт линейно с числом клиентов. Большинство ответов будут "ничего не изменилось"
>   - ПОСЛЕДСТВИЕ: x100 нагрузки на сервер при низкой polluant utility, рост счетов на CDN/трафик
>
> - [ ] **GraphQL subscriptions через HTTP**
>   - GraphQL subscriptions решают похожую задачу, но под капотом всё равно используют WebSocket (graphql-ws). Сам по себе GraphQL — про гибкую выборку полей, а не про push. Лишний слой для одной задачи push-уведомлений
>   - ПОСЛЕДСТВИЕ: добавите тяжёлый стек GraphQL ради функциональности WebSocket
>
> - [x] **WebSocket — постоянное двунаправленное соединение для push котировок**
>   - WebSocket даёт серверу возможность пушить обновления без request/response overhead. Одно TLS-соединение, бинарные frames, минимальная latency. Подходит для трейдинга где клиент тоже отправляет команды (place order)
>   - МЕХАНИЗМ: HTTP Upgrade → ws/wss соединение, full-duplex frames
>   - КОГДА: live-котировки + клиентские команды (двунаправленность), чаты, multiplayer-игры
>   - АЛЬТЕРНАТИВА: если нужно только сервер→клиент без команд — SSE проще (HTTP, авто-reconnect)
>
> - [ ] **gRPC unary RPC по каждому тику**
>   - gRPC хорош для service-to-service, но unary RPC на каждый тик — то же polling. Streaming RPC мог бы подойти, но мобильные клиенты редко используют gRPC напрямую (нужен gRPC-Web прокси и сложности с TLS)
>   - ПОСЛЕДСТВИЕ: усложнение мобильного клиента и инфраструктуры без выигрыша над WebSocket
...
> [!mcq] Set-Cookie для session-id, защищённый от XSS и CSRF одновременно — правильная конфигурация
>
> - [ ] **`Set-Cookie: session_id=abc; Path=/`**
>   - Без `HttpOnly` cookie доступна из `document.cookie`, любая XSS-инъекция уведёт сессию. Без `Secure` cookie уйдёт по HTTP в открытом виде на промежуточных узлах. Без `SameSite` любой кросс-сайт POST прокинет сессию (CSRF)
>   - ПОСЛЕДСТВИЕ: классическая дыра — украденная сессия через XSS + CSRF на платёжных операциях
>
> - [x] **`Set-Cookie: session_id=abc; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=3600`**
>   - `HttpOnly` блокирует доступ из JavaScript (защита от XSS-кражи), `Secure` гарантирует передачу только по HTTPS, `SameSite=Strict` не отправляет cookie при cross-site запросах (защита от CSRF), `Max-Age` ограничивает время жизни. Это золотой стандарт для session-cookie
>   - МЕХАНИЗМ: атрибуты cookie применяются браузером, сервер декларирует политику
>   - КОГДА: classic web (SSR, MPA) с серверной сессией; для SPA через bearer-token аналогичная защита достигается через короткий access + HttpOnly refresh
>   - НЮАНС: `SameSite=Strict` ломает переходы по внешним ссылкам (cookie не отправится); для логин-flow используйте `Lax`
>
> - [ ] **`Set-Cookie: session_id=abc; HttpOnly; SameSite=None`**
>   - `SameSite=None` без `Secure` отвергается современными браузерами вообще. И `None` сам по себе отключает CSRF-защиту через SameSite — нужны cross-site сценарии (embed) и явная защита
>   - ПОСЛЕДСТВИЕ: cookie не установится в Chrome/Firefox, либо открывается CSRF-вектор
>
> - [ ] **`Set-Cookie: session_id=abc; Secure; Domain=.example.com`**
>   - Нет `HttpOnly` → XSS уведёт сессию. `Domain=.example.com` расширяет область на все поддомены — компрометация любого поддомена сливает сессию. Это анти-паттерн для критичных cookie
>   - ПОСЛЕДСТВИЕ: уязвимость subdomain takeover превращается в полную компрометацию сессии
...
> [!mcq] На каком уровне пирамиды лучше всего проверять соответствие REST API контракту OpenAPI
>
> - [ ] **Unit-тесты на сервис-слое — проверяют бизнес-логику, заодно валидируют OpenAPI**
>   - Unit-тесты вызывают сервис напрямую, минуя контроллер и сериализацию. Они не видят реальный HTTP-ответ — структуру JSON, заголовки, статусы. OpenAPI compliance тут не проверишь
>   - ПОСЛЕДСТВИЕ: ложное чувство покрытия — на проде клиенты ловят несовпадение схемы, а CI зелёный
>
> - [x] **Contract-тесты — отдельный уровень между unit и integration, валидирует ответы против OpenAPI-схемы**
>   - Contract-тесты (через `spring-cloud-contract`, `rest-assured` + `swagger-request-validator`, Pact) поднимают только web-слой (MockMvc/WebTestClient), вызывают endpoint и сверяют ответ с OpenAPI. Быстрые, детерминированные, ловят breaking changes в схеме до интеграционного запуска
>   - МЕХАНИЗМ: парсер OpenAPI читает спеку, валидатор сравнивает структуру/типы/required-поля ответа
>   - КОГДА: каждый MR в CI, до integration-стадии; обязательно для публичного API
>   - ВЫГОДА: ловит несоответствие схемы за секунды, не нужны БД и брокеры
>
> - [ ] **E2E smoke-тесты на staging — гоняют реальные HTTP-запросы, контракт проверяется естественно**
>   - E2E медленные, нестабильные, требуют деплоя. Полагаться на них для contract validation = ловить ошибки уже после мержа. Они хороши для smoke critical paths, а не для регулярного contract-чека
>   - ПОСЛЕДСТВИЕ: цикл обратной связи 10–30 минут вместо 10 секунд, дорогая нестабильность
>
> - [ ] **Нагрузочные тесты — заодно бьют по всем endpoint-ам и видят расхождения**
>   - Нагрузочные тесты проверяют latency/RPS, а не структуру ответов. Они часто игнорируют тело (читают только статус). Использовать их как валидатор контракта — антипаттерн
>   - ПОСЛЕДСТВИЕ: контрактные баги пройдут незамеченными, тест прокачает невалидные ответы
...
> [!mcq] Алерт сработал: p99 latency `/api/v1/checkout` вырос с 200 ms до 3 s. Какой первый шаг диагностики
>
> - [ ] **Перезапустить поды checkout-сервиса**
>   - Рестарт без понимания причины — лотерея. Если проблема в downstream (БД, payment-gateway), рестарт ничего не починит. Если в утечке памяти, рестарт даст временное облегчение, но не решит root cause
>   - ПОСЛЕДСТВИЕ: проблема вернётся через час, инцидент затянется, потеряете SLO
>
> - [x] **Открыть distributed trace одного из медленных запросов и найти span, который занимает 90% времени**
>   - Трейс показывает waterfall вызовов: API gateway → checkout → payment → DB. Один взгляд — и видно где bottleneck (обычно внешний вызов или БД-запрос). Это быстрейший путь от симптома к root cause
>   - МЕХАНИЗМ: traceId связывает все span-ы запроса, span с максимальной duration — кандидат на корень
>   - КОГДА: любая локализованная деградация latency на конкретном endpoint
>   - ВЫГОДА: MTTR минуты вместо часов; не нужно гадать, видна реальная цепочка
>
> - [ ] **Включить debug-уровень логирования на checkout и собрать логи за час**
>   - Debug-логи генерируют гигабайты данных под нагрузкой, перегружают log-pipeline, могут уронить сервис. Поиск иголки в стогу без трейса — долго и шумно
>   - ПОСЛЕДСТВИЕ: лог-storage и сервис под двойной нагрузкой, диагностика на часы
>
> - [ ] **Сравнить количество подов с прошлой неделей**
>   - Saturation-метрики важны, но это четвёртый шаг (когда уже знаем, что bottleneck в CPU/memory сервиса). Без трейса непонятно, виноват ли сам сервис или его зависимости
>   - ПОСЛЕДСТВИЕ: добавите подов "на всякий случай" — деньги в air, проблема остаётся в downstream
...
> [!mcq] Нужно импортировать 100 000 пользователей одним bulk-запросом. Как правильно спроектировать endpoint?
>
> - [ ] **Синхронный `POST /users/batch` с массивом 100 000 элементов в теле, ждать 200 OK с результатами**
>   - 100k записей займут минуты — HTTP-клиенты и прокси (nginx default 60s, ELB 60s) отрубят соединение по таймауту. Тело запроса огромное, при ошибке нужно ретраить всё. Это анти-паттерн для крупных bulk-операций
>   - ПОСЛЕДСТВИЕ: гарантированные таймауты, потеря прогресса, нет атомарности и observability
>
> - [ ] **POST с массивом, при ошибке любого элемента откатить всё (атомарная транзакция)**
>   - Атомарность 100k вставок в одной транзакции рушит производительность БД (огромный transaction log, блокировки), один битый элемент роняет весь импорт. И всё ещё в синхронном режиме — таймауты остаются
>   - ПОСЛЕДСТВИЕ: импорт никогда не дойдёт до конца, БД-локи блокируют другие операции
>
> - [x] **Async-паттерн: `POST /imports` → 202 Accepted + Location, worker обрабатывает, клиент опрашивает `/imports/{jobId}`**
>   - Клиент получает `202 Accepted` сразу с `Location: /api/v1/imports/job-123`, задача попадает в очередь, worker обрабатывает в фоне. Клиент опрашивает статус (`status`, `progress`, `total`) или подписывается через webhook/SSE. Партиал-fail обрабатывается отдельно, есть retry на уровне worker-а
>   - МЕХАНИЗМ: HTTP 202 + ресурс job-а + polling/webhook на завершение
>   - КОГДА: bulk-операция занимает больше нескольких секунд, нужны прогресс и retry
>   - ВЫГОДА: нет таймаутов, видим прогресс, partial-result, изолированная нагрузка на БД
>
> - [ ] **`DELETE /users?status=inactive` для bulk-удаления неактивных — также передавать 100k IDs в query**
>   - Query-string ограничен (обычно 8 KB на nginx default), 100k IDs туда не влезут. Bulk-delete с фильтром работает только для предикатов (status, created_before), не для произвольного списка
>   - ПОСЛЕДСТВИЕ: 414 URI Too Long или отрезанные запросы, потеря данных
...
> [!mcq] Генерация годового отчёта занимает 5 минут. Какой HTTP-статус возвращать на запрос?
>
> - [ ] **`200 OK` — держать соединение открытым, по готовности вернуть отчёт**
>   - Synchronous подход на 5 минут гарантирует таймауты на любом промежуточном прокси (nginx default 60s, AWS ALB 60s, CDN). HTTP-клиенты ретраят при timeout — сервер тратит ресурсы дважды. Любой сбой клиента — потеря работы
>   - ПОСЛЕДСТВИЕ: 504 Gateway Timeout, повторная генерация, потеря ресурсов
>
> - [x] **`202 Accepted` с `Location: /reports/jobs/abc-123` — клиент потом опрашивает статус задачи**
>   - `202 Accepted` стандартно означает "запрос принят, обрабатывается, готового результата ещё нет". `Location` указывает на ресурс job-а, клиент опрашивает `GET /reports/jobs/abc-123` и получает `{status, progress, resultUrl}`. По готовности `resultUrl` ведёт на сам отчёт. Это паттерн Async Request-Reply
>   - МЕХАНИЗМ: 202 + job-ресурс + polling/webhook/SSE для уведомления о готовности
>   - КОГДА: операция дольше нескольких секунд (отчёты, ML-инференс, обработка файлов)
>   - АЛЬТЕРНАТИВА: webhook (сервер пушит callback) или SSE (push прогресса) уменьшают polling-нагрузку
>
> - [ ] **`102 Processing` каждые 30 секунд, чтобы держать соединение живым**
>   - `102 Processing` (RFC 2518, WebDAV) почти никем не реализован — большинство прокси и клиентов его не понимают. Это не валидный inter-op подход
>   - ПОСЛЕДСТВИЕ: статус игнорируется прокси, таймаут всё равно срабатывает
>
> - [ ] **`503 Service Unavailable` с `Retry-After` — пусть клиент повторит через 5 минут**
>   - `503` означает "сервис недоступен", не "работаю над запросом". Клиент будет ретраить полный запрос, генерация запустится снова и снова. Это смешивает семантику availability и async-обработки
>   - ПОСЛЕДСТВИЕ: дублирование тяжёлой работы, искажение SLO availability, путаница мониторинга
...
> [!mcq] Senior-вопрос: «как вы спроектируете API для нового продукта». Что отличает сильный ответ от среднего?
>
> - [ ] **Перечислить все REST best practices: nouns в URI, правильные методы и статусы, HATEOAS**
>   - Это уровень middle: «знаю учебник». Не упомянуты trade-offs, контекст нагрузки, операционные ограничения. Senior отличается тем, что вяжет решение к бизнес-контексту, а не цитирует Fielding
>   - ПОСЛЕДСТВИЕ: интервьюер видит "теорию без практики", раунд оценен как middle
>
> - [ ] **Сразу нарисовать диаграмму микросервисов с Kafka, gRPC и WebSocket — показать масштаб**
>   - Преждевременная сложность без контекста — красный флаг. Senior сначала уточняет требования (RPS, SLO, multi-tenant?), потом выбирает протоколы. Прыжок к "вот моя архитектура" говорит "я хочу применить любимый стек"
>   - ПОСЛЕДСТВИЕ: впечатление over-engineering и resume-driven development
>
> - [x] **Контекст → решение → trade-off → результат: с привязкой к нагрузке, SLO, contracts и операционным метрикам**
>   - Сильный senior-ответ это короткая история: (1) Контекст — публичный API, 6k RPS, p99 < 250 ms, multi-tenant. (2) Решение — resource-oriented endpoints, единая error model (RFC 7807), OpenAPI с contract-тестами в CI. (3) Trade-off — для real-time оставили WebSocket, для справочников REST с CDN-кэшем. (4) Результат — снижение интеграционных дефектов, MTTR, стабилизация p99. Это сочетание корректной семантики HTTP, осознанных trade-offs и измеримого эффекта
>   - МЕХАНИЗМ: вязка решения к бизнес-метрикам и операционной реальности
>   - КОГДА: любой senior-раунд по системному дизайну
>   - ВЫГОДА: интервьюер видит зрелость и видение влияния решений на org
>
> - [ ] **Сказать "REST устарел, используйте GraphQL" и обосновать через over-fetching**
>   - Однобокий выбор без контекста — анти-senior. GraphQL хорош для гибких выборок, но добавляет N+1, сложность кэширования, security (depth limit). Senior знает когда что выбрать, а не топит за один протокол
>   - ПОСЛЕДСТВИЕ: впечатление узости и technology evangelism, не системного мышления
...
> [!mcq] Главное преимущество HTTP/3 (QUIC) над HTTP/2 для мобильных клиентов
>
> - [ ] **HTTP/3 быстрее благодаря бинарному формату и сжатию заголовков**
>   - Бинарный формат и HPACK-сжатие уже были в HTTP/2. HTTP/3 наследует эти оптимизации, но не они отличают его от HTTP/2. Главное отличие — транспорт
>   - ПОСЛЕДСТВИЕ: путаница уровней — не понимаете, зачем нужен QUIC поверх UDP
>
> - [x] **Connection migration: соединение не разрывается при переключении сети (Wi-Fi → LTE), потому что QUIC идентифицирует соединение по Connection ID, а не IP:port**
>   - В TCP соединение жёстко привязано к 4-tuple (src IP:port, dst IP:port). Смена IP при переключении сети рвёт соединение, нужен новый TCP-handshake + TLS-handshake (1–2 RTT). QUIC использует Connection ID, который переживает смену IP — соединение продолжается, что критично для мобильных. Бонусом: QUIC устраняет TCP head-of-line blocking (потеря одного пакета не блокирует все потоки) и делает 0-RTT handshake для известных серверов
>   - МЕХАНИЗМ: QUIC = UDP + встроенный TLS 1.3 + независимые streams, идентификация по Connection ID
>   - КОГДА: мобильные клиенты с нестабильной сетью, real-time API, гео-распределённый CDN
>   - ВЫГОДА: бесшовные переходы, ниже latency на нестабильных сетях, до 0-RTT возобновление сессии
>
> - [ ] **HTTP/3 работает поверх TCP, но с улучшенным сжатием заголовков и multiplexing**
>   - HTTP/3 работает поверх UDP (QUIC), а не TCP. Это фундаментальное отличие: именно TCP head-of-line blocking был причиной создания HTTP/3
>   - ПОСЛЕДСТВИЕ: на собеседовании это сразу видно как пробел в знаниях транспортного уровня
>
> - [ ] **HTTP/3 обязательно требует TLS 1.2 как HTTP/2**
>   - HTTP/3 требует именно TLS 1.3 (встроен в QUIC), не 1.2. И TLS не «опционален» как теоретически в h2c — в HTTP/3 TLS неотъемлемая часть протокола
>   - ПОСЛЕДСТВИЕ: неверная версия TLS, путаница в требованиях безопасности
...
> [!mcq] Дашборд должен показывать прогресс длинной задачи в реальном времени. SSE или WebSocket?
>
> - [ ] **WebSocket — потому что real-time = WebSocket, это всегда правильный выбор**
>   - "Real-time = WebSocket" — заученная фраза. WebSocket даёт двунаправленный канал, но если клиент только слушает прогресс — это лишний overhead: ws/wss требует отдельной обработки в инфраструктуре, не работает через многие прокси, нужно вручную делать reconnect и heartbeat
>   - ПОСЛЕДСТВИЕ: усложнение клиента и инфраструктуры без выигрыша над SSE
>
> - [x] **SSE — для одностороннего push прогресса достаточно, проще в эксплуатации**
>   - SSE — это обычный HTTP `Content-Type: text/event-stream` с открытым соединением. Браузер через `EventSource` API делает auto-reconnect, работает через все HTTP-прокси и firewall, удобно для дашбордов/прогресс-баров/уведомлений. Если клиент только слушает — это правильный выбор. Spring WebFlux: `Flux<ServerSentEvent<T>>`, Spring MVC: `SseEmitter`
>   - МЕХАНИЗМ: один long-lived HTTP-ответ с потоком event-сообщений (`data:`, `event:`, `id:`, `retry:`)
>   - КОГДА: server→client push без команд от клиента (прогресс, live-фид, уведомления)
>   - ВЫГОДА: проще WebSocket, проходит через корпоративные прокси, auto-reconnect в браузере
>
> - [ ] **Polling каждые 2 секунды через `GET /jobs/{id}` — проще всего и работает везде**
>   - Polling — fallback, не первый выбор. Создаёт лишние запросы, latency не лучше нескольких секунд, нагружает сервер пропорционально числу клиентов. SSE при минимальной разнице в сложности даёт реальный push
>   - ПОСЛЕДСТВИЕ: рост числа запросов в разы, lag-показ прогресса, плохой UX
>
> - [ ] **Long-polling — открыть запрос, держать пока нет события, повторить**
>   - Long-polling работает, но это hack-уровень: на каждое событие новый HTTP-запрос, нет нативного reconnect, сложно делать backpressure. SSE покрывает тот же use-case стандартизованным механизмом
>   - ПОСЛЕДСТВИЕ: больше connection-churn, сложнее отладка, изобретение SSE заново
...
> [!mcq] Клиент отправляет `Accept: application/json;q=1.0, application/xml;q=0.8`. Что это значит для сервера?
>
> - [ ] **Сервер обязан вернуть JSON — клиент явно просит JSON с q=1.0**
>   - Клиент описывает _предпочтения_, не требование. q=1.0 значит «максимально хочу», но если сервер не умеет JSON и умеет XML — он вернёт XML (q=0.8 ещё приемлемо). q=0 означал бы «неприемлемо»
>   - ПОСЛЕДСТВИЕ: ошибка в понимании quality factor — некорректная реализация content negotiation
>
> - [x] **Клиент предпочитает JSON, но согласен на XML; сервер выбирает лучший вариант из доступных, при невозможности возвращает 406 Not Acceptable**
>   - `q` (quality factor) от 0 до 1 — относительный приоритет. Сервер пересекает множество клиента (JSON, XML) со своим (что умеет endpoint, см. `produces` в Spring) и выбирает с максимальным q. Если пересечение пусто — `406 Not Acceptable`. В Spring MVC это делает `ContentNegotiationManager` автоматически
>   - МЕХАНИЗМ: server-driven content negotiation по `Accept` + `produces` контроллера
>   - КОГДА: один endpoint поддерживает несколько форматов (json/xml/csv), клиенты разные
>   - НЮАНС: `Accept` ≠ `Content-Type`. `Accept` — что клиент _хочет получить_, `Content-Type` — что _уже отправлено_ в теле этого сообщения
>
> - [ ] **Сервер должен вернуть оба формата в multipart-ответе**
>   - Multipart-ответ не предусмотрен content negotiation, и клиент не просит "и то и другое". Это нестандартное толкование заголовка
>   - ПОСЛЕДСТВИЕ: нестандартный ответ, клиент не сможет его распарсить через стандартные библиотеки
>
> - [ ] **Это `Content-Type` заголовок, описывает формат тела запроса**
>   - `Accept` и `Content-Type` — разные заголовки разного направления. `Content-Type` ставит отправитель сообщения, чтобы описать тело _этого_ сообщения. `Accept` ставит клиент, чтобы выразить предпочтения для тела _ответа_
>   - ПОСЛЕДСТВИЕ: путаница на собеседовании показывает пробел в фундаменте HTTP
...
> [!mcq] Два клиента одновременно редактируют профиль пользователя. Как через ETag реализовать optimistic locking?
>
> - [ ] **Сервер ставит `ETag: "v1"`, клиент при PUT передаёт `If-None-Match: "v1"` — сервер сравнивает и отвергает при несовпадении**
>   - `If-None-Match` используется для кэширования (отвечает 304, если содержимое не изменилось), не для optimistic locking. Логика "выполнять, если не совпадает" не подходит для проверки версии перед записью
>   - ПОСЛЕДСТВИЕ: неверная семантика — сервер вернёт 304 вместо защиты от lost update
>
> - [x] **Сервер ставит `ETag: "v1"`, клиент при PUT передаёт `If-Match: "v1"`. Если версия уже изменилась — сервер вернёт `412 Precondition Failed`**
>   - `If-Match` — это conditional request, означающий "выполни операцию, _только если_ текущая версия ресурса совпадает с моим ETag". Если за время между GET и PUT кто-то другой обновил ресурс, ETag на сервере уже `"v2"` — сервер возвращает `412 Precondition Failed`, клиент перечитывает свежую версию и решает конфликт. Это стандарт optimistic locking в REST без серверных транзакций
>   - МЕХАНИЗМ: GET → ETag `"v1"`; PUT с `If-Match: "v1"` → 200 (если версия не изменилась) или 412 (если изменилась)
>   - КОГДА: одновременные правки одного ресурса несколькими клиентами без распределённых блокировок
>   - ВЫГОДА: нет lost updates, нет долгих lock-ов в БД, чистая HTTP-семантика
>
> - [ ] **Использовать `Last-Modified` и `If-Unmodified-Since` — это надёжнее ETag**
>   - `If-Unmodified-Since` похож по смыслу, но точность — секунды (формат HTTP-date). Два update в одной секунде не различимы — lost update проходит. ETag даёт побайтовую точность через hash/version
>   - ПОСЛЕДСТВИЕ: race condition в пределах секунды, редкие, но дорогие баги (особенно при high-throughput)
>
> - [ ] **Хранить timestamp на клиенте, отправлять в теле PUT, сервер сверяет**
>   - Это переизобретение стандарта в теле запроса. Не интегрируется с HTTP-кэшированием, требует кастомной валидации, не работает через посредников (proxy, CDN). ETag + If-Match — стандартный путь
>   - ПОСЛЕДСТВИЕ: фрагментированная реализация, чужие клиенты не понимают конвенцию
...
> [!mcq] Публичный API будет интегрировать сотни внешних партнёров. Какую стратегию versioning выбрать?
>
> - [x] **URI versioning: `GET /api/v1/users` — понятнее всего интеграторам, видно в логах, легко тестировать**
>   - URI versioning (`/api/v1/`, `/api/v2/`) — pragmatic выбор для публичного API. Партнёры видят версию в URL, могут тестировать в браузере, легко прокликать в Postman, CDN/cache работают из коробки без `Vary`. Чистый REST-purist выскажется за content-type versioning, но индустрия (Stripe, GitHub, Twilio) использует URI — потому что DX важнее теоретической чистоты
>   - МЕХАНИЗМ: путь URL содержит major-версию, разные контроллеры для разных версий
>   - КОГДА: публичный API с внешними партнёрами, документация и SDK на несколько версий
>   - ВЫГОДА: понятный URL, простые SDK, CDN-friendly, видно в логах/мониторинге
>
> - [ ] **Header versioning: `Api-Version: 2` в заголовке — чистые URI**
>   - Header versioning формально чище (URI идентифицирует ресурс, не версию), но для публичного API: партнёры забывают заголовок, не видят версию в браузерных тестах, кастомные заголовки часто игнорируются CDN. Норм для внутренних сервисов, плох для публичного
>   - ПОСЛЕДСТВИЕ: рост support-нагрузки от партнёров ("почему у меня ошибка?"), сложность отладки
>
> - [ ] **Content-Type versioning: `Accept: application/vnd.myapp.v2+json` — самый правильный REST**
>   - Это академически чистый подход (так делает GitHub v3 API), но на практике добавляет когнитивную нагрузку партнёрам, требует понимания vendor media types. CDN-дружелюбно при правильном `Vary: Accept`, но мало кто реализует это корректно
>   - ПОСЛЕДСТВИЕ: высокий порог входа для интеграторов, сложность для команд без зрелой API-культуры
>
> - [ ] **Без версионирования — гарантировать backward compatibility на все времена**
>   - Заманчиво, но нереалистично: рано или поздно нужны breaking changes (новая модель security, удаление устаревших полей). Без явного versioning либо вы заморожены в legacy, либо ломаете партнёров без предупреждения
>   - ПОСЛЕДСТВИЕ: технический долг растёт лавинообразно, либо массовые поломки интеграций
...
> [!mcq] DELETE /users/42 первый раз вернул 200 OK, повторный — 404 Not Found. DELETE всё ещё идемпотентен?
>
> - [ ] **Нет, потому что статусы разные — идемпотентность требует одинаковых статусов**
>   - Идемпотентность определяется по _состоянию системы_, а не по статус-коду. RFC 7231 явно говорит: «An idempotent method is one for which the effect on the server of multiple identical requests is the same as for a single request» — речь о side-effects, не о response
>   - ПОСЛЕДСТВИЕ: ложное понимание стандарта — пытаетесь "исправить" 404 на 200, нарушая HTTP-семантику
>
> - [x] **Да: оба раза состояние сервера одинаково — пользователь 42 удалён. Разные статусы лишь отражают current state, а не нарушают идемпотентность**
>   - DELETE идемпотентен по определению: повторный вызов не меняет состояние (ресурс уже удалён). Первый раз 200 OK (ресурс был и удалён сейчас), повторный — 404 Not Found (ресурса больше нет). Состояние одинаково — отсутствие пользователя. Это и есть идемпотентность: безопасно ретраить клиенту при timeout, не боясь дубль-удаления
>   - МЕХАНИЗМ: идемпотентность = state convergence, статус отражает текущий state
>   - КОГДА: ретраи DELETE при сетевых сбоях безопасны — повтор не нанесёт дополнительного урона
>   - НЮАНС: 204 No Content вместо 200 для DELETE тоже валидно — спецификация не требует конкретного 2xx
>
> - [ ] **DELETE неидемпотентен, потому что после первого вызова ресурс изменён**
>   - Это путаница с safe (безопасность). DELETE не safe (изменяет ресурс), но идемпотентен. Безопасность и идемпотентность — два разных свойства, их часто путают
>   - ПОСЛЕДСТВИЕ: в архитектуре вы откажетесь от ретраев DELETE и потеряете надёжность сетевого слоя
>
> - [ ] **POST идемпотентен, как и DELETE — оба создают/удаляют один ресурс**
>   - POST по умолчанию НЕ идемпотентен — он создаёт новый ресурс каждый вызов (два POST = две сущности). Чтобы сделать POST идемпотентным, нужен механизм `Idempotency-Key` (Stripe-pattern) — сервер кэширует результат по ключу
>   - ПОСЛЕДСТВИЕ: ретраи POST без Idempotency-Key создают дубли (двойные платежи, дублированные заказы)
...
> [!mcq] Какой Content-Type должен ставить сервер на ошибку, оформленную по RFC 7807 Problem Details?
>
> - [ ] **`application/json` — это же JSON, какая разница**
>   - Технически клиент сможет распарсить как JSON, но `application/json` сигнализирует «обычный JSON-ответ», а не структурированную ошибку. Клиентские библиотеки (`spring-cloud-openfeign`, axios interceptors) не активируют логику обработки problem-details
>   - ПОСЛЕДСТВИЕ: клиенты не различают error-response и success-response по типу, теряется auto-handling
>
> - [x] **`application/problem+json` — отдельный media type, описанный в RFC 7807, сигнализирует структурированную ошибку**
>   - RFC 7807/9457 определяет `application/problem+json` (и `application/problem+xml` для XML). Этот media type сразу говорит клиенту: «это структурированная ошибка с полями type/title/status/detail/instance + расширения». Spring Boot 3 при `spring.mvc.problemdetails.enabled=true` автоматически ставит этот Content-Type для 4xx/5xx. Клиентские библиотеки активируют свой error-mapping
>   - МЕХАНИЗМ: vendor-style media type `+json` суффикс, RFC 6839 (structured suffix)
>   - КОГДА: любая ошибка REST API, особенно публичный API и enterprise-интеграции
>   - ВЫГОДА: единый стандарт для клиентов, авто-парсинг в SDK, чёткая граница success vs error
>
> - [ ] **`text/plain` — ошибки достаточно отдать строкой**
>   - Plain text теряет всю структуру (type, status, detail, instance, расширения). Клиенту приходится парсить regex-ом, нет машинно-читаемого type, нет ссылки на документацию. Это уровень API maturity нулевых
>   - ПОСЛЕДСТВИЕ: клиенты пишут хрупкие regex-парсеры, integration-баги, нет i18n
>
> - [ ] **`application/vnd.error+json` — vendor-specific формат ошибок**
>   - Существуют vendor-форматы (JSON:API errors, vnd.error), но они конкурируют с RFC 7807. После выхода стандарта 2016 и встроенной поддержки в Spring 6 — нет причины городить свой формат
>   - ПОСЛЕДСТВИЕ: фрагментация, клиенты пишут адаптеры под каждый сервис
