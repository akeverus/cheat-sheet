...
- [Q1. (!) Какие функциональные и нефункциональные требования у сервиса?](#q1--какие-функциональные-и-нефункциональные-требования-у-сервиса)
- [Q2. (!) Сколько нужно storage и какой ожидается QPS (capacity estimation)?](#q2--сколько-нужно-storage-и-какой-ожидается-qps-capacity-estimation)
- [Q3. (!) Какие нужны API endpoints и как выглядит flow запроса?](#q3--какие-нужны-api-endpoints-и-как-выглядит-flow-запроса)

**Encoding и generation**
- [Q4. (!) Как генерировать short code (hash, counter, random)?](#q4--как-генерировать-short-code-hash-counter-random)
- [Q5. (!) Почему Base62, а не Base64?](#q5--почему-base62-а-не-base64)
- [Q6. Как избежать коллизий кодов?](#q6-как-избежать-коллизий-кодов)
- [Q19. (!) Как сделать распределённый счётчик (Snowflake, ZooKeeper key ranges)?](#q19--как-сделать-распределённый-счётчик-snowflake-zookeeper-key-ranges)

**HTTP redirect**
- [Q18. (!) HTTP 301 vs 302 vs 307 — как это влияет на аналитику?](#q18--http-301-vs-302-vs-307--как-это-влияет-на-аналитику)

**Storage и sharding**
- [Q7. (!) Как спроектировать схему БД?](#q7--как-спроектировать-схему-бд)
- [Q8. SQL или NoSQL — что выбрать?](#q8-sql-или-nosql--что-выбрать)
- [Q11. (!) Как шардировать данные?](#q11--как-шардировать-данные)
- [Q25. Как мигрировать и решардить без downtime?](#q25-как-мигрировать-и-решардить-без-downtime)

**Scalability и cache**
- [Q9. (!) Как масштабировать чтения (cache, CDN)?](#q9--как-масштабировать-чтения-cache-cdn)
- [Q10. (!) Какую стратегию кеширования выбрать (write-through или cache-aside)?](#q10--какую-стратегию-кеширования-выбрать-write-through-или-cache-aside)
- [Q27. Как обрабатывать горячий ключ (viral URL)?](#q27-как-обрабатывать-горячий-ключ-viral-url)
- [Q28. Как бороться с cache stampede на популярном коде?](#q28-как-бороться-с-cache-stampede-на-популярном-коде)

**Features**
- [Q12. Как реализовать custom aliases?](#q12-как-реализовать-custom-aliases)
- [Q13. Как реализовать TTL и истечение ссылок?](#q13-как-реализовать-ttl-и-истечение-ссылок)
- [Q14. (!) Как устроить pipeline аналитики кликов?](#q14--как-устроить-pipeline-аналитики-кликов)
- [Q21. Как поддержать кастомные домены (white-label `brand.com`)?](#q21-как-поддержать-кастомные-домены-white-label-brandcom)
- [Q22. Как сделать bulk-API с батчами и идемпотентностью?](#q22-как-сделать-bulk-api-с-батчами-и-идемпотентностью)

**Production**
- [Q15. (!) Как обеспечить надёжность и устранить SPOF?](#q15--как-обеспечить-надёжность-и-устранить-spof)
- [Q16. Как защититься от спама и фишинга?](#q16-как-защититься-от-спама-и-фишинга)
- [Q17. Как сделать rate limiting?](#q17-как-сделать-rate-limiting)
- [Q20. (!) Как выглядит высокоуровневая архитектура (CDN → LB → API → Redis → DB)?](#q20--как-выглядит-высокоуровневая-архитектура-cdn--lb--api--redis--db)
- [Q23. Как детектить ботов (one-time токены, эскалация до CAPTCHA)?](#q23-как-детектить-ботов-one-time-токены-эскалация-до-captcha)
- [Q24. (!) Как сделать гео-распределённую систему (multi-region DNS, edge reads)?](#q24--как-сделать-гео-распределённую-систему-multi-region-dns-edge-reads)
- [Q26. (!) Какие метрики мониторинга обязательны?](#q26--какие-метрики-мониторинга-обязательны)
- [Q29. Как отделить трафик ботов от реальных редиректов в аналитике?](#q29-как-отделить-трафик-ботов-от-реальных-редиректов-в-аналитике)
- [Q30. (!) Какие антипаттерны и подводные камни тут типичны?](#q30--какие-антипаттерны-и-подводные-камни-тут-типичны)

## Q1. (!) Какие функциональные и нефункциональные требования у сервиса?

Суть: сервис должен делать ровно две вещи — превращать длинный URL в короткий код и редиректить обратно. Всё остальное (alias, TTL, аналитика) — опциональные расширения, а не ядро. Главное на этом этапе — отделить «что система делает» (функциональные требования) от «как хорошо она это делает» (нефункциональные: latency, доступность, масштаб) и явно сузить scope, чтобы уложиться в 45 минут.
...
**Совет на собеседовании:** уточняйте scope у интервьюера в самом начале — он сильно влияет на дизайн. Например, ответ «нужны ли аккаунты пользователей?» определяет, добавлять ли auth-слой и шардирование по `user_id`.

## Q2. (!) Сколько нужно storage и какой ожидается QPS (capacity estimation)?

Суть прикидки: система не гигантская по объёму данных (единицы терабайт), но read-heavy — читают в ~100 раз чаще, чем пишут. Поэтому узкое место не storage, а latency чтений, и весь дизайн крутится вокруг кеширования. Считаем по шагам: сначала writes из бизнес-цифры, затем reads через отношение read:write, затем storage и память под кеш.
...
**Вывод:** ~3-5 TB данных и десятки тысяч reads/sec на пике — нагрузка средняя, но именно read latency диктует архитектуру. Отсюда многослойный кеш (CDN → Redis → DB) и read-реплики в следующих вопросах.

## Q3. (!) Какие нужны API endpoints и как выглядит flow запроса?

API сводится к двум основным операциям: создать ссылку (`POST /shorten`, write-путь) и пройти по ней (`GET /{short_code}`, read-путь, на который приходится 99% трафика). Ключевая идея — read-путь должен быть максимально лёгким (lookup + редирект), а вся «тяжёлая» логика (валидация, скан, генерация кода) живёт на write-пути.
...
## Q4. (!) Как генерировать short code (hash, counter, random)?

Короткий ответ: есть три подхода, и выбор — это компромисс между предсказуемостью, координацией и коллизиями. Hash детерминирован (даёт бесплатную дедупликацию), но ловит коллизии. Counter не коллизит, но создаёт перечислимые URL и нуждается в распределённости. Random непредсказуем и не требует координации, но коллизии нужно обрабатывать через retry. На практике побеждает гибрид — заранее сгенерированный пул случайных кодов, который снимает работу по коллизиям с онлайн-пути.
...
Коротко: Base64 содержит символы `+` и `/`, которые не URL-safe — их пришлось бы экранировать (`%2B`, `%2F`), и короткая ссылка стала бы уродливой и хрупкой. Base62 (`[a-zA-Z0-9]`) обходится без спецсимволов, поэтому код всегда чистый, копируется двойным кликом и читается без сюрпризов. Платим за это лишь чуть меньшим алфавитом — на длине 7 символов разница в ёмкости несущественна.
...
## Q6. Как избежать коллизий кодов?

Главная страховка от коллизий — UNIQUE constraint в БД плюс retry: даже если два запроса сгенерируют один код, БД примет только первый, второй получит ошибку и сгенерирует новый. Полностью избежать коллизий нельзя при hash/random-генерации (на миллиардах записей birthday paradox гарантирует совпадения), поэтому задача — не «не допустить», а «дёшево обработать». Лучший способ убрать коллизии с критического пути вообще — заранее сгенерированный пул уникальных кодов.
...
## Q7. (!) Как спроектировать схему БД?

Ключевое решение схемы — разделить данные по паттерну доступа. Основная таблица `urls` оптимизирована под точечный lookup по `short_code` (он же PRIMARY KEY), а высокочастотные клики выносятся в отдельную партиционированную таблицу `clicks`, чтобы не раздувать горячую таблицу редиректов. Счётчик кликов денормализован и обновляется батчем — джойнить `clicks` на каждый редирект недопустимо.
...
## Q8. SQL или NoSQL — что выбрать?

Короткий ответ: для масштаба предпочтительнее key-value NoSQL, потому что основной паттерн доступа — это `short_code → long_url`, ровно то, под что KV-хранилища оптимизированы (p99 ~10 ms на любом объёме, неограниченное горизонтальное масштабирование). SQL тоже работает и удобнее для аналитических джойнов, но упирается в потолок вертикального масштабирования. На малом масштабе оба варианта годятся — отвечать «зависит от масштаба» и обосновать выбор цифрами.
...
## Q9. (!) Как масштабировать чтения (cache, CDN)?
...
## Q10. (!) Какую стратегию кеширования выбрать (write-through или cache-aside)?

Для URL shortener правильный выбор — cache-aside. Причина в том, что ~90% созданных ссылок (long tail) вообще никто не откроет, поэтому класть в кеш всё при записи (write-through) — значит впустую тратить память Redis. Cache-aside наполняет кеш лениво, только реально читаемыми кодами. Write-behind (асинхронная запись в БД) здесь опасен: пользователь отправляет ссылку другу сразу после shorten, и падение Redis до flush обернётся видимым 404.
...
## Q11. (!) Как шардировать данные?

Шардировать нужно по `short_code` (основной паттерн доступа — редирект), а конкретно — через consistent hashing с virtual nodes. Это даёт две вещи: равномерное распределение и дешёвый resharding (добавление shard двигает лишь 1/N ключей вместо 100%, как у `mod N`). Шардирование по `user_id` соблазнительно для «Моих ссылок», но ломает редирект — поиск по коду превращается в scatter-gather по всем shards.
...
## Q12. Как реализовать custom aliases?

Custom alias — это когда пользователь сам задаёт код (`/myalias` вместо случайного `/abc1234`). Реализация сводится к атомарному «занять, если свободно»: UNIQUE constraint на `short_code` плюс `INSERT ... ON CONFLICT DO NOTHING`. Движок БД сам разруливает гонку двух запросов за один alias — внешний distributed lock тут избыточен. Остальное — это правила вокруг: резерв системных имён, нормализация регистра, лимиты длины и защита товарных знаков.
...
**Почему UNIQUE constraint достаточно:** он даёт атомарный check-and-insert — две параллельные транзакции не могут обе занять `promo2026`, проигравшая получит 409. Тянуть сюда distributed lock (Redis SETNX) — over-engineering: БД и так гарантирует атомарность одной строки.
...
## Q13. Как реализовать TTL и истечение ссылок?

Истечение разделяют на два уровня. **Логическое** — на чтении проверяем `expires_at` и отдаём 410 Gone, не удаляя строку; это даёт мгновенный эффект и возможность восстановления. **Физическое** удаление идёт фоном (TTL-атрибут DynamoDB или cron-зачистка батчами в окно низкого трафика). Между ними оставляют grace-окно (7-30 дней) — страховка от случайного истечения и под legal hold.
...
**Граничный случай:** TTL в DynamoDB — eventual (задержка до 48 часов); для отзыва, критичного по времени, нужна дополнительная проверка на уровне приложения через флаг `deleted_at`.

## Q14. (!) Как устроить pipeline аналитики кликов?

Главный принцип: путь редиректа остаётся read-only, а сбор аналитики — полностью асинхронный. Вместо синхронного `UPDATE click_count` (он убивает latency row lock'ом и WAL fsync на горячем URL) сервис после `302` просто публикует событие в Kafka по схеме fire-and-forget. Дальше отдельный pipeline (Kafka → Flink → ClickHouse) агрегирует клики, а в основную БД счётчик прилетает батчем раз в минуту. **Цель** — выдержать 40K+ редиректов/sec, не задев p99 redirect latency.
...
## Q15. (!) Как обеспечить надёжность и устранить SPOF?

Суть: ни один компонент не должен быть единственным (SPOF) — каждый слой дублируется, а приложение деградирует постепенно, а не падает. Ориентир — **SLA 99.99%**, то есть максимум 52 минуты downtime в год; этот бюджет так мал, что ручной failover в него не укладывается, отсюда требование автоматики и multi-region.
...
## Q16. Как защититься от спама и фишинга?

Фундаментальная проблема shortener'а: он прячет реальный destination за доверенным доменом (`bit.ly/...`), и этим пользуются для фишинга. Защита строится по принципу defense in depth — ни один слой не надёжен в одиночку, поэтому их складывают: скан при создании, rate limiting, промежуточная страница-предупреждение, периодический пере-скан (сайт могут взломать уже после создания ссылки) и жалобы пользователей.
...
## Q17. Как сделать rate limiting?

Rate limiting защищает сразу от трёх вещей: злоупотреблений (массовое создание спам-ссылок), DoS и неконтролируемых затрат. Применяют его на нескольких уровнях (per-IP, per-user, per-API-key) через token bucket в Redis с Lua-скриптом — Lua делает проверку-и-списание атомарной, что критично в распределённой среде. Ключевое: счётчик обязан быть общим для всех инстансов, иначе лимит обходится умножением на число серверов.
...
## Q18. (!) HTTP 301 vs 302 vs 307 — как это влияет на аналитику?

Главный нюанс: 301 (permanent) браузер кеширует, поэтому последующие клики до сервера не доходят — аналитика ломается, и отозвать ссылку нельзя (устаревший 301 живёт в кешах браузеров месяцами). 302 (temporary) не кешируется по умолчанию, каждый клик попадает на сервер — аналитика работает, отзыв срабатывает мгновенно. Поэтому для shortener'а дефолт — **302**: разменивают лишнее обращение к серверу на возможность считать клики и блокировать вредоносные ссылки. 307/308 отличаются лишь сохранением HTTP-метода и для редиректов «только GET» нерелевантны.
...
## Q19. (!) Как сделать распределённый счётчик (Snowflake, ZooKeeper key ranges)?

Идея распределённого счётчика — раздать каждому хосту собственный диапазон ID, чтобы убрать общую точку синхронизации. Глобальный auto-increment — это и contention (один счётчик ограничивает throughput), и SPOF. Решают это либо встраиванием machine_id прямо в ID (Snowflake), либо выдачей хостам непересекающихся диапазонов (ZooKeeper / Redis key ranges), внутри которых хост считает локально без координации.
...
## Q20. (!) Как выглядит высокоуровневая архитектура (CDN → LB → API → Redis → DB)?
...
## Q21. Как поддержать кастомные домены (white-label `brand.com`)?

Суть white-label: клиент хочет, чтобы ссылки выглядели как `customer.brand.com/abc`, а не `bit.ly/abc`. Технически это три задачи — направить чужой домен на нашу инфраструктуру (CNAME + верификация владения через TXT-запись), терминировать его TLS (свой сертификат клиента или автовыпуск через Let's Encrypt) и при lookup учитывать `domain_id` (один и тот же `short_code` у разных доменов означает разные ссылки, отсюда составной ключ `(domain_id, short_code)`).
...
## Q22. Как сделать bulk-API с батчами и идемпотентностью?

Bulk-API нужен, когда кампания создаёт сразу 10K+ ссылок одной операцией. Две ключевые проблемы здесь — идемпотентность и таймауты. Идемпотентность делают двухуровневой: `Idempotency-Key` на весь батч (повтор вернёт закешированный результат) и `client_ref` на каждый элемент (повтор не создаст дублей). Очень большие батчи (> 100K) нельзя обрабатывать синхронно — 10K вызовов Safe Browsing не уложатся в HTTP-timeout, поэтому переходят на async job-паттерн с `202 Accepted` и опросом статуса.
...
## Q23. Как детектить ботов (one-time токены, эскалация до CAPTCHA)?

Ключевая идея — не блокировать всех подряд, а эскалировать трение по мере роста подозрительности. Обычный трафик пропускаем без задержек; при превышении порога создаём challenge (CAPTCHA); при повторных провалах — временный, затем постоянный бан. Сигналы для подозрительности собирают из нескольких источников: rate по IP, fingerprint устройства, поведение (время до отправки, движение мыши), одноразовые CSRF-токены и honeypot-поля. Аутентифицированные пользователи и API-клиенты проходят без CAPTCHA — auth сам по себе устойчив к ботам.
...
## Q24. (!) Как сделать гео-распределённую систему (multi-region DNS, edge reads)?

Подход — active-active в нескольких регионах: каждый регион держит полный стек, а пользователя направляют в ближайший через latency-based DNS (Route 53). Это работает, потому что редиректы read-only и терпят eventual consistency — read-реплику можно держать в каждом регионе (DynamoDB Global Tables), и кросс-региональная задержка репликации в 1-5 sec не критична. Записи сложнее: их закрепляют за домашним регионом клиента. **Цель** — redirect latency < 50 ms из любой географии плюс disaster recovery при отказе целого региона.
...
## Q25. Как мигрировать и решардить без downtime?

Resharding без простоя строится вокруг dual-write: пока чтения идут из старых shards (source of truth), приложение параллельно пишет и в новые, а фоновый backfill копирует существующие данные. Когда счётчики строк сошлись — переключают чтения на новые shards, продолжая dual-write ради возможности отката, и лишь после нескольких дней стабильности убирают старые. Главное условие дешевизны миграции — consistent hashing (двигается 1/N ключей); `mod N` означал бы перемещение 100% данных и stop-the-world, что несовместимо с SLA 99.99%.
...
**Граничный случай:** транзакции в полёте во время переключения — дренируем соединения, завершаем по timeout.

## Q26. (!) Какие метрики мониторинга обязательны?

Минимальный набор закрывает четыре аспекта здоровья сервиса: latency редиректа (главный пользовательский SLI, p99 < 50 ms), эффективность кеша (cache hit ratio по слоям — падение Redis ниже 90% сразу даёт спайк нагрузки на БД), признаки злоупотреблений (rate-limit 429, блокировки Safe Browsing) и состояние асинхронных частей (lag analytics-pipeline, состояние circuit breaker). Дополняют это распределённый трейсинг (OpenTelemetry с trace ID через Kafka) и структурированные логи с маскированием PII.
...
## Q27. Как обрабатывать горячий ключ (viral URL)?

Проблема горячего ключа в том, что один популярный код = одна партиция/один shard, который не размазать обычным шардированием (один `short_code` всегда хешируется в одно место). Поэтому борются не распределением, а реплицированием кеша как можно ближе к пользователю: CDN edge и in-process кеш на каждом поде поглощают миллионы чтений, до origin доходят лишь miss'ы. **Проблема** — viral URL даёт 100K+ редиректов/sec на одну партицию.
...
## Q28. Как бороться с cache stampede на популярном коде?

Cache stampede (thundering herd) возникает, когда популярный код одновременно истекает в Redis: миллион параллельных запросов промахиваются и разом бьют в БД. Решают это двумя классами приёмов — не дать всем промахнуться одновременно (вероятностный ранний refresh XFetch, stale-while-revalidate) и не дать всем промахам дойти до БД (single-flight: один процесс под mutex рефрешит, остальные ждут). Плюс negative cache на 404, чтобы scanning-атака не усиливала нагрузку.
...
## Q29. Как отделить трафик ботов от реальных редиректов в аналитике?

Решение — классифицировать каждый клик при приёме и хранить два представления: **raw clicks** (всё подряд, для мониторинга затрат и безопасности) и **human clicks** (отфильтрованные, то, что показываем клиенту). Без этого аналитика врёт: до 50% редиректов дают боты — краулеры link-preview, сканеры, скраперы. Бота вычисляют по комбинации сигналов: User-Agent (`Twitterbot`, `facebookexternalhit`), поведение (нет загрузки страницы на destination, интервалы < 1 sec), класс IP (датацентр vs резидентный) и TLS-фингерпринт JA3/JA4.
...
**Граничный случай:** превьюшники ссылок (краулер Twitter Card) полезны (показ preview раскручивает CTR), но не считаются human clicks; нужно включать в счётчики с пометкой.

## Q30. (!) Какие антипаттерны и подводные камни тут типичны?

Большинство ошибок в этом дизайне сводятся к трём темам: неверная генерация кода (UUID, глобальный счётчик), синхронные операции на горячем read-пути (UPDATE счётчика, скан без timeout) и неправильное кеширование (write-through на всё, LRU вместо LFU, единственный Redis). Ниже — чек-лист с правильной альтернативой к каждому.
