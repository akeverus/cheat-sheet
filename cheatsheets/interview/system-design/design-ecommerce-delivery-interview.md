...
**Что показать на интервью:** промис считается по **графу маршрутов с расписанием**, а не по геометрическому расстоянию. СДЭК и Boxberry строят сеть так же; узкое место сети — пропускная способность СЦ (отправлений/час) и число направлений сортировки.
...
- **Срок хранения + автовозврат.** Посылка хранится ограниченно (Boxberry 7+7 дней, постамат 3+2). Невостребованное автоматически уезжает обратно — и это **отдельное состояние в state machine**, а не потеря SLA.
