...
- 10k QPS в пике
- CPU на запрос ~10 ms на одном шарде
- 10 шардов работают параллельно, давая ~100 запросов на shard-секунду
- Нужно ~10 нод, чтобы переварить 10k QPS
...
«Хороший» поиск надо уметь измерять. Метрики делятся на офлайновые (по размеченным данным — recall, MRR, NDCG), продуктовые (по кликам) и инженерные (latency). Хорошие метрики ловят не «нашли ли вообще», а «насколько высоко стоит релевантное».
...
**Precision@k:**
- Доля найденных релевантных от всех релевантных в корпусе.
