...
Каждый пункт — это деньги или сэкономленное время инцидента:

- **MTTR и частота инцидентов** снижаются, потому что слабые места находят заранее, а не в 3 часа ночи в проде.
- **Resilience-паттерны проверены делом**: circuit breaker, retry, bulkhead ([resilience-паттерны](../architecture/resilience-patterns-interview.md)) перестают быть «надеждой в коде» и становятся доказанным поведением.
- **Команда тренируется** реагировать на инциденты в безопасной обстановке, а не учится под огнём реального сбоя.
- **Runbooks и алертинг** проверяются: сработает ли алерт, корректен ли runbook — выясняется до того, как это понадобится по-настоящему.

**На собеседовании.** Чётко противопоставляйте две вещи: тестирование ищет баги в известных сценариях, chaos engineering строит уверенность в поведении системы при неизвестных сбоях.

## Q2. (!) Какие 5 принципов `Chaos Engineering`?

Канонический список с [principlesofchaos.org](https://principlesofchaos.org/). Логика порядка: сначала определяем, что считать «нормой», затем нарушаем её реалистично, делаем это там, где живут пользователи, повторяем непрерывно — и всё это под контролем масштаба воздействия.

1. **Build a Hypothesis around Steady State Behavior** — формулируем гипотезу о нормальном поведении через измеримые метрики (throughput, error rate, latency p99), а не через внутренние детали реализации. Метрика на выходе системы, а не на её внутренностях, потому что пользователю важен результат, а не как он получен.
2. **Vary Real-world Events** — инжектируем реальные события: crash инстансов, сеть, DNS, всплески трафика. Приоритизируем по частоте и потенциальному импакту — нет смысла моделировать падение метеорита, пока не проверены частые отказы.
3. **Run Experiments in Production** — только прод даёт настоящие условия: трафик, конфигурации, данные. Staging не воспроизводит все особенности, поэтому уверенность, добытая на staging, неполна.
4. **Automate Experiments to Run Continuously** — одноразовый ручной эксперимент даёт одноразовую уверенность. Система меняется каждый день, поэтому проверять её надо непрерывно: автоматизация встраивает chaos в CI/CD.
5. **Minimize Blast Radius** — эксперимент должен затрагивать минимальную часть пользователей; обязателен kill switch для немедленной остановки. Это контракт безопасности: учиться, не ломая прод по-настоящему.
...
Логика здесь научная: мы пытаемся **опровергнуть** гипотезу, а не подтвердить. Если steady state не сохранился — нашли слабое место (это ценно). Если сохранился — повысили уверенность. Чем труднее нарушить steady state инъекцией отказа, тем резилентнее система.

### Антипаттерн

Гипотеза «приложение не упадёт» бесполезна — слишком расплывчата, её нельзя ни подтвердить, ни опровергнуть. Нужны конкретные числа и временные окна. Без измеримого steady state нельзя отличить успешный эксперимент от провального — а значит, эксперимент бессмыслен.
...
- **Начинать с малого** (start small): сначала 1 pod, потом namespace, потом кластер. Расширять охват только после того, как предыдущий шаг прошёл чисто.
- **Kill switch**: возможность немедленной остановки (`chaosctl stop`, удаление CR). Без него вы не управляете экспериментом, а наблюдаете за ним.
- **Тайм-бокс** (time box): эксперимент длится минуты, не часы — короткое окно ограничивает ущерб даже при неожиданном поведении.
- **Feature flag**: завернуть chaos-инъекцию в флаг, чтобы включать и выключать её без передеплоя.
- **Canary**: направлять хаос на отдельную группу пользователей ([canary deployment](../cicd/deployment-strategies-interview.md)) — остальные не почувствуют эксперимент.
...
**Fault Injection** — это набор техник внесения сбоев, которые Chaos Engineering использует, но без обязательного формализма (гипотеза, steady state, blast radius).

**Суть различия.** Chaos Engineering — это **процесс и дисциплина** (научный метод применительно к надёжности), а fault injection — лишь **инструмент** внутри него. Можно вносить сбой в unit-тесте ([unit-тесты](unit-testing-interview.md)) — это fault injection, но не chaos engineering, потому что нет гипотезы о steady state и нет реальной среды.
...
Часть реального трафика зеркалится на экспериментальный путь, но его ответы пользователю не возвращаются. Так вы получаете реалистичную нагрузку почти как в проде, но без риска для пользователей.

**Подводный камень.** Утверждение «у нас staging как прод» почти всегда ложно: отличаются данные, конфигурации, объёмы, поведение зависимостей. Правильная позиция — «начинаем со staging, постепенно переходим в прод с малым blast radius».
...
Постоянная угроза падения инстанса заставляет писать код по правилам, которые иначе откладывают «на потом»:

1. Инстансы должны быть **stateless** (или корректно восстанавливать state) — иначе с гибелью инстанса теряются данные.
2. Пулы соединений должны переоткрываться при потере — иначе после падения зависимости клиент держится за мёртвые коннекты.
3. Балансировщик должен быстро исключать мёртвый инстанс из ротации, чтобы трафик не лился в пустоту.
4. Клиенты должны делать retry с jitter — чтобы пережить пропажу инстанса, но не устроить retry storm.
...
- Таймаут больше, чем `SLA` → запросы ждут слишком долго и копятся, исчерпывая пулы потоков и соединений.
- Нет circuit breaker → медленная зависимость утягивает вызывающий сервис, отказ распространяется каскадом.
- Retry без jitter → все клиенты ретраят синхронно и добивают зависимость (retry storm).
- Нет fallback → вместо деградации фича отключается полностью (blackout).

**Ключевой инсайт.** Эксперименты с latency часто выявляют проблемы, невидимые при простом падении сервиса. Медленный сервис опаснее упавшего: упавший быстро отвергает запросы, а медленный ломает тайминги — заявки копятся, ресурсы исчерпываются, и деградация расползается дальше.
...
**Частый баг в JVM.** Настройка `networkaddress.cache.ttl=-1` кэширует DNS-ответ навсегда. После failover зависимости (её IP сменился) приложение продолжает стучаться в старый, уже мёртвый IP — и пока процесс не перезапустят, не восстановится. DNS chaos выявляет такие «вечные кэши».
...
| Read-only | Перемонтируем файловую систему как read-only |
...
**Почему это важнее остальных.** Микросервис живёт в паутине зависимостей, и каждая из них может отказать. Эксперименты с зависимостями напрямую отвечают на бизнес-вопрос: «если упадёт платёжный шлюз / кэш / брокер — пользователь увидит ошибку или мы деградируем мягко?». Подробности по circuit breaker, bulkhead, timeout — в [resilience-паттернах](../architecture/resilience-patterns-interview.md).
...
- **Управление в runtime**: включили хаос, понаблюдали за метриками, выключили — без передеплоя и рестарта.
- **Kill switch**: `POST /disable` мгновенно гасит атаку при выходе за abort condition — это и есть тот самый минимальный blast radius на практике.
- **Интеграция с оркестратором**: внешний инструмент (CI, GameDay tooling) дёргает endpoint по API и управляет экспериментом программно.
...
- **UI dashboard** для визуализации — видно, что и где сейчас ломается.
- **Workflow** — цепочки экспериментов: например, сначала latency, затем поверх неё pod-kill.
- **Schedule** — периодический chaos (тот самый 4-й принцип «автоматизировать непрерывно»).
- **RBAC и multi-tenancy** — кто и какие эксперименты вправе запускать; критично в больших организациях.

Описание экспериментов как Custom Resources даёт ещё одно преимущество: их можно хранить в Git и ревьюить как код (GitOps).
...
**Компромисс.** За production-safe инструмент с поддержкой и compliance платят лицензией. Стартапу с ограниченным бюджетом дешевле взять open-source (Chaos Mesh, Litmus) и вложиться в его эксплуатацию самостоятельно.
...
- Предупредить смежные команды и on-call
...
**Главная мысль.** GameDay тренирует не только систему, но и **команду**: проверяются runbooks, коммуникация, алерты, процесс принятия решений под давлением. Часто самые ценные находки — не в коде, а в людях и процессах: алерт не дошёл до того, кто нужен; runbook устарел; никто не знал, кто принимает решение об откате.
...
Фокус на **системе и процессах**, а не на людях. Причина: люди ошибаются всегда, а вот система должна быть устроена так, чтобы ошибка одного человека не приводила к сбою. «Алерт не сработал, потому что метрика не экспортировалась» — указывает на починку (добавить метрику и тест). «Петя забыл настроить алерт» — тупик: завтра забудет кто-то ещё, и ничего не изменится.
...
Лучше остановить эксперимент **слишком рано**, чем дотерпеть до реального инцидента: цена ложной остановки — повторный прогон позже, цена пропущенного abort — упавший прод. Цель chaos-инжиниринга — **учиться**, а не ронять прод. Парадоксально, но именно готовность в любой момент нажать стоп — признак зрелой практики: она показывает, что эксперимент управляем, а не пущен на самотёк.
...
Не каждой команде нужно дойти до Sophisticated + Cultural Expectation — для стартапа с тремя сервисами это оверкилл. Смысл модели не в том, чтобы быть «на максималках», а в **осознанном продвижении** по матрице по мере роста бизнес-потребностей: модель — это карта, а не обязательный маршрут до конца.
...
Сопротивление — частая проблема: «вы хотите специально ломать прод?!». Отвечать абстракциями («повысит надёжность») бесполезно — нужны аргументы в деньгах и рисках, на языке руководства.

### Аргументы

1. **ROI на инцидентах**: один предотвращённый инцидент масштаба Azure-like стоит $$$, а chaos-эксперимент — несопоставимо меньше. Вы платите малую цену сейчас, чтобы не платить большую потом.
2. **MTTR**: тренированная команда восстанавливается быстрее, а каждая минута downtime — это деньги (упущенная выручка, SLA-штрафы).
3. **Compliance**: PCI, SOC2 и финрегуляторы требуют disaster recovery testing — GameDay напрямую закрывает это требование, превращая chaos из «прихоти» в обязательство.
4. **Уверенность в релизах**: код, проверенный chaos-ом, выкатывают без страха → выше скорость доставки.
5. **Привлечение талантов**: инженерная культура с SRE и chaos притягивает сильных инженеров.
...
3. **Blast radius**: явно ограниченный, продуманный заранее
4. **Abort condition**: триггер остановки, заданный до старта
5. **Результат** (outcome): action items, post-mortem, конкретные улучшения
