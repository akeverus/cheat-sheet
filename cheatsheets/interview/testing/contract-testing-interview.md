...
Разница в том, **кто задаёт контракт и что он фиксирует**. В consumer-driven контракт пишет потребитель и фиксирует только реально используемые поля. В provider-driven (schema-first) контракт задаёт провайдер и фиксирует всю схему API целиком.
...
Инструменты делятся по подходу: **consumer-driven** (контракт от потребителя — `Pact`), **producer-driven / schema-first** (контракт от провайдера — `Spring Cloud Contract`, `Schemathesis`, `Dredd`) и **service virtualization** (запись и воспроизведение трафика — `Hoverfly`).
...
Это связка из двух элементов: `PactConsumerTestExt` поднимает инфраструктуру (mock-сервер + запись pact'а), а `@PactTestFor` указывает, какой именно контракт и для какого провайдера прогнать.
...
**Рекомендация:** не коммитить pact-файлы в Git — только публиковать в брокер. Брокер становится центральным источником истины.
...
Общая ошибка одна: **state пытается делать слишком много**. Здоровый state — это детерминированная подготовка данных (один-два INSERT), а не бизнес-операция, не цепочка зависимостей и не «общий набор тестовых данных». Чем атомарнее и явнее state, тем устойчивее верификация.
...
**Эмпирическое правило:** state — это SQL INSERT или мок внешнего сервиса, а не бизнес-операция.
...
Интеграция асимметрична: **consumer-сторона Spring почти не требует** (тест проверяет HTTP-клиент против mock-сервера), а **provider-сторона поднимает реальное приложение** через `@SpringBootTest` и проигрывает pact'ы против запущенного контроллера с настоящей БД.
...
Producer side собирается из четырёх частей: **плагин** (генерирует тесты и стабы), **базовый класс** (готовит окружение для сгенерированных тестов), **контракты** в `src/test/resources/contracts/` и **публикация** `stubs.jar` в Maven-репозиторий. Плагин при сборке прогоняет сгенерированные тесты против контроллера — если они зелёные, провайдер гарантированно соответствует контракту.
...
`@AutoConfigureStubRunner` скачивает `stubs.jar` провайдера (из classpath, локального `~/.m2` или Maven-репозитория) и поднимает локальный `WireMock`-сервер по мапингам из этого артефакта. Consumer-тест делает реальный HTTP-запрос к этому WireMock и получает ровно те ответы, что описаны в контракте провайдера — отдельный mock руками писать не нужно.
...
Pipeline строится вокруг двух гейтов на брокере: consumer **публикует pact**, а перед деплоем обе стороны спрашивают `can-i-deploy`. Ключевой принцип — **деплой блокируется, пока контракт не верифицирован**, а после успешного деплоя вызывается `record-deployment`, чтобы брокер знал актуальное состояние окружений.
...
| Очень стабильный API (не меняется годами) | Накладные расходы на внедрение не окупятся |
...
**Эмпирическое правило:** контрактное тестирование окупается, когда есть 3+ независимых команд, деплоящих независимо, и боль от регрессий в интеграциях. Если этого нет — возможно, хватит OpenAPI + интеграционных тестов на `WireMock` ([подробнее](integration-testing-interview.md)).

## Q37. (!) Чем контрактное тестирование отличается от integration и E2E-тестов?

Все три проверяют разное: **contract** — совместимость формата API между двумя сервисами (без поднятия обоих), **integration** — связку сервиса с его инфраструктурой (БД, очередь, конфиги), **E2E** — сквозной бизнес-сценарий через всю систему. Они не заменяют, а дополняют друг друга: contract ловит то, что мок в integration-тесте пропустит, а E2E — то, что не видно ни тому, ни другому.
...
Большинство граблей сводятся к двум корневым причинам: **хрупкий контракт** (фиксируют точные значения и лишние поля вместо типов) и **разорванный CI-процесс** (нет версий, нет `can-i-deploy`, нет публикации результатов — и брокер перестаёт что-либо гарантировать).
...
Внедряют поэтапно, а не сразу на всё: сначала **одна пара сервисов** как пилот, затем стабилизация брокера и CI, и только потом массовый rollout. Главное — на раннем этапе включать `can-i-deploy` в режиме warn-only (предупреждает, но не блокирует), чтобы не остановить деплои, пока команда не привыкла к процессу.
...
Практики группируются вокруг трёх осей: **устойчивый контракт** (matchers по типу, только используемые поля), **рабочий CI-процесс** (брокер, `can-i-deploy`, публикация результатов, версии по git SHA) и **владение** (каждая команда отвечает за свой pact). Ниже — конкретный чек-лист.
...
Идея проста: **каждая версия API — это отдельный набор interactions**, и они живут в брокере параллельно, пока обе версии живы. Способ различения версий (URL, заголовок) определяет, что именно фиксировать в контракте: путь `/v2/...` или заголовок `Accept`. Брокер верифицирует все версии одновременно, поэтому он же показывает, когда старую версию можно безопасно удалить.
