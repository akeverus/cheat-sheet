...
> [!mcq]
> - [ ] Автоматизация полностью заменяет ручное тестирование, включая `exploratory` и `usability` | Авто-тесты не умеют ловить UX-проблемы и неожиданные сценарии — там нужен живой тестировщик. ❌ ПОСЛЕДСТВИЕ: команды Booking.com отказывались от exploratory testing в пользу 100% автоматизации — пропускали критичные UX-баги, обнаруженные пользователями в production через 2 недели после релиза.
> - [x] Авто-тесты ускоряют регрессию и фиксируют детерминированное поведение, но `exploratory testing` остаётся ручным | Цель автоматизации — стабильные повторяемые проверки в `CI/CD`; UX, юзабилити и discovery-тесты автоматизировать дорого и бесполезно. ✓ ПРИМЕНЯТЬ: Netflix запускает 100K+ unit/integration тестов в `Spinnaker` pipeline за 8 минут, а exploratory остаётся за QA-инженерами. 📋 ПРАВИЛО: «Auto = регрессия, manual = exploration». 🔗 См. Q2, Q4, Q45.
> - [ ] Главное преимущество автоматизации — снижение `headcount` команды QA на 80%+ | Автоматизация требует разработки и поддержки фреймворка — это перераспределение работы, не сокращение людей. ❌ ПОСЛЕДСТВИЕ: команда после массового сокращения QA получает за полгода рост `MTTR` с 2 часов до 2 дней — некому чинить flaky-тесты и анализировать падения.
> - [ ] Автоматизировать нужно сначала E2E через `Selenium`, потом `unit`-тесты | Это инверсия пирамиды тестирования: E2E медленные и хрупкие, unit-тесты дают мгновенный feedback. ❌ ПОСЛЕДСТВИЕ: Yahoo в 2010-х имели 80% E2E-тестов и 20% unit — `CI`-прогон занимал 6 часов, флакающих тестов было 30%, новые фичи блокировались на дни.
...
> [!mcq]
> - [ ] 70% E2E-тестов через `Selenium`, 20% интеграционных, 10% unit-тестов — «перевёрнутая пирамида» | Это `Ice Cream Cone` антипаттерн: дорого, медленно, хрупко. ❌ ПОСЛЕДСТВИЕ: Twitter в 2014 имели 80% UI-тестов — `CI` гонялся 4 часа, flakiness достигал 35%, разработчики игнорировали падения и мержили красные билды.
> - [ ] 60% интеграционных, 30% E2E, 10% unit — «бриллиант» с упором на интеграцию | Слишком много медленных тестов: интеграционные требуют БД/контейнеры, что замедляет feedback loop. ❌ ПОСЛЕДСТВИЕ: типичный enterprise-проект с 5K интеграционных тестов на `@SpringBootTest` — прогон 45 минут, разработчики коммитят без локального запуска.
> - [ ] 60-70% unit, 20-30% интеграционных, 5-10% E2E — классическая пирамида Майка Кона | Быстрые unit-тесты дают мгновенный feedback, медленные E2E проверяют только критичные user journeys. ✓ ПРИМЕНЯТЬ: Google Testing Blog рекомендует 70/20/10 для всех команд; Spotify применяет это правило в `Backstage` и сотнях микросервисов. 📋 ПРАВИЛО: «Много дешёвых внизу, мало дорогих наверху». 🔗 См. Q1, Q4, Q43.
> - [ ] Все уровни в равной пропорции 33/33/33 — «прямоугольник» для полного покрытия | Игнорирует разную стоимость и скорость уровней; одинаковое количество медленных и быстрых тестов сводит на нет преимущества unit. ❌ ПОСЛЕДСТВИЕ: проекты с равной пропорцией получают `CI` за 30+ минут и тратят 60% времени QA на поддержку дублирующихся UI-тестов поверх покрытых unit-сценариев.
...
> [!mcq]
> - [ ] Для unit-тестов в Java стандарт де-факто — `Cucumber` + `Spock` | `Cucumber` — BDD-фреймворк для feature-файлов, не unit-тесты; `Spock` — Groovy. ❌ ПОСЛЕДСТВИЕ: команда выбирает `Cucumber` для всего тестирования — 10K шагов в `*.feature` файлах, рефакторинг занимает дни, junior-разработчики теряются в Gherkin для тривиальных проверок.
> - [ ] `JUnit 5`/`TestNG` для unit-тестов, `Mockito`/`WireMock` для моков, `Testcontainers` для БД, `REST Assured` для API, `Selenium`/`Playwright` для UI | Каждый инструмент покрывает свой уровень пирамиды и имеет идиоматичную интеграцию со Spring Boot. ✓ ПРИМЕНЯТЬ: spring-boot-starter-test собирает `JUnit 5` + `Mockito` + `AssertJ` + `Hamcrest` из коробки; Testcontainers применяется в Atlassian, Confluent, Trivago. 📋 ПРАВИЛО: «На каждый уровень — свой инструмент». 🔗 См. Q14, Q18, Q44.
> - [ ] `Selenium WebDriver` универсален для всего: unit, интеграции, UI, нагрузки | Selenium управляет только браузером — для unit-тестов и API-проверок он бесполезен. ❌ ПОСЛЕДСТВИЕ: команда покрывает unit-логику через `Selenium`-сценарии — каждый тест занимает 5-10 секунд, прогон 1000 тестов занимает 2.5 часа вместо 30 секунд на JUnit.
> - [ ] Для performance-тестирования использовать `JaCoCo`, для покрытия кода `JMeter` | `JaCoCo` — code coverage, `JMeter` — нагрузка; перепутаны назначения. ❌ ПОСЛЕДСТВИЕ: разработчик включает `JaCoCo` для нагрузочного тестирования и получает только отчёт по покрытию строк, без метрик RPS/latency, в результате SLA нагрузки не валидируется перед production.
...
> [!mcq]
> - [ ] В первую очередь автоматизировать exploratory testing и UX-проверки | Это обратный приоритет: exploratory неавтоматизируется, UX требует человеческого восприятия. ❌ ПОСЛЕДСТВИЕ: команда тратит 3 месяца на автоматизацию визуальных оценок UI — тесты ловят 5% проблем, остальные находят пользователи в production через `crash reports`.
> - [ ] Автоматизировать одноразовые тесты для быстрого закрытия задачи | Одноразовые тесты не дают ROI: время на разработку не окупается одним запуском. ❌ ПОСЛЕДСТВИЕ: команда покрывает миграцию данных автотестом за 2 дня — после миграции тест выкидывается, время потрачено впустую, могло уйти на регрессию.
> - [ ] Автоматизировать тесты, которые меняются каждый спринт под новые требования | Хрупкие требования = хрупкие тесты; стоимость поддержки превышает пользу. ❌ ПОСЛЕДСТВИЕ: фронтенд-команда автоматизирует тесты для prototype-фичи — за полгода тесты переписываются 8 раз, на поддержку уходит 30% времени QA.
> - [x] Сначала автоматизировать `happy path` критичных бизнес-сценариев (оплата, авторизация), затем расширять на стабильные регрессионные кейсы | Высокая частота запуска + бизнес-критичность + стабильность требований = максимальный ROI. ✓ ПРИМЕНЯТЬ: Stripe автоматизирует все payment flows как priority 1 (запуск на каждый коммит); LinkedIn покрывает auth/feed/messaging автотестами в первую очередь. 📋 ПРАВИЛО: «Автоматизируй критичное и стабильное — остальное руками». 🔗 См. Q1, Q5, Q38.
...
> [!mcq]
> - [ ] `ROI = количество автотестов / общее количество тестов × 100%` — чем больше автоматизировано, тем выше ROI | Это `Test Automation Rate`, а не ROI; высокая автоматизация без качества может убыточной. ❌ ПОСЛЕДСТВИЕ: команда хвастается 95% автоматизации в отчётах, но 30% тестов flaky — `MTTR` бизнес-критичных багов вырастает с 2 часов до 1 дня из-за ложных срабатываний.
> - [ ] `ROI = (Экономия времени − Затраты на разработку и поддержку) / Затраты × 100%`, breakeven наступает после 5-15 запусков | Учитывает реальную стоимость: разработка теста + поддержка против многократной ручной экономии. ✓ ПРИМЕНЯТЬ: Atlassian публикует расчёты ROI в `Bitbucket Pipelines` blog — типичный E2E-тест окупается за 8-10 запусков на Selenium Grid; Microsoft Azure DevOps использует эту формулу для приоритизации. 📋 ПРАВИЛО: «ROI = (экономия − затраты) / затраты, окупаемость от 5 запусков». 🔗 См. Q1, Q4, Q42.
> - [ ] `ROI = время прогона ручного теста × количество запусков` — простая экономия времени | Игнорирует стоимость разработки и поддержки автотеста; результат всегда «положительный» и неинформативен. ❌ ПОСЛЕДСТВИЕ: менеджер показывает «ROI = 1000 часов» по этой формуле, но не видит, что разработка теста заняла 1500 часов — проект убыточен, продолжается финансирование.
> - [ ] ROI = code coverage в процентах: 80%+ означает положительный ROI | `Coverage` ≠ ROI; высокое покрытие может быть бесполезным (тесты без assertions, mock-only). ❌ ПОСЛЕДСТВИЕ: команда выбивает 95% line coverage через `assertNotNull(result)` тесты — `mutation score` всего 20%, баги не ловятся, ROI отрицательный.
...
> [!mcq]
> - [ ] `Singleton` для `WebDriver` — единственный driver на всю сьюту тестов | `Singleton` для WebDriver ломает параллельность и изоляцию: тесты делят `cookies`/`session storage`. ❌ ПОСЛЕДСТВИЕ: команда применяет `WebDriver` Singleton — параллельные тесты падают с `StaleElementReferenceException` 40% запусков, отладка занимает дни.
> - [x] `Page Object Model` (UI), `Builder` (тестовые данные), `Data-Driven` (параметризация), `Screenplay` (actor-centric для сложных user journeys) | Каждый паттерн решает конкретную проблему: POM инкапсулирует UI, Builder упрощает фикстуры, Data-Driven масштабирует, Screenplay убирает дублирование между UI/API. ✓ ПРИМЕНЯТЬ: ThoughtWorks Serenity BDD реализует Screenplay из коробки; Selenide использует POM в большинстве enterprise-проектов; JUnit 5 `@ParameterizedTest` — основа Data-Driven. 📋 ПРАВИЛО: «POM для UI, Builder для данных, Screenplay для actors». 🔗 См. Q9, Q21, Q49.
> - [ ] `God Object` для всех Page Objects — один класс инкапсулирует все страницы | Анти-паттерн: класс на 5K строк теряет читаемость, изменение одной страницы ломает другие тесты. ❌ ПОСЛЕДСТВИЕ: проект с `AppPage.java` на 8K строк — каждый PR меняет этот файл, конфликты merge возникают на каждом коммите, новые разработчики тратят неделю на onboarding.
> - [ ] `Hard-coded` локаторы и данные прямо в тесте без паттернов | Дублирование `By.id("login")` в 50 тестах: смена id ломает все тесты сразу. ❌ ПОСЛЕДСТВИЕ: команда без POM получает обновление UI — `Sprint Review` показывает 40% красных тестов, починка занимает 3 дня вместо 30 минут на обновление одного Page Object.
...
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q8. Какие стратегии поиска элементов есть в `Selenium`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q9. (!) Что такое `Page Object Model`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q10. Чем отличаются `implicit`, `explicit` и `fluent` waits в `Selenium`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q11. Что такое `Selenium Grid` и как он работает? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q12. Чем `Playwright` отличается от `Selenium`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q13. Как работать с `JavaScript`-алертами и `iframe` в `Selenium`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q14. (!) Как автоматизировать `API`-тестирование с `REST Assured`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q15. Как валидировать `JSON Schema` в API-тестах? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q16. Что такое `Contract Testing` и зачем он нужен? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q17. Как тестировать аутентификацию и авторизацию в API? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q18. (!) Что такое `Testcontainers` и как их использовать? ❌ ПОСЛЕДСТВИЕ: антипаттерн деградирует SLA при росте нагрузки или зависимостей.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q19. Как организовать тестовые данные? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q20. Что такое `WireMock` и когда его использовать? ❌ ПОСЛЕДСТВИЕ: антипаттерн деградирует SLA при росте нагрузки или зависимостей.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q21. Что такое `Data-Driven Testing`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q22. (!) Что такое `BDD` и как работает `Cucumber`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q23. Что такое `Keyword-Driven Testing`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q24. (!) Как интегрировать автотесты в `CI/CD`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q25. (!) Что такое `Continuous Testing`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q26. Как организовать параллельное выполнение тестов? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q27. Что такое `Test Orchestration`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q28. (!) Что такое `flaky`-тесты и как с ними бороться? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q29. Как диагностировать причины нестабильности тестов? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q30. Как автоматизировать `Performance Testing`? ❌ ПОСЛЕДСТВИЕ: антипаттерн деградирует SLA при росте нагрузки или зависимостей.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q31. Что такое `Visual Regression Testing`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q32. Как автоматизировать `Accessibility Testing`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q33. Как автоматизировать `Security Testing`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q34. Как автоматизировать тестирование мобильных приложений? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q35. (!) Что такое `Test Reporting` и какие метрики отслеживать? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q36. Что такое `Test Environment Management`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q37. Как автоматизировать `Smoke Testing`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q38. Как автоматизировать `Regression Testing`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q39. (!) Что такое `Mutation Testing`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q40. Как оценить качество тестового покрытия? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q41. Как спроектировать архитектуру тестового фреймворка? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q42. (!) Что такое `Test Maintenance Strategy`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q43. Как организовать тестирование [микросервисов](../architecture/microservices-interview.md)? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q44. Что такое `Test Doubles` и какие виды бывают? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q45. Best practices для автоматизации тестирования? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q46. (!) Как реализовать Page Object Model с `Playwright` для Java? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q47. Как писать `Gherkin`-сценарии и шаговые определения в `Cucumber`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q48. Как организовать `TestNG` Suite для параллельного запуска? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q49. (!) Как применять паттерн `Screenplay` в автоматизации? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q50. Как тестировать `GraphQL` API? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
