...
1. **Автор создаёт PR.**
2. **CI** прогоняет сборку, тесты и линтеры.
   - Если CI красный → возврат к автору на правки.
   - Если CI зелёный → ревьюер просматривает дифф.
3. **Ревьюер оставляет замечания.** Дальше ветвление:
   - Есть блокирующие замечания → автор вносит правки → снова прогон CI.
   - Только `nit`/`suggestion` → `Approve`.
   - Нет замечаний → `Approve`.
4. После `Approve` — **merge** в основную ветку.
...
Маршрутизация ревьюера по области изменений:

- **Pull Request создан** → определяется область изменений.
  - Пути `auth/`, `payment/` → обязательный ревьюер из списка экспертов.
  - Обычный код → ревьюер по ротации.
- Оба пути ведут к **ревью**. По итогу — решение `Approve?`:
  - `Да` → merge.
  - `Нет` → автор вносит правки и возвращается на ревью.
...
**Большую фичу** разбивают на последовательность PR (каждый следующий зависит от предыдущего):

- **PR 1: Рефакторинг** — подготовка.
- **PR 2: Доменная логика** — ядро изменений.
- **PR 3: Интеграция + API** — финальная часть.

Порядок: PR 1 → PR 2 → PR 3.
...
Разделение труда между автоматикой и человеком:

- **Автоматика (CI)** — последовательность шагов:
  1. `Checkstyle / Spotless`
  2. Компиляция
  3. Unit-тесты
  4. Integration-тесты
  5. `SonarQube` Quality Gate
  6. SAST / Dependency Check
- По итогу последнего шага (SAST / Dependency Check):
  - Зелёный CI → передача **ревьюеру (человеку)**.
  - Красный CI → автор чинит и прогон начинается заново с `Checkstyle / Spotless`.
- **Ревьюер (человек)** концентрируется на том, что машина не проверит:
  - Логика и граничные случаи.
...
Участники асинхронного цикла: **Автор** (UTC+3), **CI Pipeline**, **Ревьюер** (UTC+1). Порядок шагов:

1. Автор → CI: push PR.
2. CI → Автор: зелёный CI.
3. Автор → Ревьюер: назначение + описание PR.
4. *(Заметка: ревьюер видит PR в своё рабочее время.)*
5. Ревьюер → Автор: комментарии к коду.
6. Автор → Автор: правки в свой рабочий день.
7. Автор → Ревьюер: «Исправлено, прошу повторно взглянуть».
8. Ревьюер → Автор: `Approve`.
9. Автор → CI: merge.
...
Метрики сгруппированы по трём срезам, все сходятся в **дашборд**, а тот ведёт к **улучшению процесса**:

- **Метрики скорости:** Time to first review; Cycle time.
- **Метрики качества:** Post-merge defects; Iteration count.
- **Метрики покрытия:** Review coverage; PR size distribution.

Все шесть метрик → дашборд → улучшение процесса.
...
Лестница эскалации разногласия по шагам:

1. **Разногласие в ревью** → обсуждение в треде (2–3 обмена). Достигнуто согласие?
   - `Да` → изменение кода или принятие текущего варианта.
   - `Нет` → следующий шаг.
2. **Созвон 15 минут.** Достигнуто согласие?
   - `Да` → изменение кода или принятие текущего варианта.
   - `Нет` → следующий шаг.
3. **Привлечь архитектора / тимлида** → решение + запись обоснования → изменение кода или принятие текущего варианта.
4. После изменения/принятия — **записать решение в PR / ADR**.
...
Каждой причине узкого места соответствует решение:

- **Большие PR** → разбиение PR ≤ 400 строк.
- **Мало ревьюеров** → ротация / CODEOWNERS.
- **Нет SLA** → SLA: 4 ч на первый ответ.
- **Ручные проверки стиля** → линтеры + CI.
...
Процесс hotfix-ревью по порядку:

1. **Баг в проде** → автор пишет фикс.
2. **PR с меткой `[HOTFIX]`.**
3. **CI:** тесты + сборка.
4. **Ревьюер:** проверяет только затронутую область.
5. `Approve` (SLA 1 ч) → **merge + deploy**.
6. **Постфактум:** полный ревью в течение 24 ч; тикет на улучшение.
...
**Code Smells** группируются по четырём категориям:

- **Раздутые:**
  - `Long Method` — >30 строк.
  - `Large Class` — >200 строк.
  - `Long Parameter List` — >4 параметров.
  - `Data Clumps` — группы полей, везде встречающиеся вместе.
- **Связность:**
  - `Feature Envy` — метод работает с чужими данными.
  - `Inappropriate Intimacy` — класс знает внутреннее устройство другого класса.
