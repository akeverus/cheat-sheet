...
**Рефакторинг** — изменение внутренней структуры кода без изменения внешнего поведения. На собеседовании часто спрашивают о приёмах (по Фаулеру), связи с [[unit-testing-interview|тестами]], «запахах кода» и стратегиях в legacy. Рефакторинг тесно связан с [[design-patterns-interview|паттернами проектирования]] — многие приёмы рефакторинга приводят код к известным паттернам.
...
Правило «три удара»: при первом дублировании оставляем как есть, при втором замечаем, при третьем выносим в общий метод/класс. Рефакторинг без [[unit-testing-interview|тестов]] рискован — сначала покрыть критичные места тестами, затем менять структуру. В PR рефакторинг лучше выносить отдельно от фичи: один PR — только рефакторинг (тесты зелёные), второй — новая логика. Так проще [[code-review-interview|ревью]] и откат.
...
Замена цепочек `if/else` или `switch` по типу или коду на полиморфизм: у каждого варианта — свой класс (или стратегия), общий интерфейс. Этот приём часто приводит к паттерну [[design-patterns-interview|Strategy]].
...
Вместо поля «тип» (число или enum) и ветвлений по нему вводятся **отдельные классы состояний или стратегий**; объект делегирует поведение текущему состоянию/стратегии. Этот приём связан с [[design-patterns-interview|паттернами State и Strategy]].
...
Уменьшает размер класса, улучшает тестируемость (моки по интерфейсу) и позволяет подставлять разные реализации. В Java интерфейсы часто используют для слоёв (сервис, репозиторий), что хорошо видно в [[unit-testing-interview|вопросах по unit-тестированию]].
...
Тесты — страховка при рефакторинге: до и после рефакторинга [[unit-testing-interview|тесты]] должны оставаться зелёными. Сначала пишут или дополняют тесты на критичное поведение (особенно в legacy), затем рефакторят небольшими шагами, после каждого шага запускают тесты.
...
Запахи кода — признаки возможных проблем, указывающие места для рефакторинга. Классификация по Фаулеру помогает систематически улучшать код. Тесно связаны с [[technical-debt-interview|техническим долгом]].
...
`SonarQube` и `SpotBugs` находят часть запахов (дублирование, сложность). При [[code-review-interview|ревью]] обращать внимание на запахи из таблицы выше.
...
Если зависимости тяжёлые — вынести интерфейс и подставлять заглушку в [[unit-testing-interview|тесте]]; затем рефакторить реализацию. Подробнее о Strangler Fig — в следующем вопросе.
...
Контракт-тесты (`Pact`, `Spring Cloud Contract`) помогают не сломать клиентов — подробнее в [[integration-testing-interview|вопросах по интеграционному тестированию]]. Для внутренних модулей можно допускать breaking changes с версионированием и миграцией вызывающего кода.
...
Перед крупным рефакторингом — `SonarQube` по проекту, исправить критичные замечания. При ручном переносе — один приём за коммит, [[unit-testing-interview|тесты]] после каждого шага.
...
Не стоит: в середине горячего фикса под давлением (риск регрессии); без [[unit-testing-interview|тестов]] в критичной области; «рефакторинг ради рефакторинга» без цели.
...
Подробнее о стратегиях покрытия — в [[test-strategies-interview|вопросах по стратегиям тестирования]]. В legacy — characterization tests на вход/выход затронутого метода; затем рефакторить малыми шагами.
...
Ревьюер проверяет: поведение не изменилось (тесты зелёные, контракты сохранены); рефакторинг логичен (нет лишних изменений «заодно»); нет введения новых «запахов». Подробнее о процессе — в [[code-review-interview|вопросах по code review]].
...
Метрики — не самоцель; они помогают обосновать рефакторинг перед командой и зафиксировать улучшение. Связано с управлением [[technical-debt-interview|техническим долгом]] и мониторингом качества кода.
...
**SOLID-рефакторинг** — систематическое устранение нарушений принципов, которые накопились как [[technical-debt-interview|технический долг]].
...
## Q36. Decompose Conditional — замена if/else-цепочек паттернами
...
## Q37. Replace Primitive With Object — паттерн Value Object
...
## Q38. Introduce Parameter Object — борьба с длинными списками параметров
...
## Q39. Рефакторинг схемы БД — Expand-Contract паттерн
...
## Q40. Feature Envy — когда метод слишком интересуется чужими данными
...
## Q41. Shotgun Surgery — распознавание и лечение
...
## Q42. Extract Method Object — когда Extract Method недостаточно
...
- [[technical-debt-interview|Технический долг]] — вопросы по техническому долгу
- [[design-patterns-interview|Паттерны проектирования]] — GoF-паттерны, которые часто применяются при рефакторинге
- [[unit-testing-interview|Unit-тестирование]] — тесты как страховка при рефакторинге
- [[test-strategies-interview|Стратегии тестирования]] — планирование тестового покрытия перед рефакторингом
- [[java-oop-interview|ООП в Java]] — принципы SOLID и проектирование классов
