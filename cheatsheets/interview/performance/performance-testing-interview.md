...
Это три точки отсчёта, относительно которых интерпретируют цифры теста. Их легко спутать, но отвечают они на разные вопросы — «как сейчас», «как у других» и «как должно быть»:

- **Baseline** (как сейчас) — эталонное измерение текущего состояния системы. Фиксируется до изменений, чтобы сравнивать «до» и «после» и ловить регрессии.
- **Benchmark** (как у других) — сравнение с внешним эталоном: конкурент, референсная архитектура, теоретический максимум. Отвечает на вопрос «есть ли куда расти».
- **SLA/SLO** (как должно быть) — контрактные и целевые требования, которые система **обязана** выполнять. Именно по ним определяется, считать результат теста успехом или провалом.
...
2. Сравнивать каждое изменение с baseline (A/B) — иначе нельзя доказать, что «стало лучше».
3. Проверять, что SLO выполняется при целевой нагрузке.
4. Считать багом нарушение SLA/SLO, а не просто субъективное «медленно».

**Ключевая идея:** «медленно» — это мнение, а «p99 вырос со 180ms до 320ms, SLO нарушен» — факт. Все три понятия нужны, чтобы переводить ощущения в измеримые критерии.
...
**Принцип shift-left:** чем ниже уровень, тем чаще запуск. Базовые smoke-нагрузки гонять на каждом PR — это ловит регрессии до мержа, пока правка ещё свежа в голове. Полный load/soak дорог и шумен, поэтому его место — pre-prod окружение по расписанию (nightly/weekly).
...
Это два разных теста, которые часто путают. Короткое различие: volume давит **данными**, scalability проверяет реакцию на **ресурсы**.

**Volume testing** — проверка работы с **большим объёмом данных**: миллионы записей в БД, большие payload'ы, огромные файлы. Нагрузка по пользователям может быть совсем обычной — критичен именно объём. Смысл в том, что планы запросов, индексы и кеши ведут себя иначе на полной БД, чем на тестовой в сотню строк.
...
**Scalability testing** — проверка, **как растёт пропускная способность** при добавлении ресурсов. Это серия одинаковых тестов на разных конфигурациях: цель — понять, удваивается ли throughput при удвоении ресурсов (линейность) и где она ломается.
...
Пример: `500 RPS × 200 ms = 100 одновременных запросов в системе`. Практический вывод из закона: throughput и latency связаны через concurrency, и это объясняет частую путаницу. Если у вас «всего» 100 потоков в пуле, а ответ занимает 200ms, то выше 500 RPS вы физически не подниметесь — упрётесь не в CPU, а в число одновременных слотов. Чтобы расти, нужно либо снижать latency (быстрее освобождать слоты), либо увеличивать concurrency (больше потоков/соединений).
...
**Идея метрики:** разбить все запросы на три корзины относительно порога `T` — «доволен» (≤ T), «терпит» (до 4T) и «раздражён» (> 4T) — и свернуть это в одно число. «Терпящие» дают половину веса, «раздражённые» — ноль, поэтому Apdex резко проседает, когда хвост распределения уходит за 4T.

**Компромисс:** Apdex удобен для отчётности руководству и алертов — одно число легко читать и ставить на дашборд. Но именно потому, что он сворачивает всё в одну цифру, он скрывает причины и форму хвоста. На инженерном уровне для диагностики лучше смотреть percentile latency и error rate по отдельности.
...
Минимум — это latency, RPS и error rate приложения, но их недостаточно: без системных метрик видно *что* латенси растёт, но непонятно *почему*. Поэтому во время теста параллельно снимают метрики по всем слоям — от JVM до диска, чтобы соотнести рост latency с конкретным ресурсом.
...
- с ростом времени GC → проблема [JVM-тюнинга](jvm-performance-tuning-interview.md) или высокого allocation rate
- с насыщением connection pool БД → узкое место в базе
- с ростом сетевых ретрансмиссий → сетевая проблема
...
- GUI для визуального построения тестов — удобно для QA-команд без опыта в коде
- Огромная экосистема плагинов ([JMeter Plugins](https://jmeter-plugins.org/))
- Множество протоколов помимо HTTP (JDBC, JMS, FTP, SOAP, LDAP, MQTT)
- Distributed mode из коробки (master-slave)
...
**Почему ramp-up важен:** если все потоки стартуют разом, вы тестируете spike, а не равномерную нагрузку — кеши и пулы получают удар на холодную. Для реалистичного load-теста берут ramp-up 2-5 минут, а SLO меряют уже на steady state длиной 15-60 минут.
...
**Почему BeanShell устарел.** Корень проблемы — он интерпретирует скрипт заново при **каждом** выполнении сэмплера. Под нагрузкой это критично: при 1000 RPS скрипт парсится 1000 раз в секунду, и сам тестовый клиент становится bottleneck'ом, искажая результаты. Groovy же компилирует скрипт в байт-код один раз и кеширует — отсюда выигрыш в 10-100×.

**Важно:** в JSR223 Sampler обязательно ставьте галочку `Cache compiled script if available` — без неё Groovy тоже компилирует заново на каждый прогон, и весь смысл перехода с BeanShell теряется.
...
Две принципиально разные модели подачи нагрузки. Разница в том, что фиксируется как постоянная величина: число пользователей в системе (closed) или темп их прихода (open). Это не деталь — выбор модели меняет сами цифры latency при перегрузке.

**Closed model** — задаём **количество concurrent users**. Каждый VU отправляет следующий запрос только после ответа на предыдущий, поэтому если сервис замедлился, новые запросы **не** добавляются — фактический RPS сам падает, нагрузка автоматически «подстраивается» под сервис. Имитирует системы с ограниченным пулом сессий: банкоматы, десктоп-клиенты, фиксированный пул воркеров.
...
**Open model** — задаём **arrival rate** (пользователей в секунду). Новые запросы приходят независимо от того, успел ли сервис ответить на предыдущие — при замедлении очередь растёт, и тест честно показывает деградацию. Имитирует web/mobile-трафик: реальные люди приходят на сайт независимо друг от друга и не ждут, пока обслужат кого-то ещё.
...
Два разных механизма проверок, работающих на разном масштабе: **Check** валидирует отдельный ответ, **Assertion** — итог всего теста. Грубо: checks отвечают «корректен ли *этот* запрос», assertions — «прошёл ли *весь прогон* по SLO».

**Checks** — проверки на уровне **одного HTTP-запроса**. Если check провалился — запрос помечается как KO (Failed) и попадает в статистику ошибок, но сам тест продолжается.
...
`Scenarios` позволяют одновременно запускать **несколько разных нагрузочных профилей** в одном тесте — аналог нескольких Thread Group в JMeter. Это нужно, чтобы воспроизвести реальный микс трафика: например, фоновое чтение (constant-vus), всплеск оформлений заказов (ramping-arrival-rate) и периодические фоновые задания (constant-arrival-rate) — всё параллельно, каждое со своим executor'ом.
...
Правильный разбор результатов — это не одна цифра «p99 = X», а связная картина: как метрики ведут себя **во времени** и как они **коррелируют** между собой и с метриками системы. Цель анализа — не зафиксировать число, а найти причину.
...
**Coordinated omission** — систематическое занижение latency в инструментах с closed model: когда сервис тормозит, клиент перестаёт слать новые запросы и тем самым «не замечает» большую часть деградации. Термин ввёл Gil Tene (создатель HdrHistogram).
...
**Последствия:**
- p99 выглядит нормальным, хотя реальная деградация была на 10 секунд
- Ошибка всегда смещает оценку в «оптимистичную» сторону — latency кажется лучше, чем есть
- На таких заниженных данных принимают capacity- и SLO-решения, которые не сработают в проде
