...
**Распределённый кэш** — кэш на отдельных узлах (`Redis`, `Memcached`, `Hazelcast`). Плюсы: общий кэш для всех инстансов; централизованная инвалидация. Минусы: сетевая задержка (1-5 мс), зависимость от доступности кэш-сервера, необходимость сериализации (см. [[redis-interview|вопросы по Redis]]).
...
`TTL` в `L1` обычно короче (1-5 минут), чем в `L2` (10-60 минут), чтобы быстрее подхватывать инвалидацию. При публикации события инвалидации нужно очищать `L1` во всех инстансах (через `Redis Pub/Sub`). Подробнее о реализации в [[spring-boot-interview|Spring Boot]].
...
**Практика:** алерт при hit ratio < 70% в течение 15 минут; дашборд с трендом по кэшам; при падении после деплоя — ожидаемо (холодный кэш), дать время на прогрев. Подробнее о метриках в [[observability-interview|вопросах по наблюдаемости]].
...
**Важно:** `GenericJackson2JsonRedisSerializer` добавляет поле `@class` для десериализации; при переименовании пакета старые записи не десериализуются — используйте версию в ключе или короткий `TTL`. Подробнее о `Redis` в [[redis-interview|вопросах по Redis]].
...
Подходит для логов, метрик, счётчиков; для платежей — `Write-Through`. Требует надёжной очереди (подробнее в [[kafka-interview|вопросах по Kafka]]) и идемпотентности записи в БД.
...
Для справочников — `TTL` с jitter (базовый + случайное отклонение). Для персональных данных — инвалидация по событию. Подробнее о событийной архитектуре в [[event-driven-patterns-interview|вопросах по Event-Driven]].
...
При распределённом кэше и нескольких писателях нужна единая стратегия, иначе один сервис может инвалидировать ключ, а другой — перезаписать устаревшим значением. Подробнее в [[consistency-patterns-interview|паттернах согласованности]].
...
**Важно:** при изменении полей объекта старые записи в кэше могут не десериализоваться — используйте версию в ключе (`user:v2:123`) или короткий `TTL` при миграции. Подробнее о сериализации в [[java-serialization-interview|вопросах по Java Serialization]].
...
Часто комбинируют `L1` (локальный, короткий `TTL`) и `L2` (`Redis`); при публикации события подписчики инвалидируют свой `L1` и соответствующие ключи в `L2`. Подробнее в [[microservices-interview|вопросах по микросервисам]].
...
Подробнее о согласованности в [[consistency-patterns-interview|паттернах согласованности]].
...
В `Redis` — `INFO stats` (`keyspace_hits`, `keyspace_misses`); `Redis Exporter` для `Prometheus`. `spring-boot-starter-cache` с `micrometer-registry-prometheus` автоматически экспортирует метрики кэша в `Actuator`. Подробнее в [[metrics-tracing-interview|вопросах по метрикам и трейсингу]].
...
**Integration-тесты:** с реальным `Redis` через `Testcontainers`; проверить запись, чтение, `TTL`, инвалидацию. Подробнее в [[integration-testing-interview|вопросах по интеграционному тестированию]].
...
Для строгой консистентности между DC часто используют отказ от кэша для критичных данных. Подробнее в [[distributed-systems-interview|вопросах по распределённым системам]].
...
- [[redis-interview|Redis]] — вопросы по Redis как распределённому кэшу и хранилищу данных
- [[microservices-interview|Микросервисная архитектура]] — кэширование в контексте микросервисов
- [[distributed-systems-interview|Распределённые системы]] — консистентность и репликация кэшированных данных
- [[spring-boot-interview|Spring Boot]] — конфигурация и автоконфигурация кэша (@Cacheable, @CacheEvict)
- [[scalability-patterns-interview|Паттерны масштабируемости]] — кэш как инструмент масштабирования и снижения нагрузки на БД
- [[resilience-patterns-interview|Паттерны отказоустойчивости]] — кэш как fallback при недоступности upstream сервиса
- [[database-architecture-interview|Архитектура баз данных]] — read replica vs кэш, стратегии снижения нагрузки на БД
