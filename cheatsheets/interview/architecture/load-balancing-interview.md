...
- **L4 (Transport Layer)** — клиент → L4 Load Balancer → бэкенды по `TCP`/`UDP` (по `IP:port`):
  - L4 → `Server 1` (TCP/UDP по IP:port);
  - L4 → `Server 2` (TCP/UDP по IP:port).
- **L7 (Application Layer)** — клиент → L7 Load Balancer → сервисы по содержимому запроса:
...
- **Активная проверка** — Load Balancer сам шлёт пробы `GET /health` каждые 5с на каждый бэкенд:
  - `Backend 1` отвечает ✅ — остаётся в пуле;
  - `Backend 2` отвечает ❌ — исключается из пула.
- **Пассивная проверка** — Load Balancer шлёт реальные клиентские запросы:
  - `Backend 3` отвечает нормально ✅ — остаётся в пуле;
  - `Backend 4` отвечает ошибкой (реальный запрос → `502`) — исключается после N ошибок.
...
Например, поток запросов `1,2,3,4,5,6` Round Robin LB распределит по кругу:

- `Server A` ← запросы 1, 4;
- `Server B` ← запросы 2, 5;
- `Server C` ← запросы 3, 6.
...
По потоку запроса различие выглядит так:

- **Серверная балансировка** — клиент → `Load Balancer` → один из `Service 1` / `Service 2` / `Service 3`. Клиент обращается к одному адресу, а распределяет балансировщик.
- **Клиентская балансировка** — клиент (с LB-логикой внутри) сначала обращается к `Service Discovery`, тот возвращает ему список инстансов, после чего клиент сам соединяется напрямую с одним из `Service 1` / `Service 2` / `Service 3`.
...
По распределению трафика это выглядит так:

- **Blue-Green Deploy** — `Load Balancer` шлёт 100% трафика на `Blue v1.0`, а на `Green v2.0` трафик переключается разом с 0% на 100%.
- **Canary Deploy** — `Load Balancer` делит трафик: 90% на `Stable v1.0` и 10% на `Canary v2.0`.
...
Топология такой пары:

- `DNS / Floating IP` направляет трафик на `LB Active`, а при failover переключает его на `LB Passive`.
- `LB Active` обслуживает `Backend 1` и `Backend 2` (активно); `LB Passive` подключён к тем же `Backend 1` и `Backend 2`, но в резерве (пунктиром).
- `LB Active` и `LB Passive` обмениваются `VRRP heartbeat` — по нему резервный узел замечает падение активного.
...
По шагам процесс выглядит так (участники: `Load Balancer`, `Server (draining)`, клиент):

1. Сервер выводится из пула.
2. `Load Balancer` → `Server`: прекращение новых запросов.
3. Клиент → `Server`: существующие запросы продолжают работу.
4. `Server` → клиент: ответы на текущие запросы.
5. Истекает таймаут draining (30с).
...
По шагам: каждый клиент шлёт DNS-запрос `api.example.com` на `DNS Server`, а тот разным клиентам отдаёт разные адреса:

- клиент получает ответ `10.0.0.1` и идёт на `Server 10.0.0.1`;
- другой клиент получает ответ `10.0.0.2` и идёт на `Server 10.0.0.2`;
- третий клиент получает ответ `10.0.0.3` и идёт на `Server 10.0.0.3`.
...
По регионам это выглядит так (маршрутизация через DNS geolocation):

- Пользователь EU → `LB Европа` → `DC Франкфурт`;
- Пользователь US → `LB Америка` → `DC Вирджиния`;
- Пользователь Asia → `LB Азия` → `DC Токио`.

Между дата-центрами идёт репликация данных: `DC Франкфурт` ↔ `DC Вирджиния` ↔ `DC Токио`.
...
- **Cross-zone ВКЛЮЧЁН** — `ALB` раздаёт по 33% на каждый инстанс: `AZ-A` (1 инстанс) — 33%, `AZ-B` (1 инстанс) — 33%, `AZ-C` (1 инстанс) — 33%.
- **Cross-zone ВЫКЛЮЧЕН** — `NLB` раздаёт сначала поровну по зонам: 50% на `AZ-A` (2 инстанса → по 25% каждому) и 50% на `AZ-B` (1 инстанс → 50%).
...
Например, на хэш-кольце `0..2^32` расставлены узлы: `Node A` (позиция 1000), `Node B` (позиция 4000), `Node C` (позиция 7000). Ключ попадает на ближайший по кольцу узел:

- `key: user-123` → `hash 2500` → `Node B`;
- `key: user-456` → `hash 5500` → `Node C`;
- `key: user-789` → `hash 800` → `Node A`.
...
Например, при трёх серверах формула `hash(client_ip) % 3` разводит клиентов так:

- `Client 1` (IP `10.0.0.1`): `hash(10.0.0.1)%3=0` → `Server 1`;
- `Client 2` (IP `10.0.0.2`): `hash(10.0.0.2)%3=1` → `Server 2`;
- `Client 3` (IP `10.0.0.3`): `hash(10.0.0.3)%3=0` → `Server 1`;
- `Client 4` (IP `10.0.0.4`): `hash(10.0.0.4)%3=2` → `Server 3`.
...
Поток lookup по порядку:

1. Входящий пакет → `hash(src_ip, dst_ip, src_port, dst_port)` → значение хеша (`Hash Value`).
2. `Hash Value` → `% M` (размер lookup table) → обращение в `Lookup Table`.
3. `Lookup Table` → `server index` → конкретный `Backend Server`.
...
По географии это выглядит так: все пользователи обращаются на один `Anycast IP: 1.2.3.4`, но маршрутизация доводит их до ближайшего `PoP`:

- User (EU) → `PoP EU` (Frankfurt);
- User (US) → `PoP US` (Virginia);
- User (APAC) → `PoP APAC` (Singapore).

Все три `PoP` ведут к общим `Origin Servers`.
...
- В `Pod A` приложение `App A` ходит наружу через свой `Envoy sidecar`.
- Этот `Envoy sidecar` (Pod A) балансирует запросы между двумя инстансами `App B`: в `Pod B1` (со своим `Envoy sidecar`) и в `Pod B2` (со своим `Envoy sidecar`).
- `Control Plane` (`Istio/xDS`) раздаёт конфигурацию всем `Envoy sidecar` — и в Pod A, и в Pod B1, и в Pod B2.
