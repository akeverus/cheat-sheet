...
> [!mcq] Q22. Что такое паттерн Saga и почему именно он применяется для распределённых бизнес-операций в микросервисах вместо классической ACID-транзакции?
>
> - [ ] A. Saga — это длинная распределённая транзакция, которая через 2PC (two-phase commit) синхронно фиксирует изменения во всех сервисах одной командой; компенсации нужны только при сетевой ошибке
>
>     Это противоречит сути Saga. Паттерн появился именно потому, что 2PC не масштабируется в микросервисной архитектуре: координатор становится узким местом, блокировки удерживаются до конца глобальной транзакции, любой медленный участник стопорит всех. Saga, наоборот, отказывается от глобального коммита и работает через цепочку **локальных** транзакций.
>
>     ❌ ПОСЛЕДСТВИЕ: попытка реализовать «Saga через 2PC» приведёт к деградации latency под нагрузкой (XA-locks на Postgres + Kafka недоступны), а под отказом одного участника зависнут все остальные с висящими prepared-транзакциями.
>
> - [x] **B. Saga разбивает бизнес-операцию на последовательность локальных ACID-транзакций в разных сервисах; если шаг N падает, выполняются компенсирующие действия для шагов 1..N−1 в обратном порядке — глобальный rollback невозможен, поэтому делают семантическую компенсацию**
>
>     Это и есть классическое определение Saga (Garcia-Molina, 1987). Каждый сервис коммитит свою локальную транзакцию независимо — никаких распределённых блокировок. Согласованность достигается не атомарностью, а **eventual consistency** через явные компенсации. Это даёт работоспособный паттерн в условиях `database-per-service` + асинхронные брокеры.
>
>     МЕХАНИЗМ: T1 (Создать заказ) → T2 (Резерв товара) → T3 (Оплата) → T4 (Подтвердить). Если T3 падает: выполняется C2 (Освободить резерв) и C1 (Отменить заказ). Каждая Ti — это полноценный коммит в БД своего сервиса; rollback на уровне Saga — это **новые транзакции**, а не откат.
>
>     КОГДА: микросервисы с собственными БД, длительные процессы (оформление заказа, KYC, on-boarding), интеграции с внешними системами (платежи), где `XA/2PC` либо невозможен, либо запрещён по архитектурным принципам.
>
>     ПРЕИМУЩЕСТВО: каждый сервис остаётся автономным, нет распределённых блокировок и SPOF в виде XA-координатора; падение участника не блокирует остальных — Saga просто компенсирует уже сделанное.
>
> - [ ] C. Saga — это синоним Event Sourcing: каждый шаг Saga записывает событие в общий Event Store, и при ошибке Event Store «откатывает» поток событий до предыдущего snapshot'а
>
>     Путаница двух разных паттернов. Event Sourcing — про хранение состояния как последовательности событий **внутри одного агрегата/сервиса**. Saga — про координацию шагов **между сервисами**. Saga может работать поверх Event Sourcing (Axon так и делает), но это разные уровни: ES — формат хранения, Saga — оркестрация процесса. И главное: Event Store **не откатывается** — события immutable.
>
>     ❌ ПОСЛЕДСТВИЕ: ожидание «Saga сама отменит события через rollback» приведёт к тому, что компенсации не будут написаны вовсе; при первой же ошибке оплаты в системе останется висящий заказ с зарезервированным товаром и неконсистентным балансом.
>
> - [ ] D. Saga — это паттерн ретраев на уровне API gateway: при ошибке HTTP-вызова в цепочке сервисов gateway повторяет запрос до успеха, поэтому компенсации обычно не нужны
>
>     Это смесь circuit breaker и retry policy, не имеющая отношения к Saga. Ретраи не помогают, если ошибка — бизнесовая (недостаточно средств на карте, товара нет на складе): сколько ни повторяй, отказ останется. Saga нужна именно для случаев, когда нельзя «просто ещё раз попробовать» — нужно отменить уже сделанные шаги.
>
>     ❌ ПОСЛЕДСТВИЕ: «retry вместо Saga» приводит к storm of retries на сервис оплат и parallel double-charges; деньги списываются дважды, а заказ всё равно не создаётся — придётся вручную разбирать инциденты и делать refunds.
...
> [!mcq] Q23. У вас Saga из 7 шагов с условными ветвлениями (по результату антифрод-проверки разные продолжения) и требованием явной видимости текущего состояния процесса в админке. Что лучше — оркестрация или хореография — и почему?
>
> - [ ] A. Хореография всегда предпочтительнее: она даёт максимальную decoupling и убирает single point of failure в виде оркестратора, поэтому её и применяют в зрелых микросервисных архитектурах
>
>     Универсалистский тезис, противоречащий условиям задачи. Хореография действительно хороша для **простых** flow из 2–3 шагов без ветвлений. Но когда шагов 7+ с условными переходами, поток событий «размазан» по всем сервисам — никто не владеет процессом целиком, и просто понять, на каком шаге сейчас Saga, становится невозможно без сквозного трейсинга.
>
>     ❌ ПОСЛЕДСТВИЕ: «админка статусов Saga» в хореографии превратится в сборную сборку из логов всех 7 сервисов; добавление нового условного шага потребует править N подписчиков и зорко следить, чтобы никто не реагировал на чужие события, иначе появятся duplicate-transitions.
>
> - [ ] B. Оркестрация — потому что у неё нет single point of failure: оркестратор stateless и его можно дёшево горизонтально масштабировать
>
>     Тезис содержит фактическую ошибку. Оркестратор Saga по определению **stateful** — он хранит позицию в процессе для каждого instance (через event log Axon, БД Camunda, history Temporal). Stateless'ом его не сделать, иначе теряется суть. Масштабирование возможно, но через шардирование по `sagaId`, а не как у обычного stateless-сервиса.
>
>     ❌ ПОСЛЕДСТВИЕ: попытка задеплоить «stateless saga orchestrator» в k8s с 5 репликами без шардирования приведёт к race conditions: два инстанса одновременно обработают событие одного `orderId` и пошлют параллельные команды (двойное списание).
>
> - [x] **C. Оркестрация: 7 шагов с условными ветвлениями и требование видимости состояния — классический сценарий для оркестратора; он держит state машину Saga, явно знает текущий шаг и принимает решения о ветвлениях; админка читает state из таблицы saga_instance**
>
>     Это и есть критерий выбора по Chris Richardson: чем сложнее flow и чем больше ветвлений, тем сильнее аргументы за оркестрацию. Центральная state machine (Axon Saga / Camunda BPMN / Temporal workflow) держит текущую позицию, обрабатывает таймауты, делает retry конкретного шага и принимает решение о компенсации. Один источник истины процесса — легко мониторить и менять.
>
>     МЕХАНИЗМ: orchestrator подписан на события сервисов через `@SagaEventHandler`, держит ассоциацию по `sagaId`, при получении события переходит в новое состояние и шлёт следующую команду. Условные ветвления реализуются как `if`/`switch` в коде workflow или явные gateway-узлы в BPMN.
>
>     КОГДА: 4+ шага, условные переходы, нужны таймауты и эскалации (через 2 часа без ответа — отменить), требования аудита/видимости текущего шага, частые изменения процесса (BPM-нотация даёт бизнес-аналитику возможность редактировать flow).
>
>     ПРЕИМУЩЕСТВО: вся логика процесса собрана в одном месте — её можно прочитать, протестировать, замокать; добавление шага — это правка одного класса/диаграммы, а не координация N команд-владельцев подписчиков.
>
> - [ ] D. Разницы нет: с появлением Kafka и idempotent consumer'ов оркестрация и хореография эквивалентны по сложности и выбираются по вкусу команды
>
>     Это упрощение, скрывающее реальный trade-off. Kafka решает доставку, но не решает координацию: в хореографии нет места, где описан весь процесс целиком, и при росте flow появляются циклические зависимости между сервисами («Order Service слушает StockReserved, Stock Service слушает PaymentFailed, Payment Service слушает OrderCancelled...»).
>
>     ❌ ПОСЛЕДСТВИЕ: команда выбирает хореографию «по вкусу» для сложного flow и через год получает graph dependency между 7 сервисами; рефакторинг шага требует синхронных деплоев нескольких команд, а тестировать flow end-to-end можно только в полном окружении.
...
> [!mcq] Q24. В Saga «Создать заказ» шаг ChargePayment успешно списал деньги, но шаг ConfirmOrder упал по таймауту. Saga вызывает компенсацию payment. Какое требование к компенсирующей транзакции здесь критично и почему?
>
> - [ ] A. Компенсация должна откатить локальную транзакцию через `ROLLBACK` — то есть отменить commit в БД payment-сервиса и стереть запись о списании, как будто его не было
>
>     Это фундаментально невозможно в Saga. Локальная транзакция payment **уже зафиксирована** (`COMMIT`) в БД и в платёжном шлюзе — отменить её через `ROLLBACK` нельзя ни технически (commit необратим), ни бизнесово (деньги уже ушли клиенту/в банк). Компенсация — это **новая** операция (refund), а не rollback.
>
>     ❌ ПОСЛЕДСТВИЕ: разработчик пытается реализовать компенсацию через `DELETE FROM payment WHERE id=?` — деньги остаются списанными у клиента (банк об удалении строки в БД не знает), а в системе нет следа транзакции для аудита и сверки.
>
> - [ ] B. Компенсация должна быть атомарной с прямым шагом — `ChargePayment` и `RefundPayment` обязаны выполняться внутри одной распределённой транзакции, иначе теряется consistency
>
>     Прямо противоречит сути паттерна. Компенсация в Saga — это **отдельная** локальная транзакция, выполняемая через секунды/минуты/часы после прямого шага (в зависимости от того, когда упал следующий шаг). Никакой общей транзакции между ними не существует — иначе мы вернулись бы к 2PC, от которого Saga и отказывается.
>
>     ❌ ПОСЛЕДСТВИЕ: попытка обернуть пару (charge, refund) в одну XA-транзакцию приведёт к долгоживущим распределённым lock'ам и блокировке payment-сервиса для всех остальных операций — деградация под нагрузкой.
>
> - [ ] C. Компенсация должна выполняться синхронно сразу после прямого шага и не зависеть от внешних сервисов, чтобы её нельзя было потерять при сбое
>
>     Неверно по двум причинам. Во-первых, компенсация по определению вызывается **после** падения последующего шага — не сразу. Во-вторых, она часто как раз зависит от внешнего сервиса (банк выполняет refund, склад освобождает резерв). Чтобы её не потерять, нужны идемпотентность и retry, а не «выполнить синхронно».
>
>     ❌ ПОСЛЕДСТВИЕ: «синхронная компенсация без внешних зависимостей» означает попытку отменить операцию только в локальной БД, не дёргая шлюз банка — состояние своей таблицы и внешней системы расходятся, и сверка вечером показывает миллион расхождений.
>
> - [x] **D. Компенсация (`RefundPayment`) должна быть идемпотентной — Saga может повторно вызвать её при retry, и повторный вызов должен дать тот же эффект, что и первый; иначе двойной retry приведёт к двойному возврату**
>
>     Это и есть ключевое требование к компенсации в Saga. Сеть ненадёжна, оркестратор перезапускается, события доставляются at-least-once — компенсация может прийти дважды. Идемпотентность гарантирует, что повторный refund одного и того же `paymentId` либо не сделает ничего (refund уже выполнен), либо вернёт сохранённый результат. Дополнительно компенсации делают коммутативными (если возможно) и применяют в обратном порядке прямых шагов.
>
>     МЕХАНИЗМ: payment-сервис хранит таблицу `refund(payment_id PK, refund_id, status)`; при повторном вызове `refund(paymentId)` сначала проверяет, есть ли уже запись с этим `payment_id` — если есть, возвращает её, не дёргая банк. Идентификатор compensation request делают детерминированным (`refund-{paymentId}`), чтобы можно было safely retry.
>
>     КОГДА: всегда — это базовое требование Saga, без идемпотентности компенсаций паттерн не работает в реальном асинхронном мире. Особенно критично для финансовых операций (платежи, переводы), где повторное выполнение видимо клиенту.
>
>     ПРЕИМУЩЕСТВО: позволяет Saga-фреймворку (Axon/Camunda/Temporal) безбоязненно повторять отправку команды компенсации до подтверждения — at-least-once delivery превращается в exactly-once effect.
...
> [!mcq] Q25. В Event Store уже миллионы `OrderCreatedEvent` старого формата, а в коде вы хотите переименовать `customerName` в `firstName + lastName` и добавить поле `currency`. Какая стратегия версионирования событий правильная для production-системы Event Sourcing?
>
> - [ ] A. Переписать все исторические события в Event Store через миграционный скрипт, чтобы они соответствовали новой схеме — это даст консистентный формат и упростит код десериализации
>
>     Это нарушает фундаментальный принцип Event Sourcing — иммутабельность событий. История изменений теряет достоверность: если переписать события «как будто всегда было firstName+lastName», то аудит и легальные ребилды проекций станут невозможны, потому что в реальности этих данных в тот момент в системе не было.
>
>     ❌ ПОСЛЕДСТВИЕ: copy-and-transform на миллионах записей — это часы downtime на write side, риск повредить event log при сбое, и невосстановимая потеря audit trail для compliance (SOX/GDPR audit log должен показывать, как данные **реально** изменялись во времени).
>
> - [ ] B. Создать новый класс `OrderCreatedEventV2` рядом со старым, и в коде агрегата делать `if (event instanceof V1) ... else if (event instanceof V2) ...` — это типобезопасно и не требует трансформации payload'а
>
>     Это «новый тип события» — рабочий, но антипаттерн при частых изменениях. После 3-4 ревизий код агрегата превращается в switch по 4 версиям одного и того же события; добавление 5-й версии требует править все обработчики; рефакторинг становится невозможным. На практике этот подход допустим только для кардинального изменения семантики события, а не для «переименовали поле».
>
>     ❌ ПОСЛЕДСТВИЕ: после 5-6 итераций схемы в коде накапливается 30+ if-else по версиям, любая правка задевает все обработчики, code review становится адом, новые разработчики не понимают, какая версия актуальна.
>
> - [x] **C. Использовать Upcaster — компонент, который при чтении из Event Store трансформирует JSON старой версии в новую (`customerName` → `firstName`+`lastName`, `currency` → default `"RUB"`), при этом события в store остаются неизменными, а агрегат и проекции всегда работают с актуальной схемой**
>
>     Это канонический подход для Event Sourcing. Event Store хранит исторические события **как они были записаны** — иммутабельность сохранена, аудит достоверен. Upcaster — это lazy transformation: преобразование происходит на лету при чтении, в памяти; на диск ничего не пишется. Цепочка upcasters (v1→v2→v3) позволяет поэтапно поднимать версию.
>
>     МЕХАНИЗМ: при load aggregate Axon/EventStoreDB читает raw JSON из таблицы → прогоняет через `EventUpcasterChain` → отдаёт уже актуальную версию payload в десериализатор. Старый Java-класс `OrderCreatedEventV1` можно вообще удалить из codebase — upcaster работает с `JsonNode`, а не с типизированным объектом.
>
>     КОГДА: любые непринципиальные изменения схемы — добавление полей с defaults, переименование, изменение типа, разбиение/объединение полей. Кардинальные изменения семантики (другая бизнес-сущность) лучше делать через новый тип события.
>
>     ПРЕИМУЩЕСТВО: иммутабельность Event Store сохранена, нет downtime на миграцию, schema evolution прозрачна для бизнес-логики, новые разработчики видят только актуальную версию схемы в коде.
>
> - [ ] D. Не делать ничего — десериализатор Jackson сам по умолчанию проигнорирует отсутствующие поля и подставит null/0/empty для новых полей; в коде агрегата надо просто обработать null
>
>     Это «weak schema» подход, который работает только для **добавления** новых nullable полей. В описанном сценарии у нас не только новое поле `currency`, но и **переименование** `customerName` → `firstName+lastName` — Jackson не догадается разбить строку на две части. На выходе получим `firstName=null, lastName=null` и потерянные данные клиента.
>
>     ❌ ПОСЛЕДСТВИЕ: при попытке восстановить заказ из истории все имена клиентов будут пустыми → нарушены печать накладных, отправка email, отображение в UI. Баг проявится только на старых заказах в продакшене и будет крайне сложен для воспроизведения в тестах.
...
> [!mcq] Q26. Вы добавили upcaster, который преобразует `OrderCreatedEventV1` (поле `customerName`) в V2 (`firstName + lastName`). Параллельно в codebase удалили старый Java-класс `OrderCreatedEventV1`. Как корректно реализован upcaster, чтобы это работало в production?
>
> - [ ] A. Upcaster должен сначала десериализовать JSON в объект `OrderCreatedEventV1`, потом скопировать поля в новый `OrderCreatedEventV2`, и наконец сериализовать обратно — это типобезопасно и удобно отлаживать
>
>     Это разрушит подход upcasting: если старый Java-класс `OrderCreatedEventV1` уже удалён из codebase (а это типичная цель — иметь только актуальную схему в коде), десериализация в него невозможна. Upcaster обязан работать с **raw-представлением** (JSON/XML/bytes), а не с типизированным объектом — только так схема может эволюционировать независимо от Java-классов.
>
>     ❌ ПОСЛЕДСТВИЕ: при попытке загрузить старый агрегат после удаления V1-класса приложение упадёт с `ClassNotFoundException`. Чтобы починить, придётся либо вернуть мёртвый класс в codebase навсегда, либо переписывать upcaster — но в Event Store уже миллионы событий, и продакшен лежит.
>
> - [x] **B. Upcaster работает с `IntermediateEventRepresentation` (raw JSON-узел через `ObjectNode`/`JsonNode`), проверяет `canUpcast()` по типу и revision события, в `doUpcast()` манипулирует JSON-узлами (split customerName, добавление default currency), возвращает новый payload с обновлённой revision — старый Java-класс при этом вообще не нужен**
>
>     Это канонический способ. Upcaster — это трансформация в пространстве JSON, не в пространстве типов. Это позволяет: (1) удалять устаревшие Java-классы из codebase, оставляя только актуальную версию; (2) строить цепочки upcasters v1→v2→v3 без копипасты типов; (3) обновлять схему без миграции данных в Event Store.
>
>     МЕХАНИЗМ: `canUpcast()` фильтрует события по `type` и `revision` (через `IntermediateEventRepresentation.getType().getRevision()`). `doUpcast()` получает `JsonNode`, манипулирует им через `ObjectNode.put()/remove()`, вызывает `event.upcastPayload(newType, newRevision, ...)`. Результат — событие с новой revision, которое отдаётся следующему upcaster в цепочке или десериализатору актуального класса.
>
>     КОГДА: всегда, когда схема события меняется и старые события нужно «поднять» до новой версии при чтении. Особенно критично, если старая версия класса должна исчезнуть из codebase.
>
>     ПРЕИМУЩЕСТВО: codebase содержит только актуальные классы; schema migration не требует миграции данных; rebuild проекций работает прозрачно через upcaster chain; цепочки v1→v2→v3 декомпозированы по отдельным upcaster'ам, каждый отвечает за одну ревизию.
>
> - [ ] C. Upcaster надо запускать как batch-job ночью, который пробегается по Event Store, читает события V1 и сохраняет рядом события V2 — так upcasting происходит один раз, а не на каждое чтение
>
>     Это нарушает иммутабельность Event Store: вы добавляете «фантомные» события V2, которых в реальности никогда не происходило, а оригинальные V1 либо остаются (двойная запись), либо удаляются (потеря audit trail). Кроме того, цель upcaster — именно работа на лету, чтобы не делать долгие миграции на больших объёмах данных.
>
>     ❌ ПОСЛЕДСТВИЕ: batch-rewrite миллионов событий — это часы IO-нагрузки на event store, риск рассинхронизации позиций проекций (новые global_sequence у V2 ломают checkpoint'ы), и нарушение audit log для compliance-проверок.
>
> - [ ] D. Достаточно настроить Jackson с `@JsonIgnoreProperties(ignoreUnknown=true)` и default-значениями для новых полей — Jackson сам подхватит старые JSON-документы как V2 без явного upcaster'а
>
>     Это работает только для тривиальных случаев — добавление nullable-поля. В описанном сценарии есть **переименование+разбиение** `customerName` → `firstName + lastName`: Jackson не догадается разбить строку по пробелу, он просто проигнорирует `customerName` как unknown и оставит `firstName=null, lastName=null`. Реальная трансформация семантики невозможна без явного upcaster.
>
>     ❌ ПОСЛЕДСТВИЕ: молчаливая потеря данных при чтении старых событий — все имена клиентов из событий до миграции схемы будут `null/null`. Тесты на новых событиях зелёные, проблема всплывает только на исторических данных в проде.
...
> [!mcq] Q27. Команда внедряет CQRS на новом сервисе заказов и спорит: «надо ли обязательно делать Event Sourcing, или можно оставить обычную PostgreSQL для write side?». Какой ответ правильно отражает соотношение CQRS и Event Sourcing?
>
> - [ ] A. CQRS без Event Sourcing — это анти-паттерн: write-модель обязана хранить состояние как поток событий, иначе read-модели нельзя пересобрать, и идея CQRS теряет смысл
>
>     Это распространённое заблуждение. CQRS — это про **разделение моделей чтения и записи**, не про формат хранения write-модели. Можно держать write-модель в обычной нормализованной PostgreSQL-таблице и публиковать события в Kafka через Outbox Pattern или CDC (Debezium). Read-модели подписываются на этот поток событий и обновляются — пересборка возможна, если события сохраняются (Kafka retention или отдельный event log).
>
>     ❌ ПОСЛЕДСТВИЕ: попытка форсировать ES там, где он не нужен (CRUD-сервис справочников, простые workflows), увеличивает сложность на порядок: команде надо учить snapshots, upcasters, replay, projection rebuild — а выгоды нет, потому что аудит и rebuild read-модели не критичны.
>
> - [x] **B. CQRS и Event Sourcing — независимые паттерны, которые часто комбинируются из-за синергии: ES даёт CQRS естественный поток событий для обновления read-моделей и возможность rebuild проекций из истории; CQRS даёт ES решение проблемы сложных запросов (через проекции, а не через replay). Оба можно применять по отдельности: CQRS без ES — на обычной БД + Outbox, ES без CQRS — но это упирается в производительность запросов и почти не встречается**
>
>     Это правильное понимание соотношения. Каждый паттерн решает свою задачу: CQRS — масштабирование чтения и независимая эволюция моделей; ES — иммутабельный аудит и возможность rebuild состояния. Их соединение даёт мощный комбайн: события из ES естественно публикуются в Event Bus → проекции CQRS обновляются → read-модели готовы для сложных запросов.
>
>     МЕХАНИЗМ КОМБИНАЦИИ: Command → Aggregate → events в Event Store → publisher → проекции CQRS → read-models (PostgreSQL/Elasticsearch/Redis). Если нужен новый view — создаём новую проекцию и реплейим события из ES.
>
>     КОГДА БЕЗ ES: высокая нагрузка на чтение + простая бизнес-логика без необходимости аудита истории (каталоги, справочники, отчёты).
>
>     КОГДА БЕЗ CQRS: маленький домен с очень простыми запросами (только по ID), где replay агрегата быстрее, чем построение проекции — редкий, но возможный кейс.
>
>     ПРЕИМУЩЕСТВО: команда может вводить паттерны постепенно — сначала CQRS на обычной БД, потом, если потребуется аудит, добавить ES. Не нужно «всё или ничего».
>
> - [ ] C. Event Sourcing — это просто реализация CQRS на NoSQL: вместо нормализованных таблиц данные хранятся в MongoDB как документы-события, а CQRS — это термин из мира SQL-баз
>
>     Это смешение разнородных понятий. ES — это **способ моделирования состояния** (как последовательность фактов-событий), он не привязан к NoSQL. Event Store может быть реализован на PostgreSQL (таблица append-only), на EventStoreDB, на Kafka, на Axon Server, или вообще на файловой системе. Аналогично, CQRS никак не связан с типом БД — read-модель может быть на любой технологии.
>
>     ❌ ПОСЛЕДСТВИЕ: при выборе технологии команда привязывается к MongoDB «потому что ES», получает все недостатки NoSQL (отсутствие транзакций, eventual consistency на уровне БД, сложные индексы), хотя реляционная append-only таблица с `aggregate_id+sequence` была бы оптимальнее.
>
> - [ ] D. Эти паттерны взаимоисключающие: CQRS гарантирует мгновенную согласованность через синхронные read-модели, а Event Sourcing вводит eventual consistency, поэтому их нельзя совмещать без потери целостности
>
>     Это неверное утверждение. CQRS **не гарантирует** мгновенную согласованность — наоборот, чаще всего read-модели обновляются асинхронно (через event bus), и eventual consistency — это типичный режим CQRS даже без ES. Они не противоречат друг другу — оба паттерна одинаково совместимы с обоими режимами согласованности (sync inline-projection или async catch-up).
>
>     ❌ ПОСЛЕДСТВИЕ: команда отказывается от связки CQRS+ES, выбирая «или одно, или другое», и теряет ключевую выгоду — единый поток событий, который одновременно даёт аудит и питает проекции. Решения, принятые на ложной предпосылке, потом приходится переписывать.
...
> [!mcq] Q28. Команда пишет интеграционный тест на CQRS/ES-сервис: отправляет команду `CreateOrder` через `CommandGateway.sendAndWait()`, затем сразу делает query `findOrder(id)` через `QueryGateway` и ждёт, что заказ вернётся. Тест периодически падает на CI с «order not found». В чём причина и как правильно тестировать такой сценарий?
>
> - [ ] A. Использовать `@DirtiesContext` после каждого теста, чтобы Spring пересоздал контекст — это сбросит состояние event processors и тесты станут детерминированными
>
>     `@DirtiesContext` ничего не решает в этой ситуации: проблема не в загрязнении контекста, а в **асинхронности обработки события**. После `commandGateway.sendAndWait()` событие опубликовано в Event Store, но `TrackingEventProcessor` обрабатывает его в отдельном потоке — read-модель ещё не обновлена. К тому же `@DirtiesContext` катастрофически замедляет CI: пересоздание контекста занимает 10-30 секунд на каждый тест.
>
>     ❌ ПОСЛЕДСТВИЕ: CI-пайплайн раздувается в разы (вместо 2 минут — 30 минут), тесты всё равно периодически падают, потому что корень проблемы — не контекст, а race condition между write и read side.
>
> - [x] **B. Тест должен явно учитывать eventual consistency: после `sendAndWait` использовать `Awaitility.await().atMost(5, SECONDS).untilAsserted(() -> assertEquals(...))` — это polling до тех пор, пока проекция не догонит, с разумным таймаутом. Для unit-тестов агрегатов отдельно использовать `AggregateTestFixture.given().when().expectEvents()`, а для проекций — подавать событие на handler напрямую и проверять view**
>
>     Это правильный подход для интеграционных тестов CQRS/ES. Природа паттерна — eventual consistency между write и read side, значит тесты должны явно ждать обновления проекции. `Awaitility` (или аналог) даёт читаемый DSL для polling с таймаутом. Для unit-тестов следует разделять уровни: агрегат тестируется через Given-When-Then без шины событий вообще; проекция — изолированно, событие подаётся прямо в `@EventHandler` метод.
>
>     МЕХАНИЗМ ИНТЕГРАЦИИ: `commandGateway.sendAndWait(cmd)` ждёт только подтверждения, что command handler отработал и событие записано в Event Store. Это **не** ждёт обновления read-модели. `Awaitility.untilAsserted(() -> queryGateway.query(...))` polls query side, пока проекция не догонит.
>
>     МЕХАНИЗМ UNIT: `AggregateTestFixture<OrderAggregate>` — Axon-фреймворк для тестов агрегата. `fixture.given(events).when(command).expectEvents(events)` — никакой шины, никакого Spring-контекста, чистая бизнес-логика.
>
>     КОГДА: integration-тесты CQRS/ES — всегда через `Awaitility` или Axon's `SubscriptionQuery`. Unit-тесты агрегатов — через fixture. Unit-тесты проекций — прямой вызов `@EventHandler`.
>
>     ПРЕИМУЩЕСТВО: тесты быстрые (нет лишних перезапусков контекста), стабильные (явный wait вместо `Thread.sleep`), читаемые (Given-When-Then DSL для агрегатов).
>
> - [ ] C. Заменить `TrackingEventProcessor` на `SubscribingEventProcessor` в test-профиле — он работает синхронно в той же транзакции, что и команда, поэтому проекция всегда обновлена к моменту query
>
>     Это работающий, но опасный подход. `SubscribingEventProcessor` делает обработку синхронной в test-конфигурации, и тесты действительно становятся детерминированными. **Однако** в production используется `TrackingEventProcessor` (асинхронный), и тесты больше не воспроизводят реальное поведение: race conditions между write и read side, обработку повторных событий, восстановление checkpoint после рестарта — всё это test-окружение не покрывает. Это создаёт ложное чувство безопасности.
>
>     ❌ ПОСЛЕДСТВИЕ: тесты зелёные, но баги eventual consistency прорываются в продакшен. Например, UI делает query сразу после команды и получает «не найдено» — в тестах это никогда не воспроизводилось, потому что test-конфигурация была синхронной.
>
> - [ ] D. Добавить `Thread.sleep(500)` после `sendAndWait()` — это даст проекции время обновиться, и тесты станут стабильными
>
>     Антипаттерн, который мог бы быть на быстром локальном проекте, но катастрофичен на CI. 500ms иногда мало (CI-агент перегружен, GC-пауза), иногда слишком много (на быстром локалхосте ждать впустую). Каждый flaky-тест приходится «лечить» наращиванием sleep, в итоге набор тестов исполняется минутами вместо секунд. И главное — это не решает проблему, а маскирует её: при первой же задержке всё снова падает.
>
>     ❌ ПОСЛЕДСТВИЕ: CI-пайплайн раздувается до десятков минут, тесты по-прежнему flaky под нагрузкой, команда теряет доверие к тестам и начинает retry-ить падения вместо разбора причин.
...
> [!mcq] Q29. Команда успешно внедрила CQRS/ES в bounded context «заказы». Через полгода Event Store вырос до 800 млн событий, replay агрегата на старте сервиса занимает минуты, поддержка стала сложной. Какая проблема в этом списке наиболее недооценена командами на старте и почему?
>
> - [ ] A. Главная недооценённая проблема — недостаток инструментов: Axon Server и Kafka не справляются с нагрузкой, нужен дорогой EventStoreDB или своё хранилище на C++
>
>     Это технологическая проблема, но не главная. Axon Server и Kafka масштабируются до сотен миллиардов событий при правильной конфигурации (партиционирование, retention, snapshots). Команды, у которых проблема в производительности Event Store, обычно не настроили snapshots — а это решается за один спринт.
>
>     ❌ ПОСЛЕДСТВИЕ: команда вкладывается в смену технологии хранения, тратит квартал на миграцию, но реальные проблемы (eventual consistency UX, идемпотентность, отладка) остаются — потому что они не в инфраструктуре, а в коде и архитектуре.
>
> - [x] **B. Главная недооценённая проблема — отсутствие идемпотентности обработчиков и стратегии для eventual consistency UX: at-least-once доставка означает, что событие может прийти дважды, и без явной дедупликации проекции получают дублированные данные; параллельно пользователь не видит свои изменения сразу и жалуется на «глюки», если UI не учитывает асинхронность read-модели. Эти проблемы концептуальные — их нельзя «починить дополнительным компонентом», их надо предусмотреть в дизайне с самого начала**
>
>     Это правильный ответ. Команды обычно недооценивают именно эти два пункта, потому что они проявляются не в момент запуска, а под нагрузкой и в edge cases: дубликаты приходят редко, но накапливаются; eventual consistency не замечают локально, но пользователи жалуются в проде. Технологические проблемы (объём Event Store, replay-время) решаются известными способами (snapshots, sharding); концептуальные требуют переписывания обработчиков и UI-логики.
>
>     МЕХАНИЗМ ИДЕМПОТЕНТНОСТИ: каждый `@EventHandler` обязан быть идемпотентным — через дедупликацию по event ID (`processed_events` таблица), conditional writes (`WHERE last_seq < event.seq`), или естественную идемпотентность операций (INSERT OR UPDATE, SET-семантика).
>
>     МЕХАНИЗМ EVENTUAL CONSISTENCY UX: optimistic UI updates (показываем заказ как PENDING до подтверждения), polling с таймаутом (`Awaitility` в backend, websocket в frontend), `subscriptionQuery` в Axon (живая подписка на обновление проекции).
>
>     КОГДА: эти проблемы вылезают всегда, как только система переходит на at-least-once messaging (Kafka, RabbitMQ, Axon Server) — а это 99% production-сетапов. Игнорировать их — значит дать им проявиться в проде.
>
>     ПРЕИМУЩЕСТВО: учёт идемпотентности и UX с самого начала избавляет от дорогого переписывания обработчиков спустя полгода, когда дубликаты уже накопились в read-моделях, а пользователи привыкли к «глюкам».
>
> - [ ] C. Главная проблема — высокая сложность миграции с CRUD: без исторических событий ES не работает, поэтому надо «выдумать» начальные события для всех существующих данных, что фактически невозможно
>
>     Это реальная проблема, но не самая недооценённая — её все видят сразу. Решение известно: для существующих сущностей создаются «синтетические» события типа `OrderImportedEvent` с текущим состоянием, и дальше система работает в нормальном режиме ES. Миграция требует времени, но это понятная инженерная задача, а не концептуальная ловушка.
>
>     ❌ ПОСЛЕДСТВИЕ: фокусировка на этой проблеме при выборе внедрить ли CQRS/ES приводит к отказу от паттерна на новых проектах (где миграция вообще не нужна), хотя именно там польза наибольшая.
>
> - [ ] D. Главная недооценённая проблема — несовместимость CQRS с микросервисами: каждый сервис со своим Event Store нарушает single source of truth, поэтому CQRS можно применять только в монолите
>
>     Это в корне неверно. CQRS/ES естественно вписывается в микросервисную архитектуру — каждый bounded context имеет свой Event Store как локальный источник истины, между сервисами передаются «integration events» через Kafka (или Axon Server). Single source of truth существует на уровне отдельного bounded context, а не всей системы. Большинство публичных кейсов CQRS/ES (Netflix, Walmart, eBay) — это микросервисы.
>
>     ❌ ПОСЛЕДСТВИЕ: команда отказывается от CQRS/ES на микросервисах по ложной причине и теряет ключевую выгоду паттерна — независимое масштабирование read/write side и иммутабельный аудит каждого сервиса.
...
> [!mcq] Q30. У вас проекция `OrderAnalyticsProjection` реагирует на `ItemAddedEvent` и инкрементирует счётчик товаров в БД: `UPDATE order_view SET item_count = item_count + ? WHERE order_id = ?`. После сбоя Kafka переотправила несколько событий, и в проекции счётчики выросли вдвое. Какой подход правильно обеспечит идемпотентность?
>
> - [ ] A. Включить exactly-once-доставку в Kafka (`enable.idempotence=true` + транзакции продюсера + `read_committed` у консьюмера) — это гарантия от брокера, что событие дойдёт ровно один раз
>
>     Это распространённое заблуждение. Kafka exactly-once-семантика работает только **внутри** Kafka (producer→broker→consumer) и только при использовании Kafka-транзакций на consumer side с `consumeAndProduce`. Как только consumer пишет в PostgreSQL (внешнюю БД), exactly-once теряется: либо commit БД + commit Kafka успешны (ок), либо БД упала после commit Kafka → событие потеряно, либо БД успешна, а commit Kafka упал → переотправка. Идемпотентность обязана быть в обработчике, а не в брокере.
>
>     ❌ ПОСЛЕДСТВИЕ: команда полагается на «exactly-once Kafka», не делает дедупликацию в обработчике, и при первом же rebalance/restart получает дубликаты в read-модели — баг проявляется редко, но катастрофически (двойные начисления, неправильные отчёты).
>
> - [x] **B. Сделать обработчик идемпотентным на уровне SQL: использовать conditional update с проверкой `last_processed_seq` — `UPDATE order_view SET item_count = item_count + ?, last_seq = ? WHERE order_id = ? AND last_seq < ?`; если `event.seq <= last_seq`, апдейт не применится (`rowsAffected = 0`), повтор игнорируется. Альтернативно — хранить таблицу `processed_events (event_id PK)` и делать INSERT с `ON CONFLICT DO NOTHING` перед основной операцией**
>
>     Это правильный production-grade подход. Идемпотентность достигается на уровне самой операции — БД сама становится барьером против повторов. `last_processed_seq` использует естественный sequence_number события (моноrastущий внутри агрегата) и monotonic условие в WHERE — атомарно за один SQL-вызов. Альтернативный вариант с `processed_events` таблицей универсальнее (работает и для not-monotonic кейсов), но требует extra INSERT и периодической очистки.
>
>     МЕХАНИЗМ CONDITIONAL UPDATE: один SQL `UPDATE ... WHERE last_seq < event.seq` — атомарный, не требует транзакции, работает корректно при concurrency (если параллельно пришли event1 и event2, БД применит их в правильном порядке через WHERE-условие).
>
>     МЕХАНИЗМ DEDUP-TABLE: `INSERT INTO processed_events(event_id) ON CONFLICT DO NOTHING RETURNING event_id` — если RETURNING пустой, событие уже было обработано → выходим; иначе применяем основную операцию в той же транзакции.
>
>     КОГДА: всегда, когда обработчик делает **накопительную** операцию (`+= ?`, `INSERT INTO log`) — там дубликаты опасны. SET-семантика (`UPDATE SET status='SHIPPED'`) идемпотентна сама по себе и не требует доп. защиты.
>
>     ПРЕИМУЩЕСТВО: идемпотентность гарантирована на уровне БД, не зависит от брокера; работает после рестарта, после rebalance, после ручного replay; correlation между event_id и row позволяет легко диагностировать дубликаты.
>
> - [ ] C. Хранить в памяти приложения `Set<UUID> processedEventIds` и проверять перед применением события — это быстро (без обращения к БД) и просто реализовать
>
>     Это работает только в single-instance, single-process сетапе и не переживает рестарт. После перезапуска `Set` пуст, и при replay все события обрабатываются заново как «новые». В multi-instance (а это норма для production) каждый под имеет свой `Set`, и тот же event ID будет обработан N раз — по одному разу каждым подом. Дедупликация обязана быть persistent.
>
>     ❌ ПОСЛЕДСТВИЕ: после первого же rolling restart счётчики в read-модели задвоятся; в multi-pod scaling задвоение постоянное (каждый под считает себя «впервые видящим» event); при росте `Set` память приложения раздувается до OOM.
>
> - [ ] D. Использовать `@Transactional` вокруг `@EventHandler` — Spring обернёт обработку в БД-транзакцию, и при повторе событие просто откатится
>
>     `@Transactional` не имеет отношения к идемпотентности. Транзакция обеспечивает атомарность одной обработки (либо все SQL внутри handler выполнятся, либо ни один), но не предотвращает **повторное** выполнение того же handler с тем же event ID. При повторе будет вторая независимая транзакция, которая успешно закоммитит ещё один `+= 1`.
>
>     ❌ ПОСЛЕДСТВИЕ: команда уверена, что «у нас же @Transactional», и не реализует дедупликацию. При первом же rebalance Kafka счётчики удваиваются, отчёты неправильные, а инженеры две недели ищут причину — потому что в коде «вроде всё правильно».
...
> [!mcq] Q31. У вас агрегат `Order` обновляет три проекции: `OrderListProjection` (UI-таблица), `OrderAnalyticsProjection` (ClickHouse для отчётов) и `OrderNotificationProjection` (email/sms). Аналитическая проекция упала из-за ошибки в коде, и checkpoint не двигается уже час. Какая конфигурация Event Processors в Axon правильно изолирует проекции, чтобы падение одной не блокировало остальные?
>
> - [ ] A. Все три проекции должны быть в одном `@ProcessingGroup("orders")` — это гарантирует, что они обрабатывают события в одном порядке и rebuild делается централизованно
>
>     Это анти-паттерн в данном сценарии. Если все три проекции в одной processing group, у них **общий checkpoint** в Axon: когда `OrderAnalyticsProjection` падает на каком-то событии, processor останавливается, и список заказов в UI перестаёт обновляться — хотя `OrderListProjection` работает корректно. Бизнес-функциональность UI блокируется из-за бага в аналитике.
>
>     ❌ ПОСЛЕДСТВИЕ: падение одной проекции (аналитика, нотификации) парализует все остальные read-моделей. Пользователи не видят свои новые заказы в списке UI, потому что ClickHouse тормозит — это типичный production-incident, который сложно диагностировать.
>
> - [x] **B. Каждая проекция должна быть в собственной `@ProcessingGroup` (`order-list`, `order-analytics`, `order-notifications`) — это даёт независимые `TrackingEventProcessor`'ы с собственными checkpoint'ами в БД. Падение одной не влияет на другие; rebuild каждой делается изолированно через `resetTokens()`; параллелизм настраивается per-group (например, `order-list` — 4 сегмента, `order-analytics` — 1 сегмент)**
>
>     Это правильный production-grade подход. `@ProcessingGroup` в Axon — это unit изоляции для event processing. Каждая группа имеет: собственный checkpoint (token store), собственный thread pool, собственный сегментинг. Падение `order-analytics` не двигает checkpoint только этой группы; `order-list` продолжает обновляться, UI работает.
>
>     МЕХАНИЗМ ИЗОЛЯЦИИ: при бутстрапе Axon создаёт по `TrackingEventProcessor` на каждый `@ProcessingGroup`. Каждый процессор читает один и тот же поток событий из Event Store, но независимо хранит свою позицию в `TokenStore` (таблица `token_entry`). Сбой в одном — локальная проблема, остальные не задеты.
>
>     МЕХАНИЗМ ПАРАЛЛЕЛИЗМА: `TrackingEventProcessorConfiguration.forParallelProcessing(4)` создаёт 4 сегмента — события партиционируются по hash(aggregate_id), каждый сегмент обрабатывает свою долю параллельно. Для аналитики, где важен глобальный порядок, оставляют 1 сегмент.
>
>     КОГДА: всегда, когда у одного агрегата больше одной проекции с разными SLA, разной нагрузкой или разной критичностью. Это норма для production.
>
>     ПРЕИМУЩЕСТВО: устойчивость к локальным сбоям, гибкая настройка параллелизма per-проекция, изолированный rebuild без затрагивания других проекций, простая диагностика (lag метрика per group).
>
> - [ ] C. Использовать `SubscribingEventProcessor` для всех проекций — они работают синхронно в той же транзакции, что и команда, поэтому проекции всегда консистентны с write side, и проблема несинхронности исчезает
>
>     `SubscribingEventProcessor` отрабатывает synchronously, но это создаёт **новые** проблемы. Во-первых, обработка проекций блокирует завершение command handler — latency команд растёт пропорционально количеству проекций (3 проекции → 3x время). Во-вторых, если `OrderNotificationProjection` падает (email-сервер недоступен), отказывает вся команда — пользователь не может создать заказ из-за проблем с email. В-третьих, нет восстановления после рестарта: пропущенные события теряются (нет checkpoint).
>
>     ❌ ПОСЛЕДСТВИЕ: связность write side с инфраструктурой read side катастрофична — любой сбой ClickHouse или SMTP роняет создание заказов. Архитектура CQRS теряет ключевую выгоду — независимость scaling и устойчивости.
>
> - [ ] D. Все проекции в одном @ProcessingGroup, но при падении одной обработчики ошибок (try-catch + log) подавляют exception — это позволит processor двинуть checkpoint и не блокировать другие проекции
>
>     Это «тихая потеря событий» — событие, на котором упала проекция, никогда не будет обработано. Read-модель становится неконсистентной: заказ есть в Event Store, но в `order_analytics` его нет навсегда. Хуже того, баг скрыт от мониторинга (`catch+log` обычно никто не алертит), и проблема всплывает спустя месяцы, когда менеджер замечает странности в отчётах.
>
>     ❌ ПОСЛЕДСТВИЕ: проекция «дрейфует» от Event Store, rebuild через `resetTokens()` потенциально может починить, но к этому моменту бизнес уже принял решения на неверных данных. Лучше явный fail (processor остановлен, алерт инженеру) + изоляция через отдельные ProcessingGroup.
...
> [!mcq] Q32. В проекции `order_list_view` нашлась критическая ошибка: статусы заказов считаются неправильно из-за бага в обработчике `OrderStatusChangedEvent`. Бизнес требует пересобрать read-модель «без даунтайма» — UI должен продолжать работать всё время. Какая стратегия rebuild правильная для production?
>
> - [ ] A. Сделать `TRUNCATE order_list_view` и через Axon `resetTokens()` запустить replay в той же таблице — это очистит проекцию и даст ей перечитать события заново
>
>     Это создаёт даунтайм: пока replay идёт (минуты или часы на миллионах событий), таблица `order_list_view` либо пуста (после TRUNCATE), либо неполна — UI показывает «нет заказов» пользователям. На production это недопустимо: бизнес-функциональность ломается на время rebuild. Кроме того, новые команды, поступающие во время rebuild, тоже попадают в эту же таблицу, но запросы пользователей возвращают неполные данные.
>
>     ❌ ПОСЛЕДСТВИЕ: UI показывает «у вас нет заказов» в течение всего rebuild — пользователи паникуют, support получает шквал тикетов, бизнес теряет доверие. На больших Event Store rebuild может занимать часы, и это часы простоя.
>
> - [x] **B. Blue-Green Rebuild: создать вторую таблицу `order_list_view_v2` и запустить новый `@ProcessingGroup("order-list-v2")` от позиции 0 — старая проекция продолжает обслуживать UI, новая «догоняет» Event Store. Когда lag v2 ≈ 0 (мониторим через `eventStore.lastSequence() - tokenStore.position("order-list-v2")`), атомарно переключаем приложение на v2 (через config flag, SQL VIEW или blue-green deployment). После переключения старую `order-list-v1` останавливаем и удаляем**
>
>     Это правильный production-grade подход. Принцип: новая read-модель строится параллельно со старой, переключение атомарное. UI всё это время работает на старой модели — нулевой даунтайм. Когда v2 догнала «хвост» Event Store, переключение делается одним коммитом конфига или SQL-командой (`ALTER VIEW`).
>
>     МЕХАНИЗМ: в Axon создаём новый `@ProcessingGroup("order-list-v2")` → он автоматически начинает с позиции 0 (token не существует) → читает все исторические события и заполняет `order_list_view_v2` → параллельно `order-list-v1` обрабатывает только новые события. Метрика `event_store.last_sequence - token_store.position("order-list-v2")` показывает оставшийся lag — когда lag = 0, готовы к переключению.
>
>     МЕХАНИЗМ ПЕРЕКЛЮЧЕНИЯ: вариант 1 — Feature Flag (`@Bean OrderQueryRepository = config.useV2() ? v2Repo : v1Repo`); вариант 2 — SQL VIEW (`CREATE VIEW order_list_view AS SELECT * FROM order_list_view_v2; CREATE VIEW order_list_view AS SELECT * FROM order_list_view_v1` — атомарный swap через rename); вариант 3 — blue/green deploy самого сервиса с разной конфигурацией.
>
>     КОГДА: rebuild на production-системах с большим объёмом данных и требованием continuous availability. Особенно важно, если rebuild может занимать часы/дни.
>
>     ПРЕИМУЩЕСТВО: нулевой даунтайм, простой rollback (если v2 показала проблему — переключаем обратно на v1), возможность валидации v2 в shadow-режиме перед переключением (сравнить v1 vs v2 на тех же запросах).
>
> - [ ] C. Использовать SQL-скрипт для прямой коррекции значений статусов в `order_list_view` — это быстрее, чем replay миллионов событий, и не требует отдельной таблицы
>
>     Это снаружи кажется быстрым, но создаёт хрупкость и техдолг. Во-первых, ручной SQL-скрипт повторяет логику обработчика — двойная реализация, расхождения неизбежны. Во-вторых, при следующей правке проекции снова нужен скрипт — и так каждый раз. В-третьих, проекция дрейфует от своего «истинного источника» (Event Store) — теряется ключевая выгода CQRS/ES (read-модель — это deterministic projection from events).
>
>     ❌ ПОСЛЕДСТВИЕ: команда копит «костыли»-скрипты, документации нет, через год никто не помнит, какие коррекции уже применялись. При следующем rebuild возникают конфликты между «runtime events» и «manual corrections», результат непредсказуем.
>
> - [ ] D. Просто ничего не делать с историческими данными — пофиксить баг в коде и забыть; новые события будут обрабатываться правильно, а старые исправятся постепенно как пользователи будут менять заказы
>
>     Это работает только если каждый заказ периодически получает новые события (`OrderStatusChangedEvent`), что в реальности не так. Большинство завершённых заказов в финальном статусе и никогда больше не обновятся — некорректный статус останется навсегда. Бизнес видит неправильные данные в `order_list_view`, отчёты неправильные, decision-making компрометирован.
>
>     ❌ ПОСЛЕДСТВИЕ: бизнес работает на неверных данных, исторические заказы навсегда отображаются с багом, audit/compliance проверки могут выявить расхождение между Event Store (правильным) и read-моделью (с ошибкой) и инициировать дорогостоящее расследование.
...
> [!mcq] Q33. Пришёл GDPR-запрос «right to be forgotten» от клиента `user_42`. В Event Store за 3 года накопилось 50 000 событий, связанных с этим пользователем (заказы, платежи, обращения в support), и события **иммутабельны** по дизайну Event Sourcing. Какая стратегия соответствует GDPR без нарушения целостности event log?
>
> - [ ] A. Просто удалить все события с `user_id = user_42` из Event Store через `DELETE FROM domain_events WHERE payload->>'userId' = 'user_42'` — это самый прямолинейный способ
>
>     Это нарушает фундаментальный принцип Event Sourcing — иммутабельность событий. Последствия: (1) проекции при rebuild не смогут корректно восстановить состояние («заказ есть в `orders_view`, но нет события его создания»); (2) audit trail для регуляторов теряется (SOX, PCI DSS требуют сохранения истории операций); (3) агрегаты других пользователей могут косвенно зависеть от удалённых событий (Saga, межсервисная корреляция).
>
>     ❌ ПОСЛЕДСТВИЕ: rebuild проекций после удаления может «потерять» зависимые данные (если `OrderCreatedEvent` ссылается на `user_42`, а его нет), пройти не получится; audit регулятор не примет, потому что в event log должна быть полная история операций (только PD должны быть удалены, не сами факты).
>
> - [x] **B. Crypto Shredding: персональные данные (имя, email, телефон) шифруются отдельным AES-ключом per-user, ключ хранится в KMS (HashiCorp Vault/AWS KMS) с маппингом `user_id → key_id`. При GDPR-запросе удаляется только ключ из KMS — события в Event Store остаются, но зашифрованные поля становятся **нечитаемы**. При попытке расшифровать получаем `[FORGOTTEN]` или null. Audit log сохранён (видно, что операции были), PD недоступны (ключ уничтожен)**
>
>     Это золотой стандарт для Event Sourcing + GDPR. Принцип: данные иммутабельны, но возможность их прочитать управляется внешним ресурсом (ключом шифрования). Удаление ключа = тейп-в-сейфе без замка к нему. Audit trail полностью сохранён — регуляторы видят, что у пользователя был заказ X в дату Y на сумму Z (если сумма не PD), но имя, email и адрес доставки — `[FORGOTTEN]`.
>
>     МЕХАНИЗМ ШИФРОВАНИЯ: при записи события `OrderCreatedEvent(customerName, address, ...)` сервис `CryptoShreddingService` вызывает `kms.getKey("user:42") → AESKey`, шифрует чувствительные поля, в Event Store записывается `{customerName: "AES256(...)", encryptedKeyId: "user:42"}`.
>
>     МЕХАНИЗМ УДАЛЕНИЯ: GDPR-запрос → `kms.deleteKey("user:42")` → все события в Event Store с этим `encryptedKeyId` становятся нечитаемы. Десериализатор при попытке расшифровать получает `KeyNotFoundException` → возвращает `[FORGOTTEN]`.
>
>     КОГДА: всегда, когда Event Sourcing хранит персональные данные, попадающие под GDPR/CCPA/PD-152-ФЗ. Это де-факто стандарт в индустрии.
>
>     ПРЕИМУЩЕСТВО: иммутабельность сохранена, audit log полный, GDPR соблюдён (PD действительно недоступны), нет миграции данных при каждом запросе на удаление, ключи легко ротируются и аудируются.
>
> - [ ] C. Заменить значения персональных полей в событиях на `[REDACTED]` через batch-job, который пробегает по Event Store и делает `UPDATE` payload — события технически остаются, но PD удалены
>
>     `UPDATE` на events в Event Store нарушает иммутабельность не меньше, чем `DELETE`. Также возникают серьёзные технические проблемы: (1) изменение payload меняет хеш события, ломая любые подписи и контрольные суммы; (2) checkpoint'ы проекций могут «застрять» на mutated events; (3) на больших объёмах данных batch-job — это часы downtime на write side.
>
>     ❌ ПОСЛЕДСТВИЕ: иммутабельность Event Store нарушена → теряется одна из ключевых гарантий ES (детерминированный rebuild); подписи событий для compliance ломаются; rebuild проекций может давать другие результаты до и после redaction-job.
>
> - [ ] D. Никак — Event Sourcing и GDPR несовместимы по определению, для проектов с персональными данными надо отказаться от ES и использовать классический CRUD с soft delete
>
>     Это распространённое заблуждение, ведущее к отказу от полезной технологии. ES и GDPR полностью совместимы при правильной архитектуре — Crypto Shredding, Pseudonymization, и другие техники решают проблему. Многие финтех-компании (банки, платёжные системы), работающие под жёстким compliance, успешно используют ES для аудита.
>
>     ❌ ПОСЛЕДСТВИЕ: команда теряет преимущества ES (аудит, rebuild, temporal queries) на основании ложной предпосылки. CRUD + soft delete даёт хуже compliance (нет полной истории изменений), который при GDPR-аудите тоже надо обеспечивать.
...
> [!mcq] Q34. Разработчик пишет `@Aggregate` в Axon: добавил `@CommandHandler` для `PlaceOrderCommand` и сразу в этом же методе устанавливает `this.status = PENDING; this.orderId = cmd.getOrderId()`, а потом вызывает `AggregateLifecycle.apply(new OrderPlacedEvent(...))`. Через неделю при replay из Event Store состояние агрегата восстанавливается с `status = null`. Почему и как правильно?
>
> - [ ] A. Проблема в Axon: фреймворк не сохраняет события автоматически, нужно явно вызывать `eventStore.append(event)` после `apply()` — добавить эту строчку, и replay заработает
>
>     Это неверно: `AggregateLifecycle.apply(event)` сам публикует событие через Command Bus и сохраняет его в Event Store. Ручной `eventStore.append()` приведёт к двойной записи. Реальная проблема — в другом: при replay Axon **не вызывает** `@CommandHandler` методы (потому что команды — это не история, они уже не воспроизводимы). Он вызывает только `@EventSourcingHandler` методы, применяющие события к состоянию.
>
>     ❌ ПОСЛЕДСТВИЕ: разработчик добавляет ручной `eventStore.append()` → события записываются дважды → следующий replay инвалидирует состояние агрегата ещё хуже, проблема ухудшается.
>
> - [x] **B. Прямое присваивание `this.status = PENDING` в `@CommandHandler` не сохранится в Event Store — оно живёт только в памяти текущей команды. При replay Axon вызывает `@EventSourcingHandler` методы для всех событий из истории и **только** из них восстанавливает состояние. Правильно: в `@CommandHandler` валидировать команду и вызвать `apply(event)`; всё изменение состояния — только в `@EventSourcingHandler public void on(OrderPlacedEvent e) { this.status = PENDING; this.orderId = e.getOrderId(); }`. Тогда и при первом выполнении команды, и при replay состояние восстанавливается одинаково**
>
>     Это правильный паттерн Axon — единственный способ корректной работы Event Sourcing. `@CommandHandler` отвечает за: (1) валидацию команды, (2) бизнес-инварианты, (3) генерацию события через `apply()`. Изменение состояния поля — **только** в `@EventSourcingHandler`. Когда Axon вызывает `apply(event)`, он сразу же вызывает соответствующий `@EventSourcingHandler` на том же экземпляре, поэтому в текущем командном цикле состояние тоже корректно обновляется. При replay из истории только `@EventSourcingHandler` отрабатывают — и состояние восстанавливается идентично.
>
>     МЕХАНИЗМ: при первом выполнении команды Axon: (1) загружает агрегат через replay существующих событий (вызывая `@EventSourcingHandler` для каждого) → (2) вызывает `@CommandHandler` → (3) `apply(event)` публикует событие и одновременно вызывает `@EventSourcingHandler` → (4) сохраняет новое событие в Event Store. При replay (на старте сервиса или при rebuild) шаг 2 пропускается — только цикл replay через `@EventSourcingHandler`.
>
>     МЕХАНИЗМ РАЗДЕЛЕНИЯ: `@CommandHandler` — «что должно произойти» (валидация + emit события); `@EventSourcingHandler` — «как событие меняет состояние». Это разделение позволяет переигрывать историю без знания контекста команды (она уже undefined в прошлом).
>
>     КОГДА: всегда при работе с `@Aggregate` в Axon — других вариантов корректной работы Event Sourcing нет.
>
>     ПРЕИМУЩЕСТВО: replay детерминирован (только события восстанавливают состояние), агрегат сохраняет инварианты при rebuild, легко тестировать через `AggregateTestFixture.given(events).when(command).expectEvents()` без знания истории команд.
>
> - [ ] C. Это баг Axon 4.x — в более новой версии 5.x прямое присваивание полей в `@CommandHandler` сохраняется. Достаточно обновить версию фреймворка
>
>     Это полностью неверно. Поведение `@CommandHandler` vs `@EventSourcingHandler` — это **сознательный дизайн** Event Sourcing, не баг. Команда — это намерение, событие — это факт. История состоит из фактов, а не из намерений. Этот принцип неизменен между версиями Axon и общий для всех ES-фреймворков (EventStoreDB, Marten, NEventStore).
>
>     ❌ ПОСЛЕДСТВИЕ: апгрейд Axon не решит проблему, время потеряно, баг остаётся. Команда продолжает писать код в неправильном паттерне, накапливая техдолг.
>
> - [ ] D. Нужно добавить `@PostPersist` к полям агрегата — Axon будет сохранять прямые присваивания после команды как часть Event Store payload
>
>     Такой механизм отсутствует в Axon. Аннотация `@PostPersist` — из JPA/Hibernate, к Event Sourcing она не имеет отношения. В Axon нет способа автоматически сериализовать «изменения полей агрегата» — это противоречит концепции, что Event Store хранит **события** (named domain facts), а не state diff.
>
>     ❌ ПОСЛЕДСТВИЕ: попытка применить JPA-аннотации в Axon-агрегате сбивает с толку — фреймворк их игнорирует, разработчик не понимает, почему «вроде всё аннотировано», но replay падает.
...
> [!mcq] Q35. Два инстанса сервиса одновременно обработали команды `ConfirmOrder(orderId=42)` и `CancelOrder(orderId=42)`. Оба загрузили агрегат с версией 5, сгенерировали свои события и пытаются записать их в Event Store с `sequence_number=6`. Что должно произойти и какой механизм это обеспечивает?
>
> - [ ] A. Оба события успешно запишутся с `sequence_number=6` — Event Store поддерживает параллельные записи через partitioning, проекции потом сами решат, какое событие применить первым по timestamp
>
>     Это нарушит инварианты агрегата: два конкурирующих изменения одного и того же агрегата на одной и той же версии создадут «вилку» в его истории. Например, заказ окажется одновременно `CONFIRMED` и `CANCELED`. Никакая логика проекций не сможет корректно разрулить это — данные уже потеряли консистентность на write side. Partitioning Event Store не отменяет необходимость в optimistic locking — он решает другую задачу (scaling reads).
>
>     ❌ ПОСЛЕДСТВИЕ: дублирование `sequence_number` ломает агрегатную консистентность; rebuild проекции даёт недетерминированный результат (зависит от порядка обработки конфликтующих событий); бизнес-инварианты («заказ не может быть одновременно подтверждён и отменён») нарушены.
>
> - [x] **B. Должен сработать механизм оптимистичной блокировки: на таблице Event Store стоит уникальное ограничение `UNIQUE(aggregate_id, sequence_number)`. Первый коммит проходит, второй падает с `ConstraintViolationException` → приложение ловит исключение, перезагружает агрегат (теперь у него версия 6), повторно применяет команду (валидация может уже не пройти — например, отменить confirmed-заказ нельзя), и либо успешно делает retry, либо возвращает осмысленную ошибку клиенту**
>
>     Это правильное поведение и базовый механизм всех Event Stores (Axon, EventStoreDB, JPA-based custom). Optimistic locking без явных блокировок: каждый append проверяет, что ожидаемая версия не была кем-то занята. БД сама обеспечивает атомарность через UNIQUE constraint — никакого global lock, никакого distributed lock manager не нужно.
>
>     МЕХАНИЗМ: при `eventStore.append(aggregateId, event, expectedVersion=6)`: (1) выполняется `INSERT INTO domain_events(aggregate_id, sequence_number, ...) VALUES(42, 6, ...)`; (2) если такая пара уже есть → `unique constraint violation`; (3) приложение catches `OptimisticLockingException`, перезагружает агрегат через `eventStore.loadEvents(aggregateId)`, заново применяет команду; (4) валидация может отклонить команду (например, заказ уже отменён — confirm не пройдёт) — тогда возвращаем ошибку клиенту.
>
>     МЕХАНИЗМ RETRY: typical pattern — обернуть command handler в retry policy с jitter backoff (3 попытки, 50-200ms между ними). Большинство race conditions разрешаются за 1-2 retry; если 3 раза подряд — реальный бизнес-конфликт, возвращаем 409 Conflict.
>
>     КОГДА: всегда в production Event Store. Это базовая защита от конкурентных модификаций без дорогих pessimistic locks.
>
>     ПРЕИМУЩЕСТВО: нет distributed locks, нет блокировок чтения, инварианты агрегата сохранены, БД сама работает как координатор через UNIQUE constraint; retry прозрачно решает большинство гонок; реальные конфликты бизнес-логики транслируются в 409 для клиента.
>
> - [ ] C. Нужно использовать pessimistic locking: перед загрузкой агрегата делать `SELECT ... FOR UPDATE` на все его события — это гарантирует, что второй инстанс будет ждать, пока первый закоммитит
>
>     Pessimistic locking работает, но крайне неэффективен для Event Sourcing. (1) `SELECT FOR UPDATE` на нескольких строках держит блокировки до конца транзакции — конкурирующие команды на тот же агрегат полностью сериализуются; (2) на «горячих» агрегатах (популярный заказ) образуется очередь, latency растёт; (3) deadlock возможны, если разные команды блокируют разные строки. Optimistic locking предпочтительнее на 99% сценариев.
>
>     ❌ ПОСЛЕДСТВИЕ: throughput на горячих агрегатах падает на порядок; deadlock в production требует кастомного recovery; долгие транзакции забивают connection pool. Optimistic locking даёт те же гарантии без всего этого.
>
> - [ ] D. Event Store должен использовать distributed lock (Redis/ZooKeeper) перед каждым append — это единственный способ гарантировать атомарность в multi-instance окружении
>
>     Distributed lock — это самый медленный и сложный из вариантов, и он не нужен для Event Store. БД сама обеспечивает атомарность через ACID-транзакции и UNIQUE constraint — внешний lock manager добавит сетевую задержку, точку отказа (если Redis недоступен → весь Event Store недоступен), и необходимость fencing tokens для защиты от false unlocks.
>
>     ❌ ПОСЛЕДСТВИЕ: добавление Redis как обязательной зависимости Event Store ломает изолированность сервиса; падение Redis парализует write side; latency append'а увеличивается на сетевой RTT к Redis (10-50 ms на каждое событие).
...
> [!mcq] Q36. У вас «горячий» агрегат `Cart` с тысячами событий — добавления/удаления товаров за время сессии. Замечено, что load aggregate занимает 2-5 секунд из-за replay всех событий, и latency на UI деградирует. Какая стратегия снапшотов корректна для production?
>
> - [ ] A. Создавать snapshot на каждый append события в Event Store — это даст самую низкую latency загрузки (всегда читается только snapshot, события не нужны)
>
>     Это переворачивает соотношение цены: snapshot per-event делает каждый command-cycle в 2 раза тяжелее (надо записать и событие, и snapshot), при этом большинство снапшотов никогда не понадобятся (агрегат может больше никогда не загружаться). Также snapshot storage растёт линейно с количеством событий — преимущество хранения «один snapshot вместо тысячи событий» исчезает.
>
>     ❌ ПОСЛЕДСТВИЕ: throughput write side падает в 2-3 раза, объём snapshot storage сопоставим с Event Store, при этом большинство snapshot'ов записаны зря; на «холодных» агрегатах overhead снапшотов нивелирует их преимущество.
>
> - [x] **B. Создавать snapshot по порогу количества событий (например, каждые 50-100 событий per aggregate). При load: (1) читаем последний snapshot для aggregate_id (если есть), десериализуем состояние, (2) читаем события от `snapshot_version + 1` до конца, применяем их к состоянию. В Axon это конфигурируется декларативно через `EventCountSnapshotTriggerDefinition(snapshotter, 50)`. Snapshotting лучше делать асинхронно, чтобы не блокировать write-cycle**
>
>     Это правильный production-grade подход. Порог 50-100 — emperical sweet spot: достаточно событий между snapshot'ами, чтобы overhead был минимальным, но при этом replay не более 100 событий — это миллисекунды. Асинхронность снапшотов важна: сам командный цикл не блокируется записью snapshot, инвариант агрегата (события записаны) сохранён даже если snapshot job упадёт.
>
>     МЕХАНИЗМ LOAD: `Optional<Snapshot> snap = snapshotStore.findLatest(aggregateId)` → если есть, десериализуем `state` и получаем `fromVersion = snap.version + 1`; читаем `eventStore.loadEvents(aggregateId, fromVersion)` → применяем события к state через `state.apply(event)`. Если snapshot отсутствует — читаем всю историю с нуля.
>
>     МЕХАНИЗМ TRIGGER: после успешного append события Axon проверяет — выполнилось ли условие триггера (например, `event_count % 50 == 0`). Если да — отправляет задачу в SnapshotExecutor (отдельный пул потоков), который пересоздаёт snapshot. Sync-write не ждёт snapshot.
>
>     КОГДА: всегда для «горячих» агрегатов с длинной историей — корзины, чаты, длительные процессы. Для «холодных» агрегатов с короткой историей (заказ из 5-10 событий) снапшоты не нужны — overhead не оправдан.
>
>     ПРЕИМУЩЕСТВО: load aggregate работает за миллисекунды (snapshot deserialize + до 50 событий replay), throughput write side не страдает (snapshot async), Event Store остаётся истинным источником истины (snapshots — только оптимизация чтения).
>
> - [ ] C. Использовать снапшоты только для агрегатов с историей более 10 000 событий — для меньших объёмов проще сделать replay
>
>     Порог 10 000 событий слишком высокий — replay тысячи событий уже занимает сотни миллисекунд на больших агрегатах. UI с p99 = 800ms — это плохой UX. Правильный порог — десятки-сотни событий, не тысячи. На больших агрегатах (10000+ событий) уже нужно думать не только о снапшотах, но и об архивировании или о разделении агрегата на меньшие части.
>
>     ❌ ПОСЛЕДСТВИЕ: latency UI деградирует на «среднеактивных» агрегатах (500-1000 событий), пользователи жалуются на медленную работу; команда теряет время на оптимизацию запросов вместо включения снапшотов.
>
> - [ ] D. Снапшоты не нужны вообще — replay событий быстрее, чем десериализация snapshot из БД, потому что события можно стримить
>
>     Это эмпирически неверно для production. Replay требует: (1) network IO для каждого события (если БД на другом хосте), (2) JSON-десериализация каждого события, (3) применение к state — это сотни-тысячи мелких операций. Snapshot — это один большой объект, одна десериализация, один network round-trip. На больших агрегатах snapshot в 100-1000 раз быстрее, чем replay.
>
>     ❌ ПОСЛЕДСТВИЕ: команда отказывается от снапшотов на ложной предпосылке, load aggregate занимает секунды, UI тормозит, в итоге всё равно приходится внедрять снапшоты — но уже под давлением инцидентов в проде.
...
> [!mcq] Q37. После рестарта сервиса проекция `order-summary` должна продолжить обработку **с того места, где остановилась**, а не с нуля и не пропустить события. Какой механизм Event Processor правильно это обеспечивает в production?
>
> - [x] **Catch-up Subscription с persistent checkpoint: процессор хранит позицию (`global_sequence` последнего обработанного события) в БД (Axon TokenStore, отдельная таблица `processor_checkpoint`). При старте читает checkpoint → начинает обработку с `last_position + 1`. После каждого события (или батча) обновляет checkpoint в той же транзакции, что и обновление read-модели — это даёт exactly-once семантику на уровне БД-транзакции. В Axon это `TrackingEventProcessor`, в чистой реализации — `SELECT FROM events WHERE global_sequence > checkpoint ORDER BY global_sequence LIMIT N`**
>
>     Это правильный production-grade механизм. Принцип: позиция обработки персистентна и обновляется атомарно с применением события к read-модели. Это даёт устойчивость к рестартам, deploy'ам, OOM kill — после восстановления процессор продолжит с правильного места.
>
>     МЕХАНИЗМ CHECKPOINT: после успешной обработки события и записи в read-модель: `BEGIN; UPDATE order_summary SET ... WHERE ...; UPDATE checkpoint SET position = event.global_seq WHERE processor = 'order-summary'; COMMIT;` — обе операции в одной транзакции, атомарны. Если процесс упал после COMMIT — checkpoint сохранён; до COMMIT — обе записи откатятся, событие будет переобработано (idempotency обязательна).
>
>     МЕХАНИЗМ POLLING: `@Scheduled(fixedDelay = 1000) void process() { List<Event> batch = eventStore.readFrom(checkpoint, 100); batch.forEach(this::applyAndCheckpoint); }` — каждую секунду читаем до 100 новых событий, обрабатываем, сохраняем checkpoint.
>
>     КОГДА: всегда для production проекций; единственный вариант, дающий устойчивость к падениям и возможность rebuild (`UPDATE checkpoint SET position = 0` → процессор читает всё заново).
>
>     ПРЕИМУЩЕСТВО: рестарт прозрачен, нет дубликатов (при идемпотентных обработчиках), rebuild — это просто `checkpoint = 0`, можно параллелить через sharding по `global_sequence % N`, метрика lag тривиальна (`current_event_pos - checkpoint`).
>
> - [ ] B. Synchronous Inline Projection: каждая команда в одной БД-транзакции обновляет и Event Store, и read-модель — checkpoint не нужен, всё консистентно по определению
>
>     Это работает, но имеет ограничения, делающие его непригодным для большинства production-сценариев. (1) Только одна read-модель в той же БД, что и write-side; распределённые проекции (Elasticsearch, ClickHouse, отдельный PostgreSQL) невозможны. (2) Команда становится медленнее на сумму времени обновления всех проекций. (3) Падение проекции (бага, недоступность БД) валит команду — write-side зависит от read-side. (4) Невозможен rebuild — нет истории событий вне Event Store, только текущее состояние проекции.
>
>     ❌ ПОСЛЕДСТВИЕ: scalability ограничена write-side, нельзя добавить новую read-модель без trip to source, нельзя rebuild при изменении схемы — нужно мигрировать БД. На больших системах этот подход становится узким местом и блокирует развитие.
>
> - [ ] C. In-Memory Subscription через `EventBus.subscribe(listener)` — Spring Events + `@Async` обрабатывает события асинхронно прямо из памяти, без БД-checkpoint
>
>     Это работает только в single-instance сетапе и не переживает рестарт — пропущенные события (опубликованные пока сервис был down) теряются навсегда. В multi-instance каждый под получает свою копию события (если bus локальный) или random (если используется Kafka topic), что ломает upper-bound delivery. Без checkpoint в БД нет возможности rebuild проекции с произвольной позиции.
>
>     ❌ ПОСЛЕДСТВИЕ: после рестарта проекция теряет события, накопленные за время downtime; в multi-pod scaling одни события обрабатываются дважды, другие — ни разу; rebuild при изменении схемы невозможен; невозможно восстановить consistency.
>
> - [ ] D. Хранить позицию обработки в Redis с TTL = 1 день — это быстро и не нагружает основную БД
>
>     Хранение checkpoint в Redis даёт два критических риска. (1) Redis по умолчанию не имеет durability как у PostgreSQL — при падении Redis (AOF не синхронизирован) checkpoint может потеряться → проекция начнёт с нуля. (2) TTL = 1 день автоматически уничтожит checkpoint — после долгого даунтайма или редкого события проекция будет считать себя «новой» и реплейнет всё с нуля, давая дубликаты в read-модели. Checkpoint должен быть persistent, без TTL, и желательно в той же БД, что и read-модель (для atomic transaction).
>
>     ❌ ПОСЛЕДСТВИЕ: периодическое (раз в день) задвоение событий в read-модели; падение Redis приводит к полному реплею проекции на больших объёмах данных — часы downtime и дубликатов.
...
> [!mcq] Q38. Пользователь нажал «Создать заказ», UI редиректит на страницу заказа, но read-модель ещё не обновилась — пользователь видит «Заказ не найден». Какая стратегия UX корректно решает eventual consistency в CQRS, не ломая бизнес-логику?
>
> - [ ] A. Сразу после `commandGateway.sendAndWait()` сделать `Thread.sleep(2000)` в API-handler — за 2 секунды проекция точно успеет обновиться, и UI получит данные
>
>     Это переносит проблему с UX на API latency. `sleep(2000)` блокирует поток обработчика на 2 секунды → throughput сервиса деградирует пропорционально (если было 100 RPS, станет 0.5 RPS на сценарии создания заказа), а пользователь всё равно ждёт 2 секунды каждый раз — UX не лучше. Кроме того, на медленной CI/staging 2 секунд может не хватить, на быстром локалхосте — это бесполезное ожидание.
>
>     ❌ ПОСЛЕДСТВИЕ: производительность API катастрофически падает под нагрузкой; пользователи всё равно ждут видимое время; на разных средах sleep либо мал, либо чрезмерен — нужны разные значения, поддержка превращается в кошмар.
>
> - [x] **B. Optimistic UI: после нажатия «Создать» UI сразу показывает заказ в состоянии «PENDING/обрабатывается» (без запроса к API за подтверждением); фоновая команда отправляется → при успехе UI обновляется на финальный статус, при ошибке — откатывается и показывает alert. Для критичных сценариев (платежи) дополнительно используется `subscriptionQuery` Axon — живая подписка через WebSocket, которая push'ит обновление projection моментально, как только оно появилось. Альтернативно — Read-Your-Writes pattern: API endpoint после команды делает `Awaitility.until()` polling read-модели с таймаутом 1-2 сек, в течение которого ждёт обновление**
>
>     Это правильный production-grade подход. Принцип: UX не должен видеть eventual consistency как баг — она маскируется через UI-патерны и push-обновления. Optimistic UI работает мгновенно, потому что не ждёт server roundtrip; subscription query даёт push-уведомление через WebSocket, минуя задержку polling; Read-Your-Writes — компромисс, когда нужна простота backend без UI-сложности.
>
>     МЕХАНИЗМ OPTIMISTIC UI: пользователь кликает «Создать» → UI создаёт local представление заказа в кеше (Redux/MobX/Pinia) со статусом PENDING → асинхронно POST /orders → при 200 OK обновляет local state на «CREATED»; при ошибке — удаляет и показывает toast «Не удалось создать».
>
>     МЕХАНИЗМ SUBSCRIPTION QUERY: `queryGateway.subscriptionQuery(new FindOrderQuery(id), Order.class, Order.class)` создаёт WebSocket-подобный канал; UI делает initial query → если данных нет, подписывается на обновления; как только проекция обновится, push приходит мгновенно (миллисекунды).
>
>     МЕХАНИЗМ READ-YOUR-WRITES: `Awaitility.await().atMost(2, SECONDS).until(() -> queryService.find(orderId).isPresent())` в API-методе — простой, но блокирующий thread (хуже для throughput, лучше для простоты).
>
>     КОГДА: всегда в production CQRS с асинхронными проекциями. Конкретный выбор зависит от UX-критичности и сложности frontend.
>
>     ПРЕИМУЩЕСТВО: пользователь не видит «глюков», throughput API не страдает, scalability сохранена; есть разные уровни сложности (Optimistic UI требует frontend-логики, R-Y-W — только backend); push через subscription query даёт near-real-time UX.
>
> - [ ] C. Сделать команды синхронными — обновлять read-модель в той же транзакции, что и Event Store; так eventual consistency исчезает, и UI всегда видит актуальные данные
>
>     Это убивает ключевые преимущества CQRS. (1) Команда блокируется на время обновления всех проекций — latency возрастает кратно количеству проекций; (2) write-side становится зависим от availability read-side (упал ClickHouse → нельзя создать заказ); (3) невозможно добавить новую проекцию без переписывания command-handler'ов; (4) масштабируемость read-side ограничена write-side. По сути, это анти-CQRS — у вас просто один монолитный сервис с двумя представлениями данных.
>
>     ❌ ПОСЛЕДСТВИЕ: команда теряет независимость scaling read/write, изолированность сервисов нарушена; ситуативный рост нагрузки на read (например, маркетинговая кампания) роняет write; невозможно ввести новые виды read-моделей без полного редизайна.
>
> - [ ] D. Никак не объяснять пользователю — eventual consistency это техническая деталь, пользователь не должен про неё знать. Достаточно показать loading-spinner на 5 секунд, чтобы проекция точно успела обновиться
>
>     5-секундный спиннер — это плохой UX даже без объяснений. По стандартам Nielsen Norman Group, любое ожидание дольше 1 секунды воспринимается как «приложение тормозит». Пользователи разучились ждать — конкуренты дают мгновенный feedback, и ваш сервис выглядит сломанным. К тому же 5 секунд не гарантирует — иногда нужно 10, иногда хватило бы 200ms, спиннер всегда показывается фиксированное время.
>
>     ❌ ПОСЛЕДСТВИЕ: потеря пользователей на critical path (создание заказа); кажущаяся медленность приложения по сравнению с конкурентами; команда поддержки получает жалобы «всё тормозит», хотя backend работает нормально.
...
> [!mcq] Q39. Запускается новый bounded context «Order Processing» с цепочкой шагов: создать заказ → резервировать инвентарь → создать платёж → подтвердить заказ. На любом шаге может быть отказ (нет товара, нет денег) → нужна компенсация предыдущих шагов. Какая реализация Saga в CQRS правильна для production?
>
> - [ ] A. Хореография через цепочку EventListener'ов: Order Service публикует `OrderCreated` → Inventory Service слушает и публикует `InventoryReserved` → Payment Service слушает и публикует `PaymentCreated` → Order Service слушает и подтверждает заказ. Каждый сервис автономен и не знает о других; компенсация — отдельные обработчики на failure-events
>
>     Хореография работает, но в этом конкретном сценарии она хуже. (1) Сценарий имеет чёткую sequence (4 шага в строгом порядке) и условные ветвления (что делать если нет товара? компенсировать заказ; если нет денег? компенсировать резерв и заказ) — это типичная задача для оркестратора, в хореографии логика размазана по 4 сервисам, каждый знает свой кусок «когда какое событие порождает какое следующее». (2) Видимость потока: бизнес-аналитик хочет видеть «где сейчас Saga 42?» — в хореографии нет одного места, где это можно посмотреть, надо обходить все сервисы и собирать. (3) Изменение цепочки (добавить шаг «антифрод» между резервом и платежом) требует править несколько сервисов.
>
>     ❌ ПОСЛЕДСТВИЕ: добавление шагов в Saga требует координированных изменений в нескольких сервисах; отладка проблем разбросана по логам разных команд; визуализация состояния процесса невозможна без построения отдельного monitoring-сервиса, агрегирующего события от всех участников.
>
> - [x] **B. Оркестрация через `@Saga` в Axon: один Saga-класс `OrderSaga` владеет всем сценарием — `@StartSaga` на `OrderCreatedEvent` → отправляет `ReserveInventoryCommand`; на `InventoryReservedEvent` → отправляет `CreatePaymentCommand`; на `PaymentCreatedEvent` → `ConfirmOrderCommand` и `@EndSaga`. На failure-events явные компенсации: `PaymentFailedEvent` → `ReleaseInventoryCommand` + `CancelOrderCommand`. State Saga (поля `orderId`, `paymentId`, `reservationId`) хранится в `SagaStore`, корреляция через `@SagaEventHandler(associationProperty = "orderId")`**
>
>     Это правильный паттерн для чёткой sequence с условными ветвлениями. Преимущества для данного сценария: (1) весь процесс читается в одном классе — обзор + debug проще; (2) условная логика (если нет денег → компенсировать инвентарь) явная в одном месте; (3) изменение цепочки = изменение одного класса; (4) state Saga персистентен — переживает рестарты, можно поднять статус «где находится Saga 42 сейчас» одним запросом.
>
>     МЕХАНИЗМ AXON SAGA: фреймворк сам создаёт экземпляр `OrderSaga` при `@StartSaga`, сохраняет его state в `saga_entry` таблице, и при каждом релевантном событии (по `associationProperty`) загружает state, вызывает соответствующий `@SagaEventHandler`, сохраняет обновлённый state. `commandGateway.send()` отправляет команду в другой сервис через Command Bus (Axon Server или Kafka).
>
>     МЕХАНИЗМ КОМПЕНСАЦИИ: при `PaymentFailedEvent` Saga отправляет команды компенсации в обратном порядке — `ReleaseInventoryCommand` → `CancelOrderCommand`. Каждая компенсация — отдельная бизнес-операция, не «откат», а семантически обратное действие.
>
>     КОГДА: оркестрация — для чётких многошаговых процессов с условными ветвлениями и требованием визуализации состояния (заказы, KYC, on-boarding, биллинг). Хореография — для простых независимых реакций (notification on event).
>
>     ПРЕИМУЩЕСТВО: централизованная логика процесса, лёгкая отладка и мониторинг, простое изменение сценария, явные компенсации; Saga state перенастраивается через `@SagaEventHandler` без миграции БД.
>
> - [ ] C. Использовать 2PC (two-phase commit) через распределённую транзакцию между Order DB, Inventory DB и Payment DB — это даёт ACID-гарантии и компенсации не нужны
>
>     2PC в микросервисной архитектуре с отдельными БД даёт катастрофические проблемы. (1) Любой медленный участник стопорит всех — координатор держит prepared-транзакции на всех узлах; (2) Падение участника требует ручного recovery prepared-транзакций; (3) Не все хранилища поддерживают XA (Kafka вообще не XA-compliant). Saga появилась именно как альтернатива 2PC для распределённых систем — попытка использовать 2PC обнуляет смысл микросервисов.
>
>     ❌ ПОСЛЕДСТВИЕ: throughput системы определяется самым медленным участником; SPOF в виде координатора; в случае его падения — manual recovery с риском потери данных или дубликатов; невозможно интегрировать NoSQL/Kafka/внешние API.
>
> - [ ] D. Реализовать Saga как стейт-машину в одном Order Service, где все шаги — это методы одного класса, синхронно вызывающие другие сервисы через HTTP: при ошибке вызывать сервисы компенсации тоже синхронно
>
>     Это анти-паттерн: одна синхронная цепочка HTTP-вызовов. (1) latency = сумма latency всех шагов (если каждый 100ms, итог 400ms+); (2) любой timeout/ошибка ломает всю цепочку, retry дорогой (надо повторить всё); (3) при недоступности сервиса инвентаря Order Service не может создать заказ; (4) compensating actions тоже синхронны — двойная цепочка вызовов на rollback. Saga из book Hohpe — это про асинхронность и независимость, не про синхронную цепочку.
>
>     ❌ ПОСЛЕДСТВИЕ: latency недопустима для UI (создание заказа 1-2 секунды); availability системы = product availability всех сервисов; retry-storm при сбоях; нет возможности параллелизации независимых шагов.
...
> [!mcq] Q40. После года работы Event Sourcing команда добавляет в `UserRegisteredEvent` поле `email`, которое раньше не было, и разбивает `name` на `firstName + lastName`. В Event Store 5 млн старых событий без `email` и со строковым `name`. Какое правило проектирования событий правильно обеспечивает обратную совместимость на годы вперёд?
>
> - [ ] A. Каждое изменение схемы должно ломать обратную совместимость — добавлять `@Revision` и обязательное поле, чтобы старые события явно «не работали» и команда вынужденно их мигрировала
>
>     Это анти-паттерн. (1) Иммутабельность Event Store означает, что старые события мигрировать нельзя; (2) «обязательное» поле в новой версии — это null/missing в старых событиях, и десериализатор упадёт на чтении; (3) намеренная ломка совместимости приводит к downtime каждый раз при добавлении поля. Хорошее правило — наоборот: schema evolution должна быть совместимой по умолчанию, ломающие изменения — это исключение, требующее отдельной процедуры (новый тип события или сложный upcaster).
>
>     ❌ ПОСЛЕДСТВИЕ: каждое добавление поля = production-инцидент с replay errors; команда боится менять схему и накапливает технический долг; rebuild проекций после миграции невозможен (старые события не парсятся).
>
> - [x] **B. Правила хорошего тона: (1) добавлять только nullable/optional поля с default-значениями — это backward-compatible изменение, старые события парсятся без upcaster; (2) никогда не удалять и не переименовывать существующие поля без upcaster — это сразу ломает старые события; (3) изменение типа поля (`String` → `Long`) требует upcaster; (4) всегда указывать revision события (`@Revision("2")` в Axon) — даже если upcaster пока не нужен, это позволит его добавить в будущем без переписывания истории. Кардинальные изменения семантики делать через **новый тип события** (`UserRegisteredEventV2`), а не через перегрузку старого**
>
>     Это правильный набор правил schema evolution для Event Sourcing. Принцип: каждое изменение должно быть либо backward-compatible (старые события парсятся через weak schema), либо обработано upcaster'ом, либо введено как новый тип события. Прямые мутации существующих полей запрещены.
>
>     ПРАВИЛО 1 (nullable additions): добавление поля `email` в `UserRegisteredEvent` — пишем как `Optional<String> email` с default null. Jackson `@JsonIgnoreProperties(ignoreUnknown=true)` подхватит старые события без `email`, обработчики работают с `email.orElse("")`.
>
>     ПРАВИЛО 2 (rename via upcaster): переименование `name` → `firstName + lastName` — это не nullable addition, это semantic change. Делаем через upcaster, который при чтении разбивает старое `name` по пробелу и заполняет два новых поля.
>
>     ПРАВИЛО 3 (semantic change → new event type): если бизнес-смысл события радикально меняется (например, было `OrderCreated`, стало `OrderRequested` + `OrderApproved` — разделение на два этапа), правильно ввести новые типы событий и в коде агрегата отрабатывать оба «старая модель» + «новая модель».
>
>     КОГДА: всегда при дизайне новых событий — закладывать совместимость с самого начала. Use `@Revision`, держать поля nullable, избегать union types.
>
>     ПРЕИМУЩЕСТВО: годы schema evolution без необходимости миграции данных; rebuild проекций работает на всей истории; новые версии backward-compatible с минимумом upcaster кода; команда не боится менять схему — есть прозрачный процесс.
>
> - [ ] C. Не использовать `@Revision` вообще — Jackson автоматически разрулит любые изменения схемы через reflection и default-значения, лишний синтаксис не нужен
>
>     Без `@Revision` (или аналога в других фреймворках) у вас нет способа узнать, **какую версию события вы читаете**, чтобы применить правильный upcaster. Jackson + reflection справится только с тривиальными случаями (добавление nullable поля), но при первой же rename/type-change миграции у вас нет механизма поднять старую версию до новой — все исторические события ломаются разом.
>
>     ❌ ПОСЛЕДСТВИЕ: первая же нетривиальная schema evolution требует downtime на миграцию Event Store или ввод сложной логики «угадывания» версии по форме payload; через 2-3 ревизии код десериализации становится спагетти из if-else.
>
> - [ ] D. Использовать Protobuf или Avro вместо JSON — у них встроенная schema evolution и upcaster не нужен
>
>     Protobuf и Avro действительно дают встроенную forward/backward compatibility для добавления nullable полей и оптимизированы по размеру, но они **не решают** задачу upcasting. Когда вам нужно разбить `name` на `firstName+lastName` — никакой бинарный формат сам не догадается, как это сделать; нужна явная логика трансформации. Также Protobuf-схемы требуют отдельной инфраструктуры (schema registry), что усложняет setup для небольших команд.
>
>     ❌ ПОСЛЕДСТВИЕ: переход на Protobuf даст оптимизацию размера и базовую совместимость, но не отменяет необходимость в upcaster'ах при нетривиальных изменениях; затраты на schema registry и инструментарий могут не окупиться выгодой; разработчики думают «у нас же Protobuf, всё совместимо» и не реализуют upcaster, накапливая баги.
...
> [!mcq] Q41. Read-модель `orders_view` нужно пересобрать после изменения схемы (добавлена колонка `total_with_tax`). В Event Store 200 млн событий, rebuild занимает 6 часов. Бизнес требует zero-downtime — UI работает 24/7. Какая стратегия rebuild правильна и как её реализовать?
>
> - [ ] A. Остановить приложение на 6 часов в нерабочее окно (например, в воскресенье ночью), сделать `TRUNCATE orders_view`, запустить rebuild с нуля, после завершения поднять приложение — это самая простая стратегия
>
>     Это нарушает требование zero-downtime. Даже если у бизнеса есть «нерабочее окно», на современных глобальных сервисах оно фактически не существует — пользователи из других часовых поясов всё ещё пытаются работать с системой. Кроме того, 6 часов downtime — это нарушение типичных SLA (99.9% доступности = 8.76 часов downtime в год, 6 часов одного rebuild съедают всё годовое budget'е).
>
>     ❌ ПОСЛЕДСТВИЕ: SLA нарушен одним rebuild'ом, бизнес теряет доход на 6 часов, пользователи в чужих часовых поясах паникуют; rollback при ошибке в rebuild — ещё 6 часов downtime; команда боится rebuild'ов и копит технический долг в проекциях.
>
> - [x] **B. Blue-Green rebuild с метрикой lag и переключением через config flag: (1) создаём новую таблицу `orders_view_v2` и новый `@ProcessingGroup("orders-v2")` с собственным checkpoint от позиции 0; (2) запускаем v2 параллельно с v1 — UI продолжает работать на v1; (3) мониторим `lag = eventStore.lastGlobalSequence() - tokenStore.position("orders-v2")` через Prometheus метрику `projection.lag{name="orders-v2"}` (alerting если lag не убывает); (4) когда lag ≈ 0 (последние тысячи событий), feature flag `orders.read.useV2 = true` атомарно переключает QueryHandler на новую таблицу; (5) даём день на наблюдение → если ок, останавливаем v1 processor и `DROP TABLE orders_view_v1`. Rollback тривиален — возвращаем flag в `false`. На multi-region инсталляциях rebuild можно делать в read-replica и потом swap'ить через DNS**
>
>     Это золотой стандарт rebuild'а в production. Принцип: новая read-модель строится **рядом** со старой, переключение атомарное, rollback моментальный. UI всё это время работает на старой модели. Метрика lag даёт точное понимание прогресса — не «должно скоро закончиться», а «осталось N событий, ETA 23 минуты».
>
>     МЕХАНИЗМ PARALLEL BUILD: новая ProcessingGroup в Axon (или новая consumer group в Kafka) имеет свой checkpoint, начинающийся с 0. Она читает тот же поток событий, что и старая, но пишет в `orders_view_v2`. Старая ProcessingGroup продолжает работать как обычно — на неё это не влияет.
>
>     МЕХАНИЗМ ATOMIC SWITCH: вариант 1 — Feature Flag через Spring `@ConditionalOnProperty` или Unleash (`if (flagService.isEnabled("orders.read.useV2")) repo = v2Repo`); вариант 2 — `ALTER VIEW` (создан VIEW поверх таблиц, swap через `CREATE OR REPLACE VIEW`); вариант 3 — DNS/Service Mesh routing на другой Kubernetes deployment.
>
>     МЕХАНИЗМ МОНИТОРИНГА: `meterRegistry.gauge("projection.lag", Tags.of("name", "orders-v2"), lag)` → Grafana dashboard показывает кривую убывания lag; alerting на `lag.rate < 0` (lag растёт быстрее, чем убывает — что-то сломалось).
>
>     КОГДА: всегда для rebuild'ов на production-системах с continuous availability требованием. Особенно при больших объёмах данных (10M+ событий).
>
>     ПРЕИМУЩЕСТВО: нулевой даунтайм, моментальный rollback через feature flag, прозрачный прогресс через метрики, возможность shadow-валидации (сравнить v1 и v2 на тех же queries перед swap), параллельный rebuild не нагружает write-side.
>
> - [ ] C. Запустить SQL-скрипт, который напрямую обновит `orders_view` с расчётом `total_with_tax` через JOIN с таблицей налогов — это в 100 раз быстрее, чем replay 200 млн событий
>
>     SQL-патч даёт мгновенный результат, но создаёт критические проблемы. (1) Read-модель дрейфует от Event Store как от источника истины — следующий rebuild через replay даст другие значения (без учёта прямого patch); (2) логика расчёта `total_with_tax` дублируется (в SQL-скрипте и в EventHandler) — расхождения неизбежны; (3) аудит и compliance не примут «прямые правки read-модели»; (4) патч одноразовый — следующее изменение схемы снова требует патч.
>
>     ❌ ПОСЛЕДСТВИЕ: команда копит «костыли» в виде SQL-патчей вместо архитектурного решения; при следующем rebuild результаты не совпадают с тем, что было после патча; теряется одно из ключевых свойств CQRS/ES — детерминированность read-модели от истории событий.
>
> - [ ] D. Делать rebuild постепенно через batched UPDATE: каждые 5 минут джоба обрабатывает 1000 ещё не обновлённых записей в `orders_view`, добавляя `total_with_tax` — за неделю всё обновится без даунтайма
>
>     Это вариант incremental rebuild, но он плох по нескольким причинам. (1) Не использует Event Store как источник истины — пишет «лоскутный» апдейт прямо в read-модель, что нарушает архитектуру; (2) Неделя rolling-rebuild означает целую неделю несогласованных данных в read-модели — половина заказов с новой колонкой, половина без; (3) Невозможно правильно обрабатывать новые события во время rolling rebuild — обработчик не знает, какие записи уже мигрированы; (4) Rollback невозможен — patches уже распределены по миллионам записей.
>
>     ❌ ПОСЛЕДСТВИЕ: неделя UI-инконсистентности (одни заказы с tax, другие без); невозможен rollback при обнаружении бага в логике расчёта; следующий rebuild через replay перетрёт все patches; команда теряет контроль над состоянием read-модели.
