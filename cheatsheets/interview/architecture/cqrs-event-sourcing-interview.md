...
**CQRS** (Command Query Responsibility Segregation) -- архитектурный паттерн, который разделяет операции записи (команды) и чтения (запросы) на две независимые модели. Вместо одной модели, обслуживающей и то и другое, появляются отдельная модель записи и отдельная модель чтения.
...
- **Независимое масштабирование** -- нагрузка на чтение обычно в 10-100 раз выше, чем на запись. С раздельными моделями можно поднять десяток read-реплик, не трогая write-сторону.
- **Оптимальные структуры данных** -- модель записи нормализована ради консистентности, модель чтения денормализована ради скорости (данные уже собраны под конкретный запрос, без JOIN-ов).
- **Чистое разделение ответственности** -- команды валидируют бизнес-правила и меняют состояние, запросы только возвращают данные. Логика чтения и записи перестаёт переплетаться.
- **Простор для сложной бизнес-логики** -- запись может опираться на DDD-агрегаты с инвариантами, а чтение -- на плоские проекции, не таща доменную модель в каждый SELECT.

**Контекст:** термин ввёл Грег Янг (Greg Young) в 2010 году, развив принцип CQS Бертрана Мейера до уровня архитектуры.
...
Короткий ответ: **CQS** -- про дисциплину метода, **CQRS** -- про архитектуру системы. CQRS вырос из CQS, подняв тот же принцип «разделяй чтение и запись» на несколько уровней выше.

**CQS** -- принцип проектирования интерфейсов: метод либо команда (меняет состояние, возвращает `void`), либо запрос (возвращает данные без побочных эффектов), но не то и другое сразу. Это делает поведение предсказуемым: вызов запроса гарантированно ничего не ломает.

**CQRS** -- тот же водораздел, но уже между объектами и моделями: отдельные классы под команды и запросы, разные модели данных, потенциально разные хранилища. Можно следовать CQS и при этом не применять CQRS; обратное -- почти всегда подразумевает и CQS.
...
Две модели решают противоположные задачи, поэтому и устроены по-разному: write-сторона отвечает за корректность изменений, read-сторона -- за быструю выдачу готовых данных. Между ними поток событий: write публикует факты изменений, read их применяет и обновляет свои представления.
...
Все плюсы вытекают из одной идеи: убрав необходимость обслуживать чтение и запись одной структурой, мы получаем свободу оптимизировать каждую сторону под её реальную нагрузку.
...
Главная плата за CQRS -- сложность: вместо одной модели появляются две, между ними асинхронная связь, и вся эта инфраструктура требует поддержки. Конкретные следствия:

- **Eventual consistency** -- read-модель отстаёт от write-модели, и пользователь может не сразу увидеть свои изменения. Это не баг, а свойство паттерна, с которым придётся работать на уровне UX.
- **Рост сложности** -- две модели, механизм синхронизации, дополнительная инфраструктура (брокер, проекторы) -- всё это нужно спроектировать и эксплуатировать.
- **Дублирование данных** -- одни и те же данные лежат в нескольких формах (write + проекции).
- **Сложность отладки** -- путь данных «команда -> событие -> проекция» проходит через несколько компонентов; без correlation ID разобраться, где потерялись данные, тяжело.
- **Операционные затраты** -- больше движущихся частей для мониторинга и поддержки.
- **Обработка ошибок** -- сломавшаяся проекция требует механизма replay/retry, иначе read-модель «застрянет».
- **Порог входа** -- команде нужно понимать паттерн; для пришедших из CRUD это смена парадигмы.

**На собеседовании важно** показать, что вы видите не только плюсы, но и эту цену. CQRS -- не серебряная пуля: он оправдан там, где выигрыш в масштабируемости и гибкости перевешивает добавленную сложность.

## Q6. Когда стоит и не стоит применять CQRS?

Ориентир простой: CQRS оправдан, когда сложность домена или асимметрия нагрузки уже больно бьют по обычной модели. Если такой боли нет -- его сложность не окупится.
...
**Event Sourcing** -- паттерн, при котором источником истины является не текущий снимок состояния, а полный упорядоченный журнал неизменяемых событий. Текущее состояние объекта -- это производная величина: оно вычисляется заново, применяя все его события по порядку.
...
**Event Store** -- специализированное хранилище, оптимизированное под append-only запись и последовательное чтение событий по агрегату.

Ключевое отличие от обычной БД: здесь нет UPDATE и DELETE. Запись только добавляется (INSERT), а строки никогда не меняются. Поэтому Event Store хранит не текущее состояние, а всю его историю, и это меняет всё -- от способа конкурентного доступа до типов запросов.
...
Состояние не хранится -- оно восстанавливается через **replay**: создаём пустой агрегат и прокручиваем все его события по порядку, каждое из которых сдвигает состояние на один шаг. Сумма этих шагов и есть текущее состояние.
...
**Важно:** метод `apply` обязан быть чистым -- только мутация внутреннего состояния, без побочных эффектов (вызовов внешних систем, записи в БД, отправки писем). Иначе при каждом восстановлении агрегата эти эффекты сработают повторно: отправите письмо столько раз, сколько событий проиграете.
...
**Важно:** снапшот -- это производный кэш, а не источник истины. Источник истины всегда события; снапшот можно в любой момент удалить и пересоздать. Поэтому его наличие никогда не должно влиять на корректность -- только на скорость.
...
**Event Replay** -- повторное проигрывание событий из Event Store, чтобы заново построить проекцию или восстановить состояние. Это возможно именно потому, что события неизменяемы и хранятся навсегда: история всегда под рукой, и любое read-представление -- лишь её проекция, которую можно пересчитать с нуля.

Это одно из главных практических преимуществ Event Sourcing: ошиблись в логике проекции -- исправили код и пересчитали, как будто бага и не было.
...
Проблема фундаментальная: неизменяемость событий -- основа всего паттерна (на ней держатся replay, аудит, snapshot-инвалидация), но она прямо конфликтует с требованиями удалить данные, прежде всего с GDPR (право на забвение). Просто стереть событие нельзя -- это сломает целостность журнала и все производные от него проекции.

Идея решений: удалить не само событие, а возможность прочитать персональные данные в нём. Подходы:
...
**Проекция** (Projection) -- компонент, который слушает поток событий и поддерживает на их основе оптимизированное read-представление (view). Она «проецирует» одну и ту же историю событий в удобную для конкретного потребителя форму -- отсюда и название.
...
Вопрос сводится к компромиссу: насколько свежей должна быть read-модель и какой ценой. Есть три основных подхода -- от строгой согласованности с потерей масштабируемости до полностью асинхронной синхронизации.
...
```
- **Плюс:** простота, strong consistency -- read-модель всегда актуальна.
- **Минус:** не масштабируется (read и write связаны в одной транзакции), фактически это уже не «настоящий» CQRS.
...
```
- **Плюс:** масштабируется, write и read развязаны.
- **Минус:** eventual consistency, и нужны обработка ошибок и retry на стороне проекции.

**3. Change Data Capture (CDC):**
- Инструменты вроде Debezium читают WAL (журнал изменений) базы данных и превращают его в поток событий.
- Синхронизация происходит автоматически на уровне инфраструктуры.
- Не требует менять код приложения -- write-сторона даже не знает, что её изменения куда-то реплицируются.
...
**Eventual Consistency** -- гарантия, что read-модель *в конце концов* придёт в согласованное состояние с write-моделью, но не мгновенно, а с задержкой на распространение события. «Eventual» -- ключевое слово: согласованность гарантирована, момент её достижения -- нет.

В CQRS это прямое следствие асинхронной синхронизации: команда уже выполнена и событие записано, но пока проекция его не обработала, запрос вернёт **устаревшие данные**. Окно рассогласованности обычно десятки-сотни миллисекунд, но проектировать систему нужно так, будто оно есть всегда.
...
**Команда** -- объект, выражающий *намерение* изменить состояние системы. По смыслу это приказ в повелительном наклонении («сделай это»), а не констатация факта. И, в отличие от факта, приказ может быть отклонён -- если он нарушает бизнес-правила.
...
Разница в одном слове -- во времени. Команда смотрит в будущее («хочу, чтобы произошло»), событие -- в прошлое («уже произошло»). Из этого вытекает всё остальное: команду можно отклонить, событие -- нет; у команды один адресат, у события -- сколько угодно подписчиков.
...
Здесь важно разделить две роли, которые часто путают. Агрегат сначала *решает*, что должно произойти, и только потом *применяет* это к себе. Эти две фазы намеренно разнесены:

1. **Command Handler** -- принимает команду, проверяет бизнес-правила (инварианты) и решает, какие события опубликовать. Он НЕ меняет состояние напрямую -- только вызывает `apply(event)`.
2. **Event Sourcing Handler** -- применяет уже опубликованное событие к внутреннему состоянию. Чистая мутация без побочных эффектов и без проверок.

Зачем такое разделение: те же Event Sourcing Handlers переиспользуются при восстановлении агрегата из истории (replay). Если бы валидация и мутация были в одном методе, replay старых событий заново «прогонял» бы все проверки -- что бессмысленно (события -- это уже свершившиеся факты) и опасно (правила могли измениться).
...
**Axon Framework** -- Java-фреймворк, который даёт инфраструктуру CQRS/ES «из коробки», чтобы не писать самому Command Bus, Event Store, маршрутизацию событий и Saga-механику. Вы описываете агрегаты, проекции и саги аннотациями, а Axon берёт на себя водопровод между ними.

Экосистема состоит из двух частей:

- **Axon Framework** -- библиотека: аннотации, шины сообщений, инфраструктура внутри приложения.
- **Axon Server** -- отдельный процесс: выделенный Event Store и маршрутизатор команд/событий/запросов между сервисами. Опционален -- вместо него можно использовать JPA-хранилище и обычный брокер.
...
Минимальное приложение состоит из пяти кусочков, и удобно держать в голове именно этот порядок: команда -> агрегат -> событие -> проекция -> запрос. Команда приходит в агрегат, тот публикует событие, проекция превращает его в read-модель, запрос её читает.
...
Axon -- не обязательное условие: CQRS -- это в первую очередь дисциплина проектирования, а не фреймворк. Достаточно вручную развести команды и запросы по разным сервисам, а синхронизацию read-модели повесить на Spring-события. Базовый вариант -- два сервиса (command/query) плюс проекция-слушатель:
...
**Saga** -- паттерн управления распределёнными транзакциями: длинная бизнес-операция разбивается на цепочку локальных транзакций (по одной на сервис), а вместо общего отката используются компенсации -- операции, семантически отменяющие уже сделанные шаги.
...
Это два способа ответить на вопрос «кто знает весь сценарий целиком». В **оркестрации** есть дирижёр (Saga-оркестратор), который знает все шаги и рассылает команды. В **хореографии** дирижёра нет: каждый сервис знает лишь, на какое событие как реагировать, а общий поток складывается из этих локальных реакций -- как танцоры, каждый из которых знает свои движения.
...
**Компенсирующая транзакция** -- операция, которая семантически отменяет эффект ранее выполненного шага Saga. Принципиально отличается от `ROLLBACK`: откат стирает изменения, как будто их не было, а компенсация -- это *новая* транзакция, которая добавляется в историю. Возврат денег -- не то же, что отсутствие платежа: в журнале остаются и списание, и возврат.

Из этого вытекает важное: компенсация не обязана вернуть систему ровно в исходное состояние, она лишь приводит её в корректное с точки зрения бизнеса. Отменённый заказ -- не то же самое, что несуществующий: он мог отправить клиенту письмо, попасть в аналитику и т.д.

**Важные свойства:**
- **Идемпотентность** -- компенсация может вызваться повторно (retry, дубль события), результат должен быть тем же. Двойной возврат денег недопустим.
- **Коммутативность** (по возможности) -- порядок применения нескольких компенсаций не должен влиять на итог.
- **Обратный порядок** -- компенсируют в порядке, обратном выполнению: последний успешный шаг откатывают первым.
...
**Версионирование событий** -- то, как уживаться с тем, что схема событий со временем меняется, а старые события переписать нельзя. В обычной БД миграцию решает один `ALTER TABLE`. В Event Sourcing так не выйдет: события неизменяемы и хранятся вечно, поэтому код обязан уметь читать одновременно и старый, и новый формат.

**Проблема конкретно:** вы изменили `OrderCreatedEvent` (добавили поле, переименовали, поменяли тип) -- но Event Store уже содержит миллионы событий в старом формате, и все они должны корректно проигрываться.
...
`CQRS` и `Event Sourcing` -- независимые паттерны, но в паре каждый закрывает слабое место другого. Event Sourcing даёт CQRS готовый поток событий для построения read-моделей; CQRS даёт Event Sourcing удобные проекции вместо медленных запросов через replay. Именно поэтому их так часто видят вместе -- они складываются почти без швов:
...
Хорошая новость: CQRS/ES тестируется на удивление приятно, потому что вход и выход везде -- это сообщения (события и команды), а не скрытое состояние. Тесты строятся по уровням, и для агрегата фреймворки дают готовый стиль «given события -> when команда -> then ожидаемые события».
...
На бумаге CQRS/ES выглядит элегантно, но в проде боль приходит с практических деталей -- от UX eventual consistency до роста Event Store и культуры команды. Что встречается чаще всего:
...
Идемпотентность означает, что повторная обработка того же события даёт тот же результат, что и однократная. Она критична, потому что брокеры гарантируют доставку at-least-once, а не exactly-once: дубли событий -- норма (retry после таймаута, ребаланс consumer-группы, replay). Без идемпотентности дубль `ItemAddedEvent` дважды прибавит товар, и read-модель «поедет».

**Стратегии (от самой простой к надёжной):**
...
Когда меняются требования -- нужно новое поле или другая логика агрегации в read-модели -- в CRUD-системе пришлось бы писать рискованную миграцию данных. В CQRS/ES всё проще: read-модель целиком производна от событий, поэтому её можно построить заново через **event replay** -- прокрутить всю историю событий через обновлённую проекцию. Главная задача -- сделать это без даунтайма, пока пользователи продолжают работать со старой версией.
...
**Суть конфликта:** Event Sourcing построен на неизменяемости -- события хранятся вечно и не редактируются. Но GDPR (статья 17, «право на забвение») требует по запросу удалить персональные данные. Стереть события напрямую нельзя: это сломает целостность журнала и все производные проекции. Решение -- сделать данные нечитаемыми, не трогая сами события.
...
**Axon Framework** — специализированный Java-фреймворк от AxonIQ для реализации CQRS/ES. Его ценность в том, что вся «водопроводная» инфраструктура -- Command Bus, Event Bus, Event Store, Query Bus, Saga-механика -- идёт из коробки. Разработчик описывает доменную логику аннотациями, а маршрутизацию сообщений между компонентами берёт на себя Axon.
...
**Event Store** — append-only хранилище событий: записи только добавляются, никогда не обновляются и не удаляются. Самостоятельно его несложно реализовать на обычном PostgreSQL -- весь фокус в двух деталях схемы: как задать порядок событий и как защититься от конкурентной записи. Обе решаются одним грамотным набором колонок и ограничений.
...
Механизм держится на ограничении `UNIQUE (aggregate_id, sequence_number)`. При записи нового события мы заранее знаем, какой `sequence_number` ожидаем (текущая версия + 1). Если за это время другой процесс уже записал событие с тем же номером, INSERT упрётся в нарушение уникальности -- и это сигнал «агрегат изменился под нами». Тогда мы перезагружаем его и повторяем операцию. Никаких блокировок строк при чтении -- отсюда «оптимистичная».
...
**Снапшот (Snapshot)** — сериализованное состояние агрегата после N-го события, сохранённое отдельно. Назначение чисто оптимизационное: вместо проигрывания всей истории при каждой загрузке агрегата мы стартуем с последнего снапшота и применяем только «хвост» событий после него. Логика везде одинакова -- «снапшот плюс события после него»; различаются лишь стратегии, *когда* снапшот создавать.
...
**Проекция (Projection)** — Read Model, построенная из событий Event Store. Способов «подключить» проекцию к потоку событий несколько, и выбор между ними -- это всё тот же компромисс «свежесть данных против масштабируемости». Три основных механизма: синхронный (в транзакции записи), асинхронный push (через события/брокер) и catch-up (проекция сама вычитывает события из хранилища по позиции).
...
Проекция сама вытягивает события из Event Store, начиная с сохранённой позиции (checkpoint), и постепенно «догоняет» конец журнала. Это самый гибкий вариант: чтобы пересобрать проекцию с нуля, достаточно сбросить checkpoint на позицию 0 -- и она перечитает всю историю. Именно так работает rebuild read-моделей.
...
Этот вопрос на интервью проверяет, умеете ли вы переводить техническое свойство в продуктовое решение. Технически eventual consistency неизбежна, но пользователю нельзя показывать «устаревшие данные» -- значит, надо либо скрыть задержку, либо честно её обозначить.

**Проблема глазами пользователя:** создал заказ, обновил страницу -- и не видит его. Write Model уже обновлён, а Read Model ещё нет.
...
**Saga** — паттерн управления распределённой транзакцией как цепочкой локальных транзакций с компенсациями. В CQRS она ложится естественно: Saga слушает события через Event Bus и в ответ шлёт команды через Command Bus -- то есть это просто ещё один обработчик событий, но с состоянием и памятью о ходе процесса. Реализовать Saga можно двумя способами -- оркестрацией и хореографией.

**Оркестрация (Orchestration) — централизованный координатор:**

Один класс-оркестратор знает весь сценарий: получает событие, решает, какой шаг следующий, и шлёт команду. Весь поток виден в одном месте.
...
Оркестратора нет. Каждый сервис подписан на нужные ему события и сам решает, что делать и какое событие опубликовать дальше. Сценарий нигде не записан целиком -- он «складывается» из локальных реакций.
...
**Проблема:** события в Event Store неизменяемы, но требования к ним со временем меняются -- добавляются поля, переименовываются, разбиваются. Переписать миллионы старых событий нельзя, а читать их система обязана. Решение -- «апгрейдить» старый формат до нового на лету, при чтении.

**Upcaster** — компонент, который преобразует старую версию события в новую в момент его чтения из Event Store, до того как событие попадёт в агрегат или проекцию. В хранилище событие остаётся прежним, наружу выходит уже актуальная схема.
...
Перестройка (rebuild) нужна, когда поменялась логика проекции, в Read Model добавилось поле, нашлась ошибка в обработке событий или потребовалась новая проекция под новый сценарий. Поскольку read-модель полностью производна от событий, rebuild всегда возможен -- вопрос лишь в том, как сделать его без даунтайма. Базовая идея во всех стратегиях одна: строить новую проекцию рядом со старой, держа старую в проде до самого переключения.
