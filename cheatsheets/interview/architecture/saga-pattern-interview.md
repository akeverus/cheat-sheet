...
> [!mcq]
>
> **A) Choreography лучше Orchestration всегда — нет SPOF**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: «всегда лучше» — ложный универсализм. Без центрального координатора сложно отследить Saga in-flight, реализовать conditional branching, добавить timeout и retry на уровне шагов. SPOF брокера тоже остаётся (Kafka HA — отдельная инфраструктура).
>
> ---
>
> **B) Нет центрального координатора; каждый сервис публикует доменное событие и реагирует на события других; coordination через Kafka topic с correlation-id**
>
> ✓ ВЕРНО
>
> МЕХАНИКА: координация распределена между участниками — каждый сервис знает свою часть flow, после локальной транзакции публикует событие в общий event bus, другие сервисы подписываются по типу события и фильтруют по `correlation-id` (= sagaId/orderId).
>
> ПРИМЕНЯТЬ: простые 2-3 шаговые Saga без сложного branching (notification pipeline, cache invalidation cascade, simple order flow).
>
> ПРАВИЛО: choreography = event-driven + decoupled; orchestration = centralized flow. См. Q6 (orchestration), Q11 (correlation-id), Q12 (когда что выбрать).
>
> ---
>
> **C) В Choreography Saga сервисы вызывают друг друга напрямую через REST**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: подмена event-driven на point-to-point REST. Прямые REST-вызовы создают temporal coupling; при сбое получателя — потеря события; нужен MQ как буфер для at-least-once delivery с retention.
>
> ---
>
> **D) Choreography требует общей БД для передачи состояния между сервисами**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: shared DB ломает microservice boundaries. Состояние передаётся через события (payload с sagaId/orderId); каждый сервис имеет свою БД — это базовый принцип Database per Service.
...
> [!mcq]
>
> **A) Сервисы вызывают друг друга напрямую через REST — событий достаточно для согласованности**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: путаница «событие = REST-вызов». REST-вызовы без MQ = при сбое потребителя событие теряется; нужен Kafka/RabbitMQ для at-least-once delivery с retention и retry. Сами «события» без брокера — это не event-driven архитектура.
>
> ---
>
> **B) Для correlation-id достаточно использовать timestamp — он уникален**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: timestamp неуникален при конкурентных запросах в той же миллисекунде; correlation-id должен быть UUID (или ULID), генерируемый в точке входа Saga и прокидываемый через все события как sagaId/orderId.
>
> ---
>
> **C) Каждое событие несёт sagaId/orderId как correlation-id; сервис публикует событие → следующий сервис реагирует; Outbox Pattern гарантирует at-least-once**
>
> ✓ ВЕРНО
>
> МЕХАНИКА: цепочка `OrderCreated → ItemsReserved → PaymentCharged` связана общим `correlation-id`; каждое событие пишется в outbox-таблицу в той же транзакции, что и state change → CDC/poller публикует в Kafka → потребители получают at-least-once и обязаны быть идемпотентны (`processedEventIds` в БД).
>
> ПРИМЕНЯТЬ: decoupled choreography flow с надёжной доставкой; Outbox = атомарность «изменение состояния + публикация события».
>
> ПРАВИЛО: correlation-id + Outbox + idempotent consumer = надёжная choreography без потерь и дубликатов. См. Q10 (что такое choreography), Q12 (когда выбирать).
>
> ---
>
> **D) Kafka автоматически удаляет дублирующиеся события — idempotency не нужна**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: Kafka доставляет at-least-once (даже с `enable.idempotence=true` это про producer, не про consumer); идемпотентность на стороне потребителя обязательна — хранение `processedEventIds` в БД либо UPSERT по бизнес-ключу.
...
> [!mcq]
>
> **A) Оркестрацию выбирать всегда — хореография принципиально ненадёжна**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: ложный универсализм. Хореография отлично работает для 2-4 шагов и не несёт SPOF координатора; принудительная оркестрация добавляет доп. сервис, БД для SagaState и операционную сложность там, где это не нужно.
>
> ---
>
> **B) Хореографию выбирать когда нужен audit trail всего процесса**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: перепутаны критерии. Audit trail сложен при хореографии (нужен distributed tracing, корреляция событий по correlation-id из нескольких сервисов); оркестрация даёт SagaState в БД и таблицу переходов = готовый audit log с одной точкой запроса.
>
> ---
>
> **C) Можно смешивать оркестрацию и хореографию только теоретически**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: ложная дихотомия. Комбинирование — стандартная практика: внешний оркестратор управляет верхнеуровневыми шагами с branching и audit, а внутри каждого шага могут быть короткие event-driven цепочки между несколькими сервисами (например, fanout уведомлений).
>
> ---
>
> **D) Оркестрация: сложные 5+ шаговые процессы с branching, audit trail; Choreography: 2-4 шага без ветвлений**
>
> ✓ ВЕРНО
>
> МЕХАНИКА: critical thresholds — >4 шагов, conditional branching, требование audit trail и визуализации (BPMN/Temporal UI), HA-кластер для оркестратора → orchestration; короткие реактивные цепочки 2-4 шагов без сложного flow → choreography с Kafka topic + correlation-id.
>
> ПРИМЕНЯТЬ: кредитный процесс / supply chain / order с 7+ шагов → orchestration; cache invalidation cascade / notification fanout → choreography. Можно комбинировать (см. вариант C).
>
> ПРАВИЛО: >4 шагов или нужен явный контроль/audit → orchestration; всё остальное → choreography. См. Q6 (orchestration), Q10 (choreography), Q13 (риск роста choreography).
...
> [!mcq]
>
> **A) Implicit coupling monster: flow невидим, 5+ сервисов подписаны друг на друга, возможны cyclic dependencies, отладка только через distributed tracing; при 5+ шагах — мигрировать на оркестрацию**
>
> ✓ ВЕРНО
>
> МЕХАНИКА: каждый сервис подписан на события других и публикует свои; нет одного места, где описан полный процесс — flow восстанавливается только чтением кода всех участников и трассировкой через брокер. Добавить шаг = править несколько сервисов и их топики; легко получить event loop (A → B → A).
>
> ПРИМЕНЯТЬ: при росте choreography до 5+ шагов переходить на orchestration-based Saga с явным SagaState и оркестратором (Temporal/Camunda/Axon); сохранять choreography для простых 2–3 шаговых notification/cache-invalidation flow.
>
> ПРАВИЛО: choreography 5+ шагов = `distributed big ball of mud` (Sam Newman) → миграция на orchestration ради видимости flow и контроля жизненного цикла Saga. См. Q10 (что такое choreography), Q12 (когда что выбирать).
>
> ---
>
> **B) При росте до 7+ сервисов в хореографии достаточно добавить центральный event registry**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: event registry (каталог типов событий) решает discovery, но не возвращает видимость flow и не устраняет coupling — Saga по-прежнему распределена по обработчикам в N сервисах. Нужна не «регистратура», а оркестратор с явным state machine и единым местом описания процесса.
>
> ---
>
> **C) Хореография масштабируется лучше оркестрации при любом количестве сервисов**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: подмена `scalability` и `maintainability`. Брокер действительно масштабируется горизонтально, но связность через события растёт нелинейно с числом сервисов; при 5+ шагах поддержка choreography обходится дороже, чем orchestration через выделенный coordinator.
>
> ---
>
> **D) Главный риск хореографии — SPOF брокера сообщений**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: Kafka/RabbitMQ разворачиваются в HA-кластере и не являются SPOF на практике. Реальный риск — invisible flow и accidental coupling между сервисами: распределённый процесс без одного владельца, который никто целиком не видит.
...
> [!mcq]
>
> **A) Compensating transaction = SQL `ROLLBACK` на удалённой БД через распределённый координатор**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: подмена уровня. После коммита локальной транзакции SQL `ROLLBACK` физически невозможен — WAL уже зафиксирован. Compensation работает в прикладном слое: новая локальная транзакция с обратной бизнес-семантикой, а не отмена записи в журнале.
>
> ---
>
> **B) Новая локальная транзакция, семантически отменяющая T_i; должна быть идемпотентной и коммутативной; это не SQL `ROLLBACK`, а новое forward-действие (refund, CANCELLED, reversing entry)**
>
> ✓ ВЕРНО
>
> МЕХАНИКА: после коммита T_i эффект уже виден другим — нельзя «развидеть». C_i = новая транзакция, переводящая систему в состояние «как будто T_i не было»: вместо `DELETE order` → `UPDATE order SET status=CANCELLED, reason=...`; вместо reverse-charge → `payment refund`; вместо удаления проводки → reversing entry в бухучёте.
>
> ПРИМЕНЯТЬ: проектировать C_i вместе с T_i; делать C_i идемпотентной (повторный вызов после network retry не должен дать двойной refund) и по возможности коммутативной (порядок компенсаций между шагами не должен менять итог).
>
> ПРАВИЛО: compensation = new business action, не database rollback; обязательны idempotency и semantic correctness; не удалять — помечать CANCELLED с причиной для audit trail. См. Q15 (типы шагов compensable/pivot/retryable), Q16 (compensation ≠ rollback), Q34 (идемпотентность шагов).
>
> ---
>
> **C) Компенсация должна физически удалять запись, чтобы не оставлять «грязных» данных**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: ломает audit trail и compliance (SOX, бухучёт, GDPR-журнал). Правильно — пометить статус (`CANCELLED`, `REFUNDED`) с причиной и timestamp; в финансовых системах создавать reversing entry (списание + зачисление), а не удалять проводку.
>
> ---
>
> **D) Компенсацию не нужно делать идемпотентной — Saga вызывает её строго один раз**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: в распределённой системе при network timeout / failure orchestrator повторит компенсацию по at-least-once семантике; без идемпотентности — двойной refund, отрицательный остаток на складе, двойной email. Дедуплицировать по `(sagaId, stepId)` обязательно.
...
> [!mcq]
>
> **A) Retryable транзакции можно ставить в начало Saga — они не требуют компенсации, что упрощает дизайн**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: нарушает design invariant Richardson'а. Retryable шаги по определению идут ПОСЛЕ pivot; до pivot шаги должны быть compensable, иначе при сбое более позднего шага систему невозможно вернуть в согласованное состояние. Положив retryable в начало, теряем возможность отката всей Saga.
>
> ---
>
> **B) Pivot transaction — это самая долгая по времени операция Saga, требующая отдельного timeout**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: подмена критерия. Pivot определяется по бизнес-семантике («после этого шага откат невозможен или экономически бессмыслен»), а не по длительности. Обычно pivot — это payment charge или stock write-off; они быстрые, но необратимые.
>
> ---
>
> **C) Compensable (до pivot, с явной компенсацией C_i), Pivot (точка невозврата, без компенсации), Retryable (после pivot, гарантированно успешны при retry); все необратимые шаги pushни за pivot**
>
> ✓ ВЕРНО
>
> МЕХАНИКА: классификация Richardson разделяет шаги по обратимости. До pivot: `CreateOrder(PENDING)` и `ReserveInventory` — компенсируются. Pivot: `ChargePayment` — после успеха Saga считается принятой, дальше откат не делаем. После pivot: `SendEmail`, `UpdateAnalytics`, `GrantBonus` — могут только повторяться до успеха (retry с backoff), бизнесово не падают.
>
> ПРИМЕНЯТЬ: при проектировании Saga для каждого шага явно проставлять тип; всё, что нельзя компенсировать (внешний API без refund, физическая отгрузка) — двигать за pivot и делать retryable; payment почти всегда = pivot; уведомления и аналитика = retryable.
>
> ПРАВИЛО: `compensable* → pivot → retryable*` — единственный допустимый порядок; pushни необратимые шаги вправо за pivot, чтобы Saga оставалась откатываемой до точки невозврата. См. Q14 (что такое compensation), Q16 (compensation ≠ rollback), Q17 (что если компенсация падает).
>
> ---
>
> **D) Если один шаг помечен как retryable, все последующие автоматически становятся retryable — свойство транзитивно**
>
> ❌ НЕВЕРНО
>
> ПОСЛЕДСТВИЕ: тип шага не транзитивен. Каждый шаг классифицируется явно по своей бизнес-семантике; шаг после retryable может оказаться pivot или compensable, если он взаимодействует с новым внешним ресурсом. Автоматическое распространение свойства приводит к ошибке проектирования и неверному поведению при сбое.
...
> [!mcq]
> - [ ] Commutative updates означают, что операции выполняются в случайном порядке, и компенсация всегда сводится к простому DELETE | ❌ ПОСЛЕДСТВИЕ: commutative — это design свойство (порядок не влияет на результат), не «случайный порядок»; компенсация = inverse operation (`-= delta`), не DELETE
> - [ ] Pessimistic view — это блокировка записи в БД на всё время Saga через SELECT FOR UPDATE | ❌ ПОСЛЕДСТВИЕ: pessimistic view ≠ DB-блокировка; это переупорядочивание шагов так, чтобы «опасные» обновления шли в retryable-фазе; никаких глобальных локов
> - [ ] Reread value сравнивает значение перед коммитом transaction в начале Saga | ❌ ПОСЛЕДСТВИЕ: reread выполняется ПЕРЕД компенсацией (а не перед коммитом T_i); проверяет, не изменилось ли значение с момента T_i, чтобы решить компенсировать или эскалировать
> - [x] Countermeasures Richardson: commutative (`+= delta` вместо `set`), pessimistic view (опасные обновления — последними), reread value (проверка перед C_i), version file (inverse operation по ID, не значению) | ✓ ПРИМЕНЯТЬ: финансовые Saga с риском lost update / dirty read 📋 ПРАВИЛО: countermeasures = инженерные приёмы вокруг отсутствия isolation 🔗 См. Q19
...
> [!mcq]
> - [x] By value — runtime-выбор механизма транзакции по бизнес-параметрам: маленькая retail-сумма → Saga + eventual, VIP/high-risk → 2PC или manual approval; не отдельный countermeasure, а tier-стратегия | ✓ ПРИМЕНЯТЬ: финтех, банкинг, где риск аномалии стоит миллионы 📋 ПРАВИЛО: разные risk tiers = разные механизмы консистентности 🔗 См. Q20
> - [ ] By value — это синоним commutative updates: операции применяются в любом порядке без потери результата | ❌ ПОСЛЕДСТВИЕ: by value ≠ commutative; by value — это выбор стратегии (Saga/2PC) на основе значения параметра, commutative — design операций
> - [ ] By value требует обязательного использования синхронного 2PC для всех операций выше определённой суммы | ❌ ПОСЛЕДСТВИЕ: «обязательного» нет — это runtime-решение бизнес-уровня; может быть и manual approval, и более строгая Saga, и отклонение запроса
> - [ ] By value — это шаблон выбора partition key в Kafka по бизнес-полю для гарантии порядка | ❌ ПОСЛЕДСТВИЕ: partition key — другой паттерн (causal ordering); by value strategy — выбор механизма транзакции, не маршрутизация сообщений
...
> [!mcq]
> - [ ] Saga и 2PC одинаково подходят для микросервисов: разница только в производительности | ❌ ПОСЛЕДСТВИЕ: 2PC требует XA/JTA, недоступного в Kafka/большинстве NoSQL; кросс-сервисная блокировка ресурсов на минуты неприемлема — fundamentally different applicability
> - [ ] 2PC обеспечивает eventual consistency, Saga — strong consistency через компенсации | ❌ ПОСЛЕДСТВИЕ: ровно наоборот: 2PC = strong (атомарно all-or-nothing), Saga = eventual (компенсации после видимого коммита); классическая ошибка
> - [x] 2PC = strong consistency + ACID + блокировки + coordinator SPOF + только XA-ресурсы + не long-running; Saga = eventual + ACD + async + гетерогенные ресурсы + длительность часы/дни, но компенсации + countermeasures | ✓ ПРИМЕНЯТЬ: микросервисы → Saga, монолит с XA-ресурсами → 2PC 📋 ПРАВИЛО: в микросервисах 2PC — антипаттерн 🔗 См. Q23
> - [ ] Saga применяется только для read-операций, 2PC — для write-операций | ❌ ПОСЛЕДСТВИЕ: read не требует распределённой транзакции вовсе; и Saga, и 2PC — про write-coordination через несколько сервисов/БД
...
> [!mcq]
> - [ ] Главная проблема 2PC в микросервисах — низкая скорость сети между датацентрами | ❌ ПОСЛЕДСТВИЕ: сеть — лишь усугубляющий фактор; root cause = блокировки ресурсов, in-doubt при падении coordinator, отсутствие XA в Kafka/NoSQL, невозможность long-running
> - [ ] 2PC не работает в микросервисах, потому что не поддерживает идемпотентность | ❌ ПОСЛЕДСТВИЕ: идемпотентность — отдельная проблема consumer'ов; 2PC проваливается из-за locks + SPOF + heterogeneous resources + 3x round-trip overhead
> - [ ] Проблема 2PC решается заменой coordinator на распределённый Raft-кластер | ❌ ПОСЛЕДСТВИЕ: даже HA-coordinator не решает блокировки ресурсов и отсутствие XA в Kafka/современных NoSQL; устраняется один симптом из шести
> - [x] 2PC плох в микросервисах из-за: блокировок ресурсов (deadlock под нагрузкой), in-doubt при coordinator-failure, network partition с зависшими локами, отсутствия XA в Kafka/NoSQL, ~3x overhead, невозможности long-running | ✓ ПРИМЕНЯТЬ: проектировать вместо 2PC — Saga + Outbox 📋 ПРАВИЛО: 2PC = антипаттерн в микросервисах 🔗 См. Q22
...
> [!mcq]
> - [ ] TCC — это синоним Saga, два разных названия одного паттерна | ❌ ПОСЛЕДСТВИЕ: TCC отличается моделью: каждый шаг = Try + Confirm/Cancel (3 операции); Saga = T_i + C_i (2 операции); isolation в TCC лучше за счёт reservation
> - [x] TCC = Try (reserve, не коммит) → Confirm (финализация) / Cancel (release); ресурсы зарезервированы и не видны другим = лучше isolation, чем Saga, но требует 3 метода на каждый сервис | ✓ ПРИМЕНЯТЬ: финтех, Alibaba Seata, где видимые intermediate states недопустимы 📋 ПРАВИЛО: TCC = Saga с reservation 🔗 См. Q19
> - [ ] В TCC нет компенсаций, поэтому он применим только для idempotent операций | ❌ ПОСЛЕДСТВИЕ: Cancel в TCC — это и есть компенсация (откат резерва); просто она лёгкая (release reservation), а не reversing business action
> - [ ] TCC требует обязательной поддержки XA-транзакций от всех участников | ❌ ПОСЛЕДСТВИЕ: TCC работает без XA — Try создаёт reservation как обычную запись в БД с локальной транзакцией; XA не нужен
...
> [!mcq]
> - [ ] Достаточно сохранять state в HashMap в памяти оркестратора — при рестарте Saga просто перезапустится | ❌ ПОСЛЕДСТВИЕ: in-memory state теряется при рестарте JVM; in-progress саги «потеряются», заказы зависнут в промежуточном состоянии; обязательно persistent state в БД
> - [ ] Команды можно отправлять напрямую через kafka.send(...) в @Transactional методе, и Spring сделает atomic commit с БД | ❌ ПОСЛЕДСТВИЕ: классический dual-write: БД-транзакция и Kafka-publish не атомарны; падение между ними = потеря команды; нужен Outbox
> - [ ] @Version для optimistic locking не нужен, потому что reply'ы обрабатываются строго последовательно одним consumer | ❌ ПОСЛЕДСТВИЕ: несколько partitions / consumer instances → параллельные reply'ы на один sagaId; без @Version — lost update в state Saga
> - [x] Saga state = JPA entity с @Version; одна транзакция: load state → mutate → save → write to outbox; @KafkaListener с idempotent handling (проверка currentStep); таймауты через scheduler; recovery job для stuck sagas | ✓ ПРИМЕНЯТЬ: минимальная production-grade реализация без фреймворков 📋 ПРАВИЛО: state в БД + Outbox + idempotency = базовая триада 🔗 См. Q26
...
> [!mcq]
> - [ ] Outbox не нужен, если использовать Kafka transactions (exactly-once): producer atomic-commit гарантирует доставку при коммите БД | ❌ ПОСЛЕДСТВИЕ: Kafka transactions ≠ atomic между Kafka и сторонней БД; нет distributed-XA между PostgreSQL и Kafka; Outbox — единственный надёжный путь
> - [x] Outbox = в одной локальной транзакции INSERT в saga_state + INSERT в outbox; отдельный publisher (poll или Debezium CDC) читает outbox и публикует в Kafka, помечая как PUBLISHED | ✓ ПРИМЕНЯТЬ: любая Saga с Kafka/RabbitMQ + БД 📋 ПРАВИЛО: dual-write = баг; Outbox = решение 🔗 См. Q25
> - [ ] Достаточно отправить в Kafka сначала, а потом сохранить в БД — если БД упала, мы знаем что команда уже ушла | ❌ ПОСЛЕДСТВИЕ: классическая dual-write наоборот; команда ушла, состояние не сохранено → дубли при рестарте, неконсистентность; решение всё равно — Outbox
> - [ ] Publisher должен синхронно ждать ACK от Kafka внутри @Transactional, чтобы гарантировать доставку | ❌ ПОСЛЕДСТВИЕ: блокирует транзакцию БД (locks!) на время сети; если Kafka недоступен — БД-транзакция держит блокировки; publisher должен быть асинхронным (отдельный job)
...
> [!mcq]
> - [ ] @Saga в Axon — это просто аннотация над контроллером, который вызывает participants синхронно через REST | ❌ ПОСЛЕДСТВИЕ: @Saga — не контроллер; это event-handler с автоматическим persistence state; общается через commandGateway (async), не sync REST
> - [x] @Saga + @StartSaga + @SagaEventHandler(associationProperty); Axon создаёт instance по первому event'у, state автоматически персистится; SagaLifecycle.associateWith добавляет ассоциации (paymentId); @EndSaga завершает | ✓ ПРИМЕНЯТЬ: CQRS/ES системы на Axon, где Saga — first-class citizen 📋 ПРАВИЛО: декларативная Saga через event-handlers 🔗 См. Q40
> - [ ] State Axon Saga нужно вручную сохранять в БД через @Transactional save(state) после каждого event | ❌ ПОСЛЕДСТВИЕ: Axon делает persistence автоматически (JPA/JDBC/MongoDB) — ручной save не требуется и приведёт к двойной записи / конфликтам
> - [ ] Axon Saga привязывается к одному агрегату через aggregate identifier, additional associations невозможны | ❌ ПОСЛЕДСТВИЕ: SagaLifecycle.associateWith позволяет добавлять любые association properties (paymentId, shipmentId), чтобы будущие events с этими полями попадали в тот же instance
...
> [!mcq]
> - [ ] Eventuate Tram использует синхронные HTTP-вызовы между оркестратором и participants для надёжности | ❌ ПОСЛЕДСТВИЕ: Eventuate Tram — асинхронный поверх Kafka + Outbox (через Debezium CDC); sync HTTP противоречит Saga-философии и решаемой проблеме
> - [x] Eventuate Tram = DSL `step().invokeParticipant().withCompensation()` + автоматический Outbox через Eventuate CDC + Debezium + Kafka; работает с MySQL/PostgreSQL; managed persistence, retry, idempotency | ✓ ПРИМЕНЯТЬ: orchestration-Saga в Spring Boot без написания своего workflow-движка 📋 ПРАВИЛО: явный DSL для шагов и компенсаций 🔗 См. Q25
> - [ ] withCompensation() в DSL Eventuate означает, что компенсация запускается автоматически при любом исключении в shаге, без явного onReply | ❌ ПОСЛЕДСТВИЕ: компенсация запускается при failure-reply от participant'а, а не на любое exception; onReply разруливает Success/Failure ветки явно
> - [ ] Eventuate Tram заменяет Spring Boot и работает только в standalone-режиме | ❌ ПОСЛЕДСТВИЕ: Eventuate — это библиотека поверх Spring Boot / Micronaut / Quarkus, не замена; интегрируется с обычными контейнерами
...
> [!mcq]
> - [x] BPMN-процесс в Camunda: serviceTask + isForCompensation + boundaryEvent с compensateEventDefinition; participant'ы как external workers (subscription по topic); визуализация для бизнес-аналитика, встроенная compensation semantics, версионирование процессов | ✓ ПРИМЕНЯТЬ: enterprise-процессы с длительным lifecycle и требованиями к visibility 📋 ПРАВИЛО: BPMN = визуальная Saga 🔗 См. Q42
> - [ ] BPMN-процесс автоматически генерирует код всех participant-сервисов из диаграммы | ❌ ПОСЛЕДСТВИЕ: BPMN описывает workflow и compensation semantics, но participant-логику (как зарезервировать товар) пишет разработчик как ExternalTaskHandler
> - [ ] Camunda compensation запускается автоматически на любое RuntimeException в сервисе | ❌ ПОСЛЕДСТВИЕ: compensation триггерится через BPMN error / compensate event (handleBpmnError); нужно явно объявить boundary event и compensation activity, не любое исключение
> - [ ] Главный минус Camunda — невозможность развернуть в Kubernetes-кластере | ❌ ПОСЛЕДСТВИЕ: Camunda 8 (Zeebe) спроектирован cloud-native в Kubernetes; настоящие минусы — runtime overhead, сложность деплоя движка, lock-in на BPMN
...
> [!mcq]
> - [ ] State persistent оркестратора достаточно хранить в Redis с TTL: быстрое чтение и автоматическая очистка | ❌ ПОСЛЕДСТВИЕ: Redis с TTL может потерять state до завершения Saga; нет транзакционности с Outbox; для long-running и обязательного eventual completion нужна durable БД с @Version
> - [x] State в одной таблице с @Version (optimistic locking) + изменение state + публикация команды в одной транзакции через Outbox + load-on-event + scheduled recovery job для stuck sagas + idempotent participants + метрика saga_stuck_duration_seconds | ✓ ПРИМЕНЯТЬ: production оркестратор без фреймворков 📋 ПРАВИЛО: state в БД + Outbox + recovery = базовая триада 🔗 См. Q33
> - [ ] In-memory state с периодическим snapshot в файл — компромисс между скоростью и долговечностью | ❌ ПОСЛЕДСТВИЕ: между snapshot'ами state теряется при падении; файл vs БД — не даёт shared state между instance'ами оркестратора; масштабирование невозможно
> - [ ] Recovery job не нужен — Kafka сам перебалансирует partitions и переотправит reply'ы при падении | ❌ ПОСЛЕДСТВИЕ: Kafka перебалансирует consumer'ов, но не сами Saga; если reply потерян до коммита offset или потерян message — recovery job ищет stuck sagas по updatedAt и продолжает их
...
> [!mcq]
> - [ ] Главный антипаттерн — использовать Saga вместо 2PC; 2PC надёжнее и проще | ❌ ПОСЛЕДСТВИЕ: 2PC сам по себе антипаттерн в микросервисах (locks + SPOF + no XA); Saga — правильный выбор, антипаттерны — про неправильную реализацию Saga
> - [ ] Компенсация через UPDATE status='CANCELLED' — антипаттерн, нужно DELETE для чистоты данных | ❌ ПОСЛЕДСТВИЕ: всё наоборот — DELETE теряет audit trail и ломает foreign keys; UPDATE с status='CANCELLED' = правильный паттерн (reversing entry)
> - [x] Антипаттерны: бизнес-логика в оркестраторе, sync REST вместо async, DELETE для компенсации, нет countermeasures, нет pivot, choreography 10+ events, игнор stuck sagas / DLQ, нет correlation-id, не-идемпотентные компенсации, отсутствие таймаутов | ✓ ПРИМЕНЯТЬ: чек-лист при code review Saga 📋 ПРАВИЛО: оркестратор = тупой координатор, не domain service 🔗 См. Q32
> - [ ] Pivot transaction — антипаттерн: лучше делать все шаги compensable для безопасности | ❌ ПОСЛЕДСТВИЕ: отсутствие pivot увеличивает количество сценариев компенсации и риск зависания; pivot — best practice, не антипаттерн
...
> [!mcq]
> - [ ] Distributed monolith — это монолит, разнесённый по нескольким серверам через load balancer | ❌ ПОСЛЕДСТВИЕ: это не distributed monolith; distributed monolith — это набор сервисов, технически разделённых, но настолько связанных, что нельзя деплоить независимо
> - [ ] Saga по определению устраняет distributed monolith, потому что разделяет сервисы | ❌ ПОСЛЕДСТВИЕ: Saga не устраняет — может провоцировать: жёсткие event-контракты в хореографии, оркестратор-знающий-internals, циклические зависимости, coordinated release
> - [x] Distributed monolith = технически разделённые сервисы с жёсткими связями (sync calls, нестабильные event-контракты, циклы); Saga провоцирует через choreography 10+ events, орк-знающий internals, отсутствие schema registry; лечится контрактами + версионированием | ✓ ПРИМЕНЯТЬ: code review архитектуры на coordinated-deploy 📋 ПРАВИЛО: deploy одного сервиса ≠ deploy остальных 🔗 См. Q31
> - [ ] Distributed monolith — позитивный термин: показывает, что Saga обеспечивает atomicity через несколько сервисов | ❌ ПОСЛЕДСТВИЕ: distributed monolith — антипаттерн (плохая декомпозиция); смешивает недостатки монолита и распределённой системы
...
> [!mcq]
> - [ ] Stuck saga решается перезапуском оркестратора: при старте он перечитает state и продолжит | ❌ ПОСЛЕДСТВИЕ: рестарт оркестратора не разбудит stuck saga — обычный load-on-event ждёт reply, которого нет; нужен отдельный recovery job по updatedAt
> - [ ] Stuck saga нужно сразу принудительно переводить в COMPLETED, чтобы освободить ресурсы | ❌ ПОСЛЕДСТВИЕ: принудительный COMPLETED = неконсистентность (деньги списаны, заказ не подтверждён); нужно либо retry, либо force-compensation, либо manual intervention
> - [x] Stuck saga: обнаружение через updatedAt + scheduled job + метрика saga_age_seconds; ресолюшн = retry команды текущего шага → timeout/force compensation → admin UI с manual override; профилактика = таймауты на шаг + circuit breaker | ✓ ПРИМЕНЯТЬ: production-ready operability 📋 ПРАВИЛО: каждая stuck saga — инцидент с runbook 🔗 См. Q39
> - [ ] Достаточно поднять log-уровень и логировать каждую транзакцию — оператор сам найдёт зависшие | ❌ ПОСЛЕДСТВИЕ: ручной поиск в логах при тысячах саг в час нереалистичен; нужны метрики, алерты, scheduled job + admin UI
...
> [!mcq]
> - [ ] Идемпотентность достигается выставлением Kafka `enable.idempotence=true` — больше ничего не нужно | ❌ ПОСЛЕДСТВИЕ: producer idempotence гарантирует только отсутствие дублей внутри одной session producer'а; consumer всё равно может получить дубль (rebalance, redelivery); нужна application-level дедупликация
> - [x] Идемпотентность: уникальный messageId + табл. processed (existsById → skip); upsert by business id (create order with id=X); optimistic locking через @Version; conditional compensation (проверка что шаг был выполнен) | ✓ ПРИМЕНЯТЬ: каждый @KafkaListener в Saga 📋 ПРАВИЛО: at-least-once delivery = обязательная дедупликация 🔗 См. Q26
> - [ ] Достаточно использовать exactly-once семантику Kafka: брокер сам отбросит повторы | ❌ ПОСЛЕДСТВИЕ: Kafka exactly-once покрывает только Kafka↔Kafka стрим; при write в стороннюю БД дубли всё равно возможны; нужна явная дедупликация в consumer'е
> - [ ] Компенсации должны быть НЕ-идемпотентными, чтобы вызываться только один раз | ❌ ПОСЛЕДСТВИЕ: компенсация может вызваться повторно при retry/recovery; не-идемпотентная компенсация = двойной refund/release; идемпотентность обязательна
...
> [!mcq]
> - [ ] Kafka гарантирует глобальный порядок сообщений независимо от количества партиций | ❌ ПОСЛЕДСТВИЕ: Kafka гарантирует порядок только внутри одной партиции; разные partitions = независимые потоки; нужен правильный partition key (aggregate id) для строгого порядка по сущности
> - [x] Решения: partition key = aggregate id (строгий порядок в партиции), causal ordering через sequenceNo + буферизация, state check (откладывать если предусловие не готово), Process Manager как локальный state machine | ✓ ПРИМЕНЯТЬ: хореография с несколькими publisher'ами 📋 ПРАВИЛО: porder design > retry chaos 🔗 См. Q10
> - [ ] Достаточно увеличить linger.ms у producer — он сам отправит сообщения в нужном порядке | ❌ ПОСЛЕДСТВИЕ: linger.ms — batching на producer, не глобальный ordering между топиками/сервисами; не решает out-of-order между разными publishers
> - [ ] Out-of-order невозможен, если consumer работает в одном потоке | ❌ ПОСЛЕДСТВИЕ: даже один поток получит события в порядке Kafka-партиций, но если events публикуются разными сервисами в разные топики/партиции — порядок не гарантирован
...
> [!mcq]
> - [ ] Достаточно unit-тестов на каждый шаг — integration не нужны, так как замедляют CI | ❌ ПОСЛЕДСТВИЕ: unit-тесты ловят бизнес-логику в одном сервисе; cross-service Saga (compensation, idempotency, concurrency, message ordering) проверяется только в integration с Testcontainers
> - [x] Многоуровневая стратегия: unit (шаги + state machine), integration с Testcontainers (happy path), chaos (PaymentService alwaysFail), idempotency (дважды одну команду), concurrency (две параллельные Saga), contract (Pact); в Axon — SagaTestFixture | ✓ ПРИМЕНЯТЬ: все Saga в production-системах 📋 ПРАВИЛО: тестируем переходы state machine + cross-service behavior 🔗 См. Q9
> - [ ] Chaos-тесты для Saga избыточны — production-инциденты редки, не оправдывают сложность | ❌ ПОСЛЕДСТВИЕ: компенсация = самая редко-выполняемая ветка и одновременно самая критичная; без chaos-тестов невозможно проверить compensation flow надёжно
> - [ ] Contract-тесты не нужны: оркестратор и participants — части одной команды, контракт всегда согласован | ❌ ПОСЛЕДСТВИЕ: в микросервисах participants принадлежат разным командам; даже в одной — изменение схемы команды/reply без contract-теста ломает Saga в production
...
> [!mcq]
> - [ ] Главная метрика — saga_count (количество созданных саг); остальное факультативно | ❌ ПОСЛЕДСТВИЕ: одного count недостаточно для production; нужны duration, stuck, compensation rate, ratio compensated/started — каждая отлавливает разный класс инцидентов
> - [ ] Логирование переходов state саги в production не нужно — это создаёт шум в Kibana | ❌ ПОСЛЕДСТВИЕ: переходы state с sagaId — основной forensic-инструмент для разбора stuck saga в production; без них восстановить цепочку невозможно
> - [x] Метрики: saga_started/completed_total{status}, saga_duration_seconds histogram, saga_stuck_total, saga_step_duration{step}, saga_compensation_total; structured-логи с sagaId; алерты на rate компенсаций и stuck > 5min; dashboard воронки completion | ✓ ПРИМЕНЯТЬ: каждая production Saga 📋 ПРАВИЛО: метрики + алерты + dashboard = observability триада 🔗 См. Q33
> - [ ] Достаточно одной метрики saga_failures_total — если ошибок нет, всё в порядке | ❌ ПОСЛЕДСТВИЕ: stuck saga ≠ failed; saga может висеть без ошибок и без завершения; нужна метрика по возрасту и статусу
...
> [!mcq]
> - [ ] Distributed tracing для Saga работает автоматически без передачи sagaId в Kafka-headers | ❌ ПОСЛЕДСТВИЕ: OpenTelemetry/Sleuth требуют явной propagation context'а через Kafka headers (traceparent); без headers — разорванная цепочка spans в UI Jaeger/Tempo
> - [x] sagaId = correlationId/traceId; пропагируется через Kafka headers (sagaId, traceparent); OpenTelemetry + Jaeger/Tempo/Zipkin для HTTP+Kafka spans; MDC.put в логах; в UI трассировки виден полный flow orchestrator → participants → orchestrator с временем каждого шага | ✓ ПРИМЕНЯТЬ: production debugging распределённых процессов 📋 ПРАВИЛО: один sagaId across all logs/traces 🔗 См. Q37
> - [ ] OpenTelemetry заменяет Prometheus-метрики Saga: tracing достаточен для observability | ❌ ПОСЛЕДСТВИЕ: tracing и метрики решают разные задачи; traces = детали одного запроса, metrics = агрегаты по всей системе; нужны оба
> - [ ] correlation ID в логах не нужен, если использовать distributed tracing — Jaeger покрывает всё | ❌ ПОСЛЕДСТВИЕ: tracing — sampling (1-10% запросов); 100% событий в логах с sagaId — единственный путь найти конкретную stuck saga через несколько дней
...
> [!mcq]
> - [ ] Timeout достаточно реализовать через @Timeout аннотацию на @KafkaListener; Spring сам отменит обработку | ❌ ПОСЛЕДСТВИЕ: timeout на handler не равен timeout на шаг Saga (ожидание reply часами); нужен timer event через EventScheduler или scheduled job
> - [x] Timeout на шаг через EventScheduler.schedule (или Quartz/scheduled job) — отдельный timer event запускает compensation/retry; Circuit Breaker (Resilience4j) перед participant — деградировавший сервис размыкается, Saga сразу идёт в compensation; Bulkhead — отдельные thread pool'ы | ✓ ПРИМЕНЯТЬ: production resilience 📋 ПРАВИЛО: timer = first-class event Saga 🔗 См. Q33
> - [ ] Circuit Breaker для оркестратора не нужен — оркестратор сам управляет потоком, не вызывает participants | ❌ ПОСЛЕДСТВИЕ: оркестратор отправляет команды и ждёт reply; если participant деградировал — растут zone offline saga, оркестратор зашлёт ещё команд; CB перед participant предотвращает каскад
> - [ ] Достаточно одного глобального timeout на всю Saga, не на каждый шаг отдельно | ❌ ПОСЛЕДСТВИЕ: глобальный timeout не локализует проблему (какой шаг завис); per-step timeout позволяет точечную compensation и точечный alert по типу шага
...
> [!mcq]
> - [ ] В CQRS/ES Saga вытесняется проекциями: каждое событие попадает в read model, Saga не нужна | ❌ ПОСЛЕДСТВИЕ: проекции = read-model (запросы); Saga = координация команд между агрегатами (write-side); это разные роли, не взаимозаменяемые
> - [x] Saga в CQRS/ES = process manager: подписан на события из event store, реагирует и отправляет команды в другие агрегаты; state Saga может сам быть event-sourced; проекции read model обновляются независимо; в Axon Saga — first-class citizen наряду с Aggregate/Projection | ✓ ПРИМЕНЯТЬ: координация агрегатов в event-sourced системах 📋 ПРАВИЛО: Saga = координатор write-side 🔗 См. Q27
> - [ ] Saga в Event Sourcing избыточна — событий в event store достаточно для координации | ❌ ПОСЛЕДСТВИЕ: event store хранит факты, но не реагирует на них; нужен компонент (Saga/process manager), который читает события и отправляет команды
> - [ ] Saga в Axon обязательно должна сама быть aggregate — иначе не работает persistence | ❌ ПОСЛЕДСТВИЕ: Axon Saga и Aggregate — разные концепции с раздельным persistence; Saga имеет свой репозиторий (JPA/Mongo), не привязана к aggregate
...
> [!mcq]
> - [ ] При новой версии Saga нужно мигрировать все in-progress instances в новую схему через update-скрипт | ❌ ПОСЛЕДСТВИЕ: massiver migration ломает long-running Saga; чаще лучше держать несколько версий параллельно (deprecation period) до завершения старых instances
> - [x] Версионирование: @Column sagaVersion в state + logicRegistry.get(version); zero-break (новые шаги в конец); deprecation period (две версии параллельно); workflow-системы (Temporal, Camunda) с встроенным versioning; новая семантика = новая команда (V2) | ✓ ПРИМЕНЯТЬ: long-running Saga, эволюционирующие бизнес-процессы 📋 ПРАВИЛО: никогда не меняем семантику команды задним числом 🔗 См. Q42
> - [ ] Все старые in-progress Saga нужно завершить вручную через admin UI перед deploy новой версии | ❌ ПОСЛЕДСТВИЕ: для long-running (дни/недели) это нереально; нужны параллельные версии + версионная маршрутизация в коде
> - [ ] Достаточно изменить логику in-place — оркестратор сам определит, какие шаги уже выполнены | ❌ ПОСЛЕДСТВИЕ: in-place изменение ломает текущие in-progress instance'ы (новый код в середине старого процесса); классическая причина production-инцидентов
...
> [!mcq]
> - [ ] Long-running Saga можно держать в HashMap оркестратора — Java GC не удаляет live-references | ❌ ПОСЛЕДСТВИЕ: рестарт JVM, redeploy, миграция БД случаются за неделю обязательно; in-memory state теряется; persistent state в БД — критичен
> - [x] Особенности long-running: обязательно persistent state, таймеры через EventScheduler/Quartz/workflow-engines, идемпотентность (за неделю несколько redeploy), versioning обязателен (выйдет 2-3 релиза), humans-in-the-loop UI, externalize state, persistent audit log, compensation учитывает изменившиеся данные (reread, commutative) | ✓ ПРИМЕНЯТЬ: Process Loan Approval, Book Travel, Subscription Renewal 📋 ПРАВИЛО: использовать Temporal/Camunda/Step Functions 🔗 См. Q41
> - [ ] Long-running Saga работает идентично short-running — отличие только во времени жизни | ❌ ПОСЛЕДСТВИЕ: за неделю случается рестарт JVM, миграции БД, выход 2-3 релизов кода; нужны persistent state, versioning, recovery, humans-in-the-loop — критически отличные требования
> - [ ] Достаточно увеличить Kafka retention до года — long-running Saga просто переиграет events после рестарта | ❌ ПОСЛЕДСТВИЕ: переигрывание events ≠ продолжение Saga; Saga state живёт не только в Kafka, а в собственной БД с current step, optimistic version, ассоциациями; нужен persistent state + recovery
...
> [!mcq]
> - [ ] Eventual consistency нужно скрывать от пользователя — UI всегда показывает финальный consistent state | ❌ ПОСЛЕДСТВИЕ: «скрытие» = блокирующий sync-request на время всей Saga (минуты для long-running); либо ложно-консистентный UI с rollback'ами; правильно — явные processing states
> - [ ] Достаточно показать пользователю «обработка...» и не обновлять статус до завершения Saga | ❌ ПОСЛЕДСТВИЕ: бесконтрольный спиннер без feedback — плохой UX; нужны polling/SSE/WebSocket + явные intermediate states + email на финал
> - [ ] UX и backend Saga — изолированные слои; UX команда не должна знать про processing states | ❌ ПОСЛЕДСТВИЕ: processing states (PENDING_PAYMENT, READY_TO_SHIP) — это бизнес-словарь; согласование UX↔backend обязательно, иначе UI показывает один статус, а БД — другой
> - [x] UX-паттерны: Optimistic UI, явные processing states, 202 Accepted + polling, SSE/WebSocket push, webhooks для B2B, email/push на финал; eventual consistency = design choice; критично явно сообщать о failed saga с refund | ✓ ПРИМЕНЯТЬ: customer-facing Saga (заказы, платежи, бронирования) 📋 ПРАВИЛО: Saga = инструмент бизнеса, UX команда — со-владелец states 🔗 См. Q1
