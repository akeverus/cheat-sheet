...
> [!mcq]
> - [ ] eBPF выполняет код в user-space после перехвата syscall | ❌ ПОСЛЕДСТВИЕ: eBPF выполняется прямо в kernel-space без переключения контекста — в этом и смысл нулевого overhead
> - [ ] eBPF требует патчинга ядра Linux при каждом новом use-case | ❌ ПОСЛЕДСТВИЕ: eBPF проверяется верификатором и загружается динамически без патчинга ядра; это и делает его безопасным
> - [x] eBPF выполняет sandboxed программы прямо в ядре; Cilium использует eBPF для L3/L4/L7 policy без kube-proxy overhead | ✓ ПРИМЕНЯТЬ: Cilium CNI, XDP packet processing, observability без агентов 📋 ПРАВИЛО: eBPF = safe kernel programmability; Cilium = eBPF CNI 🔗 См. Q39
> - [ ] eBPF применяется только для network tracing, не для security | ❌ ПОСЛЕДСТВИЕ: eBPF используется для сетевой политики (Cilium), security (Falco), observability (Pixie) — не только трейсинг
...
> [!mcq]
> - [ ] Certificate pinning — пинировать весь сертификат (не публичный ключ), это надёжнее | ❌ ПОСЛЕДСТВИЕ: при ротации сертификата (каждые 1-2 года) все старые клиенты перестают работать; public key pinning гибче — ключ живёт дольше
> - [ ] Certificate pinning применим к любым HTTPS соединениям, включая браузеры | ❌ ПОСЛЕДСТВИЕ: в браузерах HPKP deprecated; pinning актуален только для мобильных приложений и backend-to-backend
> - [x] Клиент принимает только конкретный fingerprint публичного ключа; защищает от компрометированного CA; OkHttp CertificatePinner | ✓ ПРИМЕНЯТЬ: мобильные банковские приложения, high-security API 📋 ПРАВИЛО: pinning = хардкод fingerprint + защита от CA compromise 🔗 См. Q39
> - [ ] Certificate pinning заменяет TLS — достаточно только pinning без TLS | ❌ ПОСЛЕДСТВИЕ: pinning — надстройка над TLS, не замена; без TLS данные не зашифрованы
...
> [!mcq]
> - [ ] Всегда выбирать TCP — UDP ненадёжен и не подходит для production | ❌ ПОСЛЕДСТВИЕ: UDP идеален для game state, video streaming, market data feed — потеря старого кадра/тика некритична; принудительный TCP добавляет head-of-line blocking
> - [ ] UDP подходит для финансовых транзакций — меньше overhead | ❌ ПОСЛЕДСТВИЕ: financial orders требуют гарантии доставки и порядка; потерянный order = потеря денег; использовать TCP/FIX protocol
> - [ ] QUIC — это просто TCP over UDP, никакого преимущества | ❌ ПОСЛЕДСТВИЕ: QUIC устраняет head-of-line blocking через мультиплексирование потоков; HTTP/3 на QUIC в 2-3x быстрее на mobile networks с потерями
> - [x] UDP: video streaming, game state, DNS, market data (latency критична, потери допустимы); TCP: HTTP, databases, file transfer (гарантия нужна) | ✓ ПРИМЕНЯТЬ: UDP когда данные быстро устаревают; TCP когда нельзя терять 📋 ПРАВИЛО: UDP = fast&lossy; TCP = reliable&ordered; QUIC = best of both 🔗 См. Q1
