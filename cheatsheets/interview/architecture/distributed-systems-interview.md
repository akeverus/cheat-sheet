...
- [Q33. Как работает gossip-протокол и где он применяется?](#q33-как-работает-gossip-протокол-и-где-он-применяется)
- [Q34. Чем Raft отличается от Paxos: ключевые отличия](#q34-чем-raft-отличается-от-paxos-ключевые-отличия)
- [Q35. Какие есть алгоритмы leader election (Bully, Ring, Raft)?](#q35-какие-есть-алгоритмы-leader-election-bully-ring-raft)
- [Q36. Что такое Idempotency Key и зачем он нужен?](#q36-что-такое-idempotency-key-и-зачем-он-нужен)
- [Q37. Two-Phase Commit (2PC) против Saga: какие компромиссы?](#q37-two-phase-commit-2pc-против-saga-какие-компромиссы)
- [Q38. Распределённые транзакции: XA и Outbox Pattern](#q38-распределённые-транзакции-xa-и-outbox-pattern)
- [Q39. Что такое backpressure и какими механизмами он реализуется?](#q39-что-такое-backpressure-и-какими-механизмами-он-реализуется)
- [Q40. Consistent hashing: что дают virtual nodes и как бороться с hotspots?](#q40-consistent-hashing-что-дают-virtual-nodes-и-как-бороться-с-hotspots)
...
Все характеристики распределённой системы вытекают из одного факта: узлы не делят память и общаются только по ненадёжной сети. Отсюда — и сила (масштабируемость, отказоустойчивость), и трудности (нет общих часов, недетерминизм).
...
Коротко: распределённость покупает масштаб и отказоустойчивость ценой сложности и потери строгой согласованности. Это компромисс, а не «бесплатное улучшение» — вы платите за каждый плюс соответствующим минусом.

**Плюсы:**
- **Горизонтальное масштабирование** — добавляем узлы вместо апгрейда одного сервера (который упирается в потолок железа)
- **Отказоустойчивость** — реплики и несколько зон доступности: падение одного узла не роняет систему
- **Распределение нагрузки** — балансировка трафика между узлами
- **Гибкость** — каждый компонент использует оптимальный для своей задачи стек технологий
- **Географическая близость** — узлы ближе к пользователям, меньше latency (`CDN`, мультирегиональные деплои)

**Минусы:**
- **Сложность** разработки и отладки — сетевые задержки, недетерминизм, race conditions
- **Проблемы согласованности** данных между узлами (см. CAP-теорему)
- **Частичные отказы** — один узел упал, остальные работают; такие состояния нужно явно обрабатывать
- **Латентность** и зависимость от сети — каждый сетевой вызов на порядки дороже локального
- **Операционная сложность** — мониторинг, деплой и конфигурация десятков и сотен сервисов

**Эмпирическое правило:** не распределяйте систему, пока монолит реально не упёрся в потолок. Преждевременная распределённость добавляет всю сложность без выигрыша в масштабе.
...
**Консенсус** — задача согласования: все корректные (не-сбойные) узлы должны **выбрать одно и то же значение**, даже когда часть участников отказала или сеть теряет сообщения. На консенсусе строятся выбор лидера, распределённые блокировки, упорядоченный лог транзакций.

Важная теоретическая граница — теорема FLP: в полностью асинхронной системе, где возможен сбой хотя бы одного узла, детерминированный консенсус гарантированно завершиться не может. Реальные алгоритмы обходят это таймаутами и предположением о частичной синхронности сети (подробнее — Q31).
...
Сначала выбирают **модель согласованности** под требования бизнеса (strong, eventual, causal), а уже под неё — конкретный механизм. Чем строже согласованность, тем дороже она обходится в latency и доступности — это прямое следствие CAP-теоремы.
...
**Доступность** (availability) — доля времени, в течение которого система корректно отвечает на запросы. Её измеряют в «девятках»: каждая дополнительная девятка сокращает допустимый downtime примерно в 10 раз — и кратно удорожает инфраструктуру.
...
**Частичный отказ** — отказ одного или нескольких компонентов при работающих остальных: упал один сервис, выросла сетевая задержка, истёк таймаут. В монолите сбой чаще «всё или ничего»; в распределённой системе частичные отказы — **норма**, а не исключение, потому что узлов и сетевых вызовов между ними много.
...
**Компромисс:** слишком «агрессивный» circuit breaker даёт лишние отказы, а слишком «мягкий» — не защитит от каскадного сбоя; пороги всегда проверяют нагрузочными тестами.

## Q13. Что такое Retry с экспоненциальной задержкой?

**Retry с экспоненциальной задержкой** (exponential backoff) — стратегия повторных попыток, где пауза перед каждой следующей попыткой растёт экспоненциально (500 мс → 1 с → 2 с …). Смысл: если сервис перегружен, частые повторы только добивают его — наоборот, нужно дать ему время восстановиться, отступая всё дальше.
...
**Bulkhead** (переборка) — изоляция ресурсов: каждому внешнему сервису выделяется отдельный, ограниченный пул потоков или соединений, который не делится с остальными. Зачем: без изоляции один зависший сервис исчерпает общий пул потоков, и приложение перестанет обслуживать вообще все запросы, включая те, что с медленным сервисом не связаны. Bulkhead локализует ущерб — падение одной зависимости не валит остальные.
...
Разница в одном: **когда primary подтверждает запись клиенту**. Синхронная ждёт записи на реплики (или их кворум) — данные гарантированно не потеряются при падении primary, но клиент ждёт дольше. Асинхронная подтверждает сразу после записи на primary — быстро, но при сбое primary незареплицированные данные теряются (replication lag).

То есть это прямой компромисс между **долговечностью данных** и **latency/throughput**.
...
**Идемпотентность** — свойство операции, при котором её повторное выполнение с теми же входными данными даёт тот же результат и то же состояние системы, что и однократное. Это фундамент надёжности при retry: из-за таймаутов и сбоев сети клиент часто не знает, дошёл ли запрос, и повторяет его — идемпотентность делает такой повтор безопасным.

**Без идемпотентности** повтор приводит к дублям: второй платёж, два заказа, двойное списание денег.
...
**Distributed tracing** — отслеживание пути одного запроса через цепочку сервисов. Нужен потому, что в микросервисах один пользовательский запрос проходит через десяток сервисов, и по логам отдельных сервисов невозможно понять, где именно он замедлился или упал. Tracing сшивает все шаги в единую картину.
...
**Практический смысл:** FLP запрещает только **детерминированный** консенсус с гарантией завершения. Реальные алгоритмы обходят запрет, ослабляя одно из условий теоремы:
- **Рандомизация** — случайные таймауты (randomized election timeout) ломают симметрию, из-за которой возникает вечный тупик
- **Частичная синхронность** — допущение, что задержки в сети ограничены сверху (bounded network delay), хотя бы временами
- Из-за этого `Paxos` теоретически может зациклиться (livelock), когда два proposer'а бесконечно перебивают друг друга; на практике это лечат выбором единственного лидера

**Вывод для практики:** случайные таймауты в `Raft` — не костыль, а сознательный способ выйти из ситуации, которую FLP запрещает решить детерминированно. Системы жертвуют гарантией «всегда завершится» ради «почти всегда завершится очень быстро».
...
**Условие:** чтобы пережить `f` Byzantine-узлов, нужно минимум `3f + 1` узлов — против `2f + 1` для обычного crash fault. Разница в том, что лживым узлам нельзя верить на слово: системе нужно достаточно честных голосов, чтобы перевесить и сбойные, и активно дезинформирующие узлы, поэтому запас больше.
...
## Q33. Как работает gossip-протокол и где он применяется?

**Gossip protocol** (эпидемический протокол) — децентрализованный способ распространения информации в кластере: каждый узел периодически выбирает случайных соседей и обменивается с ними состоянием. Информация расходится «как слух» — экспоненциально, без центрального координатора.

Идея в том, что не нужно, чтобы каждый узел знал обо всех остальных или существовал единый источник правды. Достаточно, чтобы каждый постоянно делился тем, что знает, со случайными соседями — и за `O(log N)` раундов новость дойдёт до всего кластера из `N` узлов.

**Принцип работы:**
1. Каждые `T` миллисекунд узел выбирает `k` случайных узлов (fan-out)
2. Отправляет им своё состояние (или только дельту изменений)
3. Получатели обновляют своё состояние и на следующем цикле распространяют его дальше
4. За `O(log N)` раундов информация достигает всех `N` узлов

**Свойства:**
- **Eventual consistency** — информация расходится постепенно, без гарантии конкретного момента доставки
- **Высокая отказоустойчивость** — нет единой точки отказа, протокол продолжает работать при частичных сбоях
- **Масштабируемость** — узел общается только с `k` соседями, поэтому нагрузка на каждый узел не растёт с размером кластера
...
## Q34. Чем Raft отличается от Paxos: ключевые отличия

Оба решают одну задачу — консенсус для репликации лога, — но `Raft` спроектирован ради понятности, а `Paxos` исторически известен как «правильный, но трудный». Главная практическая разница: `Raft` даёт цельную спецификацию (leader election + log replication + safety в одном описании), тогда как базовый `Paxos` оставляет многие детали реализатору.
...
## Q35. Какие есть алгоритмы leader election (Bully, Ring, Raft)?

**Leader election** — процесс выбора единственного узла-координатора (лидера) среди равноправных узлов кластера. Нужен везде, где для корректности важна **единственная точка принятия решений**.

**Зачем нужен лидер:**
- Координация распределённых транзакций
- Управление распределёнными блокировками
- Единственная точка принятия решений — иначе два узла начнут принимать конфликтующие решения (split-brain)

Алгоритмы делятся на классические учебные (Bully, Ring — простые, но шумные и без защиты от split-brain) и production-стандарт на базе кворума (Raft), где лидера выбирает большинство, а term-номер исключает двух лидеров одновременно.
...
## Q36. Что такое Idempotency Key и зачем он нужен?

**Idempotency Key** — уникальный идентификатор операции, который клиент присылает в запросе, чтобы сервер мог безопасно отбросить повторы и не выполнить операцию дважды.

**Проблема, которую он решает:** при сбое сети клиент не знает, дошёл ли запрос и выполнилась ли операция (платёж, создание заказа). Естественная реакция — повторить запрос. Но без защиты повтор приведёт ко второму списанию или дублю заказа. Idempotency Key превращает «опасный повтор» в «безопасный»: сервер по ключу узнаёт, что эту операцию уже обработал, и просто возвращает сохранённый прошлый ответ.
...
## Q37. Two-Phase Commit (2PC) против Saga: какие компромиссы?

Оба обеспечивают целостность распределённой транзакции, но разными ценами. `2PC` даёт **сильную согласованность** (ACID) ценой блокировок и низкой доступности: пока идёт протокол, ресурсы заблокированы, а падение координатора подвешивает всех. `Saga` отказывается от изоляции и общей атомарности в пользу **доступности**: вместо отката работают компенсирующие транзакции, а согласованность становится eventual. Отсюда правило: `2PC` — для одной БД и XA, `Saga` — для микросервисов.
...
## Q38. Распределённые транзакции: XA и Outbox Pattern

Базовая проблема: нужно атомарно изменить БД **и** опубликовать событие в брокер. Два разных ресурса — обычная транзакция их не покрывает, а двойная запись «сначала в БД, потом в Kafka» теряет согласованность при сбое между шагами. Есть два подхода: тяжёлый `XA` (распределённая транзакция через 2PC) и лёгкий `Outbox` (всё в одну локальную транзакцию + отдельный relay). На практике почти всегда выбирают Outbox.

**XA-транзакции:**
XA — стандарт X/Open для распределённых транзакций. Координирует несколько resource managers (БД, JMS) в одной глобальной транзакции через 2PC под капотом.
...
## Q39. Что такое backpressure и какими механизмами он реализуется?

**Backpressure** (обратное давление) — механизм, при котором потребитель (downstream) сигнализирует производителю (upstream) о своей реальной пропускной способности, заставляя его замедлиться. Это защита от ситуации, когда быстрый производитель захлёбывает медленного потребителя.

**Что происходит без backpressure:**
- Producer быстрее Consumer → промежуточные очереди и буферы переполняются → OOM или скачки latency
- В распределённой системе перегрузка распространяется вверх по цепочке зависимостей — это и есть каскадный сбой (cascade failure)
...
## Q40. Consistent hashing: что дают virtual nodes и как бороться с hotspots?

**Consistent hashing** — алгоритм распределения данных по узлам, при котором добавление или удаление узла перемещает **минимальное** количество ключей (в среднем `K/N`), а не почти все.

**Почему обычный `hash % N` плох:** при формуле `node = hash(key) % N` изменение `N` (добавили/убрали узел) меняет остаток от деления почти для каждого ключа — в результате перераспределяется почти весь набор данных. Для кэшей и шардированных БД это катастрофа: массовая миграция и промахи кэша. Consistent hashing решает это, размещая узлы и ключи на кольце хэшей, где изменение числа узлов затрагивает только соседний сегмент.
...
- [Service Discovery](service-discovery-interview.md) — Consul, Eureka, etcd, ZooKeeper, Kubernetes DNS; client-side vs server-side;…
- [System Design: Распределённый Key-Value Store](../system-design/design-key-value-store-interview.md) — consistent hashing, replication, quorum (R+W>N), vector clocks, gossip, LSM…
