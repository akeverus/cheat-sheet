...
**Горизонтальное масштабирование** (`scale out`) — добавление новых узлов в систему. Плюсы: линейный рост производительности, отказоустойчивость. Минусы: нужна stateless-архитектура, балансировка, согласованность данных и более сложная эксплуатация. Типично для веб-приложений, микросервисов и воркеров очередей; масштабирование достигается добавлением реплик за [[load-balancing-interview|балансировщиком]] (`Kubernetes Deployment`, облачные группы).
...
**Горизонтальное** — когда нужен рост без потолка; для веб-приложений и [[microservices-interview|микросервисов]]; когда важна отказоустойчивость и эластичность (автоскейлинг).
...
| Воркеры [[kafka-interview|Kafka]] / `RabbitMQ` | Горизонтально |
| `Read replicas` БД | Горизонтально |

## Q4. Что такое эластичное масштабирование (autoscaling)?

Эластичное масштабирование — автоматическое добавление или удаление узлов в зависимости от метрик (`CPU`, память, длина очереди, `latency`). В [[kubernetes-interview|Kubernetes]] — `HPA` (`Horizontal Pod Autoscaler`) по `CPU`/памяти или кастомным метрикам; в облаках — группы автоскейлинга.
...
Выявление:
1. **Профилирование** — `CPU`, память, `GC`, потоки (см. [[application-profiling-interview|профилирование приложений]])
...
Кэш снижает нагрузку на источник данных (БД, внешний `API`) и уменьшает латентность. Часто читаемые данные кэшируются в памяти (локальный кэш) или в распределённом кэше ([[redis-interview|Redis]]). Подробнее о стратегиях — в [[caching-strategies-interview|вопросах по кэшированию]].
...
При горизонтальном масштабировании каждый инстанс держит свой локальный кэш; для согласованности между инстансами используют распределённый кэш или инвалидацию по событиям ([[kafka-interview|Kafka]], `Redis Pub/Sub`).
...
При инвалидации нужно очищать оба уровня: `L2` через `Redis`, `L1` через `Redis Pub/Sub` или [[kafka-interview|Kafka]], чтобы все инстансы сбросили локальный кэш.
...
В масштабируемой системе `event-driven` инвалидация через [[kafka-interview|Kafka]] — самый надёжный способ: при изменении данных публикуется событие, все инстансы-подписчики инвалидируют свой локальный кэш.

## Q9. (!) Что такое шардирование и когда его применять?

Шардирование — горизонтальное разделение данных по ключу между несколькими узлами (БД, топики [[kafka-interview|Kafka]]). Применяют при росте объёма данных, когда один узел не справляется.
...
**Шардирование** — данные физически на разных серверах; нужен роутинг и может понадобиться [[consistency-patterns-interview|согласованность]] между шардами.

## Q12. (!) Как асинхронность и очереди помогают масштабированию?

Синхронный запрос блокирует поток до ответа; при пиках все потоки заняты и `latency` растёт. Асинхронная обработка: `API` принимает запрос, ставит задачу в очередь ([[kafka-interview|Kafka]], `RabbitMQ`) и сразу отвечает; воркеры обрабатывают очередь параллельно.
...
В [[kafka-interview|Kafka]] `backpressure` косвенный: мониторинг `consumer lag` триггерит алерт или автоскейлинг воркеров через `KEDA`.
...
Ограничение: `eventual consistency` для чтения — реплики могут отставать. Подробнее о моделях консистентности — в [[consistency-patterns-interview|паттернах согласованности]].
...
Без метрик невозможно понять, где узкое место и когда масштабировать. Подробнее — в [[observability-interview|вопросах по observability]] и [[metrics-tracing-interview|метрикам и трейсингу]].
...
В [[kubernetes-interview|Kubernetes]] `HPA` использует метрики из `Metrics Server` или внешних адаптеров (`Prometheus`); `KEDA` добавляет метрики из очередей ([[kafka-interview|Kafka]] `lag`, `RabbitMQ` length). Трассировка (`Jaeger`, `Zipkin`) помогает выявить медленные звенья в цепочке вызовов.
...
**Auto-scaling по кастомным метрикам** — масштабирование подов по метрикам, отличным от `CPU`: `RPS`, `latency`, длина очереди [[kafka-interview|Kafka]], число сообщений в `RabbitMQ`.

В [[kubernetes-interview|Kubernetes]] `HPA` по умолчанию масштабирует по `CPU` и памяти; кастомные метрики подключают через `Prometheus Adapter` или `KEDA`.
...
1. **Stateless** — не хранить состояние в памяти; сессии в [[redis-interview|Redis]]
...
8. **Health checks** — readiness/liveness для [[kubernetes-interview|Kubernetes]]
...
1. **Оптимизация запросов** — индексы, `EXPLAIN`, [[sql-interview|SQL]]-оптимизация
2. **Кэширование** — [[redis-interview|Redis]], `Caffeine`
...
- Лимит соединений и настройки [[hibernate-interview|Hibernate]]
...
Поисковые движки ([[elasticsearch-interview|Elasticsearch]], `OpenSearch`, `Solr`) масштабируются горизонтально: кластер из нескольких узлов; индексы шардируются и реплицируются.
...
Каждый [[microservices-interview|микросервис]] масштабируется независимо: горизонтально за [[load-balancing-interview|балансировщиком]] ([[kubernetes-interview|Kubernetes]] `Deployment`, облачные группы); автоскейлинг по `CPU`, памяти или кастомным метрикам.

Ключевые принципы:
- Состояние — во внешних хранилищах (БД, кэш, очереди)
- `API` — идемпотентные где возможно
- Service discovery — `Kubernetes Service`, `Consul`, [[spring-cloud-interview|Spring Cloud]]
...
**Consistent hashing** решает проблему `resharding`: при добавлении/удалении узла перемещается минимум данных. Используется в [[redis-interview|Redis Cluster]], `Cassandra`, [[kafka-interview|Kafka]].
...
Инструменты: `Gatling`, `k6`, `JMeter`; в [[kubernetes-interview|Kubernetes]] — запуск нескольких реплик и проверка равномерного распределения запросов.
...
- [[load-balancing-interview|Балансировщик]] исключает нездоровые инстансы
...
Типичные сценарии:
1. Убийство случайного пода в [[kubernetes-interview|Kubernetes]]
2. Задержка или отказ вызова к БД / внешнему `API`
3. Переполнение очереди [[kafka-interview|Kafka]]
4. Исчерпание пула соединений
5. Сетевой partition между сервисами (см. [[cap-theorem-interview|CAP-теорему]])
...
- [[distributed-systems-interview|Распределённые системы]] — CAP теорема, репликация, партиционирование и согласованность
- [[caching-strategies-interview|Стратегии кэширования]] — Cache-Aside, Write-Through, CDN и локальные кэши для снижения нагрузки
- [[load-balancing-interview|Балансировка нагрузки]] — алгоритмы балансировки, health checks и горизонтальное масштабирование
- [[database-architecture-interview|Архитектура баз данных]] — шардирование, read replica и партиционирование для масштабирования
- [[microservices-interview|Микросервисная архитектура]] — декомпозиция и независимое масштабирование сервисов
- [[cqrs-event-sourcing-interview|CQRS и Event Sourcing]] — разделение read/write нагрузки для масштабирования
- [[kafka-interview|Apache Kafka]] — партиционирование и потоковая обработка для высокопроизводительных систем
- [[kubernetes-interview|Kubernetes]] — HPA, VPA и автоматическое масштабирование приложений
