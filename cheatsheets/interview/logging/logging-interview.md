...
> [!mcq]
> - [ ] Импортировать `ch.qos.logback.classic.Logger` напрямую — кодовая база зависит от конкретной реализации | Прямой импорт привязывает к Logback и блокирует переключение бэкенда. ❌ ПОСЛЕДСТВИЕ: при переходе на Log4j2 (CVE-фикс или async) — массовый refactor 200+ файлов на 2 спринта.
> - [ ] Использовать `java.util.logging` (JUL) для всего нового кода | JUL лишён `{}` placeholder, `MDC`, structured-encoders, async из коробки. ❌ ПОСЛЕДСТВИЕ: интеграция с ELK через GELF/Logstash требует кастомного `Handler` — техдолг на годы.
> - [x] Писать код против `SLF4J` API, а реализацию (`Logback`/`Log4j2`) подключать только через classpath | `SLF4J` — фасад: `LoggerFactory.getLogger(...)` возвращает абстрактный `Logger`, биндинг выбирается в момент сборки. ✓ ПРИМЕНЯТЬ: дефолт в `spring-boot-starter` (Logback под SLF4J). 📋 ПРАВИЛО: «API в коде, реализация в classpath». 🔗 См. Q8, Q16.
> - [ ] Использовать `Log4j 1.x` — он проще и стабилен | Log4j 1.x EOL с 2015, без CVE-патчей и `{}` placeholder. ❌ ПОСЛЕДСТВИЕ: security-аудит блокирует релиз — найден `org.apache.log4j:log4j:1.2.x` с открытыми CVE.
...
> [!mcq]
> - [ ] `Logger` пишет в файл, `Appender` форматирует строку | Перепутаны роли: запись создаёт `Logger`, форматирует `Layout`/`Encoder`, доставляет `Appender`. ❌ ПОСЛЕДСТВИЕ: невозможно настроить multi-destination (один `Logger` → несколько `Appender`) — логи теряются.
> - [x] `Logger` создаёт запись, `Appender` доставляет, `Layout`/`Encoder` форматирует, `Filter` — фильтрует | Четырёхступенчатый pipeline: каждое звено имеет одну ответственность; иерархия логгеров наследуется по имени-пакету. ✓ ПРИМЕНЯТЬ: Logback в Spring Boot — `RootLogger` → `ConsoleAppender` + `RollingFileAppender` через тот же `LogstashEncoder`. 📋 ПРАВИЛО: «Logger создаёт, Appender везёт, Layout одевает». 🔗 См. Q3, Q4, Q5.
> - [ ] Достаточно одного `Appender` — он сам форматирует и фильтрует | Без `Encoder` `Appender` пишет `toString()` события, без `Filter` нет фильтрации по маркеру. ❌ ПОСЛЕДСТВИЕ: в файле сырые `LoggingEvent@hash` строки — Filebeat не парсит, Kibana показывает мусор.
> - [ ] Иерархия логгеров не нужна — каждый класс имеет свой уровень | Без иерархии `logging.level.com.example=DEBUG` не работает, придётся прописывать каждый класс. ❌ ПОСЛЕДСТВИЕ: 500-строчный `application.yml` с уровнями для каждого класса — невозможно поддерживать.
...
> [!mcq]
> - [ ] `private Logger log = LoggerFactory.getLogger(this.getClass())` — берёт реальный класс, удобнее для наследования | На каждом инстансе создаётся новый `Logger`-lookup, при subclass логгер привязан к subclass-имени — наследник попадает не в тот пакет логирования. ❌ ПОСЛЕДСТВИЕ: `logging.level.com.example.parent=DEBUG` не действует на subclass из другого пакета — DEBUG молча отключён.
> - [x] `private static final Logger log = LoggerFactory.getLogger(MyClass.class)` — один логгер на класс, имя совпадает с FQCN | `static final` — ноль накладных на инстанс; FQCN-имя даёт корректную иерархию `com.example.*`. ✓ ПРИМЕНЯТЬ: канонический паттерн Spring Boot и `@Slf4j` от Lombok. 📋 ПРАВИЛО: «static final + FQCN — один раз, навсегда». 🔗 См. Q1, Q8.
> - [ ] `Logger log = LoggerFactory.getLogger("MyLogger")` — кастомное имя удобнее | Произвольное имя ломает иерархию по пакетам, `logging.level.com.example=*` не действует. ❌ ПОСЛЕДСТВИЕ: рефакторинг переименовал класс — все настройки уровней слетели, `DEBUG` молча отключился.
> - [ ] Создавать логгер в каждом методе через `LoggerFactory.getLogger(...)` | Lookup в `LoggerContext` через `ConcurrentMap` на каждом вызове — лишний CPU в hot path. ❌ ПОСЛЕДСТВИЕ: в горячем endpoint p99 latency вырастает на 5–10 мс из-за concurrent map lookup.
...
> [!mcq]
> - [ ] `FileAppender` без ротации — для production-сервиса с большим объёмом | Файл растёт безгранично, без `maxFileSize`/`maxHistory`. ❌ ПОСЛЕДСТВИЕ: `app.log` 200 ГБ забил `/var/log`, kubelet выкинул pod по `DiskPressure`, downtime 30 минут.
> - [ ] `ConsoleAppender` в production-кластере с локальным диском | stdout буферизуется, при контейнере без log-driver логи теряются после рестарта. ❌ ПОСЛЕДСТВИЕ: после OOM-kill нет ни одной строки причины — RCA невозможен.
> - [x] `RollingFileAppender` для prod (с ротацией по дате+размеру), `ConsoleAppender` для контейнеров (читает Docker/k8s log-driver) | Каждый назначению свой Appender; в Kubernetes stdout → log-driver → CloudWatch/ELK без файлов. ✓ ПРИМЕНЯТЬ: Spring Boot в k8s — `ConsoleAppender` с `LogstashEncoder` → Fluent Bit → Elasticsearch. 📋 ПРАВИЛО: «контейнер — stdout, VM — RollingFile». 🔗 См. Q11, Q20, Q31.
> - [ ] `SocketAppender` с прямой отправкой в Logstash без локального буфера | При недоступности Logstash блокирует поток приложения или теряет записи. ❌ ПОСЛЕДСТВИЕ: ELK 5 минут лежит — все Tomcat threads висят на `socket.write()`, сервис не отвечает.
...
> [!mcq]
> - [ ] Использовать `%msg` без `%n` в конце pattern | Без `%n` все записи склеятся в одну строку, Filebeat не разделит события. ❌ ПОСЛЕДСТВИЕ: Kibana показывает один document на весь час — отфильтровать по `traceId` невозможно.
> - [ ] В production использовать `%C` (caller class) и `%L` (line number) | `%C` и `%L` требуют создания stack trace на каждом логе — overhead 5–10x. ❌ ПОСЛЕДСТВИЕ: throughput сервиса упал с 1000 RPS до 200 RPS после включения caller info — APM показывает 80% CPU в `Thread.getStackTrace()`.
> - [x] В production — `LogstashEncoder` (JSON) с `%X{traceId}` через MDC; в dev — `PatternLayout` с цветами и кратким `%logger{36}` | JSON для машинной обработки в ELK, текст с цветами — для человеческого чтения локально; `%X{key}` достаёт MDC-поле. ✓ ПРИМЕНЯТЬ: Netflix logger-config — pattern по `springProfile`, `%X{traceId}` для корреляции с Zipkin. 📋 ПРАВИЛО: «prod — JSON, dev — colored pattern». 🔗 См. Q9, Q10, Q36.
> - [ ] Один и тот же `PatternLayout` для dev и prod | В prod нужен JSON для ELK, в dev — читаемый текст. ❌ ПОСЛЕДСТВИЕ: ELK не может распарсить `%d %level %msg` — поля `level`, `traceId` не индексируются, поиск по logs работает только grep'ом.
...
> [!mcq]
> - [ ] Логировать каждую SQL-операцию на `INFO` в production | `INFO` льётся в централизованный лог; на 5K RPS получаем десятки тыс. записей/сек только от Hibernate. ❌ ПОСЛЕДСТВИЕ: ELK ingestion node при 100 МБ/мин падает, индекс `app-logs-*` отстаёт на 4 часа — алерт по ошибке прилетает после инцидента.
> - [ ] Любую `Exception` в `catch` логировать как `WARN` | `WARN` — для recoverable; необработанное исключение — это `ERROR` с stack trace. ❌ ПОСЛЕДСТВИЕ: PagerDuty alert настроен на `level=ERROR`, реальный NullPointerException под `WARN` — никто не разбудился, дефект 12 часов в проде.
> - [x] `INFO` — бизнес-события, `WARN` — recoverable (retry/fallback), `ERROR` — необработанная ошибка с stacktrace, `DEBUG`/`TRACE` отключены в prod | Каждый уровень имеет чёткую семантику: ERROR триггерит алерт, INFO — для post-mortem, DEBUG — только локально. ✓ ПРИМЕНЯТЬ: SRE-handbook Google — alerting rules по `level=ERROR`, dashboard counts по `WARN`. 📋 ПРАВИЛО: «ERROR будит, WARN просыпается утром, INFO для разбора». 🔗 См. Q19, Q33, Q22.
> - [ ] `FATAL` использовать для всех ошибок API-вызовов | `FATAL` зарезервирован под «приложение нерабочее»; в SLF4J его вообще нет (только в Log4j2). ❌ ПОСЛЕДСТВИЕ: при переходе со SLF4J 1.x на Log4j2 все `log.fatal(...)` через рефлексию упадут с `NoSuchMethodError`.
...
> [!mcq]
> - [ ] Использовать `logback.xml` вместо `logback-spring.xml` в Spring Boot | `logback.xml` загружается до `Environment`, `<springProfile>` и `${spring.application.name}` не работают. ❌ ПОСЛЕДСТВИЕ: dev/prod конфигурация одинаковая — DEBUG-логи Hibernate улетают в prod, инцидент по приватным данным в логах.
> - [x] `logback-spring.xml` для prod (с `<springProfile>` и Spring placeholders), `logging.level.*` в `application.yml` для быстрых правок, `/actuator/loggers` для runtime-смены | Каждый канал решает свою задачу: XML для архитектуры, YAML — для конкретики, Actuator — для on-call. ✓ ПРИМЕНЯТЬ: Spring Boot Admin показывает `loggers` UI — SRE меняет уровень без рестарта. 📋 ПРАВИЛО: «XML — структура, YAML — настройки, Actuator — runtime». 🔗 См. Q22, Q35.
> - [ ] Конфигурировать только программно через `LoggerContext.getLogger(...).setLevel(...)` | Программная настройка не переживает `LoggerContext.reset()` от auto-reload, перетирается. ❌ ПОСЛЕДСТВИЕ: Spring Boot DevTools перезагрузил контекст — кастомные уровни сбросились на дефолт, dev-логи пропали.
> - [ ] `-Dlogging.level.root=DEBUG` в JVM-args для production | DEBUG для root залогирует Hibernate, Spring, Tomcat — ливень в десятки тысяч строк в секунду. ❌ ПОСЛЕДСТВИЕ: на нагрузке 200 RPS диск заполнен за 15 минут, k8s evict pod, каскадный отказ сервиса.
...
> [!mcq]
> - [ ] SLF4J нужен только для красоты — `LogManager.getLogger()` из Log4j2 даёт то же самое | Прямая зависимость на Log4j2 API делает невозможным смену бэкенда; CVE-2021-44228 потребовал бы переписывать импорты во всех файлах. ❌ ПОСЛЕДСТВИЕ: Equifax/похожие — найден log4j 2.14 в проде, SLA на патч 4 часа, реальный refactor занял 3 дня → штраф регулятора.
> - [ ] `log.info("user " + user + " did " + action)` — конкатенация в SLF4J не отличается от placeholder | Конкатенация выполняется ДО `isInfoEnabled()` — `toString()` тяжёлых объектов сжигает CPU даже при отключённом INFO. ❌ ПОСЛЕДСТВИЕ: hot path на 10K RPS — 30% CPU уходит на `User.toString()` для DEBUG, который выключен.
> - [x] SLF4J — фасад: `LoggerFactory.getLogger(...)` + `log.info("user {} did {}", user, action)`; биндинг (`logback-classic` / `log4j-slf4j2-impl`) подключается отдельно | Код не знает о реализации; placeholder проверяет уровень ДО `toString()`; единый API для всех library-deps. ✓ ПРИМЕНЯТЬ: все Spring-libs логируют через SLF4J — переключение на Log4j2 одной зависимостью. 📋 ПРАВИЛО: «фасад API + placeholder = ноль связности и ноль CPU». 🔗 См. Q1, Q12, Q16.
> - [ ] Использовать `Apache Commons Logging` (JCL) как фасад вместо SLF4J | JCL делает classloader-based discovery — в OSGi/Spring DevTools ломается, к тому же без `{}`. ❌ ПОСЛЕДСТВИЕ: «commons-logging conflict» при двух классах `LogFactory` в classpath — приложение не стартует.
...
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q10. (!) Что такое структурированное логирование (`JSON`)? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q11. Как настроить ротацию лог-файлов (`RollingFileAppender`)? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q12. (!) Почему параметризованные вызовы логгера предпочтительнее конкатенации? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q13. Что такое асинхронный аппендер и когда его использовать? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q14. (!) Как связать логи с распределённой трассировкой? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q15. Как не логировать чувствительные данные? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q16. Чем `Logback` отличается от `Log4j2`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q17. Как уменьшить объём логов от сторонних библиотек? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q18. (!) Что такое `correlation id` и как его использовать в микросервисах? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q19. Как логировать исключения правильно? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q20. (!) Как интегрировать логи с `ELK`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q21. Что такое `Markers` в `SLF4J`/`Logback` и когда их использовать? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q22. Как настроить уровни логирования по окружению (`dev`/`prod`)? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q23. Что такое `log sampling` и когда его применять? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q24. Как логировать в многопоточном и асинхронном коде? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q25. Что такое `Fluent API` в `Log4j2`? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q26. Как не раздувать логи при высокой нагрузке? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q27. Как интегрировать логи с метриками (`Micrometer`, `Prometheus`)? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q28. Что такое `log aggregation` и зачем он нужен? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q29. Как обеспечить консистентность формата логов в микросервисах? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q30. Как логировать в реактивных стеках (`WebFlux`, `Project Reactor`)? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q31. (!) Как настроить EFK-стек (Elasticsearch, Fluent Bit, Kibana)? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q32. Как правильно использовать MDC в многопоточном коде с thread pools? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q33. Какие best practices по уровням логирования в production? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q34. (!) Чем Log4j2 async loggers отличаются от Logback AsyncAppender? ❌ ПОСЛЕДСТВИЕ: антипаттерн деградирует SLA при росте нагрузки или зависимостей.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q35. Как работает Spring Boot Logging Auto-configuration? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q36. Что такое ECS Layout и зачем он нужен? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q37. Как тестировать логирование с MemoryAppender в unit-тестах? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q38. Как использовать StructuredArguments для обогащения JSON-логов? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q39. Как работает Log4j2 garbage-free logging? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q40. Как настроить Graylog / GELF для приёма логов из Java? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление## Q41. Что такое Log Appender для Kafka и когда его применять? ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> - [ ] Альтернативное решение которое не подходит | Почему ошибка в этом подходе ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Другая альтернатива с критическим недостатком | Это смежное, но отличное понятие ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
> - [ ] Третий вариант который не работает в production | Противоположное направление- [AI Agents](../ai-ml/ai-agents-interview.md) ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
