...
При переполнении bulkhead бросает `BulkheadFullException`. Её перехватывают через `onErrorResume` и превращают в осмысленную «сервис перегружен», вместо того чтобы пускать запрос дальше и усугублять давку.
...
- **Хореография** — единого дирижёра нет. Каждый сервис подписан на события и в ответ публикует свои. Связность низкая, но общий ход саги «размазан» по сервисам, и его сложно отследить.
...
**Хореографическая реализация — пошагово.** Шиной служит брокер событий (Kafka или RabbitMQ), каждый сервис знает только «свои» события и ничего не знает о соседях:

1. `OrderService` создаёт заказ в статусе `PENDING` и публикует событие `OrderCreated`.
2. `StockService` подписан на `OrderCreated`: пытается зарезервировать склад и публикует `StockReserved` (успех) или `StockRejected` (отказ).
3. `PaymentService` подписан на `StockReserved`: проводит оплату и публикует `PaymentCompleted` или `PaymentFailed`.
4. `OrderService` подписан на исходы: по `PaymentCompleted` переводит заказ в `CONFIRMED`, по `StockRejected` или `PaymentFailed` — в `CANCELLED`.

Компенсации — тоже события отмены, а не прямые вызовы: на `PaymentFailed` подписан `StockService` — он снимает резерв; «обратных» событий ровно столько, сколько шагов вперёд. В реактивном стеке каждый участник — это reactive-consumer (например, `reactor-kafka` или Spring Cloud Stream): обрабатывает входной `Flux` событий и публикует результат обратно в брокер.

Пример кода ниже — **оркестрационный контраст**, а не хореография: в нём есть центральный класс-дирижёр, который зовёт сервисы напрямую цепочкой `flatMap` и сам откатывает шаги через `onErrorResume` (при сбое шага 2 откатывается шаг 1, при сбое шага 3 — шаги 2 и 1). Хореография отличается тем, что таких прямых вызовов и единого класса нет вообще — каждый шаг и каждая компенсация запускаются реакцией сервиса на событие в брокере.
...
Backpressure — это ситуация, когда producer выдаёт элементы быстрее, чем consumer успевает обрабатывать.

**Нативный механизм Reactor — pull-модель.** Подписчик сам объявляет спрос через `request(n)`, и обычные cold-источники (коллекции, запросы к БД, `WebClient`) производят ровно столько, сколько запрошено, — backpressure работает «из коробки», без специальных операторов. Оператор `limitRate(n)` управляет этим спросом в середине цепочки: запрашивает у источника порциями по `n`, не давая жадному подписчику выкачать всё разом.

Операторы `onBackpressure*` нужны для источников, которые спрос **игнорируют**: hot-источники вроде `Flux.interval`, потока UI-событий или внешнего брокера эмитят в собственном ритме, не глядя на `request(n)`. Для них Reactor не даёт одного решения на все случаи, а предлагает выбрать стратегию исходя из того, что важнее: ничего не потерять или не переполнить память.
...
- `sampleTimeout(...)` — ждёт **тишины**: эмитит элемент, только если после него в течение окна ничего не пришло. Идеален для поиска по вводу — запрос уходит, когда пользователь перестал печатать, а не на каждое нажатие. Принимает не `Duration`, а функцию-компаньон (`sampleTimeout(v -> Mono.delay(d))`) — окно тишины можно вычислять для каждого элемента своё.
- `sampleFirst(Duration)` — пропускает **первый** элемент за период и глушит остальные. Подходит для защиты от дабл-кликов и частых кнопок.
...
Коротко: sampleTimeout — «дождаться паузы», sampleFirst — «первый в окне», sample — «последний в окне». Важный нюанс именования: привычные `debounce` и `throttleFirst` — это операторы RxJava, в Project Reactor их просто нет; соответствие имён — debounce → `sampleTimeout`, throttleFirst → `sampleFirst`.

```java
// Debounce — поиск по вводу (не запускать пока пользователь набирает)
Flux<String> searchResults = searchInput
        .sampleTimeout(q -> Mono.delay(Duration.ofMillis(300))) // подождать 300ms тишины
...
Flux<Void> buttonClicks = clickEvents
        .sampleFirst(Duration.ofSeconds(1));      // максимум 1 клик в секунду
