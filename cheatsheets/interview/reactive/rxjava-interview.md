...
> [!mcq]
> - [ ] `RxJava` использует **pull**: подписчик сам тянет элементы через `next()`, как `Iterator` | Это модель `Stream`/`Iterator`, а не `RxJava`. В реактивной парадигме потребитель пассивен. ❌ ПОСЛЕДСТВИЕ: попытка реализовать «next()-цикл» поверх `Observable` ломает асинхронность и блокирует поток на `BlockingObservable`.
> - [x] `RxJava` использует **push**: `Observable` сам вызывает `onNext()` подписчику; `Flowable` добавляет pull через `request(n)` для backpressure | Push — основная модель `Observer`-паттерна, а `Flowable` гибрид с pull-сигналом. ✓ ПРИМЕНЯТЬ: Netflix Hystrix, Android RxBinding, RxKafka — везде источник проталкивает события подписчикам. 📋 ПРАВИЛО: «`Observable` пушит, `Flowable` пушит и слушает `request(n)`». 🔗 См. Q31, Q32.
> - [ ] `RxJava` всегда работает в hybrid push-pull: каждый `Observable` уведомляет о готовности через `request(n)` | `Observable` НЕ поддерживает backpressure и `request(n)` — это API только `Flowable` из `Reactive Streams`. ❌ ПОСЛЕДСТВИЕ: код, рассчитанный на `request()` для `Observable`, упирается в `MissingBackpressureException` при подключении быстрого источника.
> - [ ] Это зависит от `Scheduler`: `io()` — push, `computation()` — pull | `Scheduler` управляет потоком исполнения, а не моделью доставки. ❌ ПОСЛЕДСТВИЕ: попытка «переключить» режим через смену `Scheduler` приводит к запутанному коду без эффекта — данные продолжают пушиться.
...
> [!mcq]
> - [ ] `onNext()` 0..N раз, `onComplete()` 0..N раз, `onError()` 0..N раз | `onComplete`/`onError` — терминальные сигналы и допускают только один вызов. ❌ ПОСЛЕДСТВИЕ: повторный `onError()` после `onComplete()` падает в `RxJavaPlugins.onError` и убивает процесс при отсутствии глобального handler.
> - [ ] `onNext()` ровно 1 раз, `onComplete()` 0..N раз, `onError()` 1 раз | `Observable` рассчитан на 0..N значений; ограничение в 1 элемент описывает `Single`, а не `Observable`. ❌ ПОСЛЕДСТВИЕ: при попытке использовать такой контракт для Kafka-стрима теряются все события кроме первого.
> - [ ] `onNext()` 0..N раз, `onComplete()` максимум 1 раз, `onError()` максимум 1 раз — после любого терминального сигнала поток закрыт | Контракт `onNext* (onComplete | onError)?` гарантирует ровно один терминальный сигнал. ✓ ПРИМЕНЯТЬ: ReactiveX-спецификация, RxJava 2/3, Reactor `Flux` — все следуют этому контракту. 📋 ПРАВИЛО: «`onNext*` потом ровно один из `onComplete` или `onError`». 🔗 См. Q3, Q35.
> - [ ] Все три метода можно вызвать любое число раз — `RxJava` сама дедуплицирует | `RxJava` не дедуплицирует терминалы; повторные вызовы либо игнорируются (RxJava 1), либо уходят в plugin-handler (RxJava 2+). ❌ ПОСЛЕДСТВИЕ: рассыпание `onComplete()` в нескольких местах custom-оператора создаёт data race и `IllegalStateException` под нагрузкой.
...
> [!mcq]
> - [ ] После `onComplete()` следующий `onNext()` пробрасывается дальше как обычное значение | После терминала события игнорируются emitter'ом; в RxJava 2+ они уходят в `RxJavaPlugins.onError`. ❌ ПОСЛЕДСТВИЕ: код, который "досылает" значения после `onComplete()`, теряет данные тихо — баг проявляется только в проде через метрики-расхождения.
> - [ ] `onNext()` можно вызывать пока есть подписчики — даже после `onError()` | После терминала emitter становится disposed; повторные `onNext` игнорируются. ❌ ПОСЛЕДСТВИЕ: попытка retry «вручную» через повторный `onNext` после `onError` приводит к молчаливой потере событий — ошибка ловится только PR-ревью.
> - [ ] `onNext()` от 0 до ∞, `onComplete()` ≤1, `onError()` ≤1, причём `onComplete` и `onError` взаимоисключающие | Это формальный контракт `onNext* (onComplete | onError)?`. ✓ ПРИМЕНЯТЬ: документация ReactiveX и Reactive Streams JVM TCK тестирует именно это правило. 📋 ПРАВИЛО: «один поток — один финал». 🔗 См. Q2, Q31.
> - [ ] `onComplete()` обязательно вызывается всегда после `onError()` для cleanup | `onError()` сам по себе финализирует поток; `onComplete` после него запрещён контрактом. ❌ ПОСЛЕДСТВИЕ: cleanup-логика, навешанная на `onComplete` после ошибки, никогда не выполнится — ресурсы (`FileInputStream`, `Connection`) текут.
...
> [!mcq]
> - [ ] `Single<T>` отдаёт от 0 до 1 значений и поддерживает backpressure | `Single` отдаёт **ровно одно** значение или ошибку и НЕ имеет backpressure. ❌ ПОСЛЕДСТВИЕ: миддл, выбравший `Single` для batch-загрузки 10K строк из БД, получит `OnErrorNotImplementedException`, когда репозиторий вернёт `Iterable`.
> - [ ] `Maybe<T>` всегда испускает ровно 1 элемент, как `Single` | `Maybe` отдаёт **0 или 1** — это `Optional<T>` в реактивном виде. ❌ ПОСЛЕДСТВИЕ: при `cache.get(key)` с пустым результатом подписчик `Maybe` корректно получит `onComplete` без значения, а реализация под `Single` упадёт с `NoSuchElementException`.
> - [ ] `Completable` испускает ровно одно значение `Void` для совместимости с `CompletableFuture` | `Completable` НЕ испускает значений — только `onComplete()` или `onError()`. ❌ ПОСЛЕДСТВИЕ: попытка получить «Void»-значение через `blockingGet()` возвращает `null` и роняет downstream NPE при `result.toString()`.
> - [x] `Observable` (0..N без backpressure), `Flowable` (0..N с backpressure), `Single` (ровно 1), `Maybe` (0 или 1), `Completable` (только сигнал завершения) | Пять типов покрывают разные кардинальности и backpressure-семантику. ✓ ПРИМЕНЯТЬ: Spring WebFlux mapping, RxJava-Retrofit adapter — выбирают `Single` для запроса-ответа, `Completable` для PUT/DELETE без тела. 📋 ПРАВИЛО: «кардинальность диктует тип». 🔗 См. Q31, Q32, Q41.
...
> [!mcq]
> - [ ] `RxJava 1` уже поддерживает `Flowable` и `Reactive Streams` | `Flowable` появился в `RxJava 2`; в первой версии backpressure был встроен в `Observable` без отдельного типа. ❌ ПОСЛЕДСТВИЕ: миграция legacy-кода с `rx.Observable` 1.x путём добавления `Flowable` ломает компиляцию — пакет `io.reactivex` ещё не существовал.
> - [ ] `null` разрешён в потоках во всех версиях `RxJava` | `null` запрещён начиная с `RxJava 2` — `onNext(null)` бросает `NullPointerException`. ❌ ПОСЛЕДСТВИЕ: миграция кода с RxJava 1 → 2 без проверки `null` приводит к NPE в production при первом `null` из БД.
> - [ ] `RxJava 2` использует пакет `io.reactivex`, `RxJava 3` — `io.reactivex.rxjava3`, `null` запрещён в обеих, обе совместимы с `Reactive Streams` | Пакет `io.reactivex.rxjava3.*` позволяет одновременно держать обе версии в classpath. ✓ ПРИМЕНЯТЬ: Android-проекты в фазе миграции часто используют RxJava 2 + 3 параллельно через jakewharton/RxRelay-3. 📋 ПРАВИЛО: «`io.reactivex` для v2, `io.reactivex.rxjava3` для v3». 🔗 См. Q1, Q4.
> - [ ] `RxJava 3` отказался от `Disposable` в пользу `Subscription` из `Reactive Streams` | Наоборот: `RxJava 2/3` ушли от `Subscription` (RxJava 1) к `Disposable`. ❌ ПОСЛЕДСТВИЕ: Android-разработчик, ожидающий старый `Subscription.unsubscribe()` в RxJava 3, получает `NoSuchMethodError` при сборке release-build с R8.
...
> [!mcq]
> - [x] Marble-диаграмма — визуализация оператора через временную ось с элементами-«шариками», где `|` обозначает `onComplete`, а `X` — `onError` | Нотация стандартизирована в ReactiveX и используется в JavaDoc. ✓ ПРИМЕНЯТЬ: ReactiveX.io документация, Reactor reference guide, JavaDoc на `flatMap`/`switchMap` — везде marble-диаграммы для семантики. 📋 ПРАВИЛО: «время слева направо, `|` = complete, `X` = error». 🔗 См. Q15, Q16.
> - [ ] Marble-диаграмма — это UML-диаграмма последовательности с `Observable` и `Subscriber` | UML sequence-диаграммы показывают вызовы методов между объектами, а не временной поток событий. ❌ ПОСЛЕДСТВИЕ: попытка задокументировать `flatMap` через UML sequence теряет суть параллельности — ревьюер не может оценить порядок emission'ов.
> - [ ] Marble-диаграмма — текстовое представление потока в формате JSON для тестирования | JSON-snapshot — это `TestObserver.values()`, а не marble. Marble — графическая нотация. ❌ ПОСЛЕДСТВИЕ: попытка автоматически парсить «marble JSON» провалится — стандарта не существует, тесты падают на отсутствии формата.
> - [ ] Marble-диаграмма обязательна в коде через аннотацию `@Marble` для генерации документации | Аннотации `@Marble` в RxJava нет — это просто визуальная нотация в комментариях/JavaDoc. ❌ ПОСЛЕДСТВИЕ: попытка добавить `@Marble` в код приводит к ошибке компиляции и блокирует CI на merge.
