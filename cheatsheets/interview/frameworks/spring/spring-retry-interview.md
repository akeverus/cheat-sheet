...
> [!mcq] Как корректно протестировать `@Retryable`-метод и убедиться, что повторы реально выполняются?
>
> - [ ] **A) Создать сервис через `new PaymentService()` и вызвать метод в обычном JUnit-тесте без Spring-контекста**
>     - **Что на самом деле:** `@Retryable` работает только через Spring AOP-прокси. При `new` объект создаётся напрямую, прокси нет — ретраи не сработают, тест ничего не проверит, кроме одного вызова.
>     - **Откуда путаница:** в unit-тестах часто инстанцируют классы вручную. Для retry это не работает — нужен контекст с включённым AOP.
>     - **Если бы это было правдой:** Spring Retry работал бы как чистая Java-библиотека без прокси — но тогда исчезли бы декларативные `@Retryable` и `@Recover`.
> - [ ] **B) Запустить `@SpringBootTest`, но забыть про `@MockBean` и работать с реальным `PaymentGateway`**
>     - **Что на самом деле:** контекст поднимется, прокси будет, но реальный gateway либо не упадёт (нечего ретраить), либо ходит во внешнюю систему — тест становится нестабильным и медленным.
>     - **Откуда путаница:** интеграционные тесты иногда оправданы, но для проверки логики retry нужен контролируемый источник исключений — это работа `@MockBean`.
>     - **Если бы это было правдой:** мы бы зависели от реальной сети — flaky-тесты в CI и невозможность воспроизводить сценарии 429/5xx детерминированно.
> - [ ] **C) Использовать `@SpringBootTest`, но в `@Backoff` оставить delay = 5000ms без переопределения**
>     - **Что на самом деле:** тест технически пройдёт, но будет тормозить (3 retry × 5s = 15 секунд на один кейс). В CI с десятками тестов это неприемлемо.
>     - **Откуда путаница:** иногда забывают, что в тестах `@Backoff` нужно делать минимальным (0–10 мс) или вообще обходить через `RetryTemplate` без backoff.
>     - **Если бы это было правдой:** реальный backoff в production имеет смысл — но в тесте мы проверяем количество попыток и условия, а не таймауты.
> - [x] **D) Поднять `@SpringBootTest`, замокать зависимость через `@MockBean`, заставить мок бросать исключение и проверить `verify(..., times(N))`**
>     - **Развёрнутое объяснение:** `@SpringBootTest` (или `@SpringJUnitConfig` + `@EnableRetry`) поднимает контекст с AOP-прокси. `@MockBean` подменяет реальную зависимость моком, который контролируемо бросает нужное исключение. `verify(mock, times(maxAttempts))` подтверждает, что прокси действительно повторил вызов нужное число раз. Для проверки `@Recover` мокаем тот же сценарий и проверяем, что вернулся fallback-результат.
...
> [!mcq] Команда решает: нужен только retry с экспоненциальным backoff и `@Recover`-fallback для нескольких legacy-методов в монолите на Spring Boot 3. Какой выбор более уместен и почему?
>
> - [x] **A) Spring Retry — он закрывает ровно эту задачу декларативно (`@Retryable` + `@Backoff` + `@Recover`), без лишних зависимостей и инфраструктуры**
>     - **Развёрнутое объяснение:** Spring Retry — это узкоспециализированная библиотека для retry-логики. Она даёт декларативные аннотации, integration со Spring AOP, `RetryTemplate` для императивного контроля и встроенный fallback через `@Recover`. Для сценария «нужен только retry» — это минимальный по весу инструмент: одна зависимость `spring-retry`, никакого нового config-DSL, никаких лишних абстракций (RateLimiter, Bulkhead, TimeLimiter). В монолите без реактива и без необходимости в полноценном Circuit Breaker Spring Retry окупает себя проще, чем Resilience4j.
...
>     - **Если бы это было правдой:** не было бы смысла оставлять Spring Retry в Spring Batch/Integration — но он там жив именно потому, что покрывает узкую нишу проще.
> - [ ] **C) Не нужно ни то ни другое — достаточно `try/catch` в цикле `for (int i = 0; i < 3; i++)`**
>     - **Что на самом деле:** вручную написанный retry-цикл быстро превращается в копипасту: backoff, jitter, классификация исключений, логирование, метрики, fallback — всё придётся писать заново для каждого метода. Spring Retry даёт это декларативно за одну строку.
>     - **Откуда путаница:** для одноразового скрипта `for`-цикл оправдан, но в production-сервисе с десятками retry-точек это технический долг.
>     - **Если бы это было правдой:** Spring Retry и Resilience4j не существовали бы — но обе библиотеки активно развиваются именно потому, что ручной retry неудобен и багогенен.
> - [ ] **D) Resilience4j, но без Circuit Breaker — только Retry-модуль, потому что Spring Retry «устарел»**
>     - **Что на самом деле:** Spring Retry не устарел — он входит в актуальный стек Spring 6 / Boot 3, активно развивается и используется в Spring Batch/Integration. «Устарелость» — миф.
>     - **Откуда путаница:** Resilience4j заменил Hystrix (который действительно в maintenance mode), и иногда эту репутацию переносят на Spring Retry. Это разные библиотеки с разными нишами.
>     - **Если бы это было правдой:** Spring Team убрала бы `spring-retry` из BOM и не выпускала бы новые версии — но релизы продолжаются.
