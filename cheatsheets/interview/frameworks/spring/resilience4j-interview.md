...
Главные плюсы Resilience4j Retry на собеседовании: **из коробки интегрируется с CircuitBreaker** (один retry-«виток» учитывается брейкером как вызов), **даёт Micrometer-метрики без доп. кода** и **работает с реактивным стеком** (Reactor/RxJava). Spring Retry проще, если в проекте больше ничего из Resilience4j нет.
...
На один метод можно повесить сразу несколько аннотаций — Resilience4j оборачивает вызов в цепочку декораторов в **фиксированном** порядке. Это частый вопрос на собеседовании: важно понимать, что порядок не зависит от того, как вы написали аннотации в коде, — он жёстко задан библиотекой.
...
Почему такой порядок логичен: `Retry` стоит **внутри** `CircuitBreaker`, поэтому все попытки одного логического вызова брейкер видит как один результат, а не как N отдельных провалов (иначе ретраи сами размыкали бы цепь). `RateLimiter` и `Bulkhead` стоят перед методом — чтобы ограничения проверялись до фактического вызова.
...
**Читать как:** если rate limit пройден и цепь замкнута → выполнить с retry → если после всех попыток всё равно ошибка → CircuitBreaker засчитывает один failure.

Когда нужен **нестандартный** порядок (аннотации его не позволяют менять), переходят на программный API через `Decorators` — там декораторы навешиваются явно, в указанной последовательности:
...
4. **Порядок декораторов фиксирован:** через аннотации его не поменять (см. Q15) — для своего порядка нужен программный API.
