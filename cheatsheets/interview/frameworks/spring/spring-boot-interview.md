...
> [!mcq]
> - [x] Атрибут `matchIfMissing = true` у `@ConditionalOnProperty` делает условие истинным, если свойство вообще не задано в конфигурации. | `matchIfMissing=true` = bean активен по умолчанию (отсутствие свойства = enabled). ✓ ПРИМЕНЯТЬ: feature flag `app.feature.x.enabled` с safe default; A/B testing с включением для контрольной группы; legacy compatibility. 📋 ПРАВИЛО: "matchIfMissing = default behavior; false = opt-in, true = opt-out feature".
> - [ ] Атрибут `matchIfMissing = true` у `@ConditionalOnProperty` делает условие истинным только если свойство явно равно пустой строке. | Пустая строка ≠ отсутствие свойства; `matchIfMissing` срабатывает ТОЛЬКО при absent. ❌ ПОСЛЕДСТВИЕ: пустая строка `app.feature=` интерпретируется как "false" → bean не создаётся при ожидаемом enabled. 📋 ПРАВИЛО: "absent = matchIfMissing logic; empty = false-equivalent".
> - [ ] Атрибут `matchIfMissing = true` у `@ConditionalOnProperty` бросает исключение при отсутствии свойства, чтобы явно указать проблему. | Не бросает — наоборот активирует bean при absent. ❌ ПОСЛЕДСТВИЕ: ожидание исключения = пропуск silent activation; обнаружение через debug в проде. 📋 ПРАВИЛО: "matchIfMissing = silent fallback, не fail-fast; для required property используй `@Value` с required=true".
> - [ ] Атрибут `matchIfMissing = true` у `@ConditionalOnProperty` ищет свойство в переменных окружения, игнорируя `application.yml`. | Все source'ы Environment работают одинаково: env, yaml, command-line, secrets. ❌ ПОСЛЕДСТВИЕ: ожидание env-only = не учёт overrides из application-prod.yml → unexpected behavior. 📋 ПРАВИЛО: "Spring property resolution = priority chain (CLI > env > profile > default); matchIfMissing работает на финальном результате".
...
> [!mcq]
> - [ ] AOT-обработка в Spring Boot 3.x выполняется в runtime при первом старте приложения. | AOT = Ahead-Of-Time = на этапе СБОРКИ, не runtime. ❌ ПОСЛЕДСТВИЕ: ожидание runtime обработки = непонимание зачем AOT нужен (а нужен для GraalVM где reflection ограничен). 📋 ПРАВИЛО: "AOT = до запуска (build time); JIT = во время запуска".
> - [x] AOT-обработка в Spring Boot 3.x выполняется на этапе сборки: генерирует Java-исходники, reflect-config и proxy-config для создания бинов без рефлексии в runtime. | `processAot` task генерирует `build/generated/aotSources/`; артефакты компилируются с приложением. ✓ ПРИМЕНЯТЬ: AWS Lambda (cold start 50ms vs 5s), serverless workloads, Kubernetes scale-to-zero. 📋 ПРАВИЛО: "AOT = код вместо рефлексии = быстрый старт + GraalVM native". 🔗 См. `application.yml` profiles, GraalVM section.
> - [ ] AOT-обработка в Spring Boot 3.x выполняется только для тестов и никак не влияет на production-билд. | AOT применима К ПРОДАКШЕНУ (особенно для GraalVM); тесты — отдельная задача `processTestAot`. ❌ ПОСЛЕДСТВИЕ: пропуск AOT в prod build = JIT warmup 30+ сек на старте, проигрыш в SLO для serverless. 📋 ПРАВИЛО: "processAot = main; processTestAot = test".
> - [ ] AOT-обработка в Spring Boot 3.x автоматически заменяет все Java Proxy на CGLIB Proxy. | AOT генерирует ХИНТЫ (`proxy-config.json`), не меняет тип прокси. ❌ ПОСЛЕДСТВИЕ: ожидание автоматического CGLIB перехода → недоумение почему `final` методы всё ещё не проксируются. 📋 ПРАВИЛО: "AOT генерирует metadata; Java vs CGLIB proxy выбор остаётся за Spring как был".
...
> [!mcq]
> - [x] Кастомный `HealthIndicator` создаётся через бин, реализующий интерфейс `HealthIndicator` с методом `Health health()` — Spring Boot автоматически его обнаруживает. | Auto-discovery через component scan; имя в JSON = bean name минус "HealthIndicator". ✓ ПРИМЕНЯТЬ: Spotify (50+ deps), readiness probe в K8s (`/actuator/health/readiness`), API gateway dependency check. 📋 ПРАВИЛО: "@Component + implements HealthIndicator → автоматически в /health". 🔗 См. Q42 (Actuator security).
> - [ ] Кастомный `HealthIndicator` создаётся через аннотацию `@HealthCheck` на любом методе — Spring Boot сам интегрирует его в `/actuator/health`. | `@HealthCheck` НЕ СУЩЕСТВУЕТ — health через interface. ❌ ПОСЛЕДСТВИЕ: попытка использовать выдуманную аннотацию = silent ignore (метод не вызовется), false sense of monitoring; реальный inci dent — k8s liveness reports OK while DB down. 📋 ПРАВИЛО: "Spring health = interface, не аннотация".
> - [ ] Кастомный `HealthIndicator` создаётся через наследование `AbstractEndpoint<Health>` с переопределением метода `invoke()`. | `AbstractEndpoint` — устаревший API из Spring Boot 1.x; в 2.x+ используется HealthIndicator. ❌ ПОСЛЕДСТВИЕ: попытка наследования = compile error на актуальных версиях; время на дебаг устаревшей документации. 📋 ПРАВИЛО: "Spring Boot 2.x+ = HealthIndicator interface; legacy AbstractEndpoint забыть".
> - [ ] Кастомный `HealthIndicator` создаётся через регистрацию лямбды `MeterRegistry::registerHealth` в конфигурации. | `MeterRegistry` — Micrometer API для metrics, не для health. ❌ ПОСЛЕДСТВИЕ: смешение Metrics и Health = неверный архитектурный подход; health = boolean state, metrics = numeric series. 📋 ПРАВИЛО: "Health = HealthIndicator; Metrics = MeterRegistry; не путай".
...
> [!mcq]
> - [ ] При активации нескольких профилей через запятую Spring Boot применяет только первый из них, игнорируя остальные. | Применяются ВСЕ профили — последующие override предыдущие. ❌ ПОСЛЕДСТВИЕ: ожидание single profile = не понимание layered configs; вторая часть `prod,metrics` будет молча применена и переопределит свойства. 📋 ПРАВИЛО: "comma-separated = all applied; later wins на коллизиях".
> - [x] При активации нескольких профилей через запятую Spring Boot применяет их все: более поздний в списке перекрывает совпадающие ключи более раннего. | Loading order = base → profile1 → profile2 → profileN; last wins. ✓ ПРИМЕНЯТЬ: Netflix 15+ profiles (region+env+canary), Profile Groups (Spring Boot 2.4+) для алиасов набора. 📋 ПРАВИЛО: "профили = слои сверху base; последний слой выигрывает". 🔗 См. Q38 (@ConditionalOnProperty).
> - [ ] При активации нескольких профилей через запятую Spring Boot бросает `ProfileConflictException` при совпадающих ключах. | Такого исключения нет — коллизии разрешаются "последний wins". ❌ ПОСЛЕДСТВИЕ: ожидание fail-fast = unexpected silent override; дебаг 30+ мин в проде. 📋 ПРАВИЛО: "Spring profile merging = last-wins, не conflict detection".
> - [ ] При активации нескольких профилей через запятую Spring Boot объединяет только секции верхнего уровня, оставляя остальные неизменными. | Полное рекурсивное наложение (deep merge) на всех уровнях. ❌ ПОСЛЕДСТВИЕ: ожидание top-level only = неверная картина; вложенные `spring.datasource.url` тоже override. 📋 ПРАВИЛО: "deep override на всех уровнях; используй `@ConfigurationProperties` для type-safe доступа".
...
> [!mcq]
> - [x] В production Actuator endpoints обычно выносят на отдельный порт через `management.server.port`, закрытый в Ingress/LoadBalancer, и защищают через Spring Security. | Изолирует management traffic; Security `EndpointRequest.toAnyEndpoint()` + RBAC. ✓ ПРИМЕНЯТЬ: Google SRE (port 9090 internal-only), K8s `containerPort` для Prometheus scraping без external exposure. 📋 ПРАВИЛО: "management port + закрыт в Ingress + Security RBAC; defense in depth". 🔗 См. spring-security-interview.
> - [ ] В production Actuator endpoints рекомендуется открывать через `management.endpoints.web.exposure.include=*` без какой-либо защиты. | Открытие `*` = утечка через `/env` (env vars), `/heapdump` (memory dump), `/threaddump`. ❌ ПОСЛЕДСТВИЕ: Capital One 2019 — `/env` exposed → $100M data leak (190M users); GraphQL Spring Boot 2.6 CVE — RCE через `/env` POST. 📋 ПРАВИЛО: "exposure: `*` = NEVER в prod; whitelist health,info,prometheus".
> - [ ] В production Actuator endpoints рекомендуется закрывать полностью через `management.endpoints.enabled-by-default=false`. | Полное закрытие = слепота на проде; нужны метрики и health для K8s. ❌ ПОСЛЕДСТВИЕ: liveness/readiness probes ломаются → K8s restart loop, нет доступа в Prometheus → нет alerting. 📋 ПРАВИЛО: "selective expose + защита, не blanket disable".
> - [ ] В production Actuator endpoints рекомендуется оставлять на том же порту, что и приложение, и защищать их через CORS. | CORS защищает ТОЛЬКО browser clients (Same-Origin), не curl/exploit. ❌ ПОСЛЕДСТВИЕ: ложное чувство security; attacker через curl получает `/env` мгновенно. 📋 ПРАВИЛО: "CORS = browser only; security = Spring Security + isolated port".
