...
> [!mcq]
>
> - [ ] **A. Подключать реальный Redis через Testcontainers — это единственный корректный способ протестировать, что `@Cacheable` действительно работает: без сетевого кэша тесты ничего не доказывают.**
>
>     **Что на самом деле:** для проверки самой логики кэширования (вызывается метод повторно или нет) достаточно `@SpringBootTest` с дефолтным `ConcurrentMapCacheManager` и `@MockBean` репозитория — проверяется, что повторный вызов не идёт в репозиторий. Testcontainers с Redis нужны для интеграционных тестов сериализации, TTL и cluster-поведения, но не для unit-тестирования аннотаций.
>
>     **Откуда путаница:** смешивают «протестировать кэш-аннотации» и «протестировать конкретного провайдера». Это разные уровни тестов.
>
>     **Если бы это было правдой:** любое подключение `@EnableCaching` требовало бы Docker в CI — на деле большинство проектов прекрасно тестируют логику кэширования на in-memory cache manager.
>
> - [ ] **B. Достаточно вызвать метод дважды и проверить `assertEquals` на возвращаемых значениях — если они равны, значит кэш работает.**
>
>     **Что на самом деле:** равенство результатов ничего не доказывает — метод может вызываться повторно и каждый раз возвращать тот же `Product` из БД. Проверять надо количество обращений к нижележащему слою через `verify(repository, times(1)).findById(...)`. Только это подтверждает, что второй вызов был обслужен из кэша.
>
>     **Откуда путаница:** интуитивно кажется, что «результат одинаковый → кэш сработал», но кэш — это про **отсутствие повторного вычисления**, а не про идентичность возвращаемого объекта.
>
>     **Если бы это было правдой:** тест проходил бы даже при полностью отключённом `@EnableCaching` — он не различал бы наличие и отсутствие кэша.
>
> - [x] **C. Использовать `@SpringBootTest` с `@MockBean` для репозитория, чистить кэш в `@BeforeEach` и через `verify(repository, times(N))` проверять, что повторный вызов метода обслужен из кэша.**
>
>     **Развёрнутое объяснение:** корректный тест кэша строится на трёх элементах: (1) поднят реальный Spring-контекст с `@EnableCaching` (иначе AOP-прокси не сработает), (2) репозиторий замокан, чтобы изолировать кэш от БД, (3) проверка через `Mockito.verify` количества обращений — это единственная семантически верная проверка работы кэша. Очистка `cacheManager.getCacheNames().forEach(... .clear())` в `@BeforeEach` обязательна, иначе тесты влияют друг на друга через shared cache state.
>
>     **Пример:** в тесте `shouldCacheResult` дважды дёргается `productService.getProduct(1L)` и проверяется `verify(repository, times(1)).findById(1L)` — это прямо показывает, что второй вызов был обслужен из кэша, а не из репозитория.
>
>     **Когда применять:** для unit/component-тестов слоя сервиса, когда нужно убедиться, что аннотации `@Cacheable`/`@CacheEvict` развешаны корректно и AOP-прокси создан.
>
...
>
>     **Что на самом деле:** функциональное поведение `@Cacheable` (вызвался метод повторно или нет) полностью тестируется на изолированных unit-сценариях с моками. Нагрузочные тесты нужны для проверки производительности и stampede-поведения, но не для проверки самой логики кэширования.
>
>     **Откуда путаница:** концепцию «кэш ускоряет систему» путают с «кэш не пускает повторный вызов» — это две разные характеристики, и вторая прекрасно проверяется без нагрузки.
>
>     **Если бы это было правдой:** не существовало бы `@MockBean` и `cacheManager.getCache().clear()` — а они в Spring Boot Test есть именно для unit-тестирования кэша.

---

## Q18. (!) Как защититься от cache stampede в Spring Cache и зачем нужен `@Cacheable(sync = true)`?

**Cache stampede (thundering herd)** — ситуация, когда множество параллельных запросов одновременно получают cache miss и все идут в БД/upstream, создавая всплеск нагрузки.
...
**Что делает `sync = true`:** Spring оборачивает доступ к кэшу в `Cache.get(key, valueLoader)` — этот контракт гарантирует, что для одного ключа метод-loader будет вызван **ровно один раз** даже при параллельных промахах. Это паттерн single-flight.

**Важные ограничения `sync = true`:**
...
**Что НЕ делает `sync = true`:**
- Не сочетается с `unless`, `condition`, `CompletableFuture`/`Mono` — Spring выбросит `IllegalStateException` на старте.
- Не работает поверх нескольких кэшей одновременно (`cacheNames = {"a", "b"}`).
- Не защищает от stampede в распределённой системе — для этого нужны Redis-локи (Redisson `RLock` с `tryLock` и leaseTime) или версия в Redis с CAS.

**Полная защита в кластере:**
```java
...
> [!mcq]
>
> - [ ] **A. `sync = true` включён по умолчанию во всех Spring Cache провайдерах начиная с Spring 5 — отдельно ничего настраивать не нужно.**
>
>     **Что на самом деле:** атрибут `sync` по умолчанию равен `false`. Без явного `@Cacheable(sync = true)` Spring не делает никаких синхронизаций — N параллельных промахов кэша приводят к N походам в БД. И даже когда `sync = true` указан, поведение зависит от провайдера: для Redis distributed-локов нет.
>
>     **Откуда путаница:** новичкам кажется, что раз есть аннотация, она «по-умолчанию умная». Spring Cache намеренно консервативен: он не лезет в семантику параллелизма, пока разработчик явно об этом не попросит.
>
>     **Если бы это было правдой:** не было бы регулярных production-инцидентов с thundering herd при инвалидации горячих ключей.
>
> - [ ] **B. Достаточно поставить Redis с TTL и распределённый кэш сам решит проблему cache stampede, так как все узлы читают из одного места.**
>
>     **Что на самом деле:** общий Redis решает проблему **рассинхронизации между узлами**, но не stampede. Когда ключ протухает по TTL, все 50 параллельных запросов с разных узлов одновременно увидят cache miss и одновременно пойдут в БД — Redis тут вообще не участник, он просто «нет такого ключа» отвечает всем. Нужны явные механизмы: Redisson `RLock`, probabilistic early expiration или single-flight на уровне приложения.
>
>     **Откуда путаница:** общий кэш и защита от stampede — это ортогональные свойства. Redis закрывает первое, не закрывает второе.
>
>     **Если бы это было правдой:** не существовало бы паттерна «cache stampede protection» в литературе по distributed caching и Redisson не имел бы `RLock`/`RBucket.compareAndSet` API.
>
> - [ ] **C. `@Cacheable(sync = true)` можно совмещать с `unless`, `condition` и реактивным возвращаемым типом `Mono<T>` — Spring сам разрулит конкурентность.**
>
>     **Что на самом деле:** Spring явно запрещает эти комбинации и бросает `IllegalStateException` на старте контекста. Контракт `Cache.get(key, valueLoader)` не поддерживает условную логику и асинхронные возвращаемые типы — это документированное ограничение. Для реактивного стека нужно использовать `Mono.cache()` или сторонние решения вроде Spring Data Redis Reactive с ручной координацией.
>
>     **Откуда путаница:** атрибуты `sync`, `condition` и `unless` находятся в одной аннотации и кажутся ортогональными. На деле single-flight несовместим с условностью результата — нельзя «единственный поток» вычислить значение, которое потом будет отвергнуто условием `unless`.
>
>     **Если бы это было правдой:** Spring 5+ не выдавал бы `@Cacheable(sync=true) is not supported with...` в логах при старте.
>
> - [x] **D. `@Cacheable(sync = true)` работает только локально на узле (через `Cache.get(key, valueLoader)`), даёт single-flight для `ConcurrentMapCache`/Caffeine, но не предотвращает stampede в кластере с Redis — для распределённой защиты нужны явные локи (Redisson `RLock`) или probabilistic early expiration.**
>
>     **Развёрнутое объяснение:** `sync = true` транслируется в вызов `cache.get(key, () -> method())` — контракт этого метода обязывает имплементацию провайдера гарантировать, что для одного ключа loader вызывается ровно один раз. Для `ConcurrentMapCache` это делается через `computeIfAbsent`, для Caffeine — через `LoadingCache.get()`. Для `RedisCache` (Spring Data Redis) `Cache.get(key, loader)` обёрнут в локальный `synchronized`, который держит лок только в JVM текущего узла — на 10 узлах кластера будет до 10 параллельных походов в БД. Полная защита от stampede в распределённой системе — это Redisson `RLock.tryLock` с leaseTime, либо XFetch-алгоритм (probabilistic early refresh).
>
>     **Пример:** для in-process защиты — `@Cacheable(value = "products", sync = true)`. Для распределённой — обернуть загрузку в `redisson.getLock("product:" + id).tryLock(0, 5, TimeUnit.SECONDS)` с двойной проверкой кэша после захвата лока.
>
>     **Когда применять:** `sync = true` — для горячих in-memory кэшей с дорогим computation (Caffeine + тяжёлый агрегирующий запрос). Redisson `RLock` — для распределённых сценариев с дорогим upstream-вызовом (внешний API, тяжёлый JOIN), где stampede на 50 подов = 50× нагрузка на БД.
>
