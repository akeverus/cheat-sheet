...
> - [ ] JWT состоит из трёх частей (header, payload, signature), разделённых точкой; header содержит данные пользователя, а payload — алгоритм подписи. | Header содержит метаданные токена (alg, typ), а не данные пользователя. Payload содержит claims — утверждения о субъекте. Они перепутаны местами.
...
> - [ ] Claim `sub` (Subject) содержит Unix timestamp истечения срока действия токена и обязательно проверяется сервером при валидации. | Это описание `exp`. `sub` (Subject) — идентификатор субъекта токена (обычно userId), строковое значение, не timestamp.
...
> [!mcq]
> - [ ] Payload JWT зашифрован Base64URL, поэтому для его декодирования требуется ключ подписи; без ключа прочитать claims невозможно. | Base64URL — это кодирование, а не шифрование. Оно легко обратимо без всякого ключа. Для защиты содержимого payload нужен JWE, а не JWS.
...
> [!mcq]
> - [ ] `ES256` — симметричный алгоритм на основе AES-256 с общим секретом, применяется в системах, где и AS, и RS принадлежат одной организации. | ES256 — асимметричный алгоритм (ECDSA с кривой P-256), а не симметричный AES. Он использует пару приватный/публичный ключ, а не общий секрет.
...
> [!mcq]
> - [ ] JWT — формат токена; JWS — зашифрованный JWT (RFC 7516); JWE — подписанный JWT (RFC 7515); оба являются реализациями JWT. | JWS и JWE перепутаны: JWS (RFC 7515) — это подписанный JWT, а JWE (RFC 7516) — зашифрованный. Это частое заблуждение из-за похожих аббревиатур.
