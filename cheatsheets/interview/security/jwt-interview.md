...
> [!mcq]
> - [ ] JWT состоит из двух частей (header и payload), разделённых точкой; подпись добавляется только для зашифрованного токена. | Неверно: JWT всегда состоит из трёх частей — header, payload и signature, разделённых точками. Signature присутствует в JWS-токене (стандартный вариант) независимо от наличия шифрования.
> - [x] JWT состоит из трёх частей (header, payload, signature), разделённых точкой; каждая часть кодируется Base64URL, а payload не зашифрован. | Верная структура: три части через точку, Base64URL-кодирование. Ключевой момент — payload виден любому без ключа, поэтому в JWT нельзя хранить секреты без JWE.
> - [ ] JWT состоит из трёх частей (header, payload, signature), разделённых точкой; payload зашифрован AES-256, что делает токен конфиденциальным. | Это описание JWE (JSON Web Encryption), а не стандартного JWS. Обычный JWT/JWS шифрование не применяет — только Base64URL-кодирование, что легко обратимо.
> - [ ] JWT состоит из трёх частей (header, payload, signature), разделённых точкой; header содержит данные пользователя, а payload — алгоритм подписи. | Header содержит метаданные токена (alg, typ), а не данные пользователя. Payload содержит claims — утверждения о субъекте. Они перепутаны местами. Частая ошибка в реальном коде.
...
> [!mcq]
> - [ ] Части JWT кодируются стандартным Base64 с padding-символами `=`, что обеспечивает совместимость с email-протоколами. | Стандартный Base64 содержит символы `+`, `/` и padding `=`, которые несовместимы с URL и HTTP-заголовками. JWT использует именно Base64URL-вариант, специально модифицированный для URL-safe контекста.
> - [x] Части JWT кодируются Base64URL (вариант Base64 без padding, с заменой `+` на `-` и `/` на `_`), что делает токен безопасным для URL и HTTP-заголовков. | Base64URL — это URL-safe модификация Base64 согласно RFC 4648. Замена проблемных символов позволяет передавать JWT в URL-параметрах, Authorization-заголовках и cookie без дополнительного URL-encoding.
> - [ ] Части JWT кодируются hex (шестнадцатеричным представлением), что делает токен безопасным для URL и HTTP-заголовков. | Hex даёт двухкратное увеличение размера по сравнению с Base64URL (1 байт → 2 символа против 3 байт → 4 символа). Это делает hex неэффективным для компактного формата, которым должен быть JWT.
> - [ ] Части JWT кодируются URL-encoding (percent-encoding), что позволяет безопасно передавать спецсимволы в заголовках. | URL-encoding (percent-encoding) применяется к URL-параметрам с произвольными символами, но неэффективен для бинарных данных: многие байты превращаются в 3-символьные последовательности `%XX`, сильно раздувая размер.
...
> [!mcq]
> - [ ] RFC 7519 определяет два типа claims: registered (стандартизированные IANA, обязательные) и private (кастомные); public claims не существуют. | RFC 7519 определяет именно три типа: registered, public и private. Кроме того, registered claims рекомендованы, но не обязательны — это частое заблуждение.
> - [ ] RFC 7519 определяет три типа claims: registered (стандартизированные IANA), private (публично зарегистрированные) и public (кастомные, договорные). | Типы public и private перепутаны местами. Public claims — публично зарегистрированные (name, email в OpenID), а private — договорные между конкретными сторонами (tenantId, role).
> - [x] RFC 7519 определяет три типа claims: registered (стандартизированные IANA: iss, sub, exp), public (публично зарегистрированные: name, email) и private (кастомные, договорные: role, tenantId). | Верная классификация из RFC 7519. Registered claims имеют короткие имена для компактности; public избегают коллизий через регистрацию; private — соглашение между конкретными участниками обмена.
> - [ ] RFC 7519 определяет три типа claims: registered (стандартизированные IANA), public (публично зарегистрированные) и private (кастомные); все три типа обязательны для каждого JWT. | Claims любого типа не являются обязательными — ни registered, ни public, ни private. Единственное требование RFC 7519: если claim присутствует, он должен быть правильно обработан.
...
> [!mcq]
> - [ ] Claim `iss` (Issuer) содержит Unix timestamp истечения срока действия токена и обязательно проверяется сервером при валидации. | Unix timestamp истечения — это `exp` (Expiration Time), а не `iss`. `iss` — это Issuer, строковый идентификатор того, кто выдал токен (обычно URL Authorization Server).
> - [ ] Claim `sub` (Subject) содержит Unix timestamp истечения срока действия токена и обязательно проверяется сервером при валидации. | Это описание `exp`. `sub` (Subject) — идентификатор субъекта токена (обычно userId), строковое значение, не timestamp. Частая ошибка в реальном коде.
> - [x] Claim `exp` (Expiration Time) содержит Unix timestamp истечения срока действия токена и обязательно проверяется сервером при валидации. | Верно согласно RFC 7519. `exp` — обязательная проверка при валидации; допускается небольшой clock skew (leeway) для компенсации рассинхронизации часов между серверами.
> - [ ] Claim `aud` (Audience) содержит Unix timestamp истечения срока действия токена и обязательно проверяется сервером при валидации. | `aud` — это Audience (получатель токена, обычно идентификатор Resource Server), а не timestamp. Проверка `aud` обязательна для защиты от Token Substitution Attack, но значение — строка или массив строк.
...
> [!mcq]
> - [ ] Payload JWT зашифрован Base64URL, поэтому для его декодирования требуется ключ подписи; без ключа прочитать claims невозможно. | Base64URL — это кодирование, а не шифрование. Оно легко обратимо без всякого ключа. Для защиты содержимого payload нужен JWE, а не JWS. Частая ошибка в реальном коде.
> - [x] Payload JWT не зашифрован — он лишь Base64URL-закодирован и доступен любому, у кого есть токен; для конфиденциальности payload нужен JWE. | Стандартный JWT (JWS) гарантирует целостность и подлинность через подпись, но не конфиденциальность. Payload декодируется одной командой `base64 -d`. Для шифрования — только JWE.
> - [ ] Payload JWT подписан и зашифрован одновременно: HMAC гарантирует целостность, а AES-GCM — конфиденциальность в одной операции. | JWS обеспечивает только подпись (целостность + аутентичность). Одновременное шифрование и подпись — это Nested JWT (JWS внутри JWE), отдельный и более сложный механизм.
> - [ ] Payload JWT зашифрован только в режиме RS256; при использовании HS256 payload остаётся открытым и доступен без ключа. | Алгоритм подписи (RS256 или HS256) не влияет на шифрование payload. Ни RS256, ни HS256 не шифруют payload — это всегда только Base64URL. Шифрование — функция JWE, а не алгоритма JWS.
...
> [!mcq]
> - [ ] Для микросервисной архитектуры с несколькими Resource Server рекомендуется `HS256` как самый быстрый симметричный алгоритм. | HS256 в микросервисах требует распространения общего секрета всем сервисам, что создаёт значительный attack surface: компрометация любого сервиса позволяет подделывать токены. Скорость не компенсирует риск.
> - [x] Для микросервисной архитектуры с несколькими Resource Server рекомендуется `RS256`: Authorization Server подписывает приватным ключом, каждый Resource Server верифицирует публичным ключом из JWKS. | Верно. RS256 — стандарт для OAuth2/OIDC именно из-за асимметричной модели: приватный ключ остаётся только у AS, публичный ключ распространяется безопасно через JWKS endpoint. Компрометация RS не позволяет подделывать токены.
> - [ ] Для микросервисной архитектуры с несколькими Resource Server рекомендуется `ES256` как единственный асимметричный алгоритм, поддерживаемый стандартом. | ES256 действительно асимметричный и подходит для распределённых систем, но не единственный: RS256/RS384/RS512 и PS256/PS384/PS512 тоже асимметричные. ES256 предпочитают для IoT и ограниченных по ресурсам систем.
> - [ ] Для микросервисной архитектуры с несколькими Resource Server рекомендуется `PS256` — обязательный алгоритм по RFC 7518, все остальные являются optional. | RFC 7518 определяет минимальный набор обязательных алгоритмов иначе: HS256 — Required, RS256 и none — Recommended. PS256 — Optional. В распределённых системах RS256 — практический стандарт, а не PS256.
...
> [!mcq]
> - [ ] HS256 использует пару приватный/публичный ключ; Authorization Server подписывает приватным, Resource Server верифицирует публичным, что делает HS256 безопасным для распределённых систем. | Это описание RS256. HS256 — симметричный алгоритм с одним общим секретом у всех сторон. Именно необходимость делиться секретом делает HS256 неудобным в распределённых системах.
> - [x] HS256 использует один общий секрет (симметричный HMAC), а RS256 — пару ключей RSA (асимметричный); RS256 предпочтительнее в микросервисах, так как публичный ключ можно распространить через JWKS без риска компрометации. | Ключевое различие: HS256 требует безопасного распространения секрета всем сервисам, а RS256 — только публичного ключа через JWKS endpoint, что значительно снижает attack surface.
> - [ ] HS256 использует один общий секрет (симметричный HMAC), а RS256 — пару ключей RSA (асимметричный); HS256 предпочтительнее в микросервисах из-за меньших накладных расходов на RSA-операции. | RS256 действительно медленнее по CPU, но в реальных системах это несущественно. Главная проблема HS256 в микросервисах — необходимость безопасного распределения секрета. Правильное правило: микросервисы → RS256.
> - [ ] HS256 использует один общий секрет (симметричный HMAC), а RS256 — пару ключей RSA (асимметричный); оба алгоритма равнозначны по безопасности, выбор определяется только производительностью. | Безопасность HS256 и RS256 фундаментально отличается по модели угроз. В HS256 компрометация любого сервиса, знающего секрет, позволяет подделывать токены. В RS256 только владелец приватного ключа может подписывать.
...
> [!mcq]
> - [ ] `ES256` — симметричный алгоритм на основе AES-256 с общим секретом, применяется в системах, где и AS, и RS принадлежат одной организации. | ES256 — асимметричный алгоритм (ECDSA с кривой P-256), а не симметричный AES. Он использует пару приватный/публичный ключ, а не общий секрет. Частая ошибка в реальном коде.
> - [x] `ES256` — асимметричный алгоритм на основе ECDSA с кривой P-256, предпочтителен при ограниченных ресурсах: 256-битный EC ключ эквивалентен по безопасности 3072-битному RSA. | Верное описание ES256. Эллиптическая криптография даёт ту же стойкость при значительно меньшем размере ключей и подписей, что критично для IoT и мобильных устройств с ограниченной энергией и пропускной способностью.
> - [ ] `ES256` — асимметричный алгоритм на основе RSA-PSS с маскированием, предпочтителен при ограниченных ресурсах благодаря компактности RSA-ключей. | RSA-PSS — это PS256, не ES256. Кроме того, RSA-ключи по размеру значительно больше EC-ключей (3072 бит RSA ≈ 256 бит EC по стойкости), поэтому «компактность RSA» — противоречие.
> - [ ] `ES256` — асимметричный алгоритм на основе Edwards-кривых (Ed25519), предпочтителен благодаря устойчивости к side-channel атакам. | Алгоритм на Edwards-кривых — это EdDSA (Ed25519/Ed448), а не ES256. ES256 использует NIST-кривую P-256 (secp256r1), другое семейство эллиптических кривых.
...
> [!mcq]
> - [ ] JWT — формат токена; JWS — зашифрованный JWT (RFC 7516); JWE — подписанный JWT (RFC 7515); оба являются реализациями JWT. | JWS и JWE перепутаны: JWS (RFC 7515) — это подписанный JWT, а JWE (RFC 7516) — зашифрованный. Это частое заблуждение из-за похожих аббревиатур. Частая ошибка в реальном коде.
> - [ ] JWT — формат токена; JWS — подписанный JWT (RFC 7515); JWE — зашифрованный JWT (RFC 7516); JWE обязательно содержит подпись внутри, иначе не имеет смысла. | JWE сам по себе не требует внутренней подписи — он гарантирует конфиденциальность и целостность через AEAD-шифрование. Подпись внутри JWE (Nested JWT) — опциональное расширение.
> - [x] JWT — общий формат токена; JWS — JWT с подписью (целостность, но не конфиденциальность); JWE — JWT с зашифрованным payload (конфиденциальность + целостность); «JWT» в обиходе почти всегда означает JWS. | Верная классификация семейства JOSE. На практике 99% случаев — это JWS, где payload Base64URL-кодирован и читаем без ключа. JWE применяется, когда нужно скрыть содержимое payload от промежуточных узлов.
> - [ ] JWT — общий формат токена; JWS — JWT с подписью (целостность, но не конфиденциальность); JWE — JWT с зашифрованным payload (конфиденциальность + целостность); на практике JWE используется чаще JWS в современных системах. | JWS используется несравнимо чаще JWE. JWE требует дополнительной инфраструктуры управления ключами и значительно увеличивает сложность системы, поэтому применяется только при необходимости скрыть payload.
...
> [!mcq]
> - [ ] JWKS — это список отозванных JWT, публикуемый Authorization Server по пути `/.well-known/revoked-tokens`; Resource Server проверяет каждый токен по этому списку. | JWKS — это JSON Web Key Set (RFC 7517) со списком публичных ключей, а не отозванных токенов. Список отзыва — это отдельный механизм (CRL-подобный), не стандартизированный в OAuth2 в таком формате.
> - [ ] JWKS — это JSON-формат для публикации публичных ключей по пути `/.well-known/jwks.json`; каждый ключ содержит поля kty, use, kid, и приватную часть для верификации подписи. | Приватная часть ключа никогда не публикуется в JWKS — только публичные компоненты (n, e для RSA). Приватный ключ хранится исключительно на Authorization Server.
> - [ ] JWKS — это JSON-формат для публикации публичных ключей по пути `/.well-known/jwks.json`; Resource Server не кэширует ключи и запрашивает JWKS при каждом входящем JWT. | Resource Server обязан кэшировать JWKS для производительности. Запрос JWKS на каждый JWT добавляет latency и перегружает Authorization Server. Обновление происходит только при встрече неизвестного kid.
> - [x] JWKS — это JSON-формат для публикации публичных ключей по пути `/.well-known/jwks.json`; Resource Server кэширует ключи и обновляет кэш при встрече незнакомого kid, что поддерживает ротацию ключей. | Верный механизм: JWKS содержит массив публичных ключей (kty, use, kid, n, e для RSA); kid в JWT header позволяет быстро найти нужный ключ; кэш снижает нагрузку на AS.
...
> [!mcq]
> - [ ] Resource Server извлекает публичный ключ из поля `alg` в JWT header, находит его в JWKS-кэше и верифицирует подпись. | Ключ выбирается по полю `kid` (Key ID) из JWT header, а не по `alg`. Поле `alg` указывает алгоритм подписи, но не идентифицирует конкретный ключ из набора.
> - [ ] Resource Server извлекает публичный ключ из поля `iss` в JWT header, находит его в JWKS-кэше и верифицирует подпись. | `iss` — это Issuer (claim в payload, а не header), используется для проверки, что токен выдан ожидаемым AS. Для поиска ключа используется `kid` из header, а не `iss`.
> - [x] Resource Server извлекает публичный ключ из поля `kid` в JWT header, находит его в JWKS-кэше (при отсутствии — запрашивает JWKS endpoint) и верифицирует подпись. | Верный механизм: `kid` (Key ID) в header JWT однозначно указывает, каким ключом из JWKS подписан токен. При незнакомом `kid` RS обновляет кэш — так поддерживается ротация ключей без downtime.
> - [ ] Resource Server извлекает публичный ключ из поля `jti` в JWT header, находит его в JWKS-кэше и верифицирует подпись. | `jti` — это JWT ID (уникальный идентификатор токена, находится в payload), используется для blacklist и защиты от replay. Для поиска ключа применяется именно `kid`.
