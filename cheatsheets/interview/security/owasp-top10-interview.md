...
Риск — это произведение вероятности атаки на её последствия: **`Risk = Likelihood × Impact`**. Категория опаснее не сама по себе, а когда её легко проэксплуатировать И ущерб от этого велик.
...
- **Опрос экспертов** — community survey для 3 категорий (`Insecure Design`, `SSRF`, `Integrity Failures`): это новые или редко детектируемые риски, по которым статистики недостаточно

---

## Q4. (!) Что такое Broken Access Control и почему это категория №1?

`Broken Access Control` (`A01:2021`) — это когда пользователь получает доступ к ресурсам или операциям, на которые у него нет прав: читает чужие данные, вызывает admin-функции, обходит проверки. Категория стала №1, потому что её нашли в **94% протестированных приложений** — это самая распространённая и при этом легко эксплуатируемая проблема (часто достаточно подменить один параметр в запросе, инструменты не нужны).
...
`IDOR` (`Insecure Direct Object Reference`) — атака, при которой злоумышленник подменяет идентификатор объекта в запросе (`accountId=1` → `accountId=2`) и получает доступ к чужим данным. Корень проблемы: приложение проверяет, что пользователь *аутентифицирован*, но не проверяет, что объект *принадлежит именно ему*. Защита — всегда привязывать проверку к владельцу, а не только к факту входа.
...
**Ещё надёжнее** — фильтрация на уровне запроса к БД. Чужой объект просто не попадёт в выборку, и проверку в коде невозможно забыть:
```java
...
Дополнительные меры (каждая закрывает свой пробел):
- **`UUID`** вместо sequential `Long` для идентификаторов — усложняет перебор, но это лишь обфускация: сам по себе UUID не заменяет проверку ownership
- Централизовать проверку ownership в **`PermissionEvaluator`** — чтобы логика «кто чем владеет» не дублировалась по контроллерам и её нельзя было пропустить (см. [Spring Security](../frameworks/spring/spring-security-interview.md))
- **`Row Level Security`** на уровне БД как defense in depth — последний рубеж, если проверка в коде всё-таки была забыта
...
**Горизонтальная** — более коварная: роль у атакующего правильная (`USER`), поэтому стандартный RBAC по ролям пропускает запрос. Поймать её можно только проверкой ownership на уровне бизнес-логики — «этот заказ действительно принадлежит этому пользователю?» (см. Q5).

## Q7. Как реализовать deny-by-default авторизацию в Spring Security?

Принцип `deny-by-default` означает: всё, что явно не разрешено — запрещено. Это безопаснее обратного подхода («разрешено всё, кроме перечисленного»), потому что забытый или новый эндпоинт по умолчанию оказывается закрытым, а не открытым. В `Spring Security` ключевая строка — финальный `.anyRequest().authenticated()`, который ловит всё неуказанное явно:
...
Типичные ошибки, которые ломают deny-by-default:
- `.anyRequest().permitAll()` в конце цепочки — превращает правило в allow-by-default: любой не перечисленный путь открыт
- Защита только на уровне URL, без `@PreAuthorize` в сервисах — URL-правила не ловят IDOR и вызовы методов в обход контроллера
- Забытые `Actuator` эндпоинты — они часто не попадают под основную цепочку, а `/actuator/env` отдаёт переменные окружения с секретами

---

## Q8. (!) Какие типичные криптографические ошибки допускают разработчики?

`Cryptographic Failures` (`A02:2021`) покрывает ошибки в шифровании, хэшировании и обработке чувствительных данных. Категория про то, что данные либо вообще не защищены, либо защищены неправильно — а это почти так же плохо, как отсутствие защиты, потому что создаёт ложное чувство безопасности. Основные проблемы:
...
Пароли никогда не хранят в открытом виде и не шифруют (шифрование обратимо — утечка ключа раскроет всё). Единственный правильный подход — **adaptive hashing** с уникальной солью: алгоритм медленный и настраиваемый по «стоимости», поэтому перебор дорог даже на мощном железе, а соль делает rainbow-таблицы бесполезными.
...
Для новых проектов рекомендуется `Argon2` — победитель `Password Hashing Competition`. Его преимущество над `BCrypt` — настраиваемая *memory-hardness*: хэширование требует не только времени, но и десятков мегабайт памяти, что обесценивает атаки на GPU и ASIC (у них много ядер, но мало памяти на ядро):
```java
@Bean
public PasswordEncoder passwordEncoder() {
...
`AES-GCM` — это authenticated encryption (AEAD): за один проход он и шифрует данные (конфиденциальность), и считает authentication tag (целостность). Поэтому при расшифровке вы сразу узнаёте, если шифртекст подменили — расшифровка просто упадёт с `AEADBadTagException`, а не отдаст «мусор». Именно поэтому GCM предпочтительнее «голого» CBC, где целостность надо проверять отдельным MAC:
...
Главные правила:
- **Никогда** не переиспользовать `IV` с одним и тем же ключом — для GCM это катастрофа: атакующий восстанавливает authentication key и может подделывать сообщения. IV не секретен, но обязан быть уникальным
- Хранить ключи в `Vault` / `KMS`, **не** в коде или конфиге (иначе шифрование бессмысленно — ключ утечёт вместе с данными)
- Использовать `SecureRandom.getInstanceStrong()` для генерации `IV` — обычный `Random` предсказуем
- Рассмотреть `Google Tink` — высокоуровневая обёртка, которая берёт выбор алгоритма и управление IV на себя и тем самым исключает типичные ошибки
...
1. **Параметризованные запросы** — основная защита для SQL, LDAP, NoSQL: ввод передаётся отдельно от текста запроса, поэтому интерпретатор не может принять его за команду
2. **Валидация на входе** — whitelist допустимых символов/форматов: отсекает заведомо невалидный ввод ещё до бизнес-логики
3. **Экранирование на выходе** — контекстно-зависимое (HTML, JS, URL, CSS): один и тот же символ опасен по-разному в разных контекстах, поэтому экранировать нужно под целевой контекст
4. **Принцип минимальных привилегий** — DB-пользователь без прав `DROP`/`ALTER` ограничивает ущерб, даже если инъекция всё же прошла
5. **WAF** — дополнительный слой для фильтрации очевидных payload-ов, но это сетка безопасности, а не замена параметризации

## Q12. (!) Как SQL Injection работает через Spring Data JPA и JDBC?

`Spring Data JPA` защищает от SQL Injection **по умолчанию** — derived queries и именованные параметры под капотом используют `PreparedStatement`, где значения передаются драйверу отдельно от текста запроса. Инъекция возникает там, где разработчик обходит этот механизм и сам склеивает строку. Главные ловушки:
...
**Особый случай — динамические ORDER BY / table names.** Имена столбцов и таблиц — это часть структуры запроса, а не значения, поэтому placeholder `?` для них не работает. Здесь единственная защита — whitelist допустимых значений (никакого экранирования):
```java
...
`XSS` (`Cross-Site Scripting`) — инъекция вредоносного JavaScript в страницы, которые видят другие пользователи; чужой скрипт исполняется в их браузере и может украсть сессию или cookie. Хотя итоговое экранирование делает шаблонизатор на фронте, бэкенд — первая линия обороны: именно он сохраняет и отдаёт данные. Зона ответственности бэкенд-разработчика:
...
**Security headers** как дополнительный слой — `CSP` ограничивает, откуда браузеру разрешено грузить и исполнять скрипты, поэтому даже просочившийся инлайн-скрипт не запустится:
```java
...
`Command Injection` — выполнение произвольных команд ОС через пользовательский ввод. Опасность возникает, когда строка с вводом передаётся в shell: метасимволы `;`, `|`, `&&` позволяют дописать к ожидаемой команде свою. Главный приём защиты — вообще не вызывать shell, а запускать процесс напрямую с массивом аргументов.
...
`Insecure Design` (`A04:2021`) — **новая категория** в 2021. Её главная мысль: уязвимость может быть заложена в самой *задумке* системы, ещё до единой строки кода. Implementation-баг — это «защита есть, но в ней дыра», его ловят SAST/DAST и чинят патчем. Insecure design — «защиты не предусмотрели вовсе», и идеальный код тут не спасёт: проблему видно только при проектировании, через threat modeling.
...
Это не абстрактные принципы, а правила, которые напрямую ложатся на код Spring-приложения. Ключевые паттерны безопасного дизайна:

**1. Defense in Depth** — несколько независимых уровней защиты, чтобы пробой одного не открывал систему целиком (подробнее в Q39):
```java
...
`Security Misconfiguration` (`A05:2021`) — одна из самых частых причин инцидентов, и коварна она тем, что код может быть безупречным, а дыру открывает дефолтная или удобная для разработки настройка, забытая в проде. Типичные проблемы в `Spring Boot`:
...
Security headers — это инструкции браузеру, как безопаснее обращаться со страницей; дёшево включить, но закрывают целый класс клиентских атак (clickjacking, MIME-sniffing, downgrade на HTTP, исполнение чужих скриптов). В `Spring Security` настраиваются одним блоком:
...
Ключевой принцип: **базовая конфигурация безопасна по умолчанию, а удобные для разработки послабления включаются только в dev-профиле**. Так забытый профиль означает «более строго», а не «дыра в проде» — это тот же deny-by-default, но на уровне настроек:
...
`Vulnerable and Outdated Components` (`A06:2021`) — использование библиотек с известными `CVE`. Коварство в том, что проблема часто **транзитивная**: уязвимость не в той библиотеке, которую вы подключили явно, а в её зависимости (или зависимости зависимости) — её не видно в `build.gradle`, поэтому без инструмента такую дыру легко проглядеть. Отсюда вывод: полагаться нужно не на ручной аудит, а на автоматическое сканирование всего дерева зависимостей.
...
Главная ценность — скорость реакции: когда выходит новая критическая CVE, вопрос «затронуты ли мы?» решается запросом к SBOM, а не ручным обходом всех сервисов. Конкретные сценарии:
- **Быстрая реакция на CVE** — при уязвимости вроде `Log4Shell` за секунды видно, в каких артефактах есть уязвимая версия и какие именно
- **Compliance** — многие регуляторы (FDA, CISA) требуют SBOM как обязательный артефакт поставки
- **Лицензионный аудит** — выявление несовместимых лицензий (например, GPL в проприетарном продукте)
- **Supply chain transparency** — точное понимание, что именно входит в артефакт, без «сюрпризов» из транзитивных зависимостей

## Q23. Как организовать SCA в CI/CD pipeline?

`SCA` (`Software Composition Analysis`) — автоматическая сверка ваших зависимостей с базами известных уязвимостей. Цель — встроить эту проверку в pipeline так, чтобы критичная CVE блокировала merge, а не всплывала уже в проде:
...
`Identification and Authentication Failures` (`A07:2021`) — ошибки, которые позволяют злоумышленнику войти под чужой учётной записью. Опаснее всего те, что либо облегчают подбор пароля, либо позволяют использовать токен/сессию повторно:

1. **Слабая политика паролей** — минимум 8 символов сам по себе не помогает, нужна проверка по словарям утечек: пароль может быть длинным, но уже скомпрометированным
2. **Отсутствие `MFA`** — для критичных систем `MFA` обязателен: даже украденный пароль бесполезен без второго фактора
3. **Уязвимый password recovery** — предсказуемые токены восстановления или отсутствие rate limiting превращают «забыл пароль» в обходной вход
4. **`Session fixation`** — если сессию не пересоздать после логина, атакующий подсовывает жертве свой session ID заранее и наследует её сессию
5. **Длинноживущие токены** — `JWT` без expiration или с expiration в неделю невозможно быстро отозвать при компрометации
6. **Отсутствие lockout** — неограниченные попытки ввода пароля = открытая дверь для brute force
7. **Утечка информации** — разные ответы «пользователь не найден» и «неверный пароль» подтверждают атакующему, какие логины существуют (user enumeration)
...
Базовая защита от перебора — сделать каждую следующую неудачную попытку дороже: **rate limiting + progressive delay + account lockout**. Чем больше промахов, тем длиннее блокировка — это убивает экономику автоматического перебора, почти не мешая обычному пользователю, который ошибся пару раз.
...
**Credential stuffing** — это перебор не наугад, а готовыми парами логин/пароль из чужих утечек. Поэтому классический lockout по числу попыток для одного аккаунта помогает слабо: атакующий пробует каждую пару один раз по тысячам аккаунтов. Нужны другие меры:
- Проверка паролей по базе утечек (`HaveIBeenPwned` API) — отсекает заведомо скомпрометированные пароли
- `CAPTCHA` после N неудачных попыток — удорожает автоматизацию
- Детекция аномалий: новый IP, необычный User-Agent, всплеск запросов — ловит распределённую атаку по поведению, а не по счётчику на аккаунт

## Q26. Как безопасно управлять JWT-токенами?

`JWT` — распространённый механизм stateless-аутентификации: сервер не хранит сессию, а доверяет подписанному токену. Отсюда и две главные группы рисков — подделка подписи и невозможность быстро отозвать токен. Типичные ошибки и их правильные решения:
...
Небезопасная десериализация позволяет выполнить **произвольный код** на сервере (RCE) через специально сконструированный объект. Причина: `readObject()` восстанавливает граф объектов любых классов из classpath, а у некоторых библиотек есть «gadget chains» — цепочки вызовов, которые при десериализации запускают опасный код. То есть проблема не в данных, а в том, что мы позволяем недоверенному вводу управлять тем, какие классы и методы инстанцируются.
...
CI/CD — привлекательная мишень: это привилегированная среда с доступом к секретам и правом деплоить в прод, поэтому компрометация runner-а или конфига pipeline по последствиям эквивалентна компрометации продакшена. Защита строится на изоляции и минимальных правах:
...
`Security Logging and Monitoring Failures` (`A09:2021`) — про то, что атаку, которую никто не записывает и на которую никто не реагирует, замечают месяцами. Стратегия логирования должна закрывать всю цепочку «записали → заметили → среагировали»: без любого звена остальные бесполезны. Масштаб проблемы: средний TTD (Time to Detect) breach — **287 дней** (IBM Cost of Data Breach Report).
...
Логировать нужно всё, что помогает восстановить картину атаки: попытки входа, отказы доступа и чувствительные операции. Удобно вынести это в один аспект, чтобы аудит не размазывался по бизнес-коду:
...
Алертинг — это то, что превращает пассивные логи в реальное обнаружение: лог сам по себе никто не читает в реальном времени, а алерт на аномалию (всплеск неудачных логинов, попытка доступа к admin от обычного пользователя) поднимает on-call здесь и сейчас. Идея — считать события в окне времени и бить тревогу при превышении порога:
...
`SSRF` (`Server-Side Request Forgery`, `A10:2021`) — **новая категория**: злоумышленник подсовывает приложению URL, и сервер сам идёт по нему — туда, куда самому атакующему ходу нет. Ключевая опасность в том, что запрос выполняется *от имени сервера*, с его сетевым доступом, в обход внешнего firewall — так становятся достижимы внутренние сервисы.
...
Надёжная защита — многоуровневая, потому что одной проверки мало: одной валидации схемы недостаточно, а DNS rebinding обходит проверку по имени хоста. Поэтому проверяем и схему, и хост (предпочтительно whitelist), и **реальный IP после резолва** — последнее ловит трюки, когда домен резолвится в `127.0.0.1` или `169.254.169.254`:
...
**Сетевой уровень** (defense in depth) — страховка на случай, если валидацию в коде обошли:
- `Network Policy` в Kubernetes — ограничить egress-трафик подов, чтобы приложение физически не могло достучаться до внутренних адресов
- `VPC Security Groups` / firewall — запретить обращение к metadata endpoint на уровне сети
- `IMDSv2` в AWS — требует PUT-запрос с заголовком `X-aws-ec2-metadata-token`, а простой GET (которым обычно и работает SSRF) уже не получит токен
...
Security-тесты пишут так же, как функциональные, но проверяют они негативные сценарии: «что *не* должно быть доступно». Минимальный набор закрывает топовые риски — авторизацию (включая IDOR), инъекции и наличие security-заголовков. Важно тестировать обе стороны: и что разрешённое работает, и что запрещённое отклоняется:
...
Главная цель секрет-менеджмента — чтобы секрет не оказался там, где его легко прочитать или забыть удалить. Поэтому секреты **не** хранят в коде, `application.yml` или Docker-образах (всё это попадает в git и registry), а к переменным окружения относятся осторожно: они видны в `/proc`, `docker inspect` и не дают ни аудита, ни ротации.

Подходы выстраиваются по нарастанию надёжности — чем выше в таблице, тем больше контроля (ротация, аудит, lease), но и больше сложности:
...
**Path Traversal** (CWE-22) — атака, при которой злоумышленник вставляет в имя файла последовательности `../`, чтобы выйти за пределы разрешённого каталога и прочитать чужие файлы (`/etc/passwd`, ключи, конфиги). Корень проблемы — склейка базового пути с непроверенным вводом. Защита из двух шагов: нормализовать путь (схлопнуть `../`), а затем убедиться, что результат всё ещё лежит внутри базового каталога.
...
**XXE** (XML External Entity Injection, CWE-611) — атака через XML-парсер, который по умолчанию разворачивает внешние сущности (`<!ENTITY ... SYSTEM "...">`). Через них атакующий заставляет парсер прочитать локальный файл, сходить по URL (SSRF), а в редких случаях — добиться RCE. Причина — историческое поведение Java-парсеров: внешние сущности включены по умолчанию, поэтому защита сводится к их явному отключению.
...
Загрузка файлов опасна тем, что одним запросом затрагивает сразу несколько классов уязвимостей: path traversal (через имя файла), хранение и исполнение вредоносных файлов (загруженный `.jsp`/`.php`), DoS огромными файлами, XSS через SVG/HTML с инлайн-скриптом. Поэтому защита тоже многослойна — нельзя доверять ни расширению, ни имени, ни заявленному типу: проверяют размер, белый список типов, реальную сигнатуру (magic bytes) и сохраняют под собственным безопасным именем вне webroot.
...
**Log4Shell** (CVE-2021-44228) — критическая уязвимость в `Apache Log4j 2.x` (CVSS 10.0): RCE достигался через JNDI lookup в *любом логируемом значении*. Опасность была в обыденности вектора — достаточно, чтобы приложение залогировало строку, пришедшую от пользователя (User-Agent, имя, поисковый запрос), а это делает почти каждый сервис. Корень бага: Log4j интерпретировал подстроки `${...}` в сообщениях и сам ходил по `ldap://`-ссылке, загружая чужой класс.
...
Неправильное управление ключами — одна из самых частых криптографических ошибок (OWASP A02), и она обесценивает всё шифрование: стойкий `AES-256` бесполезен, если ключ лежит в коде или его легко прочитать из окружения. Принцип простой — ключ должен жить отдельно от приложения, в защищённом хранилище (KeyStore, Vault, KMS/HSM), генерироваться криптостойко и регулярно ротироваться.
