...
> [!mcq] Что главное в современной модели `AppSec` для Senior?
> - [ ] `AppSec` -- это финальный security-аудит перед релизом командой `InfoSec` | Аудит в конце SDLC находит уязвимости когда исправить дорого. ❌ ПОСЛЕДСТВИЕ: Equifax 2017 — уязвимость Apache Struts CVE-2017-5638 не закрыли вовремя, утекли данные 147M клиентов, штраф $700M.
> - [x] `AppSec` -- непрерывный процесс на всех этапах SDLC: threat modeling, secure coding, SAST/DAST, runtime monitoring | Security строится shift-left: ранний контроль дешевле и эффективнее точечного аудита перед релизом. ✓ ПРИМЕНЯТЬ: Netflix Paved Road, GitHub Advanced Security, Snyk в pipeline. 📋 ПРАВИЛО: «Security shift-left, не shift-late». 🔗 См. Q3, Q25, Q36.
> - [ ] `AppSec` сводится к настройке `WAF` и `firewall` на уровне инфраструктуры | Network controls не защищают от логических ошибок и SQL injection в самом коде. ❌ ПОСЛЕДСТВИЕ: Capital One 2019 — SSRF через misconfigured WAF + IAM, утекли 100M записей, штраф $80M.
> - [ ] Достаточно использовать `Spring Security` и не думать о других угрозах | Framework закрывает auth/authz, но не валидацию ввода, secrets management, deserialization. ❌ ПОСЛЕДСТВИЕ: BCrypt cost factor 4 на дефолте → пароли брутфорсятся за минуты, GDPR violation на €20M.
...
> [!mcq] Что означает `Fail Secure` для контроллера, который проверяет доступ к ресурсу?
> - [ ] При исключении в `authService` логируем ошибку и возвращаем `200 OK` с пустым body, чтобы не ломать UX | Открытый доступ при сбое — нарушение CIA. ❌ ПОСЛЕДСТВИЕ: при flaky DB на authorization service любой запрос проходит, IDOR-атаки в проде, утечка PII.
> - [ ] При сбое разрешаем доступ только для `ROLE_USER`, отказываем `ROLE_ADMIN` | Логика инвертирована и зависит от роли — обходится манипуляцией claim в JWT. ❌ ПОСЛЕДСТВИЕ: атакующий с подделанным JWT_role=USER читает чужие записи, audit log не показывает аномалий.
> - [ ] При исключении возвращаем `500` с stacktrace в body для диагностики | Stacktrace раскрывает версии, package layout, внутренние имена сущностей. ❌ ПОСЛЕДСТВИЕ: атакующий по версии Spring 5.3.18 находит CVE-2022-22965 (Spring4Shell) и эксплуатирует RCE.
> - [x] При любой ошибке проверки прав возвращаем `403 Forbidden` без деталей и логируем для SOC | Default-deny при сбоях: отсутствие доказательств доступа = отказ. ✓ ПРИМЕНЯТЬ: Spring Security `AccessDeniedHandler`, AWS IAM implicit deny. 📋 ПРАВИЛО: «Нет ответа от authz — нет доступа». 🔗 См. Q4, Q23, Q38.
...
> [!mcq] В чём суть `Defense in Depth` для Java/Spring-приложения?
> - [ ] Достаточно одного сильного слоя — например, корректно настроенного `WAF` на периметре | Один слой — single point of failure: zero-day или конфиг-баг открывает всё. ❌ ПОСЛЕДСТВИЕ: Capital One 2019 — bypass WAF через SSRF, актуальные IAM права позволили читать S3, $80M штраф.
> - [ ] Применять одинаковые контроли (auth) на всех слоях, чтобы не было дублей | Одинаковые контроли = коррелированные сбои: уязвимость в одном повторяется во всех. ❌ ПОСЛЕДСТВИЕ: общий JWT-secret в gateway и downstream-сервисах — компрометация одного узла даёт доступ ко всем API.
> - [x] Накладывать разные контроли на разных слоях (network/transport/api/app/data), чтобы пробой одного не ронял систему | Слои защиты дополняют друг друга: WAF + TLS + Spring Security + DB encryption — атакующий должен пробить каждый. ✓ ПРИМЕНЯТЬ: Netflix layered defense (Zuul + Eureka + service-mesh mTLS), AWS shared responsibility. 📋 ПРАВИЛО: «Лук имеет много слоёв — и каждый слой плачет атакующего». 🔗 См. Q1, Q23, Q40.
> - [ ] Защищать только данные через encryption at rest, остальное — необязательно | Encryption at rest защищает только украденный диск; не защищает от SQL injection или скомпрометированного app-сервера. ❌ ПОСЛЕДСТВИЕ: при SQLi приложение само расшифровывает PII через KMS и отдаёт атакующему — encryption становится бесполезным.
...
> [!mcq] В чём разница между Authentication (`AuthN`) и Authorization (`AuthZ`)?
> - [ ] `AuthN` определяет права на ресурсы, `AuthZ` проверяет логин и пароль | Термины перепутаны: AuthN = «кто», AuthZ = «что разрешено». ❌ ПОСЛЕДСТВИЕ: разработчик называет `@PreAuthorize` «authentication check» и не валидирует JWT signature — leaked token открывает чужие записи.
> - [ ] `AuthN` и `AuthZ` -- это синонимы и решаются одним вызовом `Authentication.isAuthenticated()` | Метод проверяет только factum логина, не права на конкретный ресурс. ❌ ПОСЛЕДСТВИЕ: IDOR-уязвимость — `GET /accounts/{id}` авторизован любому залогиненому, любой клиент видит чужие счета.
> - [x] `AuthN` -- проверка личности (кто пользователь), `AuthZ` -- проверка прав (что ему разрешено) | После успешного AuthN формируется `Principal`, на котором AuthZ применяет правила: RBAC, ABAC, ownership-checks. ✓ ПРИМЕНЯТЬ: Spring Security `AuthenticationManager` + `@PreAuthorize`, OAuth2 access_token + scopes, AWS IAM policies. 📋 ПРАВИЛО: «AuthN отвечает кто, AuthZ -- что можно». 🔗 См. Q5, Q7, Q23.
> - [ ] `AuthN` нужна только в браузере, `AuthZ` -- только в backend | Оба нужны на backend; UI-проверки обходятся curl-ом или Postman. ❌ ПОСЛЕДСТВИЕ: hidden admin button скрыт в UI, но `/api/admin/delete` отдаёт `200` любому залогиненому — escalation за 1 запрос.
...
> [!mcq] Когда выбрать `Token-Based (JWT)` вместо `Session-Based` аутентификации?
> - [ ] Всегда — JWT современнее и универсально подходит для всех приложений | JWT хуже для server-rendered apps с критичной функцией logout: invalidation требует отдельный механизм. ❌ ПОСЛЕДСТВИЕ: украденный JWT валиден до expiration, нельзя отозвать; банковское приложение допустило кражу токена и не смогло выкинуть пользователя за 6 часов.
> - [ ] Когда нужна максимальная безопасность чтения cookies в браузере | Session-cookie с `HttpOnly+Secure+SameSite` безопаснее `localStorage` для токена; сравнение неверное. ❌ ПОСЛЕДСТВИЕ: JWT в `localStorage` крадётся через XSS, пока session-cookie с `HttpOnly` недоступен JS — XSS leak в SPA приводит к full account takeover.
> - [ ] Когда сервер должен помнить состояние всех пользователей | JWT — `stateless`, состояние не хранится; описание противоречит сути JWT. ❌ ПОСЛЕДСТВИЕ: разработчик хранит JWT в Redis и сверяет каждый запрос — теряет преимущества stateless и плодит SPOF.
> - [x] В микросервисах и SPA где нужен stateless и horizontal scaling без sticky sessions | JWT не требует общего session storage между нодами: signature-проверка локальна. ✓ ПРИМЕНЯТЬ: Netflix Zuul + JWT, Auth0, Keycloak с access_token. 📋 ПРАВИЛО: «JWT для микросервисов, session — для монолита». 🔗 См. Q4, Q7, Q8.
...
> [!mcq] Какой алгоритм хеширования паролей выбрать для нового Spring Boot-проекта в 2026?
> - [ ] `MD5` или `SHA-256` с солью — быстро и стандартно для high-throughput сервисов | Скорость — недостаток для паролей: GPU считает миллиарды SHA-256 в секунду. ❌ ПОСЛЕДСТВИЕ: LinkedIn 2012 — 117M unsalted SHA1, 90% паролей восстановлены за 3 дня; компания на $1.25M settle.
> - [ ] `BCrypt` с дефолтным `cost=4` — баланс безопасности и скорости логина | `cost=4` означает ~16ms на проверку, GPU брутфорс 100M/час. ❌ ПОСЛЕДСТВИЕ: BCrypt cost factor 4 → password brute force за минуты, GDPR violation €20M.
> - [x] `BCrypt` с `cost≥12` или `Argon2id` через `DelegatingPasswordEncoder` для миграции | BCrypt cost 12 = ~250ms на хэш, Argon2id memory-hard и устойчив к GPU/ASIC. DelegatingPasswordEncoder позволяет перехешировать со старого алгоритма при login без logout users. ✓ ПРИМЕНЯТЬ: Spring Security default с 5.7+, OWASP Password Storage Cheat Sheet, Auth0 Argon2id. 📋 ПРАВИЛО: «Хеш должен быть медленным -- иначе он бесполезен». 🔗 См. Q1, Q5, Q25.
> - [ ] `SHA-512` с глобальной солью из `application.yml` | Глобальная соль = одинаковый pepper для всех; rainbow tables строятся один раз и работают для всей БД. ❌ ПОСЛЕДСТВИЕ: leaked DB + leaked config = все пароли в открытом виде, Adobe 2013 — 153M записей.
...
> [!mcq] Какая `JWT`-конфигурация безопаснее в production-среде?
> - [ ] `alg=none` без подписи + claims в payload — экономит CPU при валидации | `alg=none` отключает подпись: атакующий меняет payload и претендует кем угодно. ❌ ПОСЛЕДСТВИЕ: CVE-2015-9235 в jsonwebtoken — atomic library accept `alg=none`, тысячи сервисов взломаны через подмену role=admin.
> - [x] `HS256/RS256` подпись + `exp ≤ 15 мин` + secret из Vault + refresh-token rotation | Короткий TTL ограничивает окно компрометации; rotation refresh-token выявляет кражу. ✓ ПРИМЕНЯТЬ: Auth0, Keycloak, AWS Cognito с RS256, Spring Authorization Server. 📋 ПРАВИЛО: «Подпись + короткий TTL + ротация = JWT с тормозами». 🔗 См. Q5, Q8, Q27.
> - [ ] `HS256` с одним global secret в `application.properties` + `exp = 30 дней` | Долгий TTL расширяет окно атаки, secret в config без шифрования утекает в Git. ❌ ПОСЛЕДСТВИЕ: JWT без expiration → leaked token валиден forever, no revocation; AWS leak через GitHub за час потерял $50k.
> - [ ] Хранить чувствительные данные (PII, password hash) в `payload` для удобства | Payload только base64-кодирован, любой клиент читает claims. ❌ ПОСЛЕДСТВИЕ: medical app поместил `diagnosis` в JWT, токен попал в browser history → утечка под HIPAA, fines $500K.
...
> [!mcq] Зачем нужна ротация (`rotation`) refresh-токенов?
> - [ ] Чтобы уменьшить размер payload и сэкономить трафик в мобильных приложениях | Размер не меняется при ротации; цель — детект кражи. ❌ ПОСЛЕДСТВИЕ: разработчик отключил ротацию ради «экономии», украденный refresh-token используется параллельно с легитимным месяцами без детекта.
> - [ ] Чтобы один пользователь мог иметь только одну активную сессию | Это session limit, не ротация — разные функции. ❌ ПОСЛЕДСТВИЕ: пользователь не может одновременно работать на телефоне и десктопе, business loses retention в B2B-сценариях.
> - [ ] Чтобы клиент мог отозвать access-token раньше его `exp` | Это revocation list, а не ротация; ротация только refresh-токены. ❌ ПОСЛЕДСТВИЕ: разработчик путает механизмы, не реализует ни blacklist, ни ротацию — leaked access-token валиден до exp.
> - [x] Чтобы выявить кражу refresh-токена: использование старого токена после refresh означает атаку | OAuth 2.0 RFC 6819 рекомендует Reuse Detection: при попытке использовать инвалидированный refresh-token Auth-сервер ревокирует всю цепочку. ✓ ПРИМЕНЯТЬ: Auth0 Refresh Token Rotation, OAuth 2.0 BCP, Spring Authorization Server. 📋 ПРАВИЛО: «Один refresh -- одна выдача, второй заход = тревога». 🔗 См. Q4, Q7, Q23.
...
> [!mcq] Какой подход к защите от SQL Injection в `Spring Data JPA` корректен?
> - [ ] Использовать `@Query("SELECT u FROM User u WHERE u.name = '" + name + "'")` с конкатенацией | Конкатенация даже в JPQL передаёт строку как литерал, не как параметр — обходится. ❌ ПОСЛЕДСТВИЕ: пользователь вводит `'; DROP TABLE users;--`, JPQL парсер пропускает, Hibernate генерирует уязвимый SQL.
> - [ ] Экранировать кавычки regex-ом перед подстановкой в native query | Blacklist всегда неполный: 2nd-order injection через UTF-8, hex, comments обходит regex. ❌ ПОСЛЕДСТВИЕ: TalkTalk 2015 — DIY-санитизация пропустила comment-injection, утечка 157K записей, штраф £400K.
> - [x] Использовать derived queries (`findByName(String name)`) или `@Query` с `:param`/`?N` биндингами | Spring Data автоматически использует PreparedStatement, параметры не интерпретируются как SQL. ✓ ПРИМЕНЯТЬ: Spring Data JPA, Hibernate, любой production Java-код. 📋 ПРАВИЛО: «Параметры -- значения, не код». 🔗 См. Q14, Q18, Q25.
> - [ ] Вызывать `entityManager.createNativeQuery(sql).getSingleResult()` со склеенной строкой | Native query без параметров идентичен прямому JDBC concat — та же уязвимость. ❌ ПОСЛЕДСТВИЕ: legacy admin-панель с `like '%' + name + '%'` → blind SQLi через time-based payload, exfiltrate password hash.
...
> [!mcq] Какой подход к защите от `XSS` максимально надёжен?
> - [ ] Глобальный `String.replace("<", "&lt;")` на входе для всех request body | Input sanitization ломает легитимный контент (`<3` в чате) и не покрывает unicode/hex-обходы. ❌ ПОСЛЕДСТВИЕ: пользователь не может ввести `<` в комментарии, при этом `<` обходит фильтр и stored XSS работает.
> - [x] Context-aware output encoding: `Encode.forHtml`, `Encode.forJavaScript`, `Encode.forUri` + `CSP` header | Каждый контекст имеет свой набор опасных символов; OWASP Java Encoder применяет правильное экранирование. CSP `script-src 'self'` блокирует inline-скрипты как defense-in-depth. ✓ ПРИМЕНЯТЬ: OWASP Java Encoder, Thymeleaf `th:text`, React JSX auto-escape. 📋 ПРАВИЛО: «Кодируй на выходе под контекст вывода». 🔗 См. Q14, Q21, Q22.
> - [ ] Использовать `th:utext` в Thymeleaf для всего пользовательского контента | `th:utext` отключает экранирование — ровно противоположное защите. ❌ ПОСЛЕДСТВИЕ: Output encoding context-mismatch (HTML вместо JS) → XSS bypass через `onclick=alert()`, stored XSS в комментариях форума.
> - [ ] Полагаться только на `X-XSS-Protection: 1; mode=block` header | Header deprecated с Chrome 78, в Firefox/Safari никогда не работал. ❌ ПОСЛЕДСТВИЕ: разработчик отключил серверную санитизацию, считая что browser защитит, — все современные браузеры игнорируют header, stored XSS активен.
...
> [!mcq] Когда корректно отключать `CSRF`-защиту в Spring Security?
> - [ ] Всегда отключать в production: ускоряет requests на 5-10мс | CSRF token не вносит заметной latency и нужен для cookie-auth. ❌ ПОСЛЕДСТВИЕ: банковское API на сессионных cookies без CSRF — атакующий через `<img src="bank.com/transfer?to=evil">` крадёт деньги жертвы.
> - [ ] Отключать для GET-запросов, оставлять для POST | Spring Security уже не применяет CSRF к safe methods (GET/HEAD/OPTIONS); настройка избыточна. ❌ ПОСЛЕДСТВИЕ: разработчик пишет CSRF в `WebSecurityCustomizer` неправильно — выключает full-on, теряет защиту для POST.
> - [x] Только для stateless API c JWT в `Authorization` header (не cookies) и `SessionCreationPolicy.STATELESS` | Browser не отправляет `Authorization` header автоматически с cross-origin запросов, поэтому CSRF не применим. ✓ ПРИМЕНЯТЬ: REST API с JWT, gRPC сервисы, mobile-only API. 📋 ПРАВИЛО: «Cookies нуждаются в CSRF, header tokens -- нет». 🔗 См. Q5, Q7, Q20.
> - [ ] При использовании HTTPS — TLS защищает от CSRF | TLS защищает confidentiality/integrity, не от cross-origin запросов с авторизованной сессией. ❌ ПОСЛЕДСТВИЕ: Github 2018 — CSRF на GraphQL endpoint работал поверх HTTPS, account takeover через одну ссылку.
...
> [!mcq] Безопаснее всего защититься от Java deserialization-атак можно так
> - [ ] Шифровать сериализованные данные AES-256 перед передачей | Шифрование защищает confidentiality, но если ключ скомпрометирован — атака proходит так же; не root cause. ❌ ПОСЛЕДСТВИЕ: leaked key через config-leak → атакующий генерирует gadget chain (CommonsCollections), RCE на сервере.
> - [ ] Десериализовать через `ObjectInputStream` и проверять hash после readObject | Hash проверяется ПОСЛЕ readObject, который уже выполнил gadget chain. ❌ ПОСЛЕДСТВИЕ: PayPal 2016 — RCE через Java deserialization, исправлено введением sandbox; Equifax CVE-2017-5638 (Struts) того же класса.
> - [x] Заменить Java Serialization на JSON (Jackson) с `@Valid` либо использовать `ObjectInputFilter` whitelist | JSON парсеры не вызывают конструкторы произвольных классов; ObjectInputFilter (JEP 290, JDK 9+) разрешает только known-safe классы. ✓ ПРИМЕНЯТЬ: Spring REST с Jackson, JEP 290 ObjectInputFilter, contrast Security agent. 📋 ПРАВИЛО: «Не доверяй readObject -- джейсон или whitelist». 🔗 См. Q9, Q14, Q25.
> - [ ] Положиться на `SerialVersionUID` -- разные версии не совместимы | UID защищает от случайной несовместимости версий, не от malicious gadget chains. ❌ ПОСЛЕДСТВИЕ: разработчик считает что SerialVersionUID = безопасность, gadget использует валидный класс с тем же UID, RCE достигнут.
...
> [!mcq] Какая защита от SSRF самая полная?
> - [ ] Регулярка проверяющая что URL не содержит `127.0.0.1`, `localhost`, `169.254.169.254` | Blacklist обходится через `0.0.0.0`, `[::1]`, IPv4-mapped IPv6, DNS rebinding на legitimate domain. ❌ ПОСЛЕДСТВИЕ: Capital One 2019 — SSRF на AWS metadata `169.254.169.254` через WAF bypass, утечка 100M записей.
> - [ ] Запретить пользователям вводить URL — пусть выбирают из списка | Парадокс: enum-выбор делает фичу непрактичной для CMS/integration; не масштабируется. ❌ ПОСЛЕДСТВИЕ: продуктовая фича «webhook URL» становится недоступной, бизнес теряет интеграции с партнёрами.
> - [x] Whitelist hostnames + проверка resolved IP (не loopback/site-local/link-local) с защитой от DNS rebinding | DNS rebinding атака возвращает разные IP при первом и втором lookup; нужно резолвить и pin-овать IP перед запросом. ✓ ПРИМЕНЯТЬ: Stripe webhook validation, GitHub webhook IP allowlist, AWS VPC isolated egress. 📋 ПРАВИЛО: «Whitelist host + pin IP -- DNS не врёт дважды». 🔗 См. Q9, Q14, Q40.
> - [ ] Поставить `SecurityManager` и доверить ему фильтрацию URL | SecurityManager deprecated в Java 17, removed в Java 24; не предназначался для URL-фильтрации. ❌ ПОСЛЕДСТВИЕ: legacy app полагается на SecurityManager, после апгрейда на JDK 21 защита снимается, SSRF активна.
...
> [!mcq] Как корректно защититься от OS Command Injection в Java?
> - [ ] Использовать `Runtime.exec(String)` с экранированием кавычек | `Runtime.exec(String)` парсит строку как shell-команду; экранирование легко обходится через `;`, `|`, `&&`. ❌ ПОСЛЕДСТВИЕ: pdf-generator вызывает `pdftk file.pdf burst` с user-input → атакующий `; rm -rf /` → wipe сервера в production.
> - [ ] Полагаться на `SecurityManager.checkExec()` | SecurityManager deprecated в JDK 17+, removed в JDK 24; не следует использовать. ❌ ПОСЛЕДСТВИЕ: после миграции на Java 21 защита снимается, security regression обнаруживается через post-incident audit.
> - [ ] Заменить `bash` на `sh` для большей безопасности | Shell версия не меняет атакующую поверхность; обе уязвимы к meta-characters. ❌ ПОСЛЕДСТВИЕ: false sense of security, та же RCE через `; whoami`.
> - [x] `ProcessBuilder` с массивом аргументов + whitelist regex для каждого параметра | `ProcessBuilder(List<String>)` не проходит через shell, аргументы не интерпретируются. ✓ ПРИМЕНЯТЬ: Spring Boot Admin restart hooks, Jenkins pipeline shell steps. 📋 ПРАВИЛО: «Аргументы массивом, не строкой -- shell идёт лесом». 🔗 См. Q9, Q13, Q18.
...
> [!mcq] Какой режим симметричного шифрования выбрать в Java для конфиденциальных данных?
> - [x] `AES/GCM/NoPadding` с уникальным IV на каждое шифрование и AAD для контекста | GCM — authenticated encryption: даёт confidentiality + integrity + защиту от tampering за один проход. Уникальный IV критичен: повторение IV при том же ключе раскрывает plaintext. ✓ ПРИМЕНЯТЬ: AWS KMS, Google Tink, Spring Security Crypto, JEP 329. 📋 ПРАВИЛО: «GCM с свежим IV -- стандарт 2026». 🔗 См. Q16, Q17, Q27.
> - [ ] `AES/ECB/PKCS5Padding` -- стандартный режим JDK | ECB шифрует одинаковые блоки одинаково: видны паттерны (классический пингвин-пример). ❌ ПОСЛЕДСТВИЕ: Adobe 2013 — пароли зашифрованы AES/ECB, leaked DB позволил восстановить большинство по структурным повторам, 153M users.
> - [ ] `DES/CBC/PKCS5Padding` -- meets PCI DSS minimum | DES имеет 56-bit key, ломается за часы; PCI DSS требует ≥AES-128. ❌ ПОСЛЕДСТВИЕ: legacy банковское приложение со 56-bit DES — auditor выписывает PCI failure, fines $5K/month, бизнес теряет лицензию на card processing.
> - [ ] `AES/CBC/PKCS5Padding` без HMAC -- проще GCM, ничем не уступает | CBC без HMAC уязвим к padding oracle (POODLE-style); нужна явная MAC проверка. ❌ ПОСЛЕДСТВИЕ: оплачивающий cookie шифруется AES/CBC, padding oracle attack извлекает session token за 256 запросов на байт.
