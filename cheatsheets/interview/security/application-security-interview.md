...
- [Q13. Как защититься от `SSRF` (`Server-Side Request Forgery`)?](#q13-как-защититься-от-ssrf-server-side-request-forgery)
...
- [Q22. Что такое `Content Security Policy` (`CSP`)?](#q22-что-такое-content-security-policy-csp)
...
Безопасность строится на нескольких фундаментальных принципах, которые задают рамку для любого технического решения. Главный из них — триада `CIA`: всё остальное (least privilege, fail secure и т. д.) лишь способы её обеспечить.

### `CIA Triad`

Три свойства, которые система обязана сохранять. Если ломается хотя бы одно — это инцидент безопасности.
...
Каждый пользователь и процесс получает минимум прав, необходимых для работы, — чтобы скомпрометированная учётка или сервис нанесли минимум ущерба:
...
При ошибке система должна закрываться, а не открываться: непредвиденное исключение в проверке доступа должно приводить к отказу, а не к выдаче ресурса. Иначе любой сбой превращается в дыру:
...
Методы различаются по тому, **где хранится состояние** (на сервере или в токене) и **сколько факторов** проверяется. Выбор диктуется архитектурой: для монолита естественны сессии, для микросервисов и SPA — токены, для делегированного доступа — OAuth. Ниже — от простого к сложному.
...
`Base64` -- это кодирование, а не шифрование, поэтому учётные данные передаются практически открытым текстом. Проблемы: небезопасно без `HTTPS`, нет logout, пароль уходит при каждом запросе. Применимо разве что для внутренних сервисов за `TLS`.
...
`Stateless`: сервер не хранит сессию, а проверяет подпись токена, который клиент шлёт в заголовке. Это снимает необходимость в общем хранилище сессий и потому идеально подходит для микросервисов и `SPA`.

### 4. `OAuth 2.0` / `OpenID Connect`

Делегированная авторизация: пользователь даёт стороннему приложению доступ к своим данным, не передавая ему пароль. Четыре роли: `Resource Owner` (владелец данных), `Client` (приложение), `Authorization Server` (выдаёт токены), `Resource Server` (хранит данные). `OpenID Connect` добавляет поверх OAuth слой аутентификации (id_token). Подробнее -- в [OAuth 2.0](oauth2-interview.md).
...
**Ключевой момент -- ротация refresh-токенов.** При каждом обновлении старый refresh-токен удаляется и выдаётся новый. Это защищает от replay-атак: если украденный токен уже был использован легитимным клиентом, повторная попытка с ним провалится. А если злоумышленник применил его первым, легитимный клиент получит отказ и инцидент станет заметен (token reuse detection).

## Q9. (!) Как защититься от `SQL`-инъекций?

`SQL`-инъекция -- атака, при которой пользовательский ввод попадает в `SQL`-запрос как часть кода, а не как данные, и злоумышленник через это выполняет произвольный `SQL`. Корень проблемы один: конкатенация ввода в текст запроса. Основная защита -- параметризованные запросы, где ввод всегда передаётся отдельно от кода и не может изменить структуру запроса.
...
Параметризация закрывает 99% случаев; остальные меры -- эшелонированная защита на случай ошибки или экзотики (например, когда динамическим должно быть имя таблицы, а его параметром не передать).

1. **Валидация ввода** -- whitelist допустимых символов; для динамических идентификаторов (имя таблицы/колонки) сверка со списком разрешённых, а не экранирование
2. **Least Privilege** -- учётная запись БД с минимальными правами: даже при успешной инъекции атакующий не сможет, например, дропнуть таблицу
3. **ORM** -- `JPA`/`Hibernate` параметризуют запросы по умолчанию, но JPQL/native-конкатенация всё равно уязвима
4. **Никогда** не конкатенируйте пользовательский ввод в `SQL` -- это правило без исключений

## Q10. (!) Как защититься от `XSS`-атак?

`XSS` (`Cross-Site Scripting`) -- внедрение вредоносного скрипта в веб-страницу, который выполняется в браузере жертвы с её правами: крадёт cookie/токены, выполняет действия от её имени, подменяет содержимое страницы. Как и SQL-инъекция, это смешение данных и кода, только на стороне браузера. Главная защита -- **контекстно-зависимое экранирование вывода**: любой пользовательский ввод должен попадать в HTML как текст, а не как разметка.
...
Для `stateless REST API` с токенной аутентификацией (`JWT` в заголовке `Authorization`) `CSRF` не нужен. Причина прямо следует из механики атаки: токен в заголовке `Authorization` браузер сам не подставляет (в отличие от cookie), значит, страница `evil.com` не сможет его приложить -- подделать запрос нечем. Защита нужна именно там, где аутентификация держится на cookie.
...
Десериализация недоверенных данных -- одна из самых опасных уязвимостей в Java (`OWASP A8`). Опасность в том, что `ObjectInputStream.readObject()` восстанавливает граф объектов и при этом вызывает их методы (`readObject`, `readResolve`, финализаторы). Подобрав цепочку классов в classpath (gadget chain, например из Apache Commons Collections), атакующий через специально собранный байтовый поток добивается выполнения произвольного кода (`RCE`) ещё до того, как вы что-либо проверите. Главный вывод: **не десериализуйте недоверенные данные нативной Java-сериализацией вообще** -- используйте JSON.
...
`SSRF` -- атака, при которой приложение по `URL` от атакующего делает запрос туда, куда сам атакующий доступа не имеет. Сервер становится прокси: через него запрашивают внутренние сервисы (`http://localhost:8080/admin`), метаданные облака (`http://169.254.169.254/` -- кража IAM-credentials в AWS), порты внутренней сети. Защита -- не дать приложению ходить по произвольным адресам: whitelist хостов/схем плюс блокировка приватных IP-диапазонов.
...
Инъекции -- это всегда одна и та же ошибка в разных интерпретаторах: пользовательский ввод смешивается с командой/запросом и меняет её смысл. SQL -- лишь самый известный случай; та же логика бьёт по shell, LDAP, логам, XML, шаблонизаторам. И защита везде одна: **разделять данные и команду** (параметризованные API, builder-ы) либо, где это невозможно, валидировать ввод по whitelist. Ниже -- три типичных не-SQL варианта.
...
В JDK шифрование делается через `javax.crypto` (`Cipher`, `KeyGenerator`, `SecretKey`). Для шифрования данных применяют симметричный `AES`, причём в режиме `GCM` -- он даёт authenticated encryption (конфиденциальность + защита от подмены). Три практических правила, которые на собеседовании ценят выше, чем сам факт «использую AES»:

- **Случайный IV на каждое шифрование** и его нельзя переиспользовать с одним ключом (для GCM повтор IV катастрофичен -- раскрывает ключ аутентификации).
- **IV не секрет** -- его кладут рядом с шифротекстом (в примере он добавляется в начало).
- **Ключи -- из защищённого хранилища** (`Vault`/`KMS`), не хардкод и не в коде.
...
Коротко: **симметричное** использует один общий ключ для шифрования и расшифровки (быстро, но возникает проблема -- как безопасно передать ключ обеим сторонам); **асимметричное** использует пару ключей -- публичный шифрует, приватный расшифровывает (решает проблему передачи ключа, но медленно). Поэтому на практике их комбинируют: асимметрией обмениваются симметричным ключом, а данные шифруют симметрией.
...
Это два разных состояния данных, и каждое защищается своими средствами. **In transit** -- данные в движении по сети (защита от перехвата): `TLS`/`mTLS`. **At rest** -- данные в покое на диске/в БД (защита от кражи носителя или дампа): шифрование БД, диска, отдельных полей. Зрелая система закрывает оба: одно без другого оставляет дыру.
...
Параметры запроса и path-переменные валидируются аннотациями ограничений (`@Min`, `@Size`, `@NotBlank`) прямо на аргументах метода, но это работает только если на классе контроллера стоит `@Validated`. Без неё аннотации на `@RequestParam`/`@PathVariable` молча игнорируются -- частая ловушка:
...
Почему так: `@Valid` срабатывает на уровне Spring MVC и применима только к `@RequestBody`/сложным объектам. `@Validated` подключает method-level validation через AOP-прокси, и только она активирует ограничения на простых параметрах метода. Нарушение даёт `ConstraintViolationException` -- его удобно отдельно обработать в `@ExceptionHandler`, иначе клиент получит 500 вместо 400.
...
`HTTP`-заголовки безопасности -- это инструкции серверу-браузеру, которые включают встроенные защитные механизмы самого браузера. Они не заменяют серверную защиту, а добавляют дешёвый и эффективный слой: одной строкой конфига заставляют браузер форсить HTTPS, запрещать встраивание в iframe, не угадывать MIME-тип и т. д. В Spring Security настраиваются через `http.headers(...)`.
...
`CSP` -- `HTTP`-заголовок, в котором сервер задаёт белый список источников, откуда странице разрешено грузить скрипты, стили, картинки и т. д. Это одна из самых мощных защит от `XSS`: даже если злоумышленнику удалось внедрить тег `<script>`, браузер не выполнит его, если источник не разрешён политикой. CSP работает как второй рубеж -- на случай, если экранирование где-то пропущено. Внедряют поэтапно через `Report-Only`, чтобы не сломать легитимный контент.
...
Безопасность API -- это не одна мера, а цепочка проверок, через которые проходит каждый запрос: транспорт (`TLS`) → ограничение частоты (`rate limiting`) → аутентификация (кто) → авторизация (что можно) → валидация ввода → бизнес-логика → аудит. Каждое звено независимо, и пропуск любого -- потенциальная дыра. Ниже -- эта цепочка и её сборка в одном `SecurityFilterChain`.
...
`Threat Modeling` -- систематический разбор «что и как может сломать злоумышленник» на этапе проектирования, до написания кода. Цель -- найти угрозы, когда исправление стоит дёшево (правка дизайна), а не дорого (инцидент в продакшене). Отвечает на четыре вопроса: что строим (DFD) → что может пойти не так (`STRIDE`) → что с этим делать (меры) → насколько хорошо защитились (валидация).

### Методология `STRIDE`

`STRIDE` -- чеклист из шести категорий угроз; по каждому элементу системы проходят все шесть и проверяют применимость.
...
`Secrets Management` -- централизованное управление секретами (пароли, API-ключи, токены, сертификаты) с контролем доступа, аудитом и ротацией. Базовое правило: секреты **никогда** не лежат в коде или репозитории -- git хранит историю вечно, и один закоммиченный ключ скомпрометирован навсегда, даже если его потом удалили. Специализированные хранилища (`Vault`, `KMS`) дают то, чего не дадут переменные окружения: кто и когда читал секрет (аудит) и автоматическую смену по расписанию (ротация).
...
`Security Code Review` -- проверка кода именно на уязвимости (а не на стиль/логику), по структурированному чеклисту типовых ошибок. Ценность в том, что человек видит контекст и намерение там, где автоматический SAST пасует: например, что данные «доверенные» только формально. Поэтому ревью и автоматику комбинируют -- инструменты отсекают рутину, человек ловит логические дыры. Ниже -- чеклист, который проходят на каждом security-sensitive PR.
...
Это встраивание автоматических проверок безопасности в pipeline так, чтобы небезопасный код не доезжал до прода («shift left» -- ловим уязвимость как можно раньше). Реализуется через security gates: на каждом шаге свой тип анализа (`SAST` → `SCA` → скан образа → `DAST`), и критичная находка останавливает сборку. Это превращает безопасность из разовой проверки в непрерывный процесс на каждый коммит.
...
Контейнер -- не граница безопасности сам по себе: процесс внутри делит ядро с хостом, поэтому уязвимость в нём или в образе может стать точкой входа на узел. Защита идёт по двум осям: **минимизировать attack surface** (тонкий образ, non-root, read-only FS, лимиты ресурсов) и **проверять то, что внутри** (скан образов на CVE, подписанные образы). Ниже -- как это выглядит в `Dockerfile`.
...
`Security Misconfiguration` -- уязвимость не в коде, а в настройках: дефолтные пароли, открытые отладочные эндпоинты, stack trace в ответе, лишние включённые фичи. Одна из самых частых причин взломов (`OWASP A05`), потому что код может быть безупречным, но небезопасная конфигурация сводит защиту на нет. Главный принцип борьбы -- **secure defaults**: всё лишнее выключено, прод сконфигурирован отдельно от dev и воспроизводимо через IaC.
...
Загрузка файлов опасна тем, что пользователь кладёт на сервер контент, которым потом управляет: вредоносный скрипт (RCE при исполнении), огромный файл (DoS), имя с `../` (path traversal -- перезапись чужих файлов), фейковый тип (картинка, на деле -- `.jsp`). Защита многослойна: проверять размер, тип **по содержимому** (magic bytes, а не по расширению, которое легко подделать), переименовывать в случайное имя и сохранять **вне webroot**, чтобы файл нельзя было выполнить запросом. Ниже -- все шаги по порядку.
...
У логирования две стороны безопасности. Первая -- логи **как защита**: без аудита событий аутентификации и доступа к данным нечем расследовать инцидент (`OWASP A09` -- Logging Failures). Вторая -- логи **как угроза**: пароли, токены, PII и номера карт в логах превращают сам лог в утечку, ведь к нему имеет доступ куда больше людей и систем, чем к боевой БД. Правило простое: логировать события, но не секреты.
...
`Security by Design` -- подход, при котором безопасность закладывается в архитектуру с самого начала, а не «прикручивается» перед релизом. Причина прагматична: исправить уязвимость на этапе дизайна стоит копейки, в продакшене -- порядки дороже (рефакторинг + инцидент). Это не отдельная фаза, а набор принципов, которым следуют во всех решениях.
...
В монолите вызовы между компонентами -- это вызовы методов внутри процесса; в микросервисах они становятся сетевыми, и каждый такой вызов нужно шифровать, аутентифицировать и авторизовать. Отсюда смещение фокуса: безопасность периметра уступает место безопасности каждого взаимодействия. Ключевые средства -- единая точка входа (`API Gateway`), `mTLS` между сервисами, проброс identity пользователя через `JWT` и централизованное управление идентификацией.
