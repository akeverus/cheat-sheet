...
(!) Как использовать `try-catch` как выражение?
...
(!) Что такое тип `Nothing` и где он используется?
...
Чем `Nothing` отличается от `Unit` и `Void`?
...
(!) Что такое `runCatching` и как работает `Result`?
...
Какие операторы доступны у `Result`?
...
Почему `Result` нельзя использовать как тип возврата публичной функции?
...
## Q34. `runCatching` — best practices и антипаттерны
