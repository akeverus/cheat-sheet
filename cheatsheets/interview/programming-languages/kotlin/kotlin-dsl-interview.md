...
Как при этом меняется неявный `this` от блока к блоку:

- `config { }` — receiver = `Config`, то есть `this = Config()`. Внутри доступны:
  - `host = 'localhost'`;
  - `port = 8080`;
  - `ssl { }` — вложенный блок, в котором receiver = `SslConfig`, то есть `this = SslConfig()`. Внутри него доступны:
    - `enabled = true`;
    - `certPath = '/certs'`.
...
Дерево узлов, которое собирает такой builder, выглядит так:

- `html { }` создаёт корневой узел `HTML`. У него два дочерних блока:
  - `head { }` → узел `Head`, внутри которого:
    - `title { }` → узел `Title`;
  - `body { }` → узел `Body`, внутри которого:
    - `div { }` → узел `Div`, внутри которого:
      - `+text` → `TextElement`;
    - `p { }` → узел `P`.
...
Разница в видимости receiver-ов по уровням вложенности:

- **Без `@DslMarker`** — `this` накапливает все внешние receiver-ы:
  - `html { }` → `this = HTML`;
  - вложенный `body { }` → `this = Body + HTML`;
  - ещё глубже `div { }` → `this = Div + Body + HTML`;
  - поэтому внутри `div` вызов `head { }` доступен — за счёт утечки внешнего `HTML`.
- **С `@DslMarker`** — виден только ближайший receiver:
  - `html { }` → `this = HTML`;
  - вложенный `body { }` → `this = Body` (только);
  - ещё глубже `div { }` → `this = Div` (только);
  - поэтому внутри `div` вызов `head { }` даёт ❌ ошибку компиляции.
...
Дерево маршрутов из примера, где вложенность блоков складывается в путь:

- `routing { }` (контекст `Route`) содержит:
  - `route('/api') { }` (контекст `Route`), внутри которого:
    - `route('/users') { }` (контекст `Route`), внутри которого три обработчика:
      - `get { }` → `GET /api/users`;
...
Что происходит при вызове `html { }` в каждом случае:

- **Без `inline`** — цепочка из четырёх шагов:
  1. вызов `html { }`;
  2. создание `Function`-объекта (аллокация lambda);
  3. `HTML().apply(fn)`;
  4. `fn.invoke(html)`.
- **С `inline`** — два шага, без аллокации и без `invoke`:
  1. вызов `html { }`;
  2. `HTML()`, после чего код блока подставляется прямо здесь.
...
Решение «нужен ли DSL?» можно пройти как дерево вопросов:

- **Структура иерархическая?**
  - **Да** → следующий вопрос: **используется многократно?**
    - **Да** (многократное использование) → следующий вопрос: **команда знает Kotlin?**
      - **Да** (команда знает Kotlin) → **создавать DSL** ✅;
      - **Нет** (команда на Java) → **Fluent API** ✅;
    - **Нет** (одноразовый код) → **обычный API / именованные аргументы** ✅.
  - **Нет** (плоская конфигурация) → **обычный API / именованные аргументы** ✅.
