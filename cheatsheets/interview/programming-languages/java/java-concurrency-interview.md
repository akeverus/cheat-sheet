...
> [!mcq]
> - [ ] Потоки одного процесса имеют изолированные адресные пространства, поэтому изменения в памяти одного потока не видны другому | На самом деле потоки одного процесса разделяют общую кучу и статические поля — именно это делает межпоточное взаимодействие быстрым, но требует синхронизации.
> - [x] Потоки одного процесса разделяют общую кучу, но каждый имеет собственный стек вызовов и счётчик команд | Это точная модель: heap и static-поля общие, а стек — изолированный для каждого потока. Благодаря этому потоки могут обмениваться данными через общую память без IPC.
> - [ ] Переключение контекста между потоками одного процесса столь же дорого, как и между разными процессами | Переключение между потоками одного процесса значительно дешевле: не требует сброса TLB и смены адресного пространства, поскольку они уже находятся в одном пространстве.
> - [ ] Каждый поток в процессе имеет свой набор файловых дескрипторов | Файловые дескрипторы принадлежат процессу и разделяются всеми его потоками. Собственный у потока только стек, счётчик команд и регистры.
...
> [!mcq]
> - [ ] Состояние `BLOCKED` возникает, когда поток вызывает `Object.wait()` без таймаута | `Object.wait()` переводит поток в состояние `WAITING`, а не `BLOCKED`. Состояние `BLOCKED` специфично для ожидания захвата монитора при входе в `synchronized` блок.
> - [x] Состояние `WAITING` возникает при вызове `Object.wait()`, `Thread.join()` или `LockSupport.park()` | Все три метода блокируют поток бессрочно до внешнего события: `notify()`/`notifyAll()`, завершения ожидаемого потока или `LockSupport.unpark()`. Это отличает `WAITING` от `TIMED_WAITING`, где есть таймаут.
> - [ ] Состояние `RUNNABLE` означает, что поток прямо сейчас выполняется на CPU | `RUNNABLE` охватывает как активное выполнение на CPU, так и состояние готовности — поток ждёт выделения процессорного времени. JVM не разделяет эти два подсостояния.
> - [ ] Из состояния `TERMINATED` поток может вернуться в `RUNNABLE` при повторном вызове `start()` | Состояние `TERMINATED` необратимо. Повторный вызов `start()` на завершённом потоке бросает `IllegalThreadStateException`.
...
> - [ ] `setDaemon(true)` можно вызвать в любой момент жизненного цикла потока | `setDaemon(true)` необходимо вызвать строго до `start()`. Вызов после запуска потока бросает `IllegalThreadStateException`.
> - [x] Daemon thread не препятствует завершению JVM — когда завершаются все не-daemon потоки, JVM останавливает daemon потоки и завершается | Именно такова семантика daemon потоков: они обслуживают другие потоки, но не удерживают JVM от выхода. Типичные примеры — GC, финализатор, фоновый мониторинг.
...
> [!mcq]
> - [ ] `sleep()` освобождает монитор объекта, на котором поток синхронизирован | `sleep()` не освобождает монитор. Поток продолжает удерживать все захваченные блокировки во время сна. Освобождает монитор только `Object.wait()`.
...
> - [x] `volatile` гарантирует видимость: запись в `volatile` поле happens-before последующего чтения этого поля другим потоком | Это точная гарантия JMM для `volatile`. Запись немедленно публикуется в главную память, а чтение всегда берёт актуальное значение, минуя локальный кэш потока.
...
> [!mcq]
> - [ ] `final` поля видны другим потокам только после явной синхронизации, например через `synchronized` блок | JMM специально гарантирует видимость `final` полей без дополнительной синхронизации — при условии что ссылка на объект не утекла из конструктора до его завершения.
...
> - [ ] Запись в `volatile` поле не устанавливает `happens-before` — для этого нужен `synchronized` | Это неверно. JMM явно задаёт правило: запись в `volatile` переменную happens-before последующего чтения этой же переменной. Это фундамент безопасной публикации через `volatile`.
> - [x] Транзитивность `happens-before` позволяет связывать гарантии через промежуточные события: если A hb B и B hb C, то действия A видны потоку, выполняющему C | Транзитивность — одно из ключевых свойств `happens-before`. Благодаря ей можно выстраивать цепочки: например, запись до `volatile` write видна потоку, читающему это `volatile` поле.
...
> [!mcq]
> - [ ] `synchronized` метод использует отдельный специальный объект-монитор, не связанный с `this` | `synchronized` метод экземпляра использует именно `this` как монитор. Статический `synchronized` метод использует объект `ClassName.class`. Никакого отдельного монитора не создаётся.
> - [ ] Монитор `synchronized` блока нельзя захватить повторно тем же потоком | `synchronized` в Java реентерабелен: поток, уже владеющий монитором, может войти в другой `synchronized` блок с тем же монитором без блокировки. Это предотвращает самоблокировку при рекурсии.
...
> - [ ] `synchronized` блок не может использовать `this` как монитор — только отдельный объект-замок | `synchronized(this)` — совершенно допустимая конструкция и эквивалентна `synchronized` методу экземпляра. Блок просто даёт возможность выбирать произвольный объект в качестве монитора.
> - [ ] `synchronized` метод и `synchronized` блок с монитором `this` работают по разным механизмам JVM | Оба используют один и тот же механизм: захват монитора (`monitorenter`/`monitorexit` в байткоде). Разница только в том, что блок даёт явный контроль над монитором и границами критической секции.
...
> [!mcq]
> - [ ] Два потока, вызывающие `synchronized` метод экземпляра на одном объекте, не блокируют друг друга | Оба потока конкурируют за один монитор `this`, поэтому они блокируют друг друга. Только если потоки обращаются к разным экземплярам — блокировки независимы.
> - [ ] Статический `synchronized` метод использует монитор `this` | Статический метод не привязан к экземпляру и использует монитор класса `ClassName.class`. Это единственный монитор для всех экземпляров класса, поэтому блокировка распространяется глобально.
...
> [!mcq]
> - [ ] `wait()` можно вызывать вне `synchronized` блока — он сам захватит монитор | `wait()` обязательно должен вызываться внутри `synchronized` блока, иначе бросается `IllegalMonitorStateException`. Это требование JVM для корректной работы монитора.
...
> [!mcq]
> - [ ] `AtomicInteger` использует `synchronized` внутри, просто скрывая его за удобным API | `AtomicInteger` использует CAS через `Unsafe.compareAndSwapInt()` — это аппаратная инструкция без блокировки. Именно отсутствие `synchronized` делает его быстрее при умеренной конкуренции.
...
> - [ ] `ReentrantLock` не поддерживает реентерабельность — повторный захват тем же потоком приведёт к deadlock | `ReentrantLock` полностью реентерабелен, отсюда и название. Поток может захватить его несколько раз подряд, но обязан столько же раз вызвать `unlock()`.
...
> - [ ] `StampedLock` реентерабелен так же, как `ReentrantReadWriteLock` | `StampedLock` не реентерабелен. Повторная попытка захвата блокировки из того же потока приведёт к deadlock. Это один из главных практических недостатков `StampedLock`.
> - [ ] `ReentrantReadWriteLock` поддерживает апгрейд блокировки чтения до записи | `ReentrantReadWriteLock` не поддерживает прямой апгрейд read→write. `StampedLock` поддерживает его через `tryConvertToWriteLock(stamp)`.
...
> [!mcq]
> - [ ] `Condition` является частью класса `Object` и доступен без `Lock` | `Condition` — это интерфейс из `java.util.concurrent.locks`, создаваемый через `lock.newCondition()`. Аналогом в мире `synchronized` являются `wait()`/`notify()` класса `Object`.
> - [ ] На одном `Lock` можно создать только одно `Condition`, как это ограничено в `synchronized` | Это главное преимущество `Condition` над `synchronized`: на одном `ReentrantLock` можно создать несколько `Condition` для независимых условий, например `notFull` и `notEmpty` в очереди.
> - [ ] `Condition.await()` не освобождает связанный `Lock`, поэтому другие потоки могут быть заблокированы | `Condition.await()` атомарно освобождает `Lock` и переводит поток в `WAITING` — аналогично `Object.wait()` для `synchronized`. Без освобождения блокировки другие потоки не могли бы изменить условие.
...
> [!mcq]
> - [ ] Deadlock требует ровно двух потоков и двух ресурсов | Deadlock может возникнуть между любым числом потоков и ресурсов — достаточно циклической зависимости. Три потока A→B→C→A образуют deadlock так же, как и классический двухпоточный сценарий.
> - [ ] Для возникновения deadlock достаточно одного из четырёх условий | Все четыре условия (Mutual Exclusion, Hold and Wait, No Preemption, Circular Wait) должны выполняться одновременно. Нарушение любого одного из них предотвращает deadlock.
...
> - [ ] Стандартный `synchronized` с fairness policy решает проблему Starvation | `synchronized` не поддерживает fairness — порядок пробуждения неопределён. Для предотвращения Starvation нужен `new ReentrantLock(true)`, который гарантирует FIFO-порядок предоставления блокировки.
...
> [!mcq]
> - [ ] `ThreadLocal` создаёт отдельный поток для каждой переменной, поэтому называется ThreadLocal | `ThreadLocal` не создаёт потоков. Это переменная, хранящая изолированную копию значения для каждого потока. Внутри реализована через `ThreadLocalMap`, хранящуюся в объекте самого `Thread`.
...
> - [x] `InheritableThreadLocal` передаёт значение дочернему потоку в момент его создания через `new Thread()`, тогда как `ThreadLocal` изолирует данные в каждом потоке независимо | Это точное описание механизма. JVM копирует значения `InheritableThreadLocal` при создании нового потока. Для пулов потоков это не работает, поскольку потоки создаются заранее.
> - [ ] `InheritableThreadLocal` позволяет дочернему потоку изменять значение и эти изменения видны родительскому потоку | После передачи значения дочернему потоку связь разрывается. Дочерний поток получает независимую копию, и изменения в ней не видны родительскому потоку.
...
> - [ ] `Executor.execute(Runnable)` возвращает `Future` для получения результата задачи | `Executor.execute()` возвращает `void` — это минимальный интерфейс без поддержки результата. Для получения `Future` нужен `ExecutorService.submit()`.
...
> - [ ] После вызова `shutdown()` можно продолжать добавлять новые задачи через `submit()` | После `shutdown()` любой вызов `submit()` или `execute()` бросает `RejectedExecutionException`. Пул переходит в режим завершения и не принимает новых задач.
...
> [!mcq]
> - [ ] `execute(Runnable)` возвращает `Future` для отслеживания статуса задачи | `execute()` возвращает `void` — нет способа получить результат или отследить завершение. Для этого нужен `submit()`, который возвращает `Future`.
...
> [!mcq]
> - [ ] `whenComplete()` может изменить результат `CompletableFuture` при обработке ошибки | `whenComplete()` вызывается для побочных эффектов (логирование, метрики) и не может изменить результат. Для изменения результата при ошибке нужен `exceptionally()` или `handle()`.
...
> - [x] `Semaphore` ограничивает количество потоков, одновременно обращающихся к ресурсу, через счётчик permits: `acquire()` уменьшает счётчик, `release()` увеличивает | Это точная семантика `Semaphore`. При нулевом числе permits `acquire()` блокируется до тех пор, пока другой поток не вызовет `release()`.
...
> - [x] `ConcurrentHashMap` в Java 8+ использует CAS для вставки в пустой бакет и `synchronized` на первом узле бакета при коллизии, что обеспечивает высокую производительность при конкурентном доступе | Это архитектура `ConcurrentHashMap` начиная с Java 8. Сегментная блокировка из Java 7 заменена более гранулярным подходом: CAS + per-bucket синхронизация + красно-чёрные деревья при длинных цепочках.
> - [ ] `ConcurrentLinkedQueue` использует `synchronized` блокировку на всю очередь для обеспечения потокобезопасности | `ConcurrentLinkedQueue` — lock-free реализация на основе CAS. Она не использует блокировки вообще, что обеспечивает высокий throughput при конкурентных операциях `offer()` и `poll()`.
...
> - [x] Pinning происходит когда virtual thread выполняет блокирующую операцию внутри `synchronized` блока или JNI-метода, удерживая carrier thread заблокированным | Это два официальных сценария pinning. JVM не может размонтировать virtual thread в этих случаях, поэтому carrier thread блокируется вместе с ним, что снижает параллелизм.
...
> - [ ] `ScopedValue` медленнее `ThreadLocal` из-за дополнительных проверок scope | `ScopedValue` быстрее `ThreadLocal`, так как использует stack-based lookup вместо `ThreadLocalMap` (HashMap-подобная структура). Это особенно заметно при частых обращениях в горячих путях выполнения.
...
> [!mcq]
> - [ ] `StampedLock` реентерабелен так же, как `ReentrantReadWriteLock` | `StampedLock` не реентерабелен — это одно из его главных ограничений. Если поток попытается повторно захватить блокировку, которую уже держит, произойдёт deadlock.
