...
> [!mcq]
> - [ ] Состояние `BLOCKED` возникает, когда поток вызывает `Object.wait()` без таймаута | `Object.wait()` переводит поток в состояние `WAITING`, а не `BLOCKED`. Состояние `BLOCKED` специфично для ожидания захвата монитора при входе в `synchronized` блок. Это частая ошибка при неправильном понимании механизма Java.
