...
> - [ ] Каждый поток в процессе имеет свой набор файловых дескрипторов | Файловые дескрипторы принадлежат процессу и разделяются всеми его потоками. Собственный у потока только стек, счётчик команд и регистры. Частая ошибка в реальном коде.
...
> - [ ] Из состояния `TERMINATED` поток может вернуться в `RUNNABLE` при повторном вызове `start()` | Состояние `TERMINATED` необратимо. Повторный вызов `start()` на завершённом потоке бросает `IllegalThreadStateException`. Это антипаттерн или неправильный выбор в production.
