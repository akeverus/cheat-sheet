...
> - [x] Информация о generic-типе в общем случае недоступна в runtime из-за type erasure, но сохраняется в метаданных класса, когда тип зафиксирован в сигнатуре при наследовании, что позволяет извлечь его через рефлексию с помощью `getGenericSuperclass()`. Именно этим пользуется паттерн Super Type Token: анонимный подкласс `new TypeReference<List<String>>(){}` фиксирует `List<String>` в метаданных, и его можно прочитать через `ParameterizedType`.
