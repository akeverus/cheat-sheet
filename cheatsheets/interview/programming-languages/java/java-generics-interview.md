...
> - [x] Generic Type Parameter — это переменная типа, объявляемая при описании класса, интерфейса или метода и заменяемая конкретным типом при использовании. Именно так работают дженерики: `T` в `Consumer<T>` при реализации заменяется на `Integer`, `String` и т.д. Это обеспечивает типобезопасность на этапе компиляции.
