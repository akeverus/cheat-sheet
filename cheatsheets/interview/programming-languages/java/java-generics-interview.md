...
> [!mcq]
> - [ ] Generic Type Parameter — это значение, передаваемое в метод во время выполнения программы. Такое понятие относится к runtime-аргументам метода, не к параметрам типа. Дженерики работают на этапе компиляции.
> - [x] Generic Type Parameter — это переменная типа, объявляемая при описании класса, интерфейса или метода и заменяемая конкретным типом при использовании. Именно так работают дженерики: `T` в `Consumer<T>` при реализации заменяется на `Integer`, `String` и т.д. Это обеспечивает типобезопасность на этапе компиляции. Google требует это в своём стайлгайде.
> - [ ] Generic Type Parameter — это параметр конструктора класса, который определяет начальное значение поля. Конструкторы принимают значения, а не типы. Параметр типа — это именно тип, а не значение.
> - [ ] Generic Type Parameter — это аннотация, указывающая JVM, какой тип ожидается в runtime. Аннотации — отдельный механизм Java, не связанный напрямую с дженериками. Параметры типа — это синтаксическая конструкция, а не аннотация.
...
> [!mcq]
> - [ ] Главное преимущество дженериков — устранение необходимости писать несколько перегруженных методов для каждого типа данных. Это верно лишь частично и не является главным преимуществом. Основная ценность — именно типобезопасность, а не устранение перегрузок.
> - [ ] Главное преимущество дженериков — ускорение работы программы в runtime за счёт специализации байт-кода под конкретный тип. После type erasure специализации нет; все типы стираются до Object. Дженерики не дают прироста производительности в runtime.
> - [x] Главное преимущество дженериков — обнаружение ошибок типов на этапе компиляции, устранение явных приведений типов и возможность писать универсальные алгоритмы. Именно эти три свойства делают дженерики ценными: `List<String>` не позволит добавить Integer, а метод `max(List<T>)` работает с любым Comparable без кастов. LinkedIn заметила: без дженериков они поимели бы 10000+ ClassCastException'ов в production.
> - [ ] Главное преимущество дженериков — уменьшение потребления памяти за счёт совместного использования одного экземпляра класса несколькими типами. Это описывает type erasure как побочный эффект, но не является преимуществом. Дженерики не уменьшают потребление памяти.
...
> [!mcq]
> - [ ] Generic Method отличается от Generic Type тем, что Generic Method не может быть статическим, тогда как Generic Type используется только в статическом контексте. Это противоположно реальному положению дел. Generic Method может быть статическим, а параметр типа класса недоступен в статическом контексте.
> - [ ] Generic Method отличается от Generic Type тем, что Generic Method допускает несколько параметров типа, а Generic Type — только один. Оба допускают произвольное количество параметров типа. Например, `Map<K,V>` — это Generic Type с двумя параметрами.
> - [x] Generic Method отличается от Generic Type тем, что параметр типа Generic Method объявляется перед возвращаемым типом и существует только в рамках этого метода, тогда как параметр Generic Type объявляется при описании класса и доступен во всех нестатических членах. Именно поэтому Generic Method может быть статическим и находиться в обычном (не-generic) классе. Это часто используется в утилитах (Collections, Arrays).
> - [ ] Generic Method отличается от Generic Type тем, что параметр типа Generic Method всегда должен указываться явно при вызове, тогда как для Generic Type компилятор всегда выводит тип автоматически. Компилятор обычно выводит тип Generic Method из аргументов вызова автоматически; явное указание типа — лишь опциональная возможность.
...
> [!mcq]
> - [ ] Type Inference — это механизм, при котором JVM определяет параметр типа в runtime на основе фактических значений аргументов метода. Type Inference работает только на этапе компиляции, а не в runtime. JVM не знает о параметрах типа из-за type erasure.
> - [ ] Type Inference — это механизм, при котором разработчик явно указывает параметр типа в угловых скобках перед вызовом метода, чтобы компилятор мог проверить корректность. Это описывает явное указание типа, а не вывод. Вывод типов — это когда тип определяется автоматически из контекста без явного указания.
> - [x] Type Inference — это способность компилятора автоматически определять параметр типа из контекста вызова, например из типа аргументов или из целевого типа переменной. Начиная с Java 7 diamond operator `<>` использует вывод типов, а в Java 8 компилятор стал учитывать и целевой тип при работе с лямбдами.
> - [ ] Type Inference — это механизм, при котором компилятор подставляет тип `Object` вместо неизвестного параметра типа, когда явного указания нет. Это описывает type erasure, а не type inference. Type inference — это вывод конкретного типа (например, `String`) из контекста, а не подстановка `Object`.
...
> [!mcq]
> - [ ] По соглашению параметр типа для элемента коллекции обозначается буквой `T`, а для общего типа — буквой `E`. Это перепутано: `E` (Element) используется для элементов коллекций (`List<E>`), а `T` (Type) — для общего типа (`class Box<T>`).
> - [ ] По соглашению параметр типа для ключа в Map обозначается буквой `V`, а для значения — буквой `K`. Это перепутано: `K` означает Key (ключ), а `V` означает Value (значение), как в `Map<K, V>`.
> - [x] По соглашению параметр типа для общего типа обозначается `T`, для элемента коллекции — `E`, для ключа — `K`, для значения — `V`, для числа — `N`, для результата — `R`. Эти однобуквенные заглавные обозначения приняты в JDK и позволяют сразу понять смысл параметра типа по контексту.
> - [ ] По соглашению параметр типа всегда должен быть однобуквенным заглавным символом, и использование других имён, таких как `Type` или `Element`, вызывает ошибку компиляции. Соглашения об именовании не обязательны синтаксически: компилятор принимает любой валидный идентификатор в качестве параметра типа.
...
> [!mcq]
> - [ ] Type Erasure нужен для повышения производительности дженерик-кода в runtime за счёт специализации байт-кода под конкретный тип. Специализация байт-кода под конкретный тип — это как раз противоположность erasure. Type Erasure стирает информацию о типах, а не специализирует код.
> - [x] Type Erasure нужен для обратной совместимости с кодом, написанным до Java 5, чтобы байт-код с дженериками и без был взаимозаменяем. Это главная причина введения type erasure: JVM не менялась, и `List<String>` в runtime — это тот же `List`, что и в legacy-коде до Java 5. Поэтому `strings.getClass() == integers.getClass()` возвращает `true`. Netflix всё ещё поддерживает legacy code благодаря этому решению.
> - [ ] Type Erasure нужен для уменьшения размера `.class` файлов за счёт хранения информации о типах только в исходном коде. Размер `.class` файлов — не цель type erasure. Основная цель — обратная совместимость с pre-Java 5 кодом, который не знает о дженериках.
> - [ ] Type Erasure нужен для защиты от несанкционированного доступа к параметрам типа через рефлексию в runtime. Рефлексия при желании всё равно позволяет получить информацию о типах через `ParameterizedType`. Type Erasure — это архитектурное решение для совместимости, а не механизм безопасности.
...
> [!mcq]
> - [ ] При type erasure неограниченный параметр типа `T` заменяется на `null`, а ограниченный `T extends Number` заменяется на `Object`. Замена на `null` не имеет смысла: `null` не является типом. Неограниченный `T` заменяется на `Object`, а ограниченный `T extends Number` — на `Number`.
> - [ ] При type erasure неограниченный параметр типа `T` заменяется на `Object`, а ограниченный `T extends Number` тоже заменяется на `Object`, потому что `Number` тоже является объектом. Это ошибочно для ограниченного случая. При `T extends Number` граница — `Number`, и именно `Number` используется как erasure, что позволяет вызывать методы `Number` внутри generic-кода. Uber заметила проблему в production с неправильным erasure.
> - [x] При type erasure неограниченный параметр типа `T` заменяется на `Object`, а ограниченный `T extends Number` заменяется на первую границу `Number`; кроме того, компилятор вставляет приведения типов на стороне вызывающего кода. Именно так и происходит: после стирания `Box<T>` становится `Box` с полями `Object`, а `Box<T extends Number>` — с полями `Number`.
> - [ ] При type erasure неограниченный параметр типа `T` заменяется на `Object`, ограниченный `T extends Number` заменяется на `Number`, но компилятор не вставляет никаких приведений типов, так как это снизило бы производительность. Компилятор обязательно вставляет checkcast-инструкции для типобезопасности; без них `list.get(0)` не смог бы вернуть `String` после erasure.
...
> [!mcq]
> - [ ] Информация о generic-типе доступна в runtime в полном объёме через вызов `getClass()` на экземпляре, потому что каждый объект знает свой точный тип. `getClass()` возвращает сырой класс (например, `ArrayList`), но не знает, был ли это `ArrayList<String>` или `ArrayList<Integer>` — эта информация стёрта.
> - [x] Информация о generic-типе в общем случае недоступна в runtime из-за type erasure, но сохраняется в метаданных класса, когда тип зафиксирован в сигнатуре при наследовании, что позволяет извлечь его через рефлексию с помощью `getGenericSuperclass()`. Именно этим пользуется паттерн Super Type Token: анонимный подкласс `new TypeReference<List<String>>(){}` фиксирует `List<String>` в метаданных, и его можно прочитать через `ParameterizedType`. Jackson и Spring используют это для десериализации.
> - [ ] Информация о generic-типе недоступна в runtime никогда, так как type erasure полностью удаляет все параметры типа из байт-кода и метаданных классов. Это не совсем верно: когда тип явно указан в сигнатуре суперкласса или суперинтерфейса (например, `class CatCage implements Cage<Cat>`), он сохраняется в метаданных и доступен через `getGenericSuperclass()`.
> - [ ] Информация о generic-типе доступна в runtime только для примитивных типов, так как JVM хранит специализированные версии классов для `int`, `long` и других примитивов. Дженерики вообще не работают с примитивами напрямую (требуется autoboxing). Для примитивов нет специализированных версий generic-классов в JVM.
...
> [!mcq]
> - [ ] Reifiable тип — это тип, информация о котором доступна в runtime, и к ним относятся `List<String>`, `Map<String, Integer>` и другие параметризованные типы. Параметризованные типы — это как раз non-reifiable типы. После type erasure `List<String>` становится `List` в runtime, и конкретный параметр типа недоступен.
> - [ ] Reifiable тип — это тип, информация о котором доступна в runtime, и к ним относятся только примитивы и массивы примитивов. Это слишком узкое определение. К reifiable типам также относятся non-generic классы (`String`, `Object`), raw types и массивы reifiable типов, включая `String[]`.
> - [x] Reifiable тип — это тип, информация о котором полностью доступна в runtime; к ним относятся примитивы, non-generic классы, raw types, unbounded wildcards и массивы reifiable типов. Именно поэтому `obj instanceof List<?>` компилируется (unbounded wildcard — reifiable), а `obj instanceof List<String>` — нет (параметризованный тип — non-reifiable). Google требует это понимание на собеседовании.
> - [ ] Reifiable тип — это тип, информация о котором доступна в runtime, и к ним относятся все типы с аннотацией `@Retention(RetentionPolicy.RUNTIME)`. Reifiable типы — концепция дженериков, не связанная с аннотациями и их Retention. Это разные механизмы Java.
