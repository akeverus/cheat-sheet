...
- [Q21. (!) Как правильно проектировать иерархию исключений?](#q21--как-правильно-проектировать-иерархию-исключений)
...
**Продвинутые темы**
- [Q35. Как устроена цепочка исключений: `initCause()`, `getCause()`, `addSuppressed()`?](#q35-как-устроена-цепочка-исключений-initcause-getcause-addsuppressed)
- [Q36. Что такое multi-catch (Java 7): синтаксис и ограничения?](#q36-что-такое-multi-catch-java-7-синтаксис-и-ограничения)
- [Q37. Как работает try-with-resources с `AutoCloseable` и в каком порядке закрываются ресурсы?](#q37-как-работает-try-with-resources-с-autocloseable-и-в-каком-порядке-закрываются-ресурсы)
- [Q38. Как правильно создавать кастомные исключения и зачем нужен `serialVersionUID`?](#q38-как-правильно-создавать-кастомные-исключения-и-зачем-нужен-serialversionuid)
- [Q39. Как обрабатывать checked-исключения внутри лямбды?](#q39-как-обрабатывать-checked-исключения-внутри-лямбды)
- [Q40. Как правильно логировать исключения и что включать в сообщение?](#q40-как-правильно-логировать-исключения-и-что-включать-в-сообщение)
- [Q41. Сколько стоит создание исключения и почему `fillInStackTrace()` дорогой?](#q41-сколько-стоит-создание-исключения-и-почему-fillinstacktrace-дорогой)
- [Q42. Как организовать глобальную обработку исключений в Spring (`@ControllerAdvice`, `ProblemDetail`)?](#q42-как-организовать-глобальную-обработку-исключений-в-spring-controlleradvice-problemdetail)
...
Несмотря на похожие имена, это разные вещи: `throw` (действие) выбрасывает исключение прямо сейчас, а `throws` (декларация) лишь объявляет в сигнатуре, что метод может его выбросить.
...
Код, способный выбросить исключение, оборачивают в `try`, ошибку ловят в `catch`, а гарантированную очистку выполняют в `finally`. `try` локализует опасный участок, `catch` реагирует на конкретный тип ошибки, `finally` выполняется всегда — независимо от того, было исключение или нет:
...
Цепочка исключений (`Exception Chaining`) — оборачивание низкоуровневого исключения в более подходящее по уровню абстракции с сохранением исходного как причины. Так верхний слой получает осмысленное доменное исключение, но не теряет техническую первопричину. Оригинальное исключение передаётся как `cause`:
...
Исключения отделяют «что делаем» от «что делать при ошибке» и не дают ошибке потеряться. Конкретные преимущества:
...
Это методы класса `Throwable` для работы с подавленными (suppressed) исключениями — побочными ошибками, которые возникли уже на фоне основной и которые нельзя бросить, не потеряв главную:

- `addSuppressed(Throwable)` — прикрепляет подавленное исключение к текущему
- `getSuppressed()` — возвращает массив `Throwable[]` всех подавленных исключений

JVM вызывает их автоматически в `try-with-resources` (когда `close()` бросает исключение поверх основного). В своём коде они нужны, когда вы вручную освобождаете несколько ресурсов и не хотите потерять ни одной ошибки закрытия:
...
- Суффикс `Exception` в имени класса — узнаваемость с первого взгляда
- Наследовать от наиболее подходящего типа, а не просто от `Exception`
- Обязательно конструктор с `Throwable cause` для цепочки — иначе при оборачивании потеряется первопричина
...
- Переиспользовать стандартные исключения (`IllegalArgumentException`, `IllegalStateException`) где уместно, а не плодить свои на каждый чих

## Q20. Когда использовать `RuntimeException` vs checked `Exception`?

Эмпирическое правило: спросите себя, **может ли и обязан ли вызывающий код осмысленно отреагировать на ошибку прямо здесь**. Если да (повторить, взять fallback) — checked, заставляющий обработать. Если нет (это баг или ситуация, которую на месте всё равно не починить) — unchecked.
...
## Q21. (!) Как правильно проектировать иерархию исключений?

Хорошая иерархия исключений отражает домен, а не технические детали реализации, и позволяет вызывающему коду ловить ошибки на нужном уровне обобщения. Базовые правила:

1. **Наследоваться от подходящего базового типа** — не от `Exception` напрямую, если есть более точный. Это даёт вызывающему коду возможность ловить как конкретный подтип, так и общий предок одним `catch`.
2. **Включать контекст** — сообщение, cause, дополнительные поля (ID ресурса, код ошибки). Чем больше контекста несёт исключение, тем меньше времени уходит на разбор инцидента.
3. **Документировать `@throws`** в Javadoc — чтобы вызывающий код знал, что и при каких условиях может прилететь.
...
5. **Не создавать исключение на каждую ошибку** — группировать по смыслу, иначе иерархия раздувается и теряет ценность
6. **Именование**: `<Причина>Exception` — `OrderNotFoundException`, `PaymentDeclinedException`

Типичный шаблон — абстрактный базовый класс домена и конкретные подтипы под него. Тогда вызывающий код выбирает уровень детализации сам: ловит конкретный подтип, если умеет его обработать отдельно, либо общий предок — для единообразной реакции на любую ошибку домена.
...
`catch (Throwable t)` ловит абсолютно всё, что может быть выброшено — и `Exception`, и `Error`, поскольку оба наследуют `Throwable`. Это максимально широкий перехват, поэтому применять его нужно только на границах системы, а не в бизнес-логике:
...
`Thread.UncaughtExceptionHandler` — это «последняя линия обороны» для исключения, которое не поймали внутри потока. JVM вызывает его перед тем, как поток умрёт, давая шанс залогировать ошибку, отправить алерт или записать метрику. Без него необработанное исключение просто уходит в `System.err` и легко теряется:
...
Это два метода `Throwable` для работы с причиной (cause) в цепочке исключений: `getCause()` — читает причину, `initCause()` — устанавливает её. Идя по `getCause()`, можно дойти от верхнеуровневого доменного исключения до самого первого технического сбоя:
...
Это две противоположные стратегии реакции на некорректное состояние. **Fail-fast** падает с исключением сразу, как только обнаружена проблема, — чтобы баг всплыл как можно раньше и ближе к причине. **Fail-safe** наоборот старается продолжить работу, не роняя процесс, жертвуя строгостью ради доступности.
...
## Q35. Как устроена цепочка исключений: `initCause()`, `getCause()`, `addSuppressed()`?

Цепочка исключений (`Exception Chaining`) — это механизм, который сохраняет первопричину при оборачивании одного исключения в другое. Без неё при `throw new ServiceException("error")` теряется стектрейс исходной ошибки, и по логу невозможно понять, что реально пошло не так. Передача `cause` склеивает оба стека в один, и логгер печатает всю цепочку: верхнеуровневое доменное исключение → его cause → cause причины.

Есть два независимых механизма связи: **cause** (одна причина — что вызвало это исключение) и **suppressed** (набор побочных исключений, возникших по пути, например при закрытии ресурса).
...
Когда основное исключение уже летит наверх, а попутно (например, в `finally` при закрытии ресурса) возникает второе — второе нельзя просто бросить, иначе первое потеряется. Его прикрепляют к основному как suppressed:
...
**Правило:** при оборачивании всегда передавайте исходное исключение как cause. `throw new ServiceException("error")` без `e` обрывает цепочку и теряет стектрейс первопричины — отладка по такому логу превращается в гадание.

---

## Q36. Что такое multi-catch (Java 7): синтаксис и ограничения?

Multi-catch (`Java 7+`) позволяет перехватить несколько несвязанных типов исключений в одном блоке `catch`, если реакция на них одинакова. До Java 7 приходилось дублировать тело обработчика в каждом `catch`:
...
**Ограничения multi-catch.** Три ключевых правила, которые часто спрашивают:
...
## Q37. Как работает try-with-resources с `AutoCloseable` и в каком порядке закрываются ресурсы?

`try-with-resources` компилятор разворачивает в обычный `try-finally` с вызовом `close()` и `addSuppressed()` под капотом — это синтаксический сахар, а не отдельная конструкция JVM. Ресурсы закрываются в **обратном порядке** объявления (LIFO): последний открытый закрывается первым. Это важно, когда ресурсы зависят друг от друга — например, `ResultSet` должен закрыться раньше `Statement` и `Connection`.
...
## Q38. Как правильно создавать кастомные исключения и зачем нужен `serialVersionUID`?

Кастомное исключение должно нести доменный смысл и весь контекст, нужный для обработки и диагностики. Ключевые правила сведены в комментариях кода ниже:
...
## Q39. Как обрабатывать checked-исключения внутри лямбды?

**Проблема:** стандартные функциональные интерфейсы (`Function`, `Consumer`, `Supplier`) не объявляют checked exceptions в сигнатуре, поэтому компилятор запрещает бросать их из лямбды. А многие полезные методы (тот же `Files.readString`) как раз бросают checked `IOException`.
...
## Q40. Как правильно логировать исключения и что включать в сообщение?

Главная цель — чтобы по логу можно было восстановить, что, где и при каких данных сломалось, не открывая дебаггер. Ключевые правила (подробности в комментариях кода):
...
## Q41. Сколько стоит создание исключения и почему `fillInStackTrace()` дорогой?

**Создание исключения — дорогая операция**, и основная цена — не сам `throw`, а конструктор `Throwable`, который вызывает `fillInStackTrace()`. Это нативный метод: он обходит весь стек вызовов и собирает массив `StackTraceElement`. Чем глубже стек, тем дороже. Поэтому исключения нельзя использовать для управления потоком в горячем пути.
...
## Q42. Как организовать глобальную обработку исключений в Spring (`@ControllerAdvice`, `ProblemDetail`)?

`@ControllerAdvice` + `@ExceptionHandler` дают единую точку обработки исключений для всех контроллеров: бизнес-код просто бросает доменное исключение, а advice превращает его в корректный HTTP-ответ с нужным статусом. Это убирает try-catch из контроллеров и гарантирует единый формат ошибок по всему API.
