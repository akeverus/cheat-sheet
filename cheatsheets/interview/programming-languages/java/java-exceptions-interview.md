...
> [!mcq]
> - [x] Корнем иерархии исключений в Java является класс `Throwable`, от которого наследуются `Error` и `Exception`. | Верно: `Throwable` — общий предок, `Error` — для фатальных сбоев JVM, `Exception` — для прикладных ошибок, `RuntimeException` — подкласс `Exception` для unchecked-случаев.
> - [ ] Корнем иерархии исключений в Java является класс `Exception`, от которого наследуются `Error` и `RuntimeException`. | Неверно: `Error` не наследуется от `Exception`, оба они — прямые потомки `Throwable`, а `catch (Exception)` не перехватывает `Error`.
> - [ ] Корнем иерархии исключений в Java является класс `RuntimeException`, от которого наследуются `Error` и `Exception`. | Неверно: `RuntimeException` — подкласс `Exception`, а не корень. Корень — `Throwable`, и только он может быть брошен через `throw`.
> - [ ] Корнем иерархии исключений в Java является класс `Object`, от которого напрямую наследуются `Exception` и `Error`. | Неверно: хотя все классы в Java наследуются от `Object`, специализированная иерархия исключений стартует именно с `Throwable`, а не напрямую от `Object`.
...
> - [x] `throw` выбрасывает конкретный объект исключения в теле метода, а `throws` объявляет в сигнатуре метода потенциально выбрасываемые типы. | Верно: `throw new IllegalArgumentException(...)` прерывает выполнение, а `throws IOException, SQLException` в сигнатуре обязывает вызывающий код обрабатывать checked-исключения.
...
> - [ ] `throw` и `throws` — синонимы, которые можно использовать взаимозаменяемо в сигнатурах и теле метода. | Неверно: это разные по смыслу ключевые слова с разным синтаксисом. Компилятор не примет `throw` в сигнатуре и `throws` в теле метода.
...
> [!mcq]
> - [ ] Блоки `catch` проверяются в обратном порядке объявления, поэтому родительский тип должен стоять раньше дочернего для корректной обработки. | Неверно: `catch`-блоки проверяются сверху вниз. Родитель раньше ребёнка даст ошибку компиляции — «unreachable catch block».
> - [ ] Блоки `catch` проверяются в произвольном порядке, определяемом JVM на основе фактического типа исключения в runtime. | Неверно: JVM не выбирает «лучший» `catch` — компилятор требует строгого сверху-вниз порядка, и порядок фиксируется в байткоде.
> - [x] Блоки `catch` проверяются последовательно сверху вниз, поэтому дочерний тип должен стоять раньше родительского для корректной обработки. | Верно: первый подходящий по типу `catch` забирает исключение. Если `Exception` стоит раньше `IOException`, последний становится недостижимым — ошибка компиляции.
> - [ ] Блоки `catch` проверяются параллельно, и JVM автоматически выбирает наиболее специфичный тип независимо от порядка объявления. | Неверно: обработка строго последовательная и детерминированная. Специфичность определяется программистом через порядок, а не автоматически.
...
> [!mcq]
> - [ ] В `multi-catch` переменная исключения `ex` mutable и её можно переприсваивать любым типом из объявленных через `|`. | Неверно: переменная в `multi-catch` неявно `final`. Компилятор запрещает переприсваивание, чтобы избежать неоднозначности типа.
> - [ ] В `multi-catch` допускается указывать связанные наследованием типы, поскольку компилятор автоматически выберет более специфичный. | Неверно: связанные типы запрещены — `catch (IOException | FileNotFoundException)` даст ошибку компиляции, т.к. `FileNotFoundException` избыточен при наличии родителя.
...
> [!mcq]
> - [x] Checked-исключения проверяются компилятором и требуют `try-catch` или `throws`, а unchecked (`RuntimeException` и наследники) — нет. | Верно: ключевое различие — compile-time проверка. `IOException` обязывает обработку, `NullPointerException` — нет, хотя оба наследуются от `Throwable`.
> - [ ] Checked-исключения всегда unchecked в runtime, а unchecked-исключения не ловятся в блоках `catch` вообще. | Неверно: путаница в терминах. Unchecked прекрасно ловятся в `catch`, разница только в требовании компилятора на этапе компиляции.
> - [ ] Checked-исключения наследуются от `Error`, а unchecked — от `Exception`, и эти ветки никогда не пересекаются. | Неверно: оба типа наследуются от `Exception`. `Error` — отдельная ветка для фатальных JVM-ошибок, не связана с checked/unchecked-классификацией.
> - [ ] Checked-исключения можно бросать только в статических методах, а unchecked — только в инстанс-методах. | Неверно: статичность метода не влияет на тип бросаемых исключений. Ограничения определяются только сигнатурой `throws`.
...
> [!mcq]
> - [ ] `Error` сигнализирует о проблемах в бизнес-логике приложения, а `Exception` — о критических сбоях JVM, требующих перезапуска. | Неверно: разбиение перевёрнуто. `Error` (OOM, SOE) — это проблемы уровня JVM, а `Exception` — это ошибки уровня приложения и внешних систем.
> - [x] `Error` сигнализирует о проблемах уровня JVM (память, стек, classloader) и обычно невосстановим, а `Exception` — это восстановимые ошибки уровня приложения. | Верно: `OutOfMemoryError` или `StackOverflowError` — невосстановимые сбои runtime-среды, тогда как `IOException`/`SQLException` предполагают осмысленную обработку.
> - [ ] `Error` и `Exception` — полные синонимы в Java, и разница только в исторически сложившихся именах классов. | Неверно: они принципиально различаются семантически и по рекомендуемой обработке. `Error` — не бизнес-код, `Exception` — уровень приложения.
> - [ ] `Error` в Java всегда checked и требует обработки, а `Exception` всегда unchecked и обработки не требует. | Неверно: `Error` — unchecked (наследник `Throwable` через отдельную ветку). Categорию checked/unchecked определяет подклассификация `Exception`, а не само имя.
...
> [!mcq]
> - [ ] Будет выброшен `NullPointerException`, потому что элемент по индексу `[1][1]` равен `null` и разыменование невозможно. | Неверно: разыменовывать `null` мы не доходим — до этого выход за границу массива. `NPE` был бы при обращении к `ints[1][0].intValue()`.
> - [ ] Будет выброшен `ClassCastException`, потому что `Integer[]` не может быть приведён к `int` автоматически. | Неверно: автобоксинг/анбоксинг между `Integer` и `int` работает без `ClassCastException`. Эта ошибка возникает только при некорректном приведении типов объектов.
> - [x] Будет выброшен `ArrayIndexOutOfBoundsException`, потому что `ints[1]` имеет длину 1, а мы обращаемся к индексу 1 (выход за границу). | Верно: `{null}` — массив из одного элемента с индексом 0. Обращение к индексу 1 выходит за границы, проверка выполняется до разыменования, поэтому `NPE` не достигается.
> - [ ] Будет выброшен `NumberFormatException`, потому что `null` нельзя преобразовать в числовое значение через `intValue()`. | Неверно: `NumberFormatException` связан только с парсингом строк (`Integer.parseInt`), а не с методом `intValue()` на обёрточных типах.
...
> [!mcq]
> - [ ] `Exception Chaining` — это автоматическое объединение нескольких одновременно выброшенных исключений в один composite-объект. | Неверно: это описание `addSuppressed`, а не chaining. Chaining связывает причину и следствие, а не параллельные исключения.
> - [ ] `Exception Chaining` — это механизм повторного выброса одного и того же объекта исключения на разных уровнях стека без потери контекста. | Неверно: повторный выброс (`throw ex`) сам по себе не создаёт цепочку. Chaining — это оборачивание одного исключения в другое через `cause`.
> - [ ] `Exception Chaining` — это последовательность `catch`-блоков от специфичного к общему, формирующая «цепочку» обработки. | Неверно: порядок `catch`-блоков — отдельная концепция. Chaining относится к связи между объектами исключений через поле `cause`.
> - [x] `Exception Chaining` — это механизм оборачивания одного исключения в другое через поле `cause` для сохранения первопричины. | Верно: например, `throw new OrderException("msg", ex)` сохраняет оригинальную `DataAccessException` как cause, и `getCause()` возвращает её. Stack trace показывает всю цепочку.
...
> - [ ] Стек формируется в момент вызова `throw`, а создание объекта только аллоцирует память без обхода стека. | Неверно: `fillInStackTrace()` вызывается из конструктора `Throwable`. Поэтому создание исключения уже дорого, даже если его не бросить.
> - [ ] Стек формируется только при первом вызове `getStackTrace()`, реализуя lazy-инициализацию. | Неверно: никакой lazy-инициализации нет — стек собирается сразу в конструкторе. `getStackTrace()` просто возвращает уже готовый массив.
> - [ ] Стек формируется в момент `catch`, чтобы показать путь от обработчика до источника. | Неверно: `catch` никак не модифицирует стек. Он фиксируется при создании объекта и далее остаётся неизменным.
...
> - [x] Подклассы создают для доменной семантики, добавления контекстных полей и группировки связанных ошибок в иерархию. | Верно: `OrderNotFoundException` с полем `orderId` несёт бизнес-смысл и позволяет ловить общий `PaymentException` для всех платёжных ошибок без перечисления конкретных подтипов.
> - [ ] Подклассы создают только ради обхода требования `throws` на checked-исключения. | Неверно: обход через `RuntimeException` — побочный эффект. Основная причина — доменная семантика, а выбор checked/unchecked определяется по Q20.
> - [ ] Подклассы обязательны для любого исключения — стандартные типы Java нельзя использовать напрямую. | Неверно: `IllegalArgumentException`, `IllegalStateException` и другие стандартные типы рекомендуется переиспользовать там, где они уместны.
...
> - [ ] Механизм исключений позволяет пропускать ошибки без всякой обработки, что уменьшает boilerplate-код. | Неверно: checked-исключения как раз требуют явной обработки или декларации. «Тихое проглатывание» — это антипаттерн (пустой `catch`), а не фича.
...
> - [ ] Механизм исключений автоматически откатывает все изменения состояния при выбросе, подобно транзакциям в БД. | Неверно: Java не предоставляет автоматический rollback. Восстановление состояния — забота программиста (через `finally` или `try-with-resources`).
...
> [!mcq]
> - [ ] `try-with-resources` закрывает ресурсы в том же порядке, в котором они были объявлены в блоке `try(...)`. | Неверно: порядок обратный (LIFO). Это важно для вложенных ресурсов: `Connection` открывается первым, но закрывается последним.
> - [ ] `try-with-resources` требует, чтобы ресурсы реализовывали интерфейс `Closeable` (не `AutoCloseable`), т.к. только он гарантирует корректное закрытие. | Неверно: достаточно `AutoCloseable` — он был введён именно для `try-with-resources` в Java 7. `Closeable extends AutoCloseable` и бросает более специфичный `IOException`.
> - [ ] `try-with-resources` не поддерживает несколько ресурсов в одной конструкции — для каждого нужен отдельный вложенный блок. | Неверно: несколько ресурсов объявляются через `;` в одной паре скобок. Они автоматически закроются в обратном порядке.
...
> [!mcq]
> - [x] Suppressed exceptions — это исключения из `close()`, которые прикрепляются к основному исключению из `try`-блока через `addSuppressed` без его подмены. | Верно: в ручном `finally` второе исключение затёрло бы первое. `try-with-resources` автоматически сохраняет обе ошибки — основное и suppressed доступны через `getSuppressed()`.
> - [ ] Suppressed exceptions — это исключения, которые полностью игнорируются JVM и не попадают в стектрейс ни при каких условиях. | Неверно: они сохраняются именно для диагностики и отображаются в стек-трейсе под меткой `Suppressed:`. Потеря информации как раз и решается этим механизмом.
> - [ ] Suppressed exceptions — это исключения, подавленные аннотацией `@SuppressWarnings` и не генерирующие предупреждений компилятора. | Неверно: `@SuppressWarnings` работает только с compile-time warnings, не связано с runtime-исключениями и механизмом `Throwable.addSuppressed`.
> - [ ] Suppressed exceptions — это те, которые не распространяются по стеку и автоматически конвертируются в `RuntimeException`. | Неверно: suppressed-исключения не влияют на распространение основного исключения по стеку. Они просто «прицеплены» к нему через поле.
...
> [!mcq]
> - [ ] `addSuppressed` и `getSuppressed` — это методы `Exception`, доступные только в checked-ветке иерархии. | Неверно: методы определены в базовом классе `Throwable`, поэтому доступны для любых наследников — `Exception`, `Error`, `RuntimeException`.
...
> - [ ] `addSuppressed` можно вызывать только один раз — повторный вызов приведёт к `IllegalStateException` (как у `initCause`). | Неверно: ограничение одного вызова действует для `initCause`, а `addSuppressed` можно вызывать многократно — исключения накапливаются в массиве.
> - [ ] `getSuppressed()` возвращает `List<Throwable>`, изменения в котором автоматически отражаются в объекте исключения. | Неверно: возвращаемый тип — `Throwable[]` (массив), и это копия, а не живая ссылка. Изменения в массиве не влияют на внутреннее состояние.
...
> [!mcq]
> - [ ] Стандартные функциональные интерфейсы (`Function`, `Consumer`) объявляют `throws Exception`, поэтому checked-исключения внутри лямбд бросаются без проблем. | Неверно: именно потому, что они НЕ объявляют `throws`, checked-исключения требуют обёртывания в unchecked (например, `UncheckedIOException`).
> - [ ] Unchecked-исключения внутри лямбды компилятор запрещает бросать, требуя оборачивать их в `try-catch`. | Неверно: как раз unchecked (`RuntimeException`, `NPE`) работают в лямбдах напрямую. Проблема только с checked.
> - [x] Checked-исключения внутри лямбды нужно оборачивать в unchecked (например, `UncheckedIOException`) или использовать кастомный функциональный интерфейс с `throws`. | Верно: стандартные интерфейсы не объявляют checked в сигнатуре, поэтому обходные пути — обёртка или `ThrowingFunction` с собственным `throws Exception`.
...
> [!mcq]
> - [ ] Дочерний метод может расширять объявленный список checked-исключений, добавляя новые типы, не указанные в родительском. | Неверно: это нарушило бы LSP — вызывающий код, работающий с типом родителя, не обрабатывает новые исключения. Компилятор это запрещает.
> - [ ] Дочерний метод обязан объявлять тот же список `throws`, что и родительский, без каких-либо изменений. | Неверно: список можно сужать (убирать исключения или заменять на подтипы) — лишь бы не расширять. Это контравариантность по исключениям.
> - [ ] Дочерний метод может бросать любые unchecked-исключения, но только если родитель их явно объявил в `throws`. | Неверно: unchecked-исключения не требуют объявления в `throws` ни на одном уровне иерархии. Их можно бросать свободно.
> - [x] Дочерний метод может сужать список checked-исключений (до подтипа или убирать совсем) и свободно бросать unchecked — но не расширять. | Верно: это контравариантность по исключениям, следующая из LSP. `throws FileNotFoundException` вместо `throws IOException` корректно, `throws Exception` — нет.
...
> [!mcq]
> - [x] Код компилируется, потому что бросается `RuntimeException` (unchecked), а `Exception` внутри — всего лишь `cause` и проверяется только тип самого выброса. | Верно: компилятор смотрит на тип объекта в `throw`, а не на его поля. Cause может быть любым `Throwable`, включая checked, это не требует декларации `throws`.
> - [ ] Код не компилируется, потому что `Exception` — checked, и его нужно либо обрабатывать в `try-catch`, либо декларировать в `throws`. | Неверно: `Exception` здесь — аргумент конструктора, а не самостоятельно выбрасываемое исключение. Объявление `throws` нужно только для реально бросаемых checked-типов.
> - [ ] Код не компилируется, потому что `RuntimeException` нельзя создавать с `cause` типа `Exception` — только `RuntimeException`. | Неверно: конструктор `RuntimeException(Throwable)` принимает любой `Throwable`, включая checked. Это базовая семантика цепочки.
> - [ ] Код компилируется, но только если добавить `throws Exception` в сигнатуру метода `doSomething`. | Неверно: декларация `throws` не требуется, потому что бросается unchecked-тип. Компилятор удовлетворён существующим кодом.
...
> [!mcq]
> - [ ] Нет, компилятор строго запрещает бросать checked-исключения без `throws` и не обойти это средствами языка. | Неверно: через type erasure и трюк с generic-переменной (sneaky throw) компилятор обманывается. Также `@SneakyThrows` из Lombok делает то же самое.
> - [x] Да, через sneaky throw — трюк со стиранием типов и generic-методом `<T extends Throwable>`, который обманывает compile-time проверку. | Верно: во время компиляции `T` выводится как `RuntimeException`, а в runtime летит реальный checked-тип. `Lombok @SneakyThrows` и библиотека Vavr используют этот приём.
> - [ ] Да, но только через JNI и вызов нативного кода, реализующего `Throwable.throw` на низком уровне JVM. | Неверно: JNI здесь не нужен. Трюк реализуется чисто на Java через generics и erasure.
> - [ ] Да, но только для исключений, помеченных аннотацией `@Unchecked`, введённой в Java 17. | Неверно: такой аннотации в Java нет. Выбор checked/unchecked определяется исключительно иерархией наследования от `RuntimeException`.
...
> [!mcq]
> - [ ] Кастомное исключение должно иметь только один конструктор без параметров — `message` и `cause` устанавливаются через сеттеры после создания. | Неверно: `Throwable` не имеет сеттеров для `message`. Стандарт — минимум два конструктора с message и с message+cause.
> - [ ] Кастомное исключение должно наследоваться строго от `Exception` или `Throwable`, но не от `RuntimeException` — последний зарезервирован для JVM. | Неверно: `RuntimeException` явно предназначен для наследования (`IllegalArgumentException` и т.п.). В современной практике большинство доменных исключений — unchecked.
> - [x] Кастомное исключение должно иметь конструкторы с message и с message+cause, включать контекстные поля (final) и суффикс `Exception` в имени. | Верно: эти три правила обеспечивают цепочку исключений (cause), диагностический контекст и читаемое именование. Опционально — `serialVersionUID`, если класс сериализуется.
> - [ ] Кастомное исключение должно иметь метод `handle()` для самостоятельной обработки — это снимает ответственность с вызывающего кода. | Неверно: обработка — ответственность catch-блока, а не самого исключения. Добавление `handle()` в класс исключения смешивает данные и поведение.
...
> [!mcq]
> - [ ] Использовать `RuntimeException` только для критических ошибок (JVM), а все бизнес-ошибки делать checked для явной обработки. | Неверно: это устаревший взгляд. Современные фреймворки (Spring) почти все доменные исключения делают unchecked для снижения boilerplate.
> - [ ] Использовать checked exception всегда, когда ошибка возможна — это гарантирует, что компилятор заставит её обработать. | Неверно: избыточное использование checked приводит к `throws Exception`-загрязнению и паттерну «поймал и проглотил». Checked имеет смысл только при реальной восстановимости.
> - [ ] Использовать `RuntimeException` для ошибок на уровне контроллера, а checked — для сервисного слоя, разделяя по архитектурным слоям. | Неверно: тип исключения определяется семантикой ошибки (восстановима или нет), а не архитектурным слоем, где она возникает.
> - [x] Использовать checked, когда вызывающий может и должен восстановиться (retry/fallback), иначе — `RuntimeException` для ошибок программиста и невосстановимых ситуаций. | Верно: это оригинальный критерий Джошуа Блоха. `IOException` при сетевой ошибке (можно повторить) — checked, `IllegalArgumentException` (баг) — unchecked.
...
> - [ ] Наследовать все доменные исключения напрямую от `Throwable`, минуя `Exception` — это уменьшает overhead. | Неверно: наследование от `Throwable` обходит всю инфраструктуру catch-блоков, которые обычно ловят `Exception`. Overhead одинаков.
> - [ ] Создавать максимально глубокую иерархию (5+ уровней) для каждого нюанса ошибки — это упрощает рефакторинг. | Неверно: глубокая иерархия усложняет поддержку и запутывает вызывающий код. Практика — 1-2 уровня: базовый класс домена + конкретные подтипы.
...
> [!mcq]
> - [ ] Блок `finally` выполняется только если в `try` не было исключения, для очистки ресурсов после успешной операции. | Неверно: `finally` выполняется всегда — и при нормальном завершении, и при исключении, и даже при `return` из `try`/`catch`. В этом его суть.
...
> - [ ] Блок `finally` выполняется только если соответствующий `catch` пойман, иначе исключение проходит мимо него к вышестоящему обработчику. | Неверно: `finally` выполняется, даже если ни один `catch` не подошёл — после чего исключение пробрасывается дальше по стеку.
> - [ ] Блок `finally` можно опустить, только если есть минимум три блока `catch`, иначе компилятор потребует его наличия. | Неверно: правило простое — `try` требует минимум один `catch` ИЛИ `finally`. Количество `catch`-блоков не влияет на обязательность `finally`.
