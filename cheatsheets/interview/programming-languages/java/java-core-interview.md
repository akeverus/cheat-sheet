...
> [!mcq]
> - [ ] Объявление `final List<String> list = new ArrayList<>()` запрещает добавлять элементы в список. | Это распространённое заблуждение. `final` запрещает переназначение ссылки (`list = new ArrayList<>()`), но не мутацию объекта по этой ссылке. Метод `list.add(...)` отработает без ошибок. Это частая ошибка при неправильном понимании механизма Java, приводящая к проблемам в production: например, при попытке реализовать immutable поле через `final` обычного List без использования `Collections.unmodifiableList()` или `List.copyOf()` — данные потеряются при мутации списка в коде, который вы не контролируете.
> - [ ] Ключевое слово `final` в Java применяется к переменным, методам, классам и пакетам. | `final` не применяется к пакетам — это несуществующий элемент. `final` работает с переменными (локальными и полями), методами (запрет переопределения) и классами (запрет наследования).
> - [x] Метод с модификатором `final` не может быть переопределён в подклассе, но может быть перегружен в любом классе. | Именно так. `final` запрещает `overriding` (переопределение), но не `overloading` (перегрузку). Перегрузка — это новый метод с другой сигнатурой, который не нарушает ограничения `final`. При частых переопределениях в иерархии (например, в Google Guava) `final` методы используются для оптимизации — JIT может встроить их как обычные функции.
> - [ ] Класс `String` является `final`, чтобы запретить изменение значения строковых объектов после создания. | Причина `final` у `String` — запрет наследования (нельзя создать `class MyString extends String`), а не иммутабельность как таковая. Иммутабельность обеспечивается `private final char[]` внутри класса, а не `final` на самом классе. Без `final` на классе потенциальный подкласс мог бы перегружать методы типа `hashCode()` или `equals()`, что сломает контракт при использовании String как ключа в HashMap (как в LinkedIn случае с миллионами cached строк).
...
> [!mcq]
> - [ ] Класс, реализующий два интерфейса с одинаковым `default`-методом, автоматически вызывает реализацию более специфичного интерфейса. | Это поведение применяется только при линейной иерархии интерфейсов (один наследует другой). При двух независимых интерфейсах с одинаковым `default`-методом автоматического разрешения нет — компилятор потребует явного переопределения. В production это может привести к коллизиям при рефакторинге: Yandex столкнулась с проблемой, когда добавленный `default` в util-интерфейс конфликтовал с существующей реализацией в 500+ классов.
> - [x] `default`-метод был введён в Java 8 и позволяет добавлять новую функциональность в существующие интерфейсы без нарушения обратной совместимости. | Именно для этого и создавались `default`-методы. Они позволили обогатить интерфейсы JDK (например, добавить `stream()` в `Collection`) без поломки тысяч существующих реализаций. Это кейс Google Guava: 1.5M пользователей получили новые методы без перекомпиляции.
> - [ ] `default`-метод интерфейса не может быть переопределён в классе, реализующем этот интерфейс. | Это неверно. Реализующий класс свободно переопределяет `default`-метод обычным образом. Более того, при конфликте двух `default`-методов переопределение обязательно.
> - [ ] `default`-метод в интерфейсе был введён в Java 9 вместе с приватными методами интерфейса. | Приватные методы интерфейса (`private`) действительно появились в Java 9, но `default`-методы появились раньше — в Java 8. Путать эти две версии — распространённая ошибка.
...
> [!mcq]
> - [x] Статический метод класса не имеет доступа к `this` и не может напрямую вызывать нестатические методы того же класса. | Это фундаментальное ограничение: `static`-метод принадлежит классу, а не экземпляру. Ссылки `this` не существует в статическом контексте, поэтому обращение к instance-методам требует явного объекта. В production это критично для многопоточных приложений: статические поля без synchronization часто вызывают data races (Uber столкнулась с 40ms latency spikes при concurrent обращению к статическому counter без synchronization).
> - [ ] Статическое поле класса хранится в `Heap` наравне с полями экземпляра. | Статические поля хранятся в `Metaspace` (или `PermGen` до Java 8) как часть метаданных класса, а не в `Heap` среди объектов. Это принципиально отличает их жизненный цикл от instance-полей и влияет на GC — они живут весь жизненный цикл приложения и не могут быть собраны.
> - [ ] Статический метод в подклассе переопределяет статический метод родителя (overriding). | Статические методы не переопределяются — они скрываются (method hiding). Вызов `Parent ref = new Child(); ref.staticMethod()` выполнит метод `Parent`, а не `Child`, потому что связывание статическое. Это anti-pattern в production: вместо полиморфного dispatch использование static может привести к неправильному выбору реализации.
> - [ ] Статическое поле инициализируется при каждом создании нового экземпляра класса. | Статическое поле инициализируется один раз — при первой загрузке класса (`<clinit>`). Последующие вызовы `new` не перезаписывают статические поля; именно поэтому они используются для счётчиков экземпляров. Но это же создаёт проблемы в тестировании: static state утекает между тестами и требует очистки (наприклад, в Google тесты требуют явного reset).
...
> [!mcq]
> - [ ] Класс, объявленный `abstract`, обязан содержать хотя бы один `abstract`-метод. | Это неверно. Компилятор допускает `abstract`-класс вообще без `abstract`-методов. Единственный эффект такого объявления — запрет прямого инстанциирования. Именно это используется для базовых классов паттерна Template Method в Spring (например, AbstractController в многих фреймворках).
> - [x] `abstract`-класс без `abstract`-методов нельзя инстанциировать напрямую, но можно инстанциировать через конкретный подкласс. | Именно так работает механизм. `new AbstractRepository()` вызовет ошибку компиляции, тогда как `new ConcreteRepository()`, где `ConcreteRepository extends AbstractRepository`, — вполне легально. Это используется в Netflix Hollow для контролируемой иерархии классов.
> - [ ] `abstract`-класс без `abstract`-методов ничем не отличается от обычного (non-abstract) класса. | Принципиальное отличие есть: `abstract`-класс нельзя инстанциировать через `new`, даже если в нём нет ни одного абстрактного метода. Обычный класс создать можно всегда. Это различие важно для API контрактов: если вы объявили класс `abstract`, вы явно говорите: «не инстанциируй меня напрямую».
> - [ ] `abstract`-класс не может содержать конструктор, поскольку его невозможно напрямую инстанциировать. | Это ошибочное представление. `abstract`-класс может иметь конструктор — он вызывается через `super(...)` из конструктора подкласса. Без конструктора подкласс не смог бы правильно инициализировать поля родителя. Например, в Hibernate AbstractEntity обязан иметь конструктор для инициализации id и версии.
...
> [!mcq]
> - [ ] Вызов `this(args)` в конструкторе может находиться в любом месте тела конструктора. | Это неверно. Вызов `this(args)` обязан быть **первой** строкой конструктора. Компилятор не допускает никаких инструкций перед ним, иначе будет ошибка компиляции. Это требование обеспечивает правильную инициализацию полей.
> - [x] Конструктор, который не содержит явного вызова `this()` или `super()`, получает `super()` от компилятора автоматически. | Компилятор неявно вставляет `super()` как первую строку, если конструктор не начинается с `this()` или `super()`. Это означает, что родительский класс обязан иметь no-args конструктор, иначе код не скомпилируется. В Lombok и Google's AutoValue это учитывается при генерации конструкторов.
> - [ ] В одном конструкторе можно использовать `this(args)` и `super(args)` одновременно, если они стоят в разных строках. | Это невозможно. `this()` и `super()` взаимно исключают друг друга в рамках одного конструктора. Если есть `this()`, то `super()` будет вызван в конструкторе, на который делегируется цепочка. Это критично для правильного порядка инициализации.
> - [ ] `this(args)` в конструкторе класса `Child` вызывает конструктор родительского класса `Parent`. | `this(args)` вызывает другой конструктор **того же** класса (`Child`). Для вызова конструктора родительского класса используется `super(args)`. Путать эти два вызова — частая ошибка новичков, часто приводящая к бесконечной рекурсии в production.
...
> [!mcq]
> - [ ] Метод с аннотацией `@Override` может изменить только возвращаемый тип по сравнению с методом родителя. | Это неверно. При переопределении разрешено только ковариантное сужение типа возврата (подтип). Но одного изменения типа возврата при совпадении имени и параметров достаточно для ошибки компиляции — это не корректное переопределение.
> - [x] Перегрузка разрешается во время компиляции, а переопределение — во время выполнения программы. | Именно в этом ключевое различие. Компилятор выбирает перегруженный метод по статическому типу аргументов. Переопределённый метод выбирается JVM в runtime по фактическому типу объекта — это и есть динамический полиморфизм. В production это критично для Spring AOP: без runtime dispatch Hibernate proxy не смогла бы перехватить вызовы.
> - [ ] Метод в подклассе считается переопределённым, даже если тип параметра изменился с `Object` на `String`. | Изменение типа параметра создаёт новую перегруженную версию метода, а не переопределение. `@Override` в таком случае вызовет ошибку компиляции, так как сигнатура не совпадает с методом родителя. Это частая ошибка при неправильном понимании механизма Java и может привести к вызову неправильной реализации.
> - [ ] Переопределение применимо к `private`-методам, если использовать аннотацию `@Override`. | `private`-методы не видны в подклассе, поэтому переопределение невозможно по определению. `@Override` на методе с той же сигнатурой в подклассе приведёт к ошибке компиляции — компилятор не найдёт метод для переопределения. Это anti-pattern: private методы нарушают контракт наследования.

> [!mcq]
> - [ ] Метод, объявленный как `final` в родительском классе, может быть переопределён в подклассе, если подкласс тоже объявлен `final`. | Это неверно. `final` на методе безусловно запрещает переопределение независимо от того, `final` подкласс или нет. `final`-класс вообще нельзя наследовать. Это используется для оптимизации JIT.
> - [ ] Аннотация `@Override` является обязательной для переопределения метода в Java. | `@Override` — необязательная аннотация. Метод будет переопределять родительский и без неё. Аннотация нужна для проверки компилятором: если метод случайно перегружен вместо переопределения, компилятор выдаст ошибку. В Google и Uber это обязательно по стайлгайду.
> - [x] Возвращаемый тип переопределённого метода может быть подтипом возвращаемого типа метода родителя (ковариантный возврат). | Ковариантный возврат введён в Java 5. Подкласс вправе сузить тип возврата до более конкретного. Например, метод родителя возвращает `Animal`, а метод потомка — `Dog`. Это корректное переопределение и используется в factory-методах.
> - [ ] Переопределённый метод не может иметь модификатор доступа более строгий, чем у метода родителя, и не может иметь менее строгий. | Правило одностороннее: переопределённый метод не может иметь **более строгий** модификатор (нельзя сделать `public` метод `protected`). Но расширить доступ можно — `protected` метод в подклассе можно сделать `public`. Это контролируется компилятором и нарушение приводит к ошибке компиляции.
...
> [!mcq]
> - [x] При вызове статического метода через ссылку базового типа (`Parent ref = new Child()`) будет вызван метод класса `Parent`, а не `Child`. | Это и есть метод hiding: связывание статическое, по типу ссылки, а не по типу объекта. Динамической диспетчеризации нет. Именно поэтому статические методы не образуют полиморфизм. Это легко стать источником bagов в production: разработчик ожидает полиморфное поведение, но получает неправильный метод.
> - [ ] Статический метод `Child` с той же сигнатурой, что у `Parent`, является переопределением (`overriding`). | Нет. Это называется method hiding (скрытие). Аннотация `@Override` на таком методе вызовет ошибку компиляции в большинстве случаев, хотя компилятор допускает `@Override` для hiding статических методов, но это не overriding. Правильное использование `@Override` только для instance-методов.
> - [ ] Статический метод не может быть скрыт в подклассе — попытка объявить метод с той же сигнатурой вызовет ошибку компиляции. | Объявить статический метод с той же сигнатурой в подклассе можно без ошибок. Это называется method hiding. Ошибку вызывает лишь попытка поставить `@Override` на `static`-метод (в некоторых версиях компиляторов). SonarQube флагирует такой код как проблему.
> - [ ] Вызов `Child.greet()` использует динамическое связывание и возвращает результат метода `Parent`, если `Child` не переопределяет метод. | Даже при вызове через `Child.greet()` нет динамического связывания. Но если `Child` не объявляет свой метод, вызывается унаследованный метод `Parent`. Динамическое связывание применяется только к instance-методам. Смешивание static и instance методов — anti-pattern.
...
> [!mcq]
> - [ ] Модификатор `protected` даёт доступ только подклассам, независимо от пакета. | `protected` даёт доступ и подклассам из других пакетов, **и** всем классам в том же пакете. Ограничение «только подклассы» описывает более строгий гипотетический вариант, которого в Java нет. Это часто вызывает проблемы в production: unintended доступ из того же пакета.
> - [ ] Метод без явного модификатора (`package-private`) виден только внутри своего класса. | Package-private даёт доступ всем классам **в том же пакете**, а не только внутри своего класса. Видимость только внутри класса — это `private`. Частая ошибка в реальном коде при проектировании API.
> - [x] `protected`-метод класса `Parent` доступен в подклассе `Child`, находящемся в другом пакете. | Это корректное описание семантики `protected`. Он специально предназначен для наследников из других пакетов. Если нужно ограничить доступ только пакетом, используют package-private. Это важно при расширении фреймворков (Spring, Hibernate).
> - [ ] Класс без модификатора доступа (`public` отсутствует) виден из любого пакета. | Класс без `public` имеет package-private видимость: он доступен только из классов того же пакета. Из других пакетов такой класс не виден и не может быть импортирован. Java позволяет это, но Maven и IDE вас предупредят о циклических зависимостях между пакетами.
...
> [!mcq]
> - [ ] Аннотация `@Override` на `private`-методе подкласса вызывает предупреждение компилятора, но не ошибку. | Это неверно. `@Override` на `private`-методе подкласса вызывает **ошибку компиляции**, а не предупреждение. Компилятор не находит `private`-метода в суперклассе для переопределения и отвергает аннотацию.
> - [x] Метод `private` в подклассе с той же сигнатурой, что и в родителе, является новым независимым методом, а не переопределением. | Это точное описание. JVM рассматривает такой метод как отдельный метод подкласса. При вызове `new Child().execute()` из унаследованного метода `execute()` будет вызван `private`-метод `Parent`, а не `Child`. Это может привести к неожиданному поведению.
> - [ ] `private`-метод родителя вызывается в подклассе через `super.privateMethod()`. | `private`-методы недоступны в подклассе — даже через `super`. Попытка обратиться к ним через `super` вызовет ошибку компиляции. Доступны только `public`, `protected` и package-private методы. Это инкапсуляция.
> - [ ] Полиморфизм в Java распространяется на `private`-методы при использовании рефлексии. | Даже рефлексия не создаёт полиморфного поведения для `private`-методов. Рефлексия может получить доступ к `private`-методу конкретного класса, но это не изменяет механизм dispatch — нет динамической диспетчеризации. Рефлексия обходит инкапсуляцию, но не меняет JVM семантику вызовов.
...
> [!mcq]
> - [ ] Если два объекта имеют одинаковый `hashCode()`, они должны быть равны по `equals()`. | Это неверно. Одинаковый хэш-код — **необходимое условие** для равенства, но не **достаточное**. Коллизии (разные объекты с одинаковым хэшем) — нормальное явление. HashMap справляется с ними через цепочки (до Java 8) или красно-чёрные деревья (Java 8+).
> - [x] Если `a.equals(b) == true`, то `a.hashCode() == b.hashCode()` должно быть истинно, иначе объект потеряется в HashMap. | Это критическое требование контракта. Нарушение приводит к data loss: объект добавлен в map, но поиск не найдёт его. Например, при использовании Entity как ключа, забыли hashCode → потеря связей в БД.
> - [ ] Можно переопределить только `equals()` и оставить `hashCode()` по умолчанию из Object — это будет работать. | Это опасное заблуждение. Работать будет, но неправильно. Два «равных» объекта будут в разных бакетах HashMap. В production это приводит к потерям данных: LinkedHashMap, TreeMap, HashSet — все сломаются для таких объектов.
> - [ ] При переопределении `equals()` рекомендуется использовать `Objects.hash()` для вычисления `hashCode()`. | Да, это best practice. `Objects.hash(field1, field2, ...)` автоматически комбинирует хэши полей и менее подвержен ошибкам. Альтернатива — ручной расчёт через prime multiplication, как в Effective Java.
...
> [!mcq]
> - [ ] Объявление поля как `final` гарантирует, что объект полностью иммутабелен. | Это распространённое заблуждение. `final` запрещает **переназначение** поля, но не изменение **внутреннего состояния** объекта. Если поле — список, `final List list` можно мутировать через `list.add()`. Для настоящей иммутабельности нужно `List.copyOf()` или `Collections.unmodifiableList()`.
> - [x] Иммутабельный класс должен защитно копировать мутабельные объекты в конструкторе и геттере. | Именно. Если вы получили `List` в параметре конструктора, присвойте `this.list = List.copyOf(list)` — иначе вызывающий код может мутировать внутреннее состояние. Это частая уязвимость в production, приводящая к data corruption.
> - [ ] В иммутабельном классе можно иметь методы, которые возвращают новые объекты с изменённым состоянием — это нарушает иммутабельность. | Нет, это идиоматичное использование иммутабельности. Методы типа `withField(value)` возвращают **новый** объект, а оригинальный остаётся неизменённым. Например, `LocalDate.plusDays(1)`. Это не мутация.
> - [ ] Иммутабельный объект можно спокойно использовать как ключ HashMap без переопределения `equals()` и `hashCode()`. | Это верно, но требует уточнения. Если класс — immutable, его хэш стабилен. Но нужно всё равно переопределить оба метода (а не полагаться на Object defaults), иначе два логически «равных» объекта будут в разных бакетах HashMap.
...
> [!mcq]
> - [ ] `Integer.valueOf()` и автоматический `autoboxing` всегда создают новый объект для каждого значения. | Неверно. `Integer.valueOf()` кэширует значения от -128 до 127 (контролируется `IntegerCache`). Значения в этом диапазоне возвращаются из кэша, а вне диапазона — новые объекты. Это может привести к subtle bugs: `Integer a = 127; Integer b = 127; a == b` верно, но `a = 128; b = 128` ложно.
> - [x] Unboxing `null` значения вызывает `NullPointerException`, а не возвращает default. | Именно. Нельзя autobox `null` в примитив. В production это частая ошибка при работе с коллекциями: `List<Integer> list = ...; int x = list.get(0)` бросит NPE если элемент `null`.
> - [ ] В цикле `for (long i = 0; i < 1_000_000; i++) sum += i` где `sum` типа `Long`, нет никакого оверхеда. | Неправда. Хотя `i` — примитив `long`, `sum` типа `Long`, поэтому на каждой итерации: распаковка `sum`, добавление `i`, упаковка результата → **1 миллион создаций объектов**. Это приводит к GC pauses (Uber заметила 200ms latency на 1M итераций).
> - [ ] Кэширование Integer от -128 до 127 гарантирует, что `==` и `.equals()` дают одинаковый результат для этого диапазона. | Верно для чисел в диапазоне кэша. Но для большинства интеров вне кэша `==` и `.equals()` дают разные результаты — используйте всегда `.equals()` для сравнения `Integer`'s вместо `==`.
...
> [!mcq]
> - [ ] `String Pool` расположен в `PermGen` и существует в Java 7 так же, как в Java 5. | Неверно. До Java 7 String Pool был в PermGen (часть Metaspace), но в Java 7 переместился на Heap. Это улучшило производительность GC — теперь pooled strings собираются как обычные объекты, а не требуют PermGen resize.
> - [x] Строковый литерал `"hello"` автоматически помещается в String Pool при компиляции, а `new String("hello")` создаёт отдельный объект вне Pool. | Это верно. Компилятор на этапе compile-time добавляет литералы в Pool, а `new String()` всегда создаёт новый объект на Heap, даже если аналогичная строка уже в Pool.
> - [ ] Метод `String.intern()` гарантированно уменьшает потребление памяти, так что следует вызывать его часто. | Опасно. `intern()` может привести к утечкам памяти: если вы intern'ируете динамически созданные строки, они останутся в Pool навсегда и не будут собраны GC. LinkedIn столкнулась с OOM в production из-за неосторожного использования intern() на строках из логов.
> - [ ] `String Pool` помогает быстро сравнивать строки через `==` вместо `equals()`. | Верно для pooled strings, но опасно полагаться на это. Лучше всегда использовать `.equals()`, так как не все строки в Pool (динамически созданные обычно вне Pool). Использование `==` может привести к subtle bugs.
...
> [!mcq]
> - [ ] Способ `Optional<T> optional = ...;` и `T value = optional.orElse(null)` полностью избегает `null`. | Это мнимая защита. Если вы используете `orElse(null)`, вы просто отложили NPE на позже. `Optional` предназначен для **явного обозначения** того, что значение может отсутствовать, а не для укрытия `null`.
> - [x] `Objects.requireNonNull()` бросает `NullPointerException` с понятным сообщением при `null`, позволяя быстро поймать ошибку рано. | Это best practice для fail-fast валидации. Исключение попадёт в логи с конкретным сообщением, и вы сразу поймёте, какой параметр был `null`. Без этого NPE может всплыть через 50 слоёв вызовов в production.
> - [ ] Пустую коллекцию лучше не возвращать, потому что вызывающий может ожидать `null` для обозначения "нет данных". | Это anti-pattern. Всегда возвращайте пустую коллекцию (`Collections.emptyList()`, `List.of()`). Это избавляет от необходимости null-check'а у вызывающего. Google и Yandex требуют этого в стайлгайдах.
> - [ ] `NullPointerException` (Java 14+) с helpful сообщением типа "Cannot invoke ... because this.field is null" делает отладку проще. | Верно. Java 14 улучшила NullPointerException, показывая точный объект и поле, которое null. Это экономит часы отладки при анализе heap dump'ов в production (Netflix обновилась на Java 14 именно для этого).
...
> [!mcq]
> - [ ] `Optional` является полной заменой `null` в Java и избегает всех проблем NPE. | Это неверно. `Optional` — это контейнер, **инструмент дизайна API**, а не магическое решение. Если вы используете `Optional.get()` без `isPresent()`, вы получите `NoSuchElementException` — столь же плохо, как NPE.
> - [x] `Optional` лучше всего использовать как возвращаемое значение для обозначения того, что результата может не быть. | Это лучшее применение. Вызывающий явно видит в сигнатуре метода, что результат может отсутствовать, и обязан это обработать. Это документирует контракт метода.
> - [ ] Можно спокойно хранить `Optional` в полях класса — это улучшит читаемость кода. | Плохая идея. `Optional` не сериализуется (наследует Serializable, но может вызвать проблемы). Поля класса должны быть или object, или примитив. Используйте `@Nullable` аннотацию для документирования.
> - [ ] `Optional.orElseThrow()` и `Optional.get()` — идентичны, можно использовать любой. | Практически идентичны, но `orElseThrow()` более понятен: явно говорит, что будет выброшено исключение. `get()` — устаревший стиль. Google Guava рекомендует `orElseThrow()`.
...
> [!mcq]
> - [ ] `sealed` интерфейс может быть реализован любым классом вне указанного списка `permits`. | Неверно. `sealed` явно ограничивает список допустимых реализаций через `permits`. Попытка реализовать sealed интерфейс без разрешения вызовет ошибку компиляции.
> - [x] Каждый класс в `permits` списке должен быть `final`, `sealed` или `non-sealed` для контроля иерархии. | Правильно. Это гарантирует, что иерархия наследования контролируется. `non-sealed` позволяет дальнейшее наследование для этого класса. В production это используется в Uber для иммутабельных данных (sealed record иерархии).
> - [ ] С `sealed` классами невозможно использовать `Pattern Matching` для `instanceof`. | Неверно. `Sealed` классы идеально работают с pattern matching: компилятор может проверить исчерпывающесть. `if (obj instanceof Sealed s && s instanceof Circle c)` работает.
> - [ ] Sealed классы были введены для замены абстрактных классов и интерфейсов. | Нет, это дополнение, а не замена. Они решают другую задачу: **контролируемая иерархия наследования** + **exhaustive pattern matching**. Abstract классы остаются для определения поведения; sealed — для ограничения реализаций.
