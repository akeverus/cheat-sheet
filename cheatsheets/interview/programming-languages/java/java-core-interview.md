...
> [!mcq]
> - [ ] Объявление `final List<String> list = new ArrayList<>()` запрещает добавлять элементы в список. | Это распространённое заблуждение. `final` запрещает переназначение ссылки (`list = new ArrayList<>()`), но не мутацию объекта по этой ссылке. Метод `list.add(...)` отработает без ошибок. Это частая ошибка при неправильном понимании механизма Java.
...
> - [x] Метод с модификатором `final` не может быть переопределён в подклассе, но может быть перегружен в любом классе. | Именно так. `final` запрещает `overriding` (переопределение), но не `overloading` (перегрузку). Перегрузка — это новый метод с другой сигнатурой, который не нарушает ограничения `final`.
...
> [!mcq]
> - [x] Статический метод класса не имеет доступа к `this` и не может напрямую вызывать нестатические методы того же класса. | Это фундаментальное ограничение: `static`-метод принадлежит классу, а не экземпляру. Ссылки `this` не существует в статическом контексте, поэтому обращение к instance-методам требует явного объекта.
> - [ ] Статическое поле класса хранится в `Heap` наравне с полями экземпляра. | Статические поля хранятся в `Metaspace` (или `PermGen` до Java 8) как часть метаданных класса, а не в `Heap` среди объектов. Это принципиально отличает их жизненный цикл от instance-полей.
> - [ ] Статический метод в подклассе переопределяет статический метод родителя (overriding). | Статические методы не переопределяются — они скрываются (method hiding). Вызов `Parent ref = new Child(); ref.staticMethod()` выполнит метод `Parent`, а не `Child`, потому что связывание статическое.
> - [ ] Статическое поле инициализируется при каждом создании нового экземпляра класса. | Статическое поле инициализируется один раз — при первой загрузке класса (`<clinit>`). Последующие вызовы `new` не перезаписывают статические поля; именно поэтому они используются для счётчиков экземпляров.
...
> [!mcq]
> - [ ] Класс, объявленный `abstract`, обязан содержать хотя бы один `abstract`-метод. | Это неверно. Компилятор допускает `abstract`-класс вообще без `abstract`-методов. Единственный эффект такого объявления — запрет прямого инстанциирования. Именно это используется для базовых классов паттерна Template Method.
> - [x] `abstract`-класс без `abstract`-методов нельзя инстанциировать напрямую, но можно инстанциировать через конкретный подкласс. | Именно так работает механизм. `new AbstractRepository()` вызовет ошибку компиляции, тогда как `new ConcreteRepository()`, где `ConcreteRepository extends AbstractRepository`, — вполне легально.
> - [ ] `abstract`-класс без `abstract`-методов ничем не отличается от обычного (non-abstract) класса. | Принципиальное отличие есть: `abstract`-класс нельзя инстанциировать через `new`, даже если в нём нет ни одного абстрактного метода. Обычный класс создать можно всегда.
> - [ ] `abstract`-класс не может содержать конструктор, поскольку его невозможно напрямую инстанциировать. | Это ошибочное представление. `abstract`-класс может иметь конструктор — он вызывается через `super(...)` из конструктора подкласса. Без конструктора подкласс не смог бы правильно инициализировать поля родителя.
...
> [!mcq]
> - [ ] Вызов `this(args)` в конструкторе может находиться в любом месте тела конструктора. | Это неверно. Вызов `this(args)` обязан быть **первой** строкой конструктора. Компилятор не допускает никаких инструкций перед ним, иначе будет ошибка компиляции.
...
> - [ ] В одном конструкторе можно использовать `this(args)` и `super(args)` одновременно, если они стоят в разных строках. | Это невозможно. `this()` и `super()` взаимно исключают друг друга в рамках одного конструктора. Если есть `this()`, то `super()` будет вызван в конструкторе, на который делегируется цепочка.
> - [ ] `this(args)` в конструкторе класса `Child` вызывает конструктор родительского класса `Parent`. | `this(args)` вызывает другой конструктор **того же** класса (`Child`). Для вызова конструктора родительского класса используется `super(args)`. Путать эти два вызова — частая ошибка новичков.
...
> - [ ] Метод в подклассе считается переопределённым, даже если тип параметра изменился с `Object` на `String`. | Изменение типа параметра создаёт новую перегруженную версию метода, а не переопределение. `@Override` в таком случае вызовет ошибку компиляции, так как сигнатура не совпадает с методом родителя. Это частая ошибка при неправильном понимании механизма Java.
> - [ ] Переопределение применимо к `private`-методам, если использовать аннотацию `@Override`. | `private`-методы не видны в подклассе, поэтому переопределение невозможно по определению. `@Override` на методе с той же сигнатурой в подклассе приведёт к ошибке компиляции — компилятор не найдёт метод для переопределения.

> [!mcq]
> - [ ] Метод, объявленный как `final` в родительском классе, может быть переопределён в подклассе, если подкласс тоже объявлен `final`. | Это неверно. `final` на методе безусловно запрещает переопределение независимо от того, `final` подкласс или нет. `final`-класс вообще нельзя наследовать.
> - [ ] Аннотация `@Override` является обязательной для переопределения метода в Java. | `@Override` — необязательная аннотация. Метод будет переопределять родительский и без неё. Аннотация нужна для проверки компилятором: если метод случайно перегружен вместо переопределения, компилятор выдаст ошибку.
...
> [!mcq]
> - [x] При вызове статического метода через ссылку базового типа (`Parent ref = new Child()`) будет вызван метод класса `Parent`, а не `Child`. | Это и есть метод hiding: связывание статическое, по типу ссылки, а не по типу объекта. Динамической диспетчеризации нет. Именно поэтому статические методы не образуют полиморфизм.
...
> [!mcq]
> - [ ] Модификатор `protected` даёт доступ только подклассам, независимо от пакета. | `protected` даёт доступ и подклассам из других пакетов, **и** всем классам в том же пакете. Ограничение «только подклассы» описывает более строгий гипотетический вариант, которого в Java нет.
> - [ ] Метод без явного модификатора (`package-private`) виден только внутри своего класса. | Package-private даёт доступ всем классам **в том же пакете**, а не только внутри своего класса. Видимость только внутри класса — это `private`. Частая ошибка в реальном коде.
> - [x] `protected`-метод класса `Parent` доступен в подклассе `Child`, находящемся в другом пакете. | Это корректное описание семантики `protected`. Он специально предназначен для наследников из других пакетов. Если нужно ограничить доступ только пакетом, используют package-private.
> - [ ] Класс без модификатора доступа (`public` отсутствует) виден из любого пакета. | Класс без `public` имеет package-private видимость: он доступен только из классов того же пакета. Из других пакетов такой класс не виден и не может быть импортирован.
...
> - [x] Метод `private` в подклассе с той же сигнатурой, что и в родителе, является новым независимым методом, а не переопределением. | Это точное описание. JVM рассматривает такой метод как отдельный метод подкласса. При вызове `new Child().execute()` из унаследованного метода `execute()` будет вызван `private`-метод `Parent`, а не `Child`.
> - [ ] `private`-метод родителя вызывается в подклассе через `super.privateMethod()`. | `private`-методы недоступны в подклассе — даже через `super`. Попытка обратиться к ним через `super` вызовет ошибку компиляции. Доступны только `public`, `protected` и package-private методы.
> - [ ] Полиморфизм в Java распространяется на `private`-методы при использовании рефлексии. | Даже рефлексия не создаёт полиморфного поведения для `private`-методов. Рефлексия может получить доступ к `private`-методу конкретного класса, но это не изменяет механизм dispatch — нет динамической диспетчеризации.
