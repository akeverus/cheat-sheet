...
> [!mcq]
> - [ ] Объявление `final List<String> list = new ArrayList<>()` запрещает добавлять элементы в список. | Это распространённое заблуждение. `final` запрещает переназначение ссылки (`list = new ArrayList<>()`), но не мутацию объекта по этой ссылке. Метод `list.add(...)` отработает без ошибок.
...
> - [ ] Метод в подклассе считается переопределённым, даже если тип параметра изменился с `Object` на `String`. | Изменение типа параметра создаёт новую перегруженную версию метода, а не переопределение. `@Override` в таком случае вызовет ошибку компиляции, так как сигнатура не совпадает с методом родителя.
