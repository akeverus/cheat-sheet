...
**Инкапсуляция** — принцип ООП, который объединяет данные и методы в единый объект и прячет внутреннюю реализацию от внешнего кода. Снаружи к данным можно обращаться только через контролируемый публичный API, а не напрямую через поля. Это даёт классу возможность гарантировать свои инварианты: никто не сломает состояние объекта в обход проверок.
...
- **Инкапсуляция — не «приватные поля + геттеры/сеттеры».** Это распространённое заблуждение. Если на каждое поле автоматом сгенерировать `getXxx`/`setXxx`, объект остаётся «голым»: любой код снаружи может выставить ему любое значение. Настоящая инкапсуляция скрывает *поведение* и допускает только осмысленные операции над состоянием.
- **`setBalance()` в примере отсутствует намеренно.** Баланс нельзя присвоить напрямую — он меняется только через бизнес-операции `deposit()` и `withdraw()`, каждая из которых проверяет корректность.
- **Валидация внутри методов держит инвариант объекта** (баланс >= 0). Объект сам отвечает за свою целостность, а не полагается на дисциплину вызывающего кода.
...
- Класс может наследоваться только от **одного** класса (`extends` — single inheritance). Это сделано осознанно, чтобы избежать diamond problem (см. Q11).
- Класс может реализовывать **несколько** интерфейсов (`implements`) — так Java компенсирует отсутствие множественного наследования классов.
- Конструкторы **не наследуются**: подкласс обязан вызвать конструктор родителя через `super(...)` (первой строкой), потому что суперкласс должен корректно проинициализировать свою часть состояния.
- `private`-члены суперкласса **недоступны** в подклассе напрямую — они часть скрытой реализации родителя.
- `final`-классы **нельзя** наследовать, а `final`-методы — переопределять: это способ зафиксировать поведение и защитить инвариант.

## Q4. (!) Что такое полиморфизм?

**Полиморфизм** (буквально «много форм») — принцип ООП, при котором объекты разных типов отвечают на один и тот же вызов метода по-своему. Благодаря этому код можно писать в терминах общего типа (`Shape`), не зная конкретной реализации, — а нужное поведение JVM подберёт сама. В `Java` выделяют два вида:
...
- «Есть ли в `Java` перегрузка операторов?» — Нет (в отличие от C++ или Kotlin). Единственное исключение — `+` для конкатенации строк, и это «зашито» в язык, а не пользовательская перегрузка.
...
- **`default`-методы** (с `Java 8`) дают интерфейсам реализацию, но при их конфликте ответственность за выбор перекладывается на программиста.
- **При конфликте `default`-методов класс обязан явно переопределить метод** — компилятор не угадывает за вас, и неоднозначность снова исключена.
...
`default`-метод (с `Java 8`) — метод интерфейса с готовой реализацией прямо в теле интерфейса. Главная цель — **эволюция интерфейсов без поломки обратной совместимости**: до Java 8 добавление нового метода в интерфейс заставляло переписывать все классы, которые его реализуют. С `default`-методом новый метод приходит с реализацией по умолчанию, и старый код продолжает компилироваться. Именно так в `Collection` добавили `stream()`:
...
Правила разрешения конфликтов (когда метод приходит из нескольких мест):
- **Реализация в классе всегда побеждает** `default`-метод интерфейса — метод класса имеет приоритет.
- **Более специфичный интерфейс побеждает менее специфичный** (подынтерфейс важнее своего родителя).
- **Если приоритет неоднозначен** (два независимых интерфейса с одинаковым `default`-методом) — класс обязан явно переопределить метод и при желании вызвать нужную версию через `Интерфейс.super.метод()`.
...
Речь о том, **когда** и **по чему** компилятор/JVM решает, какой именно метод вызвать.

**Статическое связывание** (early binding) — вызов разрешается ещё на этапе компиляции, по *типу ссылки*. Это возможно, когда метод заведомо нельзя переопределить, поэтому реальный тип объекта роли не играет. Применяется к:
- `static`-методам
- `private`-методам
- `final`-методам
- конструкторам
- выбору перегруженного метода (по типам аргументов)

**Динамическое связывание** (late binding) — вызов разрешается в рантайме, по *реальному типу объекта*. Применяется к переопределённым экземплярным методам — именно это даёт runtime-полиморфизм.
...
Это два направления приведения типа вдоль иерархии наследования:

- **Upcasting** — приведение от подкласса к суперклассу (вверх по иерархии: `Dog` → `Animal`). Делается **неявно** и **всегда безопасно**: любой `Dog` гарантированно является `Animal`, поэтому компилятору проверять нечего.
- **Downcasting** — приведение от суперкласса к подклассу (вниз: `Animal` → `Dog`). Требует **явного каста** и **потенциально опасно**: компилятор не может гарантировать, что внутри ссылки `Animal` лежит именно `Dog`. Если реальный тип другой — в рантайме прилетит `ClassCastException`.
...
**Open/Closed Principle**: классы должны быть **открыты для расширения, но закрыты для модификации**. То есть добавление нового поведения должно сводиться к написанию *нового* кода, а не к правке уже работающего и оттестированного. Каждое изменение проверенного класса — риск сломать существующую функциональность; OCP убирает этот риск, вынося точки расширения в абстракции.
...
**Liskov Substitution Principle**: объект подтипа должен быть **взаимозаменяем** с объектом базового типа без нарушения корректности программы. Иными словами, подкласс обязан соблюдать контракт родителя: не усиливать предусловия, не ослаблять постусловия и не преподносить вызывающему коду неожиданных сюрпризов. Если это не так — наследование выбрано неправильно.
...
Модификаторы доступа управляют **видимостью** класса, поля или метода — то есть тем, какой код имеет право к ним обращаться. Это основной инструмент инкапсуляции: чем уже доступ, тем меньше у внешнего кода возможностей завязаться на детали реализации. `Java` предоставляет четыре уровня, от самого закрытого к самому открытому:
...
**Практическое правило — давать минимально необходимый доступ:**
- поля — почти всегда `private` (инкапсуляция);
- методы публичного API — `public`;
- вспомогательные методы — `private`;
- методы, предназначенные для переопределения в подклассах, — `protected`;
- утилитарные классы, нужные только внутри пакета, — `package-private`.
...
Это две области памяти JVM с разным назначением. **Stack** — память потока: на нём живут локальные переменные, ссылки и параметры вызовов; кадры добавляются и снимаются по принципу LIFO вместе с входом/выходом из методов, поэтому он очень быстрый и не требует сборщика мусора. **Heap** — общая для всех потоков куча: здесь размещаются все объекты и массивы, а освобождает их сборщик мусора (GC).
...
`String` в `Java` неизменяем по дизайну — у него просто нет методов, меняющих внутренние данные. Это не случайность, а осознанное решение, у которого четыре практических причины:

1. **String pool** — JVM кеширует строковые литералы и переиспользует один объект на одинаковые значения. Если бы строки были мутабельны, изменение одной строки затронуло бы все ссылки на тот же объект в пуле.
2. **Безопасность хеш-кода** — `hashCode()` вычисляется один раз и кешируется. Поэтому строки — идеальные ключи `HashMap`: их хеш не «уплывёт» после того, как объект положили в коллекцию.
3. **Потокобезопасность** — immutable-объекты thread-safe по определению, их можно свободно делить между потоками без синхронизации.
4. **Безопасность** — строки несут пароли, пути, имена классов, URL. Будь они изменяемы, эти критические значения можно было бы подменить после проверки (TOCTOU-атака).
...
- `enum` неявно `final` (от него нельзя наследоваться) и неявно `extends Enum<E>` — поэтому добавить ему ещё один суперкласс невозможно, но интерфейсы реализовывать можно.
- Каждая константа создаётся **ровно один раз** (singleton per value) при загрузке класса — отсюда автоматическая потокобезопасность и возможность сравнивать значения через `==`.
- Поддерживает `abstract`-методы: можно задать своё поведение для каждой константы (strategy per constant), не плодя `if`/`switch`.
- Часто используется для State Machine и Strategy — подробнее в [Design Patterns](../../design-patterns/design-patterns-interview.md).
...
**Immutable объект** — объект, состояние которого нельзя изменить после создания. Любая «модификация» возвращает *новый* объект, а исходный остаётся прежним (так устроены `String`, `Integer`, `BigDecimal`). Это упрощает рассуждения о коде и автоматически даёт потокобезопасность.
