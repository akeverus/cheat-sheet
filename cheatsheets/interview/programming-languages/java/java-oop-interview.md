...
> - [ ] `instanceof` с pattern-переменной (`if (obj instanceof String s)`) не устраняет необходимость явного каста — это чисто косметическое улучшение, переменная `s` всё равно имеет тип `Object`. | Pattern matching для `instanceof` (Java 16) реально устраняет каст: переменная `s` уже имеет тип `String` в скоупе `if`-ветки. Это не косметика, а полноценная типизация. Это антипаттерн или неправильный выбор в production.
