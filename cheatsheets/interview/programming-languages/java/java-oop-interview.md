...
> [!mcq]
> - [ ] Инкапсуляция скрывает детали, наследование создаёт подклассы, полиморфизм означает переопределение методов, абстракция реализуется только через `abstract class` — без интерфейсов. | Утверждение про «только `abstract class`» неверно: `interface` — основной механизм абстракции в Java. ❌ ПОСЛЕДСТВИЕ: команда строит иерархию из одних `abstract class` → single inheritance блокирует доменное моделирование, при появлении второго родителя вынужденный рефакторинг ломает 30+ подклассов.
> - [ ] Полиморфизм — это возможность реализовывать несколько интерфейсов; наследование скрывает данные через `private`-поля; инкапсуляция объединяет состояние и поведение. | Перепутаны определения: `private`-поля — механизм инкапсуляции, не наследования; полиморфизм — «один интерфейс, разные реализации». ❌ ПОСЛЕДСТВИЕ: junior-разработчик пишет «полиморфный» сервис с `instanceof`-цепочкой → новый тип требует правки 12 мест, баг проявляется только в продакшене после релиза.
> - [x] ООП объединяет четыре принципа: инкапсуляция скрывает внутренние детали через `private` поля и публичный API, наследование строит иерархии через `extends`/`implements`, полиморфизм даёт разное поведение через overriding/overloading, абстракция скрывает реализацию через `abstract class` и `interface`. | Канонические определения четырёх столпов ООП; они работают в связке: инкапсуляция защищает данные, абстракция задаёт контракт, наследование переиспользует реализацию, полиморфизм работает с типами через единый интерфейс. ✓ ПРИМЕНЯТЬ: Spring Framework строит DI-контейнер именно на этих четырёх принципах — `BeanDefinition` (абстракция), `AbstractBeanFactory` (наследование), `BeanPostProcessor` (полиморфизм), `private`-поля бинов (инкапсуляция). 📋 ПРАВИЛО: «четыре столпа: спрятать, унаследовать, заменить, абстрагировать». 🔗 См. Q2, Q3, Q4, Q5.
> - [ ] ООП — это три принципа: инкапсуляция, наследование, полиморфизм; абстракция — следствие инкапсуляции, автоматически достигаемое скрытием деталей. | Абстракция — самостоятельный четвёртый принцип: инкапсуляция скрывает *данные*, абстракция скрывает *сложность реализации*. ❌ ПОСЛЕДСТВИЕ: архитектор пропускает абстракцию → доменный модуль завязан на JDBC напрямую без `Repository`-интерфейса, миграция на MongoDB требует переписать 80% сервисного слоя.
...
> [!mcq]
> - [x] Настоящая инкапсуляция — это скрытие не только данных, но и поведения: класс `BankAccount` не имеет `setBalance()`, потому что баланс меняется только через бизнес-операции `deposit()` и `withdraw()` с валидацией инварианта. | Инкапсуляция гарантирует, что объект всегда находится в корректном состоянии — скрытие метода такая же её часть, как и скрытие поля. ✓ ПРИМЕНЯТЬ: домены DDD в банках (Tinkoff, Альфа-Банк) и `BigDecimal`-операции в `Money` value object — `add()`/`subtract()` вместо `setAmount()`. 📋 ПРАВИЛО: «инвариант защищён операциями, не сеттерами». 🔗 См. Q1, Q5, Q36.
> - [ ] Инкапсуляция — это `private`-поля + геттеры/сеттеры для каждого поля (стандартный JavaBean-паттерн), что обеспечивает полную инкапсуляцию. | Механический JavaBean не даёт настоящей инкапсуляции: `setBalance()` открывает прямой доступ к состоянию в обход бизнес-логики. ❌ ПОСЛЕДСТВИЕ: `BankAccount.setBalance(new BigDecimal("-1000000"))` через REST endpoint Jackson + Lombok `@Data` → отрицательный баланс в проде, аудит ловит клиента-мошенника через 3 месяца, потери > 10 млн рублей.
> - [ ] Инкапсуляция и абстракция — одно и то же: оба принципа скрывают реализацию, реализуются через `private` и `protected`. | Абстракция и инкапсуляция решают разные задачи: инкапсуляция защищает *состояние*, абстракция скрывает *сложность реализации* через `interface`. ❌ ПОСЛЕДСТВИЕ: путаница на code review → разработчик «инкапсулирует» через интерфейс без `private`-полей, mutable internals leak через accessor → race condition в `ConcurrentHashMap`, потеря данных под нагрузкой 5K RPS.
> - [ ] `protected` обеспечивает более строгую инкапсуляцию, чем `package-private`, потому что ограничивает доступ только подклассами. | Наоборот: `protected` шире — это `package-private` + подклассы из любых пакетов; `package-private` строже. ❌ ПОСЛЕДСТВИЕ: разработчик помечает API-поле `protected` ради «защиты» → external library наследуется и читает поле, breaking change при следующем рефакторинге ломает downstream-клиентов на проде.
...
> [!mcq]
> - [ ] Класс в Java может наследоваться от нескольких классов через `extends A, B`, конструкторы родителя вызываются автоматически. | В Java запрещено множественное наследование классов; конструкторы родителя не наследуются — подкласс обязан явно вызвать `super(...)` первой строкой. ❌ ПОСЛЕДСТВИЕ: разработчик переписывает код с C++ → ожидает `extends Base1, Base2`, не компилируется → пытается обойти через рефлексию → `IllegalAccessError` под нагрузкой.
> - [x] Наследование реализуется через `extends`: подкласс получает все не-`private` поля и методы родителя, вызывает его конструктор через `super(...)`, может переопределять методы; нельзя наследоваться от нескольких классов и от `final`-классов. | Канонический набор правил: одиночное наследование классов, множественная реализация интерфейсов, `final`-классы (`String`, `Integer`) запрещают наследование, `final`-методы — переопределение. ✓ ПРИМЕНЯТЬ: иерархия исключений в JDK — `Throwable` → `Exception` → `RuntimeException` → `IllegalArgumentException`, Spring `AbstractApplicationContext` для шаблонного метода `refresh()`. 📋 ПРАВИЛО: «один extends, много implements, ноль для final». 🔗 См. Q11, Q12, Q21.
> - [ ] Через `extends` подкласс получает доступ к `private`-полям родителя напрямую, так как наследование переопределяет модификаторы. | `private`-члены не видны подклассу — это фундаментальное правило инкапсуляции, не меняющееся при наследовании. ❌ ПОСЛЕДСТВИЕ: разработчик пишет `subclass.privateField = ...` → не компилируется → копирует поле в подкласс → state дублируется в двух местах, баг с рассинхронизацией `balance` после `deposit()` в проде.
> - [ ] Наследование и реализация интерфейса — одно и то же: оба используют `extends`, оба разрешают множественное наследование. | Это разные механизмы: классы — `extends` (один), интерфейсы — `implements` (много); до Java 8 интерфейсы передавали только контракт. ❌ ПОСЛЕДСТВИЕ: разработчик пишет `class MyService extends UserRepository, OrderRepository` → код не компилируется → меняет на `extends UserRepository implements OrderRepository` → теряется метод `save()` Order → `NullPointerException` в продакшене при первом заказе.
...
> [!mcq]
> - [ ] Перегрузка и переопределение — синонимы: оба описывают методы с одинаковым именем, отличающиеся реализацией. | Это разные механизмы: перегрузка — разные сигнатуры в одном классе (compile-time), переопределение — та же сигнатура в подклассе (runtime). ❌ ПОСЛЕДСТВИЕ: разработчик пишет `public boolean equals(MyClass other)` без `@Override` → думает «это override», на деле перегрузка → `HashMap.contains(key)` вызывает `Object.equals` → ключи теряются, data loss в кэше под нагрузкой.
> - [ ] Полиморфизм работает только с классами, не с интерфейсами — нельзя через ссылку на интерфейс получить разное поведение. | Интерфейсы — главный инструмент полиморфизма: `Shape s = new Circle(); s.area()` — реальный метод определяется типом объекта в рантайме. ❌ ПОСЛЕДСТВИЕ: команда отказывается от интерфейсов «ради простоты» → `OrderService` зависит от `MySqlOrderRepository` напрямую → миграция на PostgreSQL ломает 40 unit-тестов, deployment откладывается на 2 спринта.
> - [x] В Java два вида полиморфизма: compile-time (перегрузка через разные сигнатуры) и runtime (переопределение через dynamic dispatch по реальному типу объекта); `static`-методы нельзя переопределить — только скрыть (method hiding). | Overloading разрешается компилятором по типам аргументов, overriding — JVM по типу объекта через vtable; `static`-методы привязаны к типу ссылки. ✓ ПРИМЕНЯТЬ: Spring `BeanPostProcessor` через runtime polymorphism применяет AOP-обёртки; Jackson `ObjectMapper` через overloading `readValue(...)` поддерживает 12 вариантов входов. 📋 ПРАВИЛО: «overload — компилятор, override — vtable, static — hiding». 🔗 См. Q15, Q16, Q17.
> - [ ] Перегрузка операторов в Java разрешена только для `+` со `String`, для остальных операторов — запрещена. | В Java нет пользовательской перегрузки операторов вообще; `+` для `String` — встроенная поддержка компилятора (через `StringBuilder.append`). ❌ ПОСЛЕДСТВИЕ: разработчик с опытом Kotlin/Scala пишет `BigDecimal a + b` ожидая overload → компилятор молчит, складывает `toString()` через `+` → `"10.001.5"` вместо `11.5` уходит в банковский reconciliation, расхождение на 100K.
...
> [!mcq]
> - [ ] Абстракция и инкапсуляция — один и тот же принцип ООП, описываемый разными словами. | Это разные принципы: инкапсуляция скрывает *данные* через модификаторы доступа, абстракция — *сложность реализации* через контракт. ❌ ПОСЛЕДСТВИЕ: архитектор объединяет понятия в design-документе → команда не вводит интерфейсов, всё инкапсулирует через `private` → unit-тесты невозможно писать без интеграции БД, CI-pipeline растёт до 40 минут.
> - [ ] Абстракция в Java — только через `abstract class`; интерфейсы не являются инструментом абстракции. | Интерфейсы — основной инструмент абстракции: задают контракт без привязки к реализации; `abstract class` — для частичной реализации со состоянием. ❌ ПОСЛЕДСТВИЕ: команда строит DI на абстрактных классах вместо интерфейсов → невозможно мокать через Mockito без CGLIB-проксей → тесты падают на финальных методах JDK 17 (StrictBeanInfo).
> - [ ] Абстракция требует хотя бы одного `abstract`-метода, иначе компилятор не считает класс абстрактным. | Класс можно объявить `abstract` без `abstract`-методов — это валидно, чтобы запретить инстанцирование; интерфейс с `default`-методами тоже может не содержать абстрактных. ❌ ПОСЛЕДСТВИЕ: разработчик добавляет фиктивный `abstract void noop()` → подклассы вынуждены реализовывать пустышку → 15 файлов с `@Override public void noop() {}`, шум в diff и code review.
> - [x] Абстракция выделяет существенные характеристики и скрывает детали реализации через `interface` или `abstract class`, задавая контракт «что делать» без «как»; инкапсуляция (скрытие данных) и абстракция (скрытие сложности) работают вместе, но решают разные задачи. | Интерфейс — чистый контракт, `abstract class` — частичная реализация со состоянием. ✓ ПРИМЕНЯТЬ: Spring `JdbcOperations` (интерфейс-контракт) и `AbstractRoutingDataSource` (абстрактная реализация с состоянием) — классический пример обоих механизмов в одном фреймворке. 📋 ПРАВИЛО: «контракт вместо реализации, что вместо как». 🔗 См. Q1, Q2, Q12.
...
> [!mcq]
> - [x] Класс — шаблон для создания объектов с полями и методами; объект — конкретный экземпляр класса в Heap; интерфейс — контракт, определяющий методы без реализации; с Java 16 для data-классов доступен `record` как компактная альтернатива. | Класс описывает структуру и поведение, объект — конкретное состояние, интерфейс — чистый контракт; `record` — специализированный класс для иммутабельных DTO. ✓ ПРИМЕНЯТЬ: Spring `BeanDefinition` — класс-шаблон, `ApplicationContext.getBean()` возвращает объект, `Repository<T>` — интерфейс-контракт, `record UserDto` — DTO с автогенерацией `equals`/`hashCode`. 📋 ПРАВИЛО: «класс — чертёж, объект — здание, интерфейс — стандарт розетки». 🔗 См. Q1, Q12, Q39.
> - [ ] Класс и интерфейс — одно и то же с точки зрения JVM, объект — просто ссылка в стеке. | На уровне JVM класс и интерфейс — разные сущности (опкоды `invokevirtual` vs `invokeinterface`); объект — структура в Heap. ❌ ПОСЛЕДСТВИЕ: разработчик хранит «маленький объект» как `Object obj = new Point(1,2)` ожидая stack allocation → escape analysis не срабатывает → создаётся 10M объектов в Heap, GC pause растёт с 50ms до 2s, latency p99 пробивает SLA.
> - [ ] Объект в Java создаётся при объявлении переменной класса (`Car myCar;`), `new` инициализирует существующий объект значениями. | Объявление переменной создаёт только ссылку (изначально `null`), объект создаётся через `new` с выделением памяти в Heap; без `new` обращение к полям даст `NullPointerException`. ❌ ПОСЛЕДСТВИЕ: `private Logger log;` без `new`/`@Autowired` → `NullPointerException` при первом вызове `log.info()` в production-методе обработки платежа, транзакция не логируется, пропадает audit trail.
> - [ ] Интерфейс в современной Java (с Java 8+) полностью эквивалентен абстрактному классу: может иметь конструкторы, поля экземпляра, `private static` init-блоки. | Интерфейс не может иметь конструктор, поля экземпляра или блоки инициализации; добавлены только `default`/`static`/`private`-методы. ❌ ПОСЛЕДСТВИЕ: разработчик пытается мигрировать `abstract class` → `interface` сохраняя поле `private final Logger log` → не компилируется → пишет `default Logger getLog() { return LoggerFactory.getLogger(...); }` создавая логгер на каждый вызов, +500ms на endpoint.
...
> [!mcq]
> - [ ] Главное преимущество ООП — лучшая производительность, чем у процедурного или функционального подхода. | ООП часто проигрывает по производительности из-за virtual dispatch и аллокаций; functional/data-oriented бывают быстрее на hot path. ❌ ПОСЛЕДСТВИЕ: команда строит OLTP-систему «как Java учит», 5 уровней абстракции от controller до persistence → latency p99 1500ms на простом read, переписывают на functional pipeline через 6 месяцев.
> - [x] ООП даёт модульность, переиспользование, расширяемость (OCP), понятность и тестируемость (через полиморфизм и DIP), но платит overhead абстракций, риском overengineering, virtual call costs и gorilla-banana problem. | Сбалансированная картина: гибридный подход — ООП для доменной модели и границ, functional для data processing, data-oriented для hot path. ✓ ПРИМЕНЯТЬ: Netflix Hystrix использует ООП для command pattern + functional Stream API для metrics aggregation; Uber H3 — data-oriented для hot path geo-indexing. 📋 ПРАВИЛО: «ООП — для доменов, functional — для данных, data-oriented — для скорости». 🔗 См. Q10, Q21, Q23.
> - [ ] Недостаток ООП — невозможность тестирования: абстракции и полиморфизм усложняют unit-тесты. | Наоборот: ООП улучшает тестируемость через полиморфизм и DIP — зависимости подменяются моками; процедурный код с глобальным состоянием тестируется хуже. ❌ ПОСЛЕДСТВИЕ: команда отказывается от интерфейсов «ради тестируемости» → завязывает service на конкретный `JdbcTemplate` → тесты требуют поднимать Testcontainers PostgreSQL, CI растёт с 3 до 25 минут на pipeline.
> - [ ] «Gorilla-banana problem» — это проблема памяти в ООП: каждый объект несёт копию иерархии классов. | Gorilla-banana — про связанность наследования («просил банан — получил гориллу с джунглями»), не про память; решается композицией. ❌ ПОСЛЕДСТВИЕ: разработчик `extends ArrayList` чтобы добавить один метод `sum()` → подкласс наследует все 50 методов List, при сериализации Jackson обходит лишние поля → JSON весит 30KB вместо 200B.
...
> [!mcq]
> - [ ] Композиция и агрегация — полные синонимы, разница только в терминологии UML. | Различие семантическое: композиция — сильное владение (часть создаётся и уничтожается целым), агрегация — слабое (часть передаётся извне и живёт независимо). ❌ ПОСЛЕДСТВИЕ: разработчик пишет `Order { List<Product> products = new ArrayList<>() }` где Product — глобальный каталог → при удалении заказа cascade-delete уносит товары из каталога, прайс-лист восстанавливают из бэкапа.
> - [ ] В агрегации часть уничтожается с целым, в композиции — живёт независимо. | Наоборот: композиция — сильная связь с общим жизненным циклом, агрегация — слабая. ❌ ПОСЛЕДСТВИЕ: junior реализует JPA-сущность `@OneToMany(cascade = ALL)` для Customer → Address ожидая «слабую связь» → удаление клиента уносит адрес, который нужен для аудита GDPR-запросов, штраф 4% годовой выручки.
> - [x] Композиция — сильное владение (часть создаётся целым и уничтожается вместе с ним, закрашенный ромб ◆), агрегация — слабое владение (часть передаётся извне и живёт независимо, незакрашенный ромб ◇); оба относятся к отношению has-a. | Практический признак композиции — поле создаётся внутри конструктора целого (`new Room()`), агрегации — приходит через конструктор/сеттер извне. ✓ ПРИМЕНЯТЬ: JPA `@OneToMany(cascade = ALL, orphanRemoval = true)` — композиция (Order owns OrderLine); `@ManyToMany` Customer-Product — агрегация (товар живёт в каталоге). 📋 ПРАВИЛО: «композиция — общая судьба, агрегация — общая работа». 🔗 См. Q9, Q10.
> - [ ] Композиция возможна только между классами одной иерархии наследования. | Иерархия наследования не связана с композицией/агрегацией; оба — has-a через поле-ссылку, независимо от иерархии. ❌ ПОСЛЕДСТВИЕ: архитектор отказывается от композиции `Order has-a PaymentGateway` потому что они «в разных пакетах» → строит наследование `OrderWithPayment extends Order` → 8 подклассов на каждый платёжный провайдер, exponential blowup при добавлении валют.
...
> [!mcq]
> - [ ] is-a — это композиция (поле-ссылка), has-a — наследование (`extends`). | Перепутаны определения: is-a — наследование (`Dog extends Animal`), has-a — композиция/агрегация (поле `Engine` внутри `Car`). ❌ ПОСЛЕДСТВИЕ: разработчик путает определения на собеседовании → проектирует `Car extends Engine` (потому что «машина имеет двигатель») → не компилируется в production-сценарии с электрокаром, переделка занимает 2 недели.
> - [ ] is-a и has-a — оба через `implements`: класс «имеет» интерфейс и «является» его реализацией. | `implements` — это is-a с интерфейсом (`Car is-a Drivable`); has-a выражается через поле-ссылку, не через `implements`. ❌ ПОСЛЕДСТВИЕ: архитектор моделирует «Car has-a Engine» через `Car implements Engine` → компилятор требует реализовать все методы Engine в Car → дублирование контракта, при добавлении turbocharger ломаются все 12 классов автомобилей.
> - [ ] LSP применим только к has-a: проверяет, что агрегированный объект можно заменить совместимым; для is-a LSP не работает. | LSP — тест именно для is-a (наследования): если подкласс нельзя подставить вместо суперкласса без нарушения контракта, иерархия некорректна. ❌ ПОСЛЕДСТВИЕ: команда не проверяет иерархию через LSP → `Square extends Rectangle` проходит code review → `Rectangle r = new Square(); r.setWidth(5); r.setHeight(10)` даёт area=100 в графическом редакторе, фигура едет, баг репортит 200 пользователей за день.
> - [x] is-a реализуется через наследование (`class Dog extends Animal`), has-a — через композицию/агрегацию (поле-ссылка); тест корректности is-a — принцип Лисков: подкласс должен безопасно заменять суперкласс; классический антипример — `Square extends Rectangle`, где `setWidth()` у квадрата ломает контракт. | Практическое правило: если нельзя сказать «X является Y» без оговорок, нужна композиция, а не наследование. ✓ ПРИМЕНЯТЬ: JDK `LinkedHashMap extends HashMap` — корректное is-a (LSP сохранён); Spring `JdbcTemplate has-a DataSource` — корректное has-a (composition over inheritance). 📋 ПРАВИЛО: «is-a — extends + LSP, has-a — поле + делегирование». 🔗 См. Q10, Q21.
...
> - [ ] `instanceof` с pattern-переменной (`if (obj instanceof String s)`) не устраняет необходимость явного каста — это чисто косметическое улучшение, переменная `s` всё равно имеет тип `Object`. | Pattern matching для `instanceof` (Java 16+) полностью устраняет каст: `s` имеет тип `String` в scope `if`-ветки благодаря flow scoping. Это не косметика, а type narrowing на уровне компилятора. ❌ ПОСЛЕДСТВИЕ: разработчик пишет `((String) obj).length()` после `instanceof obj` ради «совместимости» — двойной каст в hot path, JIT не может вырезать redundant checkcast, latency растёт на 5-10%.
...
> - [ ] Ковариантный возвращаемый тип доступен только в интерфейсах с `default`-методами, а в обычных классах возвращаемый тип переопределённого метода должен строго совпадать с родительским. | Ковариантность работает в обычных классах с Java 5 — задолго до появления `default`-методов (Java 8); применима к классам и интерфейсам одинаково. ❌ ПОСЛЕДСТВИЕ: разработчик отказывается от Builder-паттерна с правильной типизацией ради «совместимости» — каждый вызов `.build()` требует cast, IDE подсвечивает unchecked, code review грубо отвергает PR.
...
> - [x] `Sealed`-классы (Java 17) ограничивают список разрешённых подтипов через `permits`, моделируя ADT; компилятор знает все варианты для исчерпывающего `switch` без `default`; подклассы — в том же пакете/модуле, объявлены `final`/`sealed`/`non-sealed`; пример — `sealed interface Result<T> permits Success, Failure`. | Связь с pattern matching `switch` (Java 21) — ключевая мотивация: компилятор гарантирует exhaustiveness. ✓ ПРИМЕНЯТЬ: API state machines (`OrderStatus`), error-handling без exceptions (`Result<T>` Rust-стиль), Visitor-replacement в DSL. 📋 ПРАВИЛО: «sealed = closed type hierarchy → exhaustive switch без default». 🔗 См. Q40 (records), Q42 (pattern matching в switch).
