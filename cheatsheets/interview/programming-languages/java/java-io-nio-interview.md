...
> [!mcq]
> - [ ] `Java IO` использует буферы, а `Java NIO` работает напрямую с потоками байтов. | Это противоположно действительности. Именно `Java IO` работает с потоками байтов последовательно, а `Java NIO` вводит буферно-ориентированный подход с `ByteBuffer`. ❌ ПОСЛЕДСТВИЕ: команда выбирает `Java IO` для high-load API gateway «потому что NIO работает с сырыми байтами». 5000 RPS — каждое соединение блокирует поток, GC не справляется, p99 растёт с 50ms до 2s, OOM через 4 часа. Twitch 2018: похожий паттерн — миграция на NIO/Netty снизила количество потоков с 8000 до 32.
> - [ ] Оба API блокирующие, но `Java NIO` поддерживает более широкий набор кодировок. | Кодировки не являются ключевым отличием. Главное различие — модель данных (поток vs буфер) и возможность неблокирующего I/O в `NIO`. ❌ ПОСЛЕДСТВИЕ: разработчик выбирает технологию по «поддержке кодировок», игнорируя модель блокировки. На production сервер не масштабируется выше 200 connections — thread-per-connection упирается в лимит ОС. LinkedIn 2014: переписали legacy IO-сервер на Netty, уменьшили количество потоков в 100 раз.
> - [x] `Java IO` работает с данными последовательно (поток), а `Java NIO` читает данные порциями в буфер, который затем обрабатывается программой. | `Java IO` (java.io) — потоково-ориентированная модель: байт за байтом. `Java NIO` (java.nio) — буферо-ориентированная: данные попадают в `Buffer`, после чего программа обрабатывает их в произвольном порядке. ✓ ПРИМЕНЯТЬ: Netty/Spring WebFlux строят высоконагруженные серверы на NIO `Selector` (один поток обслуживает 10000+ соединений); Apache Kafka использует `FileChannel` + memory-mapped buffers для zero-copy переноса данных между диском и сетью. 📋 ПРАВИЛО: «IO = stream of bytes, NIO = buffered chunks + multiplexing». 🔗 См. Q3 (блокирующий vs неблокирующий), Q10 (Channel + Buffer), Q11 (типы Channel).
> - [ ] `Java IO` поддерживает только файлы, а `Java NIO` — только сетевые соединения. | Оба API поддерживают как файлы, так и сетевые соединения. `Java IO` имеет `Socket`/`ServerSocket`, а `Java NIO` — `FileChannel`, `SocketChannel`, `DatagramChannel`. ❌ ПОСЛЕДСТВИЕ: команда сохраняет два разных API в проекте «один для файлов, другой для сети» по неверной аналогии. Дублирование зависимостей, путаница в layered code, baeldung-style copy-paste багов с ресурсами. Проект с 200K LOC превращается в зоопарк IO-классов.
...
> [!mcq]
> - [ ] В потоково-ориентированном подходе данные читаются порциями в буфер, и можно перемещаться вперёд и назад. | Это описание буферо-ориентированного подхода (`Java NIO`). В потоково-ориентированном (`Java IO`) данные обрабатываются последовательно и прочитанные байты недоступны повторно. ❌ ПОСЛЕДСТВИЕ: разработчик ожидает, что `InputStream.read()` можно «переиграть назад», и пишет код, повторно вызывающий парсер на тех же данных. На bench всё работает (маленький файл переоткрывается), на проде с 5GB файлом — `EOFException`, потому что поток уже исчерпан. Логи показывают «странные» NPE на десериализации.
> - [x] В потоково-ориентированном подходе нельзя вернуться к прочитанным данным, а в буферо-ориентированном можно управлять позицией через `position`, `mark`, `reset`. | В `Java IO` поток однонаправлен: прочитанный байт ушёл. В `Java NIO` `ByteBuffer` хранит данные, и `position`/`mark`/`reset` позволяют перечитывать их в любом порядке. ✓ ПРИМЕНЯТЬ: парсеры бинарных протоколов (Kafka wire protocol, Cassandra CQL frame) используют `ByteBuffer.mark()`/`reset()` для look-ahead — заглянуть вперёд, проверить magic byte, при несоответствии откатиться. Spring WebFlux `DataBuffer` строится поверх той же модели. 📋 ПРАВИЛО: «stream = forward-only, buffer = random-access». 🔗 См. Q1 (поток vs буфер), Q10 (Channel + Buffer), Q11 (типы Channel).
> - [ ] В буферо-ориентированном подходе компилятор автоматически выбирает размер буфера, а в потоково-ориентированном размер задаётся вручную. | Размер буфера в `NIO` задаётся программистом явно (`ByteBuffer.allocate(1024)`). Компилятор никак не влияет на размер буфера в runtime. ❌ ПОСЛЕДСТВИЕ: команда полагается на «автоматику», использует `ByteBuffer.allocate(1)` или огромный буфер. При 1 байте каждое чтение syscall, throughput падает в 100 раз; при 100MB direct buffer — `OutOfMemoryError: Direct buffer memory`, потому что direct memory не считается в Heap. Yandex 2017: типовой инцидент с тюнингом буферов в высоконагруженном search-сервисе.
> - [ ] В потоково-ориентированном подходе можно читать несколько буферов одновременно (`scatter/gather`), а буферо-ориентированный работает только с одним каналом. | `Scatter/gather` — это возможность `Java NIO`, буферо-ориентированного API. Потоковый `Java IO` не поддерживает scatter/gather операции. ❌ ПОСЛЕДСТВИЕ: разработчик пытается реализовать «scatter» поверх `InputStream` цепочкой `read()` в разные `byte[]` без атомарности. Между чтениями тред переключается, данные интерливятся при concurrent access — гонки в парсере HTTP/2-фрейма. Netty решает это через `gathering write` на `GatheringByteChannel`.
...
> [!mcq]
> - [x] `Java IO` рекомендуется для простых файловых операций и небольшого числа соединений, а `Java NIO` — для высоконагруженных серверов с тысячами соединений. | `Java IO` прост в использовании и достаточен для типичных файловых задач. `Java NIO` с `Selector` позволяет одному потоку обслуживать тысячи соединений, что критично для серверов. ✓ ПРИМЕНЯТЬ: CLI-утилиты (читают конфиг, парсят CSV) — `Files.readAllLines()` поверх Java IO; high-load proxy/gateway — Netty/Undertow поверх NIO. Spring WebFlux выбирает NIO именно для тысяч concurrent соединений; Spring MVC (servlet) использует thread-per-request — модель IO. 📋 ПРАВИЛО: «IO для скриптов и батчей, NIO для серверов с N>1000 connections». 🔗 См. Q1 (модели), Q3 (блокировка), Q11 (типы Channel).
> - [ ] `Java IO` рекомендуется для высоконагруженных серверов, потому что проще отлаживать. | Простота отладки не делает `Java IO` лучше для высоких нагрузок. Модель «поток на соединение» плохо масштабируется: при тысячах соединений создаются тысячи потоков. ❌ ПОСЛЕДСТВИЕ: команда выбирает thread-per-connection «потому что проще». На 5000 RPS JVM создаёт 5000 платформенных потоков, OS планировщик тратит 30% CPU на context switch, GC stop-the-world растёт до 500ms. Twitch 2018: миграция на Netty снизила количество потоков с 8000 до 32, p99 упал с 800ms до 50ms.
> - [ ] `Java NIO` всегда быстрее `Java IO` вне зависимости от сценария использования. | Это неверно. Для простых последовательных операций с файлами `Java IO` может быть не медленнее `NIO`. `NIO` эффективнее при большом числе параллельных соединений или при использовании `direct buffer` / `MappedByteBuffer`. ❌ ПОСЛЕДСТВИЕ: разработчик переписывает простой скрипт-копировщик файлов с `BufferedReader` на `FileChannel` + `ByteBuffer`. Код становится в 3 раза длиннее, появляется баг с `flip()`/`clear()`, файл копируется на 2% быстрее. ROI отрицательный — дни разработки + баги ради микрооптимизации, не нужной никому.
> - [ ] `Java NIO` не поддерживает буферизацию символьных данных, только бинарные потоки. | `Java NIO` поддерживает `CharBuffer` и `CharsetEncoder`/`CharsetDecoder` для работы с символами. `Files.newBufferedReader()` возвращает `BufferedReader`, построенный поверх NIO. ❌ ПОСЛЕДСТВИЕ: команда отказывается от NIO для текстовых файлов «потому что нет работы с символами», теряет преимущество `MappedByteBuffer` при чтении 100GB логов. Альтернативный `BufferedReader` грузит файл по 8KB, тогда как memory-mapped даёт OS-level кеширование и сокращает время в 5-10 раз.
