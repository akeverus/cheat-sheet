...
**Аннотация** -- это метка-метаданные на элементе кода (классе, методе, поле, параметре), которая сама по себе ничего не делает: она лишь несёт информацию для того, кто её прочитает. Сам код от аннотации не меняется -- меняется поведение инструментов, которые её видят. Это форма метапрограммирования, введённая в `Java 5` (JSR 175) и ставшая фундаментом современной разработки, особенно в [Java Core](java-core-interview.md) и фреймворках вроде `Spring`.

Кто и когда читает аннотацию -- зависит от того, на каком этапе она «живёт» (см. `RetentionPolicy`, Q8):
- **Компилятор** -- `@Override` заставляет проверить, что метод действительно переопределяет метод суперкласса (иначе ошибка компиляции).
...
Большинство «стандартных» аннотаций живут в `java.lang` (применяются к коду) и `java.lang.annotation` (мета-аннотации, см. Q7). Самые ходовые на собеседовании -- три «компиляторные» (`@Override`, `@Deprecated`, `@SuppressWarnings`):
...
Обратите внимание на колонку `RetentionPolicy`: `@Override` и `@SuppressWarnings` -- чисто компиляторные (`SOURCE`), их в байт-коде уже нет. А `@Deprecated` -- `RUNTIME`, потому что IDE и инструменты анализа должны видеть пометку об устаревании и в скомпилированных зависимостях.

Начиная с `Java 9` у `@Deprecated` появились атрибуты `since` (с какой версии устарело) и `forRemoval` (помечено ли к удалению -- усиливает предупреждение):
...
Тип элемента аннотации строго ограничен набором **константных типов**. Причина -- значения элементов компилятор «зашивает» в байт-код как константы, поэтому произвольные объекты сюда не подходят. Допустимы только:

- Примитивные типы (`int`, `long`, `boolean` и т.д.)
- `String`
- `Class` (можно с wildcards: `Class<? extends Serializable>`)
- `Enum`
- Другая аннотация
- Массив любого из перечисленных выше типов
...
**НЕ допустимы** (даже близкие к разрешённым): `Object`, `Map`, `List`, обёртки примитивов (`Integer`, `Boolean` -- нужны именно примитивы), произвольные пользовательские классы. Частая ловушка -- попытаться использовать `Integer` вместо `int`:
...
Это два разных механизма, оба про удобство записи. `value()` -- элемент с особым именем: если при использовании задаётся только он, имя можно опустить и писать `@Tag("important")` вместо `@Tag(value = "important")`. `default` задаёт значение по умолчанию, чтобы элемент можно было не указывать вовсе.

Элемент `value()` -- единственное имя, которое разрешено опускать:
...
**Мета-аннотация** -- это аннотация над аннотацией: она навешивается не на обычный код, а на объявление другой аннотации и настраивает её поведение (где можно применять, до какого этапа доживает, наследуется ли). Стандартный набор в `Java` -- пять штук:
...
**Как выбрать** (по принципу «минимально необходимая retention»):
- **`SOURCE`** -- аннотация нужна только компилятору или `Annotation Processor` для генерации кода и проверок; в байт-коде её быть не должно. Самый «лёгкий» вариант.
- **`CLASS`** (значение по умолчанию, если `@Retention` не указан) -- нужна инструментация байт-кода, но не runtime-рефлексия. Лежит в `.class`, но JVM её в память не загружает.
- **`RUNTIME`** -- единственный вариант, при котором аннотацию видно через рефлексию; обязателен для `Spring`, `JPA`, `Jackson` и любого кода, который читает аннотации во время выполнения.
...
`@Target` -- мета-аннотация, которая задаёт **белый список** мест, куда можно ставить вашу аннотацию. Применить её к чему-то вне этого списка -- ошибка компиляции (а не молчаливое игнорирование). Список мест задаётся константами enum `ElementType`:
...
Обе -- мета-аннотации без атрибутов (маркеры), но решают разные задачи.

`@Documented` указывает, что аннотацию надо показывать в `Javadoc` сгенерированной документации. Без неё аннотация в Javadoc не попадёт. Ставится на аннотации, которые являются частью публичного контракта API.

`@Inherited` делает так, что аннотация, поставленная на класс, **видна и на его подклассах** при чтении через рефлексию -- как будто подкласс тоже ею помечен:
...
`@Inherited` работает **только** для аннотаций на классе и только по цепочке `extends`. Это узкое и часто неожиданное правило -- именно на нём ловят кандидатов. Что НЕ покрывается:
...
Это две новые цели `@Target` из `Java 8`, расширившие места, куда можно ставить аннотации, -- они открыли дорогу инструментам проверки типов вроде `Checker Framework` и `NullAway`.

`TYPE_USE` разрешает ставить аннотацию на **любое упоминание типа**, а не только на объявление переменной: на аргумент дженерика, элемент массива, тип в `cast` и в `throws`:
...
**Повторяемая аннотация** (`Java 8+`) -- это аннотация, которую разрешено применить к одному элементу несколько раз (`@Schedule(...)` дважды подряд). До `Java 8` это было запрещено. Под капотом ничего магического: компилятор просто **сворачивает** несколько одинаковых аннотаций в один массив внутри аннотации-контейнера. Поэтому нужны два объявления -- сама повторяемая аннотация (помечается `@Repeatable`) и контейнер, хранящий их массив:
...
**Составная (composed) аннотация** -- это своя аннотация, на которую навешаны другие аннотации; она объединяет их смысл под одним именем, чтобы не повторять набор каждый раз. Аналог наследования, которого у аннотаций нет (см. Q15). `Spring` построен на этом паттерне:
...
Нет, наследование аннотаций невозможно. Каждая аннотация уже неявно расширяет `java.lang.annotation.Annotation`, и явное `extends` в объявлении -- ошибка компиляции. То есть нельзя сказать «моя аннотация наследует все элементы другой».

Вместо наследования используют один из обходных приёмов:
...
Через `Reflection API` -- вызовом методов вроде `getAnnotation()` / `isAnnotationPresent()` на `Class`, `Method` или `Field`. Главное условие: аннотация обязана иметь `@Retention(RetentionPolicy.RUNTIME)`, иначе в скомпилированном классе её просто нет и рефлексия её не увидит. Основные методы:
...
Разница в одном слове -- `Declared`: методы с `Declared` смотрят **только на собственные аннотации элемента** и игнорируют унаследованные через `@Inherited`. Методы без `Declared` поднимаются по иерархии классов и учитывают `@Inherited`. Сводно:
...
Готового API в JDK для этого **нет**: рефлексия умеет читать аннотации только у уже загруженного класса, а перебрать «все классы пакета» стандартными средствами нельзя -- classloader не обязан знать полный список своих классов. Поэтому используют сторонние сканеры, которые сами обходят classpath (jar-файлы и каталоги) и читают байт-код:
...
Двумя способами. Современный (с [Java 8](java-8-interview.md)) -- через объекты `Parameter`: `method.getParameters()` и затем `param.getAnnotation(...)`. Старый -- через двумерный массив `method.getParameterAnnotations()`, где внешний индекс соответствует номеру параметра.

Отдельный нюанс: **имена** параметров (`param.getName()`) доступны только если класс скомпилирован с флагом `-parameters` -- иначе вернутся синтетические `arg0`, `arg1`. На аннотации это не влияет, они доступны всегда.
...
Процессор -- это класс, наследующий `AbstractProcessor` и переопределяющий метод `process()`; в нём через `RoundEnvironment` вы получаете аннотированные элементы, а через `Filer` пишете сгенерированные файлы. Соберём минимальный процессор, который по аннотации `@GenerateBuilder` создаёт класс-билдер.

Шаг 1 -- объявить аннотацию. Ей достаточно `SOURCE` retention: после генерации кода она не нужна:
...
**Раунд** -- один проход обработки аннотаций. Раунды нужны потому, что процессор может сгенерировать новые исходники, а в них тоже могут быть аннотации, которые надо обработать. Поэтому компилятор повторяет проходы, пока не перестанут появляться новые файлы. Каждый раунд:
...
`javac` находит процессоры через механизм SPI (`ServiceLoader`): нужно лишь объявить класс процессора в специальном файле сервисов. Сделать это можно вручную или сгенерировать автоматически -- отсюда два способа.

**Способ 1 -- файл `META-INF/services` (стандартный SPI, вручную):**
...
Главное ограничение: API **только добавляет, но не правит**. Процессор может создавать новые файлы, но не имеет права трогать уже написанные исходники -- именно поэтому, например, Lombok вынужден идти в обход (см. ниже). Полный список:
...
Очень многие -- везде, где код выгоднее сгенерировать при сборке, чем строить через рефлексию в runtime. Самые известные:
...
`Spring AOP` реализует декларативное поведение через **прокси**: увидев на бине аннотацию вроде `@Transactional`, `@Cacheable` или `@Async`, контейнер подменяет бин обёрткой-прокси. Вызов метода идёт сначала в прокси -- тот выполняет «обвязку» (открыть транзакцию, проверить кэш и т.д.), а затем делегирует реальному методу. Сам ваш код об этом ничего не знает.
...
Паттерн всегда один: своя `RUNTIME`-аннотация-маркер плюс аспект, который её «ловит» через pointcut `@annotation(...)`. Разберём на примере аннотации для логирования времени выполнения метода:
...
`Bean Validation` (JSR 380, эталонная реализация -- `Hibernate Validator`) позволяет описывать правила проверки прямо на полях класса аннотациями, а сам процесс валидации выносит в отдельный движок `Validator`. Вместо ручных `if (name == null) throw ...` вы пишете `@NotNull`, и нарушения собираются в список `ConstraintViolation`. Все аннотации валидации имеют `RUNTIME` retention -- иначе движок их не прочитал бы.
...
Нужны две вещи -- аннотация-«ярлык» и класс с самой логикой проверки, связанные через `@Constraint`:
1. Объявить аннотацию, помеченную `@Constraint(validatedBy = ВашВалидатор.class)`. Она обязана иметь элементы `message()`, `groups()`, `payload()` -- этого требует контракт Bean Validation.
2. Реализовать `ConstraintValidator<A, T>`, где `A` -- ваша аннотация, `T` -- тип проверяемого значения; вся проверка живёт в методе `isValid()`.
...
Аннотации `Jackson` -- это способ настроить маппинг между Java-объектом и JSON, не выходя в код сериализатора: переименовать поле, скрыть его, задать формат даты, указать конструктор для десериализации. Без них `Jackson` маппит «как есть» (по именам полей/геттеров), аннотации лишь корректируют это поведение:
...
Аннотации `JPA` -- это объектно-реляционный маппинг, заданный декларативно: вы помечаете класс `@Entity`, поля -- `@Column`/`@Id`/`@ManyToOne`, и провайдер `JPA` понимает, как сохранять объект в таблицу и читать обратно. `Hibernate` -- самая распространённая реализация `JPA` -- поддерживает стандартные аннотации и добавляет свои (`@CreationTimestamp` и др.):
...
Обе -- стандартные аннотации-обещания компилятору, но про разные вещи.

`@SafeVarargs` -- это ваше обещание, что метод с varargs-параметром дженерик-типа не вызовет **heap pollution** (порчу кучи из-за стирания типов, см. [дженерики](java-generics-interview.md)); компилятор в ответ убирает соответствующее предупреждение. Ставится только на методы/конструкторы, которые нельзя переопределить, -- `final`, `static` или `private`:
...
`@FunctionalInterface` -- обещание, что интерфейс пригоден для лямбды: компилятор проверяет, что в нём ровно **один** абстрактный метод, и даёт ошибку, если кто-то случайно добавит второй (подробнее в [вопросах по Java 8](java-8-interview.md)). `default`- и `static`-методы не считаются и контракт не нарушают:
...
Короткий ответ: сами по себе -- почти никак. Аннотации `SOURCE` и `CLASS` в runtime вообще не существуют (JVM их не загружает), поэтому стоят ноль. Реальная цена возникает только при **чтении** `RUNTIME`-аннотаций через рефлексию, и фреймворки её гасят кэшированием.

Что именно стоит у `RUNTIME`-аннотаций:
...
**APT** (Annotation Processing Tool) — встроенный в `javac` механизм, который во время компиляции запускает ваш код: процессоры читают аннотации в исходниках и могут генерировать новые исходники/ресурсы или выдавать ошибки. Ключевое -- понимать, *куда* в конвейере javac встроен APT: он работает после построения AST, но до финальной генерации байт-кода, и повторяется раундами, пока появляются новые файлы.
...
**Lombok** регистрируется как annotation processor, но делает то, что обычному процессору запрещено: вместо генерации новых файлов он **правит уже существующий класс** -- дописывает в него геттеры, сеттеры, `toString` и т.д. Достигается это вмешательством в AST (дерево синтаксического разбора) javac во время компиляции.
...
**MapStruct** на этапе компиляции читает интерфейс-маппер с аннотациями `@Mapper`/`@Mapping` и **генерирует обычный Java-класс**-реализацию с прямыми вызовами геттеров и сеттеров -- без рефлексии в runtime. Вы описываете «что во что мапить», APT пишет рутинный код преобразования за вас.
...
**KSP** (Kotlin Symbol Processing) — официальный от JetBrains движок обработки аннотаций для Kotlin (с Kotlin 1.5+), пришедший на смену KAPT. Главное преимущество: KSP работает **напрямую с символами Kotlin**, тогда как KAPT сначала генерирует Java-заглушки для всего кода и гоняет по ним обычный Java-APT -- лишний и медленный шаг.
...
До **Java 8** одну и ту же аннотацию нельзя было поставить на элемент дважды. `@Repeatable` снимает запрет: вы помечаете аннотацию как повторяемую и указываете для неё **container annotation** -- скрытую аннотацию-обёртку, в массив которой компилятор складывает все повторения.
...
**Важный нюанс:** `getAnnotation(Role.class)` ненадёжен для повторяемых аннотаций. Если `@Role` применена дважды и более, в байт-коде лежит уже не `@Role`, а контейнер `@Roles`, поэтому `getAnnotation(Role.class)` вернёт `null` (искать пришлось бы `getAnnotation(Roles.class)`). Если же `@Role` применена один раз -- контейнера нет, и тот же вызов вернёт её напрямую. Чтобы не зависеть от количества применений, всегда используйте `getAnnotationsByType(Role.class)` -- он сам разворачивает контейнер и в любом случае отдаёт массив всех `@Role`.

---

## Q43. Аннотации на TYPE_USE — @NonNull и Checker Framework

`TYPE_USE` (`ElementType.TYPE_USE`, Java 8+) -- цель `@Target`, разрешающая ставить аннотацию на **любое упоминание типа**, а не только на объявление переменной: на аргумент дженерика, элемент массива, тип в `cast` и в `throws`. Это нужно прежде всего инструментам проверки nullability (`Checker Framework`, `NullAway`), которые на основе таких аннотаций ловят NPE ещё на этапе компиляции.
