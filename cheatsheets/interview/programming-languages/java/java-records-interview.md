...
- Дополнительные (не канонические) конструктоrы обязаны через `this(...)` делегировать в канонический — мимо него поля инициализировать нельзя.
...
**Граничный случай:** если annotation-таргет неоднозначен (Jackson не понимает, к полю это или к методу), помогает явный `@JsonProperty` либо `@field:`/`@get:`-таргетинг в Kotlin. Для `kotlinx.serialization` — `@Serializable` на record работает аналогично data class.
...
**Почему это важно на собеседовании:** в паре с sealed-интерфейсом (Q5, Q13) деконструкция даёт исчерпывающий `switch` — компилятор проверит, что обработаны все варианты, и не потребует `default`. Это и есть «функциональный» способ работы с данными в Java: вместо цепочки `instanceof` + явных приведений — одно компактное выражение.
...
Компилятor генерирует для record **равенство по значению**: `equals()` возвращает `true`, если объект того же типа и все его компоненты попарно равны (через их собственный `equals()`), а `hashCode()` вычисляется из всех компонентов. Контракт `equals`/`hashCode` соблюдается автоматически — равные объекты гарантированно имеют одинаковый хеш, поэтому record безопасно использовать ключом в `HashMap`/`HashSet`.
...
**Рекомендация:** для межсервисного обмена и хранения предпочитайте JSON/MessagePack через Jackson или kotlinx.serialization, а не встроенную Java Serialization — она хрупка к изменению структуры, небезопасна и привязывает формат к JVM.
