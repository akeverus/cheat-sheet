...
> - [ ] `Queue` не расширяет `Collection`, потому что работает с FIFO-семантикой, несовместимой с методом `add(E)` | `Queue` как раз расширяет `Collection`. Метод `add(E)` унаследован и используется. Именно `Map` находится за пределами иерархии `Collection`.
...
> - [x] `SortedSet` расширяет `Set`, добавляя навигационные методы `first()`, `last()`, `subSet()`, а `NavigableSet` расширяет `SortedSet`, добавляя `lower()`, `floor()`, `ceiling()`, `higher()` | Это корректная цепочка: `Set → SortedSet → NavigableSet`. `TreeSet` реализует `NavigableSet`. Именно так выстроена иерархия в исходниках JDK.
> - [ ] `NavigableSet` расширяет `List`, потому что предоставляет навигацию по индексу через методы `floor()` и `ceiling()` | `floor()` и `ceiling()` ищут ближайшие элементы по значению, а не по индексу. `NavigableSet` расширяет `SortedSet`, а не `List`.
> - [ ] `Deque` расширяет `List`, потому что поддерживает доступ к элементам с обоих концов аналогично `ArrayList` | `Deque` расширяет `Queue`, а не `List`. Доступ с обоих концов — не то же самое, что произвольный доступ по индексу, который есть у `List`.
...
> - [ ] `TreeSet.add(element)` — `O(1)` амортизированно, потому что красно-чёрное дерево самобалансируется и большинство вставок не требуют ротаций | Балансировка не делает дерево константным. Каждая вставка требует обхода от корня до листа — `O(log n)` — вне зависимости от частоты ротаций.
...
> [!mcq]
> - [ ] `Map` не наследуется от `Collection`, потому что его размер ограничен — пары ключ-значение занимают вдвое больше памяти, поэтому потребовался отдельный интерфейс | Причина не в памяти. `HashMap` может хранить любое количество пар. Разделение интерфейсов — семантическое, а не техническое.
...
> - [ ] `Map` не наследуется от `Collection`, потому что не допускает дубликаты, тогда как `Collection` дубликаты разрешает | Дубликаты разрешает `List`, но не `Set`, а `Set` — это `Collection`. То есть отсутствие дубликатов — не причина быть вне `Collection`.
...
> - [x] `Iterable<T>` содержит единственный абстрактный метод `iterator()`, возвращающий `Iterator<T>`. Именно его вызывает JVM при входе в enhanced `for-loop` | `forEach()` и `spliterator()` — default-методы Java 8+, добавленные для удобства. Контракт `Iterable` определяется только через `iterator()`.
...
> - [ ] `Iterable<T>` содержит единственный абстрактный метод `hasNext()`, возвращающий `boolean` — отсюда работает проверка границ в `for-each` | `hasNext()` — метод `Iterator<T>`, а не `Iterable<T>`. Это разные интерфейсы: `Iterable` создаёт итератор, `Iterator` управляет обходом.
...
> - [ ] Новый массив создаётся в 1.25 раза больше: `newCapacity = oldCapacity + (oldCapacity >> 2)`. Сдвиг вправо на 2 даёт деление на 4, что используется в `StringBuilder` | Именно `StringBuilder` расширяется на 25% — это другой класс с другой стратегией. `ArrayList` использует сдвиг на 1 (деление на 2), а не на 2 (деление на 4).
...
> [!mcq]
> - [ ] Бросает `ConcurrentModificationException` — список изменился, итератор сломан | ConcurrentModificationException бросают fail-fast итераторы; у COWAL итератор работает на снимке и не проверяет modCount
> - [ ] Видит новый элемент при следующем вызове `next()` — список обновляет снимок автоматически | Снимок никогда не обновляется после создания итератора; это был бы итератор со слабой согласованностью, как у ConcurrentHashMap
> - [x] Продолжает работу со снимком на момент создания; новый элемент не виден итератору | COWAL при каждой записи создаёт новый массив; итератор держит ссылку на старый массив-снимок, который остаётся неизменным
> - [ ] Блокируется до завершения `add()`, затем продолжает с обновлённым состоянием | COWAL не использует read-lock; именно отсутствие блокировок при чтении — главное преимущество перед synchronizedList
...
> - [ ] Напечатает `x`, но `arr[0]` останется `a` — список хранит копии строк, а не ссылки на элементы массива | Список хранит ссылки, а не копии объектов. `String` — не примитив; в массиве `arr[0]` хранится ссылка на строку, и именно эту ссылку заменяет `list.set()`.

> [!mcq]
> - [ ] `List.of("a", null)` — допустим, потому что `null` означает «элемент отсутствует», а не «значение null», и это семантически валидно | `List.of()` вообще не допускает `null` ни по какой семантике. При встрече с `null` немедленно бросается `NullPointerException` — это fail-fast контракт.
> - [ ] `List.of("a", null)` — допустим, как и в `ArrayList`; `null` разрешён в любом `List` по контракту интерфейса | Интерфейс `List` не требует поддержки `null` — реализация вправе запретить его. `ArrayList` допускает `null`, но `List.of()` — нет.
...
> - [ ] `List.of("a", null)` возвращает `["a"]` — `null` тихо отфильтровывается, как в некоторых других коллекциях | Ни одна стандартная Java-коллекция не фильтрует `null` тихо при вставке. Это либо разрешено, либо запрещено с исключением.
...
> [!mcq]
> - [ ] `O(1)` в среднем — внутри использует хэш-таблицу, как `HashSet` | TreeSet основан на красно-чёрном дереве, а не хэш-таблице; O(1) — это HashSet
> - [ ] `O(n)` в худшем случае — при вырожденном дереве с одной длинной ветвью | Красно-чёрное дерево поддерживает баланс автоматически при каждой вставке/удалении; вырожденный вариант невозможен
> - [x] `O(log n)` всегда — красно-чёрное дерево самобалансируется, высота ≤ 2·log(n+1) | Гарантированный логарифм отличает TreeSet от HashSet; за предсказуемость платят постоянным коэффициентом по сравнению с O(1) HashSet
> - [ ] `O(log n)` амортизированно — изредка требует полной перебалансировки за `O(n)` | Перебалансировка красно-чёрного дерева — не более O(log n) вращений; полной перестройки никогда не происходит
...
> - [ ] До Java 8 при коллизиях использовалось красно-чёрное дерево с порогом 4, с Java 8 порог увеличили до 8 для баланса с памятью | В Java 8 красно-чёрное дерево впервые появилось в `HashMap`. До этого использовались только связанные списки, которые давали `O(n)` в худшем случае.
...
> [!mcq]
> - [ ] `0.5` — меньше коллизий ценой вдвое большей памяти; выбран как самый консервативный | 0.5 используют для ускорения lookup в high-performance кэшах, но это не дефолт; расход памяти удваивается
> - [ ] `0.6` — экспериментально найденный баланс между 0.5 и дефолтным значением | 0.6 не фигурирует ни в каких стандартных реализациях JDK; промежуточное значение без обоснования
> - [x] `0.75` — математически обоснованный компромисс; при нём ожидаемое число коллизий близко к 1 при равномерном распределении хэшей | Значение 0.75 документировано в Javadoc HashMap и основано на пуассоновском распределении хэшей по bucket'ам
> - [ ] `1.0` — максимальная плотность; resize происходит только при полном заполнении таблицы | loadFactor=1.0 допустим, но увеличивает коллизии и деградирует производительность; не является дефолтом
...
> [!mcq]
> - [ ] `volatile`-поля не могут хранить `null` в `Java Memory Model` — техническое ограничение JVM | volatile-поля прекрасно хранят null; это не техническое ограничение, а намеренное проектное решение
...
> - [ ] `ConcurrentHashMap` запрещает `null` ключи, но `null` значения разрешены | Запрещены симметрично и ключи, и значения — в обоих случаях по одной причине неоднозначности
> - [ ] Проверка на `null` замедляет `CAS`-операции на горячих bucket'ах | Проверка на null — O(1) сравнение; никакого влияния на производительность CAS она не оказывает
...
> [!mcq]
> - [ ] Если `a.equals(b)` — true, то `a.hashCode() == b.hashCode()`; и если `a.hashCode() == b.hashCode()`, то `a.equals(b)` — true | Второе утверждение неверно — совпадение hashCode не означает equals; это была бы биекция, но контракт однонаправленный
> - [x] Если `a.equals(b)` — true, то `a.hashCode() == b.hashCode()`; обратное не обязательно — коллизии допустимы | Контракт однонаправленный: equals → hashCode. Обратное нарушило бы возможность хэш-коллизий и сделало HashMap неработоспособным
> - [ ] Если `a.hashCode() == b.hashCode()`, то `a.equals(b)` — true; обратное не обязательно | Путаница направления: это утверждение превращает каждую хэш-коллизию в ложное равенство, что сломает любую hash-based коллекцию
> - [ ] Если `a.hashCode() != b.hashCode()`, то `a.equals(b)` может быть `true` или `false` | Неверно — это контрапозиция первого правила; разные hashCode гарантируют equals=false (иначе нарушается контракт)
...
> - [ ] `TreeMap` не допускает `null`-значения — только `null`-ключи, потому что значения не участвуют в сравнении | Всё наоборот: `null`-значения в `TreeMap` допустимы (значения не сравниваются), а `null`-ключи — нет (ключи обязаны быть сравниваемыми).
...
> [!mcq]
> - [ ] `WeakHashMap` удаляет записи, когда ключ не используется в течение определённого времени (TTL) — по аналогии с Guava Cache | `WeakHashMap` не знает о времени. Удаление происходит не по таймеру, а когда GC обнаруживает, что на ключ нет сильных ссылок (`StrongReference`). Момент сборки недетерминирован.
...
> [!mcq]
> - [ ] Ничего — итератор `synchronizedMap` является `fail-safe` и не требует внешней синхронизации | Итератор synchronizedMap — fail-fast; без внешней синхронизации во время итерации возможна ConcurrentModificationException
> - [ ] Синхронизировать на `entrySet()`: `synchronized (map.entrySet()) { for (Entry e : ...) }` | Неправильный объект монитора; блокировать нужно саму Map, а не представление entrySet()
> - [x] Явно синхронизировать на Map: `synchronized (syncMap) { for (Map.Entry<...> e : syncMap.entrySet()) { ... } }` | Документация Collections.synchronizedMap явно предписывает это; итератор не синхронизирован автоматически
> - [ ] Использовать `map.forEach()` — он безопасен без внешней синхронизации в `synchronizedMap` | forEach у synchronizedMap синхронизирован как отдельный вызов, но это не защищает от изменений между итерациями в самом forEach
...
> [!mcq]
> - [ ] `poll()` бросает `NoSuchElementException` для пустой очереди; `remove()` возвращает `null` | Поведение перепутано: именно poll() мягкий (null), а remove() строгий (исключение)
> - [x] `poll()` возвращает `null` для пустой очереди; `remove()` бросает `NoSuchElementException` | Паттерн Queue повторяется для всех операций: offer/add, poll/remove, peek/element — первый мягкий, второй строгий
> - [ ] `poll()` удаляет элемент с конца очереди; `remove()` удаляет с начала (стек-семантика) | Оба метода удаляют с головы (begin) очереди — это FIFO; для конца нужен pollLast() из Deque
> - [ ] `poll()` блокирует при пустой очереди до появления элемента; `remove()` возвращает `null` | Блокирующее поведение — это метод `take()` интерфейса `BlockingQueue`; Queue.poll() никогда не блокируется
...
> - [ ] `ArrayDeque` запрещает `null`-элементы по тем же причинам, что и `ConcurrentHashMap` — неоднозначность при конкурентном `poll()` | `ArrayDeque` не потокобезопасна и не конкурентна. Причина запрета `null` — технический маркер пустого слота, а не проблема конкурентности.
...
> - [ ] `ConcurrentModificationException` является checked-исключением, поэтому компилятор требует явной обработки через `try-catch` или объявления в `throws` | CME — это `RuntimeException`, то есть unchecked. Компилятор не требует его ловить. Именно это делает ошибку легко пропустить при код-ревью.
...
> [!mcq]
> - [ ] `ConcurrentModificationException` возникает только при многопоточном доступе — когда несколько потоков одновременно модифицируют коллекцию | Название вводит в заблуждение: CME чаще возникает в однопоточном коде при модификации в `for-each`. Многопоточный сценарий тоже возможен, но не является единственным.
> - [ ] `ConcurrentModificationException` — checked-исключение, поэтому IDE должна предупреждать о риске при итерации | CME является `RuntimeException` (unchecked). Компилятор не требует явного `try-catch`. Именно поэтому ошибку легко пропустить: код компилируется, падает только в runtime.
> - [x] `ConcurrentModificationException` чаще всего возникает в однопоточном коде при вызове `list.remove()` внутри `for-each`, потому что это увеличивает `modCount` и нарушает ожидание итератора | `for-each` — это синтаксический сахар над `Iterator`. Вызов `list.remove()` увеличивает `modCount`; следующий `next()` видит расхождение с `expectedModCount` и бросает CME.
...
> [!mcq]
> - [ ] `Collections.unmodifiableList(original)` создаёт независимую копию исходного списка — изменения в `original` не видны через обёртку | Это главная ловушка `unmodifiableList`. Метод создаёт **view** (обёртку), а не копию. Если через `original.add()` добавить элемент, он появится и в обёртке.
...
> - [ ] `Collections.unmodifiableList(original)` бросает `UnsupportedOperationException` при любом чтении через обёртку — `get()`, `iterator()`, `size()` тоже запрещены | Читающие методы (`get`, `size`, `contains`, `iterator`) работают нормально. Запрещены только мутирующие: `add`, `remove`, `set`, `clear`. Иначе обёртка была бы бесполезна.
> - [ ] `Collections.unmodifiableList(original)` и `List.copyOf(original)` эквивалентны — оба создают view без копирования данных | `List.copyOf()` создаёт **независимую копию** данных. После `List.copyOf()` изменения в `original` не отражаются в копии. Именно в этом разница: view vs snapshot.
...
> [!mcq]
> - [ ] Вернёт `Set` с двумя элементами `{"a", "b"}` — дубликат тихо игнорируется как в `new HashSet<>()` | HashSet игнорирует дубликаты, но Set.of() — строже; он явно запрещает дублирование через IllegalArgumentException
> - [ ] Вернёт `Set` с двумя элементами и выведет предупреждение `WARN: duplicate element "a"` в лог | Set.of() не логирует предупреждения; он бросает исключение немедленно при обнаружении дубликата
> - [x] Бросит `IllegalArgumentException: duplicate element: a` — `Set.of()` запрещает дубликаты при создании | Спецификация Set.of(): дубликат → IllegalArgumentException; это fail-fast контракт, а не тихое поглощение как в HashSet
> - [ ] Бросит `UnsupportedOperationException` — операция добавления дубликата не поддерживается | UnsupportedOperationException бросают методы add()/set() у immutable-коллекций; Set.of() бросает IAE именно при обнаружении дубликата во входных данных
...
> [!mcq]
> - [ ] Новую коллекцию с элементами в обратном порядке — независимую копию оригинала | reversed() возвращает view; создание копии — это Collections.reverse(new ArrayList<>(list)) или List.copyOf(list.reversed())
> - [x] `view` в обратном порядке — изменения в оригинале видны через reversed() и наоборот | Аналогично subList(), keySet() и другим view-методам Java; изменения двунаправленные
> - [ ] `Iterator<E>` в обратном порядке — аналог `descendingIterator()` у `Deque` | reversed() возвращает SequencedCollection, а не итератор; для итератора есть отдельный метод
> - [ ] `Stream<E>` в обратном порядке — для прямого использования в stream-цепочках | Stream можно получить через list.reversed().stream(); сам reversed() возвращает SequencedCollection
