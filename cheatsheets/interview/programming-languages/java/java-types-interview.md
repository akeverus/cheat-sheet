...
`java.lang.Object` — корень всей иерархии классов: любой класс наследует от него явно или неявно, поэтому ссылку на любой объект можно положить в переменную типа `Object`. Это и есть «единый супертип» ссылочного мира Java.

Массивы тоже объекты — даже массив примитивов. У них есть поле `length`, методы `Object` (`equals`, `hashCode`, `toString`), и любой массив можно присвоить переменной типа `Object`.
...
Коротко: **примитив хранит само значение, ссылочная переменная хранит адрес объекта в куче.** Отсюда вытекают все остальные различия — копирование, `null`, поведение в коллекциях и накладные расходы памяти.
...
**Ключевой нюанс:** Java всегда передаёт аргументы **по значению**. Для ссылочного типа по значению копируется сама ссылка — оба имени указывают на один объект, поэтому изменение его полей видно через обе переменные, но переприсваивание (`arr2 = new int[]{...}`) внутри метода исходную переменную не затрагивает.
...
В Java **8 примитивных типов**, и их размеры строго фиксированы спецификацией (в отличие от C, где `int` зависит от платформы) — это гарантирует одинаковое поведение программы на любой машине.

Целочисленные типы (`byte`, `short`, `int`, `long`) — **знаковые** (есть отрицательные значения). Единственное исключение — `char`: он беззнаковый и хранит код символа Unicode (0..65 535).
...
Главное различие: **`==` сравнивает идентичность (это один и тот же объект?), а `equals()` — равенство по содержимому (объекты «одинаковы по смыслу»?).**

**`==`** — это оператор, его поведение зависит от типа операндов:
- Для примитивов сравнивает **значения** (`5 == 5` → `true`).
- Для ссылочных типов сравнивает **ссылки** — указывают ли обе переменные на один объект в куче.

**`equals()`** — метод из `Object`. По умолчанию (унаследованная реализация) он делает ровно то же, что `==` — сравнивает ссылки. Поэтому осмысленное сравнение по содержимому работает только в классах, где `equals()` **переопределён** (`String`, обёртки, `record` и т.д.).
...
**Классы-обёртки** (`Wrapper classes`) — объектные аналоги примитивных типов: каждому примитиву соответствует свой класс (`int` → `Integer`, `boolean` → `Boolean` и т.д.). Они нужны там, где требуется именно объект, а не значение: generic-коллекции, `Optional<T>`, рефлексия, `null` как «нет значения».

**`Autoboxing`** — автоматическое преобразование примитива в обёртку, **`unboxing`** — обратное. Это синтаксический сахар: компилятор сам вставляет вызовы `Integer.valueOf(...)` и `intValue()`, поэтому код выглядит так, будто примитив и обёртка взаимозаменяемы. На самом деле под капотом создаётся (или достаётся из кэша) объект — и об этой «невидимой» стоимости легко забыть (см. подводные камни в Q7).
...
`Integer.valueOf(int)` кэширует готовые объекты для значений от **-128 до 127** (по умолчанию) и для них всегда возвращает **один и тот же экземпляр**. Кэш существует ради экономии: маленькие целые встречаются на каждом шагу (счётчики, индексы, размеры), и нет смысла плодить тысячи одинаковых `Integer`.

Побочный эффект кэша — то, ради чего этот вопрос и задают на собеседовании: для значений из диапазона кэша `==` возвращает `true` (один объект), а за его пределами `valueOf()` создаёт новый объект каждый раз, и `==` даёт `false`. То есть результат `==` для одинаковых чисел зависит от их величины — классическая ловушка.
...
Все ловушки происходят из одного факта: компилятор молча вставляет преобразования, и в коде они не видны. Поэтому легко не заметить ни `NPE` при разворачивании `null`, ни лишние аллокации, ни подмену `equals`-сравнения на сравнение ссылок.
...
**Эмпирическое правило:** по умолчанию берите примитив; обёртку — только когда нужен объект (коллекции, generics) или нужен `null` как «значение отсутствует». Обёртка даёт две возможности — `null` и хранение в коллекциях — но платит за это аллокацией объекта и риском `NPE` при unboxing.
...
Концептуально: **`abstract class` отвечает на вопрос «что это такое» (общая природа, состояние, частичная реализация), а `interface` — «что это умеет» (контракт-способность).** Отсюда главное практическое различие: класс можно наследовать только один, а интерфейсов реализовать сколько угодно. После Java 8 интерфейсы получили `default`-методы и сильно сблизились с абстрактными классами, но остались без состояния экземпляра и без конструктора.
...
Ограничения вытекают из природы интерфейса: он описывает контракт, а не хранит состояние экземпляра. Поэтому полей-данных у него нет — только константы, а методы либо абстрактны, либо несут реализацию, не привязанную к состоянию объекта.
...
Всё сводится к одной детали: **`inner class` хранит неявную ссылку на экземпляр внешнего класса (`Outer.this`), а `static nested class` — нет.** Из этой ссылки следует и доступ к нестатическим полям внешнего класса, и способ создания, и риск утечки памяти.
...
Ответ зависит от того, о наследовании чего речь. **Множественного наследования классов в Java нет** — наследоваться можно только от одного класса (`extends`). Причина — **diamond problem** (ромбовидное наследование): если два родителя приносят одноимённое поле или метод с реализацией, неясно, какой из них достанется потомку.
...
Анонимный класс — класс без имени, который объявляется и сразу инстанцируется «на месте», в одном выражении, чтобы расширить класс или реализовать интерфейс. Применяется, когда реализация нужна ровно один раз и заводить отдельный именованный класс избыточно.
...
Ключевой выбор — учитывать наследование или нет. `instanceof` и `isInstance()` отвечают на вопрос «является ли объект экземпляром типа **или его подтипа**», а `getClass() ==` — на вопрос «это **ровно** этот класс?». В 95% случаев нужен первый, и предпочтительный инструмент — `instanceof` (с pattern matching).
...
Приведение типов — это указание компилятору трактовать значение как другой тип. Делится на два класса по безопасности: **расширение** безопасно и происходит неявно, **сужение** требует явного `(Type)` и может либо потерять данные (для примитивов), либо упасть с `ClassCastException` (для ссылок).
...
`record` (Java 14 preview, Java 16 stable) — компактный тип для **неизменяемых данных-носителей** (DTO, ключи, value-объекты). Главная идея: вы объявляете только список компонентов, а компилятор сам генерирует весь обвязочный код — `private final`-поля, канонический конструктор, геттеры по имени компонента (`name()`, а не `getName()`), а также `equals()`/`hashCode()`/`toString()` на основе значений компонентов. Это убирает десятки строк boilerplate и гарантирует, что equals/hashCode согласованы.
...
`sealed` (Java 17) ограничивает иерархию наследования: расширять `sealed`-класс или реализовывать `sealed`-интерфейс могут **только** типы, явно перечисленные в `permits`. Это золотая середина между `final` (вообще нельзя наследовать) и открытым классом (наследует кто угодно): автор фиксирует **закрытый, заранее известный набор** подтипов.
...
**Зачем это нужно.** Раз набор подтипов известен компилятору, открывается **exhaustive switch** (Java 21+): он проверяет, что `switch` покрыл все варианты, и тогда `default` не нужен. Если позже добавить новый `permits`-тип, все такие `switch` перестанут компилироваться — компилятор сам напомнит дописать обработку. Так `sealed` + `record` дают в Java алгебраические типы данных (ADT).
...
`var` (Java 10) — вывод типа локальной переменной компилятором. Важно понимать, что это **не динамическая типизация**: компилятор выводит конкретный тип из инициализатора прямо при компиляции и навсегда его фиксирует. `var x = 1;` — это ровно тот же `int x = 1;`, просто тип не написан явно; переприсвоить `x` строкой нельзя. `var` сокращает шумные объявления, но не меняет систему типов.
...
Оба ограничивают наследование, но на разном уровне жёсткости. **`final` запрещает наследование полностью** (ноль подклассов), **`sealed` его разрешает — но только заранее названному списку** (`permits`). По сути это «спектр закрытости»: `final` (никто) → `sealed` (известные) → обычный класс (любой).
...
`enum` — тип с фиксированным набором именованных экземпляров (например, статусы заказа). В отличие от C, где enum — это просто набор `int`-констант, в Java `enum` — полноценный **класс**, неявно наследующий `java.lang.Enum<E>`. Каждая константа — это `public static final`-объект этого класса, созданный единожды (singleton), поэтому константы можно безопасно сравнивать через `==`, наделять полями и методами.
...
`Type erasure` — стирание информации о generic-параметрах при компиляции. В байт-коде `List<String>` превращается в `List` (raw type), а параметр `T` заменяется на `Object` (или на верхнюю границу, если она есть: `<T extends Number>` → `Number`). Generics в Java существуют только на этапе компиляции — это была сознательная плата за **обратную совместимость**: старый код без generics должен был работать на новой JVM без перекомпиляции. В рантайме `List<String>` и `List<Integer>` — один и тот же тип.
...
`Wildcard` (`?`) — «неизвестный тип» в generic-выражении. Он нужен, чтобы сделать API гибче: `List<Number>` не принимает `List<Integer>` (generics инвариантны, см. Q23), а `List<? extends Number>` — принимает. Цена гибкости — ограничения на операции: из `extends`-коллекции можно только читать, в `super`-коллекцию только писать.
...
**PECS** — мнемоника для выбора wildcard: **P**roducer **E**xtends, **C**onsumer **S**uper. Если коллекция **отдаёт** вам значения (источник, producer) — `? extends T`; если **принимает** ваши значения (приёмник, consumer) — `? super T`.
...
Вариантность отвечает на вопрос: если `A` — подтип `B`, то как соотносятся «обёртки» над ними — `F(A)` и `F(B)` (где `F` — массив, `List` и т.п.)?

- **Ковариантность** — связь сохраняется: `F(A)` остаётся подтипом `F(B)` (как `String` ⊂ `Object`, так и `String[]` ⊂ `Object[]`).
- **Контравариантность** — связь обращается: `F(B)` становится подтипом `F(A)`.
- **Инвариантность** — никакой связи нет (это случай generic-коллекций).
...
**Почему массивы инвариантными не сделали, а коллекции — сделали.** Ковариантность массивов — историческая ошибка: она позволяет подсунуть объект не того типа и обнаружить это лишь в рантайме (`ArrayStoreException`). Generic-коллекции намеренно инвариантны — несовместимость ловится на компиляции, безопаснее.
...
Корень различия: **`int[]` хранит значения подряд в одном блоке памяти, а `String[]` хранит лишь массив ссылок — сами объекты разбросаны по куче.** Поэтому примитивный массив компактнее и дружелюбнее к кэшу процессора (данные рядом — меньше промахов кэша), а ссылочный гибче (ковариантность, любые типы), но дороже по памяти и медленнее при обходе.
...
`Optional<T>` — контейнер-обёртка, которая либо содержит значение типа `T`, либо пуста. Его задача — сделать **возможное отсутствие результата частью типа метода**: сигнатура `Optional<User> findById(...)` явно говорит вызывающему «может ничего не вернуться», тогда как `User findById(...)` об этом молчит и провоцирует `NullPointerException`. Плюс `Optional` даёт цепочку безопасных операций (`map`, `filter`, `orElse`) вместо ручных проверок на `null`.
...
**Когда использовать:**
- Возвращаемые значения методов, где результат может отсутствовать — это основной и фактически единственный рекомендованный сценарий.
- **НЕ** для полей классов, параметров методов или коллекций. Причина: `Optional` сам может быть `null` (двойная неопределённость), он не `Serializable`, добавляет лишнюю аллокацию; для «пустой» коллекции правильнее вернуть пустой `List`, а не `Optional<List>`.
...
Тип объекта определяет, можно ли его вообще сериализовать и какие поля попадут в поток. Для стандартной Java-сериализации класс должен реализовать **маркерный** интерфейс `Serializable` (без методов — он лишь «разрешает» сериализацию). Дальше судьбу каждого поля решают его тип и модификаторы:
...
`Value-based` классы — классы, для которых важно **значение, а не идентичность объекта**. Спецификация прямо предупреждает: JVM вправе свободно создавать и подменять их экземпляры (например, выдавать один объект из кэша вместо двух одинаковых), поэтому полагаться на `==`, `synchronized` или `System.identityHashCode()` для них нельзя — две «логически равные» величины могут оказаться как одним, так и разными объектами. Эти ограничения — задел под будущие inline-типы из Project Valhalla, у которых вообще не будет стабильной идентичности.
...
Перегрузка (`overloading`) — это несколько методов с одним именем, но разными параметрами. Какой из них вызвать, компилятор решает **статически, на этапе компиляции**, по объявленным типам аргументов (в отличие от переопределения/override, которое разрешается динамически по фактическому типу объекта в рантайме).

Компилятор ищет «наиболее специфичный» подходящий метод, перебирая способы подгонки аргумента по нарастанию «стоимости» (в порядке приоритета):
...
Локальный класс — именованный класс, объявленный **внутри метода** (или блока). Виден только в пределах этого блока и захватывает его `final`/effectively final переменные. По сути это анонимный класс, которому дали имя: имя нужно, когда требуется несколько методов или повторное создание экземпляров — то, чего лямбда и анонимный класс дать не могут.
...
Решающий критерий — нужно ли **общее состояние и конструктор**. Если подклассам нужны общие поля или шаблонный метод с управляемым жизненным циклом — берите `abstract class`. Если нужен только контракт (особенно для несвязанных по природе классов) или множественное наследование поведения — `interface`. Современный совет: начинайте с интерфейса и переходите к абстрактному классу только когда состояние действительно понадобится.
...
Java 21 сделал **pattern matching в `switch`** стабильной фичей (JEP 441). Раньше `switch` умел ветвиться только по константам (числа, строки, enum); теперь `case` может содержать **паттерн типа** — и `switch` заменяет громоздкие цепочки `if (x instanceof A) ... else if (x instanceof B) ...`. Метка вида `case Circle c` одновременно проверяет тип и связывает переменную `c` уже приведённого типа, а `when` добавляет произвольное условие.
...
`Text blocks` (Java 15, стабильно) — многострочные строковые литералы, которые избавляют от `\n` и экранирования кавычек. Открываются тройной кавычкой `"""` плюс обязательный перевод строки, закрываются `"""`. Главная их хитрость — **автоматическое удаление общего отступа**: текст внутри можно красиво выровнять по коду, а в итоговую строку лишние ведущие пробелы не попадут. Это резко упрощает встроенные JSON, SQL, HTML.
...
Разница между `int` и `Integer` критична для высоконагруженных приложений и сводится к двум вещам: **обёртка занимает в разы больше памяти** (заголовок объекта + поле + ссылка вместо 4 голых байт) и **боксинг/анбоксинг создаёт лишние объекты**, нагружая аллокатор и сборщик мусора. На массиве в миллионы элементов или в горячем цикле это даёт заметный проигрыш по скорости и памяти.
...
`var` удобен, но почти все его ловушки растут из одного корня: **тип выводится из правой части буквально**, без оглядки на то, какой тип вы «имели в виду». Компилятор берёт самый конкретный тип инициализатора, а не удобный интерфейс, и навсегда его фиксирует.
...
`EnumSet` и `EnumMap` — специализированные коллекции для `enum`-ключей. Они работают быстрее и экономнее `HashSet`/`HashMap`, потому что используют главную особенность enum: у каждой константы есть фиксированный порядковый номер `ordinal()`. Это позволяет хранить данные не в хэш-таблице, а в битовой маске или плотном массиве — без хэширования, коллизий и упаковки в bucket-ы.

**`EnumSet`** — реализация `Set` только для `enum`-констант. Внутри хранит присутствие констант битами в `long`-маске (`RegularEnumSet` — для ≤ 64 констант, `JumboEnumSet` — для больших). Поэтому проверка наличия, добавление, объединение и пересечение — это просто битовые операции:
...
**`EnumMap`** — реализация `Map`, где ключами служат `enum`-константы. Внутри — обычный массив значений, индексированный по `ordinal()` ключа: получение значения сводится к доступу по индексу (никакого хэша), а итерация всегда идёт **в порядке объявления констант**:
...
`permits`-типы не обязаны жить в одном файле с sealed-типом — они могут быть в разных файлах, но при этом действует требование «близости»: в безмодульном коде все они должны лежать в **том же пакете**, а в модульном (named module) — в том же модуле. Это требование нужно компилятору, чтобы видеть весь закрытый набор подтипов и проверять полноту `switch`.
...
`Record` (Java 16) всегда имеет **канонический конструктор** — тот, что принимает все компоненты в порядке объявления; по умолчанию он сгенерирован неявно. Если нужна валидация или нормализация входных данных, его переопределяют — в одной из двух форм.

**Компактный конструктор** (compact constructor) — короткая форма: пишется `public Range {...}` без списка параметров и без присваиваний `this.x = x`. Параметры доступны как обычные переменные, а присваивание полям компилятор дописывает сам **в конце**. Внутри удобно валидировать и менять параметры — изменённые значения и попадут в поля. Это идиоматичное место для всех проверок record:
...
**Diamond problem** возникает, когда класс получает два одноимённых `default`-метода из разных интерфейсов и компилятор не может выбрать, какой использовать. Java не угадывает — она перекладывает выбор на разработчика: при настоящем конфликте класс **обязан** переопределить метод и при необходимости явно вызвать нужную реализацию через `ИмяИнтерфейса.super.метод()`. Но прежде чем требовать override, компилятор применяет два правила, которые часто разрешают «конфликт» автоматически.
