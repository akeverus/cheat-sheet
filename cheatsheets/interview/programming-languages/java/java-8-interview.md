...
> - [ ] Generics появились в Java 8 и позволяют параметризовать типы коллекций и методов для обеспечения типобезопасности на этапе компиляции | Неверно — Generics появились в Java 5 (JDK 1.5), задолго до Java 8
> - [ ] Аннотации появились в Java 8 как механизм добавления метаданных к классам, методам и полям без влияния на бизнес-логику | Неверно — аннотации появились в Java 5, а Java 8 лишь добавила повторяющиеся аннотации и аннотации типов
> - [ ] Многопоточность через Thread появилась в Java 8 и позволила запускать код параллельно в отдельных потоках ОС | Неверно — java.lang.Thread существует с Java 1.0; Java 8 добавила CompletableFuture и parallelStream, но не базовую многопоточность
...
> [!mcq]
> - [ ] В Java 16 появился оператор `var` для вывода типа локальных переменных, что сокращает шаблонный код при объявлении переменных | Неверно — `var` появился в Java 10, а не в 16
> - [x] В Java 16 появился тип `record` — неизменяемый носитель данных с автоматически генерируемыми конструктором, геттерами, equals, hashCode и toString | Верно — `record` стал стабильным именно в Java 16 (preview с Java 14), и в том же релизе появился удобный `Stream.toList()`
> - [ ] В Java 17 появился `record` — неизменяемый носитель данных с автоматически генерируемыми конструктором, геттерами, equals, hashCode и toString | Неверно — `record` стал стабильным в Java 16, а Java 17 принесла sealed-классы
> - [ ] В Java 21 появился `record` — неизменяемый носитель данных с автоматически генерируемыми конструктором, геттерами, equals, hashCode и toString | Неверно — `record` стабилен с Java 16; Java 21 принесла виртуальные потоки и SequencedCollection
...
> [!mcq]
> - [ ] Функциональный интерфейс — это интерфейс, у которого все методы помечены аннотацией `@FunctionalInterface`, что гарантирует возможность использования лямбд | Неверно — аннотация не обязательна; главное требование — ровно один абстрактный метод (SAM)
> - [ ] Функциональный интерфейс — это интерфейс без default-методов и static-методов, содержащий ровно один абстрактный метод для совместимости с лямбдами | Неверно — наличие default- и static-методов допускается; они не нарушают SAM-контракт
> - [x] Функциональный интерфейс — это интерфейс ровно с одним абстрактным методом (SAM), который является целевым типом для лямбда-выражений и ссылок на методы | Верно — именно наличие единственного абстрактного метода позволяет компилятору сопоставить лямбду с нужной сигнатурой
> - [ ] Функциональный интерфейс — это интерфейс, расширяющий `java.util.function.Function`, что обеспечивает совместимость с методами `andThen` и `compose` | Неверно — функциональный интерфейс не обязан расширять Function; это отдельный интерфейс в иерархии
...
> [!mcq]
> - [ ] `Consumer<T>` принимает аргумент типа T и возвращает результат типа R, применяя функцию преобразования через метод `apply(T) → R` | Неверно — это описание `Function<T,R>`; `Consumer<T>` принимает T и возвращает void
> - [ ] `Supplier<T>` принимает аргумент типа T и выполняет над ним действие с побочным эффектом через метод `accept(T) → void` | Неверно — это описание `Consumer<T>`; `Supplier<T>` не принимает аргументов и возвращает T через `get()`
> - [ ] `Predicate<T>` принимает аргумент типа T и возвращает результат типа T, применяя унарную операцию через метод `apply(T) → T` | Неверно — это описание `UnaryOperator<T>`; `Predicate<T>` возвращает boolean через `test(T)`
> - [x] `Predicate<T>` принимает аргумент типа T и возвращает boolean, проверяя условие через метод `test(T) → boolean`, что используется в `Stream.filter()` | Верно — именно Predicate используется для фильтрации в стримах; его можно комбинировать методами `and()`, `or()`, `negate()`
...
> [!mcq]
> - [x] Запись `a -> a.length()` — допустимый синтаксис лямбды с одним параметром, для которого скобки необязательны и тип выводится компилятором автоматически | Верно — если параметр один, скобки вокруг него можно опустить; тип выводится из контекста целевого функционального интерфейса
> - [ ] Запись `a -> a.length()` недопустима — для одного параметра без явного указания типа необходимо писать `(var a) -> a.length()` начиная с Java 11 | Неверно — `(var a)` допустим с Java 11, но и простой `a ->` без скобок работал с Java 8
> - [ ] Запись `a -> a.length()` недопустима — параметры лямбды всегда должны быть заключены в скобки: `(a) -> a.length()` | Неверно — скобки обязательны только при нескольких параметрах или при явном указании типов; одиночный параметр можно писать без скобок
> - [ ] Запись `a -> a.length()` недопустима — тело лямбды должно быть заключено в фигурные скобки с `return`: `a -> { return a.length(); }` | Неверно — однострочное тело-выражение не требует скобок и `return`; они нужны только при нескольких операторах

> [!mcq]
> - [ ] Лямбда-выражение компилируется в отдельный анонимный `.class`-файл, как и анонимный класс, что позволяет JVM переиспользовать его через classloader | Неверно — именно это и отличает лямбды от анонимных классов: лямбды используют `invokedynamic` и не создают отдельных `.class`-файлов
...
> - [ ] Лямбда-выражение компилируется в статический метод вызывающего класса и всегда создаёт новый объект через `new` при каждом вызове | Неверно — лямбды действительно могут компилироваться в static-методы, но `invokedynamic` не означает `new` при каждом вызове; JVM кэширует
> - [ ] Лямбда-выражение компилируется через reflection-вызов метода интерфейса в рантайме, что гибко но медленнее анонимных классов | Неверно — reflection не используется; `invokedynamic` + LambdaMetafactory обеспечивают производительность выше, чем у анонимных классов
...
> [!mcq]
> - [ ] Аннотация `@FunctionalInterface` обязательна — без неё компилятор не позволит использовать интерфейс как целевой тип лямбды | Неверно — аннотация необязательна; любой SAM-интерфейс принимается как целевой тип лямбды и без этой аннотации
> - [ ] Аннотация `@FunctionalInterface` запрещает добавление default-методов в интерфейс, оставляя только один абстрактный метод | Неверно — default-методы разрешены; аннотация лишь запрещает второй абстрактный метод и выдаёт ошибку компиляции при его добавлении
...
> - [ ] Аннотация `@FunctionalInterface` генерирует дополнительный байткод, обеспечивающий совместимость лямбды с интерфейсом через invokedynamic в рантайме | Неверно — аннотация не влияет на байткод; она только сигнализирует компилятору о необходимости проверки SAM-контракта
