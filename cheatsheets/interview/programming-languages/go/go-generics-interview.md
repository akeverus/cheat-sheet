...
**Нюанс Go 1.20 (частый senior follow-up):** с этой версии **интерфейсные типы** (включая `any`) удовлетворяют constraint'у `comparable`, хотя строго сравнимыми не являются. То есть `Contains[any](...)` компилируется — но если внутри интерфейсов окажутся несравнимые динамические типы (например, два slice), сравнение `==` упадёт в runtime: `panic: comparing uncomparable type []int`. Вывод: `comparable` гарантирует, что `==` скомпилируется, но для интерфейсов не гарантирует отсутствие паники.
...
При этом ради одного `Ordered` тянуть `x/exp` уже не нужно: с Go 1.21 в стандартной библиотеке есть пакет **`cmp`** с идентичным constraint'ом `cmp.Ordered` — на нём построены `slices.Sort`, `slices.Min`, `slices.Max`. В новом коде предпочтителен именно `cmp.Ordered`.
...
**GCShape** — правило группировки из design doc: два типа делят одну shape, если у них одинаковый **underlying type**, либо оба являются **указателями**. На практике это означает: общую инстанциацию (общий код + dictionary) получают ТОЛЬКО указательные типы — для GC любой `*T` выглядит одинаково. Каждый value-тип со своим underlying type получает **отдельную** копию кода, даже если размеры совпадают.

```go
func Print[T any](v T) { fmt.Println(v) }

Print[int](42)       // своя копия кода (underlying type int)
Print[int64](42)     // ОТДЕЛЬНАЯ копия — другой underlying type, хотя тоже 8 байт
Print[string]("hi")  // своя копия
Print[*User](&u)     // gcshape всех указателей — общий код + dictionary
Print[*Order](&o)    // та же shape, что у *User
```

В примере выше `int` и `int64` НЕ делят код: одинаковый размер (8 байт, без указателей) сам по себе типы не объединяет. А вот `*User` и `*Order` используют одну общую инстанциацию: все pointer-типы сведены в единственную gcshape, и различия между ними generic-код узнаёт в runtime из dictionary.
...
**GCShape** — это группировка типов, по которой компилятор решает, кому выдать общую инстанциацию generic-кода. Правило из design doc (gcshape stenciling):
- два типа попадают в одну shape, если у них одинаковый **underlying type**, либо оба — **указатели**;
- все pointer-типы (`*User`, `*Order`, `*int`, ...) — это **одна** общая shape: для GC любой указатель — просто машинное слово, указывающее в кучу;
- каждый value-тип со своим underlying type — **своя** shape: `int` и `int64` не делят код даже при одинаковом размере 8 байт. «Одинаковый размер = одна shape» — распространённое упрощение, не соответствующее реализации.

```
int, int64, *User, etc — каждое имеет свою shape
```

Практический смысл: общую копию машинного кода (с обращениями к dictionary) делят на практике только указательные типы; каждый value-тип получает собственную копию, как при monomorphization. Это та самая «единица переиспользования», на которой держится гибридная реализация из Q12.
...
maps.Keys(m)                 // iter.Seq[K] — итератор, std с Go 1.23
maps.Values(m)               // iter.Seq[V] — итератор, std с Go 1.23
slices.Collect(maps.Keys(m)) // []K — слайс собирается через slices.Collect
```

Важный нюанс с `maps.Keys`/`maps.Values`: в стандартном пакете `maps` (Go 1.21) этих функций НЕ было — они появились только в Go 1.23 и возвращают **итератор** `iter.Seq[K]`, а не слайс; слайс из него собирают через `slices.Collect`. Вариант с сигнатурой «сразу слайс» — это другой пакет, `golang.org/x/exp/maps`, прежний путь до стандартизации.

Сюда же относится пакет **`cmp`** (std с Go 1.21): его `cmp.Ordered` — стандартная замена `constraints.Ordered`, и именно на нём построены `slices.Sort`, `slices.Min`, `slices.Max`.

В Go 1.22–1.23 набор функций расширяется (итераторы, `slices.Collect`), но команда добавляет generic API в стандарт очень осторожно — чтобы не закрепить неудачные сигнатуры, которые потом нельзя будет менять из-за гарантий совместимости.
...
С Go 1.21 бо́льшая часть переехала в стандарт (`slices`, `maps`), а `constraints` целиком пока остаётся в `exp`. Но самое востребованное из него уже стандартизовано отдельно: `cmp.Ordered` из std-пакета `cmp` (Go 1.21) — полная замена `constraints.Ordered`.

**Рекомендация:** `golang.org/x/exp` помечен как **экспериментальный** — его API может меняться без гарантий совместимости. В production по возможности используйте стандартные `slices`/`maps`/`cmp`, а к `golang.org/x/exp` обращайтесь только за тем, чего в стандарте ещё нет (например, `constraints.Integer`/`Float`).
