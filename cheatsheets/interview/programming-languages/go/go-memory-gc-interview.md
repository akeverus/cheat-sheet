...
Пример состояния графа в середине обхода:

- `Root` (серый) → `A` (серый) → `B` (белый);
- `Root` (серый) → `C` (чёрный).

Здесь `Root` и `A` уже обнаружены, но их исходящие ссылки ещё досматриваются; `C` полностью обработан (чёрный); `B` пока не обнаружен (белый) и станет серым, когда до него дойдёт обход от `A`.
