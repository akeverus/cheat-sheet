...
**В чём проблема.** Мутатор (приложение) перекидывает ссылки **одновременно** с обходом GC. Без защиты возможен сценарий, когда живой объект окрасится в белый и будет ошибочно удалён:
...
Корень проблемы — нарушение инварианта «чёрный не ссылается на белый»: чёрный объект получает ссылку на ещё не отмеченный белый, и сборщик о ней не узнаёт.

**Решение.** При изменении указателя write barrier помечает новый объект серым (Yuasa-style snapshot), восстанавливая инвариант, — и затронутый объект гарантированно будет обойдён сборщиком.

В Go применяется **Dijkstra-style write barrier** (с Go 1.8). Стоимость операции — порядка ~2–5 нс на одну запись указателя; это та небольшая плата за CPU, которой Go покупает короткие паузы.
...
| Размер кучи | До сотен GB | До 64 GB | До 16 TB |
| Сложность | Простой | Сложный (regions, mixed GC) | Сложный (colored pointers) |

**Вывод:** Go GC проще и выгоднее для микросервисов с кучей < 10 GB — минимум настроек, предсказуемо короткие паузы. JVM-сборщики гибче там, где нужны огромные кучи и высокий throughput, но платят за это сложностью тюнинга.
