...
Наглядно: slice header с полями `ptr`, `len=3`, `cap=5` ссылается (`ptr →`) на underlying-массив из 5 ячеек `[10][20][30][_][_]` — первые три заняты значениями (10, 20, 30), а последние две (`_`, `_`) ещё свободны, то есть `len=3` при `cap=5`.
