...
Связи G/M/P выстраиваются по цепочке: несколько goroutines стоят в локальной очереди одного `P`, каждый `P` отдаётся одному `M` (OS thread), а каждый `M` исполняется на ядре CPU. По порядку:

- `G` (несколько goroutines) → очередь `P0`;
- `G` (ещё несколько goroutines) → очередь `P1`;
- очередь `P0` → `M` (OS thread) → ядро CPU;
- очередь `P1` → `M` (OS thread) → ядро CPU.
