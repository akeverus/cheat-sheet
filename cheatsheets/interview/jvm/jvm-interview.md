...
> [!mcq]
> - [ ] Принцип `Write Once, Run Anywhere` означает, что `.jar` запустится без изменений где угодно — включая нативные зависимости через `JNI`; библиотеки `Netty epoll`, `RocksDB`, `snappy` упакованы в jar и автоматически работают на любой ОС | WORA гарантирует портируемость только байткода (`.class`); нативные зависимости через `JNI`/`FFM` платформозависимы — для каждой комбинации (Linux x86_64, Linux ARM64, macOS, Windows) нужны отдельные `.so`/`.dylib`/`.dll` библиотеки. ❌ ПОСЛЕДСТВИЕ: команда переносит сервис с RocksDB на Apple Silicon (M1) и получает `UnsatisfiedLinkError: librocksdbjni-linux64.so` — в jar лежит только Linux-бинарник. Pipeline сборки не учитывает мульти-arch native libs, локальная разработка на Mac невозможна. Fix: rocksdbjni-multi-arch jar или пересборка native под каждую platform.
> - [ ] `HotSpot` и `GraalVM` — это разные форматы байткода; `.class` файл, скомпилированный для `HotSpot`, не запустится на `GraalVM` без перекомпиляции | Формат байткода (`.class`) определён спецификацией JVM и одинаков для всех реализаций; `HotSpot`, `GraalVM`, `OpenJ9` исполняют один и тот же `.class`-файл, разница лишь в способе его выполнения (JIT-стратегии, GC, AOT). ❌ ПОСЛЕДСТВИЕ: разработчик считает что нужно «пересобирать под GraalVM» и настраивает отдельный CI pipeline. На самом деле тот же jar запускается через `graalvm/native-image` для AOT или через `graalvm-jdk` для обычного JIT-режима. Лишний CI этап, удвоенные артефакты, путаница в Nexus.
> - [x] `WORA` гарантирует портируемость только байткода (`.class`); нативные зависимости через `JNI`/`FFM API` остаются платформозависимыми и требуют отдельных бинарников для каждой комбинации ОС+архитектура | Java-байткод действительно платформонезависим, но любой выход за пределы JVM (через JNI с Java 1.1, FFM API с Java 22) ломает WORA — нативная библиотека всегда привязана к конкретной ОС и CPU-архитектуре. ✓ ПРИМЕНЯТЬ: при использовании Netty epoll, RocksDB, snappy, JNA — публиковать multi-classifier jar (`linux-x86_64`, `linux-aarch64`, `osx-aarch64`, `windows-x86_64`); `os-maven-plugin` автоматически выбирает нужный classifier; для миграции на Graviton (ARM) проверять `aarch64` поддержку каждой native dep. 📋 ПРАВИЛО: «WORA = bytecode portable, native deps = per-platform». 🔗 См. Q3 (байткод), Q4 (JDK/JRE), Q7 (GraalVM Native Image).
> - [ ] Разные реализации `JVM` (`HotSpot`, `OpenJ9`, `GraalVM`) могут давать разное поведение для одного и того же `.class` — например, разный порядок инициализации static-полей или разную семантику `volatile` | Спецификация JVM (`JLS` + `JVM Spec`) строго определяет семантику байткода, JMM (Java Memory Model), порядок class init; реализации, прошедшие TCK, обязаны давать идентичное наблюдаемое поведение. Различия — только в performance, GC и JIT-оптимизациях. ❌ ПОСЛЕДСТВИЕ: команда пишет код «специально для HotSpot», избегая Eclipse OpenJ9 «вдруг там volatile иначе работает». Параноидальные ограничения на выбор runtime, упускают экономию памяти OpenJ9 (-30% heap) для микросервисов. Реально TCK-сертифицированные реализации семантически совместимы.
...
> [!mcq]
> - [x] `Code Cache` — отдельная область памяти для `JIT`-скомпилированного нативного кода; при её переполнении `JVM` отключает компиляцию, и горячие методы выполняются интерпретатором, что приводит к деградации performance | Code Cache хранит native-код, сгенерированный C1/C2; default размер 240MB (Java 9+), при переполнении JIT отключается с warning «CodeCache is full. Compiler has been disabled». ✓ ПРИМЕНЯТЬ: для приложений с большим кодом (Spring Boot с десятками тысяч beans, Camel, jRuby) — `-XX:ReservedCodeCacheSize=512m`; мониторинг через JMX `java.lang:type=MemoryPool,name=CodeHeap`; алерт на `CodeCache usage > 90%`; `-XX:+PrintCodeCache` при остановке JVM показывает финальный размер. 📋 ПРАВИЛО: «Code Cache full ⇒ JIT off ⇒ performance regression». 🔗 См. Q5 (JIT), Q6 (Tiered), Q11 (memory areas).
> - [ ] `Code Cache` — это часть `Heap`, рядом с `Old Generation`; `GC` управляет памятью JIT-кода так же, как объектами, и собирает неиспользуемые скомпилированные методы | Code Cache находится в нативной памяти процесса (вне Heap), не управляется GC, освобождение скомпилированных методов происходит через отдельный механизм code cache sweeping; перепутывание с Heap приводит к неверной диагностике OOM. ❌ ПОСЛЕДСТВИЕ: команда видит OOM «Java heap space» и тюнит `-Xmx`, но реальная проблема — Code Cache full с приложением jRuby или groovy-evaluating. Часы dev-time на анализ heap dump впустую — Code Cache не виден в обычном heap dump.
> - [ ] `Method Area` (`Metaspace`) хранит весь скомпилированный `JIT`-код вместе с метаданными классов; Code Cache — это устаревший термин из Java 7 PermGen-эпохи | Это две разные области: Metaspace хранит class metadata (вне Heap, в native memory с Java 8+), а Code Cache — отдельная область для JIT native code; обе живут в native памяти, но управляются разными механизмами. ❌ ПОСЛЕДСТВИЕ: разработчик настраивает `-XX:MaxMetaspaceSize=1g`, ожидая что это покроет JIT-код. Code Cache всё равно ограничен default 240MB, при переполнении — JIT off. Без понимания разделения областей tuning в production = false sense of safety.
> - [ ] При переполнении `Code Cache` `JVM` автоматически расширяет его до значения `-Xmx`, забирая память у `Heap` | JVM не «расширяет» Code Cache за счёт Heap; при переполнении JIT-компиляция полностью отключается, новые горячие методы остаются в интерпретаторе с warning. Расширить можно только через `-XX:ReservedCodeCacheSize` при старте. ❌ ПОСЛЕДСТВИЕ: команда полагается на «JVM сам разберётся» и не задаёт ReservedCodeCacheSize. После hot redeploy в Tomcat (старые скомпилированные методы остаются в Code Cache даже после class unload) — переполнение, JIT off, latency p99 растёт в 5-10x. Reproduction-сложный production incident.
...
> [!mcq]
> - [x] **Деоптимизация** — `JVM` отбрасывает `C2`-код и возвращает метод в интерпретатор, когда спекулятивное предположение нарушилось (`uncommon trap`, class hierarchy change, profile divergence) | C2 строит код на основе профиля («полиморфный вызов всегда `ArrayList`»); при нарушении условия — откат в интерпретатор для повторного профилирования. ✓ ПРИМЕНЯТЬ: новая реализация интерфейса через `ServiceLoader` в runtime — JIT деоптимизирует, latency-спайки; `-XX:+UnlockDiagnosticVMOptions -XX:+LogCompilation` показывает причины через `<uncommon_trap>`; JFR `Compilation` event для deopt rate. 📋 ПРАВИЛО: «спекуляция сломалась → метод обратно в интерпретатор». 🔗 См. Q3 (байткод), Q6 (Tiered), Q39 (hot methods).
> - [ ] **Деоптимизация** — финальный этап `JIT`, когда `C2` отбрасывает промежуточные представления и оставляет только финальный нативный код; происходит один раз для каждого метода | Это обратный процесс: JVM выбрасывает уже скомпилированный код и возвращается к интерпретации; терминология «финальный этап» полностью неверна. ❌ ПОСЛЕДСТВИЕ: разработчик игнорирует события `made not entrant`/`made zombie` в `-XX:+PrintCompilation` как «финальное состояние». Горячий метод вернулся в интерпретатор, p99 latency 50ms→500ms; production incident «без причины в коде» дебажится сутками.
> - [ ] Деоптимизация запускается только при критических ошибках `JVM` (`SIGSEGV`, `JNI` corruption); в нормальной работе её не бывает | Деоптимизация — штатный адаптивный механизм: CHA invalidation при загрузке класса, изменение полиморфного call site, null-check failure в hot loop; это не ошибка. ❌ ПОСЛЕДСТВИЕ: команда не мониторит deopt events, считая их редкими. В Spring Boot с AOP + lazy ClassLoading deopt идёт каждые секунды старта; warmup time не сокращается, CDS/AppCDS/Leyden не применяются — упущена 20-30% экономия cold-start.
> - [ ] Флаг `-XX:-Inline` отключает деоптимизацию: без инлайнинга `C2` не делает спекулятивных оптимизаций и метод остаётся в нативном коде навсегда | `-XX:-Inline` убирает только инлайнинг — одну оптимизацию из многих; CHA, polymorphic inline cache, range-check elimination продолжают работать и спекулировать. ❌ ПОСЛЕДСТВИЕ: команда выключает inlining «чтобы избежать deopt», теряя 30-50% throughput (inlining — самая эффективная JIT-оптимизация). p99 регрессирует в 2x, deopt всё равно происходит; откат через неделю после JFR-анализа.
...
> - [ ] `GraalVM` — это только платформа для `AOT`-компиляции; как замена `C2`-компилятора для `JIT` он не используется | `GraalVM` работает в двух режимах: как `Graal JIT` (замена `C2` внутри HotSpot через `-XX:+UseJVMCICompiler`) и как инструмент `AOT` (native-image) — это разные сценарии одной технологии. ❌ ПОСЛЕДСТВИЕ: команда не пробует `Graal JIT`, считая GraalVM только для Native Image; упускают +5-15% throughput на CPU-bound нагрузках (Twitter использует Graal JIT в production); performance opportunity потеряна из-за неполного понимания тулинга, на ретро никто не вспоминает про этот опцион.
...
> [!mcq]
> - [x] С `Java 9` `Extension ClassLoader` заменён `Platform ClassLoader`, работающим с модульной системой `JPMS` и загружающим модули платформы (`java.sql`, `javax.crypto`) | Модульная система Java 9 изменила иерархию: Extension исчез, его задачи взял Platform, умеющий работать с модулями JPMS. ✓ ПРИМЕНЯТЬ: frameworks с custom ClassLoader (OSGi, Tomcat WAR isolation, Spring DevTools) — понимание иерархии критично; `--add-modules` для динамической загрузки platform modules; Spring Boot `LaunchedURLClassLoader` для fat jar. 📋 ПРАВИЛО: «Bootstrap (C++) → Platform (modules) → Application (classpath) → Custom». 🔗 См. Q9 (delegation), Q10 (custom), Q11 (memory).
> - [ ] `Bootstrap ClassLoader` написан на `Java` и загружает ядро `JDK`; `Platform ClassLoader` написан на `C++` и загружает нативные библиотеки | Всё наоборот: Bootstrap реализован на `C++` внутри JVM и загружает `java.lang`/`java.util`; Platform написан на Java для модулей платформы. ❌ ПОСЛЕДСТВИЕ: custom ClassLoader полагается на `Bootstrap.getParent()` ожидая non-null parent — реально `null`, NPE в production при первом запросе к plugin classes; Bootstrap не имеет Java-представления.
> - [ ] С `Java 9` `Extension ClassLoader` переименован в `System ClassLoader` и грузит и системные, и пользовательские классы | Extension переименован в `Platform`, не System; System (= Application) ClassLoader загружает классы приложения и существовал до Java 9. ❌ ПОСЛЕДСТВИЕ: миграция Tomcat 8→9 (Java 8→11) — legacy-код использует `getContextClassLoader().getParent().getParent()` для Extension; в Java 9+ этот путь возвращает Platform, app не находит классы, многодневный debug.
> - [ ] `Application ClassLoader` (System) — корневой родитель всей иерархии; `Bootstrap` и `Platform` — его дочерние загрузчики, делегирующие вверх | Иерархия строго обратная: Bootstrap → Platform → Application; Application делегирует **вверх** к Platform, Platform — к Bootstrap; Bootstrap parent = null. ❌ ПОСЛЕДСТВИЕ: разработчик пишет custom ClassLoader, делегируя «вниз» к Application — security hole, можно подменить `java.lang.String`; CVE в legacy Tomcat 5/6 — именно из-за такой модели делегирования.

> [!mcq]
> - [ ] `Class.forName("X")` и `ClassLoader.loadClass("X")` — полные синонимы; оба вызывают `static`-инициализатор класса при первом обращении | Это ключевое различие: `Class.forName(name)` вызывает initialization (static-блок выполняется), `ClassLoader.loadClass(name)` только linking без initialization; static-блок отрабатывает лениво при первом обращении к классу. ❌ ПОСЛЕДСТВИЕ: разработчик грузит JDBC-драйвер через `loadClass("com.mysql.cj.jdbc.Driver")` ожидая регистрации в `DriverManager`. Static-блок драйвера не выполнен → драйвер не зарегистрирован → `No suitable driver found`. Часы debug, проблема не в classpath, а в выборе метода загрузки.
> - [x] `Class.forName(name)` запускает инициализацию класса (выполняет `static`-блок); `ClassLoader.loadClass(name)` загружает класс без инициализации — `static`-блок отработает позже при первом активном обращении | Разная семантика: forName делает loading + linking + initialization; loadClass останавливается на linking. Это критично для классов с side-effect static-блоками (JDBC drivers, JNI native loaders, singleton initializers). ✓ ПРИМЕНЯТЬ: для JDBC `Class.forName("com.mysql.cj.jdbc.Driver")` — обязательно forName для триггера static-регистрации (хотя с JDBC 4.0+ ServiceLoader делает это автоматически); для plugin-системы — loadClass для проверки существования без побочных эффектов; для testing с lazy init — loadClass для контроля момента инициализации. 📋 ПРАВИЛО: «forName = init now, loadClass = init later». 🔗 См. Q9 (delegation), Q11 (memory), Q3 (loading phases).
> - [ ] `Class.forName(name)` работает только с классами на `classpath`, а `ClassLoader.loadClass(name)` — с любыми источниками включая сеть | Источник класса определяется именно ClassLoader-ом, а не методом загрузки; `Class.forName(name, init, classLoader)` принимает custom ClassLoader, способный грузить из сети, БД, шифрованных архивов. ❌ ПОСЛЕДСТВИЕ: разработчик пишет plugin-систему через loadClass и удивляется почему `Class.forName` не работает с custom CL — на самом деле работает, нужна 3-arg перегрузка `Class.forName(name, true, pluginCL)`. Ненужная сложность архитектуры из-за неверного понимания API.
> - [ ] Оба метода кэшируют загруженный класс в `Metaspace`, но `Class.forName` дополнительно держит сильную ссылку на `Class<?>`, предотвращая выгрузку класса | Кэширование одинаковое — оба метода добавляют класс в `ClassLoader.classes` (внутренний Map); удержание Class-объекта зависит от живости ClassLoader-а, а не от способа загрузки. ❌ ПОСЛЕДСТВИЕ: команда переключается с forName на loadClass надеясь на «GC сможет выгрузить класс». Реально выгрузка зависит от достижимости ClassLoader-а; в Tomcat hot redeploy утечка ClassLoader keep-alive независимо от forName/loadClass. Симптомы Metaspace OOM сохраняются.
...
> [!mcq]
> - [ ] Кастомный `ClassLoader` нужен только для загрузки классов из сети — для локальных JAR достаточно `URLClassLoader` | Сетевая загрузка — лишь один из сценариев; кастомный `ClassLoader` нужен и для plugin-систем, hot reload, изоляции зависимостей, шифрованных источников. ❌ ПОСЛЕДСТВИЕ: команда отказывается от plugin-системы из-за «избыточной сложности», в результате каждая интеграция требует rebuild и redeploy всего сервиса.
> - [x] Plugin-системы (OSGi), hot reload (Spring DevTools), изоляция конфликтующих версий библиотек, загрузка из нестандартных источников — типичные сценарии для кастомного `ClassLoader` | Каждый сценарий объединяет одно: нужно загружать байт-код вне стандартного classpath или изолировать его. ✓ ПРИМЕНЯТЬ: Tomcat использует `WebappClassLoader` для изоляции webapp; Spring Boot — `LaunchedURLClassLoader` для fat JAR; Eclipse/IntelliJ — для plugin API. 📋 ПРАВИЛО: «кастомный ClassLoader = code outside classpath ИЛИ изоляция». 🔗 См. Q8 (иерархия), Q9 (делегирование).
> - [ ] Кастомный `ClassLoader` исключительно используется для тестирования (mocking классов в unit-тестах) | Mocking тестов делается через `Mockito`/`Byte Buddy` без кастомного `ClassLoader` для подавляющего большинства случаев. ❌ ПОСЛЕДСТВИЕ: разработчик пишет кастомный `ClassLoader` для теста где достаточно `@MockBean`, добавляет 200 строк сложного кода и потенциальную утечку Metaspace.
> - [ ] Достаточно использовать стандартный `AppClassLoader` для всех production-сценариев — кастомные `ClassLoader` — это устаревшая практика | Современные application servers, plugin-системы и Spring Boot активно используют кастомные `ClassLoader`. ❌ ПОСЛЕДСТВИЕ: попытка загрузить две версии одной библиотеки в один classpath → `NoSuchMethodError` в runtime; production падает после deploy зависимости.
...
> [!mcq]
> - [x] `PermGen` заменён `Metaspace` в `Java 8`; главное отличие — `Metaspace` в нативной памяти ОС и растёт динамически, устраняя проблему фиксированного лимита `PermGen` | Фиксированный `MaxPermSize` был главной причиной `OutOfMemoryError: PermGen space` при hot deploy; Metaspace растёт по запросу до native memory exhaustion или `MaxMetaspaceSize`. ✓ ПРИМЕНЯТЬ: миграция Java 6/7 → 8+ — убрать `-XX:MaxPermSize`/`-XX:PermSize` (invalid в 8+); Tomcat 8+ hot deploy стабильнее через Metaspace; Spring Boot 1→2 требовал именно Java 8 minimal. 📋 ПРАВИЛО: «Java 8+ = Metaspace (native), Java 7- = PermGen (heap)». 🔗 См. Q11 (memory), Q14 (OOM), Q12 (Stack vs Heap).
> - [ ] `PermGen` заменён `Metaspace` в `Java 7`, когда строки переехали в нативную память, освободив PermGen от лишней нагрузки | PermGen заменён в Java 8; в Java 7 произошло другое — String Pool переехал из PermGen в Heap (но сам PermGen остался). ❌ ПОСЛЕДСТВИЕ: миграция Java 6 → 7 с расчётом «PermGen issues исчезнут»; команда удивляется почему `OutOfMemoryError: PermGen space` всё ещё в production логах; настоящий fix только в Java 8.
> - [ ] `Metaspace` заменил `PermGen` в `Java 9` вместе с модульной системой `JPMS`; до `Java 9` PermGen продолжал использоваться в OpenJDK | Metaspace появился в Java 8; JPMS — отдельное изменение Java 9, не связанное с заменой PermGen. ❌ ПОСЛЕДСТВИЕ: документация компании смешивает Java 8 (Metaspace) и Java 9 (JPMS) изменения; команда применяет JPMS migration steps к Java 8 — ничего не работает, недели потеряны на distraction.
> - [ ] `Metaspace`, как и `PermGen`, имеет фиксированный размер по умолчанию; без `-XX:MaxMetaspaceSize` он не может расти сверх 256 MB | По умолчанию Metaspace **не** имеет верхнего лимита и растёт до исчерпания нативной памяти ОС; именно поэтому в production обязателен явный `-XX:MaxMetaspaceSize`. ❌ ПОСЛЕДСТВИЕ: разработчик надеется на «default лимит 256MB» и не ставит `MaxMetaspaceSize`; long-running app с ClassLoader leak (Spring Cloud + Hystrix dynamic classes) съедает 4GB native, pod OOMKilled через сутки.
...
> [!mcq]
> - [x] **Copying GC** (для `Young Generation`) копирует живые объекты в новое пространство, оставляя старое для очистки одной операцией; **Mark-Sweep-Compact** (для `Old Generation`) маркирует, освобождает мусор и уплотняет память для устранения фрагментации | Разные алгоритмы оптимальны для разных профилей: Copying быстр когда живых объектов мало (типично для Young — гипотеза поколений, 90%+ умирает молодым); Mark-Sweep-Compact эффективнее когда выживает большинство объектов (Old Gen). ✓ ПРИМЕНЯТЬ: понимать причины Survivor space ratio в G1/Parallel — Copying требует free space ≥ used space; для long-lived объектов Old Gen компактируется реже (амортизация stoптата); JFR Allocation Pause event показывает время copy vs compact. 📋 ПРАВИЛО: «Young → Copying (мало живых), Old → Mark-Sweep-Compact (много живых)». 🔗 См. Q16 (Generational), Q17 (STW), Q22 (collectors).
> - [ ] Все современные `GC` используют один универсальный алгоритм **Mark-Sweep**: сначала маркируют живые объекты, потом освобождают мусор; копирование объектов или компактация — это устаревшие техники из Serial GC | Современные сборщики комбинируют несколько алгоритмов: G1 — copying для young + mark-compact для regions; ZGC — concurrent copy + colored pointers; Shenandoah — concurrent copy + forwarding. Mark-Sweep без compact приводит к фрагментации (CMS — был именно такой и поэтому удалён). ❌ ПОСЛЕДСТВИЕ: разработчик ожидает «один алгоритм для всех», не понимает разницу между сборщиками. Tuning через cargo cult — `-XX:+UseConcMarkSweepGC` на Java 14+, JVM ругается «removed». Wasted effort на изучение неподходящего GC.
> - [ ] **Mark-and-Sweep** освобождает память в исходных местах объектов (in-place) без копирования; именно поэтому он быстрее `Copying`-сборщиков и используется во всех `low-latency` GC | In-place освобождение — это именно недостаток Mark-Sweep: оставляет фрагментацию, требует free-list для аллокации. Copying быстрее для типичной allocation pattern (bump pointer allocation в Eden), low-latency GC (ZGC, Shenandoah) используют именно concurrent copying. ❌ ПОСЛЕДСТВИЕ: команда выбирает `-XX:+UseSerialGC` (Mark-Sweep) для low-latency requirement, ожидая лучше чем G1. STW паузы Serial GC растут до секунд на heap > 4GB. Latency p99 деградирует, корректировка через неделю.
> - [ ] **Compact**-фаза в `GC` нужна только для дефрагментации `Metaspace`; в `Heap` объекты никогда не перемещаются | Compact активно используется в Heap: G1 перемещает объекты между регионами (evacuation), ZGC/Shenandoah — concurrent relocation; именно перемещение позволяет bump pointer allocation (быстрая аллокация через инкремент указателя). Metaspace, наоборот, не компактируется — отсюда proблема с ClassLoader leaks. ❌ ПОСЛЕДСТВИЕ: разработчик считает «адрес объекта стабилен» и кэширует адрес через JNI/Unsafe. После Major GC адрес меняется (объект перемещён) — segfault или corruption. JNI-код должен использовать GlobalRef, не сырые адреса.
...
> [!mcq]
> - [x] По умолчанию объект промотируется из `Survivor` в `Old Generation` после **15** циклов `Minor GC`; значение задаётся флагом `-XX:MaxTenuringThreshold` | 15 — стандартный default; высокое значение удерживает объекты в Young Gen дольше, давая им шанс умереть без промоции в Old (снижает частоту Full GC). ✓ ПРИМЕНЯТЬ: HTTP API (объекты живут <1ms) — default 15 идеально; batch processing с long-lived аккумуляторами — также 15; JFR `Tenuring Distribution` event показывает age histogram для tuning. 📋 ПРАВИЛО: «default 15 = max 4-bit age field». 🔗 См. Q15 (GC), Q17 (STW), Q22 (collectors).
> - [ ] По умолчанию объект промотируется в `Old Generation` после **5** циклов `Minor GC`; значение задаётся `-XX:MaxTenuringThreshold` | Default = 15, не 5; разные сборщики могут адаптивно понижать tenuring threshold, но базовое значение в HotSpot — 15. ❌ ПОСЛЕДСТВИЕ: tuning team понижает `MaxTenuringThreshold=5` по ложной инфе; объекты быстрее уходят в Old Gen, давление Full GC растёт; p99 регрессирует с 30ms до 100ms; откат через 2 недели после JFR-анализа.
> - [ ] По умолчанию объект промотируется в `Old Generation` после **8** циклов `Minor GC`; значение задаётся `-XX:MaxTenuringThreshold` | Default = 15, не 8; уменьшение порога ускоряет промоцию, но увеличивает давление на Old Gen и частоту Full GC. ❌ ПОСЛЕДСТВИЕ: разработчик читает старый блог про Java 6 (где default = 31) и путает с current; ставит `-XX:MaxTenuringThreshold=15` думая, что меняет — это уже default; cargo cult без эффекта.
> - [ ] По умолчанию объект промотируется в `Old Generation` после **32** циклов `Minor GC`; значение задаётся `-XX:MaxTenuringThreshold` | Default = 15; **максимально** допустимое значение тоже 15 — age хранится в 4 битах header. ❌ ПОСЛЕДСТВИЕ: команда ставит `-XX:MaxTenuringThreshold=32`; JVM выводит warning `MaxTenuringThreshold (32) is greater than maximum (15)`, capped до 15; tuning ничего не меняет, ложная уверенность в оптимизации.

> [!mcq]
> - [ ] Каждая аллокация нового объекта в `Eden` требует глобальной блокировки `Heap`-указателя; именно поэтому highly-concurrent приложения упираются в `allocation contention` на современных серверах с 32+ ядрами | Глобальная блокировка убила бы параллельную аллокацию; JVM использует `TLAB` (Thread-Local Allocation Buffer) — каждый поток получает свой кусок Eden и аллоцирует bump pointer без синхронизации. ❌ ПОСЛЕДСТВИЕ: разработчик пишет ObjectPool «потому что allocation slow due to lock» — добавляет сложность, баги с lifecycle, hard-to-debug memory leaks. Реально TLAB allocation = ~10ns без contention, ObjectPool часто медленнее (cache misses, lifecycle bugs). 99% «оптимизаций аллокации» в Java cargo cult.
> - [x] Аллокация в `Eden` происходит через **`TLAB`** (`Thread-Local Allocation Buffer`) — каждый поток получает свой буфер из `Eden` и аллоцирует через bump pointer без синхронизации; глобальный lock берётся только при пересоздании TLAB | TLAB обеспечивает lock-free allocation в highly-concurrent приложениях; размер TLAB адаптивный (`-XX:TLABSize`, `-XX:ResizeTLAB`); статистика через JFR `ObjectAllocationInNewTLAB` event. ✓ ПРИМЕНЯТЬ: для high-throughput нагрузки (Kafka consumer, web API) — мониторить TLAB waste через `-XX:+PrintTLAB`; крупные объекты (> TLAB) аллоцируются slow path с lock — избегать через streaming или chunking; `-XX:+UseTLAB` (default true) критичен для performance. 📋 ПРАВИЛО: «TLAB = lock-free per-thread bump pointer in Eden». 🔗 См. Q11 (memory), Q15 (GC), Q37 (Escape Analysis).
> - [ ] `TLAB` — это специальная область в `Old Generation` для долгоживущих объектов; объекты, которые JVM считает long-lived, сразу попадают в `TLAB` без прохождения через `Young Generation` | TLAB находится в Eden (Young Generation), а не в Old Gen; это per-thread буфер для быстрой аллокации новых объектов; направление аллокации в Old Gen для long-lived — это pretenuring, отдельный механизм, доступный только в некоторых GC. ❌ ПОСЛЕДСТВИЕ: разработчик читает «TLAB» в JFR и считает что это про Old Gen tuning. Тюнит `-XX:NewRatio` ожидая повлиять на TLAB — нулевой эффект, реально нужно `-XX:TLABSize` или `-XX:MinTLABSize`. Wasted tuning effort.
> - [ ] Гипотеза поколений — устаревшая концепция из Java 1.4; современные `ZGC` и `Shenandoah` отказались от поколений и собирают весь `Heap` единообразно | Гипотеза поколений (90%+ объектов умирает молодыми) экспериментально подтверждена и до сих пор актуальна; именно поэтому Generational ZGC появился в Java 21 (давая +30% throughput vs non-generational); G1, Parallel, Serial остаются generational. ❌ ПОСЛЕДСТВИЕ: команда мигрирует на ZGC «потому что отказались от поколений = проще» и теряет throughput на типичной web-нагрузке. Generational ZGC через `-XX:+ZGenerational` (Java 21) восстанавливает преимущество, но без знания о существовании этого флага.
...
> [!mcq]
> - [ ] `ZGC` и `Shenandoah` полностью свободны от `STW`-пауз; вся работа `GC` (маркировка, перемещение, обновление ссылок) выполняется конкурентно с приложением без остановки | У ZGC/Shenandoah остаются короткие STW-фазы: initial mark (sub-millisecond), final mark, начало/конец relocation; основной объём работы конкурентный, но «полностью STW-free» — миф. ❌ ПОСЛЕДСТВИЕ: команда мигрирует на ZGC ожидая «zero pause» и не настраивает SLA с учётом sub-ms STW. На heap > 100GB initial mark может занять несколько ms, видимо в latency p99.99. Без настройки monitoring на ZGC pause events — false confidence в low-latency contract.
> - [x] Concurrent-сборщики (`ZGC`, `Shenandoah`, `G1`) сохраняют короткие `STW`-фазы (initial mark, final mark/remark) для согласования снимка `GC Roots`; основная работа маркировки и перемещения — конкурентна | Невозможно полностью избежать STW при tracing GC: для консистентного снимка корней (стек-фреймы всех потоков) нужна короткая остановка; ZGC/Shenandoah сводят STW к sub-ms, но не к нулю. ✓ ПРИМЕНЯТЬ: для real-time SLA — мониторинг через `-Xlog:gc*,safepoint*` + JFR `GCPhasePause` events; разделять STW-time от concurrent-time в Grafana дашбордах; для p99.99 < 1ms — ZGC + tuning `-XX:ConcGCThreads`. 📋 ПРАВИЛО: «Concurrent GC = short STW phases + bulk concurrent work, not zero STW». 🔗 См. Q22 (collectors), Q24 (ZGC), Q29 (reduce pauses).
> - [ ] Длительность `STW`-паузы в `G1` определяется размером `Heap`: для 32GB Heap пауза ~500ms, для 64GB — ~1s; это фундаментальное ограничение алгоритма | G1 управляет pause через `-XX:MaxGCPauseMillis=200` (default), приоритизируя регионы и не собирая больше чем умещается в target pause; пауза не масштабируется линейно с heap. ZGC/Shenandoah — паузы O(roots), не O(heap). ❌ ПОСЛЕДСТВИЕ: разработчик настраивает `-Xmx` низко «чтобы пауза была короче», теряя throughput из-за частых GC. Реально G1 даёт ~100-200ms на 32GB heap при правильной настройке.
> - [ ] `STW` в `Minor GC` всегда короче чем в `Major/Full GC` — это закон, действующий для всех сборщиков | Обычно так, но не всегда: при сильно фрагментированном Survivor или большом количестве Humongous objects Minor GC может занимать секунды; в G1 «to-space exhausted» приводит к Full GC длительностью минуты. Закон работает в идеальном случае, но не гарантирован. ❌ ПОСЛЕДСТВИЕ: команда мониторит только Full GC duration, игнорируя длительные Minor GC. Production incident: Minor GC 3s из-за реference processing (миллионы WeakReferences в HTTP cache), latency убит, alert не сработал. Мониторить нужно total STW, не только тип.
...
> [!mcq]
> - [x] `System.gc()` — подсказка JVM без гарантии исполнения; в production отключается флагом `-XX:+DisableExplicitGC`, потому что провоцирует `Full GC` с длинной `STW`-паузой | `DisableExplicitGC` делает `System.gc()` no-op; для NIO с `DirectByteBuffer` важно сочетать с явным лимитом direct memory, иначе nativeBB cleanup откладывается. ✓ ПРИМЕНЯТЬ: backend microservice — `-XX:+DisableExplicitGC -XX:MaxDirectMemorySize=512m`; для Netty/Undertow — явный `Cleaner` для off-heap cleanup; alerting на `System.gc invocations` в Prometheus. 📋 ПРАВИЛО: «prod = `DisableExplicitGC` + явный `MaxDirectMemorySize`». 🔗 См. Q11 (memory), Q14 (OOM), Q15 (GC).
> - [ ] `System.gc()` гарантированно запускает `GC` немедленно; `JVM` обязана выполнить полную сборку синхронно до возврата из вызова | `System.gc()` — лишь рекомендация; JVM может её проигнорировать, отложить или выполнить частичную сборку; гарантии немедленного полного GC нет. ❌ ПОСЛЕДСТВИЕ: разработчик использует `System.gc()` перед benchmark для deterministic state — с `-XX:+DisableExplicitGC` (часто в prod-grade configs) это no-op; benchmark inconsistent, micro-regressions ловят шум вместо реальных трендов.
> - [ ] `System.gc()` запускает только `Minor GC` (сборку Young Generation), поэтому его влияние на latency минимально и отключать его в production не нужно | `System.gc()` запускает Full GC через весь Heap включая Old Gen; STW-пауза легко достигает сотен миллисекунд — это и есть причина антипаттерна. ❌ ПОСЛЕДСТВИЕ: legacy library вызывает `System.gc()` каждые 10 минут «для cleanup»; на trade processing с SLA p99 < 100ms каждая такая пауза = 500ms STW, нарушение SLA, штраф в SLA-credits.
> - [ ] Флаг `-XX:+DisableExplicitGC` полностью отключает `GC` в `JVM`; после его включения `JVM` никогда не запускает сборку мусора | `DisableExplicitGC` отключает только явные `System.gc()`/`Runtime.gc()`; автоматический GC при заполнении Heap работает штатно. ❌ ПОСЛЕДСТВИЕ: разработчик путает с `-XX:+UseEpsilonGC` (no-op GC для testing) и избегает `DisableExplicitGC` в production. Legacy-библиотеки продолжают триггерить Full GC, p99 latency скачет, причина не находится месяцами.
...
> [!mcq]
> - [ ] `Parallel GC` — стал default с `Java 9` благодаря многопоточности; `G1` требует явного флага `-XX:+UseG1GC` | Parallel был default до Java 8 включительно; с Java 9 default — именно G1, а не Parallel. ❌ ПОСЛЕДСТВИЕ: миграция Java 8 → 11 с legacy `-XX:+UseParallelGC` в JAVA_OPTS. Команда не убирает старый флаг, p99 регрессирует с 50ms до 200ms из-за длинных Full GC под Parallel; неделя дебага `MaxGCPauseMillis` без эффекта.
> - [ ] `ZGC` — стал default с `Java 11` как первый low-latency сборщик в `production`-статусе | ZGC получил production-статус только в Java 15; default остался G1 даже после этого. ❌ ПОСЛЕДСТВИЕ: команда читает блог 2018 про ZGC, ставит `-XX:+UseZGC` на Java 11 без `-XX:+UnlockExperimentalVMOptions` — JVM не стартует с ошибкой `Error: VM option 'UseZGC' is experimental`. CI green локально (Java 17), prod падает.
> - [ ] `Shenandoah` — стал default с `Java 12` в сборках `OpenJDK` для server-class машин | Shenandoah никогда не был default GC; он доступен как опция с Java 12 в `OpenJDK`, но не в Oracle JDK. ❌ ПОСЛЕДСТВИЕ: разработчик ожидает Shenandoah из коробки в Red Hat OpenJDK; реально нужен явный `-XX:+UseShenandoahGC`. Без флага сервис идёт с G1, low-latency contract нарушен — p99 200ms вместо обещанных 10ms.
> - [x] `G1` (`Garbage First`) — стал default с `Java 9`, заменив `Parallel GC`; оптимизирован для больших heap с предсказуемыми паузами через `MaxGCPauseMillis` | G1 приоритизирует регионы с наибольшим garbage (отсюда название), эффективнее Parallel для server-нагрузок с heap > 4 GB. ✓ ПРИМЕНЯТЬ: Spring Boot 2+ на Java 11+ автоматически на G1; heap < 2GB — оставить Parallel (throughput выше); heap > 32GB — ZGC; тюнить `-XX:MaxGCPauseMillis=200` под latency target. 📋 ПРАВИЛО: «default G1 с Java 9; Parallel = small, ZGC = large». 🔗 См. Q23 (G1), Q24 (ZGC), Q15 (GC).
...
> - [ ] `Mixed GC` в `G1` — это другое название `Full GC`; он собирает весь `Heap` за одну `STW`-паузу | `Mixed GC` — не `Full GC`: он собирает все Young-регионы плюс **часть** Old-регионов с наибольшим garbage («Garbage First»); STW управляется `-XX:MaxGCPauseMillis`, а не размером всего Heap. ❌ ПОСЛЕДСТВИЕ: команда видит `Mixed GC 200ms` в GC log и интерпретирует как Full GC, паникует, эскалирует инцидент. Реально Mixed — штатная efficient incremental collection; ложная тревога съедает on-call hours, monitoring настраивается с лишними алертами.
...
> [!mcq]
> - [ ] `ZGC` использует **forwarding pointers** (дополнительное слово в заголовке объекта) для отслеживания перемещений; это его ключевое отличие от `G1` | Forwarding pointers — механизм `Shenandoah`, а не ZGC; ZGC использует colored pointers (биты в адресе) и не добавляет слово в header. ❌ ПОСЛЕДСТВИЕ: команда планирует memory budget с расчётом «ZGC = +8 байт на объект» как у Shenandoah; реальный footprint меньше, но pod-лимит уже завышен на 15% — переплата за RAM в k8s.
> - [ ] `ZGC` использует **read barriers** и **write barriers** для отслеживания перемещений; оба барьера проверяются при каждом чтении и записи | ZGC использует только load barriers; write barriers применяет G1 для inter-region references; именно load-only barriers — одна из причин низкого overhead ZGC. ❌ ПОСЛЕДСТВИЕ: команда выбирает G1 для write-heavy нагрузки «потому что у ZGC удвоенные barrier overhead». Реально ZGC справляется лучше; latency p99 регрессирует, refactoring через 3 месяца.
> - [ ] `ZGC` достиг `production`-статуса в `Java 11`; с `Java 15` паузы улучшены до `<1 мс` | ZGC оставался experimental до Java 15; production-статус получил именно в Java 15; в Java 11 запуск требует пары флагов `-XX:+UnlockExperimentalVMOptions -XX:+UseZGC`. ❌ ПОСЛЕДСТВИЕ: команда на Java 11 ставит `-XX:+UseZGC` без `UnlockExperimental` — JVM падает с `Error: VM option 'UseZGC' is experimental`; production deploy откатывается, инцидент 30+ минут downtime; root cause — путаница milestone production-readiness vs первое появление флага.
> - [x] `ZGC` использует **colored pointers** — метаданные `GC` хранятся прямо в битах адреса указателя; это позволяет выполнять большую часть работы конкурентно без `STW` | Биты адреса кодируют состояние (marked/remapped/finalizable); load barrier мгновенно определяет действие без обращения к внешним структурам. ✓ ПРИМЕНЯТЬ: торговые платформы (Goldman Sachs, JPMorgan), realtime gaming (Riot Games), HFT-системы; Generational ZGC (Java 21+) даёт +30% throughput через generation hypothesis. 📋 ПРАВИЛО: «`ZGC` = colored pointers, sub-1ms pause, large heap». 🔗 См. Q22 (collectors), Q25 (Shenandoah).
...
> [!mcq]
> - [ ] `Shenandoah` использует **colored pointers** для конкурентного перемещения объектов — так же, как `ZGC`; основное отличие лишь в поддерживаемом размере `Heap` | Colored pointers — механизм ZGC; Shenandoah использует forwarding pointers (слово в header), что даёт другой профиль overhead. ❌ ПОСЛЕДСТВИЕ: команда планирует pod memory с расчётом «как у ZGC», игнорируя +8 байт на каждый объект; на heap с миллиардом маленьких объектов это +8GB native, OOMKilled.
> - [ ] `Shenandoah` доступен в `Oracle JDK` начиная с `Java 15`, но не входит в поставку `OpenJDK` | Всё наоборот: Shenandoah входит именно в `OpenJDK` с Java 12 (Red Hat build, Eclipse Temurin), а Oracle JDK его не включает; `ZGC` есть в обеих поставках. ❌ ПОСЛЕДСТВИЕ: команда платит Oracle JDK Subscription и ставит `-XX:+UseShenandoahGC` — JVM падает `Unrecognized VM option`; неделя дебага инфраструктуры, выясняется что Oracle и не должен включать Shenandoah; решение — миграция на Eclipse Temurin или Red Hat build, плюс пересмотр контракта с Oracle.
> - [ ] `Shenandoah` выполняет компактацию кучи в `STW`-паузе, аналогично `G1`; конкурентным является только этап маркировки | Конкурентная компактация без STW — ключевая особенность Shenandoah относительно G1; именно она обеспечивает суб-10ms паузы на больших Heap. ❌ ПОСЛЕДСТВИЕ: команда не выбирает Shenandoah для latency-критичной системы «компактация всё равно STW как в G1». p99 latency остаётся 200ms на G1 при доступной альтернативе с p99 < 10ms.
> - [x] `Shenandoah` использует **forwarding pointers** в заголовке объекта для конкурентного перемещения; это даёт низкие паузы ценой overhead ~8 байт на объект | Forwarding pointer хранит адрес новой копии при перемещении; read/write barriers проверяют его при каждом обращении — это компромисс за concurrent compaction. ✓ ПРИМЕНЯТЬ: Red Hat OpenJDK для latency-sensitive нагрузок (банки, финтех), особенно на 32-bit/старых ARM (где colored pointers не работают); Eclipse Temurin с Java 15+. 📋 ПРАВИЛО: «Shenandoah = forwarding ptr, OpenJDK only, low pause». 🔗 См. Q22, Q24 (ZGC), Q26 (selection).
...
> [!mcq]
> - [ ] Для batch/ETL-нагрузок (`Spark`, `Hadoop`) лучшим выбором будет `ZGC` — благодаря минимальным паузам он даёт максимальный throughput | Минимальные паузы — главная цель `ZGC`, но он жертвует throughput из-за read/write barriers и concurrent работы. Для batch важна суммарная пропускная способность, и `Parallel GC` обычно даёт +10-15% throughput. ❌ ПОСЛЕДСТВИЕ: команда переводит ETL-pipeline на ZGC «потому что он новый и крутой», ночное окно расчётов растёт с 4ч до 5ч, SLA по data freshness нарушен.
> - [ ] Для всех современных приложений на `Java 17+` нужно сразу включать `ZGC` — `G1` устарел и не рекомендуется | `G1` остаётся `default` GC начиная с Java 9 и подходит для 80% задач — universal, balanced, mature. ZGC оправдан только когда нужны ультра-низкие паузы (<10ms p99). ❌ ПОСЛЕДСТВИЕ: маленький REST-сервис с 1GB Heap включает ZGC, добавляет ~10% CPU overhead, при том что G1-паузы в 50ms никто бы не заметил. Деньги на cloud впустую.
> - [x] Начать с `G1` (default), измерить `GC`-паузы и throughput через `GC`-логи; перейти на `ZGC`/`Shenandoah` только если паузы неприемлемы; `Parallel GC` — для batch/throughput-задач | Подбор GC — это data-driven решение: сначала измерить текущее поведение, потом менять. Преждевременная оптимизация GC — частая ошибка. ✓ ПРИМЕНЯТЬ: Spring Boot REST API → G1; Kafka Streams с миллионами msg/sec → ZGC (low pause); ночной Spark-job → Parallel GC; Lambda с холодным стартом и 256MB → Serial GC. 📋 ПРАВИЛО: «Default G1, замеряй, потом меняй». 🔗 См. Q22 (сборщики), Q24 (ZGC), Q25 (Shenandoah).
> - [ ] Сборщик мусора нужно выбирать по размеру `Heap`: до 4GB — `Serial`, 4-32GB — `G1`, выше — `ZGC` | Размер Heap — лишь один из факторов; гораздо важнее latency vs throughput trade-off, нагрузка на CPU и доступная память контейнера. Для 64GB Heap с batch-нагрузкой Parallel GC может быть лучшим выбором. ❌ ПОСЛЕДСТВИЕ: low-latency trading система с 8GB Heap получает G1 «по таблице», но pауза 200ms убивает SLA — нужно было ZGC по latency-требованию.
...
> [!mcq]
> - [ ] `Heap Dump` используется для диагностики `deadlock`-ов и зависших потоков; `Thread Dump` — для анализа утечек памяти и роста аллокаций | Назначения перепутаны: Heap Dump = все объекты Heap для leak-анализа; Thread Dump = стеки потоков для deadlock/CPU. ❌ ПОСЛЕДСТВИЕ: при OOM снимают thread dump вместо heap dump; threads не показывают leak, время потеряно. Workflow `OOM → heap dump → Eclipse MAT` сорван, ребят утром после ночного incident.
> - [ ] `kill -3 <pid>` убивает процесс и одновременно сохраняет `Heap Dump` в файл; это быстрый способ снять дамп в production | `SIGQUIT` (`kill -3`) не убивает Java-процесс и не создаёт Heap Dump; он печатает Thread Dump в stdout. ❌ ПОСЛЕДСТВИЕ: разработчик пишет `kill -3` ожидая heap dump — dump уходит в логи (Thread, не Heap), heap dump не создан; при OOM забывают `-XX:+HeapDumpOnOutOfMemoryError`, post-mortem невозможен.
> - [ ] `Heap Dump` в формате `.hprof` содержит только статистику по типам объектов (гистограмму); полное содержимое объектов недоступно по соображениям безопасности | `.hprof` содержит полный снимок объектов со значениями полей и цепочками удержания; именно поэтому дамп может быть равным размеру Heap и содержать чувствительные данные. ❌ ПОСЛЕДСТВИЕ: команда отправляет heap dump в Slack «для анализа»; внутри — пароли, JWT токены, PII в `String`-полях; security incident, GDPR-нарушение, репорт DPO.
> - [x] `Thread Dump` (`jstack`/`jcmd Thread.print`) показывает стек каждого потока и состояние блокировок; используется для `deadlock`-ов, зависших потоков, высокой загрузки `CPU` | JVM автоматически отмечает deadlock-и в дампе; снимая 3-5 дампов с интервалом 1с, можно найти thread, постоянно занятый одной точкой стека — это hot path. ✓ ПРИМЕНЯТЬ: high CPU → серия thread dumps + diff; deadlock → строка `Found one Java-level deadlock` в дампе; `fastthread.io` автоматизирует анализ. 📋 ПРАВИЛО: «thread dump = CPU/deadlock; heap dump = memory leak». 🔗 См. Q31 (tools), Q33 (leaks), Q34 (JFR).
...
> [!mcq]
> - [ ] `Escape Analysis` выполняется `C1`-компилятором и позволяет размещать объекты на `Stack` напрямую; `C2` не участвует в этой оптимизации | Escape Analysis и scalar replacement — оптимизации именно `C2` на основе профиля от `C1`; `C1` выполняет только базовые оптимизации для быстрого старта. ❌ ПОСЛЕДСТВИЕ: команда ставит `-XX:TieredStopAtLevel=1` ожидая «C1 сам всё оптимизирует»; allocation rate в hot loop растёт в 5x, GC pressure ломает p99 — escape analysis в C1 нет.
> - [ ] `Scalar replacement` перемещает объект из `Heap` в `Stack` физически, сохраняя его как единый блок; это ускоряет доступ, но не снижает давление на `GC` | Scalar replacement разбивает объект на скалярные поля (`int`, `double`) в регистрах/стеке, полностью исключая аллокацию в Heap — именно это снижает GC pressure. ❌ ПОСЛЕДСТВИЕ: разработчик пишет «оптимизированный» код с примитивами вместо `Optional`/`Stream` «потому что Stream аллоцирует»; код становится нечитаемым, perf тот же — JIT уже elide allocation.
> - [ ] `Lock elision` работает только с `synchronized`-методами; для `ReentrantLock` и `volatile` оптимизация не применяется | Lock elision работает с любыми объектами синхронизации, не убегающими из метода; `ReentrantLock` — объект в Heap, и при scalar replacement его можно тоже элиминировать. ❌ ПОСЛЕДСТВИЕ: команда переписывает `synchronized` на `ReentrantLock` для «большей JIT-оптимизации»; реально JIT elide оба варианта, выгоды ноль, появляется boilerplate `try/finally + unlock()` — cargo cult refactoring.
> - [x] `Escape Analysis` в `C2` определяет, выходит ли объект за пределы метода/потока; если нет — `JIT` применяет `scalar replacement` (поля в регистрах/стеке) и `lock elision` (убирает синхронизацию) | Не убегает — не нужен в Heap; scalar replacement разбивает объект на поля, lock elision убирает бессмысленные `synchronized`-блоки. ✓ ПРИМЕНЯТЬ: использовать `Optional`, `Stream`, маленькие DTO без страха — JIT их элиминирует; JFR `Allocation Profiling` показывает реальные аллокации; JMH `@BenchmarkMode(AverageTime)` для замеров. 📋 ПРАВИЛО: «не убегает = не аллоцируется». 🔗 См. Q5 (JIT), Q11 (memory), Q15 (GC).
...
> [!mcq]
> - [ ] `Spring AOT` вычисляет `@Conditional` и `@Profile` при **каждом старте**, но быстрее обычного Spring, потому что AOT-код скомпилирован `C2` | Spring AOT вычисляет условия один раз во время **сборки**, а не на каждом запуске; именно поэтому профили нельзя менять в runtime без пересборки. ❌ ПОСЛЕДСТВИЕ: разработчик переключает `@Profile("dev")` через `SPRING_PROFILES_ACTIVE` в Native Image — не работает; профиль зашит при build, нужны отдельные образы per env, путаница в release pipeline.
> - [ ] `Spring AOT` генерирует только конфигурацию для `Native Image` и не влияет на время старта обычной `JVM` | AOT полезен в двух режимах: для Native Image (обязателен) и для обычной JVM (ускоряет старт через сгенерированный код инициализации `ApplicationContext`). ❌ ПОСЛЕДСТВИЕ: команда не использует AOT для обычной JVM-сборки; production startup 30 сек вместо 18 сек; в k8s rolling update медленный, readiness probe требует завышенного `initialDelaySeconds=60`.
> - [ ] `Spring AOT` совместим со всеми версиями `Spring Boot` начиная с `2.7`; поддержка добавлена как optional feature через отдельный starter | Spring AOT интегрирован именно в Spring Boot 3; в 2.x его нет (был экспериментальный проект Spring Native, deprecated). ❌ ПОСЛЕДСТВИЕ: команда на Spring Boot 2.7 ищет goal `spring-boot:process-aot` — не находит; дни на debugging build pipeline, переключение между Maven и Gradle plugins; правильный путь — миграция на Spring Boot 3, иначе native-image не получится без deprecated Spring Native.
> - [x] `Spring AOT` вычисляет `@Conditional` и `@Profile` **при сборке** (build time), а не в runtime; это ограничение: конфигурация среды не должна меняться после сборки без пересборки приложения | Перенос вычисления условий на build time — принципиальный компромисс AOT: быстрый старт ценой потери runtime-гибкости; в dev AOT-режим обычно отключают. ✓ ПРИМЕНЯТЬ: AWS Lambda + Spring Boot 3 Native — отдельные образы per env (dev/staging/prod); обычная JVM — `./mvnw spring-boot:process-aot` + `spring.aot.enabled=true`. 📋 ПРАВИЛО: «Spring AOT = build-time conditions, rebuild per env». 🔗 См. Q7 (GraalVM/AOT), Q38 (Native limits).
