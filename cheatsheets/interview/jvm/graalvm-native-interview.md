...
Ещё одна особенность, о которой стоит знать: **build-time initialization и image heap**. Часть статических инициализаторов выполняется не при запуске, а на этапе СБОРКИ, и созданные ими объекты «запекаются» в бинарник (image heap) — отчасти поэтому старт и получается мгновенным. Классическая ловушка: seed генератора случайных чисел, прочитанная env-переменная или «текущее» время фиксируются в момент сборки и одинаковы при каждом запуске (подробнее в Q10).
...
**Как AOT сокращает разрыв — PGO.** Oracle GraalVM поддерживает **profile-guided optimization**: сначала собирается инструментированный бинарник (`--pgo-instrument`), он прогоняется под типовой нагрузкой и записывает профиль выполнения, затем финальная сборка с `--pgo` использует этот профиль. Так AOT получает ту самую «картину реального выполнения», которой ему не хватало, и заметно приближается к разогретому JIT по пиковой производительности.
...
Например, граф достижимости от `main()` выглядит так:

- `main` → `ClassA`
- `ClassA` → `ClassB`; `ClassB` → `ClassD`
- `ClassA` → `ClassC` (вызов через рефлексию — `via reflection?`)
- `ClassC` ⇢ `ClassE` — связь «не видно статически» (пунктир): достижима только через рефлексию.

Здесь `ClassE` достижим только через рефлексию из `ClassC` — статический анализ этой связи не видит, класс выпадает из графа, и в рантайме вылетит `ClassNotFoundException`.
...
Разрыв по throughput не приговор: PGO-сборка в Oracle GraalVM (`--pgo`, см. Q2) скармливает AOT-компилятору профиль реальной нагрузки и возвращает значительную часть JIT-оптимизаций — на типовых бенчмарках native с PGO подбирается вплотную к разогретой JVM.
...
| Mockito / `@MockBean` | Рантайм-генерация байткода — в native tests не работает | Моки только в JVM-тестах; в native — реальные бины + WireMock/Testcontainers |
| JVMTI агенты | Profilers, debuggers на базе JVMTI | Нативные отладчики (gdb, lldb) |
| Время сборки | 3-5 минут | Неизбежно |

**Отдельная ловушка — build-time initialization.** Классы, инициализированные на этапе сборки (`--initialize-at-build-time`), выполняют свои статические инициализаторы в JVM сборочной машины, а результат «запекается» в image heap бинарника. Если в `static`-блоке захвачен seed для `Random`, env-переменная или `System.currentTimeMillis()` — это значение зафиксируется навсегда и будет одним и тем же при каждом запуске на любой машине. Лечение: перенести инициализацию проблемных классов в рантайм через `--initialize-at-run-time`.
...
public class ApplicationRuntimeHints implements RuntimeHintsRegistrar {
    ...
    public void registerHints(RuntimeHints hints, ClassLoader classLoader) {
        ...
        // JDK-прокси для Spring AOP поверх интерфейса
        hints.proxies()
...
**Важный нюанс про прокси:** `registerJdkProxy` регистрирует только **JDK-прокси**, а они работают исключительно по интерфейсам. CGLIB-прокси для классов (например, `@Transactional` на классе без интерфейса) через `proxies()`-hints не регистрируются — их код Spring AOT генерирует на этапе сборки как обычные классы, и в рантайм-регистрации они не нуждаются.
...
**Подводный камень native-тестов — моки.** Mockito генерирует моки байткодом в рантайме — ровно то, что closed world запрещает. И `@MockBean`/`@SpyBean` построены на том же Mockito, поэтому документация Spring Boot прямо говорит: **в native-тестах они не поддерживаются**. Рабочая стратегия:
- **моки — только в обычных JVM-тестах**: unit-тесты и тест-слайсы с `@MockBean` гоняются на JVM как раньше, native-сборка их не касается;
- **native-тесты идут по реальным бинам**: `@SpringBootTest` + `@AutoConfigureMockMvc` работают штатно, проверяется настоящая проводка контекста;
- **внешние зависимости подменяй не моками, а реальными заменителями**: HTTP-вызовы — WireMock, базы и брокеры — Testcontainers.
...
class UserControllerNativeTest {
    ...
    // Моков нет: native-тест идёт по реальным бинам.
    // Внешний HTTP подменяет WireMock, БД — Testcontainers
...
Итог для собеседования: native-тесты проверяют не бизнес-логику (она уже покрыта на JVM), а **собираемость и работоспособность в native** — что hints полны и контекст поднимается. Поэтому отсутствие моков здесь не ограничение, а суть: мокать значит прятать то, что native-тест и должен проверить.
