...
**Антипаттерн** — это решение, которое поначалу кажется разумным и потому повторяется снова и снова, но в долгосрочной перспективе делает код хуже: сложнее в поддержке, отладке и развитии. Это «ловушка» — естественный путь, который заводит не туда. Знание антипаттернов не менее важно, чем знание паттернов: именно оно помогает распознать проблему до того, как она укоренится в кодовой базе.
...
| Ленивая инициализация | Да, на уровне загрузки класса (JVM инициализирует `enum` при первом обращении) | DCL: да, Holder: да | Нет |

**Нюанс про «ленивость»:** enum-Singleton **ленив** — по JLS класс инициализируется только при первом обращении к константе, то есть ресурсы не выделяются раньше времени, ровно как в holder idiom (там лениво грузится вложенный класс-холдер). Честное ограничение в другом: момент инициализации нельзя контролировать вручную и нельзя передать параметры в конструктор по требованию (вида `getInstance(config)`) — ни enum, ни holder этого не умеют, такая параметризованная инициализация возможна только в DCL-варианте с явным методом-фабрикой.
...
Главная проблема — гарантия «ровно один экземпляр» держится только в однопоточном коде. Под нагрузкой ленивая инициализация без синхронизации ломается тремя способами:

1. **Race condition** — два потока одновременно проходят проверку `instance == null`, оба видят `null` и оба вызывают `new`. В итоге создаётся два экземпляра, и инвариант Singleton нарушен.
2. **Видимость изменений (instruction reordering)** — `new Singleton()` это не атомарная операция: выделение памяти, вызов конструктора и присваивание ссылки JIT/процессор могут переупорядочить. Без `volatile` второй поток способен увидеть уже не-`null` ссылку на ещё **недостроенный** объект и начать с ним работать.
3. **Несогласованное состояние** — как следствие первых двух пунктов, разные потоки получают разные экземпляры, и любое состояние внутри Singleton (кэш, счётчик, конфиг) расходится.

**Как лечить:** DCL с `volatile`, Bill Pugh Holder или `enum` (см. Q6, Q7) — все три закрывают и race condition, и проблему видимости. Подробнее о модели памяти и `volatile` — в [вопросах по Java Concurrency](../programming-languages/java/java-concurrency-interview.md).

## Q9. В чём недостатки паттерна `Singleton`?

Коротко: `Singleton` решает реальную задачу (один экземпляр), но платит за это связанностью и тестируемостью — поэтому в современной Java его почти всегда заменяют DI-контейнером.

1. **Глобальное состояние** — `Singleton` по сути замаскированная глобальная переменная. Любой код в системе может его читать и менять, что делает поведение труднопредсказуемым.
2. **Затрудняет тестирование** — зависимость нельзя подменить моком без специальных усилий: классы жёстко прибиты к конкретной реализации через `getInstance()`. Тесты начинают влиять друг на друга через общее состояние.
3. **Нарушает SRP** — класс отвечает и за свою бизнес-логику, и за контроль собственного жизненного цикла (количество экземпляров). Это две разные ответственности.
4. **Скрытые зависимости** — вызов `Singleton.getInstance()` спрятан внутри метода, поэтому зависимость не видна из сигнатуры класса. Читатель и тесты узнают о ней только заглянув в реализацию.
5. **Проблемы с classloader'ами** — в среде с несколькими `ClassLoader` (например, application server) каждый загрузчик создаёт свой экземпляр, и «единственность» нарушается.
...
`Abstract Factory` создаёт **семейства связанных объектов** без привязки к их конкретным классам. Главная ценность — гарантия **согласованности**: фабрика отдаёт только совместимые между собой объекты. Например, в примере ниже `MaterialUIFactory` физически не способна вернуть `IosCheckbox` — все элементы интерфейса будут одного стиля. Клиент работает с одной фабрикой и не рискует смешать Material-кнопку с iOS-чекбоксом.
...
- Создаётся **семейство** взаимосвязанных объектов (кнопка + чекбокс + текстовое поле одного стиля), а не один объект.
- Объекты внутри семейства должны быть **совместимы** друг с другом, и систему нужно защитить от их случайного смешивания.
- Если же создаётся **один тип** объекта — хватает `Factory Method`. Разница масштаба: `Factory Method` это один метод-фабрика, `Abstract Factory` это интерфейс из нескольких таких методов, объединённых общей «темой».

## Q12. (!) Что такое `Builder` Pattern?

`Builder` собирает сложный объект пошагово, через цепочку именованных вызовов. Решает проблему **telescoping-конструкторов**: когда у объекта много опциональных полей, без Builder приходится либо плодить десяток перегрузок конструктора, либо вызывать `new HttpRequest(url, "POST", headers, body, 5000)` — где по позиции аргументов невозможно понять, что есть что, и легко перепутать порядок. Builder даёт читаемый код (`.method("POST").timeout(5000)`), позволяет задавать значения по умолчанию и собирать **иммутабельный** объект (поля `final`, конструктор приватный).
...
`Prototype` (прототип) создаёт новые объекты, **копируя** уже готовый объект-образец, вместо построения с нуля через `new`. Полезен в двух случаях: когда инициализация дорогая (чтение из БД, тяжёлые вычисления) — дешевле склонировать готовый экземпляр; и когда клиенту нужно создать копию, не зная её конкретного класса — он просто зовёт `clone()` на прототипе.
...
**Class adapter vs object adapter** — частый follow-up по GoF, в книге описаны оба варианта:

- **Object adapter (композиция)** — адаптер хранит ссылку на adaptee и делегирует ему вызовы. Именно этот вариант в примере выше, и именно он основной в Java: работает с любым подклассом adaptee, adaptee может быть даже `final`, и один адаптер при желании оборачивает разные реализации.
- **Class adapter (наследование)** — адаптер одновременно реализует целевой интерфейс и **наследует** adaptee, переводя вызовы через переопределение/вызов унаследованных методов. В GoF он построен на множественном наследовании (C++); в Java возможен только в форме `class Adapter extends Adaptee implements Target`, то есть адаптируемый тип должен быть классом (не `final`), и адаптировать можно ровно один класс. Плюс — можно точечно переопределять поведение adaptee без делегирования; минус — жёсткая связка с конкретным классом и его иерархией.

На собеседовании достаточно сказать: «в Java по умолчанию object adapter — композиция гибче наследования», и пояснить, что class adapter ограничен одиночным наследованием.
...
`Bridge` (мост) разделяет абстракцию и её реализацию на две независимые иерархии, чтобы они могли развиваться отдельно. Применяется, когда у задачи есть **два ортогональных измерения** изменчивости — в примере ниже это «тип уведомления» (срочное / обычное) и «способ доставки» (email / SMS). Эти измерения не зависят друг от друга, и Bridge связывает их через композицию, а не наследование.
...
**Почему это выгодно.** Если связывать измерения наследованием, число классов растёт как их произведение: 2 типа × 2 способа = 4 класса (`UrgentEmail`, `UrgentSms`, `RegularEmail`, `RegularSms`), а 3 типа × 4 способа дали бы уже 12. С `Bridge` каждое измерение живёт в своей иерархии, поэтому рост **аддитивный** (2 + 2 = 4 класса, 3 + 4 = 7), а не мультипликативный. Добавить новый способ доставки — это один новый класс `MessageSender`, который сразу работает со всеми типами уведомлений.
...
`Proxy` (заместитель) подставляет вместо реального объекта суррогат с **тем же интерфейсом**, который контролирует доступ к нему. Клиент не замечает подмены — он вызывает методы прокси, а тот решает, когда и как обратиться к настоящему объекту (и обращаться ли вообще). Ключевое отличие от `Decorator`: декоратор **расширяет** поведение объекта, а прокси **управляет доступом** к нему — кэширует, проверяет права, откладывает создание, перенаправляет вызов по сети.
...
`Facade` (фасад) даёт **один простой интерфейс** к сложной подсистеме из множества классов. В примере ниже клиент вызывает один метод `play()`, а фасад сам оркестрирует кодек, микшер, парсер субтитров и рендерер. Фасад не добавляет новой функциональности и не меняет интерфейсы — он лишь скрывает сложность и снижает связанность клиента с внутренностями подсистемы.

**Отличие от Adapter:** Adapter подгоняет интерфейс под *требуемый* (совместимость), Facade *упрощает* доступ к группе классов (удобство). Adapter обычно оборачивает один объект, Facade — целую подсистему.
...
`Composite` (компоновщик) строит древовидные структуры «часть-целое» и позволяет обращаться к отдельному объекту (листу) и к группе объектов (узлу) через **один и тот же интерфейс**. В примере `File` и `Directory` оба реализуют `FileSystemComponent`, поэтому клиент вызывает `getSize()`, не зная и не проверяя, файл это или папка. Папка же рекурсивно суммирует размеры детей — и клиентский код не разрастается ветвлениями `if (это папка) ... else ...`. Именно в этом ценность: единообразие обхода вместо проверок типа на каждом шаге.
...
`Flyweight` (легковес) экономит память, разделяя одно и то же неизменяемое состояние между множеством объектов. Ключ к паттерну — разделение состояния на две части:

- **Intrinsic (внутреннее)** — общее, неизменяемое, не зависит от контекста. В примере это `name`, `color`, `texture` дерева. Хранится **внутри** flyweight'а и переиспользуется фабрикой.
- **Extrinsic (внешнее)** — уникальное для каждого использования, зависит от контекста. Это координаты `x`, `y`, которые передаются **снаружи** в метод `draw(x, y)`.

Вместо миллиона объектов-деревьев с дублирующимися текстурами создаётся горстка `TreeType` (по одному на уникальную комбинацию), а позиции хранятся отдельно. Так память тратится на один экземпляр intrinsic-состояния, а не на каждое дерево в лесу.
...
| `Mediator` | Централизованное взаимодействие | чистых примеров в JDK нет (в Spring — `DispatcherServlet`) |
| `Visitor` | Операция над структурой без изменения классов | `FileVisitor` |
| `Memento` | Сохранение/восстановление состояния | `javax.swing.undo.UndoManager` (хранит снимки для undo) |
...
`State` (состояние) позволяет объекту менять поведение при изменении внутреннего состояния — со стороны кажется, будто объект меняет свой класс. Каждое состояние выносится в отдельный класс, а объект-контекст делегирует ему вызовы. Это убирает громоздкий `switch (status)` в каждом методе: вместо проверки «если статус NEW, то..., если PAID, то...» объект просто вызывает `state.next()`, а конкретный класс состояния знает, что делать и в какое состояние перейти. В примере заказ переходит `New → Paid → Shipped`, и каждый класс сам определяет допустимые переходы (например, отгруженный заказ нельзя отменить).
...
`Observer` (наблюдатель) задаёт зависимость «один ко многим»: когда издатель (subject) меняет состояние, он автоматически уведомляет всех подписчиков (observers). Решает проблему связанности: издатель не знает, кто и сколько на него подписано, и не вызывает подписчиков напрямую — он лишь публикует событие. Подписчиков можно добавлять и удалять в runtime, не трогая код издателя. В примере `EventBus` публикует событие `order.created`, а кто его слушает (отправка email, запись в лог) — издателю безразлично.
...
**Подводные камни Observer:**

- **Lapsed listener problem (утечка памяти).** Издатель держит **сильные ссылки** на подписчиков. Если объект больше не нужен приложению, но забыл отписаться, GC не сможет его собрать — он «жив» через список слушателей издателя-долгожителя. Классический источник утечек в UI и долгоживущих сервисах. Лечение: явная отписка в методе очистки (`close()`/`destroy()`), либо хранение слушателей через `WeakReference` — тогда забытый подписчик не мешает сборке мусора, но появляется недетерминизм «слушатель может молча исчезнуть». В примере выше `EventBus` именно с сильными ссылками и без `unsubscribe` — для продакшена метод отписки обязателен.
- **Синхронная vs асинхронная доставка.** Синхронная (как в примере: `publish` обходит слушателей в текущем потоке) проста, сохраняет порядок и видит контекст вызова (например, транзакцию), но медленный подписчик тормозит издателя, а исключение в одном слушателе ломает доставку остальным. Асинхронная (очередь + отдельный поток/пул) изолирует издателя от подписчиков и друг от друга, но добавляет вопросы порядка событий, обработки ошибок и того, что событие может обработаться уже после отката транзакции. В Spring `ApplicationEvent` по умолчанию доставляется **синхронно**, асинхронность включают явно (`@Async` на слушателе); для «после коммита» есть `@TransactionalEventListener`.
...
`Chain of Responsibility` (цепочка обязанностей) пропускает запрос через цепочку обработчиков: каждое звено решает само, обработать запрос, передать дальше или сделать и то, и другое. Отправитель не знает, кто именно обработает запрос — он просто кидает его в начало цепочки. Это развязывает отправителя и получателей: набор и порядок обработчиков можно менять, не трогая клиента.

Есть два варианта поведения цепочки: остановиться на первом подходящем обработчике (как в Servlet-фильтрах с прерыванием) или прогнать запрос через все звенья. В примере ниже реализован второй вариант — каждый логгер проверяет уровень и в любом случае передаёт сообщение следующему.
...
`Command` (команда) превращает запрос (действие) в самостоятельный объект. Почему это полезно: как только действие становится объектом, его можно передавать, хранить в переменной, складывать в очередь или историю, логировать и откатывать. В примере объект `LightOnCommand` несёт в себе и сам вызов (`execute`), и обратную операцию (`undo`) — поэтому история команд `CommandHistory` умеет отменять последнее действие, а `MacroCommand` — собирать несколько команд в одну. Команда развязывает «того, кто инициирует действие» и «того, кто его выполняет».
...
`Visitor` (посетитель) выносит операции над структурой объектов в отдельные классы-визиторы, позволяя добавлять новые операции **без изменения** самих классов структуры. Чтобы посчитать слова или экспортировать документ в HTML, не нужно дописывать методы в `Paragraph` и `Image` — достаточно написать новый визитор.

**Механизм — double dispatch.** Вызываемый метод выбирается по двум типам сразу: элемента и визитора. Работает так: клиент зовёт `element.accept(visitor)` (первый dispatch — по типу элемента), а внутри `accept` элемент зовёт `visitor.visit(this)` (второй dispatch — по типу элемента через перегрузку). Это обходит то, что в Java перегрузка методов разрешается на этапе компиляции, а не в runtime.

**Компромисс:** добавлять новые **операции** легко (новый визитор), но добавлять новые **типы элементов** дорого — придётся править интерфейс визитора и все его реализации. Поэтому Visitor хорош для стабильной иерархии типов с растущим набором операций.
...
`Iterator` (итератор) даёт единый способ последовательно обходить элементы коллекции, не раскрывая её внутреннее устройство (массив, связный список, дерево). Клиент работает с интерфейсом `hasNext()`/`next()` и не зависит от того, как коллекция хранится внутри — один и тот же цикл обходит и `ArrayList`, и `HashSet`. В Java паттерн встроен в язык: реализовав `Iterable`, класс автоматически работает с `for-each`, как в примере `NumberRange` ниже.
...
`Mediator` (посредник) централизует взаимодействие группы объектов: вместо того чтобы каждый знал о каждом и обращался напрямую, все общаются через одного посредника. Это решает проблему «паутины связей»: при прямом взаимодействии N объектов в худшем случае образуется до N×N связей, и любое изменение задевает многих. С медиатором каждый объект знает только о посреднике — связи становятся «звездой», а логика взаимодействия собирается в одном месте. В примере `User` не держит ссылок на других пользователей: он шлёт сообщение в `GroupChat`, а тот рассылает его остальным.
...
`Memento` (хранитель) позволяет сохранить снимок состояния объекта и позже восстановить его, **не раскрывая внутреннее устройство** объекта наружу. Главное — инкапсуляция: тот, кто хранит снимки, не видит и не может менять их содержимое, он работает с непрозрачным «токеном». В паттерне три роли: **Originator** (`TextEditor`) — объект, чьё состояние сохраняем; **Memento** (`EditorMemento`) — неизменяемый снимок; **Caretaker** (`EditorHistory`) — хранит снимки в стопке и инициирует откат, но не заглядывает внутрь. Классическое применение — undo/redo.
...
Оба паттерна решают одну задачу — доставить классу его зависимости, но с противоположным направлением управления. При **DI** зависимости *внедряются* извне (контейнер/вызывающий код передаёт их в конструктор), и класс пассивен. При **Service Locator** класс сам *запрашивает* зависимость у глобального реестра внутри метода. Отсюда ключевая разница: при DI зависимости видны в сигнатуре и легко подменяются в тестах, а Service Locator прячет их внутри реализации и привязывает класс к реестру.
...
Несмотря на одинаковое название, это разные вещи, и путать их — частая ошибка. GoF `Singleton` гарантирует один экземпляр **на JVM/ClassLoader** силами самого класса (приватный конструктор). Spring `singleton` scope гарантирует один экземпляр **на `ApplicationContext`** силами контейнера, при этом обычный конструктор класса остаётся доступен. Поэтому в разных контекстах будут разные бины, а тестировать Spring-бин легко (его подменяют через DI) — в отличие от классического Singleton.
...
`Null Object` (нулевой объект) заменяет `null` объектом с «ничего-не-делающим» поведением, реализующим тот же интерфейс. Выгода — клиентский код избавляется от проверок `if (x != null)`: он всегда вызывает методы как обычно, а нулевой объект просто молча ничего не делает. Это убирает риск `NullPointerException` и упрощает логику. В примере `NullLogger` реализует `Logger`, но его `log()` пуст, поэтому сервис может звать `logger.log(...)` без оглядки на `null`.
...
`Interpreter` (интерпретатор) представляет грамматику языка в виде иерархии классов: каждое правило грамматики — отдельный класс с методом `interpret()`, а выражение разбирается в дерево из таких объектов и вычисляется рекурсивным обходом. Применяется для разбора **грамматик** и **DSL** (Domain-Specific Languages). На практике встречается редко: для нетривиальной грамматики число классов взрывается, а поддержка усложняется.
...
`Specification` (спецификация) — паттерн (не из GoF 23, но широко применяется в DDD), который оборачивает одно **бизнес-правило** в отдельный объект с методом-предикатом `isSatisfiedBy()`. Главная ценность — **комбинируемость**: мелкие правила собираются в сложные через `and`/`or`/`not`, как кубики. Вместо громоздкого условия `if (c.isActive() && c.getTotalPurchases() > 10000 && c.getAge() >= 18)`, разбросанного по коду, каждое правило живёт в своём классе, переиспользуется и тестируется отдельно, а нужная комбинация собирается декларативно (см. `eligibleForLoan` ниже).
