...
> [!mcq]
> - [x] `Design Patterns` — это проверенные шаблоны решения типичных задач проектирования, описывающие взаимодействие классов и объектов. | Точное определение: паттерн — это абстракция над решением, а не код. GoF каталогизировал 23 таких шаблона в 1994 году.
> - [ ] `Design Patterns` — это готовые библиотеки классов, которые подключаются к проекту как зависимости. | Распространённое заблуждение: паттерны не поставляются как артефакты, это концепции. Их реализация зависит от языка, контекста и требований.
...
> - [ ] `Design Patterns` — это инструменты статического анализа для поиска ошибок в коде. | Путается с линтерами (SonarQube, Checkstyle). Паттерны — про структуру проектирования, а не про автоматический анализ.
...
> [!mcq]
> - [ ] `Creational` — 7, `Structural` — 5, `Behavioral` — 11 (всего 23). | Путаются цифры структурных и порождающих: порождающих 5, структурных 7. Итоговое количество 23 указано верно.
...
> - [ ] `Creational` — 5, `Structural` — 7, `Behavioral` — 9 (всего 21). | Неверное число поведенческих: их 11, включая `Interpreter` и `Mediator`, которые часто забывают.
> - [ ] `Creational` — 6, `Structural` — 6, `Behavioral` — 11 (всего 23). | Путается категоризация: `Prototype` относится к Creational, `Flyweight` — к Structural, категории не содержат по 6 паттернов.
...
> [!mcq]
> - [ ] `God Object` — это когда один объект делегирует всю работу другим, нарушая инкапсуляцию. | Наоборот: `God Object` всё делает сам, не делегирует. Антипаттерн — концентрация ответственности, а не её размытие.
...
> - [x] `God Object` — один класс концентрирует слишком много ответственности и знает обо всём в системе. | Каноничное определение: God Object нарушает SRP и Low Coupling. Решается разбиением на классы с узкой ответственностью и инжекцией зависимостей.
> - [ ] `God Object` — это паттерн, при котором все классы наследуются от единого базового класса-родителя. | Описывается глубокая иерархия наследования, а не God Object. God Object — про размер одного класса, а не про иерархию.
...
> [!mcq]
> - [ ] Главный риск паттернов — это снижение производительности из-за дополнительных вызовов виртуальных методов. | JIT в HotSpot инлайнит такие вызовы, производительность редко страдает. Реальный риск — сложность и количество классов, а не байткод.
> - [ ] Главное преимущество — автоматическое соответствие стандартам безопасности и ISO-сертификации. | Паттерны не связаны со стандартами безопасности (OWASP, ISO). Это чисто структурные решения, не дающие никаких compliance-гарантий.
> - [ ] Главный риск — несовместимость паттернов между языками, что усложняет миграцию проекта. | Большинство GoF-паттернов переносимы между ООП-языками: Java, C#, C++, Python. Проблема миграции — в библиотеках и рантайме, не в паттернах.
> - [x] Главный риск — overengineering: применение `Strategy` из 2-х классов, `Abstract Factory` там, где хватило бы `new`, резко раздувает кодовую базу. | Классическая ошибка джуна/мидла: применять паттерны «потому что правильно». Пока нет второго варианта, простое решение лучше абстракции.
...
> - [ ] `OCP` напрямую реализуется только через `Singleton`, так как гарантирует единственную точку расширения. | Singleton не про расширение — он его ограничивает. OCP реализуется через полиморфизм, не через глобальное состояние.
...
> - [ ] `SRP` запрещает любое использование `Observer`, потому что подписчик берёт на себя несколько обязанностей. | Наоборот: Observer помогает разделять ответственность — издатель не знает о логике подписчиков. SRP реализуется в том числе через Observer.
...
> - [ ] `Eager initialization` не потокобезопасна и требует отдельной синхронизации при первом обращении. | `static final` поле инициализируется JVM потокобезопасно через class initialization lock. Никакая дополнительная синхронизация не нужна.
...
> - [ ] `enum` Singleton требует ручной реализации `readResolve()` для защиты от десериализации. | `enum` автоматически сериализуется через имя, а не через поля, и десериализация всегда возвращает тот же экземпляр. `readResolve()` не нужен.
...
> [!mcq]
> - [ ] Главная проблема — исключение `ConcurrentModificationException` при одновременном вызове `getInstance()` из нескольких потоков. | `CME` возникает при модификации коллекции во время итерации — никак не связан с Singleton и `getInstance()`.
...
> - [x] Ключевая проблема — race condition: два потока проходят проверку `instance == null`, оба создают объект, результат — два разных экземпляра и несогласованное состояние. | Классическая гонка при lazy-init без синхронизации. Решается либо `synchronized`, либо DCL с `volatile`, либо Bill Pugh Holder, либо `enum`.
...
> - [ ] Singleton несовместим с `final` полями, что ломает immutability. | Никакой несовместимости нет: `private static final Singleton INSTANCE` — корректная и распространённая реализация (eager init).
...
> [!mcq]
> - [ ] `Simple Factory` — это канонический GoF-паттерн, а `Factory Method` — его упрощённая вариация. | Ровно наоборот: `Factory Method` входит в каталог GoF, `Simple Factory` — просто идиома (часто статический метод с `switch`), которой в GoF нет.
...
> - [ ] `Factory Method` всегда создаёт один конкретный класс, заранее заданный в конструкторе. | Суть паттерна — отложить выбор класса до подкласса. Если класс заранее известен — никакой паттерн не нужен, хватит `new`.
...
> [!mcq]
> - [ ] `Abstract Factory` — порождающий паттерн для создания одного объекта без указания конкретного класса, синоним `Factory Method`. | Это определение `Factory Method`. `Abstract Factory` отличается тем, что создаёт **семейство** взаимосвязанных продуктов.
> - [ ] `Abstract Factory` применяется только в UI-фреймворках для реализации тем оформления. | UI — распространённый пример, но паттерн применим везде, где нужны совместимые семейства (драйверы БД с разными диалектами, cross-platform toolkits).
...
> - [ ] `Abstract Factory` используется только вместе с `Singleton` — фабрика обязана быть глобальной. | Singleton — частая, но необязательная комбинация. Фабрика может быть инжектирована через DI в нужной области видимости.
...
> [!mcq]
> - [ ] `Builder` предназначен для динамической смены поведения объекта в runtime через замену алгоритма. | Это описание `Strategy`. Builder — про конструирование, а не про смену поведения после создания.
> - [ ] `Builder` используется для клонирования существующих объектов с модификациями. | Это описание `Prototype` + with-методов. Builder создаёт объект с нуля через fluent-API, а не копирует.
> - [ ] `Builder` — это способ объединить несколько объектов в одно дерево с единым интерфейсом. | Это `Composite`. Builder не про иерархии, а про пошаговое конструирование одного объекта.
...
> - [ ] `Object.clone()` всегда делает **deep copy** всех полей, включая вложенные коллекции. | Это неверно и опасно: JVM физически не может сделать полную deep-копию без знания семантики полей. Deep copy нужно реализовать вручную.
> - [ ] `Object.clone()` требует, чтобы класс наследовался от `Prototype`. | Такого класса в JDK нет. Контракт — это маркер-интерфейс `Cloneable` и переопределение protected метода `clone()`.
...
> [!mcq]
> - [ ] Структурные паттерны описывают, как объекты создаются и разрушаются в течение жизненного цикла приложения. | Это описание Creational-паттернов. Structural — про компоновку объектов и классов в большие структуры.
...
> - [ ] Структурные паттерны определяют алгоритмы и распределение обязанностей между объектами (`Strategy`, `Command`, `Observer`). | Это поведенческие паттерны (Behavioral). Путаница категорий — частая ошибка на собеседовании.
> - [ ] Структурные паттерны содержат только `Adapter` и `Facade`, остальные — поведенческие. | Неверное разделение: в GoF структурных паттернов ровно семь, включая `Proxy`, `Decorator`, `Bridge`, `Composite`, `Flyweight`.
...
> [!mcq]
> - [ ] `Adapter` предоставляет упрощённый интерфейс к сложной подсистеме, скрывая её внутреннее устройство. | Это `Facade`, не Adapter. Facade упрощает сложное; Adapter меняет интерфейс с несовместимого на нужный.
> - [ ] `Adapter` разделяет абстракцию и реализацию, чтобы они менялись независимо. | Это `Bridge`. Bridge — проектирование заранее с двумя иерархиями; Adapter — реакция на уже существующий несовместимый код.
> - [x] `Adapter` преобразует интерфейс класса в другой, ожидаемый клиентом, например `Arrays.asList()` адаптирует массив к `List`. | Точное определение и канонический JDK-пример. Применяется постфактум для интеграции с legacy/third-party API.
> - [ ] `Adapter` создаёт новые объекты через копирование прототипа, избегая вызова `new`. | Это `Prototype`. Adapter никогда не клонирует — он делегирует вызовы к обёрнутому объекту, меняя сигнатуру метода.
...
> [!mcq]
> - [ ] `Bridge` — это порождающий паттерн, который создаёт объекты через копирование существующих. | Неверно по всем пунктам: Bridge — структурный, не клонирует. Это описание Prototype.
> - [ ] `Bridge` применяется только для совмещения несовместимых API старых и новых систем. | Это задача Adapter (постфактум). Bridge проектируют заранее, когда известны два независимых измерения изменчивости.
> - [ ] `Bridge` всегда требует `Abstract Factory` для создания конкретных реализаций. | Bridge работает с уже созданными реализациями через композицию. Abstract Factory — опциональная комбинация, а не требование.
...
> [!mcq]
> - [x] `Adapter` решает проблему постфактум — связать уже существующие несовместимые интерфейсы; `Bridge` проектируется заранее для разделения двух измерений изменчивости. | Главное различие — **когда** применяется. Adapter — реакция на чужой код, Bridge — проектное решение с анти-комбинаторным взрывом классов.
> - [ ] `Adapter` и `Bridge` полностью идентичны, разница только в названии у разных авторов. | Хотя структуры похожи (композиция), семантика и намерение различаются. Adapter и Bridge решают разные задачи.
> - [ ] `Bridge` работает только с интерфейсами, а `Adapter` — только с абстрактными классами. | Оба паттерна могут использовать как интерфейсы, так и абстрактные классы. Различие не в механизме, а в намерении.
> - [ ] `Bridge` всегда нарушает OCP, а `Adapter` его соблюдает. | Наоборот: оба обычно соблюдают OCP. Bridge специально спроектирован для добавления измерений без изменений существующего кода.
...
> [!mcq]
> - [ ] `Decorator` жёстко фиксирует порядок оборачивания во время компиляции через наследование. | Наоборот: главное преимущество — динамическая композиция в runtime. Наследование требуется только для общего интерфейса, не для иерархии декораторов.
...
> - [ ] `Decorator` всегда реализуется через `Abstract Factory` для создания оборачивающих объектов. | Фабрика — опциональный способ создать нужную комбинацию декораторов, но не часть паттерна. Обычно декораторы создаются прямым вызовом new.
> - [ ] `Decorator` и `Proxy` — это один и тот же паттерн с разными именами. | Структурно похожи, но различаются намерением: Decorator расширяет поведение, Proxy контролирует доступ. Это типичный trap-вопрос на собеседовании.
...
> [!mcq]
> - [ ] Наследование позволяет менять поведение в runtime, а `Decorator` — только во время компиляции. | Ровно наоборот: иерархия наследования фиксируется при компиляции, тогда как Decorator собирается в runtime через композицию.
> - [ ] При добавлении N независимых фич `Decorator` требует 2^N классов, наследование — только N классов. | Наоборот: наследование даёт экспоненциальный рост (`EncryptedCompressedBuffered...`), Decorator — линейный (по одному декоратору на фичу).
> - [x] Наследование даёт доступ к `protected`-полям родителя, но фиксирует расширение на этапе компиляции и даёт экспоненциальный рост классов при сочетании фич. | Точный trade-off: сильнее связь с родителем, но жёстче архитектура. Decorator теряет доступ к protected, но выигрывает в гибкости.
...
> [!mcq]
> - [ ] `Proxy` всегда добавляет новое поведение к объекту (логирование, форматирование), расширяя его интерфейс. | Это задача `Decorator`. Proxy сохраняет интерфейс оригинала и **контролирует доступ**, а не расширяет поведение.
> - [ ] `Proxy` существует только в виде `JDK Dynamic Proxy` для интерфейсов, реализация через наследование невозможна. | CGLIB как раз создаёт прокси через наследование, именно это Spring использует для классов без интерфейсов (`proxyTargetClass=true`).
> - [ ] `Proxy` в Spring активируется только при включённой аннотации `@EnableCaching`. | Proxy-механизм всегда активен в Spring AOP (через `@EnableAspectJAutoProxy`). `@EnableCaching` включает конкретный aspect, но не сам механизм прокси.
> - [x] `Proxy` предоставляет заместителя того же интерфейса для контроля доступа к объекту — кэширование, lazy loading, авторизация, remote (`@Transactional`, `@Cacheable` в Spring). | Каноничное определение и главные виды: Virtual/Protection/Caching/Remote/Logging. Spring AOP строит всё на прокси через CGLIB или JDK Dynamic Proxy.
...
> [!mcq]
> - [x] `Facade` предоставляет упрощённый унифицированный интерфейс к сложной подсистеме, скрывая детали (`DriverManager.getConnection()` вместо явной работы с `Driver`). | Ключ: упрощает, но не меняет возможности подсистемы. Клиент всё ещё может обращаться к внутренним классам напрямую, facade — это удобная «дверь».
> - [ ] `Facade` создаёт новые объекты через клонирование прототипа для ускорения инициализации. | Это `Prototype`. Facade не про создание объектов, а про упрощение взаимодействия.
> - [ ] `Facade` меняет интерфейс одного класса в другой, ожидаемый клиентом. | Это `Adapter`. Facade не преобразует существующий интерфейс, он создаёт новый поверх нескольких классов.
> - [ ] `Facade` обязательно скрывает подсистему полностью, запрещая прямой доступ к её внутренним классам. | Facade — это **удобная альтернатива**, а не стена. Часто внутренние классы остаются публичными для advanced-сценариев.
...
> [!mcq]
> - [ ] `Composite` позволяет динамически оборачивать объекты для добавления нового поведения без изменения базового класса. | Это `Decorator`. Composite не про оборачивание, а про объединение объектов в древовидные иерархии.
> - [x] `Composite` объединяет объекты в древовидные структуры и позволяет работать с отдельным элементом и группой через один интерфейс (`File` и `Directory` реализуют `FileSystemComponent`). | Точное описание: `part-whole hierarchy`. Рекурсивная структура, где контейнер сам является элементом, позволяет единообразную обработку.
> - [ ] `Composite` — это способ объединить две несовместимые иерархии в одну через общий супер-класс. | Это скорее Bridge/Adapter. Composite — про единый интерфейс для листа и узла одного типа иерархии.
> - [ ] `Composite` используется только в GUI-фреймворках для контейнеров виджетов. | GUI — популярный пример (`java.awt.Container`), но паттерн применим везде: файловые системы, XML-деревья, меню, организационные структуры.
...
> [!mcq]
> - [ ] `Flyweight` повышает производительность за счёт создания нового объекта при каждом запросе вместо кэширования. | Наоборот: Flyweight снижает количество объектов через разделение общего состояния. Создание при каждом запросе — анти-паттерн для Flyweight.
...
> - [x] `Flyweight` экономит память, разделяя общее (`intrinsic`) состояние между множеством объектов, а уникальное (`extrinsic`) принимается параметром; классический пример — `Integer.valueOf()` с кэшем от -128 до 127. | Точная идея. Integer cache показывает выигрыш: миллион вызовов `Integer.valueOf(42)` вернут один и тот же объект вместо миллиона.
> - [ ] `Flyweight` — это поведенческий паттерн, реализующий цепочку обработчиков для запросов. | Flyweight — структурный паттерн, не поведенческий. Описание подходит к `Chain of Responsibility`.
...
|
> [!mcq]
> - [x] Поведенческие паттерны определяют алгоритмы и распределение обязанностей между объектами | Они описывают, как объекты общаются и координируют действия
> - [ ] Поведенческие паттерны описывают композицию классов в более крупные структуры | Это Structural-паттерны (Adapter, Decorator, Proxy)
> - [ ] Поведенческие паттерны отвечают за создание объектов без привязки к конкретным классам | Это Creational-паттерны (Builder, Factory, Abstract Factory)
> - [ ] В GoF всего 8 поведенческих паттернов | Правильно 11 Behavioral, 5 Creational, 7 Structural
...
> [!mcq]
> - [x] `Strategy` определяет семейство алгоритмов и делает их взаимозаменяемыми через composition | Клиент выбирает стратегию в runtime, соблюдая Open/Closed принцип
> - [ ] `Strategy` инкапсулирует состояние так, чтобы поведение менялось автоматически при смене состояния | Это State. Strategy выбирает клиент, State переключается внутри объекта
> - [ ] `Strategy` требует наследования базового класса для каждого алгоритма | Strategy использует composition (has-a), не наследование (is-a)
> - [ ] `Strategy` и `Decorator` — это один паттерн с разными названиями | Strategy меняет алгоритм, Decorator расширяет поведение
...
|
> [!mcq]
> - [x] `State` позволяет объекту менять поведение при изменении состояния; объект сам переключает состояния | В `Strategy` клиент выбирает алгоритм, стратегии независимы друг от друга
> - [ ] `State` и `Strategy` идентичны по структуре и семантике | Структура похожа, но State — это FSM с переходами, Strategy — выбор алгоритма
> - [ ] `Strategy` применяется для изменения поведения машины состояний | Наоборот: State = FSM (Order lifecycle), Strategy = алгоритм (Comparator)
> - [ ] В `State` клиент явно вызывает `setState()` как в `Strategy` `setStrategy()` | В State переходы инициирует сам объект (в методах состояния)
...
> [!mcq]
> - [x] `Observer` определяет зависимость один-ко-многим: издатель уведомляет подписчиков о событиях | Observer разделяет ответственность и соблюдает SRP
> - [ ] `Observer` создаёт копии объекта через клонирование при подписке | Это Prototype. Observer про установку связи между издателем и слушателем
> - [ ] В `Observer` издатель должен знать конкретные классы подписчиков | Нарушение DIP: издатель зависит от интерфейса Observer, а не от реализаций
> - [ ] `java.util.Observable` — полнофункциональная production реализация паттерна | Deprecated с Java 9. Spring ApplicationEventPublisher и RxJava Observable лучше
...
> [!mcq]
> - [x] `Chain of Responsibility` передаёт запрос по цепочке обработчиков; каждый решает: обработать или передать | Примеры: Servlet Filters, Spring Security FilterChain
> - [ ] `Chain of Responsibility` — это просто цикл по коллекции обработчиков | Цикл не паттерн. CoR — про построение цепи с решением о передаче
> - [ ] `Chain of Responsibility` гарантирует, что первый подходящий обработчик обработает (как if-else-if) | Обработчик может обработать И передать дальше (как Servlet Filter)
> - [ ] Все обработчики взаимозаменяемы и имеют одинаковую ответственность | Каждый обработчик специализирован на разных типах запросов
...
> [!mcq]
> - [x] `Command` инкапсулирует запрос как объект, позволяя ставить в очередь, логировать, отменять (undo/redo) | В Java это Runnable, Callable. MacroCommand показывает композицию команд
> - [ ] `Command` просто оборачивает вызов метода; практического использования нет | Критичен для undo/redo, операция в очереди, асинхронного выполнения
> - [ ] `Command` и `Strategy` идентичны: оба оборачивают поведение в объект | Command инкапсулирует действие (execute/undo), Strategy инкапсулирует алгоритм
> - [ ] `CommandHistory` должна использовать `Deque` для гарантии порядка undo операций | Это реализационная деталь. Главное — инкапсуляция команды с `undo()`
...
> [!mcq]
> - [x] `Visitor` позволяет добавлять новые операции к структуре объектов без изменения классов через double dispatch | Это FileVisitor, ElementVisitor. Double dispatch выбирает метод по двум runtime-типам
> - [ ] `Visitor` — способ обхода дерева, альтернатива итератору для иерархических структур | Обход — побочное свойство. Главная цель — операции на структуре
> - [ ] `Visitor` требует, чтобы элементы знали обо всех visitor'ах; нужно менять классы при добавлении visitor'а | Наоборот: классы элементов не меняются. Новый visitor просто реализует интерфейс
> - [ ] `Visitor` используется только в компиляторах и парсерах; слишком сложен для бизнес-объектов | Применим везде: обход файловой системы, поиск, сериализация
...
> [!mcq]
> - [x] `Iterator` предоставляет последовательный доступ к элементам, скрывая внутреннюю структуру | В Java встроен через java.util.Iterator и for-each
> - [ ] `Iterator` — это просто интерфейс с `hasNext()` и `next()`; паттерн в их наличии | Паттерн — это идея инкапсуляции структуры. Iterator как паттерн включает дизайн коллекции
> - [ ] `Iterable` возвращает один и тот же iterator; это гарантирует потокобезопасность | `iterator()` обычно возвращает новый iterator для каждого вызова
> - [ ] `Iterator` устарел; вместо него используйте Stream API; Iterator не должен быть в коде | Stream дополняет, не заменяет. Iterator удобнее для модификации во время итерации
...
> [!mcq]
> - [x] `Mediator` инкапсулирует взаимодействие множества объектов, уменьшая связанность | Объекты общаются через mediator, не напрямую. В Spring это DispatcherServlet
> - [ ] `Mediator` используется для создания иерархических структур, где родители управляют детьми | Это Composite. Mediator про горизонтальное взаимодействие
> - [ ] `Mediator` требует, чтобы объекты передавали друг другу сообщения; это упрощает понимание потока | Прямые ссылки создают Spaghetti Logic. Mediator скрывает взаимодействие в одном месте
> - [ ] `Mediator` и `Facade` идентичны | Facade упрощает интерфейс к существующей подсистеме (one-to-many), Mediator управляет равными объектами (many-to-many)
...
> [!mcq]
> - [x] `Memento` сохраняет состояние объекта без раскрытия деталей реализации; EditorMemento — снимок state | Ключ: инкапсуляция. Caretaker хранит снимки, объект восстанавливает через restore()
> - [ ] `Memento` и `Prototype` одинаковые: оба сохраняют состояние объекта | Prototype клонирует объект (shallow/deep copy), Memento сохраняет снимок state
> - [ ] `Memento` требует сериализации объекта для сохранения состояния | Не обязательно. Memento может быть простым record с полями state
> - [ ] `EditorHistory` может быть `static` для совместного использования историей | Static приведёт к утечкам памяти. Caretaker создаётся per-instance или per-session
...
|
> [!mcq]
> - [x] Алгоритм выбора: (1) определить проблему, (2) выбрать категорию, (3) сузить варианты до 2-3, (4) спросить: нужен ли паттерн | YAGNI: не вводи абстракцию до второго случая
> - [ ] Всегда применяйте most advanced паттерн для профессионального вида кода | Паттерны добавляют сложность. Простое решение сейчас лучше, чем сложный паттерн на будущее
> - [ ] Выбор паттерна делайте в начале проекта; рефакторинг дорог | Некоторые паттерны нужно проектировать заранее, но большинство вводятся incrementally
> - [ ] Если паттерн в Spring/JDK, он всегда применим в вашем коде | Контекст важен. Spring DI часто предпочтительнее Singleton для бизнес-объектов
...
> [!mcq]
> - [x] Abstract Factory реализуется через Factory Method; Decorator и Composite похожи; Strategy и State разная семантика | Паттерны комбинируются: Iterator + Composite, Observer + Mediator
> - [ ] Паттерны независимы; комбинирование приводит к излишней сложности | Комбинация часто решает сложные задачи. Spring использует Proxy + Template Method + Factory
> - [ ] Все паттерны это вариации одного базового паттерна | Неправильная классификация: 3 категории с разной семантикой
> - [ ] Паттерны связаны только если упомянуты в одной главе GoF | Связи обнаруживаются через анализ сходства структур и целей
...
> [!mcq]
> - [x] `Null Object` — альтернатива `null`-проверкам; возвращается объект с пустым поведением | Паттерн устраняет NPE и null-проверки. Collections.emptyList() похожий пример
> - [ ] `Null Object` требует наследования, усложняя иерархию | Современно: через composition и интерфейсы. Иерархия не усложняется
> - [ ] `Null Object` не официальный GoF паттерн, не используйте в production | Не-GoF паттерны часто полезнее. Если решает проблему — используй
> - [ ] `Null Object` должен выбрасывать `NullPointerException` при вызове методов | Нет: Null Object gracefully handles. Если выбрасывает — это mock для тестов
...
|
> [!mcq]
> - [x] Factory + Strategy = фабрика создаёт стратегии; Command + Memento = Undo/Redo | Комбинации решают реальные задачи. Spring построен на таких compositions
> - [ ] Паттерны комбинируйте только если явно требуется; иначе overengineering | Не проектируй заранее, но узнавай при refactoring. Фичи могут естественно комбинироваться
> - [ ] Template Method + Strategy плохая комбинация; обе определяют алгоритм | Наоборот: Template Method фиксирует скелет, Strategy меняет подстепи
> - [ ] Комбинация паттернов может быть выражена одним паттерном; не используй | Комбинация часто понятнее one-off решения. Iterator + Composite проще
...
> [!mcq]
> - [x] `Interpreter` используется для разбора и интерпретации грамматик (DSL, regex) | На практике редко: используют ANTLR или SpEL вместо pure form
> - [ ] `Interpreter` применяется для обхода дерева объектов с выполнением операций | Это Visitor или Composite Iterator. Interpreter про parsing и evaluation выражений
> - [ ] `ScriptEngine` — замена `Interpreter` для поддержки JavaScript и Groovy | ScriptEngine применение Interpreter'а, не замена. Для own DSL нужен Interpreter
> - [ ] `Interpreter` гарантирует расширяемость грамматики без изменения существующих правил | Новое правило = новый класс (OCP), но сама грамматика фиксируется
...
> [!mcq]
> - [x] Хороший ответ включает контекст (проект), проблему, паттерн и альтернативы | На собеседовании покажите реальное применение, не книжное знание
> - [ ] Назовите как можно больше паттернов; количество свидетельствует о квалификации | Контрпродуктивно: интервьюер видит понимание или зубрёжку. 2-3 паттерна с контекстом лучше
> - [ ] Паттерны должны быть спроектированы точно как в GoF; отступления — минус | Отступления ценятся: адаптация под контекст. Упрощение через лямбды — профессионально
> - [ ] Не обсуждайте альтернативы которые не выбрали; они только запутают | Обсуждение альтернатив показывает, что вы думали о trade-offs
...
> [!mcq]
> - [x] `Object Pool` управляет пулом заранее созданных объектов, избегая дорогого создания/уничтожения | Примеры: HikariCP (DB connections), ExecutorService (threads), Netty ByteBuffer pool
> - [ ] `Object Pool` и `Flyweight` это один паттерн; оба переиспользуют объекты | Flyweight разделяет intrinsic state (общее значение), Pool переиспользует mutable объекты
> - [ ] `Object Pool` требует ручного управления; GC автоматизировал это | GC не решает: дорогие ресурсы нельзя оставлять GC. Pool требует явного управления
> - [ ] Главная проблема Object Pool — fairness: потоки голодают в очереди | Valid concern, но управляемый: timeout в acquire(), мониторинг, fair queue
...
> [!mcq]
> - [x] `Specification` инкапсулирует бизнес-правило в объект, позволяя комбинировать через and()/or()/not() | В DDD используется для queries и validation. Spring Data переводит в JPA Predicate
> - [ ] `Specification` только для фильтрации в JPA; для validation существуют отдельные решения | Specification универсален. In-memory filter через stream, в JPA переводится в WHERE
> - [ ] `Specification` требует наследования и переопределения методов для каждого правила | Modern: functional интерфейс, методы and()/or() — default методы. Композиция
> - [ ] Spring Data Specification идентично стандартному Specification паттерну | Spring использует для JPA Criteria queries (SQL WHERE), паттерн работает с объектами памяти
