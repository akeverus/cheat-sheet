...
Проект имеет статус **CNCF graduated** (2024) и стоит 2-м по активности после Kubernetes — это значит, что стандарт стабилен и поддержан индустрией.
...
Поток данных: несколько приложений сходятся в один Collector, а он раздаёт телеметрию в несколько бэкендов:

- `App 1` → `OTel Collector`
- `App 2` → `OTel Collector`
- `App 3` → `OTel Collector`
