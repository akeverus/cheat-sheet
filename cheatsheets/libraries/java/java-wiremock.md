...
- [WireMock](https://wiremock.org/) — официальный сайт
...
- [Integration с Spring](#integration-с-spring)
...
public class `UserServiceTest` {
    ...
    public void `testUserCreation`() {
        `wireMockServer`.`stubFor`(post(`urlEqualTo`("/api/users"))
            .`willReturn`(created()
                .`withHeader`("Location", "/api/users/1")));
...
```java
// Статические файлы
`stubFor`(get(`urlEqualTo`("/api/file.json")))
    .`willReturn`(`aResponse`()
...
`WireMock`.`startRecording`("https://api.example.com");
...
public class `ApiMockDsl` {
    ...
    public void `mockUserApi`() {
        ...
        server.`stubFor`(post("/api/users")
            .`willReturn`(created().`withHeader`("Location", "/api/users/1")));
    }
