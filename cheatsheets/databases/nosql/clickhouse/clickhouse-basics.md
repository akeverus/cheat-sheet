...
- [Введение в **ClickHouse**](#введение-в-clickhouse)
  ...
  - [Почему **ClickHouse** уникален?](#почему-clickhouse-уникален)
    - [1. Колоночное хранение данных](#1-колоночное-хранение-данных)
    - [2. Векторные вычисления](#2-векторные-вычисления)
    - [3. Специализированные движки таблиц](#3-специализированные-движки-таблиц)
    - [4. Продвинутая оптимизация запросов](#4-продвинутая-оптимизация-запросов)
  - [Технические характеристики производительности](#технические-характеристики-производительности)
    - [Бенчмарки и сравнения](#бенчмарки-и-сравнения)
  - [Архитектурные принципы](#архитектурные-принципы)
    - [1. **Shared-Nothing** архитектура](#1-shared-nothing-архитектура)
    - [2. **LSM-Tree** подход](#2-lsm-tree-подход)
    - [3. **Push-down** оптимизации](#3-push-down-оптимизации)
    - [4. **Vectorized query execution**](#4-vectorized-query-execution)
  - [Ограничения и компромиссы](#ограничения-и-компромиссы)
    - [**OLTP** операции](#oltp-операции)
    - [Транзакции](#транзакции)
    - [**JOIN** операции](#join-операции)
  - [Сравнение с другими системами](#сравнение-с-другими-системами)
    - [**ClickHouse** vs **Apache Druid**](#clickhouse-vs-apache-druid)
    - [**ClickHouse** vs **Elasticsearch**](#clickhouse-vs-elasticsearch)
  - [Производственные сценарии использования](#производственные-сценарии-использования)
    - [1. Аналитика веб-трафика](#1-аналитика-веб-трафика)
    - [2. Финансовая аналитика](#2-финансовая-аналитика)
    - [3. **IoT** и телеметрия](#3-iot-и-телеметрия)
    - [4. **Clickstream** аналитика](#4-clickstream-аналитика)
  - [Оптимизация производительности](#оптимизация-производительности)
    - [**Hardware** оптимизации](#hardware-оптимизации)
    - [Конфигурационные оптимизации](#конфигурационные-оптимизации)
    - [**Query** оптимизации](#query-оптимизации)
  - [Безопасность в **ClickHouse**](#безопасность-в-clickhouse)
...
- [Основные характеристики](#основные-характеристики)
  ...
  - [**SQL** совместимость](#sql-совместимость)
  - [Надежность](#надежность)
- [Варианты использования](#варианты-использования)
  - [Аналитика в реальном времени](#аналитика-в-реальном-времени)
...
- [Архитектура и компоненты](#архитектура-и-компоненты)
  - [Компоненты **ClickHouse**](#компоненты-clickhouse)
  - [Ключевые компоненты](#ключевые-компоненты)
    - [**ClickHouse Server**](#clickhouse-server)
    - [**MergeTree Storage Engine**](#mergetree-storage-engine)
    - [**Distributed Engine**](#distributed-engine)
    - [**ZooKeeper**](#zookeeper)
...
- [Основные концепции](#основные-концепции)
  ...
  - [Таблицы и движки](#таблицы-и-движки)
  - [Партиционирование](#партиционирование)
  - [Первичный ключ и индексы](#первичный-ключ-и-индексы)
- [Колоночное хранилище](#колоночное-хранилище)
  - [Принцип работы](#принцип-работы)
  - [Преимущества колоночного хранения](#преимущества-колоночного-хранения)
    - [Быстрые аналитические запросы](#быстрые-аналитические-запросы)
...
Основной файл конфигурации: `config.xml` (обычно `/etc/clickhouse-server/config.xml`)
...
Проектируйте таблицы под типичные запросы: выбирайте подходящий движок (`MergeTree`, `ReplacingMergeTree`, `SummingMergeTree`) и порядок ключей сортировки так, чтобы фильтры и группировки максимально использовали первичный ключ. Избегайте избыточной гранулярности — агрегируйте данные на уровне, достаточном для аналитики, чтобы уменьшить объём и ускорить запросы.

Используйте партиционирование для больших таблиц по дате или логическим сегментам; это ускоряет выборки по диапазонам и упрощает удаление устаревших данных. Регулярно выполняйте `OPTIMIZE TABLE` там, где нужна финальная консолидация (например, для `ReplacingMergeTree`). Мониторьте использование диска, количество частей и метрики запросов, чтобы вовремя выявлять деградацию производительности.

## Решение проблем

**Сервер не запускается:** проверьте логи (`/var/log/clickhouse-server/` или вывод консоли), права на каталоги данных и конфигурацию. Частые причины — нехватка памяти, повреждённые метаданные, конфликт портов. Восстановление из бэкапа при повреждении.

**Медленные запросы:** убедитесь, что в условиях используются ключевые столбцы и партиции. Добавьте индексы, уменьшите объём данных в запросе. Проверьте `EXPLAIN` и системные таблицы (`system.query_log`). Избегайте тяжёлых JOIN без фильтров по ключу.

**Нехватка памяти (Memory limit exceeded):** увеличьте `max_memory_usage`, разбейте запрос на части (по партициям), упростите агрегации. Проверьте настройки для больших группировок и джойнов.
