---
title: "Cassandra: Производительность и оптимизация - Тюнинг и мониторинг высоконагруженных кластеров"
description: "Комплексное руководство по оптимизации производительности Apache Cassandra: настройка JVM, тюнинг запросов, мониторинг метрик и устранение узких мест в высоконагруженных системах."
tags: ["databases", "nosql", "cassandra-performance"]
...
- [Cassandra Operations](https://cassandra.apache.org/doc/latest/operating/) — настройка и эксплуатация
- [Cassandra Configuration](https://cassandra.apache.org/doc/latest/configuration/) — конфигурация и **JVM**
- [Hardware Requirements](https://cassandra.apache.org/doc/latest/operating/hardware.html)

### Дизайн и оптимизация
- [Cassandra Best Practices](https://cassandra.apache.org/doc/latest/cassandra/operating/hardware.html)
- [DataStax Architecture Guide](https://docs.datastax.com/en/dse-planning/doc/planning/planningArchitecture.html)
- [Performance Tuning](https://cassandra.apache.org/doc/latest/operating/metrics.html)

### Инструменты
- [Cassandra Reaper](https://cassandra-reaper.io/) — ремонт и мониторинг
- [cassandra-stress](https://cassandra.apache.org/doc/latest/tools/cassandra_stress.html) — нагрузочное тестирование
...
- [Архитектура производительности](#архитектура-производительности)
  - [Ключевые факторы производительности](#ключевые-факторы-производительности)
    - [**Read Performance Factors**](#read-performance-factors)
    - [**Write Performance Factors**](#write-performance-factors)
  - [**Performance Trade-offs**](#performance-trade-offs)
    - [**CAP Theorem** в **Cassandra**](#cap-theorem-в-cassandra)
- [Оптимизация **JVM**](#оптимизация-jvm)
  - [**JVM Memory Tuning**](#jvm-memory-tuning)
    - [**Heap Size Configuration**](#heap-size-configuration)
- [Оптимальная конфигурация heap](#оптимальная-конфигурация-heap)
- [cassandra-env.sh](#cassandra-envsh)
- [Для серверов с 64GB+ RAM](#для-серверов-с-64gb-ram)
- [Heap = min(32GB, RAM/2)](#heap-min32gb-ram2)
- [Young gen = Heap/4](#young-gen-heap4)
- [Максимальный heap](#максимальный-heap)
- [Минимальный heap (равен максимальному для предотвращения resizing)](#минимальный-heap-равен-максимальному-для-предотвращения-resizing)
- [Young generation](#young-generation)
- [Survivor ratio](#survivor-ratio)
- [Максимальный размер tenured generation](#максимальный-размер-tenured-generation)
    - [**Garbage Collection Tuning**](#garbage-collection-tuning)
- [G1GC для Cassandra 3.0+](#g1gc-для-cassandra-30)
- [CMS GC (legacy, для Cassandra < 3.0)](#cms-gc-legacy-для-cassandra-30)
...
- [Создание thread dump](#создание-thread-dump)
- [Или с помощью jstack](#или-с-помощью-jstack)
- [Анализ thread dump](#анализ-thread-dump)
- [Поиск:](#поиск)
...
- [Метрики производительности, memory usage, hotspots](#метрики-производительности-memory-usage-hotspots)
...
- [Для XFS (рекомендуется для Cassandra)](#для-xfs-рекомендуется-для-cassandra)
- [Mount options](#mount-options)
- [Для EXT4](#для-ext4)
  - [**Resource Limits**](#resource-limits)
    - [**System Limits**](#system-limits)
- [/etc/security/limits.conf](#etcsecuritylimitsconf)
- [Cassandra user limits](#cassandra-user-limits)
- [Root limits (for maintenance)](#root-limits-for-maintenance)
    - [**Cassandra User Setup**](#cassandra-user-setup)
- [Создание пользователя cassandra](#создание-пользователя-cassandra)
- [Настройка директорий](#настройка-директорий)
- [Настройка логов](#настройка-логов)
  - [**CPU Affinity and NUMA**](#cpu-affinity-and-numa)
    - [**CPU Pinning**](#cpu-pinning)
- [CPU affinity для JVM](#cpu-affinity-для-jvm)
- [Taskset для привязки процесса](#taskset-для-привязки-процесса)
- [Для систем с NUMA](#для-систем-с-numa)
...
- [Индексы и кэширование](#индексы-и-кэширование)
  - [**Index Strategy Optimization**](#index-strategy-optimization)
    - [**Choosing Index Types**](#choosing-index-types)
  - [**Caching Strategies**](#caching-strategies)
    - [**Row Cache Configuration**](#row-cache-configuration)
    - [**Key Cache Optimization**](#key-cache-optimization)
- [cassandra.yaml](#cassandrayaml)
- [Counter cache](#counter-cache)
    - [**Custom Caching Strategy**](#custom-caching-strategy)
- [Оптимизация хранения](#оптимизация-хранения)
  - [**Compaction Strategy Selection**](#compaction-strategy-selection)
    - [**Choosing Compaction Strategy**](#choosing-compaction-strategy)
...
- [Масштабирование производительности](#масштабирование-производительности)
  - [**Horizontal Scaling**](#horizontal-scaling)
    - [**Cluster Expansion Planning**](#cluster-expansion-planning)
...
- [Простой write тест](#простой-write-тест)
- [Mixed workload тест](#mixed-workload-тест)
- [Custom тест с YAML конфигурацией](#custom-тест-с-yaml-конфигурацией)
- [Тест с различными консистентностями](#тест-с-различными-консистентностями)
    - [**Custom Stress Profile**](#custom-stress-profile)
- [stress-profile.yaml](#stress-profileyaml)
    - [**Results Analysis**](#results-analysis)
  - [**Performance Profiling Tools**](#performance-profiling-tools)
    - [**Async Profiler**](#async-profiler)
- [Установка async-profiler](#установка-async-profiler)
- [Профилирование CPU](#профилирование-cpu)
- [Профилирование allocation](#профилирование-allocation)
- [Профiliрование locks](#профiliрование-locks)
...
- [Лучшие практики](#лучшие-практики)
  ...
  - [**Configuration Management**](#configuration-management)
    ...
    - [2. **Testing Changes**](#2-testing-changes)
    - [1. **Systematic Approach**](#1-systematic-approach)
    - [2. **Common Tools**](#2-common-tools)
  - [**Performance Culture**](#performance-culture)
    - [1. **Team Practices**](#1-team-practices)
    - [2. **Continuous Improvement**](#2-continuous-improvement)
  - [Архитектурные решения:](#архитектурные-решения)
  - [Конфигурация и тюнинг:](#конфигурация-и-тюнинг)
