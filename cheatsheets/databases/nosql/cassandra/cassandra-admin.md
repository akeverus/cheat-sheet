...
tags: ["databases", "nosql", "cassandra-admin"]
...
- [Проверка статуса всех узлов](#проверка-статуса-всех-узлов)
- [Детальный статус с нагрузкой](#детальный-статус-с-нагрузкой)
- [Статус конкретного датацентра](#статус-конкретного-датацентра)
- [Проверка ring состояния](#проверка-ring-состояния)
- [Информация о версиях](#информация-о-версиях)
    - [Проверка ключевых метрик](#проверка-ключевых-метрик)
- [Проверка нагрузки на узлы](#проверка-нагрузки-на-узлы)
...
- [1. Проверка логов на ошибки](#1-проверка-логов-на-ошибки)
- [2. Проверка дискового пространства](#2-проверка-дискового-пространства)
- [3. Проверка нагрузки на узлы](#3-проверка-нагрузки-на-узлы)
- [4. Проверка pending операций](#4-проверка-pending-операций)
- [5. Ротация логов (если не автоматическая)](#5-ротация-логов-если-не-автоматическая)
    - [Еженедельные задачи](#еженедельные-задачи)
- [1. Запуск repair для всех keyspaces](#1-запуск-repair-для-всех-keyspaces)
- [2. Очистка snapshots старше 7 дней](#2-очистка-snapshots-старше-7-дней)
- [3. Проверка консистентности данных](#3-проверка-консистентности-данных)
- [4. Очистка commit log (если необходимо)](#4-очистка-commit-log-если-необходимо)
    - [Ежемесячные задачи](#ежемесячные-задачи)
- [1. Создание полного backup](#1-создание-полного-backup)
- [Использование Medusa или другого инструмента](#использование-medusa-или-другого-инструмента)
- [2. Проверка hardware health](#2-проверка-hardware-health)
- [Проверка дисков, памяти, CPU](#проверка-дисков-памяти-cpu)
- [3. Обновление JVM (если необходимо)](#3-обновление-jvm-если-необходимо)
- [Тестирование в staging среде](#тестирование-в-staging-среде)
- [4. Аудит конфигурации](#4-аудит-конфигурации)
- [Сравнение с best practices](#сравнение-с-best-practices)
- [5. Performance baseline](#5-performance-baseline)
- [Запуск benchmark тестов](#запуск-benchmark-тестов)
...
- [JMX Exporter configuration](#jmx-exporter-configuration)
...
- [Основные параметры кластера](#основные-параметры-кластера)
- [Параметры узла](#параметры-узла)
- [Каталоги данных](#каталоги-данных)
- [Параметры JVM](#параметры-jvm)
- [Параметры сети](#параметры-сети)
- [Параметры безопасности](#параметры-безопасности)
- [Параметры репликации](#параметры-репликации)
- [Параметры обслуживания](#параметры-обслуживания)
    - [**jvm.options**](#jvmoptions)
- [Heap settings](#heap-settings)
- [GC settings](#gc-settings)
- [Memory settings](#memory-settings)
- [Diagnostic settings](#diagnostic-settings)
- [JMX settings](#jmx-settings)
- [Cassandra specific](#cassandra-specific)
    - [Версионирование конфигурации](#версионирование-конфигурации)
- [Структура директории конфигурации](#структура-директории-конфигурации)
- [Версионирование с Git](#версионирование-с-git)
- [Создание тегов для версий](#создание-тегов-для-версий)
...
- [1. Создание backup текущей конфигурации](#1-создание-backup-текущей-конфигурации)
- [2. Изменение настроек](#2-изменение-настроек)
- [3. Валидация конфигурации](#3-валидация-конфигурации)
- [4. Перезапуск узла (rolling restart)](#4-перезапуск-узла-rolling-restart)
...
- [Остановка compaction (осторожно!)](#остановка-compaction-осторожно)
- [Принудительная major compaction (не рекомендуется)](#принудительная-major-compaction-не-рекомендуется)
...
- [Создание полного backup](#создание-полного-backup)
- [Создание incremental backup](#создание-incremental-backup)
- [Список backups](#список-backups)
- [Проверка статуса backup](#проверка-статуса-backup)
...
- [2. Очистка данных (если полное восстановление)](#2-очистка-данных-если-полное-восстановление)
- [3. Восстановление из backup](#3-восстановление-из-backup)
- [4. Запуск Cassandra](#4-запуск-cassandra)
- [5. Проверка целостности данных](#5-проверка-целостности-данных)
- [6. Repair для восстановления консистентности](#6-repair-для-восстановления-консистентности)
...
- [Создание keystore](#создание-keystore)
- [Создание truststore](#создание-truststore)
...
- [Для Cassandra 4.0+](#для-cassandra-40)
