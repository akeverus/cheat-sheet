---
title: "MongoDB: Основы документо-ориентированной NoSQL базы данных"
description: "Комплексное руководство по основам MongoDB: архитектура, установка, основные концепции, документы и BSON."
tags:
  - databases
  - nosql
  - mongodb-basics
...
- [Введение в MongoDB](#введение-в-mongodb)
  ...
  - [Варианты использования MongoDB](#варианты-использования-mongodb)
    ...
    - [IoT и Big Data](#iot-и-big-data)
    - [E-commerce каталоги](#e-commerce-каталоги)
...
- [Установка и настройка MongoDB](#установка-и-настройка-mongodb)
  - [Системные требования](#системные-требования)
    - [Hardware требования](#hardware-требования)
    - [Software требования](#software-требования)
  ...
  - [Встроенные документы vs Ссылки](#встроенные-документы-vs-ссылки)
    - [Embedded Documents (денормализация)](#embedded-documents-денормализация)
    - [References (нормализация)](#references-нормализация)
- [Работа с MongoDB Shell](#работа-с-mongodb-shell)
  - [Подключение к MongoDB](#подключение-к-mongodb)
  - [Основные команды MongoDB Shell](#основные-команды-mongodb-shell)
    - [Навигация по базам данных](#навигация-по-базам-данных)
    - [Работа с коллекциями](#работа-с-коллекциями)
  - [CRUD операции в MongoDB Shell](#crud-операции-в-mongodb-shell)
    - [Create (Вставка)](#create-вставка)
    - [Read (Чтение)](#read-чтение)
    - [Update (Обновление)](#update-обновление)
    - [Delete (Удаление)](#delete-удаление)
  - [Индексы в MongoDB Shell](#индексы-в-mongodb-shell)
  - [Агрегация в MongoDB Shell](#агрегация-в-mongodb-shell)
- [Работа с базами данных](#работа-с-базами-данных)
  - [Создание и управление базами данных](#создание-и-управление-базами-данных)
  - [Системные базы данных](#системные-базы-данных)
...
- [Коллекции и документы](#коллекции-и-документы)
  - [Типы коллекций](#типы-коллекций)
    ...
    - [Time Series Collections (MongoDB 5.0+)](#time-series-collections-mongodb-50)
  - [Управление коллекциями](#управление-коллекциями)
  - [Документы](#документы)
    - [Валидация документов](#валидация-документов)
- [Структура данных](#структура-данных)
  - [Data Modeling в MongoDB](#data-modeling-в-mongodb)
    - [Embedded Documents (One-to-One, One-to-Many)](#embedded-documents-one-to-one-one-to-many)
    - [References (Many-to-Many)](#references-many-to-many)
    - [Массивы в документах](#массивы-в-документах)
  - [Нормализация vs Денормализация](#нормализация-vs-денормализация)
    - [Нормализация (как в реляционных БД)](#нормализация-как-в-реляционных-бд)
    - [Денормализация (оптимизация для чтения)](#денормализация-оптимизация-для-чтения)
  - [Atomic Operations](#atomic-operations)
- [MongoDB Compass](#mongodb-compass)
  - [Установка и настройка](#установка-и-настройка)
  - [Подключение к MongoDB](#подключение-к-mongodb-1)
  - [Основные функции Compass](#основные-функции-compass)
...
- [Лучшие практики для MongoDB](#лучшие-практики-для-mongodb)
  - [1. Дизайн схемы](#1-дизайн-схемы)
    - [Правильное использование embedded documents](#правильное-использование-embedded-documents)
    - [Оптимальный размер документов](#оптимальный-размер-документов)
  - [2. Индексирование](#2-индексирование)
    - [Создавайте индексы на часто используемые поля](#создавайте-индексы-на-часто-используемые-поля)
    - [Избегайте избыточного индексирования](#избегайте-избыточного-индексирования)
  - [3. Connection Management](#3-connection-management)
    - [Используйте connection pooling](#используйте-connection-pooling)
  ...
  - [6. Безопасность](#6-безопасность)
    - [Включите аутентификацию](#включите-аутентификацию)
    - [Шифрование данных](#шифрование-данных)
  - [7. Backup и Recovery](#7-backup-и-recovery)
    - [Регулярные бэкапы](#регулярные-бэкапы)
    - [Тестирование восстановления](#тестирование-восстановления)
  - [Ключевые концепции освоены:](#ключевые-концепции-освоены)
  - [Основные преимущества:](#основные-преимущества)
  - [Когда использовать MongoDB:](#когда-использовать-mongodb)
  - [Когда НЕ использовать MongoDB:](#когда-не-использовать-mongodb)
- [Введение в MongoDB](#введение-в-mongodb-1)
  - [Почему MongoDB?](#почему-mongodb-1)
  - [Архитектура MongoDB](#архитектура-mongodb-1)
    - [Ключевые компоненты:](#ключевые-компоненты)
  - [Сравнение с реляционными базами данных](#сравнение-с-реляционными-базами-данных-1)
- [Решение проблем](#решение-проблем)
  - [Преимущества MongoDB](#преимущества-mongodb-1)
    - [Гибкость схемы](#гибкость-схемы)
...
```yaml
# Логи, storage и сеть для разработки (без auth)
systemLog:
  destination: file
  path: /var/log/mongodb/mongod.log
...
```yaml
# Production: логи, WiredTiger, репликация, аудит, auth
systemLog:
  destination: file
  path: /var/log/mongodb/mongod.log
  logRotate: reopen
...
```yaml
# Высокая производительность: кэш, сжатие, лимиты соединений
storage:
  wiredTiger:
    engineConfig:
      cacheSizeGB: 16
...
```javascript
// Обработка ошибок вставки: дубликат ключа, валидация
try {
    // `MongoDB` операция
...
**Ошибки подключения (connection refused / timeout):** проверьте хост, порт, `bind_ip` в конфигурации и файрвол. При использовании replica set укажите все узлы в connection string. Проверьте логи `mongod` на ошибки запуска.

**Медленные запросы:** включите профайлер, анализируйте медленные операции, добавляйте индексы под частые предикаты и сортировки. Избегайте полного скана коллекций (`COLLSCAN`). Используйте `explain()` для анализа планов.

**Дубликаты или конфликты по _id:** при вставке без `_id` MongoDB генерирует ObjectId; при вставке с существующим `_id` возникает ошибка. Используйте `insertOne`/`insertMany` с обработкой дубликатов или `updateOne` с `upsert`.

**Нехватка диска или памяти:** настройте размер кэша WiredTiger (`wiredTigerEngineConfig.cacheSizeGB`), мониторьте рост данных и индексов. Очистите старые данные, включите сжатие, при необходимости масштабируйте шардинг.
