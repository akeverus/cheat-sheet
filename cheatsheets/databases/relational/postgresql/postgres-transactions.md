...
- [**PostgreSQL**: транзакции, **MVCC** и блокировки](#postgresql-транзакции-mvcc-и-блокировки)
...
- [Советы по блокировкам](#советы-по-блокировкам)
...
- [Блокировки данных — основы](#блокировки-данных-основы)
  ...
  - [Поиск активных блокировок](#поиск-активных-блокировок)
  - [Поиск блокировок и очередей ожидания](#поиск-блокировок-и-очередей-ожидания)
  - [Поиск дедлоков](#поиск-дедлоков)
- [Устранение блокировок и дедлоков](#устранение-блокировок-и-дедлоков)
  - [Мягкое снятие блокировки](#мягкое-снятие-блокировки)
  - [Жесткое снятие блокировки](#жесткое-снятие-блокировки)
  - [Предотвращение блокировок](#предотвращение-блокировок)
- [Аномалии изоляции](#аномалии-изоляции)
  - [**Dirty Read** (**грязное чтение**)](#dirty-read-грязное-чтение)
  - [**Non-Repeatable Read** (**неповторяющееся чтение**)](#non-repeatable-read-неповторяющееся-чтение)
  - [**Phantom Read** (**фантомное чтение**)](#phantom-read-фантомное-чтение)
  - [**Serialization Anomaly** (**аномалия сериализации**)](#serialization-anomaly-аномалия-сериализации)
...
- [Продвинутые техники работы с транзакциями](#продвинутые-техники-работы-с-транзакциями)
  - [Двухфазная фиксация (**Two-Phase Commit**)](#двухфазная-фиксация-two-phase-commit)
  - [Вложенные транзакции и автономные транзакции](#вложенные-транзакции-и-автономные-транзакции)
  - [Транзакции и курсоры](#транзакции-и-курсоры)
  - [Транзакции и временные таблицы](#транзакции-и-временные-таблицы)
  - [**Read-Only** транзакции](#read-only-транзакции)
  - [Внутренняя структура версий строк](#внутренняя-структура-версий-строк)
  - [**Visibility** и **Snapshot**](#visibility-и-snapshot)
  - [Влияние **MVCC** на производительность](#влияние-mvcc-на-производительность)
- [Продвинутые блокировки](#продвинутые-блокировки)
  - [**Advisory Locks**](#advisory-locks)
  - [**Conditional** и **Skip Locks**](#conditional-и-skip-locks)
  - [**Row-level security** и блокировки](#row-level-security-и-блокировки)
  - [Расширенная диагностика блокировок](#расширенная-диагностика-блокировок)
  - [Профилирование транзакций](#профилирование-транзакций)
  - [Инструменты для мониторинга](#инструменты-для-мониторинга)
  - [Настройка параметров **PostgreSQL**](#настройка-параметров-postgresql)
  - [Индексы и блокировки](#индексы-и-блокировки)
  - [**Partitioning** для больших таблиц](#partitioning-для-больших-таблиц)
  - [**Connection Pooling**](#connection-pooling)
- [pgbouncer.ini](#pgbouncerini)
- [Распределенные транзакции](#распределенные-транзакции)
  - [Использование с **Foreign Data Wrappers**](#использование-с-foreign-data-wrappers)
  - [**Logical Replication** и транзакции](#logical-replication-и-транзакции)
- [Безопасность транзакций](#безопасность-транзакций)
  - [Безопасность на уровне строк (**RLS**)](#безопасность-на-уровне-строк-rls)
  - [Аудит транзакций](#аудит-транзакций)
- [Интеграция с приложениями](#интеграция-с-приложениями)
  - [**Spring Framework** интеграция](#spring-framework-интеграция)
  - [**JDBC** транзакции](#jdbc-транзакции)
  - [**ORM** интеграция (**Hibernate**)](#orm-интеграция-hibernate)
- [Мониторинг и алертинг](#мониторинг-и-алертинг)
  - [Настройка алертов](#настройка-алертов)
- [prometheus.yml](#prometheusyml)
- [SQL запросы для экспорта метрик](#sql-запросы-для-экспорта-метрик)
- [pg_stat_activity для активных транзакций](#pg_stat_activity-для-активных-транзакций)
- [pg_locks для количества блокировок](#pg_locks-для-количества-блокировок)
- [pg_stat_database для статистики баз данных](#pg_stat_database-для-статистики-баз-данных)
...
- [Ежедневный vacuum и analyze](#ежедневный-vacuum-и-analyze)
- [Еженедельный reindex](#еженедельный-reindex)
- [Мониторинг размера баз данных](#мониторинг-размера-баз-данных)
- [Ротация WAL файлов](#ротация-wal-файлов)
  - [Ключевые **takeaways**:](#ключевые-takeaways)
...
```sql
-- Начало транзакции, фиксация или откат
BEGIN;
-- изменения
...
- Строковые блокировки: при **SELECT FOR UPDATE** и т.п.
...
2. **ROW SHARE** — используется при **SELECT FOR SHARE**. Совместима с **ACCESS SHARE**, **ROW SHARE**, **ROW EXCLUSIVE**. Блокирует **EXCLUSIVE** и **ACCESS EXCLUSIVE**.
...
Строковые блокировки управляют доступом к отдельным строкам таблицы. Они используются при **SELECT FOR UPDATE** и автоматически при **UPDATE** / **DELETE**.
...
1. **Используйте индексы:**
   - Индексы на столбцах в **WHERE** для **UPDATE** / **DELETE** уменьшают время блокировки.
