...
- [Streaming Replication (физическая репликация)](#streaming-replication-физическая-репликация)
  ...
  - [Настройка Primary сервера](#настройка-primary-сервера)
    - [1. Настройка postgresql.conf](#1-настройка-postgresqlconf)
    - [2. Настройка pg_hba.conf](#2-настройка-pg_hbaconf)
    - [3. Создание пользователя для репликации](#3-создание-пользователя-для-репликации)
  - [Создание базовой реплики](#создание-базовой-реплики)
    - [1. Остановка PostgreSQL на Standby сервере](#1-остановка-postgresql-на-standby-сервере)
    - [2. Резервное копирование с Primary сервера](#2-резервное-копирование-с-primary-сервера)
    - [3. Настройка recovery.conf (PostgreSQL 12+)](#3-настройка-recoveryconf-postgresql-12)
    - [4. Альтернативный способ (PostgreSQL < 12)](#4-альтернативный-способ-postgresql-12)
  ...
  - [Мониторинг репликации](#мониторинг-репликации)
    - [На Primary сервере](#на-primary-сервере)
    - [На Standby сервере](#на-standby-сервере)
...
- [Logical Replication](#logical-replication)
  ...
  - [Настройка Logical Replication](#настройка-logical-replication)
    - [1. Настройка Primary сервера](#1-настройка-primary-сервера)
    - [2. Создание публикации](#2-создание-публикации)
    - [3. Создание подписки на Standby](#3-создание-подписки-на-standby)
...
- [Настройка Master-Slave](#настройка-master-slave)
  ...
  - [Настройка нескольких Standby серверов](#настройка-нескольких-standby-серверов)
    - [1. На Primary сервере](#1-на-primary-сервере)
    - [2. Создание слотов для каждой реплики](#2-создание-слотов-для-каждой-реплики)
    - [3. Настройка каждой Standby реплики](#3-настройка-каждой-standby-реплики)
...
- [Настройка Master-Master (Bidirectional Replication)](#настройка-master-master-bidirectional-replication)
  ...
  - [Настройка двунаправленной репликации](#настройка-двунаправленной-репликации)
    - [1. Настройка Server A](#1-настройка-server-a)
    - [2. Настройка Server B](#2-настройка-server-b)
  - [Обработка конфликтов](#обработка-конфликтов)
- [Мониторинг репликации](#мониторинг-репликации-1)
  - [Ключевые метрики](#ключевые-метрики)
    - [1. Lag (задержка репликации)](#1-lag-задержка-репликации)
    - [2. Статус репликации](#2-статус-репликации)
    - [3. Replication Slots](#3-replication-slots)
  - [Алерты и мониторинг](#алерты-и-мониторинг)
    - [Скрипт проверки репликации](#скрипт-проверки-репликации)
- [Failover и автоматическое переключение](#failover-и-автоматическое-переключение)
  - [Ручной failover](#ручной-failover)
    - [1. Промоутинг Standby в Primary](#1-промоутинг-standby-в-primary)
    - [2. Обновление подключений](#2-обновление-подключений)
  ...
  - [Автоматический failover с pg_auto_failover](#автоматический-failover-с-pg_auto_failover)
    - [Установка](#установка)
...
- [Оптимизация репликации](#оптимизация-репликации)
  - [Настройка производительности](#настройка-производительности)
    - [1. Оптимизация WAL](#1-оптимизация-wal)
    - [2. Оптимизация сети](#2-оптимизация-сети)
    - [3. Параллельная репликация](#3-параллельная-репликация)
...
- [Advanced Failover Strategies](#advanced-failover-strategies)
  ...
  - [Failover with pg_rewind](#failover-with-pg_rewind)
