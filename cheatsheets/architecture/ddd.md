...
description: "Domain-Driven Design (DDD) — это подход к разработке программного обеспечения, который фокусируется на моделировании бизнес-домена и использовании единого языка (Ubiquitous Language) для общения между разработчиками и экспертами предметной области. DDD помогает создавать сложные "
...
- [**Ubiquitous Language**](#ubiquitous-language)
  - [Примеры **Ubiquitous Language**](#примеры-ubiquitous-language)
  - [Использование **Ubiquitous Language** в коде](#использование-ubiquitous-language-в-коде)
- [**Bounded Contexts**](#bounded-contexts)
  - [Примеры **Bounded Contexts**](#примеры-bounded-contexts)
  - [Контекстные карты (**Context Mapping**)](#контекстные-карты-context-mapping)
- [**Entities** и **Value Objects**](#entities-и-value-objects)
  - [**Entities** (**Сущности**)](#entities-сущности)
  - [**Value Objects** (**Объекты-значения**)](#value-objects-объекты-значения)
- [**Aggregates**](#aggregates)
  - [Правила **Aggregates**](#правила-aggregates)
...
- [Лучшие практики](#лучшие-практики)
  - [1. Избегайте **Anemic Domain Model**](#1-избегайте-anemic-domain-model)
  - [2. Используйте **Value Objects** для примитивов](#2-используйте-value-objects-для-примитивов)
  - [3. Защищайте инварианты](#3-защищайте-инварианты)
  - [4. Используйте **Domain Events** для слабой связанности](#4-используйте-domain-events-для-слабой-связанности)
- [Решение проблем](#решение-проблем)
  - [Проблема: Слишком большие **Aggregates**](#проблема-слишком-большие-aggregates)
  - [Проблема: **Anemic Domain Model**](#проблема-anemic-domain-model)
  - [Проблема: Нарушение границ **Bounded Context**](#проблема-нарушение-границ-bounded-context)
...
Подход **DDD** даёт следующие преимущества при разработке сложных систем:
...
**DDD** целесообразен в следующих ситуациях:
...
Ниже — пример использования **Ubiquitous Language** в коде домена (**Java**).
```java
...
**Когда имеет смысл применять DDD?** При сложной доменной логике, долгоживущих продуктах и необходимости единого языка с бизнесом. Для простых CRUD и утилит DDD часто избыточен.

**Чем Aggregate отличается от Entity?** Entity имеет идентичность и жизненный цикл; Aggregate — кластер сущностей и value objects с одной корневой Entity, через которую меняется состояние. Граница консистентности — весь агрегат.

**Как проводить границы Bounded Context?** По командам/подкомандам, организационным границам и зонам изменения. Контексты общаются через явные контракты (API, события), а не общую модель.
