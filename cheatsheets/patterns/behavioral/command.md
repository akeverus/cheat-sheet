...
description: "Command инкапсулирует запрос как объект, позволяя параметризовать клиенты разными запросами, ставить запросы в очередь, логировать их, а также поддерживать отмену операций."
tags: ["patterns", "behavioral", "command"]
...
- [Java Runnable](https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/lang/Runnable.html)
...
**Суть в одном предложении:** Запрос — это объект с методом execute(); Invoker вызывает команду, не зная получателя; удобны очередь, undo и логирование.

**Запомнить:**
- Command инкапсулирует действие и получателя (Receiver).
- Invoker хранит команду и вызывает execute(); можно добавить undo().
- В Java: Runnable, Callable — примитивные команды без undo.
...
Сравнение: жёстко закодированные кнопки пульта vs параметризация через Command (undo).
...
**Когда применять Command?** Когда нужна параметризация операций (кнопки GUI, меню), поддержка undo/redo, очереди команд или логирование. Command инкапсулирует запрос как объект, что позволяет ставить операции в очередь и отменять их.

**Command vs Strategy?** Command инкапсулирует *одну* операцию с получателем; Strategy — семейство взаимозаменяемых алгоритмов. Command подходит для undo/redo и очередей; Strategy — для выбора алгоритма в runtime.

**Как сочетать Command с Memento?** Memento сохраняет состояние перед выполнением команды; при undo команда восстанавливает объект из memento. Комбинируют при необходимости полного отката complex-объектов.
