...
- [Реализация на Java](#реализация-на-java)
  - [Базовая реализация (не потокобезопасная)](#базовая-реализация-не-потокобезопасная)
  - [Потокобезопасная реализация (synchronized)](#потокобезопасная-реализация-synchronized)
...
- [Лучшие практики](#лучшие-практики)
  ...
  - [3. Тестирование Singleton](#3-тестирование-singleton)
...
**Суть в одном предложении:** Один экземпляр класса на всё приложение и одна точка доступа к нему (часто `getInstance()`).

**Запомнить:**
- Приватный конструктор; статический метод или поле для доступа.
- Потокобезопасность: double-checked locking, holder или enum.
- Часто антипаттернь: усложняет тесты и скрывает зависимости.
...
Сравнение: множественные экземпляры соединения vs один экземпляр(Singleton).
...
Клиент получает экземпляр только через статический метод; конструктор закрыт.
...
### Базовая реализация (не потокобезопасная)
...
### Потокобезопасная реализация (synchronized)
...
|--------|-------------------|------------|
| Множественные экземпляры в многопоточности | Отсутствие синхронизации | Использовать holder, enum или volatile + DCL |
| Сложность тестирования | Глобальное состояние | Внедрять зависимость через DI; в тестах подменять mock-ом |
| Сериализация создаёт новый экземпляр | readResolve не реализован | Добавить readResolve() { return getInstance(); } |

## Частые вопросы

**Holder vs Enum vs DCL?** Holder — ленивая инициализация без параметров, потокобезопасно. Enum — простейший, но не ленивый. DCL — когда нужна инициализация с параметрами.

**Singleton — антипаттерн?** В больших системах глобальное состояние усложняет тесты и скрывает зависимости. Предпочитайте DI, где возможно; Singleton — для драйверов, логгеров, пулов.
