...
description: "Proxy предоставляет суррогат или заместителя другого объекта для контроля доступа к нему. Паттерн позволяет добавить дополнительную функциональность при доступе к объекту."
tags:
  - patterns
  - structural
  - proxy
...
- [Реализация на Java](#реализация-на-java)
  - [Виртуальный Proxy (Lazy Loading)](#виртуальный-proxy-lazy-loading)
  - [Защитный Proxy (Protection Proxy)](#защитный-proxy-protection-proxy)
  - [Кэширующий Proxy (Caching Proxy)](#кэширующий-proxy-caching-proxy)
  - [Динамический Proxy с Java Reflection](#динамический-proxy-с-java-reflection)
- [Примеры использования](#примеры-использования)
  - [1. Remote Proxy (RMI)](#1-remote-proxy-rmi)
  - [2. Smart Reference Proxy](#2-smart-reference-proxy)
- [Лучшие практики](#лучшие-практики)
  - [1. Выбор типа прокси](#1-выбор-типа-прокси)
  - [2. Тестирование Proxy паттерна](#2-тестирование-proxy-паттерна)
...
**Суть в одном предложении:** Заместитель с тем же интерфейсом, что и реальный объект; контролирует доступ (ленивая загрузка, кэш, проверки прав).

**Запомнить:**
- Клиент работает с интерфейсом; за ним может быть RealSubject или Proxy.
- Виртуальный proxy — отложенная инициализация; защитный — проверка прав; кэширующий — возврат из кэша.
- В Java: java.lang.reflect.Proxy для динамического прокси по интерфейсу.

**Когда применять:** тяжёлые объекты (ленивая загрузка), контроль доступа, логирование, кэширование, удалённые объекты (RMI).
...
Сравнение: прямая работа с тяжёлым объектом(RealImage) vs ленивая загрузка через **Proxy**.
...
|--------|-------------------|------------|
| Прокси не создаёт реальный объект при необходимости | Не реализована ленивая инициализация | Проверить, что создание отложено до первого вызова; использовать volatile |
| Клиент получает разные результаты при повторных вызовах | Кэш не реализован или инвалидируется | Реализовать кэширование в proxy; при изменениях — инвалидировать кэш |
| Сложность тестирования | Прямая зависимость от Proxy | Внедрять Subject через интерфейс; подменять proxy в тестах |

## Частые вопросы

**Proxy vs Decorator?** Proxy управляет доступом (ленивая загрузка, кэш, защита); Decorator расширяет поведение. Один прокси обычно на один объект; декораторов может быть несколько слоёв.

**Когда использовать динамический Proxy?** Когда нужен proxy для любого объекта, реализующего интерфейс, без написания явного класса. Java: `Proxy.newProxyInstance()`.


## Заключение

**Proxy** паттерн — инструмент для контроля доступа к объектам и добавления дополнительной функциональности. Он позволяет решать различные задачи, от ленивой инициализации до защиты и кэширования.
