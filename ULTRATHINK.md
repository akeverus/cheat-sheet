...
### Content-readability аудит (качество прозы MCQ, 2026-06-02)

Помимо структуры прогнан семантический аудит ЧЕЛОВЕКОЧИТАЕМОСТИ содержимого json (9503 вопроса / 38020 опций). Эвристики: terse explanation (<80 симв), wall-of-text (>900 симв), english-heavy prose (latin>cyr после strip кода), мета-паддинг (формульные фразы-наполнители).

- **terse (<80): 3 опции** — 2 приемлемы (краткое объяснение + богатые example/when_to_apply/edge_cases несут содержание); 1 (go-memory-gc Q20 «Да, влияет.») дополнена.
- **wall-of-text: 0.**
- **english-heavy: 284 опции / 84 файла — НЕ дефект.** Это принятый билингвальный технический регистр корпуса (русская связка + стандартные англо-термины: `high-risk`, `use case`, `human-in-the-loop`, `CV screening`). Так реально говорят рус-инженеры; форсить кириллицу = churn принятого пользователем контента + потеря узнаваемости терминов, которые кандидат встретит по-английски. **Не трогать.**
- **«именно это и …» в `if_it_were_true`: ~128 опций — НЕ дефект.** Это корректный дидактический приём wrong-опции («Будь утверждение верным, X не происходил бы — но именно это и происходит»), ровно как задумано схемой.
- **🟢 Единственный реальный дефект — `go-memory-gc-interview`:** 26/27 correct-объяснений несли роботизированную обёртку «Ключевая идея: … Именно это поведение и проверяют на собеседовании.» (артефакт ранней волны). Снято детерминированно; Q5/Q11/Q20 (которые паддинг искусственно удлинял) дополнены реальным содержанием; gate-PASS сохранён (commit 8ddc2edc).

**Вывод:** контент-читаемость корпуса в норме; structural gate (5/4 секции + info-ratio ≤4 + 0 empty) даёт надёжный пол качества, а билингвальный регистр — осознанный стиль, не дефект.
