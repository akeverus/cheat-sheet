...
## Порция 12 — C7: унификация overflow-guard в .stats-grid (2026-06-16)

`.stats-grid` имел три расходящихся рецепта треков: settings auto-fit с guard
`minmax(min(120px, 100%), 1fr)` (L1259), stats auto-fit БЕЗ guard
`minmax(120px, 1fr)` (L1396), и планшетные полосы `repeat(3, 1fr)` без guard
(L1415/L1418). Соседний комментарий файла (L1402-1404) сам объясняет, зачем нужен
`minmax(0,…)`/`min(…)`: при крупном масштабе шрифта (A+) обычный `1fr`-трек не
ужимается ниже min-content плиток и распирает сетку → горизонтальный скролл.
Guard был применён к settings и мобильной полосе, но пропущен на stats-странице
и в планшетных полосах — код игнорировал собственное задокументированное правило.

Фикс (применяем guard единообразно): stats auto-fit →
`minmax(min(120px, 100%), 1fr)`; обе планшетные полосы → `repeat(3, minmax(0, 1fr))`.
При нормальном масштабе поведение идентично (120px < 100% контейнера, `minmax(0,1fr)`
≡ `1fr`), guard срабатывает только в краевом случае A+/узкий контейнер.

Верифицировано live (chrome-devtools, 1440px, v76→v77): /stats — 6 плиток в ряд
(auto-fit, пустые треки схлопнуты в 0px), без переполнения; /settings (рецепт не
менялся — эталон) — 2 колонки в узкой sidebar-колонке, без переполнения; страница
без горизонтального скролла на обеих. Консоль чиста.
