...
- **MCQ option rows:** flat **ring-hairline cards** on `--color-bg-secondary` (1px `--color-border-primary` ring, 8px radius, no shadow) — the *same* one-altitude ring elevation as §6, never a shadow-floating tile and never a radius+colored-left-stripe slop tile. On the wide focus board they lay out as a deliberate **2×2 exam-form grid** (fills the desktop column instead of a half-empty single column; the A B / C D letter counter runs by DOM order, keys 1–9 unaffected), collapsing to a single column on narrow widths and after answering — when each option expands an explanation and a vertical scan compares better. Hover lifts the row to `--color-bg-tertiary`; pre-answer mouse-selection gets a faint `--color-accent-wash` tint + a terracotta letter marker; the chosen-correct state uses `--color-status-success-wash` tint + a success-color letter marker + success label — tint+marker, never a colored left bar; chosen-wrong uses `--color-status-error-wash` tint + error marker. Keyboard selection draws the terracotta focus ring. *(The earlier "single-column list-divider" phrasing is superseded by the 2×2 exam-form decision; the ring-hairline / one-altitude / no-shadow signature is preserved.)*
