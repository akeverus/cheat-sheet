...
CARICATURE_GROUPS = {
    # NB (2026-07-07): группа "absolute" (всегда/никогда/полностью/гарантированно) УДАЛЕНА.
    # По каноничному своду из 20 правил (mcq-quality-fixer) категоричные слова допустимы внутри
    # правдоподобного заблуждения (напр. «Maybe всегда испускает один элемент» — законная подмена
    # кардинальности; см. rxjava-interview.json как эталон). Запрет §7 касается АСИММЕТРИИ
    # (категоричность только у дистракторов при осторожном correct), а не самого слова, и детектится
    # ненадёжно — поэтому не флагуем словом. Реальные tell'ы — токсично-абсурдные группы ниже.
    "toxic_management": (2, [       # «злодейский менеджер»
        # NB (2026-07-07): маркеры «заставить»/«заставля» УДАЛЕНЫ — 100% ложных срабатываний
        # (29/29 в дистракторах: «аннотация заставляет Spring», «config заставит consumer»,
        # «заставить подклассы реализовать» — нейтральное техническое «forces»). Реальный
        # toxic-tell — угроза/публичное унижение/наказание, а не «заставить». Как с группой
        # `absolute`: убираем over-broad маркер, оставляем настоящие.
        r"публично и жёстк", r"публично и жестк", r"\bунльтиматум", r"\bультиматум",
        r"\bотчитать\b", r"прилюдно", r"пристыдить", r"\bнаказать\b",
        r"угрожа", r"\bпригрозить", r"насильно",
    ]),
