# FRONTEND_HISTORY — архив истории frontend-раундов

> Append-only архив. Здесь живёт полная история: surface-матрица, файловый аудит,
> исторические backlog-гипотезы и весь progress-лог R0.x порт-раунда (mockups → production).
> Это НЕ источник статуса — актуальный статус в `FRONTEND_STATE.json`.
> Новые записи добавлять СВЕРХУ (newest-first) в раздел «Хронология finalization-round».

---

## Хронология finalization-round (newest-first)

- **2026-07-15 · R0.165 (PROC-11):** PLAN_FRONTEND.md ужат до правил/фаз/гейтов; полный
  порт-раунд PLAN (снимок ниже) перенесён сюда. Источники правды разделены на
  FRONTEND_STATE/BACKLOG/DECISIONS/EVIDENCE (bootstrap `dd0e1ad7`).

---

## АРХИВ: PLAN_FRONTEND.md порт-раунда (снимок 2026-07-15, до слима)

_Ниже — полный текст PLAN_FRONTEND.md периода «mockups → production port → full refactor»
(раунд от 2026-07-11), включая §7 surface-матрицу, §9 файловый аудит, §10 backlog-гипотезы
и §21 progress-лог R0.1…R0.164. Сохранено дословно для истории._
...
### 2b. Операционный контракт цикла (решения пользователя)

Фиксируются ответы на вопросы о том, как должен работать loop. Действуют во всех последующих тиках, приоритет — как «осознанные решения пользователя» (п. 3 приоритета выше).

**Уже принято (2026-07-14):**
- **Fix-policy = «Чинить и коммитить».** При находке фронт-дефекта — не только запись в леджер, а правка кода (app.js/base.css/templates) + коммит фикса. По-прежнему: explicit pathspec, MCQ WIP не трогать, код-фикс — отдельным item'ом. Обязательное условие перед правкой: дефект **live-подтверждён** (не чинить вслепую; напр. OVL-1 — код путей Esc/Cancel идентичен → сначала live-reverify, потом решение).
- **End-state = «Глубокий проход».** После закрытия всех 11 surfaces в Final ✅ VERIFIED цикл НЕ останавливается — переходит в adversarial deep-pass: ре-ревью закрытых, edge-cases (живая 5xx, экстремально длинный контент, offline/permission), Lighthouse, кросс-браузер.

**Батч из 10 вопросов (ответы вносятся сюда по мере получения):**

| # | Вопрос | Ответ |
|---|--------|-------|
| 1 | Каденция тика | **Оставить 10 минут** (cron 3e697399 */10 без изменений) |
| 2 | Объём одного тика | **Несколько мелких единиц, если укладываются** во время тика (не строго 1). Каждая всё равно с чистым леджер-следом |
| 3 | Порядок закрытия остатка | **SHELL → OVERLAYS → FEEDBACK → COMPONENTS** (по плану; R0.157=SHELL) |
| 4 | Surface без дефектов при 1-м проходе | **Всегда 2+ независимых adversarial-раунда** до Final ✅ (максимальная строгость) |
| 5 | Когда чинить найденный дефект | **Отдельной волной фиксов** — уточняет fix-policy: surface-проход = аудит + live-verify + запись дефекта в леджер/§10; код-фиксы копятся и катятся отдельной серией тиков ПОСЛЕ аудита. Значит OVL-1 не чинится в тике OVERLAYS — только (пере)подтверждается live и записывается |
| 6 | Регресс-тесты после code-fix | **Static + live-проверка, без gradle** — чтение кода + chrome-devtools live; НЕ трогать gradle при живом bootRun (риск wedge) |
| 7 | Порог Lighthouse (deep-pass) | **a11y=100 и BP=100 жёстко; perf — справочно** (dev-bootRun занижает perf) |
| 8 | Кросс-браузер (deep-pass) | **Только Chromium** (chrome-devtools MCP); CSS без экзотики → репрезентативно |
| 9 | Порядок больших фаз | **Аудит → фиксы → deep-pass.** (1) закрыть все 11 surfaces (Final ✅, дефекты записаны); (2) волна фиксов всего накопленного; (3) глубокий adversarial проход |
| 10 | Уведомления | **Молча, только леджер.** Никаких PushNotification; весь прогресс в §21. Пуш допустим только на blocker, реально требующий решения пользователя |

**Итоговый operating-contract (digest для быстрого чтения в §0 каждого тика):**

1. Каденция — **10 мин** (cron неизменен, ScheduleWakeup не звать).
2. Тик — **1+ мелких единиц**, сколько уложится; каждая с чистым леджер-следом.
3. Порядок surfaces — **SHELL → OVERLAYS → FEEDBACK → COMPONENTS**.
4. Пустой surface — **2+ adversarial-раунда** до Final ✅.
5. Дефект — **live-verify + запись сейчас; код-фикс отдельной волной** (не в тике находки).
6. Верификация фикса — **static + live chrome-devtools, без gradle** при живом bootRun.
7. Lighthouse — **a11y=100 + BP=100 жёстко**, perf справочно.
8. Кросс-браузер — **только Chromium**.
9. Большие фазы — **Аудит (11 surfaces) → волна фиксов → deep-pass**.
10. Комм-режим — **молча в леджер**; пуш только на blocker-решение.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `FOCUS` | Главная / тренировка / вопрос | `focus-training.html` | `focus-question.html` | flashcard; MCQ; selected; correct; wrong; explanation; session; empty branches; long/code-heavy | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **🎯 FINAL ✅ VERIFIED (R0.150) — 1-я полностью закрытая поверхность.** Весь state-family live R0.143–149 (MCQ/post-answer/flashcard/STUDY/session-progress/FT-15/empty×4/error-CSRF); 375→2560; light+dark; a11y-снапшоты; console-clean; 0 overflow. DoD-свип R0.150: #11 design-swap live (editorial+linear+swiss рендерят MCQ чисто, 0 overflow) + #12 zoom200% (reflow в 1 колонку, 0 h-scroll) + DPR1/DPR2. Все 20 DoD (§19) закрыты |
| `RESULT` | Результат ответа / no-JS fallback | `result.html` | `result.html` | correct; wrong; explanations; related; extra controls; loading/error; no-JS | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **🎯 FINAL ✅ VERIFIED (R0.152) — 2-я полностью закрытая поверхность.** Parity R0.151 (no-JS native POST /answer SSR; wrong-вердикт, option correct/wrong/other, per-option+main «Пояснение», SM-2 disclosure, RES-16 related sticky-рейка ≥1200px / инлайн 375, favorite aria-pressed двунаправл. A1). DoD-свип R0.152: correct-вердикт live («Верно!» green status-correct, option-correct, hasWrongOption=false) + #11 design-swap (linear/swiss/editorial 0 overflow) + #12 zoom200% (0 h-scroll) + DPR1 light (0 overflow, AA-green). Refactor ✅ (dead-AI вырезан, RES-16 семантика, контракт-enforce) + Regression ✅ (TemplateFragmentContractTest result.html-контракты + RES-16 regex, зелёный R0.134, шаблон не менялся). Все 20 DoD (§19) закрыты |
| `SUMMARY` | Итоги сессии | `session-summary.html` | `session-summary.html` | score; mistakes; recommendations; empty; share; print; long table | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **🎯 FINAL ✅ VERIFIED (R0.153) — 3-я полностью закрытая поверхность.** non-zero state-family live (EXAM 1верно+1неверно → /session-summary): score «50,0%» warning-token, mode, duration, stats-grid, topic-table (role=table tabindex=0, 5 колонок), mistakes ×1 (кликабельны), recommendations ×1, скрытый h2 «Результат сессии» (H-nav якорь). share (STA-20: `buildShareText` c[0]/c[2]/c[1]/c[4] = тема/верно/всего/точность — маппинг цел, «Arrays strings: 1/2 (50,0%)») + print (@media print прячет nav/tools/actions, break-inside:avoid). DoD-свип: design-swap linear/swiss/editorial 0 overflow + zoom200% 0 h-scroll + DPR2/DPR3 (DPR1 CSS-px-инвариант) + 375 mobile (0 body-overflow, table internal-scroll tabindex=0) + light+dark (AA warning-amber/correct-green/wrong-red) + console-clean. Refactor ✅ + Regression ✅ (TemplateFragmentContractTest: summary-page/скрытый-h2/role=table/rowheader/aria-live status, зелёный R0.134, шаблон не менялся) |
| `SETTINGS` | Настройки | `settings.html` | `settings.html` | session; appearance; data; validation; saved feedback; reset dialog; danger states | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **🎯 FINAL ✅ VERIFIED (R0.154) — 4-я полностью закрытая поверхность.** Live прод-parity: ARIA-вкладки (tablist/roving-tabindex/aria-selected/aria-controls, tab-switch panel display + tab-state персист localStorage), 3 панели (Сессия: today-hero+фильтры+режим SET-15 CTA; Оформление: 7 осей персонализации — 6 radiogroup Дизайн×11/Тема/Раскладка/Ширина/Плотность/Движение + font-stepper, revealed JS; Данные: export JSON/CSV revealed + danger-zone). reset-confirm overlay (`.prompt-overlay` role=alertdialog aria-modal scrim rgba(0,0,0,.6) fixed z100, focus-trap→«Отмена», Esc+Cancel dismiss — **сброс НЕ выполнялся**). dark-1280 + light-1280 + 375-mobile: 0 overflow, console-clean. Refactor/Regression уже ✅ |
| `STATS` | Аналитика | `stats.html` + `stats.js` | `stats.html` | normal; cold start; no match; charts; long table; collapsed/expanded; sort; print | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **🎯 FINAL ✅ VERIFIED (R0.155) — 5-я полностью закрытая поверхность.** Live прод-parity: stats-grid + 3 next-action (recommended is-recommended), фильтр+поиск, **2 Chart.js-графика** (topicProgress+topicAccuracy, `Chart.getChart`=instance, цвета адаптируют тему), **topic-table 319 строк** (sort: клик th→aria-sort+live-region «отсортирована: … по возрастанию»+reorder; collapse=12→expander «Показать все (307 ниже)»/«Свернуть», aria-expanded; mobile card-view `tr`/`td` display:grid + data-label), forecast (дата «Завтра» гуманизирована), no-match-поиск («ничего не найдено»+reset /stats, дашборд остаётся). design-swap linear/swiss/editorial 0 overflow + zoom200% + DPR2/DPR3 + 375-mobile 0 body-overflow. console: только benign chart.js `.map` CSP-block (devtools-only source-map, сам chart.js грузится). cold-start empty template-gated (данные есть). Refactor/Regression уже ✅ |
| `ERROR` | Ошибки 4xx/5xx | `error.html` | `error.html` | 400; 401; 403; 404; 5xx; developer disclosure; recovery | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **🎯 FINAL ✅ VERIFIED (R0.156) — 6-я полностью закрытая поверхность.** Live прод-parity: **404** (GET несущ. URL → код «404», title «Такой страницы нет», 404-recovery-body, disclosure СКРЫТ (path/message пусты), **нет stacktrace**); **403** (POST /start битый CSRF + Accept:text/html → код «403», «Доступ запрещён», body про устаревший токен, **disclosure показан** path=/start/status/Forbidden/timestamp — минимум, **без stacktrace**). Content-negotiation корректна (fetch без Accept→JSON `{status,error,message,path}`; браузер Accept:text/html→error.html). recovery-nav (На главную/Аналитика/Настроить). 400/401/5xx = тот же th:with code-driven th:text бранч. design-swap linear/swiss/editorial 0 overflow + zoom200% + DPR2/DPR3 + 375-mobile (error-code 64px scaled, 0 body-overflow) + light+dark AA. error.html без JS (console-clean by construction). Refactor/Regression уже ✅ |
| `SHELL` | Глобальная оболочка / header / navigation | `fragments/header.html` + `head.html` | `shell.html` | desktop; mobile nav; themes; designs; skip link; back-to-top; no-JS | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **🎯 FINAL ✅ VERIFIED (R0.157) — 7-я полностью закрытая поверхность.** Live прод-parity: masthead (brand+kicker), nav 3 линка (aria-current active), 3 тоггла (layout/design/theme — все revealed JS, 44×44, aria+meta-color sync: theme dark↔light, design editorial→linear цикл, layout flow→split→grid), skip-link (translateY reveal on-focus 175×44 → #main-content), back-to-top (появляется >1.5 экрана opacity1, клик→top+фокус main-content, motion-aware), progress-bar, sprite. **Мобильная навигация = responsive flex-wrap (гамбургера/`#mobile-menu` в проде НЕТ — прежнее допущение ложное, испр. R0.157):** nav всегда видна, переносится на свою строку, 0 overflow @375. no-JS (SSR): noscript-alert + тогглы hidden + back-to-top JS-only + nav plain anchors. DoD: все 11 дизайнов masthead 0 overflow + zoom200% + light+dark AA + console-clean. persistence-API `window.__{theme,design,layout,motion,fontScale}` pre-first-frame (head.html). Refactor/Regression уже ✅ (фрагменты design-agnostic, в сессии не менялись) |
| `OVERLAYS` | Dialogs / popovers / dropdowns / confirmations | templates + `app.js` triggers | `overlay-state-atlas.html` | admin reset; confirmations; menus; popovers; tooltips; any discovered overlay | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | **🎯 FINAL ✅ VERIFIED (R0.158) — 8-я поверхность.** kbd-help live: open «?»→role=dialog/aria-modal/scrim rgba(0,0,0,.6)/z100/фокус→closeBtn/фон inert+aria-hidden/7 строк; все 4 пути закрытия (Esc/closeBtn/«?»-toggle/backdrop) восстанавливают фокус на trigger. reset-confirm live (R0.154) + **OVL-1 ОПРОВЕРГНУТ (R0.158):** с активной вкладкой Данные оба пути (Esc+Cancel) симметрично восстанавливают фокус на `.danger-btn` — прошлая «асимметрия» = артефакт замера (overlay открывался при `display:none` кнопке → lastFocused=body). Деструктив НЕ выполнялся. Prev-Notes: | R0.106: атлас СОЗДАН + QA (file:// dark/light/narrow + живой демо scrim/focus-trap/Esc) — dialog/alertdialog/drawer/disclosure/tooltip (зеркалят прод `#kbd-help-overlay`/`#reset-confirm`) + menu/popover (кандидаты, не в проде). **`#mobile-menu` в проде НЕТ** (испр. R0.157 — мобнав = flex-wrap, не drawer). **R0.154 живьём:** reset-confirm (`.prompt-overlay`) прод-verified — role=alertdialog/aria-modal/scrim/focus-trap(→«Отмена»)/Esc+Cancel dismiss. **OVL-1 (P3, требует live-reverify):** прошлый A/B показал Esc→body vs Cancel→trigger, НО код путей в `app.js` promptModal **идентичен** (Esc стр.410 и Cancel стр.407 оба зовут `settle(cancelValue)` → restore `lastFocused`) → «Esc→body» вероятно артефакт замера chrome-devtools, не баг. Сначала live-подтвердить в OVERLAYS-тике; фикс (если подтвердится) — **отдельной волной** (§2b п.5), не в тике находки. Final ◐ — прод-parity gated (live-verify kbd-help + OVL-1) |
| `FEEDBACK` | Alerts / toasts / status / validation | fragments + templates + `app.js` | `feedback-state-atlas.html` | assertive; polite; success; warning; error; pending; validation; offline/permission if reachable | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | R0.105: атлас СОЗДАН + QA (file:// dark/light/390) — 6 секций (inline-alert error/success/info/warn, toast, validation норма/ошибка, pending спиннер/скелет, verdict, aria-live легенда), зеркалит `base.css:1275`. **Final ◐→✅ (R0.160):** прод-parity live-verified — verdict success/error (зел150/красн27, тройное кодир.), aria-live assertive(`#interview-alert`)+polite(`#result-feedback` atomic), validation (нативный Constraint API, рус. сообщения); **warning-gap ЗАКРЫТ Instrument** (`--color-status-warning` amber → `.result-penalty-note`). Открыто: **FB-1** (нет inline `:invalid`-визуала, фикс-волна) + pending/toast не триггерились (CSS есть) |
| `COMPONENTS` | UI primitives and all states | `base.css` + fragments + JS | `component-state-atlas.html` | default; hover; active; focus; disabled; loading; selected; checked; expanded; dark; long content | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | R0.107: атлас СОЗДАН + QA (file:// dark/light/narrow, 0 overflow) — buttons 4×6 состояний/links/icon/inputs/select/seg-control/switch/native/tabs/MCQ-опция 6 состояний/chips/badges/kbd/surfaces; demo-классы зеркалят `:hover`/`:active`/`:focus`. R0.108: +expanded/long-content → все 11 state-family §7 покрыты. **Final ◐→✅ (R0.161):** прод-parity live-verified — focus-visible (teal outline, ≠ selected → ревью #11 clean), seg-btn/tab checked (distinct), :hover 58/:active 20/:focus-visible 17/:disabled 23 правил, .is-active для checked/expanded, reduced-motion. Cross-surface находки: **CMP-1** (design-ось в settings ещё 11 дизайнов) + **CACHE-1** (tokens.css ?v=64 не бампнут) → cleanup-2 |
...
| `result.html` | page | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ◐ | ◐ | ✅ | ◐ | ◐ | ◐ | ⏸ | Чисто; **R0.126: regenerate-UI + `.question-side` удалены** (app.js/base.css v=94); **R0.127: RES-15 related-list-семантика** (`div role=list` + `role=listitem`, app.js v=64, контракт 10/10); RES-14 снят (код в `.answer`), RES-8 контраст AA все 20; RES-3 favorite=🚫DECISION_LOCK R0.142 (юзер: result-only) |
...
| `js/app.js` | script | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ◐ | ✅ | ◐ | ✅APP-8 regenerate удалён R0.126; ✅RES-15 related-list-семантика R0.127 (renderRelatedQuestions обёрнут role=list); 🌱APP-9 comparison-table th без scope/caption; 🚫APP-5 favorite DECISION_LOCK R0.142 (юзер: result-only); **v=64** (RES-15 R0.127; regenerate-removal был v=63 R0.126) |
...
| `FT-15` | `training-actions` include (Проверить ответ) | CTA далеко при длинных вариантах | sticky/видимый submit | 🌱 BACKLOG | 🔶 BLOCKED R0.132 (app-gated + design-decision, НЕ стале-resolved): submit = «собранный футер сцены» (base.css:997-1012), не sticky; «слишком далеко при длинных вариантах» проверяемо ТОЛЬКО live-длинные-опции (app-gated); fix (sticky bottom-bar) = дизайн-решение (sticky header намеренно забанен контракт-тестом за FNO-риск, base.css:497/632). |
| `FT-17` | `app.js(v=51)` в конце body | Версионный контракт JS | бампать при правке app.js | 🔁 ONGOING | 🔁 CONTRACT_MONITOR |
| `FT-18` | Чип/подсказка «ФЛЕШКАРТЫ» vs фактический MCQ-UI (`resolveFocusModeChipText`/`resolveFocusModeHintText`, `MvcModelAttributeMapper:85/98`) | **Находка R0.147 (app-gated live QA):** в browse-состоянии `/training?mode=FLASHCARD` (без активной сессии) на вопросе С seed-опциями чип показывает «ФЛЕШКАРТЫ» + подсказку «сначала вспомни ответ, затем раскрой», НО UI рендерит 4 radio + «Проверить ответ» (MCQ). Причина: `flashcardMode` boolean (`FocusTrainingPageService:68` = `sessionFlashcardMode \|\| noAiFlashcard`) = **false** (нет сессии + вопрос с опциями), а чип/подсказка кейятся на `mode == InterviewMode.FLASHCARD` (условие `if (flashcard \|\| mode == FLASHCARD)`) → рассинхрон чип↔UI. Подтверждено прямым URL. Reachability: `/training?mode=FLASHCARD` — реальный URL (генерится `resolveFocusEmptyRetryHref:112` как retry-ссылка empty-состояния FLASHCARD). **Согласованность OK на:** (а) истинной флешкарте — вопрос без опций, `noAiFlashcard=true` (R0.147 verified: чип «ФЛЕШКАРТЫ» + «Показать ответ» reveal-UI совпадают); (б) активной FLASHCARD-сессии (`sessionFlashcardMode=true` форсит reveal на всех вопросах). Mismatch только в browse+mode-параметр без сессии на option-вопросе. | Убрать `\|\| mode == FLASHCARD` из условий чипа/подсказки (стр.85/98) → чип «ФЛЕШКАРТЫ» только когда `flashcardMode` boolean истинен = когда UI реально флешкарта. В активной сессии `flashcard` уже true (не регресс); в browse+mode+MCQ → чип станет MCQ-корректным. Требует проверки: нет ли состояния, где нужен FLASHCARD-чип при `flashcard=false` (не найдено). Фикс — mapper-уровень, контракт-тест + live re-verify. | 🆕 FINDING R0.147 (chip↔UI mismatch, P4 minor — cosmetic, edge-reachable; natural empty-retry-flow reachability НЕ подтверждена, только прямой URL) | ◐ OPEN — кандидат на фикс-тик; сначала подтвердить natural-flow reachability (active FLASHCARD → empty → retry → option-вопрос) |
| `RES-3` | `.btn-favorite` (звезда, aria-pressed) | Избранное — флоу под вопросом юзера | НЕ трогать без решения | ⛔ DEFERRED | 🚫 DECISION_LOCK R0.142 (решение юзера 2026-07-14: **оставить result-only**; избранное функц. полно — toggle result + фильтр /stats + /api/favorite) |
...
| `RES-16` | `a.related-question-item[role="listitem"]` (result.html:126 SSR + app.js:1557 focus-flow) | **Находка R0.144 (app-gated live QA, confirmed через computed a11y-tree):** RES-15 (R0.127) добавил list-семантику, поставив `role="listitem"` **прямо на `<a href>`** → ARIA-роль перекрывает нативную `link`. Проверено live: 3 `<a href="/?topic=…">` в раскрытом `#extra-analysis-content` имеют computed-role `listitem` (a11y-снапшот показал `listitem`+StaticText, НЕ `link`); href есть, фокусируемы, Enter-активируемы (нативный `<a>`), НО SR не находит их в rotor'е ссылок и анонсирует «элемент списка», не «ссылка» → WCAG 4.1.2 (role≠function). Два вхождения: `result.html:126` (server-render) + `app.js:1557` (`renderRelatedQuestions`). **Активация РАБОТАЕТ** (нативный anchor) — потеря только в announcement/discoverability. | WAI-ARIA канон списка-ссылок = `role=list` › `role=listitem`-обёртка › `<a>` (не `a[role=listitem]`). Фикс: снять `role=listitem` с `<a>` (вернёт link-роль) + обернуть каждый в `[role=listitem]` контейнер (или нативные `<ul>/<li>/<a>`); сверить CSS `.related-questions-list`/`.related-question-item` (base.css) — селекторы не должны сломаться при смене вложенности; app.js v-бамп + `TemplateFragmentContractTest`. Двухфайловый (SSR+JS), как RES-15. | 🆕 FINDING R0.144 (a11y role≠function, P4 minor — активация цела) | ✅ DONE R0.145 (оба рендера: `<a>` обёрнут в `<div role="listitem">`, role снят с `<a>` → нативная link-роль возвращена; `result.html` SSR + `app.js:1557` focus-flow; app.js v=64→65; CSS не затронут — `.related-question-item` таргет по классу; **контракт-тест +RES-16 guard зелёный** — нестинг `list›listitem-div›a` + ассерт «нет role= на `<a>`»; live focus-flow a11y-снапшот — deferred confirmation R0.146, SSR-путь контракт-верифицирован) · **✅ LIVE-CONFIRMED R0.146**: после рестарта app (свежий app.js v=65) отвечен вопрос с related → a11y-снапшот показал 3 related как **`link`** (было `listitem` в R0.144), DOM-роли `list›div[role=listitem]›a[role=null,href]` (evaluate) = канон WAI-ARIA. Роль listitem→link подтверждена в AX-дереве |
...
| `SET-3` | Design seg-control без preview | Краткое описание/preview пресета при выборе | M | 🌱 BACKLOG | ◐ ПРЕМИСА СДВИНУЛАСЬ R0.132: live-**preview УЖЕ есть** (дизайн применяется мгновенно при выборе — весь UI ре-темизуется; hint settings.html:185 «Превью применяется сразу») + generic-описание группы есть. Остаток = per-preset текстовое описание (напр. «Editorial — газетная типографика») = design-decision enhancement, low (live-preview уже показывает эффект нагляднее текста). |
| `SET-9` | a11y-оси разбросаны | Сгруппировать движение/контраст в раздел Accessibility | M | 🌱 BACKLOG | ◐ ПРЕМИСА УСТАРЕЛА R0.132: после tabs-редизайна ([[project_settings_tabs_redesign]], 0eb762a5) ВСЕ оси (Дизайн/Тема/Размер шрифта/Раскладка/Ширина/Плотность/**Движение**) co-located в ОДНОЙ панели «Оформление» (settings.html:164-249) — **НЕ «разбросаны»**. Отдельной оси «контраст» нет (в AA-токенах темы/дизайна). Остаток = выделенный под-раздел «Доступность» = design-decision; текущий unified «Оформление» — намеренный выбор редизайна. |
...
| `HEAD-7` | self-host chart.js vs CDN (CSP sourcemap) | Архитектурный + download-gated выбор | — | ⛔ DEFERRED | 🚫 DECISION_LOCK R0.142 (решение юзера 2026-07-14: **оставить CDN** jsdelivr+SRI для dev; fallback-таблицы защищают; self-host — если целиться в offline/CSP, отдельное решение) |
| `ICO-3` | нет dedicated warning-иконки (danger исп. `#i-flag`). **Уточнено р110:** `#i-flag` = double-duty на… | Намеренный reuse flag для danger-zone; distinct warning-символ = опц. визуал-полиш, только с IC-4 + требует settings.html (blocked). Не срочно | L | 🌱 BACKLOG (decorative, opt-polish с IC-4) | 🚫 DECISION_LOCK R0.142 (решение юзера 2026-07-14: **оставить reuse #i-flag**; distinct warning-иконка — только заодно с IC-4/warning-токеном, Фаза G) |
| `IC-5` | CTA-кнопки несут сырой юникод «→» (SR-шум + вне icon-системы). **Точный скоуп (р38-аудит):** 7 `btn… | **⚠️ БЛОКЕР — convention-решение (р38):** не механический swap. Все icon-кнопки ВЕДУТ иконкой (icon-lead, R34-конвенция); directional «next→» семантически ТРЕЙЛИТ → iconify создаёт mixed lead/trail. 3 варианта, каждый t… | M | 🌱 BACKLOG (design-decision; 2.4.4 CLEAN р117) | ◐ ПРЕМИСА УСТАРЕЛА R0.135: «SR-шум» **снят** — ВСЕ реальные CTA-стрелки «→» уже обёрнуты `<span aria-hidden="true">` (result:102, session-summary:44-45, stats:120/139, focus:130, error:52, today-widget:46-47; прочие «→» в шаблонах — в комментах/коде, не UI). SR их не читает; 2.4.4 CLEAN. Остаток = чисто визуальная конвенция (юникод vs Lucide-иконка) — WONTFIX-рекоменд.: иконизация даст mixed lead/trail (R34), a11y-выигрыша 0. Приоритет ~0. Decision-brief: `docs/ui-critique/session-window-turnkey.md` §A4. **🚫 DECISION_LOCK R0.142 (решение юзера 2026-07-14: оставить юникод «→»; WONTFIX).** |
...
| `APP-5` | favorite toggle (`btn-favorite`) | Флоу под решением юзера | НЕ трогать без решения | ⛔ DEFERRED | 🚫 DECISION_LOCK R0.142 (= RES-3; юзер выбрал result-only 2026-07-14) |
...
### R0.132 (2026-07-14) — Систематический recheck остатка ⬜ RECHECK §10 → статический не-decision backlog ИСЧЕРПАН (маркер maintenance-порога)

- **Единица (цель-1 recheck, продолжение R0.130/131):** сверить оставшиеся ⬜ RECHECK-строки §10 против ground-truth кода и реклассифицировать из размытого «нужна проверка» в точный статус. В отличие от R0.130/131 (стале-RESOLVED баги от `c0b997ba`), эта партия — enhancement'ы/решения; проверял сдвиг премисы.
- **Сверено 4 строки (app down, всё статически):**
  - **FT-15** (sticky submit) ⬜→🔶 **BLOCKED**: submit = «собранный футер сцены» (base.css:997-1012), не sticky; «далеко при длинных вариантах» проверяемо только live (app-gated); fix (sticky bar) = design-decision (sticky забанен контракт-тестом за FNO-риск). НЕ стале-resolved.
  - **SET-3** (seg-control preview) ⬜→◐ **ПРЕМИСА СДВИНУЛАСЬ**: live-**preview УЖЕ есть** (дизайн применяется мгновенно, весь UI ре-темизуется; hint settings.html:185 «Превью применяется сразу»). Остаток = per-preset текст-описание = low-value design-decision (live-preview нагляднее).
  - **SET-9** (a11y-оси разбросаны) ⬜→◐ **ПРЕМИСА УСТАРЕЛА**: после tabs-редизайна (0eb762a5) ВСЕ 7 осей co-located в панели «Оформление» (settings.html:164-249) — не разбросаны; оси «контраст» нет (в AA-токенах). Остаток = выделенный «Доступность» = design-decision (unified «Оформление» намеренный).
  - **HEAD-7** (self-host chart.js) ⛔→⛔ **DEFERRED confirmed**: chart.js на jsdelivr CDN (head.html:316, SRI-пиннинг); self-host = download-gated (скачать файл) + арх-решение. Статус точен.
- **Verify (grep §10 после правок):** остаток ⬜ RECHECK = **ровно 4** — `RES-3`+`APP-5` (favorite → user-decision §5), `IC-5` (raw «→», convention-3-варианта design-decision-БЛОКЕР), `ICO-3` (dedicated warning-иконка — декоративный opt-polish, blocked на IC-4+settings). **ВСЕ 4 — decision-gated или decorative-blocked; ни одна статически-разрешима автономно.**
- **МИЛСТОУН:** статический не-decision §10-backlog **ИСЧЕРПАН**. За R0.125–R0.132: window-тик 5/5 (SET-21/regenerate/RES-15/STA-20/AIR-6) исполнен, 3 стале-resolved бага сверены (AIR-1/AIR-3/RIA-2), 4 enhancement/decision реклассифицированы. Всё оставшееся в §10 требует **решения пользователя** (favorite ×2, IC-5 convention, chart.js self-host) либо **живого app** (parity-QA §7/§9, FT-15 long-options). Это и есть maintenance-порог (цель 7) — «без бесконечного произвольного редизайна».
- **Дальше:** автономных статических §10-единиц без решения/app не осталось. Следующий тик — **собрать единый turnkey-пакет для пользователя**: (a) decision-brief 4 открытых решений (favorite-флоу, IC-5 «→»-конвенция 3 варианта, chart.js self-host, ICO-3 warning-иконка) с рекомендацией по каждому; (b) turnkey parity-QA-чеклист для session-окна (§7 Parity ◐ по поверхностям + RES-15 a11y-снапшот + STA-20 визуал/share-print + extra-analysis related-раскрытие + FT-15 long-options). Один артефакт → максимально продуктивное окно взаимодействия. Нумерация R0.133+.

### R0.133 (2026-07-14) — Сверка §9 Contract/Tests-ячеек по фактическому покрытию контракт-теста (window-тик закрыт → гейт снят)

- **Единица (продвижение §9-матрицы, статически):** window-тик закрыт 5/5 (R0.129) и `TemplateFragmentContractTest` зелёный (10/10) — но §9 держал ⏸ в Contract/Tests-колонках по многим файлам («ждёт гейта window-тик контракт-тестов»). В R0.129 закрыл только focus-training.html (консервативно). Теперь сверил **фактическое покрытие теста** и закрыл все подтверждённые ячейки.
- **Покрытие `TemplateFragmentContractTest` (прочитан построчно, 10 тестов):** ассертит маркеры/ARIA-контракты по **templates** focus-training/result/stats/settings.html + **fragments** training-actions/result-zone-head/post-answer-controls/inline-alert.html + **JS** app.js/stats.js + **CSS** base.css/tokens.css. НЕ покрывает: session-summary/error.html, fragments/head/header.html.
- **Применено (Contract ⏸→✅ и Tests ⏸→✅, 8 файлов — только те, что тест реально ассертит):** `result.html`, `settings.html`, `stats.html`, `fragments/post-answer-controls.html`, `fragments/training-actions.html`, `css/base.css`, `js/app.js`, `js/stats.js`. (focus-training.html/result-zone-head/inline-alert/tokens.css уже были ✅.)
- **Оставлено ⏸ честно (вне покрытия теста):** `session-summary.html`, `error.html`, `fragments/head.html`, `fragments/header.html` — Contract/Tests ⏸ остаётся (контракт-тест их не ассертит; закрывать не на чем статически). **Final** ⏸/◐ остаётся у всех app-gated поверхностей (result/post-answer/session-summary) — parity-QA требует живой app, не флипается.
...
### R0.135 (2026-07-14) — Turnkey-пакет для пользователя собран; IC-5 SR-шум устарел (понижен до WONTFIX)

- **Единица:** собрать единый actionable-артефакт для окна взаимодействия с пользователем — автономная статика исчерпана (R0.132+R0.134), дальше нужен либо выбор пользователя, либо живой app. Перед написанием — собрал ground-truth по всем 4 решениям.
- **Ground-truth-находка (IC-5 стале, как AIR/RIA):** премиса «CTA несут сырой юникод «→» → SR-шум» **устарела** — проверил все «→» в шаблонах: **каждая реальная CTA-стрелка уже обёрнута `<span aria-hidden="true">`** (result:102, session-summary:44-45, stats:120/139, focus:130, error:52, today-widget:46-47). Прочие «→» — в комментах/JS-стрелках (`v2→v3`, `←→↑↓`), не в UI. → SR-шума нет, 2.4.4 CLEAN. IC-5 понижен: остаток = чисто визуальная конвенция (юникод vs Lucide), **WONTFIX-рекомендация** (иконизация даст mixed lead/trail R34, a11y-выигрыша 0). §10 IC-5 ⬜→◐ ПРЕМИСА УСТАРЕЛА.
- **Создан `docs/ui-critique/session-window-turnkey.md`:**
  - **Часть A — decision-brief 4 решений** (state / варианты / рекомендация / трудоёмкость): A1 favorite-флоу (result-only vs +focus; реком. оставить — избранное функц. полно: toggle result + фильтр stats + /api/favorite живой), A2 chart.js self-host vs CDN (реком. CDN для dev, self-host для offline/CSP; download-gated), A3 ICO-3 warning-иконка (реком. оставить reuse #i-flag), A4 IC-5 «→» (WONTFIX, понижен). **Ни один не блокирует «готовность»** — все «оставить» или архитектурно-опционально.
  - **Часть B — parity-QA-чеклист session-окна:** предусловия (docker postgres :5432 + bootRun); B1 FOCUS (state-family + FT-15 long-options), B2 RESULT (RES-15 a11y-снапшот related-list, extra-analysis раскрытие, no-JS), B3 SUMMARY (share/print + STA-20 stats), B4 остальные §7 (атласы→прод-snap), B5 что флипнуть после QA. Маршруты, вьюпорты (375/768/1280/1440/1920/2560 ×2 темы), метрики, pass-критерии.
- **Verify:** favorite verified (result.html:41-46 star+aria-pressed+/api/favorite:124 живой); §7 Parity ◐ = FOCUS/RESULT/SUMMARY (остальные Parity✅/Final◐ prod-gated); chart.js jsdelivr+SRI (head.html:335). Файл — чистый md, app не нужен.
- **Итог:** все 4 §10-решения охарактеризованы с рекомендациями; **при согласии с рекомендациями (в основном «оставить») фронт функционально завершён** — остаётся только parity-QA (нужен app). Автономный статический путь к конечной цели пройден до конца.
- **Дальше:** parity-QA (Часть B) требует поднятого app — вне автономного окна. §10-остаток ⬜ RECHECK = 3 (RES-3/APP-5 favorite, ICO-3 — все decision/decorative). Следующие тики без app/решения — либо микро-полиш обнаруженных мелочей, либо холд до app-окна; при подъёме app — исполнить turnkey Часть B. Нумерация R0.136+.

### R0.136 (2026-07-14) — Orphan-фрагмент/dead-template аудит CLEAN → закреплён регресс-гвардом

- **Единица:** статический dead-code-аудит слоя шаблонов (Фаза E: dead code + Фаза F: regression) — не проходился в леджере (прежние sweep'ы чистили dead-CSS SET-21 и dead-колонку STA-20, но инвентарь шаблонов/фрагментов на orphan не сертифицировался). App-независим.
- **Аудит (результат — CLEAN):**
  - **Фрагменты:** 12 объявленных `th:fragment` (head, header, sprite, inline-alert, mermaid-init, post-answer-controls, result-zone-head, stats-grid, stats-grid-content, today-hero, today-chip, training-actions) — **все 12 подключены** хотя бы одним `th:replace`/`th:insert`. 0 orphan. (3 «подозрительных» — post-answer-controls/result-zone-head/training-actions — подключены многострочным `th:replace` из focus-training/result, потому не всплыли в однострочном grep.)
  - **Page-шаблоны:** 6 (error, focus-training, result, session-summary, settings, stats) ↔ **идеальная биекция** с 6 view-именами (`MvcNavigationService`: focusView/settingsView/resultView/statsView/sessionSummaryView + `error` из `GlobalExceptionHandler` ModelAndView). Каждый view-метод вызывается ровно **1 раз** (не dead-константа); нет висячего view-имени (→ 0 runtime-500), нет шаблона-сироты (→ 0 dead-page).
- **Закрепление (durable):** ручной аудит превращён в автоматический инвариант — `TemplateFragmentContractTest.everyDefinedTemplateFragmentIsReferencedNoOrphans` (15-й тест). **Сам перечисляет** все `th:fragment` под `templates/` (Files.walk + regex, не хардкод-роестр) → авто-покрывает будущие фрагменты; требует `:: name(`/`:: name}` в корпусе (граница `(`/`}` не даёт `stats-grid` ложно засчитаться через `stats-grid-content`, `head` — через `header`); санити-минимум из 12 имён. **App НЕ нужен** — чистый JUnit ClassPathResource.
...
### R0.140 (2026-07-14) — Cache-busting версии консистентны + guard; статический derive-well иссякает

- **Единица:** аудит консистентности `?v=` cache-busting-версий (контракт HEAD-1/APP-7/FT-17/SET-12, app-независим) — из «Дальше» R0.139. Ассет, подключённый из нескольких шаблонов, обязан нести одну версию; дрейф = устаревший ассет на части страниц.
- **Аудит (CLEAN):** синтаксис — Thymeleaf `@{/js/app.js(v=N)}` (не `?v=`).
  - **app.js** = `v=64` в focus-training:222 / result:135 / settings:291 (3 шаблона) — **все совпадают** ✅ (это реальный кросс-шаблонный контракт; app.js грузится 3 страницами — самый дрейфо-опасный).
  - **stats.js** = `v=15` (stats:340, единственная ссылка) — n/a.
  - **tokens.css v=64 / base.css v=95** — обе в head.html (единый источник, включается всеми). Per-file версии независимы (bump только изменённого файла — tokens не менялся с v=64, base дошёл до v=95 через R0.125-128). Равенства НЕ требуется (memory-заметка «бампать ОБА» — консервативная эвристика волны 2026-06-23, не инвариант). ✅
- **Закрепление:** `TemplateFragmentContractTest.versionedAssetsAreConsistentAcrossTemplates` (19-й тест) — парсит `@{/js|css/NAME(v=N)}` по всем шаблонам, группирует по NAME, требует `hasSize(1)` версий на ассет; санити `containsKey("app.js")`. Ловит app.js-дрейф. Чистый JUnit, app не нужен.
- **Verify:** `./gradlew :quiz-app:test --tests "*TemplateFragmentContractTest"` → BUILD SUCCESSFUL 2s; `tests="19" failures="0" errors="0"`. Правка только тестовая → без `?v=`-бампа.
...
### R0.142 (2026-07-14) — ТРИГГЕР: юзер разблокировал Docker + принял все 4 решения; app поднят, старт parity-QA

- **Событие:** пользователь (1) разрешил мне самому пользоваться Docker («Докером ты можешь пользоваться сам»), (2) через AskUserQuestion принял **все 4 turnkey-решения Части A — все «оставить как есть» (рекомендованные)**.
- **Решения залочены (evidence = решение юзера 2026-07-14, §10 → 🚫 DECISION_LOCK):**
  - **RES-3/APP-5 favorite** → оставить **result-only** (звезда после ответа; избранное функц. полно — toggle + фильтр /stats + /api/favorite).
  - **HEAD-7 chart.js** → оставить **CDN** (jsdelivr+SRI для dev; fallback-таблицы защищают; self-host = отдельное решение под offline/CSP).
  - **ICO-3 warning-иконка** → оставить **reuse #i-flag** (distinct — только с IC-4/warning-токеном, Фаза G).
  - **IC-5 «→»** → **WONTFIX** (оставить юникод; уже aria-hidden, 2.4.4 CLEAN, a11y-выигрыш 0).
  - → **ВСЕ decision-gated пункты §10 закрыты.** Остался один визуальный A/B — accent-линейка `border-left 3px` (§4) — требует живого рендера, разберём в ходе parity-QA.
- **Инфра поднята мной:** `docker compose up -d postgres` (quiz-postgres healthy на **:5432**, DB/user/pass=interview — факт из docker-compose.yml + application.yml-дефолтов совпадают → env-переменные НЕ нужны; память про :5433/`SPRING_DATASOURCE_URL` устарела). `./gradlew bootRun` → app **:8080 отвечает 200**, 9818 вопросов загружено (seed-first, 0 options — flashcard-режим по дефолту), DispatcherServlet готов.
- **Разблокировано:** app-gated parity-QA (turnkey Часть B) — единственный оставшийся путь к §7 Parity ◐→✅ (FOCUS/RESULT/SUMMARY), §9 Final ⏸→✅ (result/session-summary/post-answer-controls), RES-15 a11y-снапшот, STA-20 визуал/share-print, FT-15 long-options, accent-line A/B. **Веха R0.140/141 «статика исчерпана» снята триггером — переходим в parity-QA-фазу.**
- **Дальше:** исполнить turnkey Часть B через chrome-devtools (скрины+DOM/computed на 6 вьюпортах ×2 темы), пофиксить найденное, флипнуть леджер-ячейки по факту. Нумерация R0.143+.

### R0.143 (2026-07-14) — parity-QA B1: FOCUS MCQ-default state — полная responsive+theme+a11y+console сверка

- **Единица тика (§16):** FOCUS-поверхность (`focus-training.html`), состояние **MCQ default/selected** — исчерпывающая parity-проверка живого прод-рендера (`http://localhost:8080/`, TRAINING без сессии, вопрос «Как отвечать на алгоритмические вопросы (шаблон)?», ALGORITHMS). Инструмент — chrome-devtools MCP (эмуляция device-viewport + colorScheme + `evaluate_script` overflow-замеры).
- **Console:** `list_console_messages` → **0 сообщений** (чисто, ни ошибок, ни ворнингов на загрузке FOCUS).
- **A11y-дерево (`take_snapshot`, 1280 dark):** образцовое — skip-link «Перейти к вопросу»→`#main-content`; `banner` c h1; `navigation "Основная навигация"` (3 ссылки); 3 header-кнопки с описательными `aria-label`+`description` (раскладка Поток→Два пейна / дизайн Editorial→Linear / тема авто→светлая); `main`; `complementary "Сессия"` (9818 к повтору); `form` с 4 `radio`, у каждого полное accessible-name «Вариант N: X. …»; фокус на radio A (клавиатурный старт); кнопка «Проверить ответ» `disabled`+`keyshortcuts=Enter`+description «1–4 выбор, Enter ответить»; `live=polite` регион для анонсов.
- **`invalid="true"` на radio — РАЗОБРАНО, не баг:** в шаблоне (`focus-training.html:148-151`) radio несёт `required`, **без** явного `aria-invalid`. `invalid` в a11y-дереве — нативная HTML5 constraint-валидация required-группы без выбора (`valueMissing`). Убрать `required` = потерять нативную валидацию. **Действий не требует.**
- **Overflow-матрица (`evaluate_script`, getBoundingClientRect по всем узлам):**
  - **375** (истинный device-viewport, mobile+touch, dark): innerWidth=375, scrollWidth=375, `overflowX=false`, **0 offenders**; заголовок вопроса адаптивно 27px/31px (даунскейл от desktop-3xl); CTA «Проверить ответ» на всю ширину (корректный mobile-паттерн), keyboard-hint скрыт. Скрин подтверждает: шапка стекается чисто, 4 MCQ-карточки A–D с бейджами, A выбрана (accent-ring).
  - **485/500** (window-min-clamp, light): скрин — кремовый фон, тёплый тёмный текст, оранжевый accent (подчёркивание ФОКУС, ring карточки A), editorial-serif заголовок. Light-палитра корректна.
  - **768** (light): clientWidth 753, `overflowX=false`, **0 offenders**, bg=rgb(250,249,245).
  - **1440** (light): `overflowX=false`, **0 offenders**, main-контейнер 1280px (max-width), центрирован (left 73).
  - **2560** (dark): `overflowX=false`, **0 offenders**, main остаётся 1280px (**НЕ** full-bleed, центрирован left 633) → FOCUS не подпадает под BAS-18 (prose `--measure` full-width — только для читалок, не для квиза). bg=rgb(38,38,36).
- **Инструментальная заметка:** `resize_page` клампит окно до ~485/500px (macOS Chrome min-width) → для истинного 375 нужна `emulate viewport`. `emulate` params **не кумулятивны** (colorScheme сбрасывает viewport-override) → передавать viewport+colorScheme вместе.
- **Итог по §7 FOCUS Parity:** MCQ default/selected state = **✅ parity-verified** (responsive 375→2560, light+dark, a11y, console, 0 overflow). Но полная FOCUS-parity (state-family §7: flashcard; correct; wrong; explanation; session-progress; empty-ветки; long/code-heavy FT-15; STUDY learn-фаза) **требует POST /answer и alt-режимов** → отдельные тики R0.144+. §7 FOCUS Parity остаётся **◐** с этой отметкой (не флипаю в ✅ до покрытия всех состояний — честный статус).
- **Дальше:** R0.144 — FOCUS post-answer states (correct/wrong/explanation) через POST /answer в TRAINING (мутация — с ведома юзера, авторизовано Docker-разблокировкой); затем flashcard + STUDY learn-фаза; после полного FOCUS state-family — флип §7 FOCUS Parity ◐→✅.

### R0.144 (2026-07-14) — parity-QA B1/B2: FOCUS post-answer states (correct+wrong) + RES-15 live + новая находка RES-16

- **Единица тика (§16):** FOCUS post-answer состояния через живой POST /answer (TRAINING; мутация БД авторизована Docker-разблокировкой R0.142). Сняты **оба** вердикт-состояния + RES-15 live-верификация + адверсари-проверка.
- **WRONG state** (ответил B на «Как отвечать на алгоритмические вопросы», A — верный): рендерится образцово —
  - **Per-option вердикт-аннотации** с a11y-префиксами в accessible-name: «Правильный ответ: …» (A, зелёная карточка circle-check + «ПРАВИЛЬНЫЙ ОТВЕТ» + разбор Развёрнутое объяснение/Пример/Когда применять/Подводные камни/Связанные вопросы+3 ссылки), «Твой выбор (ошибка): …» (B, красная circle-x + 4-частный дидактический разбор Что-на-самом-деле/Откуда-путаница/Если-бы-правда/Как-правильно), «Не выбран: …» (C/D, нейтраль + тот же 4-частный разбор).
  - Radio → `disabled` (повторно не ответить). **ПОСТ-РАЗБОР**: вердикт-карточка «⊗ Неверно» (red) + пронумерованные шаги 1–7 + Рекомендация.
  - **Вердикт в `status`-регионе** `atomic live=polite`, **`focused`** (фокус ушёл на вердикт — корректный SR-анонс). Console чист.
- **CORRECT state** (ответил верно на «С чего начать, услышав задачу?»): вердикт «⊙ **Верно**» (green circle-check, зелёная карточка + orange focus-ring — статус в фокусе), разбор с bold-акцентами, **инлайн-SSR-таблица** «Чек-лист первых 30 секунд» (ВОПРОС/ЗАЧЕМ + code-chips n≤20/O(n log n)/int·long·float) корректна, виджет уверенности «Насколько ты уверен» (Угадал/С трудом/Знал точно). `status` role/aria-live/focused симметрично wrong-state.
- **RES-15 live-подтверждение (закрывает app-gated хвост R0.127):** «Похожие вопросы» expandable → клик → `aria-expanded=true`, `aria-controls=extra-analysis-content`, кнопка **скрывается**; регион = `h3 "Похожие вопросы для закрепления:"` (**`h3InsideList=false`** — вне списка) + `div[role=list]` (прямые дети — 3× listitem) → a11y-tree читает «список, 3 элемента». RES-15 семантика РАБОТАЕТ.
- **🆕 НАХОДКА RES-16 (адверсари-верифицирована):** при RES-15 `role="listitem"` поставлен **прямо на `<a href>`** → перекрывает нативную `link`-роль. Live-доказательство: 3 related-`<a href="/?topic=…">` имеют computed-role `listitem` (не `link`); href/фокус/Enter работают (нативный anchor), но SR не видит их как ссылки (rotor + announcement). WCAG 4.1.2 role≠function, P4 minor (активация цела). Два вхождения: `result.html:126` + `app.js:1557`. Занесено в §10 RES-16 (◐ OPEN). **Не фикшу этим тиком** (§16 один юнит; фикс = реверс собств. документир. RES-15-паттерна + CSS-сверка вложенности → отдельный тик R0.145).
- **Мутация БД:** 2 ответа записаны в TRAINING (question 2012 wrong + algorithms-interview Q correct) — ожидаемо для parity-QA, в рамках Docker-авторизации. daily_activity/user_topic_stats инкрементнутся (обратимо, не критично).
- **Статус §7:** FOCUS Parity — MCQ-default (R0.143) + post-answer correct/wrong (R0.144) = ✅; остаток state-family (flashcard-режим, STUDY learn-фаза, empty-ветки) → R0.146+. RESULT Parity (B2): correct/wrong/related/extra-controls верифицированы через focus-flow (тот же app.js-рендер), но no-JS `result.html` напрямую не снят → остаётся ◐. §7 FOCUS/RESULT Parity пока **◐** (честно).
- **Дальше:** R0.145 — фикс RES-16 (снять role=listitem с `<a>`, обернуть в listitem-контейнер, оба рендера, v-бамп, контракт-тест, re-verify a11y). Затем R0.146 — FOCUS flashcard/STUDY/empty состояния.

### R0.145 (2026-07-14) — фикс RES-16: link-роль related-ссылок восстановлена (оба рендера + контракт-guard)

- **Единица тика (§16):** исправить находку RES-16 (R0.144) — `role="listitem"` стоял прямо на `<a href>`, перекрывая нативную `link`-роль.
- **Фикс (канон WAI-ARIA списка-ссылок `role=list › role=listitem-обёртка › <a>`):**
  - `result.html:125-134` (SSR): `<a>` обёрнут в `<div th:each="rq…" role="listitem">`, `role="listitem"` снят с `<a>` (+коммент RES-16). `th:each` переехал на обёртку.
  - `app.js:1555-1567` (`renderRelatedQuestions`, focus-flow): тот же transform в JS-строке — `'<div role="listitem"><a class="related-question-item" href=…>…</a></div>'` (+коммент). `node --check` чист.
  - **CSS не затронут:** `.related-question-item` таргетится по классу (base.css:2554), класс остаётся на `<a>`; sticky-рейка (base.css:3116/3126) таргетит `#result-related-questions` — внутренняя вложенность её не трогает. 0 render-риска.
  - `app.js` v=64→**65** в 3 шаблонах (result/settings/focus-training).
- **Контракт-guard (регресс-защита):** `TemplateFragmentContractTest` +3 ассерта в `keyPagesReferenceSharedFragmentsInsteadOfLocalCopies`: (1) нестинг-regex `related-questions-list" role="list"› <div…role="listitem"› <a`; (2) `<a class="related-question-item">`-тег присутствует; (3) этот `<a>`-тег `doesNotContain("role=")` (link-роль сохранена). **`./gradlew :quiz-app:test --tests "*TemplateFragmentContractTest"` → BUILD SUCCESSFUL** (wedge не случился — gradle поднял свежий daemon рядом с живым app; app остался :8080=200).
- **Верификация:** SSR-путь (result.html) = **контракт-верифицирован авторитетно** (guard прошёл); app.js-путь = идентичный структурный transform, node-syntax чист + тривиально инспектируем (та же выходная разметка, что у SSR). Живой browser a11y-re-снапшот focus-flow **отложен на R0.146**: static-хендлер живого app держит app.js в памяти (devtools делает live-reload статики по browser-триггеру, не по внешней gradle-правке; `build`+`src` оба содержат RES-16, но отдаётся кеш) → нужен рестарт app. Не рестартую mid-tick (over-scope §16 + wedge-риск). Ровно как RES-15/R0.127 отложил свой a11y-снапшот — корректность уже установлена, live-снап = confirmation, не гейт.
- **Коммит:** `f55737d7` (5 файлов explicit pathspec: result/settings/focus-training/app.js/contract-test; MCQ-WIP `edge-computing-interview.json` НЕ тронут).
- **Дальше:** R0.146 — рестарт app для свежей статики → live a11y-снап RES-16 focus-flow (подтвердить `<a>`=link в rotor'е) + FOCUS flashcard/STUDY/empty состояния (следующий кусок state-family).

### R0.146 (2026-07-14) — RES-16 live-подтверждён: рестарт app + a11y-снапшот focus-flow

- **Единица тика (§16):** закрыть deferred-confirmation RES-16 (R0.145) — рестарт app для свежей статики + живой a11y-снапшот focus-flow related-list.
- **Рестарт app:** убит старый java-процесс (PID 98883, держал app.js в static-кеше); свежий `./gradlew bootRun` в фоне; дождался :8080=200 (curl-retry). **Живой app теперь отдаёт app.js v=65** — маркер `RES-16` + новый нестинг-паттерн `role="listitem"><a class="related-question-item"` присутствуют в отдаваемом `/js/app.js` (было 0 до рестарта — static-хендлер держал старую копию в памяти; devtools live-reload статики не срабатывал на внешнюю gradle-правку).
- **Live-верификация RES-16 focus-flow** (ответил вопрос «Какие парадигмы решения существуют?» с related):
  - **DOM-роли** (evaluate_script): `div[role=list]` › 3× `div[role=listitem]` › `a.related-question-item[role=null, hasHref=true, parentRole=listitem]` — `role="listitem"` больше НЕ на `<a>`, нативная `link`-роль восстановлена. Канонический WAI-ARIA паттерн списка-ссылок.
  - **A11y-дерево** (take_snapshot, authoritative AX): 3 related экспонируются как **`link`** с полными URL и текстом вопроса («Как отвечать на алгоритмические вопросы (шаблон)?» etc.) — **было `listitem`+StaticText в R0.144** (роль listitem→link подтверждена в реальном AX-дереве). h3 «ПОХОЖИЕ ВОПРОСЫ…» вне списка. MCP-снапшот сворачивает generic list/listitem-обёртки (показывает leaf-ссылки) — сами обёртки в DOM подтверждены evaluate-скриптом, стандартно AX-экспонируются (AXList/AXListItem).
  - **Вывод:** RES-16 достиг цели — related теперь proper links (находятся в rotor'е ссылок, анонсируются «ссылка»), внутри валидного списка. SSR-путь был контракт-верифицирован (R0.145), focus-flow теперь live-верифицирован → **RES-16 полностью закрыт**.
- **Бонус-наблюдения (свежий рендер):** пост-разбор рендерит богатую SSR-таблицу парадигм (ПАРАДИГМА/КОГДА ПРИМЕНЯТЬ/СМ.), per-option аннотации все на месте, session-state «1 день» streak + «14 из 16 к повтору» (parity-QA-ответы накопились). Console чист (0 при загрузке).
- **Мутация БД:** +1 ответ (question 2014). daily_activity/user_topic_stats инкрементнуты (ожидаемо, Docker-авторизация).
- **Дальше:** R0.147 — FOCUS оставшийся state-family: flashcard-режим (`/training?mode=FLASHCARD`, кнопка «Показать ответ» вместо MCQ), STUDY learn-фаза («Проверить себя»), empty-ветки (вне сессии / сессия завершена). После полного FOCUS state-family → флип §7 FOCUS Parity ◐→✅.

### R0.147 (2026-07-14) — parity-QA B1: FOCUS flashcard reveal-state + находка FT-18 (chip↔UI mismatch)

- **Единица тика (§16):** FOCUS flashcard-состояние (state-family §7). Найдена БД-дорога к истинной флешкарте + разобрана логика режимов.
- **Как дошёл до истинной флешкарты:** `/training?mode=FLASHCARD` на вопросе С seed-опциями показал MCQ (не reveal) — потому что `flashcardMode` (`FocusTrainingPageService:68` = `sessionFlashcardMode || noAiFlashcard`) требует активной сессии ИЛИ вопроса без опций. Нашёл через psql (docker exec quiz-postgres, таблица `answer_options`) топик **`preparation/interview-preparation`** — вопросы с 0 опций → `noAiFlashcard=true` → истинный flashcardMode в browse.
- **Истинный flashcard reveal-state (`/?topic=preparation/interview-preparation`) — ✅ CLEAN:**
  - Чип «ФЛЕШКАРТЫ» + подсказка «сначала вспомни ответ, затем раскрой и оцени себя» + метка «ФЛЕШКАРТА — ВСПОМНИ ОТВЕТ ДО РАСКРЫТИЯ» — **чип↔UI согласованы** (в отличие от option-вопроса).
  - `▸ Показать ответ` = native `<details>/<summary>` browse-reveal (без JS/сети). Клик → `▾ Скрыть ответ` (expanded, focused), раскрыл богатый ответ: markdown-проза с code-chips, **mermaid-flowchart** (`graphics-document flowchart-v2`: HR Screening→Technical→Onsite→Финал→Оффер/Отказ, цветные ноды), h3-секции «Этапы интервью»/«Критерии оценки», SSR-таблица КРИТЕРИЙ/ЧТО ОЦЕНИВАЮТ, кросс-ссылки (Java Core/Concurrency/Behavioral) как proper `link`. Console чист. A11y: DisclosureTriangle summary, headings-иерархия, links корректны. `▸ Пример кода` — отдельный code-disclosure.
- **🆕 НАХОДКА FT-18 (chip↔UI mismatch, P4 minor):** `/training?mode=FLASHCARD` (browse без сессии) на вопросе С опциями → чип «ФЛЕШКАРТЫ»+flashcard-подсказка, но UI = MCQ. Причина: чип кейится на `mode==FLASHCARD` (`resolveFocusModeChipText:85`), а UI — на `flashcardMode` boolean (false: нет сессии + есть опции). `/training?mode=FLASHCARD` reachable — retry-href empty-состояния (`resolveFocusEmptyRetryHref:112`). Согласованность OK на истинной флешкарте + активной сессии; mismatch только в browse+mode+option-вопрос. Занесено §10 FT-18 (◐ OPEN, фикс = убрать `|| mode==FLASHCARD` из чип/подсказка-условий; natural-flow reachability не подтверждена — только прямой URL).
...
### R0.149 (2026-07-14) — parity-QA B1: FOCUS empty-ветки (4 шт.) → §7 FOCUS Parity ◐→✅

- **Единица тика (§16):** последний FOCUS state-family — empty-ветки (`focus-training.html:183-217`, 4 взаимоисключающие `th:if`). Гейт для флипа §7 Parity.
- **Разбор 4 веток:** (1) `reviewMode`; (2) `!reviewMode && session==null` (фильтры пусты); (3) `session!=null && !finished` (сессия без вопроса); (4) `session!=null && finished` (завершена). CTA-эмфаза адаптивна: `settingsPrimary=!reviewMode&&session==null` → primary=настройки; `finished` → primary=«Посмотреть итоги» (POST /finish); иначе primary=retry. `resolveFocusEmptyRetryHref/Text` дают href+текст под режим (`/review`·`/training`·`/training?mode=STUDY|FLASHCARD`).
- **Завершение R0.148-сессии (попутно):** «Завершить сессию» (session-bar) → POST `/finish` → **`/session-summary` — бонус B3 SUMMARY**: serif-h1 «Итоги сессии», score-card «ИЗУЧЕНИЕ / 0,0% / ТОЧНОСТЬ СЕССИИ» + 3 stat-карты ВСЕГО/ВЕРНО/ОШИБКИ (0/0/0 — завершил без ответов = **zero-score + empty-mistakes вариант**, секций ошибок/рекомендаций нет), «Время: 7 мин 18 сек», кнопки **«Скопировать итог» (share) + «Печать / PDF» (print)** (STA-20 present), nav «Что дальше» (Продолжить/Новая сессия/Аналитика). Сессия корректно очистилась (след. `/` = TRAINING без progressbar).
- **Ветка reachability — важное наблюдение:** активная сессия **перекрывает URL-фильтры** (`/?important=true` при сессии показал вопрос сессии, не empty). `important=true` = **4038 вопросов** (seed-флаг «важное», НЕ пустое избранное) → не даёт empty. Гарантированно-пустой фильтр = редкий топик + `onlyWrong=true` (нет неверных ответов там).
- **Ветка 2 (фильтры пусты, без сессии) — ✅ LIVE (1280 dark):** `/?topic=databases/scylladb-interview&onlyWrong=true` → h2 «Сейчас нет вопросов» + «Нет доступных вопросов по выбранным фильтрам. Открой настройки и расширь выборку.» + CTA **«Открыть настройки» (primary/orange →/settings)** + «Обновить тренировку» (secondary →/training, mode==null default). settings-primary совпадает с текстом-инструкцией. AIR-6 вычищен (нет мёртвого AI-абзаца). empty-card центрирован, 0 overflow.
- **Ветка 1 (review-empty) — ✅ LIVE (1280 dark):** `/review?topic=databases/scylladb-interview` → h2 «Сейчас нет вопросов» + «Нет вопросов с ошибками для review.» + CTA **«Обновить review» (primary/orange →/review)** + «Открыть настройки» (secondary). **Primary-эмфаза инвертирована** vs ветка 2 (retry primary, не settings) — settingsPrimary=false при reviewMode; href+текст retry адаптированы под режим. Зеркальное подтверждение логики.
- **Ветки 3 и 4 — template-verified defensive edges:** прочитаны построчно (`focus-training.html:194,199-217` + R0.129). Не воспроизводимы естественным browse (ветка 3 = race «сессия без вопроса»; ветка 4 «Сессия завершена» = finished-on-focus — нормальный finished-флоу идёт через result-баннер «Сессия завершена!» + `/session-summary`, live-подтверждён этим тиком). Разметка корректна (finished → «Посмотреть итоги» form POST /finish, settings+retry secondary).
- **Мутация БД:** +1 finish R0.148-сессии (session очищена). Empty-ветки/review — read-only.
- **🎯 ФЛИП §7 FOCUS Parity ◐→✅:** весь FOCUS state-family live-верифицирован за R0.143–149 — MCQ default/selected, post-answer correct/wrong, flashcard-reveal, STUDY learn/check, session-progress, FT-15 long-options, empty-ветки (1+2 live, 3+4 template-edge), error (CSRF), 375→2560, light+dark, a11y-снапшоты, console-clean, 0 overflow. **Production parity доказана (DoD #7).**
- **Final остаётся ◐ (честно):** DoD #12 (zoom 200%) и #11 (др. дизайны live, помимо editorial) для FOCUS не покрыты этим проходом (дизайны — token-only + AA-audit CLEAN, но live-swap не гонял; zoom200% не спот-чекал). Остальные ~18/20 DoD-критериев закрыты.
- **Дальше:** R0.150 — FOCUS Final DoD-свип: zoom 200% + DPR1 + 1-2 design-swap (linear/swiss) спот-чек на FOCUS → флип §7 FOCUS **Final ◐→✅** + счётчик «Final verified surfaces: 0→1». Затем B2 RESULT (no-JS fallback) / B3 SUMMARY (полный, не zero-score).

### R0.150 (2026-07-14) — FOCUS Final DoD-свип → §7 FOCUS Final ◐→✅ (1-я полностью закрытая поверхность)

- **Единица тика (§16):** закрыть остаток DoD (§19) для FOCUS — #11 (relevant designs live) + #12 (DPR 1/2 + zoom 200%). Гейт для флипа Final.
- **#11 design-swap live (editorial + linear + swiss, 1280 dark, MCQ-вопрос «Какие структуры данных…»):** переключал `data-design` (tokens.css кейится на `html[data-design]`).
  - **linear:** sans-serif вопрос, indigo-акцент (nav-underline/чип/focus-ring), option-карты с circle-badge, monospace code-chips. `scrollWidth==clientWidth==1265`, **0 overflow**.
  - **swiss:** grotesque sans, **red-акцент**, **square badge-боксы** + острые (не rounded) option-карты + square focus-ring (swiss-сигнатура base.css), square toggle-иконки. **0 overflow**.
  - Вывод: 3 дизайна с разными token-системами рендерят FOCUS MCQ структурно идентично и без поломок. Логика state-family design-agnostic (base.css structure), дизайны меняют только токены → AA-audit (`design-token-audit.py` CLEAN) + atlas-QA покрывают остальные 8. #11 закрыт.
- **#12 zoom 200% (editorial, CSS `zoom:2` на 1280):** контент **рефлоуит в одну узкую колонку** (nav переносится под бренд, option-карты стопкой), `bodyScrollWidth=633 < innerWidth=1280` → **0 горизонтального скролла**, без наложений/клиппинга; вопрос+submit присутствуют. Текст масштабируется (WCAG 1.4.4 resize-text + 1.4.10 reflow — оба удовлетворены).
- **#12 DPR 1/2:** DPR2 — все прошлые тики (deviceScaleFactor 2); **DPR1** этим тиком (`1280x900x1`, `devicePixelRatio=1`) — editorial рендерит полный MCQ, иконки/границы криспные на 1×, orange-акцент, focus-ring, **0 overflow**.
- **Мутация БД:** нет (design-swap/zoom/DPR — клиентские, read-only).
- **🎯 ФЛИП §7 FOCUS Final ◐→✅:** все 20 DoD-критериев (§19) закрыты — inventory/mockup/states/mockupQA/mapping/port (1-6) ✅ ранними раундами; production-parity (7) R0.149; refactor (8) ✅; responsive 375→2560 (9) + light/dark (10) R0.143–149; **designs (11) + DPR1-2/zoom200% (12) R0.150**; keyboard/focus (13) R0.143/148; WCAG=0 (14, RES-16 закрыт R0.146); loading/empty/error/recovery (15) R0.149; no-JS/PE (16 — FOCUS native-form submits: /answer·/study-confirm·/flashcard-reveal, работают без JS by construction); console/network (17) clean; targeted tests (18, TemplateFragmentContractTest+Mockito green); screenshots (19) актуальны; pixel-diff vs mockup (20) объяснён. **FOCUS — 1-я поверхность с Final ✅.**
- **Счётчик:** блок «Начальные значения нового раунда» (стр.~40) — round-start baseline snapshot, НЕ live-тэлли (оставлен как исторический маркер; авторитетная веха — §7 FOCUS Final ✅ + эта запись). FOCUS = первая Final-verified поверхность раунда.
- **Дальше:** R0.151 — **B2 RESULT** (`result.html`): no-JS fallback (native POST /answer, JS off), related-list a11y (RES-16 уже закрыт), favorite-звезда `aria-pressed` тоггл, обе темы → цель §7 RESULT Parity ◐→✅ + Final ⏸→. Затем B3 SUMMARY (полный не-zero-score вариант: mistakes + recommendations).

### R0.151 (2026-07-14) — parity-QA B2: RESULT (`result.html`) no-JS fallback + favorite → §7 RESULT Parity ◐→✅

- **Единица тика (§16):** RESULT-поверхность (`result.html`) — no-JS full-page fallback + state-family (correct/wrong/explanations/related/extra-controls/favorite).
- **Ключевое различие 2 result-рендеров (разобрано):** с JS submit MCQ-формы перехватывается → **inline post-answer** на focus-training (URL остаётся `/?topic=…`; покрыт R0.144). **Без JS** форма делает **native POST `/answer`** → **standalone `result.html`** (URL `/answer`). Форма: `action=/answer method=post`, radio `optionId`, hidden `_csrf/questionId/topic/group/ordered`.
- **no-JS SSR-проба (fetch POST /answer, optionId=7378):** status **200**, finalUrl `/answer`, HTML **53969 симв.**, `class="result-page"`, вердикт, `option-correct`+`option-other` маркеры, «Пояснение» h3, `result-related-questions`, **RES-16 nesting** (`related-questions-list" role="list"` › `role="listitem"`-обёртка › `<a>` — regex-подтверждено), «Следующий вопрос», `app.js?v=65`. **Полная SSR-страница без JS.**
- **Визуальный рендер result.html (native `form.submit()` минует JS submit-handler, optionId=7379 → wrong):** URL→`/answer`, полная страница.
  - **Layout:** header «Результат», **top-row stats-grid** (ВСЕГО 34/К ПОВТОРУ 33/ВЫУЧЕНО 0/ВЕРНО 0/ОШИБКИ 2/ТОЧНОСТЬ 0,0%), без session-bar (нет сессии — верно), main-card + **правая sticky related-рейка** (`.ed-page > #result-related-questions`, wide ≥1200px).
  - **Вердикт «⊗ Неверно»** (red, circle-x icon); опции: A green «ПРАВИЛЬНЫЙ ОТВЕТ» (option-correct) + вложенное «Развёрнутое объяснение» (explanation-correct box), C «ТВОЙ ВЫБОР» (option-wrong), B/D other + per-option аннотации («Что на самом деле»/«Откуда путаница»/«Если бы правдой»/«Как правильно»). Главное «Пояснение» h3 (markdown: сложности/где применяется). **SM-2 disclosure** (review-state есть). «РАЗБОР ОТВЕТА» chip. «Следующий вопрос» + related 3× proper `link`.
  - **correct-вердикт** не гонял отдельно — тот же `result.html:52` th:classappend бранч (`${ok}?status-correct:status-wrong`), структурно идентичен (template-verified).
- **Favorite aria-pressed тоггл (A1) — ✅ двунаправленный:** `#btn-favorite` был `pressed`(true)/«Убрать из избранного»/active (вопрос important=true). Клик → aria-pressed **false**/«Добавить в избранное»/active снят (JS POST /api/favorite + клиент-стейт). Клик обратно → **восстановлен** (true/«Убрать»/active) — состояние не испорчено.
- **Темы + responsive:** **dark** (1280) + **light** (1280, emulate на живой /answer — GET недоступен) оба чисты (red-вердикт контрастен на cream, green correct-box). **375 mobile:** `scrollWidth==clientWidth==375`, **0 overflow**; related = `position:static` ниже карточки (top 4428) — sticky-рейка корректно сворачивается инлайн <1200px.
- **Мутация БД:** +2 ответа (fetch optionId=7378 на q1845, native q1845 optionId=7379); favorite тоггл туда-обратно (нетто-ноль). daily_activity/user_topic_stats инкрементнуты (Docker-авторизация).
- **🎯 ФЛИП §7 RESULT Parity ◐→✅:** no-JS fallback + весь state-family + favorite + 2 темы + responsive live-верифицированы. **Final ⏸** остаётся: DoD-свип (zoom200%/DPR1/design-swap) + correct-вердикт live → R0.152.
- **Дальше:** R0.152 — RESULT Final DoD-свип (zoom200% + DPR1 + design-swap + correct-вердикт live) → §7 RESULT Final ⏸→✅. Затем **B3 SUMMARY** полный (non-zero-score: mistakes + recommendations, long table, share/print).

### R0.152 (2026-07-14) — parity-QA B2 финал: RESULT (`result.html`) DoD-свип → §7 RESULT Refactor/Regression/Final ◐/◐/⏸→✅

- **Единица тика (§16):** закрытие RESULT до Final ✅ — DoD-свип §19 (correct-вердикт live + design-swap + zoom200% + DPR1) + честная верификация Refactor/Regression.
- **correct-вердикт live (закрывает пробел R0.151):** native `form.submit()` optionId=**7382** (правильный для q1846 «Что такое хеш-функция…», SM-2 ушёл с q1845→q1846) → `/answer`. DOM: `statusText`=**«Верно!»**, `status status-correct`, color `rgb(111,181,133)` (green), `option-correct`+метка «Правильный ответ», **`hasWrongOption:false`** (при верном ответе option-wrong не рендерится — другая ветка `th:classappend` чем wrong-кейс R0.151). related=`role=list`×**3** (RES-16), favorite `aria-pressed=true`/«Убрать из избранного» (q1846 important). Скрин dark 1280: зелёный вердикт + чек-иконка, favorite-звезда orange, HASH TABLES badge, related в правой sticky-рейке, разбор «Что на самом деле/Откуда путаница».
- **DoD #11 design-swap live** (`data-design` linear/swiss/editorial на живом /answer): все три **hOverflow=0** (scrollW==clientW==1265), card видима, success-токен зелёный per-design (linear `rgb(63,185,80)`, swiss `rgb(63,180,99)`, editorial `rgb(111,181,133)`), related в рейке (left~806px) во всех.
- **DoD #12 zoom 200%** (`style.zoom=2`): **hOverflow=0**, «Верно!» читаемо, card видима — reflow без горизонтального скролла.
- **DoD DPR1 + light** (`emulate 1280x900x1` light): **hOverflow=0**, «Верно!» light-зелёный `rgb(46,107,69)` (AA на cream), 3 related, card видима. Скрин light подтверждён визуально (editorial serif, favorite orange, related-рейка).
- **Refactor ◐→✅ (верифиц. чтением):** `result.html` полностью отрефакторен — dead-AI-путь вырезан (нет `extra-analysis-toggle-result`, `.question-side` снят C24/R0.126), RES-16 семантика (role=list/listitem-обёртка/`<a>`), app.js в конце body (C31), favorite-кнопка корректна (aria-pressed/aria-label/title тоггл). Контракт-тест **enforce-ит** рефактор (`doesNotContain "id=\"extra-analysis-toggle-result\""`).
- **Regression ◐→✅ (таргет-покрытие подтверждено):** `TemplateFragmentContractTest` содержит result.html-контракты: `body class="result-page"`, result-zone-head fragment, `question-main flow-stack-md`, `result-actions`, session-finished-условие, **RES-16 nesting regex** (`related-questions-list" role="list"` › `role="listitem"` обёртка), related-anchor `related-question-item`, `id="result-related-questions"`. Suite зелёный на R0.134; шаблон в этой сессии **не менялся** (только читался) → зелёный держится без ре-рана (gradle-ран пропущен — wedge-риск при живом bootRun, [[project_local_run_gotchas]]).
- **Мутация БД:** +1 ответ (native q1846 optionId=7382, верный). daily_activity/user_topic_stats инкрементнуты.
- **🎯 ФЛИП §7 RESULT Refactor ◐→✅ + Regression ◐→✅ + Final ⏸→✅:** все 20 DoD (§19) закрыты — 2-я полностью верифицированная поверхность после FOCUS. correct+wrong вердикты live, design-swap/zoom/DPR, light+dark, related/favorite/no-JS/sticky-rail.
- **Дальше:** R0.153 — **B3 SUMMARY** (`session-summary.html`): завершить сессию (EXAM/MARATHON) → POST /finish → /session-summary; non-zero-score вариант (mistakes + recommendations + long table + share/print STA-20). §7 SUMMARY Refactor/Regression/Final ◐/◐/⏸.

### R0.153 (2026-07-14) — parity-QA B3: SUMMARY (`session-summary.html`) non-zero → §7 SUMMARY Parity/Regression/Final ◐/◐/⏸→✅

- **Единица тика (§16):** довести живую EXAM-сессию до `/session-summary` с non-zero score + mistakes → верифицировать весь state-family (score/mistakes/recommendations/table/share/print) в dark+light+mobile → закрыть SUMMARY до Final ✅.
- **Как дошёл:** /settings → session-form (`action=/start`, `mode=EXAM`, `count=5`, `ordered=true`) submit → сессия. Ответы: Q1 неверно + Q2 верно (правильные варианты запрошены из БД `answer_options.is_correct`, таблица=`answer_options` db=interview/interview на :5432) → POST /finish (кнопка «Завершить сессию») → **/session-summary одноразовая** (GET после consume редиректит на /). **Урок:** `emulate` с переключением `isMobile` **перезагружает** страницу → одноразовый summary теряется; вьюпорт для мобильного скрина ставить ДО захода (регенерировать сессию per-вьюпорт).
- **Non-zero state-family live (dark 1280):** mode «Экзамен»; score **«50,0%»** `text-warning` (amber `rgb(217,164,65)`, верно т.к. 50≥50<80); duration «3 мин 44 сек»; stats-grid Всего 2/Верно 1/Ошибки 1; **topic-table** `role=table tabindex=0` (Safari/VO семантика + WCAG 2.1.11 scrollable-region-focusable), 5 колонок [Тема/Всего/Верно/Ошибки/Точность], строка-агрегат [Arrays strings, 2, 1, 1, 50,0%] (correct green/wrong red/accuracy amber); **mistakes ×1** (кликабельны → /?topic=); **recommendations ×1** («точность 50% → повторить»); **скрытый h2 «Результат сессии»** (`visually-hidden`, H-nav якорь на счёт) + 3 видимых section-h2; summary-actions: «Повторить ошибки→/review» (mistakes есть → review-путь) + «Новая сессия→/settings» + «Аналитика→/stats» + «Продолжить→/».
- **STA-20 share (маппинг колонок цел):** `buildShareText` (реплика из DOM) → `c[0]`(тема)/`c[2]`(верно)/`c[1]`(всего)/`c[4]`(точность) = «- Arrays strings: 1/2 (50,0%)» = верно/всего (точность). copyBtn wired, status `role=status aria-live=polite`.
- **Print (@media print, base.css:3313):** прячет skip-link/masthead-actions/progress/back-to-top/**`.summary-actions`/`.summary-tools`**/overlays/noscript; белый фон; карточки border+`break-inside:avoid`; цвета score/table сохранены → чистый лист (score+table+mistakes+recs).
- **DoD-свип §19:** design-swap linear/swiss/editorial — все **hOverflow=0**, score+table present, warning-amber per-design; **zoom200%** hOverflow=0; **DPR2** (1280×900×2) + **DPR3** (375×812×3 mobile) покрыты live (DPR1 CSS-px-инвариант: layout в CSS-px не зависит от DPR, raster-ассетов нет → 0 overflow при DPR2/DPR3 ⇒ 0 при DPR1); **console-clean** (no messages).
- **375 mobile dark (регенерация #1):** `bodyHOverflow=0`; table `display:block overflowX:auto` скроллится **внутри контейнера** (98px internal, tabindex=0 keyboard); score/mistakes/recs/tools на месте; скрин — score-card центрирован, stats 3-up, tools стопкой, «Повторить ошибки» primary.
- **light 1280 (регенерация #2):** `bodyHOverflow=0`; score warning `rgb(122,84,0)`, table correct `rgb(46,107,69)`/wrong `rgb(179,38,30)`/accuracy amber — все AA на cream; скрин — editorial serif, «Hash tables»-строка, все секции.
- **Refactor ◐(уже был ✅)/Regression ◐→✅:** `TemplateFragmentContractTest` покрывает session-summary.html: `body.summary-page`, skip-link/main-content, скрытый h2 `summary-section-title visually-hidden`, `summary-score-value`, `nav.summary-actions aria-label`, `table.summary-table role=table tabindex`, caption visually-hidden, `scope=col role=columnheader`, `role=rowheader`, JS-tools `role=status`+`aria-live=polite`. Зелёный R0.134; шаблон в сессии не менялся → зелёный держится без ре-рана (gradle пропущен — wedge-риск).
- **Мутация БД:** 3 EXAM-сессии × 2 ответа (6 ответов: q1932/1929, q1878/1853, q1870/1865), все завершены. daily_activity/user_topic_stats инкрементнуты (Docker-авторизация).
- **🎯 ФЛИП §7 SUMMARY Parity ◐→✅ + Regression ◐→✅ + Final ⏸→✅:** все 20 DoD (§19) закрыты — **3-я полностью верифицированная поверхность** (после FOCUS R0.150, RESULT R0.152).
- **Дальше:** R0.154 — **B4 остальные поверхности** прод-parity: SETTINGS/STATS/ERROR/SHELL — по одному прод-снапу vs атлас (обе темы, 2-3 вьюпорта, 0 overflow); OVERLAYS живьём (scrim/focus-trap/Esc на `#kbd-help-overlay`/`#reset-confirm`/`#mobile-menu`). §7 эти строки Final ◐.

### R0.154 (2026-07-14) — parity-QA B4: SETTINGS (`settings.html`) прод-parity → §7 SETTINGS Final ◐→✅ + находка OVL-1

- **Единица тика (§16):** SETTINGS-поверхность прод-parity live (вкладки/панели/персонализация/данные + reset-confirm overlay) → Final ◐→✅. Reset-confirm overlay попутно продвигает OVERLAYS.
- **§0 health:** app 200; **postgres перезапущен** (`Up 15s`, был 3ч) — проверил app+DB связность (home 200, `questions`=9818, Hikari переподключился) до работы.
- **Вкладки (ARIA tabs live):** tablist видим (JS снял `.hidden`, `.js-tabs` на **body**), 3 таба (Сессия/Оформление/Данные) roving-tabindex (active=0, остальные -1) + aria-selected/aria-controls. Клик tab-data → aria-selected/tabindex переставлены, panel-data `is-active` display:block, остальные display:none. tab-state персистится в localStorage (после reload осталась Оформление).
- **Панели live:** Сессия (today-hero 9806/9818 + streak, фильтры-форма group/topic/checkboxes/order, режим-форма — CTA «Начать тренировку» + скрытый счётчик при TRAINING = SET-15). Оформление (`#personalization-card` revealed JS: 6 seg-control `role=radiogroup` — Дизайн×11/Тема×3/Раскладка×3/Ширина×2/Плотность×2/Движение×3 + font-stepper `role=group`; 7 осей). Данные (`#data-export-block` revealed JS: export JSON/CSV кнопки; danger-zone reset-форма).
- **reset-confirm overlay live (БЕЗ подтверждения сброса — деструктив не выполнялся):** клик `.danger-btn` (data-confirm) → `.prompt-overlay` появился: **role=alertdialog**, aria-modal=true, aria-labelledby=prompt-modal-title, заголовок «Сбросить банк вариантов?», кнопки [Отмена, Подтвердить]. Scrim `rgba(0,0,0,0.6)` position:fixed inset:0 z-index:100. Focus-trap: начальный фокус → «Отмена» (безопасный дефолт), focusInsideOverlay. **Esc** → overlay удалён из DOM (dismissable). **Cancel-клик** (переоткрыл) → overlay удалён + фокус восстановлен на `.danger-btn`.
- **🔴 Находка OVL-1 (P3, surface OVERLAYS):** асимметрия focus-restore — Cancel-клик восстанавливает фокус на trigger (`danger-btn` ✅), но **Esc уводит фокус на `<body>`** (не на trigger). WCAG 2.4.3 Focus Order: после закрытия диалога фокус должен вернуться к триггеру. Esc-путь в app.js, видимо, не вызывает focus-restore (в отличие от Cancel-обработчика). Не блокирует SETTINGS (это OVERLAYS-концерн, Esc всё же закрывает, риска данных нет). Чинить в OVERLAYS-тике вместе с прод-верификацией `#kbd-help-overlay` + `#mobile-menu`. app.js этот тик НЕ трогал (scope=ledger).
- **Скрины/метрики:** dark-1280 (Сессия — today-hero/фильтры/режим), light-1280 (Оформление — 11 design-кнопок wrap 2 ряда, все seg-контролы AA на cream), 375-mobile-dark (Оформление — табы в 1 ряд, design-кнопки wrap, session-launcher grid `auto-fit minmax` → 1 колонка). Все: **0 body-overflow**, **console-clean**.
- **🎯 ФЛИП §7 SETTINGS Final ◐→✅** (Refactor/Regression уже были ✅): **4-я полностью верифицированная поверхность** (после FOCUS/RESULT/SUMMARY). OVERLAYS +находка OVL-1 (Final остаётся ◐).
- **Дальше:** R0.155 — **STATS** (`stats.html`+`stats.js`) прод-parity: normal/cold-start/no-match, charts (Chart.js зоны), long-table (STA-20 sort/share/print, колонка «Сброшено» убрана, Зрелость data-col=4), collapsed/expanded; обе темы + 2-3 вьюпорта + 0 overflow → §7 STATS Final ◐→✅.

### R0.155 (2026-07-14) — parity-QA B4: STATS (`stats.html`+`stats.js`) прод-parity → §7 STATS Final ◐→✅

- **Единица тика (§16):** STATS-поверхность прод-parity live (normal + charts + sortable/collapsible table + no-match + responsive card-view) → Final ◐→✅.
- **§0 health:** app 200, pg Up ~1ч.
- **Normal state live (dark 1280):** stats-grid (Всего 9818/К повтору 9806/Выучено 0/Верно 5/Ошибки 10/Точность 33,3% + progress-bar); **3 next-action** («Повторить сегодня» is-recommended orange-border т.к. due>0, «Разобрать ошибки» 10, «Тренировать слабые темы» — data-weak-topic-summary live: «Слабее всего: Hash tables · 33%», 319 тем); фильтр-форма (group/topic/important/onlyWrong) + поиск-форма (type=search).
- **Chart.js графики (2 canvas live):** `topicProgressChart` (999×640, `Chart.getChart`=instance) — bar «Прогресс по темам» (hash-tables 34/algorithms 16/arrays-strings 36, Выучено-green/Осталось-grey); `topicAccuracyChart` — bar «Точность по темам» (3 темы ~33% red, «сначала слабые»). Оба рендерятся в **dark+light** (цвета адаптируют тему: grey/red на dark → адаптированные на cream).
- **topic-table (319 строк) live:** **sort** — клик th[data-col=1 «Всего»] → aria-sort=ascending (остальные none), `#table-sort-status` live-region «Таблица отсортирована: Всего, по возрастанию.», reorder (Agentic patterns→Spring rest client). **collapse** — data-collapse-rows=12, MutationObserver, expander «Показать все темы (307 ниже)» → клик → 12→**319** видимых, aria-expanded=true, текст «Свернуть таблицу» (WCAG 2.5.3 имя=видимый текст). **mobile card-view** (375) — `tr` display:grid + `td` display:grid со stacked data-label (ВСЕГО/ПРОГРЕСС/ТОЧНОСТЬ/ЗРЕЛОСТЬ), progress-бары, maturity-badge.
- **forecast:** «Прогноз повторений на 7 дней», дата гуманизирована («Завтра» вместо ISO через Intl ru-RU), bar 12.
- **no-match поиск:** `?q=zzznonexistentquery123` → «По запросу «…» ничего не найдено. Измени запрос или сбрось поиск и фильтры» + reset-ссылка /stats; **дашборд (таблица+2 графика) остаётся** (поиск не стирает stats). 0 overflow.
- **DoD-свип:** design-swap linear/swiss/editorial — 0 overflow (charts+table present); zoom200% 0 h-overflow; DPR2 (1280×900×2) + DPR3 (375 mobile) live (DPR1 CSS-px-инвариант; Chart.js сам держит devicePixelRatio для canvas). 375-mobile: charts стек (327px-трек), next-actions 1 колонка, **0 body-overflow**.
- **Console:** 1 error + 1 issue — оба про **source-map** `chart.umd.min.js.map` (CSP connect-src 'self' блокирует). **Benign devtools-only:** сам chart.js загрузился (SRI+CDN, HEAD-7/A2 decision) и рендерит; .map тянет только открытый devtools для отладки — реальные юзеры не видят; CSP-блок .map = корректное security-поведение, не дефект.
- **cold-start empty (`.stats-empty`):** template-gated (данные в БД полные — 319 тем со stats из seed + сессий), live не триггернулся; ветка = точная инверсия гейтов секций + `searchQuery` пуст (структурно verified в шаблоне, тот же паттерн что no-match, который verified live).
- **🎯 ФЛИП §7 STATS Final ◐→✅** (Refactor/Regression уже ✅): **5-я полностью верифицированная поверхность** (после FOCUS/RESULT/SUMMARY/SETTINGS).
- **Дальше:** R0.156 — **ERROR** (`error.html`) прод-parity: триггернуть 404 (несущ. URL) + 403 (CSRF/admin) + developer-disclosure gate; recovery-ссылки; обе темы + 0 overflow → §7 ERROR Final ◐→✅. Затем SHELL + OVERLAYS (с OVL-1 фиксом).

### R0.156 (2026-07-14) — parity-QA B4: ERROR (`error.html`) прод-parity → §7 ERROR Final ◐→✅

- **Единица тика (§16):** ERROR-поверхность прод-parity live (404 + 403 + developer-disclosure gate + content-negotiation + recovery) → Final ◐→✅.
- **§0 health:** app 200, 404-trigger=404, pg Up 2ч.
- **404 live (GET `/nonexistent-page-abc123`, dark 1280):** `body.error-page`; error-eyebrow «ОШИБКА»; декоративный error-code «404»; error-title «Такой страницы нет» (404-бранч th:text); error-body «Возможно, ссылка устарела или ты свернул не туда…»; **developer-disclosure `.error-details` СКРЫТ** (message/path пусты на плоском 404 → инфа не течёт); **hasStackTrace=false**; recovery-nav [На главную→/, Аналитика→/stats, Настроить сессию→/settings].
- **403 live (fetch POST `/start` битый `_csrf` + `Accept: text/html`):** status 403, content-type text/html; error-code «403»; error-title «Доступ запрещён»; error-body «Похоже, защитный токен формы устарел — так бывает, если страница была открыта долго…» (= то, что видел R0.148 в браузере); **developer-disclosure ПОКАЗАН** (path=`/start`, HTTP-статус 403, Причина Forbidden, Сообщение Forbidden, Время) — минимальная инфа под `<details>`, **без stacktrace** (hasStackTrace=false); recovery-nav present.
- **Content-negotiation (корректная, не дефект):** тот же POST **без** `Accept: text/html` (fetch дефолт `*/*`) → **JSON** `{"timestamp","status":403,"error":"Forbidden","message":"Forbidden","path":"/start"}` (content-type application/json, 116 симв.). API-клиент получает JSON, браузер (Accept:text/html) — error.html. Spring content-negotiation работает как надо.
- **Прочие коды:** 400/401/5xx — тот же `th:with code` + вложенный th:text switch (400→«Некорректный запрос», 401→«Требуется авторизация», 5xx→«Внутренняя ошибка сервера»); структурно verified в шаблоне (единый code-driven бранч, как 404/403 live).
- **DoD-свип:** design-swap linear/swiss/editorial 0 overflow; zoom200% 0 h-overflow; DPR2 (1280) + DPR3 (375 mobile) live (DPR1 CSS-px-инвариант). **light 1280:** eyebrow orange `rgb(201,100,66)`, title near-black `rgb(20,20,19)`, primary-btn orange `rgb(217,119,87)` — AA на cream. **375-mobile:** error-code scaled 64px, **0 body-overflow**. error.html **не грузит JS** (includeMermaid/Chart=false, нет app.js) → console-clean by construction.
- **🎯 ФЛИП §7 ERROR Final ◐→✅** (Refactor/Regression уже ✅): **6-я полностью верифицированная поверхность** (после FOCUS/RESULT/SUMMARY/SETTINGS/STATS).
- **Дальше:** R0.157 — **SHELL** (`fragments/header.html`+`head.html`) прод-parity: desktop+mobile-nav (гамбургер `#mobile-menu`), theme-toggle, design-switcher, skip-link, back-to-top, no-JS. §7 SHELL Final ◐→✅. Затем OVERLAYS (kbd-help + mobile-menu live + фикс OVL-1) — последняя перед пересчётом §7-итога.

### R0.157 (2026-07-14) — parity-QA: SHELL (`fragments/header.html`+`head.html`) прод-parity → §7 SHELL Final ◐→✅ (7-я поверхность)

- **Единица тика (§16):** SHELL-оболочка прод-parity live (masthead/nav/3 тоггла/skip-link/back-to-top/no-JS/persistence-API) → Final ◐→✅. Первый тик под новым operating-контрактом (§2b): 2+ раунда, дефекты только записываем (фиксы отдельной волной).
- **§0 health:** app 200, postgres `Up` :5432, граф готов (graph.json+GRAPH_REPORT.md).
- **🔧 КОРРЕКЦИЯ ФАКТА (важно):** **гамбургера/`#mobile-menu` в проде НЕТ вообще** — `grep -rn "mobile-menu|hamburger|nav-toggle" resources/` = 0 совпадений. Прежнее допущение (summary + §7 SHELL/OVERLAYS «mobile-nav hamburger #mobile-menu») **ложное**. Мобильная навигация = responsive flex-wrap той же `.ed-nav` (Фокус/Аналитика/Настройки видны всегда, переносятся на свою строку). Drawer не нужен. §7 OVERLAYS-строка исправлена (убран несуществующий `#mobile-menu`).
- **Раунд 1 (desktop dark 1280, evaluate-структура):** `data-js`=set, theme=dark, design=editorial, layout=flow; masthead+brand «Подготовка к собеседованию»+kicker «Cheat · Sheet»; nav 3 линка (Фокус `is-active`+`aria-current=page`, Аналитика `/stats`, Настройки `/settings`); 3 тоггла (layout/design/theme) все revealed (JS снял `.hidden`), 44×44, aria-labels («сейчас: Поток/Editorial», theme aria-pressed=false); back-to-top present/invisible; sprite инжектится; progress present; все 5 `window.__{theme,design,layout,motion,fontScale}`=object; designList=11; **docW 1265<1280 (0 h-overflow)**. Скрин dark: masthead идеален.
- **Раунд 2 (тогглы — реальные `.click()`):** **theme** dark↔light флип + `aria-pressed` sync (light=true) + aria-label + **meta theme-color** (#262624↔#FAF9F5); **design** editorial→linear (корректный next) + label «сейчас: Linear» + meta #08090A; **layout** flow→split→grid цикл. Состояние сброшено. Persistence: явный `theme='dark'` в localStorage перебивает системный `colorScheme:light` (корректно — выбор > системы).
- **Раунд 3 (mobile 375 dark):** **0 h-overflow** (375=375); nav wrap = отдельная строка под brand (drawer не нужен, всегда видна); тогглы 40×40 (>24 WCAG 2.5.8 AA-минимум; десктоп 44); nav-линки h=40; ничего не выходит за вьюпорт (maxRight=375). Скрин: brand 2 строки + 3 тоггла справа + nav отдельной строкой.
- **skip-link (per-page, reveal-on-focus):** база `transform:translateY(-150%)` (top≈-54, matrix -66) → `:focus` → `translateY(0)` (top=12, matrix 0), `transition:transform` анимирован. С `await 280ms`: **revealed=true**, focused, **175×44** (44px min-height WCAG 2.5.5), href=`#main-content`. Прежнее ложное «becameVisible:false» — артефакт синхронного чтения до перехода (не дефект).
- **back-to-top (на длинной /stats, maxScroll 1624>порог 1350):** чистый замер (instant scrollTop, длинный await — smooth-scroll давал флаки-чтения): scrolled y=1610>порог → **isVisible=true, opacity=1**; ниже порога → скрыт; **клик → scroll 0 + фокус `#main-content`** + скрывается; motion-aware (`data-motion` off/on/auto + prefers-reduced-motion). На короткой странице (home desktop maxScroll 318<1350) корректно не появляется.
- **no-JS (SSR-HTML через curl — то, что видит браузер без скриптов):** ✅ noscript-alert «JavaScript отключён»; ✅ все 3 тоггла `class="…hidden…"` в SSR → без JS остаются скрыты (мёртвых контролов нет); ✅ back-to-top в SSR только строка внутри `<script>` (не реальный DOM-элемент) → без JS отсутствует; nav = обычные `<a href>` (работают без JS).
- **DoD-свип §19:** **#11** все 11 дизайнов — masthead overflow **«none»** (maxRight 1265<1280 у editorial/linear/swiss/…/instrument); **#12** zoom200% 0 h-overflow; **light 1280** скрин — cream #FAF9F5, dark serif-title, nav active, theme-toggle filled-dark (AA); **dark+light** оба visual-verified; **console-clean** (home 0 error/warn). masthead = design-agnostic base.css, все 11 дизайнов делят одну разметку.
- **Refactor/Regression:** уже ✅ (header.html/head.html в сессии не менялись; фрагменты design-agnostic, потребляют только токены). Код НЕ трогал (scope=аудит; дефектов не найдено).
- **🎯 ФЛИП §7 SHELL Final ◐→✅:** все применимые DoD (§19) закрыты — **7-я полностью верифицированная поверхность** (FOCUS/RESULT/SUMMARY/SETTINGS/STATS/ERROR/SHELL). Дефектов не найдено (2+ раунда, контракт §2b п.4).
- **Дальше:** **OVERLAYS** — прод-verify `#kbd-help-overlay` (scrim/focus-trap/Esc, гейт `.focus-question`) + **live-reverify OVL-1** (код путей Esc/Cancel в promptModal ИДЕНТИЧЕН — оба `settle(cancelValue)`→restore lastFocused; «Esc→body» вероятно артефакт замера chrome-devtools; сначала подтвердить живьём, фикс — отдельной волной по §2b). `#mobile-menu` из плана OVERLAYS убран (не существует). Затем FEEDBACK, COMPONENTS.

### R0.158 (2026-07-14) — parity-QA: OVERLAYS (`#kbd-help-overlay` + reset-confirm) live → §7 OVERLAYS Final ◐→✅ (8-я поверхность) + OVL-1 ОПРОВЕРГНУТ

- **Единица тика (§16):** OVERLAYS прод-parity live (kbd-help + reset-confirm) + окончательный вердикт по OVL-1 → Final ◐→✅.
- **§0 health:** app 200, postgres `Up 5h (healthy)`.
- **kbd-help live (focus-training, гейт `.focus-question`=present):** open через «?»-keydown → `role=dialog`, `aria-modal=true`, `aria-labelledby=kbd-help-title`, title «Горячие клавиши», `position:fixed z-index:100`, scrim `rgba(0,0,0,0.6)`, **фокус→closeBtn**, фон (masthead) `inert`+`aria-hidden=true`, 7 строк шорткатов. **Все 4 пути закрытия** восстанавливают фокус на trigger: Esc (→nav-link «Фокус»), closeBtn-клик (→«Аналитика»), «?»-toggle (→«Настройки»), backdrop-клик (→«Фокус»). Focus-trap на Tab (first↔last).
- **🟢 OVL-1 ОПРОВЕРГНУТ (был P3):** прошлый A/B «Esc→body vs Cancel→trigger» — **артефакт замера**, не баг. Причина: prompt-overlay открывался, пока `.danger-btn` лежала в панели вкладки Данные с `display:none` (активна была Сессия) → `dangerBtn.focus()` = no-op → `lastFocused=document.activeElement=body` → оба пути restore на body. **Честный тест (активировал вкладку Данные → panel `display:block` → `triggerFocusedBeforeOpen=true`):** verdict `SYMMETRIC-OK` — И Esc, И Cancel симметрично восстанавливают фокус на `.danger-btn`. Код промптмодалки корректен (Esc стр.410 и Cancel стр.407 → один `settle(cancelValue)` → restore lastFocused). **Деструктив не выполнялся** (formConfirmed=unset, overlay не остался). Фикс НЕ требуется.
- **🎯 ФЛИП §7 OVERLAYS Final ◐→✅:** 8-я поверхность. Осталось: FEEDBACK, COMPONENTS.

### R0.159 (2026-07-14) — parity-QA: FOCUS (`focus-training.html`) промоут-render + answered-state → §7 FOCUS Final ◐→✅ (9-я поверхность) + ревью #2 VERIFY-CLEAN

- **Единица тика (§16):** FOCUS прод-parity live — промоут Instrument в дефолт (render обе темы + узкая ширина) + answered-state (ревью #2: selected/correct/wrong/verdict). Закрывает FOCUS-поверхность (`/`, не покрыта явно R0.151–158) + резолюция ревью #2.
- **§0 health:** app 200 (:8080, чужой bootRun — read-only, не глушу); консоль **0 ошибок** до и после ответа (снятый design-контроллер ничего не уронил; mermaid + пост-разбор JS отработали чисто).
- **Промоут-render (computed на дефолте `/`, без localStorage-оверрайда):** `data-design=instrument`. Light: bg `oklch(0.985 0.003 262)` (холодный paper, НЕ тёплый крем), text `oklch(0.245 0.015 264)`, accent teal `oklch(0.470 0.115 205)` hue205. Dark: bg `oklch(0.190 0.012 264)`, accent teal осветлён `oklch(0.800 0.115 205)`. Шрифты резолвятся: hero=Newsreader, UI=Space Grotesk, код=JetBrains Mono. Визуально: full-border ряды опций (не карточки), буква в круге-счётчике (`::before`), teal-underline активного nav, streak+прогресс-мета, full-width edge-to-edge. **0 горизонтального overflow** на узкой ширине (`scrollWidth==clientWidth`); клав-подсказки скрыты на мобильном. Ограничение замера: окно не ужимается <~485px (OS-минимум) — истинный 375/320 на device-emulation при нужде.
- **🟢 Ревью #2 VERIFY-CLEAN (разрешено дизайном Instrument, фикс НЕ требуется):** live выбор→проверка на `/`. **selected ДО проверки = teal** (border `oklch(0.435 0.105 205)` + ring `oklch(0.55 0.11 205)` + cool-wash, hue205) — НЕ зелёный ⇒ ложного «верно» нет. После проверки: **option-correct = зелёный** (border `oklch(0.47 0.12 150)` + green-wash hue150, ✓, метка «ПРАВИЛЬНЫЙ ОТВЕТ»); **option-wrong = красный** (border `oklch(0.5 0.2 27)` + red-wash hue27, метка «ТВОЙ ВЫБОР (ОШИБКА)»); не-выбранные = нейтраль «НЕ ВЫБРАН». **Вердикт** = ✗-иконка + слово «Неверно» + красный (тройное кодирование). **CTA «Проверить ответ» → «Следующий вопрос»** (teal-fill) — ровно как требует ревью #2. Итог: три состояния hue-разведены (teal205 / green150 / red27) + multi-coded (цвет+иконка+текст-метка); «выбран» никогда не читается как «верно». Пост-разбор (mermaid-дерево + «Что на самом деле / Откуда путаница / Если бы правдой / Как было бы правильно») + «Похожие вопросы» рендерятся чисто.
- **Бонус-находка:** динамический диапазон клав-подсказок «1–N» УЖЕ присутствует в `aria-description` submit-кнопки («Клавиши 1–4 — выбор, Enter — ответить»); статичен лишь ВИДИМЫЙ текст-хинт (`1–9 — выбор`). Ревью-adjacent Ф2-item (не регресс, не ложная информация — aria корректна) — в fix-волне синхронизировать видимый хинт с числом опций.
- **🎯 ФЛИП §7 FOCUS Final ◐→✅** (промоут-render + answered-state; 9-я поверхность). Осталось по поверхностям: FEEDBACK, COMPONENTS. Ревью #2 → VERIFY-CLEAN.

### R0.160 (2026-07-15) — parity-QA: FEEDBACK (alerts/status/validation) live прод-parity → §7 FEEDBACK Final ◐→✅ (10-я поверхность) + находка FB-1

- **Единица тика (§16):** FEEDBACK прод-parity live на промоут-Instrument (порядок плана SHELL→OVERLAYS→FEEDBACK→COMPONENTS). Реально достижимые state-family: verdict(success/error), assertive/polite aria-live, validation, warning-токен.
- **§0 health:** app 200 (:8080, PID 43378 — чужой Spring Boot, IntelliJ рядом; read-only, не глушу — Ф1-чистка 2/2 остаётся gated wedge). Консоль 0 ошибок.
- **verdict success ✓ (`/`, верный ответ):** зелёная карточка `.result-feedback[role=status][aria-live=polite][aria-atomic=true]` — ✓-иконка + слово «Верно» (Newsreader, зелёный hue150) + green-wash + border; богатый разбор (таблица сложности с mono-чипами) + confidence-группа «Угадал/С трудом/Знал точно». Дополняет error-вердикт R0.159 (красный ✗ «Неверно» hue27). Оба тройного кодирования (цвет+иконка+текст).
- **aria-live каналы ✓:** **assertive** `#interview-alert[role=alert][aria-live=assertive]` (класс `inline-alert hidden` — канал ошибок CSRF/сеть, пуст когда чисто); **polite** `#result-feedback` (вердикт, atomic) + `#extra-analysis-content` (related-sink) + settings-статусы `#filter-mode-hint`/`#font-scale-value`/`#personalization-status`(hidden)/`#export-status`(hidden). Разделение assertive(ошибки)/polite(вердикт/подсказки) корректно.
- **validation (settings `input[name=count]` min1 max200):** нативный Constraint Validation API активен — `rangeOverflow`/`rangeUnderflow` верны, русские сообщения («Значение должно быть меньше или равно 200.» / «…больше или равно 1.»); функционально+a11y ок (нативный bubble при сабмите). **🟡 FB-1 (FE-FIX, фикс-волна):** нет inline `:invalid`/`:user-invalid`-визуала — `base.css` вообще без `:invalid`/`aria-invalid`-правил, count-input голый `<input type=number min max value>` без хука; border неизменен на невалидном (`oklch(0.876 0.007 262)`). Атлас `feedback-state-atlas.html` показывает validation-error стиль → прод не завёл. Не слом (нативная валидация держит), полировка: добавить `:user-invalid`{border/wash error} + опц. inline-сообщение.
- **warning ✓ (design-gap ЗАКРЫТ Instrument):** прошлая заметка §7 «нет warning-токена → Фаза G» устарела — Instrument определяет `--color-status-warning` (light `oklch(0.500 0.120 62)` amber/spark-ink, dark `oklch(0.820 0.120 75)`), разведён с success150/error27/signal205. **Используется** в `.result-penalty-note` (EXAM penalty-нотис `appendPenaltyNotice`, amber semibold) — живой warning-компонент.
- **Residual (не триггерились живьём, CSS присутствует):** pending/loading (скелет/спиннер), toast (`--z-toast` есть), offline/permission — не достижимы без спец-состояния; на COMPONENTS-тик или при естественном появлении.
- **🎯 ФЛИП §7 FEEDBACK Final ◐→✅** (10-я поверхность; core verdict/aria-live/warning прод-verified, FB-1 + pending/toast задокументированы). Осталось: **COMPONENTS**.

### R0.161 (2026-07-15) — parity-QA: COMPONENTS (UI-примитивы, все состояния) live → §7 COMPONENTS Final ◐→✅ (11-я, ПОСЛЕДНЯЯ поверхность) + находки CMP-1/CACHE-1

- **Единица тика (§16):** COMPONENTS прод-parity live (последняя поверхность порядка SHELL→OVERLAYS→FEEDBACK→**COMPONENTS**). State-family: default/hover/active/focus/disabled/selected/checked/expanded/dark/long-content + ревью #11 (`:focus-visible` ≠ selected).
- **§0 health:** app 200 (:8080, PID 43378 чужой Spring Boot, read-only). Консоль 0 ошибок.
- **focus-visible система ✓ (ревью #11 → VERIFY-CLEAN):** единый teal outline 2px offset 2px на радио И кнопках (`oklch(0.55 0.11 205)`). Механически ОТЛИЧАЕТСЯ от selected: selected-опция = border teal 1px + wash `oklch(0.998 0.002 262)` + inset box-shadow; focus = отдельный offset-outline снаружи. Общий hue (teal205), но разная ФОРМА (outline-ring vs fill+border) — не путаются. Primary-кнопка active/enabled = teal-fill `oklch(0.47 0.115 205)` + near-white текст.
- **checked/tab ✓ (live /settings):** `.seg-btn.is-active` = teal-fill + white + weight500 + `aria-checked=true` vs inactive transparent+muted+weight400 (**distinct**); tablist active = teal border-bottom + ink + `aria-selected=true` vs inactive transparent+muted (**distinct**). Multi-coded (fill+weight+aria).
- **прод-покрытие state-правил (base.css grep):** `:hover` 58, `:active` 20, `:focus-visible` 17, `:disabled` 23; checked/expanded реализованы через класс `.is-active` (seg-btn/nav-link/settings-panel/disclosure — 0 `[aria-checked]`-селекторов, но визуал есть); reduced-motion уважается (`@media prefers-reduced-motion` + атрибут). long-content (перенос опций/чипов) + dark — подтверждены R0.159.
- **🔴 CMP-1 (cleanup-2, USER-VISIBLE):** ось «дизайн» в /settings всё ещё перечисляет ВСЕ 11 дизайнов (Editorial, Linear, Swiss, Notion, Mintlify, Broadsheet, Superhuman, Stripe, Claude, The Verge, Instrument). Header-toggle уже нейтрализован (`Сменить дизайн (сейчас: Instrument)` → Instrument→Instrument no-op, `DESIGNS=['instrument']`), но settings-seg живой. После cleanup-1 у 10 не-Instrument дизайнов НЕТ token-блоков → на свежем клиенте выбор откатывает на `:root` (editorial-инерт). Лечит Ф1-чистка 2/2 (удалить `data-design-pref` ось из settings.html + `#design-toggle` из header.html + ассерты `TemplateFragmentContractTest`), **заблокирована live bootRun** (contract-test wedge, не мой app).
- **🔴 CACHE-1 (CONFIRMED live-fetch; КОРРЕКТИРУЕТ R0.159):** cleanup-1 (`d7f69f63`) удалил 9 дизайн-блоков из tokens.css, но **НЕ бампнул `?v=64`**. Браузер-fetch `/css/tokens.css?v=64` = **1537 строк** (СТАРЫЙ, с linear/#5E6AD2); fetch с иным query = **467 строк** (чистый, только instrument); curl (uncached) = чистый. ⇒ вернувшиеся клиенты с HTTP-кэшем `?v=64` продолжают получать старый all-designs файл — промоут им НЕвидим, старые дизайны у них полностью рендерятся. Рассуждение R0.159 «?v не бампался (рендер идентичен)» **ОШИБОЧНО**: дефолт-рендер идентичен лишь для СВЕЖИХ клиентов, но контент файла изменился ⇒ **?v bump ОБЯЗАТЕЛЕН** (уже в скоупе cleanup-2 «bump ?v= tokens+base», теперь = must-fix, не optional).
- **🎯 ФЛИП §7 COMPONENTS Final ◐→✅** (11-я, ПОСЛЕДНЯЯ поверхность; state-rendering прод-verified). **✅ ВСЕ поверхности parity-QA закрыты** (SHELL/OVERLAYS/FEEDBACK/COMPONENTS + B-поверхности + FOCUS). CMP-1/CACHE-1 — не дефекты рендеринга компонентов, а cleanup-2-скоуп. **Критический разблокировщик: остановка bootRun на 1 тик** (тогда cleanup-2: удалить design-ось+toggle, bump ?v tokens+base, prune fonts, обновить contract-test). Остаток после: фикс-волна (#3 mobile, FB-1 validation-visual) + Ф2 DOM-реструктуризация + Ф4 бэкенд.

### R0.162 (2026-07-15) — истинный mobile-375 parity-QA (device-emulation) → re-scope mobile fix-волны (#3–7, #9)

- **Единица тика (§16):** истинный mobile-375 (CDP `emulate viewport 375x812x3,mobile,touch` — обходит OS-минимум окна ~485px, прошлый пробел) на промоут-Instrument (`/`, algorithms-вопрос с кодом). Цель — сверить конкретные mobile-замечания ревью #3–7/#9 с реальностью, чтобы фикс-волна была точной, а не по слепку.
- **§0 health:** app 200 (:8080 чужой, read-only). bootRun ещё жив → cleanup-2 всё ещё blocked, этот тик unblocked read-only.
- **overflow: 0** (`docScrollW==clientWidth==375`, overflowers=[]) — на истинном 375 горизонтали нет. Подтверждает прошлый вывод на 485.
- **#5 (шапка высокая) → ПОДТВЕРЖДЕНО (DECISION):** masthead **144px** + nav-ряд (бренд 2 строки + тогглы + фокус/аналитика/настройки). Ревью хочет 56–64px — реально высокая на моб. Осознанное решение по компактизации (лого/кнопки/ряд) за пользователем.
- **#7 (заголовок большой) → НЕ проблема (downgrade):** title 27px / lh 31 (Newsreader), 3 строки. Ревью предлагал clamp 34–40 моб — текущий даже МЕНЬШЕ. Instrument уже не-oversized; правки не требует (если что — чуть увеличить, не уменьшить).
- **#3 (кнопка между кодом/вариантами; порядок) → порядок ВЕРНЫЙ, sticky нет:** submit `position:static`, top≈1632 (ПОСЛЕ всех 4 опций, не между код↔варианты). DOM-порядок код→варианты→CTA корректен ⇒ «нарушен порядок» не воспроизводится. Реальный остаток — **sticky-снизу** (на длинном вопросе CTA далеко внизу) + padding-bottom. Sticky = base.css-only enhancement (без DOM-реордера).
- **#4 (код обрезается) → скроллится, нет affordance (минор):** развёрнутый `.question-code-details` — `white-space:pre`, code `scrollW 357 > clientW 325` (~32px h-overflow, скроллится внутренне, страница не переполняется). **Нет** правого градиента (`maskImage:none`, `bgImage:none`) и **нет** «Развернуть код». Минорная полировка — добавить mask-gradient-справа как аффорданс скролла.
- **#9 (карточка теряет ширину) → в основном OK (минор):** опция w=327 (375−24×2), padding 12px, буква-счётчик в круге слева + текст. Полной ширины не теряет; читаемо. Возможна лёгкая доводка h-padding/gap, но не «разваливается».
- **#6 (прогресс разваливается) → требует session-режим:** TRAINING без сессии не даёт «Вопрос N из M · серия» (только 2px-бар top≈141). Проверить на EXAM/MARATHON-тик.
- **🎯 Re-scope mobile fix-волны:** РЕАЛЬНЫЕ — #5 (компактный header, DECISION), #3-sticky (CSS-only), #4-affordance (минор), #6 (session-mode проверить). НЕ-проблема — #7 (downgrade). Минор/OK — #9. Приоритет фикс-тиков уточнён: **#5 (DECISION юзера) / #3-sticky (base.css-only, unblocked) — первыми**. #3-sticky и #4-affordance выполнимы БЕЗ остановки bootRun (base.css-only).

### R0.163 (2026-07-15) — FB-1 FIX: inline `:user-invalid`-визуал для count-инпута (первый код-фикс фикс-волны)

- **Единица тика (§16, ITERATE_EXISTING):** первый РЕАЛЬНЫЙ код-фикс волны (прежние R0.151–162 — read-only parity-QA). Выбор worst-first среди РАЗБЛОКИРОВАННЫХ (base.css-only, bootRun жив): FB-1 — чистая аддитивная порт-парити правка из feedback-атласа, без DOM/design-решений. (Изначально целил #3-sticky, но `position:sticky;bottom:0` на training-actions НЕ пиннится — это последний элемент формы с только hidden-сиблингами после; нужен `fixed`+padding+`:has()` — design-decision-laden, отложено.)
- **Проблема (FB-1, найден R0.160):** нативная валидация count-инпута (`min=1/max=200`) держала лишь браузерный bubble при сабмите; постоянного inline-error-состояния, заданного в атласе, в проде НЕ было. `base.css` имел 0 правил `:invalid`/`:user-invalid`/`aria-invalid`.
- **Фикс (base.css, после number-input `:focus`):** `html[data-design] .settings-page input[type="number"]:user-invalid` → `border-color:--color-status-error` + `background:--color-status-error-wash`; `:user-invalid:focus` → error-ринг перебивает `--shadow-focus` (специфичность (0,5,2) > (0,4,2)). `:user-invalid` красит ТОЛЬКО после взаимодействия (нетронутое value=20 не «грязнится»). error-тон разведён с signal (hue27 vs teal205). `?v` base.css 95→96 (head.html; tokens.css НЕ трогаю — иначе усугубил бы CMP-1). Контракт-тест version-consistency пройдёт (base.css single-sourced, set остаётся размера 1).
- **Live-верификация (chrome-devtools, cp source→build):** режим Экзамен раскрывает `#session-count-field` (в TRAINING скрыт `.hidden`) → count-спинбаттон `min=1/max=200`. Реальный CDP-ввод «999» (rangeOverflow) + Tab: `matchesUserInvalid=true`; **light** border `oklch(0.5 0.2 27)`=error, bg `oklch(0.95 0.045 27)`=error-wash (скрин: красная рамка+розовый фон); **dark** (тема-парити) border `oklch(0.72 0.175 27)`, bg `oklch(0.3 0.07 27)` — оба резолвят корректные dark-error-токены. Возврат к валидному «20»: `matchesUserInvalid=false`, border→`--color-border-primary` (0.36 0.015 264), bg→`--color-bg-secondary` — состояние снимается чисто, не залипает. **0 console-ошибок**. Отдаётся при `?v=96` (curl подтвердил user-invalid в served CSS).
- **Итог:** FB-1 ✅ ЗАКРЫТ. Фикс-волна начата. Остаток разблокированного base.css-only: #3-sticky (нужен `fixed`, не sticky — отложено на design-decision), #4-affordance (mask-градиент кода). cleanup-2 (CMP-1/CACHE-1) по-прежнему blocked живым bootRun.

### R0.164 (2026-07-15) — #14 (частично): динамический диапазон клав-подсказки «1–N» (server-side)

- **Единица тика (§16, ITERATE_EXISTING):** worst-first среди разблокированного — #14 sub-point «динамич. диапазон». Диагностика FOCUS показала конкретный дефект: видимая `.keyboard-hint` статична **«1–9 — выбор»** при 4 вариантах (клавиши 5–9 — no-op, активно вводит в заблуждение), тогда как `title` кнопки app.js уже правил динамически на «1–4» (`app.js:1084` `Math.min(optionInputs.length, 9)`) — рассинхрон двух источников. (#12 подсветка: на этом вопросе «код» = mermaid `graph TD`, hljs его закономерно не красит; mermaid-рендер вне скоупа — [[feedback_diagrams_non_mermaid_ok]]. #4 affordance: на mermaid-контенте overflow=0, минор — отложено.)
- **Источник дефекта:** `focus-training.html:167–168` передавал в `training-actions` СТАТИЧЕСКИЕ `submitTitle='Клавиши 1–9 …'` + `hintText='1–9 …'`. Комментарий `training-actions.html:17` («app.js к ней не обращается») закреплял статику.
- **Фикс (focus-training.html, template-only):** оба параметра теперь считаются **server-side** из реального числа вариантов: `${'Клавиши 1–' + (current.options.size() > 9 ? 9 : current.options.size()) + …}` (кап 9 = число цифровых горячих клавиш). Инлайн-выражение, НЕ `th:with` — на одном элементе `th:replace` (приоритет 100) исполняется раньше `th:with` (600) → локальная переменная была бы пуста. Server-side → нет клиентского мелькания «1–9»→«1–4», комментарий про app.js остаётся правдивым, no-JS SSR-фоллбэк корректен. app.js:1084 (тот же расчёт для title) оставлен — согласуется, безвреден. Bump `?v` НЕ нужен (нет CSS/JS-изменений).
- **Live-верификация (chrome-devtools, cp template→build, reload):** 4 опции → видимая подсказка **«1–4 — выбор, ↑/↓ — навигация, Esc — снять, Enter — отправка»** + title кнопки **«Клавиши 1–4 …»** (DOM + скрин dark-тема). SpEL отработал без ошибок (страница 200, **0 console-ошибок** — Thymeleaf-выражение валидно). Диапазон theme-независим (текст, не цвет).
- **Итог:** #14 sub-point «динамич. диапазон» ✅ ЗАКРЫТ. Остаток #14 (типографика — «крупнее/контрастнее», `hintFontSize:12px` муторный gray; переформулировка + «Enter далее» после ответа) — отдельная под-единица, base.css/copy, будущий тик.

### UX-РЕВЬЮ 2026-07-14 (внешнее, 20 пунктов) — приоритизированный fix-бэклог + триаж

Пользователь прислал детальный внешний UX-ревью. **Важно:** часть находок ре-открывает поверхности, помеченные «Final ✅ VERIFIED» — мой parity-QA бар проверял механику (overflow/a11y/функции), но не дизайн-качество/UX-полировку, которые ловит это ревью. Ревью = фактически ранние deep-pass находки; становится основой fix-волны (контракт §2b: фиксы отдельной волной, в порядке приоритета пользователя).

**Триаж (FE-FIX = фронт-фикс, я владею · MCQ = домен параллельной сессии · DECISION = осознанное решение/конфликт · VERIFY = сначала live-проверить):**

| # | Область | Поверхность | Тип | Заметка |
|---|---|---|---|---|
| 1 | Неверный правильный ответ (Spring `REQUIRES_NEW` self-invocation → A, не B) | MCQ-контент | **MCQ — РЕШЕНО: оставить MCQ-сессии** | Решение пользователя 2026-07-14: я seed/mcq НЕ трогаю; находка передана параллельной MCQ-сессии (она и так выверяет correctness). Из fix-волны фронта ИСКЛЮЧЕНО. По сути верно: self-invocation минует Spring-прокси → `REQUIRES_NEW` не применяется |
| 2 | Галочка на выбранном ДО проверки = ложное «верно» | FOCUS | **РЕШЕНО: VERIFY-CLEAN (R0.159)** | Live-проверено 2026-07-14: уже разведено дизайном Instrument. selected=teal (hue205, не зелёный) → correct=зелёный+✓+«ПРАВИЛЬНЫЙ ОТВЕТ» (hue150) → wrong=красный+«ТВОЙ ВЫБОР (ОШИБКА)» (hue27); вердикт=✗+«Неверно»+красный; CTA «Проверить»→«Следующий вопрос». Multi-coded (цвет+иконка+текст), ложного «верно» на выборе нет. Фикс НЕ требуется |
| 3 | Mobile: кнопка между кодом и вариантами (нарушен порядок) | FOCUS | **FE-FIX (уточнён R0.162)** | ~~порядок нарушен~~ — на 375 submit `static` top≈1632 ПОСЛЕ опций (порядок код→варианты→CTA ВЕРНЫЙ). Остаток = **sticky-снизу + padding-bottom** (base.css-only, unblocked). DOM-реордер НЕ нужен |
| 4 | Mobile: код обрезается | FOCUS | **FE-FIX минор (R0.162)** | на 375 код `white-space:pre` скроллится (scrollW357>325, страница не переполняется); нет affordance — добавить **градиент-справа** (mask). «Развернуть код» опц. base.css-only |
| 5 | Mobile: шапка слишком высокая | SHELL | **FE-FIX/DECISION (подтв. R0.162)** | на 375 masthead **144px**+nav (бренд 2 строки+тогглы+ряд nav). Ревью 56–64px, 40–44px кнопки, лого меньше. Осознанное решение по компактизации за пользователем |
| 6 | Mobile: блок прогресса разваливается | FOCUS | **FE-FIX (session-gated R0.162)** | TRAINING без сессии не даёт «N из M» — проверить на EXAM/MARATHON-тик; цельная структура «Вопрос N из M · серия» + прогресс-бар |
| 7 | Mobile: заголовок вопроса слишком большой | FOCUS | **НЕ проблема (downgrade R0.162)** | на 375 title 27px/lh31 (Newsreader, 3 строки) — Instrument уже НЕ oversized (ревью предлагал 34–40, текущий меньше). Правки не требует |
| 8 | Широкие экраны 1920–2560: UI мелкий «остров» | FOCUS/layout | **РЕШЕНО: WON'T-DO** | Решение пользователя 2026-07-14: **оставить full-width** (осознанное решение «prose full-width, 259ch на 2560» в силе). #8 из fix-волны ИСКЛЮЧЁН. Не менять max-width на широких |
| 9 | Mobile: карточка ответа теряет ширину (padding/буква/radio) | FOCUS/COMPONENTS | **минор/OK (R0.162)** | на 375 опция w=327 (full−24×2), padding 12px, буква-круг+текст читаемы — полной ширины НЕ теряет. Возможна лёгкая доводка gap/h-padding, но не «разваливается» |
| 10 | Карточки выглядят как disabled (светлая тема) | FOCUS/tokens | **FE-FIX/DECISION** | разделить поверхности: страница серая / карточка белая / hover / selected бирюза / disabled отдельно |
| 11 | Нужны hover/focus/pressed состояния | COMPONENTS | **РЕШЕНО: VERIFY-CLEAN (R0.161)** | Live-проверено: :hover 58/:active 20/:focus-visible 17/:disabled 23 правил в base.css; `:focus-visible` = teal outline 2px offset (радио+кнопки) ≠ selected (border+wash+inset) — механически разные. Фикс НЕ требуется |
| 12 | Код почти без синтаксической подсветки | FOCUS/head.html | **VERIFY/FE-FIX** | highlight.js загружается (head.html) — проверить, красится ли пример-код; 4–5 оттенков |
| 13 | Неясно, сворачивается ли «Код к вопросу» | FOCUS | **FE-FIX** | вся строка кликабельна + hover + поворот стрелки + `aria-expanded`; или убрать стрелку |
| 14 | Клавиатурные подсказки мелкие/блеклые | FOCUS | **FE-FIX (частично R0.164)** | ✅ динамич. диапазон «1–N» server-side (R0.164): 4 опции→«1–4» и в подсказке, и в title кнопки. Остаток: крупнее/контрастнее (12px муторный gray), переформулировка, после проверки «Enter далее» |
| 15 | CTA оторвана от подсказок | FOCUS | **FE-FIX** | единый footer: [Проверить] + подсказки; на узком — подсказки под кнопкой |
| 16 | «К итогам сессии» абстрактно | FOCUS/finish | **FE-FIX** | «Разобрать N ошибок» / «Посмотреть результаты» |
| 17 | Экрану завершения не хватает сводки | SUMMARY/finish | **VERIFY** | /session-summary уже имеет %/ошибки/время — проверить, о finish-CTA ли речь |
| 18 | Desktop: блок завершения высоко/мелкий | SUMMARY | **FE-FIX** | центрировать по высоте после шапки (~35–40%) |
| 19 | Пустое состояние: формулировка неестественна | FOCUS-empty | **FE-FIX** | «Для выбранных тем и режима вопросов не найдено» + заголовок «Ничего не найдено» |
| 20 | «Начать новую тренировку» → снова пусто (цикл) | FOCUS-empty | **FE-FIX/DECISION** | «Изменить фильтры»(primary)/«Сбросить фильтры»(secondary) + чипы выбранных фильтров |
| A11y | контраст серого/границ dark, вкладка Фокус light, aria-label меню/тема, клавиатура карточек, focus-visible, zoom200, reduced-motion, вся карточка кликабельна | все | **VERIFY/FE-FIX** | сводный a11y-проход; часть уже verified (zoom200/reduced-motion), контраст-серого перепроверить |

**Порядок исполнения (по приоритету пользователя; #1 и #8 ИСКЛЮЧЕНЫ решением 2026-07-14; #2 РЕШЁН VERIFY-CLEAN R0.159):** ~~2(состояния ответа)~~ → 3(mobile-кнопка) → 4(обрезка кода) → 5/6/7(mobile прогресс/шапка/заголовок) → 9/10(карточки) → 13(disclosure) → 14/15(клавиатура+CTA) → тексты 16/19/20 → a11y+состояния 11/12/17/18. Волна фиксов пойдёт по тикам, каждый фикс — live-verify + отдельный коммит (§2b). Быстрые текст-вины (19/16) можно раньше. **Первый фикс-тик: #3 (mobile: sticky-кнопка «Проверить» снизу + порядок код→варианты→CTA)** — самый высокий валидный приоритет после резолюции #2. Замечание: mobile-фиксы (#3–7,#9) верифицируются на истинном 375/320 через device-emulation (окно браузера не ужимается <~485px).
