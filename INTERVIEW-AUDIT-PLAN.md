...
|---|---|---|---|
| `NO_INLINE` | 211 | inline MCQ нет (уже мигрированы в JSON или их не было) | проверить наличие/качество JSON-сидера |
| `CLEAN 1:1` | 11 | каждый `## Q<N>` ↔ ровно 1 `[!mcq]` | **прямая миграция** MCQ→JSON по q_number, затем strip inline |
| `MULTI` | 31 | есть структура `## Q`, но MCQ/вопрос ≠ 1 | разбор по блокам, маппинг block_idx, затем strip |
| `COLLAPSED` | 41 | `nq≤2`, а `[!mcq]` 22–42 — тело схлопнуто, MCQ не привязаны | **сначала восстановить `## Q<N>`-структуру** (`/interview-writer`), потом миграция |
