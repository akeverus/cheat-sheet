...
### ↩️ Reverted — aria-controls стоит оставить `result-feedback` (самокоррекция)
- Сначала я поменял `#extra-analysis-toggle` `aria-controls` с `result-feedback` на `details`, решив (по устаревшему чтению про `toggleDetails`/`/details`), что кнопка раскрывает `#details`. **Перепроверка кода (`app.js:1490+`) опровергла это:** кнопка грузит takeaway / code-trace / related и вставляет их через `feedbackDiv.after(...)` — то есть **соседними блоками сразу после `#result-feedback`**. `#details` — мёртвый пустой div, никуда не рендерится. Значит исходный `aria-controls="result-feedback"` был корректнее (якорь ровно перед вставляемым контентом), а мой `details` указывал в пустоту. **Откатил обратно на `result-feedback`.** Урок: не доверять старому чтению большого файла — перечитывать фактический путь рендера. (Соседние disclosure — `toggle-code-btn`→`#codeBlock`, `browse-reveal-btn`→`#browse-answer` — корректны.)
