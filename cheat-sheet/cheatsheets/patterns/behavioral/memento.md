...
- [Java Serializable](https://docs.oracle.com/javase/8/docs/api/java/io/Serializable.html)
- [Java Cloneable](https://docs.oracle.com/javase/8/docs/api/java/lang/Cloneable.html)

### См. также
- `../../java/java-collections.md` - Java Collections
- `../../patterns/behavioral/command.md` - Command Pattern
- `../../patterns/behavioral/state.md` - State Pattern
...
│  │                                                   │    │    │
│  │  // Создает memento содержащий snapshot своего     │    │    │
│  │  // текущего внутреннего состояния                │    │    │
│  │  // Использует memento для восстановления состояния │    │    │
│  └─────────────────────────────────────────────────┘    │    │
└─────────────────────────────────────────────────────────────┼─┘
                                                              │
┌─────────────────────────────────────────────────────────────┼─┐
...
│  │           state: State                              │    │ │
│  │                                                     │    │ │
│  │  // Хранит внутреннее состояние Originator'а         │    │ │
│  │  // Защищает от доступа внешних объектов            │    │ │
│  │  // Для Caretaker'а memento - непрозрачный объект   │    │ │
│  └─────────────────────────────────────────────────────┘    │ │
└─────────────────────────────────────────────────────────────┘
                                                              │
┌─────────────────────────────────────────────────────────────┼─┐
...
│  │  └─────────────────────────────────────────────────┘ │    │ │
│  │                                                     │    │ │
│  │  // Отвечает за хранение memento                    │    │ │
│  │  // Не работает с содержимым memento                │    │ │
│  │  // Не знает о внутреннем состоянии Originator      │ │ │
│  └─────────────────────────────────────────────────────┘    │ │
└─────────────────────────────────────────────────────────────┘
                                                              │
┌─────────────────────────────────────────────────────────────┼─┐
...
│                                                             │ │
│  caretaker.addMemento(originator.save())  // Сохранить     │ │
│  originator.doSomething()                 // Изменить       │ │
│  originator.restore(caretaker.getMemento()) // Восстановить │ │
│                                                             │ │
└─────────────────────────────────────────────────────────────┘
```
...
Memento паттерн — мощный инструмент для реализации механизмов сохранения и восстановления состояния объектов. Он обеспечивает безопасное хранение состояния без нарушения инкапсуляции и позволяет реализовать сложную функциональность undo/redo.
