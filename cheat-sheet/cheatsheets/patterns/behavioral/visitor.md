...
### См. также
- `../../java/java-annotations-reflection.md` - Аннотации и рефлексия
- `../../patterns/behavioral/strategy.md` - Strategy Pattern
- `../../algorithms/data-structures.md` - Data Structures
...
```java
// Visitor с Spring AOP для логирования и транзакций
@Component
...
// AOP аспект для visitor паттерна
@Aspect
@Component
public class VisitorAspect {

    @Around("execution(* com.example.ElementVisitor+.visit(..))")
    public Object logVisitorExecution(ProceedingJoinPoint joinPoint) throws Throwable {
        String visitorName = joinPoint.getTarget().getClass().getSimpleName();
        String elementName = joinPoint.getArgs()[0].getClass().getSimpleName();

        long startTime = System.nanoTime();
        System.out.println("Visitor " + visitorName + " visiting " + elementName);

        try {
            Object result = joinPoint.proceed();

            long duration = (System.nanoTime() - startTime) / 1_000_000;
            System.out.println("Visitor " + visitorName + " completed in " + duration + "ms");

            return result;
        } catch (Exception e) {
            System.err.println("Visitor " + visitorName + " failed on " + elementName + ": " + e.getMessage());
            throw e;
        }
    }
}
...
// Document elements
abstract class AbstractDocumentElement implements DocumentElement {
    protected String content;

    public AbstractDocumentElement(String content) {
...
@Component
class ParagraphElement extends AbstractDocumentElement {
    public ParagraphElement() {
        super("Sample paragraph text");
    }
...
@Component
class HeaderElement extends AbstractDocumentElement {
    private int level;
...
@Component
class ImageElement extends AbstractDocumentElement {
    private String src;
...
// Visitors
@Component
class HTMLRenderVisitor implements ElementVisitor {
    ...
    public void visit(DocumentElement element) {
        if (element instanceof ParagraphElement) {
            ...
            HeaderElement header = (HeaderElement) element;
            ...
            html.append("<img src=\"").append(image.getSrc())
                .append("\" alt=\"").append(element.getContent()).append("\"/>\n");
        }
    }
...
@Component
class WordCountVisitor implements ElementVisitor {

    private int wordCount = 0;

    @Override
    public void visit(DocumentElement element) {
        String[] words = element.getContent().split("\\s+");
...
@Component
@Scope("prototype")
class ValidationVisitor implements ElementVisitor {

    private List<String> errors = new ArrayList<>();

    @Override
    public void visit(DocumentElement element) {
        if (element.getContent() == null || element.getContent().trim().isEmpty()) {
            errors.add("Empty content in " + element.getClass().getSimpleName());
        }

        if (element instanceof HeaderElement) {
            HeaderElement header = (HeaderElement) element;
            if (header.getLevel() < 1 || header.getLevel() > 6) {
                errors.add("Invalid header level: " + header.getLevel());
            }
        }

        if (element instanceof ImageElement) {
            ImageElement image = (ImageElement) element;
            if (image.getSrc() == null || !image.getSrc().matches(".*\\.(jpg|png|gif)$")) {
                errors.add("Invalid image source: " + image.getSrc());
            }
        }
    }
...
public class DocumentProcessor {
    ...
    @Autowired
    private HTMLRenderVisitor htmlRenderer;

    @Autowired
    private WordCountVisitor wordCounter;

    @Autowired
    private Provider<ValidationVisitor> validationVisitorProvider;
    ...
    public ValidationResult validate() {
        ValidationVisitor validator = validationVisitorProvider.get();
        for (DocumentElement element : elements) {
...
public class VisitorConfig {
    ...
    public List<DocumentElement> documentElements(
            ...
            ImageElement image) {
        return Arrays.asList(paragraph, header, image);
    }
}
...
public class SpringVisitorDemo {
    ...
    public void demonstrate() {
        // Валидация документа
        DocumentProcessor.ValidationResult validation = processor.validate();
        ...
        // Рендеринг HTML
        String html = processor.renderHTML();
...
Visitor паттерн — мощный инструмент для работы с древовидными структурами данных и добавления новых операций без изменения существующих классов. Он обеспечивает двойную диспетчеризацию и позволяет четко разделить структуру данных от операций над ней.
