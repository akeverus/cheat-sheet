...
Это запустит приложение в Docker, доступ по `localhost:8887/messages`. Важно сопоставить порт на хосте (8887) с портом внутри Docker (8888), указанным в приложении.
...
JAR в `target` с именем `docker-spring-boot-postgres-0.0.1-SNAPSHOT.jar`. Копируем в `src/main/docker`:

```bash
cp target/docker-spring-boot-postgres-0.0.1-SNAPSHOT.jar src/main/docker
```

Dockerfile в `src/main/docker`:
...
Compose файл `docker-compose.yml` в `src/main/docker`:
...
- Подключение к `db:5432/compose-postgres`, креды compose-postgres, Hibernate ddl-auto=update.
...
Видим `BOOT-INF/layers.idx`, который сопоставляет зависимости, ресурсы и код приложения по слоям. Перечисление слоёв:
