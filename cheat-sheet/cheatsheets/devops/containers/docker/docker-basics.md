...
См. также: `./docker-containers.md`, `./docker-spring-boot.md`.
...
Печать `/etc/hosts` внутри контейнера:
...
- Используйте образы, поддерживаемые авторами ПО (пример `emqx/emqx:v4.1.3` с переменными окружения).
- Если нет официального — ищите референс (пример H2 от третьих лиц).
- Не всегда нужен свой образ: можно монтировать конфиг в готовый образ (`haproxy:2.2.2` + `-v my-config.cfg:/usr/local/etc/haproxy/haproxy.cfg:ro`).
