...
- [OkHttp Documentation](https://square.github.io/okhttp/) - Документация

### См. также
- `../libraries/java-apache-httpclient.md` - Apache HttpClient
- `../libraries/java-retrofit.md` - Retrofit (использует OkHttp)
...
OkHttpClient client = new OkHttpClient.Builder()
    .connectTimeout(10, TimeUnit.SECONDS)      // Таймаут установки TCP соединения (10 секунд)
...
OkHttpClient client = new OkHttpClient.Builder()
    .followRedirects(true)      // Следовать HTTP редиректам
    .followSslRedirects(true)   // Следовать HTTPS редиректам
...
Request request = new Request.Builder()
    .url("ws://echo.websocket.org")  // WebSocket URL (ws:// для незашифрованного, wss:// для SSL)
    .build();
...
webSocket.close(1000, "Goodbye");  // Закрываем соединение с кодом 1000 (нормальное закрытие) и причиной "Goodbye"
