...
### См. также
- `../libraries/java-junit5.md` - JUnit 5 для тестирования
- `../libraries/kotlin-mockk.md` - MockK для Kotlin
- `../testing/unit-testing/mockito.md` - Unit тестирование с Mockito
...
- [Advanced Mockito Features](#advanced-mockito-features)
...
assertEquals(100, spiedList.size()); // Вернет 100, а не 1
...
User capturedUser = userCaptor.getValue();  // Получаем захваченный User объект
...
List<User> capturedUsers = userCaptor.getAllValues();  // Получаем список всех захваченных User
...
Answer<User> userAnswer = invocation -> {
    ...
    user.setId(System.currentTimeMillis());  // Генерация уникального ID на основе времени
...
void testWithStrictMock() {
    ...
    // strictMock.get(0); // UnnecessaryStubbingException в Mockito 3+
...
// META-INF/services/org.mockito.plugins.MockMaker
...
void testConcurrentAccess() throws InterruptedException {
    ...
    when(userRepository.save(any(User.class))).then(invocation -> {
        ...
        user.setId(Thread.currentThread().getId()); // Разные ID для разных потоков
        return user;
    });
...
public class MockSpyGuidelines {
    ...
    void useSpyWhen() {
        ...
        assertTrue(spiedList.isEmpty()); // Возвращает true, хотя элемент добавлен
    }
...
public class MockitoTroubleshooting {
    ...
    void fixTooManyInvocations() {
        ...
        // verify(mockList, times(2)).add("test"); // TooManyActualInvocations
...
- [AssertJ](testing/assertj.md) - Fluent assertions
- [Testcontainers](java-testcontainers.md) - Интеграционные тесты
