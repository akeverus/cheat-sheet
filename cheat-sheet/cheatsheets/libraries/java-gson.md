...
- [Gson API](https://www.javadoc.io/doc/com.google.code.gson/gson/latest/index.html) - API документация

### См. также
- `../libraries/jackson.md` - Jackson для JSON обработки
- `../libraries/java-kotlinx-serialization.md` - Kotlinx Serialization
...
Gson gson = new GsonBuilder()
    ...
    .disableHtmlEscaping()                  // Отключить экранирование HTML
...
String json = gson.toJson(object);  // Быстрее, чем создавать новый Gson каждый раз
```
...
Gson gson = new GsonBuilder()
    .disableJdkUnsafe()  // Отключить использование sun.misc.Unsafe
    .create();
...
public class LenientGsonParser {
    ...
    public Gson createLenientGson() {
        return new GsonBuilder()
            .setLenient()  // Разрешить malformed JSON
            .create();
    }
...
// Использование
Gson gson = new GsonBuilder()
    .setVersion(1.2)  // Показывать поля до версии 1.2
    .create();
```
...
public class GsonPerformanceTips {
    ...
    public Gson createOptimizedGson() {
        return new GsonBuilder()
            .disableHtmlEscaping()  // Если HTML не нужен
...
## Полезные ссылки
- [Официальная документация Gson](https://github.com/google/gson)
...
## См. также
- [Jackson](jackson.md) - Альтернативная JSON библиотека
- [Moshi](kotlin-moshi.md) - JSON библиотека для Kotlin
- [Protocol Buffers](java-protobuf.md) - Бинарная сериализация
