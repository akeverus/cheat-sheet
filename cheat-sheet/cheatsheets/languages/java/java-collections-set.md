...
Если набор изменяется в любой момент после создания итератора каким-либо образом, кроме как с помощью собственного метода удаления итератора, итератор выдает исключение **ConcurrentModificationException.**
