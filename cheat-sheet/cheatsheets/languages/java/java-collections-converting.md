...
- Преобразование Map в Array, List или Set
...
Наконец, давайте воспользуемся **API** **CollectionUtils.addAll Apache Commons** **Collections,** чтобы заполнить элементы массива в пустом списке:
...
Важно отметить, что изменения в исходном массиве отражаются в списке, созданном с помощью **Arrays.asList()**, если элементы являются объектами, а не примитивами**:
...
Однако если мы используем **new ArrayList<>(Arrays.asList())**, изменения в исходном массиве не будут отражаться в списке, так как создается новый независимый список**:
