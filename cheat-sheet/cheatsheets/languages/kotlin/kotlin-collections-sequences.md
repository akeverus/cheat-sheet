...
- [Kotlin Sequence API](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.sequences/)
...
- `./kotlin-collections-list.md` - списки (List)
- `./kotlin-collections-operations.md` - операции над коллекциями
- `../java/java-streams.md` - Java Streams для сравнения
...
// takeWhile / dropWhile
val taken = sequence.takeWhile { it < 5 }      // берет пока условие true
val dropped = sequence.dropWhile { it < 5 }    // пропускает пока условие true
```
...
// firstOrNull
val firstOrNull = sequence.firstOrNull { it > 10 }  // null
...
// lastOrNull
val lastOrNull = sequence.lastOrNull { it > 10 }    // null

// single
val single = sequenceOf(42).single()            // 42
val singleOrNull = sequence.singleOrNull { it == 5 }  // 5 или null
```
...
val processed = readLinesSequence(largeFile)
    ...
    .map { it.trim() }
    .take(1000)  // Обработать только первые 1000 строк
    .toList()
```
...
fun analyzeLogs(logFile: File): LogAnalysis = runBlocking {
    val errors = readLinesSequence(logFile)
        ...
        .map { parseLogEntry(it) }
        .take(100)  // Анализировать только первые 100 ошибок
        .toList()
