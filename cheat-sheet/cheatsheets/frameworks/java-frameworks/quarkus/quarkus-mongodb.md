...
### 4. Используйте aggregation для сложных запросов

```java
// ✅ Хорошо
public List<Document> aggregateData() {
    return User.mongoCollection().aggregate(pipeline).into(new ArrayList<>());
}
```

### 5. Используйте change streams для real-time обновлений

```java
// ✅ Хорошо
User.mongoCollection().watch().forEach(this::handleChange);
```
