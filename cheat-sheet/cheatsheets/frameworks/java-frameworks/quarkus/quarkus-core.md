...
public class Service {
    @Inject
    Dependency dependency;  // Field injection менее предпочтителен
}
```
...
@ApplicationScoped  // Для stateless сервисов
@RequestScoped      // Для request-specific данных
@Singleton          // Для singleton без прокси
```
...
String appName;  // Менее удобно для сложных структур
```

### 4. Минимизируйте использование @Produces

```java
// ✅ Хорошо - используйте обычные бины где возможно
@ApplicationScoped
public class Service {
    // Простой бин
}

// ❌ Плохо - избыточное использование @Produces
@ApplicationScoped
...
### 5. Используйте events для слабой связанности
...
public class Service {
    ...
    public void createUser(User user) {
        ...
        event.fire(new UserCreated(created));
    }
}
```
...
### 6. Используйте build-time processing где возможно
...
- [Quarkus Arc Documentation](https://quarkus.io/guides/cdi-reference#arc)
