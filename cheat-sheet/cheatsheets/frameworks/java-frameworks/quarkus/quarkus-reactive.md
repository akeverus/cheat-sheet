...
// ❌ Плохо
return uni;  // Ошибки не обрабатываются
```

### 2. Используйте правильный тип для операций

```java
// ✅ Хорошо - Uni для single item
public Uni<User> getUser(Long id) { ... }

// ✅ Хорошо - Multi для streams
public Multi<User> getUsers() { ... }

// ❌ Плохо - неправильный выбор типа
public Multi<User> getUser(Long id) { ... }
```
...
return Uni.createFrom().item(() -> nonBlockingOperation());

// ❌ Плохо - блокирующая операция
return Uni.createFrom().item(blockingOperation());
```

### 4. Используйте backpressure для больших потоков
...
// ❌ Плохо - нет обработки переполнения
return multi.onItem().transform(this::process);
```

### 5. Настраивайте timeout для внешних вызовов
...
### 6. Используйте circuit breaker для внешних сервисов
...
- [MicroProfile Reactive Messaging](https://microprofile.io/projects/eclipse/microprofile-reactive-messaging)
