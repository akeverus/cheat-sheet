...
// ✅ Хорошо
public List<User> findUsersWithOrders() {
    return find("SELECT u FROM User u JOIN FETCH u.orders").list();
}
```
...
```java
// ✅ Хорошо - избегайте N+1 проблемы
public List<User> findUsersWithOrders() {
    return find("SELECT u FROM User u JOIN FETCH u.orders").list();
}
```

### 5. Используйте lazy loading где возможно
...
### 6. Настраивайте connection pooling
...
- [Quarkus Reactive Hibernate](https://quarkus.io/guides/hibernate-reactive)
