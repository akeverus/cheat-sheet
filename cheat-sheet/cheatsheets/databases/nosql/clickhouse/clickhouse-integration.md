...
Комплексное руководство по интеграциям ClickHouse: Kafka, Spark, Airflow, BI инструменты, языки программирования и облачные сервисы.
...
- [Language Clients](https://clickhouse.com/docs/integrations/language-clients)
- [Third-party Integrations](https://clickhouse.com/docs/integrations/third-party)

### Baeldung
- [ClickHouse Integrations](https://www.baeldung.com/clickhouse-integrations)

### См. также
- `databases/clickhouse-performance.md` - Оптимизация производительности
- `databases/clickhouse-replication.md` - Кластеры и репликация
...
- [BI инструменты и визуализация](#bi-инструменты-и-визуализация)
...
CREATE TABLE events_backup (
    timestamp DateTime,
    data String
) ENGINE = S3('https://my-bucket.s3.amazonaws.com/backups/events.parquet', 'AWS_ACCESS_KEY_ID', 'AWS_SECRET_ACCESS_KEY', 'Parquet');
...
CREATE TABLE azure_backup (
    ...
    'https://myaccount.blob.core.windows.net/mycontainer/backup.parquet',
    'account_name',
    'account_key',
    'Parquet'
);
...
<processor>
    ...
    <property name="Database Connection URL">jdbc:clickhouse://clickhouse-host:8123/default</property>
