...
difficulty: "beginner"
prerequisites: []
next: ["databases/redis-data-structures.md", "databases/redis-persistence.md"]
updated: "2026-01-16"
related: ["databases/postgres-basics.md", "databases/redis-data-structures.md"]
---
...
**Файл redis.conf** — это основной конфигурационный файл Redis. Он содержит сотни параметров, которые влияют на производительность, безопасность и функциональность Redis сервера. Параметры логически сгруппированы по областям применения.

**Расположение файла:**
- **Linux**: `/etc/redis/redis.conf`
- **macOS**: `/usr/local/etc/redis.conf`
- **Docker**: Монтируется как volume
- **Windows**: Устанавливается при установке Redis
...
# 127.0.0.1 - только localhost, 0.0.0.0 - все интерфейсы
bind 127.0.0.1 ::1

# Порт Redis (стандартный 6379)
port 6379

# Таймаут бездействия клиента в секундах (0 = отключено)
timeout 0

# TCP keepalive в секундах для поддержания соединений
tcp-keepalives 300

# Максимальный размер очереди TCP соединений
tcp-backlog 511

# Защищенный режим (отключает опасные команды в незащищенных сетях)
protected-mode yes

# Пароль для аутентификации (закомментируйте для отключения)
...
- [Введение в Redis](#�-ведение-в-redis)
  - [Основные возможности Redis](#�-�-новн�-е-возможно�-�-и-redis)
  - [Архитектура Redis](#�-�-�-и�-ек�-�-�-а-redis)
