...
- [PostgreSQL Tutorial](https://www.baeldung.com/postgresql)


См. также: `./postgres-basics.md`, `./postgres-indexes.md`, `./postgres-transactions.md`.
...
- В `pg_hba.conf` задавайте доступ по принципу наименьших прав; предпочитайте `md5`/`scram-sha-256`.
...
- Для горячего standby:
  ...
  - На standby выполнить `pg_basebackup -h master -D /data/pg -R -P`.
