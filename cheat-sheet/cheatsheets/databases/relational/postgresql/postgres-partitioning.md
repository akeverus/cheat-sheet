...
- [PostgreSQL CREATE TABLE - Partitioning](https://www.postgresql.org/docs/current/sql-createtable.html#SQL-CREATETABLE-PARTITIONING)
- [PostgreSQL Partitioning Best Practices](https://www.postgresql.org/docs/current/ddl-partitioning.html#DDL-PARTITIONING-OVERVIEW)
...
- [PostgreSQL Partitioning Tutorial](https://www.postgresqltutorial.com/postgresql-partition/)
- [Partitioning Strategies](https://www.cybertec-postgresql.com/en/postgresql-partitioning-overview/)
- [Partitioning Performance](https://www.percona.com/blog/2019/05/24/postgresql-table-partitioning/)
...
- Периодически `VACUUM/ANALYZE` по партициям с активной записью.
...
```sql
-- Создание таблицы аудита с monthly партициями
...
-- Функция для автоматической ротации
CREATE OR REPLACE FUNCTION rotate_audit_partitions()
RETURNS void AS $$
DECLARE
...
BEGIN
    ...
    partition_name := 'audit_log_' || to_char(current_month + INTERVAL '1 month', 'YYYY_MM');
    EXECUTE format(
    ...
    -- Создание индексов
    EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I (table_name, changed_at)', partition_name || '_table_idx', partition_name);
    EXECUTE format('CREATE INDEX IF NOT EXISTS %I ON %I (user_id, changed_at)', partition_name || '_user_idx', partition_name);

    -- Удаление старых партиций (старше 3 лет)
    PERFORM drop_old_partitions('audit_log', INTERVAL '3 years');
END;
$$ LANGUAGE plpgsql;

-- Настройка регулярного обслуживания
SELECT cron.schedule('audit-rotation', '0 2 1 * *', 'SELECT rotate_audit_partitions();');

-- Функция аудита для триггеров
...
-- Создание партиций по дням для высокой частоты данных
...
CREATE INDEX idx_sensor_metadata ON sensor_data USING GIN (metadata);
...
ORDER BY sensor_id, hour DESC;

-- Рефреш материализованного представления
REFRESH MATERIALIZED VIEW CONCURRENTLY hourly_sensor_stats;

-- Запросы для аналитики
-- Последние показания всех датчиков
SELECT DISTINCT ON (sensor_id)
    sensor_id, timestamp, temperature, humidity, battery_level
FROM sensor_data
...
-- Датчики с низким зарядом батареи
SELECT sensor_id, max(timestamp) as last_reading, max(battery_level) as battery
FROM sensor_data
WHERE timestamp >= CURRENT_DATE - INTERVAL '1 day'
GROUP BY sensor_id
HAVING max(battery_level) < 20;
```
...
WITH (
    fillfactor = 80,  -- Оптимизация для updates
    autovacuum_vacuum_scale_factor = 0.1
);
...
CREATE INDEX idx_transactions_status_date ON transactions (status, transaction_date) WHERE status != 'processed';
...
BEGIN
    -- Проверка баланса для debit операций
    IF p_type = 'debit' THEN
        SELECT balance INTO v_current_balance
        FROM account_balances WHERE account_id = p_account_id;

        IF v_current_balance < p_amount THEN
            RAISE EXCEPTION 'Insufficient funds: available %, required %', v_current_balance, p_amount;
        END IF;
    END IF;
    ...
    INSERT INTO transactions (
        ...
        p_account_id, p_type, p_amount, p_description, 'processed'
...
BEGIN
    ...
    IF NEW.transaction_type = 'debit' THEN
        IF EXISTS (
            ...
            GROUP BY account_id
            HAVING sum(amount) + NEW.amount > 10000  -- Дневной лимит
        ) THEN
            RAISE EXCEPTION 'Daily debit limit exceeded';
        END IF;
    END IF;
    ...
    IF NEW.reference_number IS NOT NULL THEN
        IF EXISTS (
            ...
            RAISE EXCEPTION 'Duplicate transaction reference: %', NEW.reference_number;
        END IF;
    END IF;
