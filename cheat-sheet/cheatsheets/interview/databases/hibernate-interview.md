...
**Комплексное руководство по вопросам собеседования на тему Hibernate для Senior Java Developer. Включает детальные объяснения концепций, практические примеры на Java + Spring, best practices и troubleshooting.**
...
- [Q2. (ВАЖНО) - Каковы преимущества Hibernate перед JDBC?](#q2-важно---каковы-преимущества-hibernate-перед-jdbc)
- [Q3. (ВАЖНО) - Назовие наиболее важные интерфейсы Hibernate?](#q3-важно---назовие-наиболее-важные-интерфейсы-hibernate)
...
- [Q8. (ВАЖНО) - В чем разница между First Level Cache и Second Level Cache?](#q8-важно---в-чем-разница-между-first-level-cache-и-second-level-cache)
...
- [Q11. (ВАЖНО) - Что такое Hibernate Inheritation Mapping?](#q11-важно---что-такое-hibernate-inheritation-mapping)
...
- [Q24. (ВАЖНО) - Что такое Hibernate Caching?](#q24-важно---что-такое-hibernate-caching)
...
- [Q29. (ВАЖНО) - Что происходит, когда конструктор без аргументов отсутствует в компоненте Entity?](#q29-важно---что-происходит-когда-конструктор-без-аргументов-отсутствует-в-компоненте-entity)
- [Q30. (ВАЖНО) - Можем ли мы объявить Entity класс final?](#q30-важно---можем-ли-мы-объявить-entity-класс-final)
- [Q31. (ВАЖНО) - Какие состояния могут быть у Entity?](#q31-важно---какие-состояния-могут-быть-у-entity)
- [Q32. Что такое Query Cache?](#q32-что-такое-query-cache)
- [Q33. (ВАЖНО) - Что такое проблема N+1?](#q33-важно---что-такое-проблема-n1)
- [Q34. Как решить проблему N+1?](#q34-как-решить-проблему-n1)
- [Q35. (ВАЖНО) - Какие Concurrency Strategy доступны в Hibernate?](#q35-важно---какие-concurrency-strategy-доступны-в-hibernate)
- [Q36. (ВАЖНО) - Что такое @Version?](#q36-важно---что-такое-version)
...
## Q2. (ВАЖНО) - Каковы преимущества Hibernate перед JDBC?
...
4. Exception Handling:Hibernate** оборачивает исключения **JDBC** и генерирует **unchecked** исключения, такие как **JDBCException** или **HibernateException. Это, наряду со встроенной системой управления транзакциями, помогает разработчикам избежать написания нескольких блоков **try-сatch** для обработки исключений. В случае **JDBC** он выдает проверенное исключение, называемое **SQLException, тем самым обязывая разработчика писать блоки **try-сatch** для обработки этого исключения во время компиляции.
...
## Q3. (ВАЖНО) - Назовие наиболее важные интерфейсы Hibernate?
...
Session** это объект, который поддерживает связь между объектным приложением **Java** и базой данных.Session** также имеет методы для хранения, извлечения, изменения или удаления данных из базы данных с использованием таких методов, как **persist(), load(), get(), update(), delete()** и т. д. Кроме того, он имеет фабричные методы для возврата **Query, Criteria** и **Transaction objects.
...
Нет, Session** не является потокобезопасным объектом, что означает, что любое количество потоков может одновременно получать доступ к его данным.
...
## Q8. (ВАЖНО) - В чем разница между First Level Cache и Second Level Cache?
...
| Кэш первого уровня доступен только пока **Session** открыт, после закрытия **Session** кеш уничтожается. | Кэш второго уровня доступен на протяжении всего жизненного цикла приложения, он уничтожается и создается заново только при перезапуске приложения. |
...
Файл конфигурации Hibernate** или **hibernate.cfg.xml** один из наиболее необходимых файлов конфигурации в **Hibernate. По умолчанию этот файл находится в папке **src/main/resource.
...
Если мы используем **XML-**форму конфигурации, то класс можно сделать неизменяемым, пометив его как **mutable=false.

В случае использования аннотаций неизменяемые классы могут быть созданы с помощью аннотации **@Immutable.



## Q11. (ВАЖНО) - Что такое Hibernate Inheritation Mapping?
...
3. **Table per class (таблица на каждый класс)**: В этом подходе каждый класс в иерархии наследования сопоставляется с отдельной таблицей базы данных, которая содержит только поля этого класса. В результате создается таблица для каждого класса в иерархии. Этот подход полезен, когда наследование должно быть полностью отражено в схеме базы данных и не требуется присоединение таблиц. Однако это может привести к дублированию данных и сложности в поддержке базы данных.



## Q12. Подвержен ли Hibernate атаке путем внедрения SQL-кода?

Атака с внедрением SQL-кода** представляет собой серьезную уязвимость с точки зрения веб-безопасности, при которой злоумышленник может вмешиваться в запросы, сделанные приложением/веб-сайтом к своей базе данных, тем самым позволяя злоумышленнику просматривать конфиденциальные данные, которые, как правило, невозможно восстановить. Это также может дать злоумышленнику возможность изменить/удалить данные, что приведет к повреждению поведения приложения.Hibernate** не обеспечивает иммунитет к **SQL Injection. Тем не менее, следуя передовым методам, можно избежать атак с помощью **SQL-**инъекций. Всегда рекомендуется следовать любому из следующих вариантов:
...
Эти файлы полезны, когда в проекте используются сторонние классы, где нельзя использовать аннотации **JPA, предоставляемые спящим режимом.
...
2. Session: это недолговечный объект, который действует как интерфейс между объектами **Java-**приложения и данными базы данных.
 1. Session** можно использовать для создания транзакций, запросов и критериев.
 2. Он также имеет обязательный **First Level Cache.
...
Оба метода предоставляются **Session Factory. Основные отличия приведены ниже:
...
| Эта область объекта сеанса принадлежит **Hibernate Context, и для того, чтобы этот **Hibernate Configuration File** работал, необходимо изменить его, добавив **&lt;property name = "hibernate.current_session_context_class"&gt; thread &lt;/property&gt;. Если не добавить, то использование метода вызовет исключение **HibernateException. | Для каждого запроса в многопоточной среде необходимо создавать новый объект **Session. Следовательно, вам не нужно настраивать какое-либо свойство для вызова этого метода. |
...
Этот **API** позволяет программно разрабатывать объекты запроса критериев. Для этих целей используется интерфейс **org.hibernate.Criteria. Интерфейс **Session** имеет метод **createCriteria(), который принимает **Entity rlfcc** в качестве параметров и возвращает экземпляр его, когда выполняется запрос критериев.

Это также позволяет очень легко включать ограничения для выборочного извлечения данных из базы данных. Этого можно добиться с помощью метода **add(), который принимает объект **org.hibernate.criterion.Criterion, представляющий индивидуальное ограничение.Примеры использования:Чтобы вернуть все данные класса сущностей InterviewBitEmployee.

Criteria criteria = session.createCriteria(InterviewBitEmployee.class);

List&lt;InterviewBitEmployee&gt; results = criteria.list();Для получения объектов, свойство которых имеет Value, равное ограничению, мы используем метод Restrictions.eq(). Например, чтобы получить все записи с именем «Hibernate»:

Criteria criteria= session.createCriteria(InterviewBitEmployee.class);

criteria.add(Restrictions.eq("fullName","Hibernate"));

List&lt;InterviewBitEmployee&gt; results = criteria.list();Для получения объектов, свойство которых имеет Value «не равно» ограничению, мы используем метод Restrictions.ne(). Например, чтобы получить все записи, имя сотрудника которых не Hibernate:
...
**Чтобы получить все объекты, свойства которых соответствуют заданному шаблону, мы используем Restrictions.like() (для учета регистра) и Restrictions.ilike() (для нечувствительности к регистру).
...
Точно так же он также имеет другие методы, такие как **isNull(), isNotNull(), gt(), ge(), lt(), le()** и т. д., для добавления дополнительных разновидностей ограничений. Следует отметить, что начиная с **Hibernate 5** функции, возвращающие объект **typeCriteria, устарели. Версия **Hibernate 5** предоставляет интерфейсы, такие как **CriteriaBuilder** и **CriteriaQuery, для достижения этой цели:
...
Это использует интерфейс **Query, предоставляемый **Hibernate. Объект **Query** получается путем вызова метода **createQuery()** интерфейса **Session.
...
5. public Query setParameter(int position, Object value): этот метод устанавливает значение атрибута/столбца в определенной позиции. Этот метод соответствует стилю параметра запроса **JDBC**.
6. public Query setParameter(String name, Object value): этот метод устанавливает значение для именованного параметра запроса. Пример: чтобы получить список всех записей из таблицы InterviewBitEmployee:
...
Метод session.lock()** используется для повторного присоединения отсоединенного объекта к сеансу.Метод session.lock()** не проверяет синхронизацию данных между базой данных и объектом в контексте постоянства, и, следовательно, это повторное присоединение может привести к потере синхронизации данных.



## Q24. (ВАЖНО) - Что такое Hibernate Caching?
...
Метод **merge()** можно использовать для обновления существующих значений. Особенность этого метода заключается в том, что после обновления существующих значений метод создает копию объекта сущности и возвращает ее. Этот объект результата переходит в **Persistence Context** и затем отслеживается на предмет любых изменений. Объект, который использовался изначально, не отслеживается.



## Q26. Может ли сопоставление коллекций быть выполнено с использованием связей One-To-One и Many-To-One?

False, сопоставление коллекций возможно только с ассоциациями **One-to-Many** и **Many-to-Many.



## Q27. В чем разница между setMaxResults() и setFetchSize()?

setMaxResults()** функция работает аналогично **LIMIT** в **SQL. Здесь мы устанавливаем максимальное количество строк, которые мы хотим вернуть. Этот метод реализован всеми драйверами баз данных.setFetchSize()** работает для оптимизации того, как **Hibernate** отправляет результат вызывающей стороне, например: буферизируются ли результаты, отправляются ли они фрагментами разного размера и т. д. Этот метод реализован не всеми драйверами базы данных.
...
Query query = session.createSQLQuery("select \* from interviewbit_employee ibe where ibe.fullName =:fullName").addEntity(InterviewBitEmployee.class).setParameter("fullName", "Hibernate");
...
## Q29. (ВАЖНО) - Что происходит, когда конструктор без аргументов отсутствует в компоненте Entity?


Платформа **Hibernate** внутренне использует **Reflection API** для создания экземпляров компонента управления данными при вызове методов **get()** или **load(). Используется метод **Class.newInstance(), который требует присутствия конструктора без аргументов. Когда у нас нет этого конструктора в компонентах управления данными, тогда **Hibernate** не может создать экземпляр компонента и, следовательно, выдает **HibernateException.



## Q30. (ВАЖНО) - Можем ли мы объявить Entity класс final?


Нет, мы не должны определять класс сущностей **final, потому что **Hibernate** использует прокси-классы и объекты для **lazy** загрузки данных и обращается к базе данных только тогда, когда это абсолютно необходимо. Это достигается расширением компонента управления данными. Если класс сущности **(или bean-компонент)** сделан окончательным, то он не может быть расширен и, следовательно, lazy** загрузка не может поддерживаться.



## Q31. (ВАЖНО) - Какие состояния могут быть у Entity?
...
employee.setEmail("hibernate@interviewbit.com");Persistent:
...
Это только одно из решений проблемы **N**+**1** в **Hibernate, альтернативными решениями являются использование механизма **join fetch, разделения запросов или использование вторичного кэша. Однако, приведенный пример с жадной загрузкой самый простой и популярный способ справиться с этой проблемой.

Вот несколько примеров других решений проблемы **N**+**1** в **Hibernate:1\. Использование механизма 'join fetch': Вместо ленивой загрузки или жадной загрузки можно использовать '**join fetch**', чтобы выполнить один запрос, который объединяет основные данные объекта и его связанные данные. Например:
...
## Q35. (ВАЖНО) - Какие Concurrency Strategy доступны в Hibernate?
...
}):fullName** относится к параметру, который определяется программистом и может быть установлен с помощью метода **query.setParameter** при использовании именованного запроса.Использование:
...
Чтобы понять преимущества **NamedQuery, давайте сначала разберемся с недостатками **HQL** и **SQL. Основной недостаток разбросанных по объектам доступа к данным **HQL** и **SQL** заключается в том, что код становится нечитаемым. Следовательно, в качестве хорошей практики рекомендуется сгруппировать все коды **HQL** и **SQL** в одном месте и использовать только их ссылку в фактическом коде доступа к данным. Для этого **Hibernate** дает нам именованные запросы.

Именованный запрос это статически определенный запрос с предопределенной неизменной строкой запроса. Они проверяются при создании фабрики сеансов, что ускоряет сбой приложения в случае ошибки.
