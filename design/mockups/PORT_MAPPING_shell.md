...
| — (в макете нет) | `#design-toggle` + `#layout-toggle` (циклы по __design.list/__layout.list) | Прод-фичи без макетного аналога — сохранить. **НАХОДКА ЗАКРЫТА БЕЗ КОДА (R0.54):** параллельный WIP уже добавил `instrument: 'Instrument'` в NAMES и в фолбэк order(); живьём подтверждено — aria «Сменить дизайн (сейчас: Instrument)», title «Дизайн: Instrument → Editorial». Правка не нужна. |
