...
- **CSS-правок НЕ требуется** (обёртка стилей не несёт) → **v-бамп НЕ нужен для CSS**. app.js меняется → v-бамп app.js (v=60→61) + live-sync + это APP-7 контракт.
...
```
+ v-бамп app.js в 3 шаблонах (result/settings/focus-training) v=60→61 + live-sync build/resources.

## План применения (окно, когда app поднят)
1. Collision-guard: `git status` result.html / app.js / шаблоны с `?v=` чисты.
2. Применить оба ханка; бампнуть app.js v=60→61 (3 шаблона); `cp` source→build/resources/main.
3. `node --check app.js`.
