...
7. `.regen-badge` — **этот план трогает ТОЛЬКО `.result-page .regen-badge` (2457)** (+ result.html:51).
   `.stats-page .regen-badge` (**1892**) — НЕ трогать здесь: (а) это **общее** правило с живым
   `.maturity-badge` (1893) → удалить можно лишь строку-селектор 1892, не блок; (б) stats regen-badge
   рендерится живой разметкой (stats.html:224-227, колонка «Сброшено») — это **отдельный** мёртвый
   AI-остаток (`regenSum()` всегда 0 → колонка всегда «—»), вынесен в **§10 `STA-20`** (свер. R0.116),
   а не в этот result-page removal. grep `regen-badge` перед применением — подтвердить только 2457.
