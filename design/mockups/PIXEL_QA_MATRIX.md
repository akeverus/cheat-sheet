...
|---|---|---|---|---|---|---|---|---|---|
| shell (chrome) | PASS | PASS✧ | PASS* | PASS✧ | PASS | PASS(R0.53; kbd-help/prompt-модалки R0.57) | PENDING-TEST¤ | PASS(static R0.62) | **E DONE шаг 1 (C2-i v=79; модалки-тайтлы medium v=82 R0.57)**; no-js RECHECK закрыт статик-аудитом served-HTML (noscript-алерт role=alert, 3 тоггла hidden = PE без мёртвых контролов, nav — plain links, sticky 0); NAMES закрыт без кода (R0.54); drawer/футер/?-кнопка = решения R0.26 |
...
| settings | PASS | PASS | PASS* | PASS✦+LH100(R0.58) | PASS | PASS(R0.52) | PENDING-TEST¤ | PASS(static R0.32) | **E DONE шаг 1 (C4-i применён, v=78)**; Lighthouse instrument: a11y/BP 100, label-in-name степпера ПОЧИНЕН (R0.58); CLS ПОЧИНЕН (R0.64, v=83): pre-paint data-js/data-settings-tab → первый кадр = вкладочное состояние; замер под троттлингом 0.285→0.0004; сброс банка = alertdialog-модалка вместо window.confirm (R0.66, app.js v=57); focus-visible аудит: невидимый фокус tabpanel починен (R0.68, v=85), остальное покрытие чисто; карт-сетка осей/switch-тогглы → шаг 2/Фаза G |
