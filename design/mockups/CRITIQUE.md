...
### cr.192 — WCAG 2.4.6 Headings and Labels (AA): verified-clean

**Проверка:** заголовки и подписи форм описывают тему/назначение; иерархия h1–h6 без пропусков уровней.

**Метод:** эмпирический обход достижимых поверхностей (focus `/`, `/settings`, `/stats`) — перечисление всех h1–h6 + accessible-name всех контролов и radiogroup + заголовков колонок таблиц.

**Результат — чисто.**
- **focus (`/`):** h1 «Подготовка к собеседованию» → h2 (текст вопроса) → h3 «Горячие клавиши». 0 пропусков, все радио с `<label>`.
- **`/settings`:** h1 «Настройки сессии» → h2 (вкладки Сессия/Оформление/Данные + srOnly «Сегодня к повтору») → h3 (Фильтры/Режим/Прогресс, Экспорт данных/Опасная зона). 6 radiogroup с описательными именами: Дизайн, Тема оформления, Раскладка, Ширина чтения, Плотность, Движение. 0 неподписанных.
- **`/stats`:** h1 «Аналитика» → h2 (Что делать дальше / Фильтры / Поиск / Прогресс по темам / Точность по темам / Детализация по темам) → h3 «Горячие клавиши». Заголовки колонок: Тема/Всего/Прогресс/Точность/Сброшено/Зрелость. 0 пропусков, 0 неподписанных.
- Везде ровно один h1; ни одного пропуска уровня (h1→h2→h3 монотонно); все подписи содержательны (нет «Click here»/пустых).

`result.html` / `session-summary.html` (нужна сессия) — по шаблону: result h2=вопрос, h3 «Пояснение», h3 «Похожие вопросы для закрепления»; summary — `<caption>` + описательные заголовки секций. Кросс-проверка в статическом workflow-аудите (см. ниже по журналу). Правок не требуется.


### cr.191 — WCAG 2.4.3 Focus Order (A): verified-clean

**Проверка:** focusable-компоненты получают фокус в порядке, сохраняющем смысл и операбельность.

**Метод:** grep всех `tabindex` в шаблонах+JS (положительных нет) + разбор roving-паттернов + пост-ответного управления фокусом + эмпирика естественного tab-порядка на `/` (flow).

**Результат — чисто.**
- **Положительный `tabindex` (>0) отсутствует полностью** → tab-порядок следует DOM-порядку (осмысленность DOM подтверждена R1.95 / cr.190).
- **Эмпирика (flow):** skip-link(top −54) → бренд(12) → nav Фокус/Аналитика/Настройки(L→R) → тогглы раскладка/дизайн/тема(L→R) → варианты ответа(сверху вниз 450→588→725→894). Монотонно, = DOM = порядок чтения. `main#main-content tabindex=-1` (цель skip-link) корректно вне tab.
- **Roving tabindex** (seg-controls ×7, ARIA-tablist настроек, sortable th статистики): один tab-stop на виджет, внутри — ←→↑↓/Home/End; корректный composite-widget паттерн, не нарушение 2.4.3.
- **Пост-ответное управление фокусом** (app.js 1696–1708): после ответа фокус уходит ВПЕРЁД на раскрытую ленту вердикта (`feedbackDiv` tabindex=-1 + focus()); CTA «Следующий вопрос» переносится в зону `post-answer-controls` НИЖЕ вердикта → в tab-порядке идёт после разбора (осознанно: нельзя проскочить разбор, коммент 1690–1695).
- **Focus-trap оверлеев** (app.js 413, stats.js 372): фокус заперт внутри диалога — корректно.

Правок не требуется.


### cr.190 — WCAG 1.3.2 Meaningful Sequence (A): verified-clean

**Проверка:** программный (DOM) порядок контента образует осмысленную последовательность чтения на многоколоночных wide/split-раскладках (screen-reader / CSS-off линеаризация).

**Метод:** grep base.css на `order:` и `flex-*-reverse` (0 совпадений) + разбор всех 4 wide-раскладок + эмпирика на `/stats` (DOM-порядок детей vs bounding-rects, masthead).

**Результат — чисто.** Механизм единый во всех раскладках: позиционирование через `grid-column`/`grid-row` БЕЗ CSS `order` и БЕЗ `flex-direction: *-reverse` → DOM-поздний элемент всегда визуально правее/ниже, не выше по чтению.
- **result wide:** `#interview-card` (col1, DOM-первый) → `#result-related-questions` (col2 sticky rail, DOM-позже). Линейно: результат → похожие вопросы. Визуал L→R = DOM.
- **focus split:** мета→вопрос(sticky)→код (col1) → форма-ответы (col2, DOM после кода, `grid-row:1/-1` align-start) → вердикт/пост-контролы (`grid-row:6/7`, full-width внизу, DOM-последние). Линейно = вопрос ПЕРЕД ответами (педагогически верный порядок), 2D-размещение справа не меняет смысл линеаризации.
- **stats wide:** таблица тем (col1, `row span 2`) / forecast+gaps (col2, auto-flow, DOM-позже). Эмпирика: при отсутствии forecast/gaps грид не активируется (гейт `:has`), поток одноколоночный, DOM=визуал (top 158→388→671→1002→1500).
- **summary wide:** score-геро `grid-template-areas` (режим/%/подпись слева, плитки центр, время справа) + таблица(col1)/ошибки(col2)/рекомендации(full-width футер) — линеаризуется осмысленно.
- **masthead** (на всех страницах): при 1280 flex, дети brand→actions слева-направо, DOM=визуал; grid-areas-вариант (brand/nav/toggles) на др. брейкпоинте — конвенциональный header-порядок.

Правок не требуется.


### cr.189 — WCAG 3.2.2 On Input (A): verified-clean

**Проверка:** изменение настройки контрола не вызывает автоматическую смену контекста (навигация/сабмит/новое окно/скачок фокуса) без предупреждения.

**Метод:** аудит всех `change`-обработчиков app.js/stats.js + эмпирический тест (смена select mode на /settings, замер URL/фокуса/навигации).

**Результат — чисто.**
- **`toggleTopic`** (change на shuffle/ordered): только переключает `disabled` смежных полей (topic/group/ordered) + обновляет live-region-подсказку. Изменение состояния связанных контролов в той же форме — НЕ смена контекста (нет навигации/сабмита/фокус-скачка).
- **`modeSelect` change:** обновляет метку кнопки «Начать …» + дефолт count. **Эмпирика:** TRAINING→STUDY → URL не изменился, навигации нет, метка «Начать тренировку»→«Начать изучение» (related-update).
- **`countInput` change:** валидация/кламп, без навигации.
- **Персонализация (seg-control/тогглы):** смена темы/дизайна/раскладки = презентация, явно НЕ смена контекста (WCAG note).
- **Навигация/сабмит** (`finishForm.submit()`, `location.href=nextLink.href`) — ТОЛЬКО внутри `onclick`/`onkeydown` явной активации ссылки/кнопки, не «on input».

Ввод обновляет смежные поля/презентацию; смена контекста только по явной активации. Правок не требуется.


### cr.188 — WCAG 2.1.4 Character Key Shortcuts (A): verified-clean (намерение), + минорная заметка

**Проверка:** одноклавишные шорткаты (буква/цифра/символ) отключаемы, переназначаемы ИЛИ активны только на фокусе.

**Метод:** аудит обработчиков + эмпирический тест перехвата ввода (chrome-devtools: диспатч `?`/`/` в поле поиска и вне его).

**Результат — намерение SC выполнено.**
- **`1-9`** (выбор варианта): фокус-гейт `form.contains(activeElement)` (app.js) → строго опция «active on focus» ✓ (R1.63).
- **`?` / `/`** (справка / фокус поиска): guard `if (target.tagName in [INPUT,TEXTAREA,SELECT]) return;` ДО обработки клавиши (app.js:1932, stats.js:388-389). **Эмпирика:** в поле поиска `?`/`/` НЕ сработали, фокус остался в поле; вне поля `?` открыл справку. → главный вред 2.1.4 (перехват символьного ввода, в т.ч. speech-to-text в поле) предотвращён; оба шортката недеструктивны и обратимы (Esc закрывает справку, `/` лишь фокусирует поиск).
- **ArrowUp/Down, Esc** — не одиночные печатные символы (навигация/отмена), вне буквы SC.

**Минорная заметка (НЕ правлю):** `?`/`/` — глобальные одноклавишные вне текст-полей, без явного turn-off/remap (строгая буква SC). Не исправляю: (1) вред минимален и обратим, (2) это стандартный паттерн (GitHub/GitLab `/`,`?`), (3) тоггл «отключить шорткаты» = новая persistent config-ось (user-gated решение, ср. [[project_personalization_settings]]). Если пользователь захочет строгую букву — добавить ось «горячие клавиши: вкл/выкл» в /settings. Кода не менял.


### cr.187 — WCAG 1.4.1 Use of Color (A): verified-clean

**Проверка:** цвет не единственный носитель информации.

**Метод:** аудит всех информативных цвето-кодированных состояний в шаблонах + CSS на наличие нецветового дубля (текст/иконка/форма/число/подчёркивание).

**Результат — чисто (у каждого состояния есть нецветовой сигнал).**
- **Вердикт (Верно/Неверно):** иконка `#i-circle-check`/`#i-circle-x` + ТЕКСТ «Верно!»/«Неверно» + цвет.
- **Correct/wrong опции:** текст-метки «Правильный ответ»/«Твой выбор» + aria-label-префиксы («Правильный: »/«Твой ответ: »).
- **Точность / зрелость (/stats):** цвет-класс high/mid/low + ЧИСЛОВОЕ значение («85.0%», число зрелости) + title-тултип.
- **Активная навигация:** `border-bottom` accent (подчёркивание) + `aria-current="page"`, не только цвет текста.
- **Активный seg-btn:** фоновая ЗАЛИВКА accent (различие по fill/luminance, не hue-only) + `aria-checked` (role=radio).
- **Выбранный MCQ-вариант:** рамка + wash + буква-бейдж (::before) + `aria-checked`.
- **Бонус:** `@media (forced-colors: active)` кодирует все эти состояния системной парой Highlight/HighlightText → выживают в Windows High Contrast (устойчивость сверх минимума).

Правок не требуется.


### cr.186 — WCAG 2.4.7 Focus Visible (AA): verified-clean (сортируемые заголовки таблицы /stats)

**Проверка:** у клавиатурного фокуса есть видимый индикатор — прицельно для `th[tabindex=0].sortable` (новоинструментированный фокусируемый элемент, не покрытый общим проходом R1.62).

**Метод:** grep правил outline для th/.sortable + эмпирический фокус в браузере с чтением `:focus-visible`-состояния и computed outline.

**Результат — чисто.**
- Выделенное правило base.css:1870: `th.sortable:focus-visible { outline: 2px solid var(--color-border-focus); outline-offset: -2px; }` — инсет-offset, чтобы кольцо не обрезалось краем ячейки.
- **Эмпирика:** фокус на th → `matches(':focus-visible')=true`, computed outline **2px solid rgb(217,119,87)** (акцент), offset −2px — реально рендерится.
- Контраст кольца ≥3:1 (4.86 dark / 3.85 light, по R1.84) → индикатор и видим, и достаточно контрастен.
- Бонус: `⇅`-афорданс сортируемости через `::after` (визуальный signifier), корректный tabindex=0.

Смежные фокусируемые элементы (кнопки/ссылки/инпуты/seg-control/вкладки/нативные чекбоксы) покрыты в R1.62. Правок не требуется.


### cr.185 — WCAG 3.1.2 Language of Parts (AA): verified-clean

**Проверка:** фрагменты на языке ≠ `lang` страницы (ru) несут собственный `lang` — КРОМЕ исключений (имена собственные, технические термины, код, слова неопределённого языка).

**Метод:** проверка `<html lang>` + grep англ. UI-фраз в шаблонах + эмпирический скан видимой прозы на / на цельные англ. пассажи (≥5 подряд латинских слов) + баланс кириллица/латиница.

**Результат — чисто.**
- `<html lang="ru">` на всех 6 шаблонах ✓ (3.1.1 фундамент).
- Хардкод английских предложений в chrome — нет (UI полностью русский).
- Проза на / : **0** длинных английских пассажей, кириллица **88%** — латиница это изолированные технические термины/идентификаторы (exempt).
- Контент data-driven из seed, русифицирован в LANE 5/6 (verify_russify.py, порог cyr>0.45); английский остаётся только в exempt-категориях: технические термины, имена собственные (названия тем/технологий), код (`<code>`/`<pre>` — не natural-language passage).
- Вложенной `lang`-разметки нет — и не требуется (нет иноязычных пассажей для маркировки).

Ограничение: длинная пост-ответная проза POST-гейтед (не скана напрямую), но она из того же русифицированного корпуса LANE 6; выборка вопроса/опций репрезентативна. Правок не требуется.
...
**Метод:** чтение разметки + stats.js sort-логики + эмпирическая проверка семантики и транзиции `aria-sort` в браузере (клик = клиентская DOM-сортировка, без серверной мутации).

**Результат — чисто (таблица тем размечена образцово).**
- `<caption class="visually-hidden">Статистика по темам</caption>` — программный заголовок таблицы ✓.
- Обёртка: `role="region" aria-label="Детализация по темам"` + `tabindex=0` (скролл-регион фокусируем) ✓.
- Все колонки `<th scope="col">`; строки-темы `<th scope="row">` ✓.
- **Сортируемые заголовки:** stats.js добавляет `tabindex=0` + `aria-keyshortcuts="Enter Space"` + `aria-sort` (PE: без JS — обычные неинтерактивные `<th>`, без ложного обещания клавиш). Обработчики И `click`, И `keydown` (Enter/Space, preventDefault). Несортируемая «Прогресс» корректно БЕЗ интерактивных атрибутов.
- **Эмпирика:** клик по «Всего» → его `aria-sort=ascending`, у остальных сортируемых → `none` (эксклюзивно); live-регион `#table-sort-status`: «Таблица отсортирована: Всего, по возрастанию.» Имена колонок — чистый текст (aria-label не подменяет).

Покрывает 1.3.1 + смежные 4.1.2 (aria-sort state) и 2.1.1 (keyboard). Прочие 1.3.1-элементы (иерархия заголовков, list-семантика опций `role=list`, label↔контрол) затронуты в прежних тиках (R1.69/R1.70/3.3.2). Правок не требуется.


### cr.182 — WCAG 2.5.8 Target Size (Minimum) (AA, 2.2): verified-clean

**Проверка:** интерактивные цели ≥24×24 CSS-px ЛИБО с достаточным интервалом (24px-круг не пересекает соседние), кроме исключений (инлайн-текст, UA-дефолт).

**Метод:** эмпирический замер bounding-box всех интерактивных контролов на /, /settings (все 3 вкладки), /stats; для radio/checkbox — эффективная цель = label-обёртка; замер center-to-center интервалов для маленьких целей.

**Результат — чисто.**
- **`/`**: 0 целей <24px. (Скрытые radio 1×1 не в счёт — реальная цель это label-опция ≥44px.)
- **`/settings`** (Сессия/Оформление/Данные — прокликал все вкладки): 0 целей <24px. Seg-control-опции, font-stepper (min-height 32px), чекбоксы (label-цель), вкладки, кнопки — все ≥24px.
- **`/stats`**: 12 ссылок-тем в таблице (`th.topic-name a`, высота 17px) — проходят через **spacing-исключение**: вертикальный интервал центр-в-центр соседних = **56-57px** (≥24), высота строки 56px; горизонтально ссылка единственная интерактивная в строке (`td` неинтерактивны). 24px-круги не пересекаются.

Все интерактивные цели удовлетворяют 2.5.8 (размером или интервалом). Правок не требуется.
...
**Метод:** CDP-эмуляция 320×640; замер `scrollWidth−clientWidth` страницы + поиск элементов шире вьюпорта, не являющихся допустимыми исключениями (overflow-x:auto / pre / table).

**Результат — чисто.**
- `/` (вопрос): горизонтальный оверфлоу **0**, нарушителей **0**.
- `/settings`: оверфлоу **0**, нарушителей **0** (вкладки + seg-control + форма стекаются).
- `/stats`: оверфлоу **0**, нарушителей **0**. Таблица тем **реформатируется в стек** (`data-label`-карточки: `overflow-x:visible`, `scrollableX:false`) — даже допустимый 2D-скролл не понадобился.

Все достижимые страницы держат одну колонку на 320px без горизонтального скролла. **Ограничение:** result/session-summary POST-гейтед (не мерил напрямую), но они наследуют те же responsive-правила base.css и структуру header/main, проверенную на достижимых страницах. Правок не требуется.


### cr.179 — WCAG 1.4.11 Non-text Contrast (AA): частично чисто + design-tension (РЕШЕНИЕ ПОЛЬЗОВАТЕЛЯ)

**Проверка:** границы контролов / индикаторы состояния / фокус-кольца / информативная графика ≥3:1 к соседним цветам.

**Метод:** эмпирический замер контрастов на /settings в ОБЕИХ темах (chrome-devtools, hex+rgba парсер, альфа-композит): фокус-кольцо, границы инпута/селекта/кнопок, заливка vs фон.

**Проходит ✓:**
- **Фокус-кольцо** `--color-border-focus` vs фон: **3.85:1** (light) / **4.86:1** (dark) — ключевой keyboard-индикатор ОК.
- **Primary-кнопка** (accent border) — 4.86:1 (dark) ✓.
- **Select** — 10.47:1 (light), но это фактически **UA-дефолтная рамка нативного select**, не авторский токен (в light её цвет #3E3C36 = dark-primary, а не текущий `--color-border-primary` #D1CFC5).

**Ниже 3:1 (измерено) — но это осознанный design-choice:**
- **Текст-инпут** (`input[type=number]`): заливка 1.1:1, бордюр **1.48:1** (light) / **1.37:1** (dark).
- **Ghost/secondary-кнопки** (`.font-step-btn`, прозрачный фон): бордюр **1.48/1.37:1**.
- Причина: `--color-border-primary` — **намеренный hairline** (спека: «Cloud Light hairline» #D1CFC5 / #3E3C36), фирменный editorial-приём через все 11 дизайнов.

**Вердикт — НЕ ПРАВЛЮ (эскалация):** это не дефект стиля, а зафиксированный дизайн-язык. Поднятие hairline-границ до 3:1 сломало бы editorial-эстетику во всех темах/дизайнах (запрещено протоколом — «не переоткрывать осознанные решения, не плодить хаотичный редизайн»). Смягчающие факторы: критичное фокус-кольцо проходит, у инпутов есть `<label>` + слабая заливка как доп. идентификация, а 1.4.11 для помеченных текст-полей трактуется мягко.

**Решение пользователя (A/B):** (A) сохранить hairline-эстетику как есть (границы <3:1, идентификация через label+focus); (B) поднять контраст границ **только у текст-инпутов и ghost-кнопок** до ≥3:1 (напр. отдельный `--color-border-control` ≥3:1), сохранив hairline для декоративных разделителей. Замеры выше — исходные данные для решения. Кода не менял.
...
**Метод:** grep всех `<button>/<a>` с `aria-label` + видимым текстом; эмпирическое вычисление visible-text vs accessible-name (chrome-devtools), в т.ч. сверка code-points глифа «A−».

**Результат — чисто (во всех случаях видимый текст ⊂ accessible name).**
- `stats` «Применить фильтры» ⊂ «Применить фильтры статистики»; «Искать» ⊂ «Искать по вопросам» (подтверждено браузером).
- font-stepper «A−» (U+0041 U+2212) ⊂ «A−: уменьшить размер шрифта»; «A+» ⊂ «A+: увеличить размер шрифта» (глиф минуса совпал точь-в-точь, подтверждено code-points).
- grade-кнопки «Не помню/Трудно/Хорошо/Легко» ⊂ «Оценка N: …» (источник).
- CTA-ссылки: видимый «Следующий вопрос»/«На главную» + декоративная стрелка `<span aria-hidden> →</span>` → accessible name = чистый текст-метка (R1.59).
- `aside aria-label="Сессия"` — лендмарк-регион, не UI-контрол → вне scope 2.5.3.

**Benign-заметка (не баг):** grade-кнопки держат видимый текст в СУФФИКСЕ accessible name («Оценка N: Не помню») — для 2.5.3 достаточно вхождения (выполнено); префикс-позиция для speech-input была бы «приятнее», но не требуется. Правок не требуется.


### cr.176 — WCAG 1.4.12 Text Spacing (AA): verified-clean

**Проверка:** при пользовательском переопределении интервалов (line-height 1.5, para 2em, letter-spacing 0.12em, word-spacing 0.16em) не теряется содержимое/функциональность.

**Метод:** инъекция стандартного тест-CSS 1.4.12 через chrome-devtools на /, /settings, /stats при 1280px и 375px (mobile-эмуляция); замер горизонтального оверфлоу страницы + поиск боксов с обрезанием текста (overflow:hidden && scrollHeight>clientHeight) и боксов шире вьюпорта.

**Результат — чисто.**
- `/` (вопрос): 1280 и 375 — оверфлоу 0, обрезаний 0, широких боксов 0.
- `/settings`: 375 — оверфлоу 0, широких боксов 0. Два «clip»-хита оказались **visually-hidden** SR-only заголовками (`h2.settings-panel-title` = `height:1px; overflow:hidden; position:absolute`, дублирует видимую вкладку; `h2.visually-hidden`) — их клип это сам паттерн скрытия для скринридера, не потеря видимого контента → вне scope 1.4.12 (ср. R1.60).
- `/stats`: 375 (с исключением visually-hidden) — оверфлоу 0, обрезаний 0, широких боксов 0. Таблица тем при узости уходит в `overflow-x:auto`-скролл (допустимо).

Макет флюидный/responsive, переопределение интервалов переживает без потери контента. Правок не требуется.


### cr.175 — WCAG 2.5.4 Motion Actuation (A): verified-clean по отсутствию

**Проверка:** функции на движении устройства (тряска/наклон/акселерометр) имеют UI-альтернативу и отключаемы.

**Метод:** grep по JS/шаблонам на `devicemotion`/`deviceorientation`/`DeviceMotionEvent`/`accelerometer`/`gyroscope`/`shake`/`requestPermission`.

**Результат — чисто (motion-функций нет).** Ни одного sensor-обработчика; нет `DeviceMotionEvent.requestPermission`-гейта. Все действия — только через явные UI-контролы (кнопки, radio, ссылки, тогглы) и клавиатуру. Критерий выполнен по отсутствию применимой функциональности. Правок не требуется.


### cr.174 — WCAG 3.3.7 Redundant Entry (A, 2.2): verified-clean

**Проверка:** данные, введённые ранее в рамках одного процесса, авто-подставляются или доступны для выбора, а не запрашиваются повторно (кроме существенных/security/expired-случаев).

**Метод:** трассировка проброса параметров сессии/фильтра по всему флоу (settings → start → вопрос → answer → result → next) + проверка предзаполнения формы /settings.

**Результат — чисто (повторного ввода нет).**
- **settings → start:** hidden-инпуты `topic/group/important/onlyWrong/shuffle/weakTopics/ordered` переносят текущий фильтр в форму `/start` (settings.html:103-109).
- **вопрос → answer:** hidden-инпуты в focus-training.html несут фильтр + `questionId` в `POST /answer` (135-142).
- **result → next:** ссылка «Следующий вопрос» тащит все параметры фильтра в query-string (result.html:106).
- **Форма /settings предзаполнена** из текущего состояния: `th:selected` на topic/group/ordered/mode, `th:checked` на чекбоксах фильтра — прежний выбор пользователя показан, не обнулён.

Единственный «ввод» (фильтр/count/режим — на /settings, предзаполнены; выбор ответа — новый на каждый вопрос, не дубль). Полей персональных данных, которые пришлось бы вводить дважды, нет. Критерий выполнен. Правок не требуется.


### cr.173 — WCAG 2.4.11 Focus Not Obscured (Minimum) (AA, 2.2): verified-clean

**Проверка:** элемент в фокусе не скрыт целиком автор-контентом (sticky/fixed-шапка/футер/оверлей).

**Метод:** инвентаризация всех `position: sticky|fixed` в base.css + эмпирический замер на /stats (chrome-devtools: rect'ы фокуса vs sticky-заголовка, скролл страницы, развёртка таблицы).

**Результат — чисто.**
- `.ed-masthead` — `position: relative` (НЕ sticky) → фокус у верха страницы шапкой не перекрывается.
- Sticky `thead th` таблицы тем (`top:0; z-index:1`) — **инертен на уровне страницы**: скролл-контейнер `.topic-table-wrap` (`overflow-y:auto`, `max-height:none`) не скроллится внутренне ни свёрнутым (12 строк, height 722px), ни развёрнутым (319 строк) — растёт под контент, скроллится страница. Замер: после `scrollTo(0,1200)` thead top = −43 (уехал над вьюпортом со страницей), к `top:0` вьюпорта не прилипает → строку-ссылку не закрывает.
- Модалки `fixed inset:0` (kbd-help, admin) — забирают фокус (focus-trap), не перекрывают фоновый.
- `.back-to-top` `fixed` 44×44 в углу (space-5-отступ, после скролла) — full-width строки/кнопки целиком за ней не прячутся (порог Minimum = «entirely hidden»).
- Sticky-рейки wide/split-layout (`#result-related-questions`, `.focus-question`) — `align-self:start` в собственной колонке грида, чужой фокусируемый контент не оверлеят.

**Латентная заметка (не баг сейчас):** `top:0` у thead безвреден только пока у `.topic-table-wrap` нет `max-height`. Если будущий enhancement сделает таблицу внутренне-скроллящей — тогда залипший заголовок начнёт перекрывать строку в фокусе, и понадобится `scroll-padding-top: <высота thead>` на обёртке. Сейчас добавлять инертный `scroll-padding` не нужно (правка без эффекта). Правок не требуется.


### cr.172 — WCAG 1.3.4 Orientation (AA): verified-clean

**Проверка:** контент не блокирует отображение/работу в одной ориентации (портрет/ландшафт), если это не существенно.

**Метод:** grep CSS на `@media (orientation)`-ограничения; JS на `screen.orientation.lock()`; шаблоны на «поверните устройство»; проверка viewport-меты.

**Результат — чисто (локов нет).**
- Нет ни одного `@media (orientation: portrait|landscape)`-правила, ограничивающего контент.
- Нет `screen.orientation.lock()` в JS.
- Viewport-мета: `width=device-width, initial-scale=1.0` — без `maximum-scale`/`user-scalable=no`/orientation-лока → обе ориентации и zoom разрешены.
- Нет сообщений «поверните устройство».
- base.css `@media (max-width:859px)` стекает двухколоночный разбор на узких/портретных экранах — это responsive-reflow В ПОДДЕРЖКУ портрета (противоположность локу).

Контент операбелен в обеих ориентациях, раскладка адаптируется. Правок не требуется.
...
**Метод:** перечислены ВСЕ `<input>`/`<textarea>`/`<select>` по всем шаблонам, каждое сверено со списком назначений.

**Результат — чисто (нет применимых полей).**
- Видимые поля: `count` (число вопросов — app-параметр), `q` (`type=search` — поисковый запрос), чекбоксы-фильтры, radio (выбор ответа), select'ы (group/topic/mode/ordered). Ни одно не относится к персональным данным о пользователе.
- Hidden-поля (questionId/topic/group/флаги фильтра) — носители состояния, вне scope 1.3.5 (не user-facing).
- `<textarea>` — нет.

Приложение — анонимный локальный тренажёр: нет логина/профиля/адреса/оплаты/ФИО/tel/email/DOB. Ни одного поля из списка Input Purposes → критерий выполнен по отсутствию. Добавлять `autocomplete` к count/search/фильтрам НЕЛЬЗЯ — валидного токена для них не существует, это ухудшило бы семантику. Попутно корректно: `type=search` и `type=number` (min/max) уже проставлены. Правок не требуется.


### cr.169 — WCAG 3.2.4 Consistent Identification (AA): FIX «Завершить сессию»

**Проверка:** компоненты с одинаковой функцией обозначены одинаково между страницами.

**Находка:** три контрола, отправляющие `POST /finish` (завершить текущую сессию — идентичная функция), назывались по-разному:
- `result.html` → «Завершить сессию»
- `focus-training.html` → «Завершить сессию»
- `settings.html` → «Завершить **текущую** сессию» ← расхождение

**Причина правки (консистентность, a11y 3.2.4).** Реконсиляция к мажоритарной метке «Завершить сессию» (2 из 3). Квалификатор «текущую» на /settings избыточен: кнопка гейтится `th:if="interviewSession != null"` (появляется только при активной сессии) и стоит рядом с кнопкой-антонимом «Начать» — глаголы Начать/Завершить сами разводят контролы, ложной неоднозначности нет.

**Правка:** `settings.html` — текст кнопки `.btn-finish-small` «Завершить текущую сессию» → «Завершить сессию». Только текст; форма, `id`, action `/finish` целы. Шаблон (server-rendered) — cache-bust не нужен.

**Смежное — чисто (не трогаю):** skip-link'и различаются по тексту («Перейти к вопросу/результату/итогам/…»), но это описание пункта назначения (все → `#main-content`) — намеренная и полезная дифференциация в духе 2.4.4/2.4.9 (назначение ссылки), уравнивать нельзя (потеря информации). Тогглы/бренд/back-to-top живут в общем фрагменте header → консистентны по построению.


### cr.168 — WCAG 3.2.3 Consistent Navigation (AA): verified-clean

**Проверка:** повторяющиеся навигационные механизмы идут в одинаковом относительном порядке на всех страницах.

**Метод:** источник — единственный фрагмент `fragments/header :: header(...)`, который рендерят все 6 шаблонов (focus-training, result, session-summary, stats, settings, error). Плюс эмпирика: DOM-снимок шапки на `/` и `/settings` через chrome-devtools.

**Результат — чисто.** На обеих страницах идентично:
- `nav.ed-nav` порядок ссылок: **Фокус → Аналитика → Настройки** (одинаков);
- группа тогглов `.ed-masthead-toggles`: **layout → design → theme** (одинакова);
- бренд-ссылка `href="/"`, skip-link `href="#main-content"` (совпадают).
- Меняется только `aria-current="page"` — переезжает на активную страницу (это и есть требуемое поведение, не нарушение порядка).

Различия по вызовам фрагмента (`pageTitle`, `activePage`) корректны: `pageTitle` = собственный H1 страницы, `activePage` управляет `aria-current`. Единый фрагмент = навигация консистентна по построению; дублирующей/расходящейся навигации нет. Правок не требуется.


### cr.167 — WCAG 2.4.5 Multiple Ways (AA): ≥2 способа найти контент — verified-clean

**Вопрос:** доступно более одного способа локации страницы/контента в наборе (кроме страниц-шагов процесса).

**Метод (эмпирический, chrome-devtools):** инвентаризация wayfinding-механизмов на / и /stats — поиск, полный список тем-ссылок, primary-nav, topic-select, контекстные рекомендации.

**Результат — 2.4.5 чисто (пять независимых путей, требуется ≥2):**
- **Поиск** (/stats `input[type=search]` «Поиск по вопросам») — техника G161 (поиск).
- **Обзор полного списка тем** — 319 topic-ссылок в таблице тем = исчерпывающий индекс/ToC всех тем (G126/G64).
- **Primary-nav** (Фокус/Аналитика/Настройки на всех страницах) — G125 навигация.
- **Topic-select** — выпадающий список 320 опций (все темы) в фильтрах.
- **Контекстные рекомендации** — 321 ссылка (weakTopics/onlyWrong/related/today-widget) — пути «что дальше».

Три из них (поиск + полный список тем + навигация) самодостаточны и независимы → требование ≥2 выполнено с запасом. Страница вопроса — шаг процесса (сессия), но темы/вопросы напрямую локализуемы поиском и списком → исключение процесса не нужно.

**Итог:** код не менялся — строгая верификация. 2.4.5 AA выполнен.


### cr.166 — WCAG 3.3.1 Error Identification (A): ошибки ввода идентифицируются текстом — verified-clean

**Вопрос:** при автодетекте ошибки ввода поле-в-ошибке идентифицируется, а ошибка описывается пользователю текстом.

**Метод (эмпирический, chrome-devtools, Constraint Validation API БЕЗ сабмита):** на `session-form` — установка невалидных значений в `count` и чтение `checkValidity()`/`validationMessage`/`validity.*`; инвентаризация прочих форм на наличие ошибочно-заполняемых полей.

**Результат — 3.3.1 чисто:**
- **`count`** (`<input type=number min=1 max=200>`, форма без `novalidate`, method=post /start):
  - over max (500) → invalid, native текст **«Значение должно быть меньше или равно 200.»** (`rangeOverflow`).
  - under min (0) → invalid, **«Значение должно быть больше или равно 1.»** (`rangeUnderflow`).
  - empty → valid (`required=false`, сервер дефолтит — не ошибка).
  - valid (20) → ok.
  Нативная constraint-валидация при сабмите блокирует отправку, фокусирует/подсвечивает поле и показывает текст ошибки → идентификация + текстовое описание (3.3.1 A) выполнены. Валидная техника (G84/G85 native).
- **Прочие формы без ошибочного ввода:** filters-form (GET, селекты/чекбоксы — только валидные значения), search `q` (любой текст, empty допустим), mode/topic/ordered селекты (фиксированные опции) — поверхности ошибки нет.

**Наблюдение (не провал, не правил):** `count` без `aria-invalid`/`aria-describedby` — приложение полагается на нативный UI валидации (для A-уровня достаточно). Кастомный inline-error c aria-invalid+описанием был бы более робастным enhancement (и закрыл бы 3.3.3 Error Suggestion явно), но не требуется 3.3.1; JS-blast-radius — оставлено.

**Итог:** код не менялся — строгая верификация. 3.3.1 A выполнен. Значение `count` восстановлено (20), сабмита не было (сессия не создана).


### cr.165 — WCAG 2.4.6 Headings and Labels (AA): описательность заголовков/подписей — verified-clean

**Вопрос:** заголовки и подписи описывают тему/назначение; иерархия логична (бонус — 1.3.1 без пропусков уровней).

**Метод (эмпирический, chrome-devtools):** обход всех `h1..h6` на /, /stats, /settings, странице вопроса — accessible-name (минус aria-hidden/svg), пустые, дубли на уровне, пропуски уровней (jump >1), число h1; проверка описательности подписей вкладок/групп.

**Результат — 2.4.6 чисто на всех 4 поверхностях:**
- **/**: h1«Подготовка к собеседованию» → h2«Как отвечать на алгоритмические вопросы (шаблон)?» → h3«Горячие клавиши».
- **/stats**: h1«Аналитика» + 6 описательных h2 (Что делать дальше/Фильтры/Поиск/Прогресс по темам/Точность по темам/Детализация по темам) + h3«Горячие клавиши».
- **/settings**: h1«Настройки сессии» + h2 панелей (Сессия/Оформление/Данные) + h3 подсекций (Фильтры/Режим/Прогресс/Экспорт данных/Опасная зона) + srOnly h2«Сегодня к повтору». Вкладки «Сессия/Оформление/Данные» — описательны.
- **Страница вопроса**: h1«Подготовка к собеседованию» + h2 = сам вопрос (описательно) + h3«Горячие клавиши»; группа опций `aria-label`«Варианты ответа».
- Везде: **0 пустых, ровно один h1, 0 пропусков уровней (1.3.1 иерархия чиста), 0 дублей** на уровне.

**Наблюдение (не провал, не правил):** /stats и /settings имеют page-specific h1 («Аналитика»/«Настройки сессии»), а home/вопрос — generic site-title h1 «Подготовка к собеседованию» с контентом в h2. Лёгкая нестыковка; заголовок описателен, вопрос ясно помечен h2; одна template обслуживает dashboard+вопрос → смена h1 по состоянию = SSR-правка без a11y-провала. Оставлено.

**Итог:** код не менялся — строгая верификация. 2.4.6 AA (+ 1.3.1 иерархия) выполнен.
