...
| C3 | P2 | Sticky-шапка непоследовательна: `shell` sticky, остальные 5 — статичные | Хром обязан быть ОДНИМ компонентом через все экраны; sticky-нав = хорошее UX, применить единообразно (sticky + solid, без блюра) | все 6 | TODO |
| C4 | P3 | kbd-help модалка живёт только в `shell.html` | По контракту (§8) это глобальный оверлей экрана вопроса; при Фазе 2 привязать к `focus-question` | `shell.html`→`focus-question.html` | TODO |
| C5 | P4 | Входной ритм всех экранов одинаков (kicker + headline + grid) | Taste VARIANCE 6: набор ощущается системным, но монотонным; проверить, стоит ли дифференцировать вход по типу экрана (stats — данные вперёд, home — редакционный герой) без разрушения единства | все | TODO (оценить, не факт что дефект) |
| C6 | P4 | Подвал только на `shell` (домашний), у 5 задача-экранов его нет | Решить осознанно: задача-экраны намеренно без подвала (фокус) ИЛИ нужен слим-мета внизу; зафиксировать как решение, не «чинить» молча | все | TODO (решение) |
| C7 | — | focus-ring/reduced-motion/AA-каркас | Уже верифицированы по PROGRESS it.10–21; при каждой правке — re-check, не регресс | все | RECHECK |
