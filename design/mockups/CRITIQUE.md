...
### cr.164 — WCAG 3.3.2 Labels or Instructions (A): подписи/инструкции форм — verified-clean

**Вопрос:** когда контент требует ввода, предоставлены видимые подписи или инструкции.

**Метод (эмпирический, chrome-devtools):** обход всех контролов форм /settings (обе панели через клиентское переключение вкладок) и /stats с проверкой ВИДИМОЙ подписи (wrapping-`<label>` / `label[for]` / `aria-labelledby`→видимый элемент) и инструкций; спец-проверка условно-скрытых полей.

**Результат — 3.3.2 чисто:**
- **/settings → Оформление:** 7 групп (6 `radiogroup` + степпер шрифта) — все с видимыми подписями (Дизайн, Тема оформления, Размер шрифта, Раскладка, Ширина чтения, Плотность, Движение); 7 inline-подсказок.
- **/settings → Сессия:** 8 native-контролов — все с видимыми wrapping-`<label>` (Группа:/Тема:/Избранные/Ошибки/Микс тем/Слабые темы/Порядок:/Режим:).
- **Условно-скрытое поле `count`:** в режиме TRAINING его `<label>`«Вопросов:» `display:none` (бесконечная тренировка без числа) — не презентуется, вне 3.3.2. При EXAM поле показывается **с видимой подписью «Вопросов:»** (замерено переключением mode, восстановлено на TRAINING), min=1/max=200 (диапазон enforced браузером — самоочевиден).
- **/stats:** фильтр-чекбоксы «Избранные»/«Ошибки» — видимые wrapping-подписи; search-input — `placeholder`«Поиск по вопросам…» (видимая in-field инструкция) + `aria-label` + смежная кнопка «Искать» → назначение ясно (конвенциональный search-паттерн, 3.3.2 A удовлетворён).

**Наблюдение (не провал, не правил):** /stats search опирается на placeholder, а не на постоянный `<label>`. Приемлемо для search-паттерна (placeholder + labeled-submit); постоянная подпись была бы чуть надёжнее (placeholder исчезает при вводе), но конвенционально опускается для поиска. Не меняю.

**Итог:** код не менялся — строгая верификация. 3.3.2 A выполнен. Состояние восстановлено (mode→TRAINING).


### cr.163 — WCAG 1.4.5 Images of Text (AA): текст не подаётся картинкой — verified-clean

**Вопрос:** визуально отображаемый текст — это реальный текст, а не изображение текста (кроме логотипов и essential-случаев).

**Метод:** греп шаблонов/CSS на растровые источники + сканирование живого DOM трёх поверхностей (/, /settings, /stats) на `<img>`, `url()`-фоны, SVG с `<text>/<tspan>`, canvas; проверка реальности бренда/заголовков/подписей и наличия текст-эквивалента у графиков.

**Результат — 1.4.5 чисто:**
- **Нет растровых источников:** 0 `<img>` в шаблонах и в живом DOM всех страниц; 0 `background-image:url()` в base.css/tokens.css (фоны — только CSS-градиенты, не текст).
- **SVG-иконки:** спрайт Lucide через `<use>` (5 на /), **0 SVG с `<text>/<tspan>`** — иконки path-only, декоративные (`aria-hidden`), не изображения текста.
- **Реальный текст:** бренд «Cheat · Sheet» (`<a>` текстом, без img/svg-лого), h1, все заголовки/подписи, кнопки степпера «A−»/«A+» (текст+aria-label) — настоящий текст, масштабируется (см. R1.60).
- **Chart.js canvas** (/stats, 2 шт. `topicProgressChart`/`topicAccuracyChart`): текст на канвасе — часть **визуализации данных**, а не «текст-картинка» для вёрстки. Оба `role="img"` + описательный `aria-label` («Гистограмма … Полные данные — в таблице ниже»); полные данные доступны **реальным текстом** в таблице тем (319 строк). Text-эквивалент есть → 1.4.5 удовлетворён (essential-исключение для графиков + текст-альтернатива).

**Итог:** код не менялся — строгая верификация. 1.4.5 AA выполнен.
...
**Метод:** статический анализ обработчиков (авторитетно — критерий про привязку событий) + read-only проверка живого DOM. Греп down-event активации (`mousedown`/`pointerdown`/`touchstart`/inline `on*`) по всему static JS и шаблонам; проверка drag-механик; подтверждение нативности контролов.

**Результат — 2.5.2 чисто:**
- **Ноль down-event обработчиков** в `app.js`/`stats.js`/остальном static JS и шаблонах (греп пуст). Активация только через `click`-слушатели (15 в app.js, 3 в stats.js) + нативный submit формы.
- **Живой DOM:** 0 элементов с inline `onmousedown/onpointerdown/ontouchstart`, 0 `draggable=true`. Все семплированные контролы (nav-ссылки, theme-toggle, .btn, submit, back-to-top) — нативные `A`/`BUTTON`.
- **Следствие:** нативный `click` срабатывает на up-event, причём отменяем — увод указателя с цели до отпускания отменяет клик (нативное поведение браузера) = требование 2.5.2 «up-event + abort» выполнено конструктивно.
- **Бонус 2.5.7 Dragging Movements (WCAG 2.2 AA):** нет `draggable`, нет `pointermove`/`mousemove`-драга, нет range-слайдеров → drag-only функционала нет, тривиально чисто.

**Итог:** код не менялся — строгая верификация. 2.5.2 A выполнен. Состояние не мутировано (только греп + read-only DOM).


### cr.160 — WCAG 2.1.2 No Keyboard Trap (A): kbd-help оверлей выходим с клавиатуры — verified-clean

**Вопрос:** если фокус можно навести клавиатурой, его можно и увести стандартным механизмом — нет клавиатурной ловушки (особенно в модалках/оверлеях).

**Метод (эмпирический, chrome-devtools CDP press_key):** на живой / — записать опенер, «?» открыть kbd-help, замерить состояние оверлея и инертность фона, Tab (проверка утечки в фон), Esc (выход + возврат фокуса).

**Результат — 2.1.2 чисто:**
- **Открытие:** «?» с nav-ссылки «Фокус» → оверлей `role=dialog aria-modal=true`, фокус переходит на close-кнопку «×» внутри. Все дети `<body>` (skip-link/header/back-to-top/main) получают `inert`+`aria-hidden=true` (`backgroundFullyInert:true`).
- **Ловушка держит без утечки:** Tab оставил фокус на «×» внутри оверлея (в inert-фон не ушёл; в оверлее 1 фокусируемый).
- **Стандартный выход:** Esc → оверлей `hidden`, **фокус вернулся на опенер** («Фокус», `focusReturnedToOpener:true`), `inert`/`aria-hidden` с фона сняты (`backgroundInertRemoved:true`). Плюс альтернативные выходы: клик/Enter по «×», повторное «?».
- Соответствие 2.1.2: клавиатурный фокус покидает компонент документированным механизмом (Esc указан в самой справке).

**Вне зоны:** prompt-modal регенерации (`app.js:342` — своя focus-trap/Esc/возврат) не тестировался: рендерится под `th:if=aiEnabled`, а AI выключен (seed-first) → в DOM отсутствует. Roving tabindex вкладок /settings (R1.64) by design исключает ловушку в tablist (Tab выходит из группы).

**Итог:** код не менялся — строгая верификация. 2.1.2 A выполнен. БД/localStorage не тронуты (клиентские «?»/Tab/Esc).


### cr.159 — WCAG 4.1.2 Name/Role/Value (A): контролы /settings — verified-clean

**Вопрос:** у каждого UI-компонента программно определимы имя, роль и значение/состояние (кастомные ARIA-виджеты и icon-only кнопки — под особым подозрением).

**Метод (эмпирический, chrome-devtools):** обход всех интерактивных контролов трёх панелей (Сессия/Оформление/Данные) + шапки; вычисление accessible-name (aria-label → aria-labelledby → label[for]/обёртка → text минус aria-hidden), проверка role, состояния (`aria-checked`/`aria-pressed`/`aria-disabled`), roving tabindex, live-региона значения.

**Результат — 4.1.2 чисто:**
- **6 seg-control radiogroup** (Дизайн 11, Тема 3, Раскладка 3, Ширина 2, Плотность 2, Движение 3): каждая `role=radiogroup` + доступное имя (`aria-labelledby`); каждая опция `role=radio` + имя; **ровно один `aria-checked=true`** на группу; roving tabindex корректен (checked=`0`, прочие=`-1`); 0 пропущенных `aria-checked`, 0 безымянных.
- **Степпер шрифта:** `role=group` + имя «Размер шрифта»; кнопки с описательными `aria-label` («A−: уменьшить размер шрифта», «A+: увеличить размер шрифта», «Сбросить»); значение в `<span role=status aria-live=polite>100%</span>` (изменения озвучиваются); «Сбросить» `aria-disabled=true` при дефолтных 100% (корректный state).
- **9 нативных контролов панели Сессия** (select group/topic/ordered/mode, checkbox important/onlyWrong/shuffle/weakTopics, number count): все имеют доступное имя через label; implicit-роли корректны.
- **3 header-тоггла:** icon-only, но с имя-несущими `aria-label` c текущим значением: «Сменить раскладку (сейчас: Поток)», «Сменить дизайн (сейчас: Editorial)», «Включить светлую тему».

**Наблюдение (не провал, не правил):** `theme-toggle` несёт `aria-pressed="false"` на 3-состояночном (авто→светлая→тёмная) cycle-контроле, тогда как соседние cycle-тогглы layout/design его опускают — мелкая ARIA-нестыковка. Не провал (имя/role/value валидны, значение читается из имени), атрибут связан со state-логикой app.js (снятие из HTML сломало бы JS) → оставлено.

**Итог:** код не менялся — строгая верификация. 4.1.2 A выполнен. localStorage/дизайн не мутированы (контролы не кликались).


### cr.158 — WCAG 2.1.1 Keyboard (A): MCQ-флоу полностью операбелен с клавиатуры — verified-clean

**Вопрос:** весь функционал страницы вопроса доступен с клавиатуры без мыши (выбор варианта, навигация, снятие выбора, отправка), без клавиатурной ловушки.

**Метод (эмпирический, chrome-devtools CDP press_key — реальные клавиши):** на живой странице вопроса (GET `/?topic=…`) прогон последовательности «1 → ArrowDown → 3 → Esc» с замером `input[type=radio]:checked`, состояния submit и фокуса между нажатиями. Enter-сабмит проверен СТРУКТУРНО (без firing — POST /answer мутирует БД).

**Результат — 2.1.1 чисто:**
- **Автофокус:** при загрузке вопроса фокус садится на опцию[0] (app.js:1240) — шорткаты работают сразу, без ручного Tab.
- **«1-9» выбор:** «1» → опция A `checked`, submit включился (`disabled:false`). «3» → опция C. Шорткат **focus-gated** (`form.contains(activeElement)`, app.js:1221) — печатная цифра активна только при фокусе в форме = осознанное соответствие 2.1.4 Character Key Shortcuts (R1.44).
- **↑/↓ навигация:** ArrowDown с опции A → опция B (выбор и фокус синхронно двигаются, wrap по модулю).
- **Esc снятие:** снял выбор (`checkedCount:0`), submit снова `disabled`, фокус остался в форме на бывшей опции.
- **Enter отправка:** реальный `<button type=submit>` «Проверить ответ» в form `POST /answer`, tabbable, **disabled пока нет выбора** (защита от пустого сабмита) — нативно активируется Enter/Space. Не firing.

**Ложная тревога (объяснена):** первый прогон «1» ничего не выбрал — предшествующий baseline-скрипт вызвал `blur()`, фокус ушёл на BODY, и гвард 2.1.4 корректно заблокировал цифру. После reload (автофокус восстановлен) всё работает. Это подтверждает корректность гварда, а не дефект.

**Итог:** код не менялся — строгая верификация. 2.1.1 A выполнен. БД чиста (только клиентские radio-операции, без сабмита).
...
**Метод (эмпирический, chrome-devtools):** перебор всех фокусируемых (`a[href],button,input,select,textarea,[tabindex≥0],[role=tab]`), для каждого — `.focus()` и сравнение сигнатуры до/после: `location.href`, число открытых диалогов, кража фокуса (`activeElement` ушёл не на цель), переключение `role=tabpanel`, изменение `body.innerHTML.length` >200, а на странице вопроса — маркеры раскрытия (`.option-correct/.option-wrong/.status-*`), авто-`:checked` радио, смена вердикта.

**Результат — 3.2.1 чисто на всех поверхностях:**
- **/settings** — 54 контрола (ARIA-вкладки, seg-controls, тогглы, форма): 0 нарушений. Вкладки на **ручной активации** (фокус НЕ переключает панель — WAI-ARIA best-practice для существенного контента).
- **/** (дашборд) — 13 контролов: 0.
- **/stats** — 345 контролов (nav + 319 topic-ссылок + disclosure): 0.
- **Живая MCQ-страница вопроса** (GET `/?topic=…`, БД не мутирует) — 4 radio + 17 фокусируемых: **0 реальных**. Фокус на каждой radio-опции подтверждённо НЕ раскрывает ответ, НЕ авто-выбирает, НЕ меняет вердикт (`anyContextChangeOnFocus:false`, все 4 `isFocused:true`). Раскрытие только по явной активации (click/Enter+сабмит) — корректно.

**Ложные срабатывания (объяснены):** на странице вопроса тест дал 4 `FOCUS_STOLEN` на `<label>` — артефакт: `<label>` не фокусируемы, `.focus()` на них не переводит `activeElement`. Перепроверка на самих `input[type=radio]` подтвердила чистоту.

**Итог:** код не менялся — строгая верификация. 3.2.1 A выполнен. БД чиста (только focus/blur, без clic'ов/сабмитов).


### cr.155 — WCAG 1.4.4 Resize Text (AA): текст масштабируется до 200% без потери контента — verified-clean

**Вопрос:** весь текст ресайзится до 200% без потери контента/функциональности и без горизонтального скролла страницы.

**Метод (эмпирический, chrome-devtools):** строгий text-only resize — удвоение корневого `font-size` (16→32px, масштабирует только rem/em, жёстче page-zoom), затем замер: (1) коэффициент масштаба у выборки текстовых элементов, (2) `documentElement.scrollWidth vs clientWidth` (h-скролл), (3) элементы с `overflow:hidden` и `scrollWidth>clientWidth` (клиппинг), (4) `text-overflow:ellipsis` с обрезкой, (5) перенос seg-control, (6) иконки тогглов.

**Результат — 1.4.4 чисто на /, /settings, /stats:**
- **Текст масштабируется:** body/h2/h3/p/label/.btn/.seg-control button/a — ratio **2.0** (rem-based). `h1` на `clamp()` растёт 1.39× (осознанный потолок дисплейного размера — контент не теряется, читаемость сохраняется; реальный браузерный text-zoom множит поверх clamp).
- **Нет горизонтального скролла страницы** при 200% на всех трёх (scrollW==clientW==1265).
- **Клиппинг:** /settings дал 2 срабатывания — оба ложные (`visually-hidden "Сегодня к повтору"` и `.settings-panel-title "Оформление"`, который под js-tabs visually-hidden — оба SR-only, намеренный clip). / и /stats — 0.
- **Ellipsis-обрезки текста нет** нигде (0).
- **seg-control:** 0/6 переполнений при 200% — переносятся корректно.
- **Широкая таблица тем** (`.topic-table-wrap`) при 200% скроллится в своём `overflow-x:auto`-контейнере — санкционированный WCAG-паттерн для табличных данных (контент достижим, потери нет; см. аудит 2.4.11 cr.*).

**Наблюдение (не нарушение, не правил):** header icon-only тогглы (layout/design/theme) держат SVG-иконку ~13.3px и при 200% (их `font-size` = UA-дефолт, не наследует масштабированный root). Иконки — не «текст» по 1.4.4, находятся внутри 44px tap-target'ов, компактны намеренно. Правку не делаю: `font: inherit` изменил бы нормальный размер иконок в шапке во всех 11 дизайнах (blast radius shared header) при отсутствии WCAG-провала — спекулятивно.

**Итог:** код не менялся — строгая верификация. 1.4.4 AA выполнен.


### cr.154 — WCAG 2.4.4 Link Purpose (In Context): аудит + a11y-polish декоративных «→»

**Вопрос:** назначение каждой ссылки определяемо из её текста/программного контекста (пустые/иконочные без имени, неоднозначный текст, same-text→diff-href).

**Метод:** живой аудит `a[href]` на 3 страницах — accessible-name (aria-label/labelledby/видимый текст минус aria-hidden/title), детекция пустых, неоднозначных (здесь/тут/подробнее/→/» и т.п.), коллизий same-text→diff-href, дубль-имён.

**Результат — 2.4.4 чисто везде:**
- **/** (6 ссылок), **/settings** (6), **/stats** (327: 319 topic + nav/related/disclosure) — 0 пустых, 0 неоднозначных, 0 коллизий, 0 дубль-имён. Все topic-ссылки несут уникальное имя темы (ясная цель в контексте). nav/brand/skip — ясны.

**a11y-polish (сделан):** «→» в 9 CTA-лейблах был литеральным текстом → попадал в доступное имя (напр. «Начать повторение →»), тогда как декоративная стрелка — визуальный аффорданс, не часть имени; AT мог озвучивать «rightwards arrow». Обёрнуто в `<span aria-hidden="true"> →</span>` в 6 шаблонах (today-widget ×2, session-summary ×2, stats ×2, error, focus-training, result). Причина: чистые доступные имена + консистентность + меньше AT-шума. Динамические title-строки в header (`дизайн X → Y`, смысловая стрелка перехода) НЕ тронуты.

**Верификация:** git diff 6 файлов, 9/9 замен, голых «→» в лейблах не осталось; live /settings: CTA `accName="Начать повторение"` (стрелка убрана), `rawText="Начать повторение →"` (визуально видна) — зрячие видят «→», AT слышит чистое имя. Thymeleaf отрендерил без ошибок. Шаблоны server-rendered → version-bump не нужен; gradle пропущен (template-only + live-verified при живом bootRun).
