...
## DEC-006 — Прогрессивный пост-разбор (UX-13) — **NEEDS-USER (BLOCKED)**

**Статус:** OPEN · заблокировано: реализация «ядра» UX-13 конфликтует с осознанным решением «полный ответ доступен сразу». НЕ инвертирую молча.
...
**Собранный вопрос (задать вместе с BLK-BOOTRUN + DEC-003):** нужен ли прогрессивный пост-разбор, и если да —
(A) инвертировать раскрытие клиентом (полный разбор под клик; противоречит «доступно сразу»), либо
(B) добавить backend-поле «краткое почему» (needsGradle, отдельный tier), либо
(C) оставить как есть (вердикт + полный разбор сразу — текущая deliberate-модель, UX-13 закрыть verified-clean)?
До ответа UX-13-ядро BLOCKED; no-JS-consistency часть закрыта.
