...
public class PreloadService {
    private static final int MAX_PRELOAD_RETRY_ATTEMPTS = 2;
    private static final int TEST_MODE_DEFAULT_WARMUP_LIMIT = 10;
    ...
    private final TaskExecutor preloadExecutor;
    private final TaskExecutor warmupExecutor;
     ...
     */
    private final ConcurrentMap<String, PreloadQueue> queues = new ConcurrentHashMap<>();
        ...
        preloadExecutor.execute(() -> {
            try {
                List<Long> ids = questionRepository.findQuestionIdsForPreload(
                        ...
                        batch, optionGenerationService.requiredOptionsCount());
                ...
                RetryStats stats = ensureOptionsWithRetryQueue(questions, queue, false);
                log.info("Preload batch completed: requested={}, saved={}, accepted_with_warnings={}, reduced_options={}, hard_invalid={}, requeued={}, failed_after_retry={}",
                        questions.size(), stats.saved(), stats.acceptedWithWarnings(), stats.reducedOptions(),
                        stats.hardInvalid(), stats.requeued(), stats.failed());
            } catch (Exception e) {
        ...
        warmupExecutor.execute(() -> {
            try {
                List<Long> allIds = questionRepository.findAllQuestionIdsWithoutOptions(
                        optionGenerationService.requiredOptionsCount());
                if (allIds.isEmpty()) {
                ...
                List<Question> questions = loadQuestionsPreservingOrder(ids);
                RetryStats stats = ensureOptionsWithRetryQueue(questions, null, true);
                ...
                log.info("Полный прогрев завершён: total={}, saved={}, accepted_with_warnings={}, reduced_options={}, hard_invalid={}, requeued={}, failed_after_retry={}, elapsed={} сек",
                        questions.size(), stats.saved(), stats.acceptedWithWarnings(), stats.reducedOptions(),
                        stats.hardInvalid(), stats.requeued(), stats.failed(), totalSec);
            } catch (Exception e) {
    ...
    public void preloadQuestions(List<Long> ids) {
        preloadExecutor.execute(() -> {
            List<Question> questions = loadQuestionsPreservingOrder(ids);
            RetryStats stats = ensureOptionsWithRetryQueue(questions, null, false);
            log.info("Preload specific questions completed: requested={}, saved={}, accepted_with_warnings={}, reduced_options={}, hard_invalid={}, requeued={}, failed_after_retry={}",
                    questions.size(), stats.saved(), stats.acceptedWithWarnings(), stats.reducedOptions(),
                    stats.hardInvalid(), stats.requeued(), stats.failed());
        });
    }

    private RetryStats ensureOptionsWithRetryQueue(List<Question> questions, PreloadQueue queue, boolean generateDiagram) {
        LinkedList<Question> pending = new LinkedList<>(questions);
        Map<Long, Integer> attempts = new ConcurrentHashMap<>();
        int saved = 0;
        int acceptedWithWarnings = 0;
        int reducedOptions = 0;
        int hardInvalid = 0;
        int requeued = 0;
        int failed = 0;

        while (!pending.isEmpty()) {
            Question question = pending.poll();
            int attempt = attempts.getOrDefault(question.id(), 0) + 1;
            attempts.put(question.id(), attempt);
            try {
                OptionGenerationService.GenerationOutcome outcome = optionGenerationService.ensureOptionsWithOutcome(question);
                if (queue != null) {
                ...
                saved++;
                if (outcome.acceptedWithWarnings()) {
                    acceptedWithWarnings++;
                }
                if (outcome.reducedOptions()) {
                    reducedOptions++;
                }
            } catch (AiGenerationException e) {
                if (attempt <= MAX_PRELOAD_RETRY_ATTEMPTS) {
                    requeued++;
                    pending.offer(question);
                    log.warn("Question {} requeued for preload retry ({}/{}): {}",
                            question.id(), attempt, MAX_PRELOAD_RETRY_ATTEMPTS, e.getMessage());
                } else {
                    failed++;
                    hardInvalid++;
                    log.error("Question {} failed after preload retries: {}", question.id(), e.getMessage());
                }
            } catch (Exception e) {
                failed++;
        ...
        return new RetryStats(saved, acceptedWithWarnings, reducedOptions, hardInvalid, requeued, failed);
    }
