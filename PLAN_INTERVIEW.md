...
- ⏳ Следующая цель round 3: верх матрицы §7 (CAR=11 тир, исключая занятые параллельной сессией) — `long-context-vs-rag`/`load-balancing`/`authentication-authorization-patterns`/`application-profiling`/… Продолжать воркфлоу-бэндами ≤3 concurrent с offset-band против гонки; после каждого батча — `gen-plan-matrix.py --write`.
