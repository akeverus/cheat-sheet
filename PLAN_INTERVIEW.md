...
### ROUND-8 · selenium · c4 stub-expand 4 блоков (Q1,Q4,Q5,Q8) → **FINAL** — 2026-07-15
- **Диагноз:** 4 оставшихся stub-блока — длинный correct (26–42 сл, 2–3 предл.) + 3 коротких односложных дистрактора (9–15 сл). Давали ВСЕ остаточные fail: OPTION_LENGTH 4.44, CORRECT_WRONG 1.122, SENTENCE 2, skel HARD Q1/Q5/Q8, STYLE 0.417.
- **Правка:** 12 дистракторов развёрнуты до формы correct (совпадение skel-марок + предложений + код-спаны + реальная мисконцепция), correct БАЙТ-ЛОК. Мисконцепции: Q1 (direct-JS-no-protocol / record-playback-no-API / is-the-browser-no-driver), Q4 (session-expires-on-idle / spurious-wakeup-early / WebDriverWait=fixed-sleep-inside), Q5 (reporting-artifacts-store / assertions-inside-PO / one-shared-static-class), Q8 (waiting-lib-replaces-Wait / sequential-single-machine / Chrome-only).
- **Способ (dynamic Workflow — ускорение по запросу юзера):** pipeline по 4 блокам, стадия author (расширить 3 дистрактора под spec) → стадия blind (независимый ревьюер шафла), 4 блока concurrent, 8 агентов. Q1/Q5/Q8 → blind ACCEPT сразу. **Q4 → workflow-REVISE (каррикатура:** исходные stub-мисконцепции «sleep закрывает сессию»/«не приостанавливает поток»/«не хуже Wait» слишком абсурдны) → **переавторен вручную** 3 правдоподобными-но-ложными диагнозами, свежий блайнд → ACCEPT.
- **Гейты c4: ВСЁ ЗЕЛЁНОЕ — parity PASS 1/1** (OPTION_LENGTH 1.33, CORRECT_WRONG 0.923, SENTENCE 1, DUPLICATE 2, STYLE 0.217, POS SEQ 2, SOURCE ok), **skel HARD 0.** ΣCAR=0.
- **selenium → FINAL (15/15 блоков реаудировано с блайндом).** ДАЛЬШЕ: следующий worst-first tractable файл по `gen-plan-matrix.py`.
- **УРОК (stub-expand):** короткие односложные дистракторы = отдельный класс работы (не де-дуп, а авторинг контента); параллельный workflow-авторинг эффективен, НО inherited-stub-мисконцепции при разворачивании легко читаются как каррикатура — блайнд ловит, переавторивать правдоподобными-но-ложными «диагнозами» (subtle domain-wrong, напр. spurious-wakeup для sleep).

### ROUND-8 · spring-boot-3-migration · c1 bootstrap (POS de-cycle + freshness/review sidecars) — 2026-07-15
- **Выбор:** следующий worst-first tractable по `gen-plan-matrix.py` после selenium FINAL — `frameworks/spring/spring-boot-3-migration` (9 gate-fails, OPTION_LENGTH 3.56 — худший среди мелких tractable, 15 блоков). Чист, без сайдкаров, последний трогавший коммит 8a7f390a (чужой seed-add, не конфликт).
- **Диагноз (класс дефекта):** фактически СИЛЬНЫЙ файл (correct длинные/детальные со списками+кодом; мисконцепции дистракторов уже добротные — взяты из `sections`, НЕ карикатура), но все 15 блоков stub-паттерн (correct 2-3 предл. + 3 коротких однопредложных дистрактора). Baseline: CORRECT_LONGEST 0.867, OPTION_LENGTH 3.56, CORRECT_WRONG 1.983, SENTENCE 2, DETAIL 1.0, ABSOLUTE 0.311, POS_SEQUENCE 15 (строгий ABCD-цикл), SOURCE missing, STYLE 0.933. DUPLICATE 0 (ok), DISTRIBUTION 1.33 (ok). ΣCAR=0.
- **Правка c1:** (1) POS de-cycle — per-block циклическая ротация опций к BASE15=[2,0,3,1,0,2,1,3,2,0,3,1,0,2,1]; контент КАЖДОЙ опции байт-идентичен HEAD (только label+order) → blind НЕ нужен. SEQ 15→2, NEW SEQ CADBACBDCADBACB, DISTRIBUTION 1.33. (2) freshness-sidecar (Spring Boot 3.x, checked_date 2026-07-15) → SOURCE_COVERAGE ok. (3) review-sidecar bootstrap.
- **Гейты c1:** POS SEQUENCE 15→2, SOURCE_COVERAGE ok. Остаются контент-fail для c2: CORRECT_LONGEST 0.867, OPTION_LENGTH 3.56, CORRECT_WRONG 1.983, SENTENCE 2, DETAIL 1.0, ABSOLUTE 0.311, STYLE 0.933.
- **ДАЛЬШЕ (c2, рывок к FINAL):** stub-expand всех 15 блоков — 3 дистрактора/блок развернуть до формы длинного correct (skel-марки + предложения + код-спаны + СОХРАНЕНИЕ добротной мисконцепции из sections + opening_class), correct БАЙТ-ЛОК. Через dynamic Workflow author→blind по всем 15 блокам. Риск каррикатуры НИЗКИЙ (мисконцепции уже плотные).

### ROUND-8 · spring-boot-3-migration · c2 stub-expand 15 блоков + 3 correct-fix + де-карикатура — 2026-07-15
- **Способ:** dynamic Workflow (author→blind pipeline, 30 агентов, 0 ошибок, 224s) по всем 15 блокам — 3 stub-дистрактора/блок развёрнуты до формы длинного correct. c2-workflow blind: **6 ACCEPT сразу** (Q5/Q6/Q7/Q9/Q11/Q14), **9 REVISE**.
- **3 FACT_ERR в byte-locked correct (§1 факт>байт-лок — ревьюер поймал реальные ошибки, исправил):** Q8-D завышенный «ускоряет старт на 30-50%» (реальный AOT-на-JVM выигрыш скромный) → «ощутимо ускоряет»; Q12-B «реактивный мост через `exchange`» (RestClient.exchange() синхронный, НЕ реактивный) → «синхронный `exchange`»; Q13-A «`spring.factories` ещё поддерживается ради совместимости» (в 3.0 регистрация авто-конфигов через него УДАЛЕНА — это поведение 2.7) → «в 3.0 удалена (в 2.7 deprecated)». Правились и text, и sections.edge_cases.
- **7 карикатурных дистракторов смягчены** (inherited stub-мисконцепции при разворачивании читались абсурдно): Q1-A (org.springboot-rename→частичный Spring-package-rename), Q1-B (MVC-removed→WebFlux-default+MVC-в-поддержке), Q3-C (mvn migrate-3 magic→OpenRewrite-silver-bullet-без-ручной-проверки), Q12-A (тихо-возвращает-null→retrieve()-бросает-only-5xx), Q13-B (auto-config-disabled-все-@Bean→opt-in-флагом-spring.autoconfigure.enabled), Q15-A (rollback-Java8→pin-Tomcat9), Q15-D (Java17-syntax-incompat→JPMS-module-info-required). form_guess по code-density НЕ гнал в карикатуру (§30-inherent).
- **Метрик-твики:** OPTION_LENGTH (Q4-A trim, Q15-A/D raise, Q13-D trim) + SENTENCE (Q13-D→3 предл.) + ABSOLUTE (снял не-несущие абсолюты Q1-B/D, Q4-A, Q15-D) + skel (Q15-A `;`-снят).
- **re-blind 7 контент-изменённых** (Q1,Q3,Q4,Q8,Q12,Q13,Q15, свежий SEED, workflow) → 6 ACCEPT + Q13 REVISE (ещё карикатура file-B «auto-config отключена») → смягчил (opt-in-флагом) → Q13 отдельный re-blind ACCEPT. **Итог 15/15 blind-реаудировано.**
- **Гейты c2: все стандартные ЗЕЛЁНЫЕ** — OPTION_LENGTH 1.29, SENTENCE 1, ABSOLUTE 0.222, CORRECT_LONGEST 0.2, CORRECT_WRONG 1.04, DUPLICATE 0, POS SEQ 2/DIST 1.33, SOURCE ok, skel HARD 0. **Остаток: DETAIL_PARITY 0.867 + STYLE_GUESSABILITY 0.767 (code-density)** — энциклопедичность byte-locked correct (несёт больше backtick-спанов); §30-inherent, факты чисты по единогласию ревьюеров.
- **ДАЛЬШЕ (c3 → FINAL):** DETAIL/STYLE code-density enrichment — обогатить дистракторы в ~8 code-плотных блоках (Q1/Q2/Q3/Q5/Q6/Q9/Q10/Q13) правдоподобными API-терминами в backtick, чтобы correct не угадывался по плотности кода → STYLE/DETAIL зелёные → spring-boot-3-migration FINAL. Каждый изменённый блок с re-blind.
- **УРОК c2:** (1) свежий blind-ревьюер ловит РЕАЛЬНЫЕ фактические ошибки в byte-locked correct (устаревшие/завышенные факты) — §1 факт>байт-лок, исправлять correct+sections, это улучшает файл сверх parity; (2) form_guess по code-density на энциклопедичном byte-locked correct = §30-inherent (не гнать в карикатуру); реальный рычаг = обогащение дистракторов кодом (c3), а не урезание correct.

### ROUND-8 · spring-boot-3-migration · c3 code-density enrichment + c4 length/detail rebalance → **FINAL** — 2026-07-15
- **c3 (dynamic Workflow enrich→blind, 20 агентов):** 10 code-доминантных блоков (Q1,2,3,4,5,6,9,10,11,13, где correct нёс больше backtick-спанов) — 3 дистрактора/блок обогащены РЕАЛЬНЫМИ Spring/Java backtick-идентификаторами в ЛОЖНОМ контексте до code-density ~correct (тезис/мисконцепция сохранены, correct БАЙТ-ЛОК). **10/10 blind ACCEPT** — ни один вписанный код не сделал дистрактор случайно верным. Итог: STYLE code-spans 0.77→0.27, DETAIL 0.867→0.467 (code больше не доминирует; остаток — parens/nums).
- **c4 (dynamic Workflow rebal→blind, 8 агентов + ручная доводка):** c3 раздул/перекосил прозу-длину (OPTION_LENGTH 1.84 Q4, STYLE by-length 0.48 — correct стал самым длинным в ~9 блоках). Выровнял: Q3 (bump A), Q4 (trim A/C/D), Q9 (raise A>correct), Q13 (raise B>correct) — 4/4 ACCEPT. Q4-агент пере-ужал прозу (11-12 слов при correct 19) → **ручная пере-балансировка Q4 A/C/D** до ~19-21 прозы-слов (nsent=2, marks [':','('] как correct, тезисы сохранены). DETAIL 0.467 (7/15 parens/nums байт-локнутого correct) → un-dominate Q6 естественным HTTP-статусом `500` в дистракторе A (уместно для Problem Details) → DETAIL 0.40 (6/15).
- **re-blind Q4+Q6** (после ручных правок, свежий SEED) → **2/2 ACCEPT** (single_correct, no factual_error, no other_defensible, no caricature, no form_guess).
- **Гейты FINAL: answer-parity 1/1 PASS** — OPTION_LENGTH 1.33, CORRECT_LONGEST 0.267, CORRECT_WRONG 1.05, SENTENCE 1, ABSOLUTE 0.133, POS 2/1.33, DUPLICATE 0, SOURCE ok, **DETAIL 0.40, STYLE 0.35**; skel HARD 0. Correct байт-лок к HEAD во всех 15 блоках. **15/15 блайнд-верифицировано (c2+c3+c4). spring-boot-3-migration FINAL.**
- **УРОК c3/c4:** обогащение дистракторов кодом (c3) устраняет code-density guessability БЕЗ урезания энциклопедичного correct, но перекашивает прозу-длину → нужен парный c4 length-rebalance. Workflow-агенты при «сократи длину» склонны выкидывать прозу и оставлять код (прозу-слова падают, код держится) — целевой каунт задавать явно как **прозу-слова (вне backtick)**, иначе получается 11 прозы-слов при 12 код-фрагментах. DETAIL parens/nums = реальные обучающие глоссы correct (`(JDBC API)`/`(JMX)`); un-dominate точечным естественным числом/глоссом в 1 дистракторе, а не массовым стаффингом (§1: DETAIL LOW, не портить читаемость).

### ROUND-8 · mutation-testing · c7 de-dup + de-caricature + DETAIL/skel regression-fix — уже-FINAL, honest-green — 2026-07-15
- **Контекст:** файл был помечен FINAL в c6, но принятый accepted-профиль §30 содержал DUPLICATE_NGRAMS=92 и OPTION_LENGTH_RATIO=1.64 (Q4) — minimal-pair блоки делили общий 10-словный каркас. c7 довёл до **честно-зелёного**, а не accepted.
- **c7 (dynamic Workflow de-dup→blind, 13 блоков Q2-Q20):** разведены формулировки общего каркаса (вводные/определения-обёртки/хвосты) РАЗНЫМИ словами при сохранении дискриминирующего ложного тезиса → **DUPLICATE 92→0, OPTION_LENGTH 1.64→1.35, STYLE 0.421→0.387**. 20/20 блоков blind-ACCEPT.
- **c7b (re-author Q14/Q17):** blind поймал остаточную каррикатуру (de-caricature c7 не до конца сняла dismissive-тон) → переписал дистракторы под явные plausible-false тезисы (неверные language-маппинги Gremlins↔Cosmic Ray; оверобобщения «ценен для любого кода»), запретив обороты «не существует/миф/заброшен/анализ не нужен» → ACCEPT.
- **РЕГРЕСС от de-dup (c7c) и его фикс (c7d):** de-dup/trim переписывания сорвали структурный паритет дистракторов → **DETAIL 0.45 (baseline 0.1!) и skel HARD 0→3** (Q10/Q11/Q12 semicolon_presence). Хирургическое восстановление: Q10 B/C/D — вернул `{ i++; }` (семиколон-паритет к байт-локнутому correct A); Q11-D/Q12-D — убрал лишний `;` + добавил parens дистрактору, чтобы correct C (parens=3) не доминировал по DETAIL (delta≥2 снят) → **DETAIL 0.35, skel HARD 0**. re-blind Q10/Q11/Q12 (SEED=Q*7717+91, 3 незав. ревьюера, свежий сид) → **3/3 ACCEPT** (single_correct, correct_label=expected, no flags).
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ метрики зелёные** — OPTION_LENGTH 1.35, DUPLICATE 0, DETAIL 0.35, CORRECT_LONGEST 0.15, CORRECT_WRONG 0.98, SENTENCE 1, ABSOLUTE -0.1, POS 1.0/3, STYLE 0.387, SOURCE ok; skel HARD 0. Correct байт-лок к HEAD во всех 20 блоках. **mutation-testing честно-FINAL (accepted-профиль §30 устранён).**
- **УРОК c7:** de-dup minimal-pair каркасов (переформулировка обёртки) — правильный путь убить DUPLICATE, НО переписывание легко срывает структурный паритет (parens/semicolons/code-spans) дистракторов относительно байт-локнутого correct → регресс DETAIL и skel. Правило: **при разведении формулировок сохранять не только длину/тезис, но и punctuation/paren/code-профиль каждого дистрактора ≈ correct** (иначе correct начинает доминировать по DETAIL, а уникальный `;` correct'а ломает skel-паритет). Не REGRESS-ить метрику, которая на baseline проходила (§1 corollary).

### ROUND-8 · java-reflection · c1 bootstrap + c2 enrichment 16 блоков + c3 re-author/trim/opening → **FINAL** — 2026-07-15
- **Выбор файла:** worst-first tractable по live-скану answer-parity (`gen-plan-matrix.py` не находит маркеры → скан всех 318 сидеров через API гейта, фильтр ≤20 блоков + пропуск FINAL-sidecar). `programming-languages/java/java-reflection` — 9 провальных метрик (худший tractable), 16 блоков, sidecar не было. Параллельной MCQ-сессии нет (все pedago-коммиты мои) → offset не нужен.
- **Профиль baseline = «сырой»:** файл фактически ВЫСОКОГО качества (все correct — точные каноничные ответы), но дистракторы = короткие однофразные мисконцепции → correct систематически самый богатый/длинный: CORRECT_LONGEST 0.875, OPTION_LENGTH 2.44, CORRECT_WRONG_AVG 1.42, DETAIL 1.0, STYLE 1.0, ABSOLUTE_MARKER_GAP 0.271 (дистракторы абсолют-тяжёлые), POS_SEQUENCE 16 (строгий ABCD), skel HARD 11, SOURCE missing.
- **c1 bootstrap (детерминированно, без блайнда):** POS de-cycle (TARGET seq maxrun=1, content байт-идентичен HEAD, 15/16 сменили позицию) → SEQUENCE 16→2; freshness sidecar (Java SE 21) → SOURCE ok; review sidecar.
- **c2 enrichment (dynamic Workflow enrich→blind, 32 агента):** все 16 блоков — дистракторы обогащены РЕАЛЬНЫМИ `java.lang.reflect`-идентификаторами в ЛОЖНОМ контексте до длины/код-плотности/пунктуации correct + де-абсолют, ложный тезис сохранён, correct БАЙТ-ЛОК. **15/16 blind-ACCEPT сразу**; Q2 REVISE (каррикатура: «литерал `String.class` возвращает `Field`» — абсурд). Итог метрик: CORRECT_LONGEST 0.875→0.125, DETAIL 1.0→0.375, STYLE 1.0→0.24, ABSOLUTE→норма.
- **c3 (dynamic Workflow 5 блоков + ручные правки):** (a) Q2 re-author B/C/D → правдоподобные заблуждения (getClass-основной / forName-без-инициализации / TYPE==class-синонимы); (b) length-trim over-enriched Q3/Q4/Q8/Q11 (OPTION_LENGTH 2.4→1.33; Q3 correct=10 прозы-слов но 13 код-спанов → дистракторы ужаты до ~12 прозы, код сохранён); (c) opening-class нормализация Q3/Q7/Q12/Q14/Q15 (skel HARD 11→0, formatting-only prose↔btick); (d) точечные: Q2-B `;`→`:`, Q11-C +длина (min-outlier), Q12-A +paren gloss (DETAIL un-dom 7→6). **5/5 c3 blind ACCEPT + 3/3 hand-edit reblind ACCEPT + 3/3 opening reblind (Q7/Q14/Q15) ACCEPT.** Correct БАЙТ-ЛОК во всех 16 блоках.
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ метрики** — CORRECT_LONGEST 0.25, OPTION_LENGTH 1.33, CORRECT_WRONG 1.03, SENTENCE 1, DETAIL 0.375, ABSOLUTE -0.062, POS 1.0/2, DUPLICATE 0, SOURCE ok, STYLE 0.354; skel HARD 0. **16/16 блоков блайнд-верифицировано. java-reflection FINAL.**
- **УРОК enrichment (raw-профиль):** для файла «богатый correct + бедные дистракторы» фикс = поднять дистракторы до richlevel correct плаузибл-ложью с РЕАЛЬНЫМИ API-идентификаторами в ложном контексте (не трогая correct) — чинит CORRECT_LONGEST/OPTION_LENGTH/DETAIL/STYLE/ABSOLUTE/skel разом. Grъли: (1) над-обогащение перекашивает в ОБРАТНУЮ сторону (Q3/Q4/Q8 дистракторы стали длиннее короткого code-dense correct → нужен парный trim); (2) enrichment-агент может выдать каррикатуру при агрессивном «обогати ложь» (Q2) — блайнд ловит, переавторивать subtle-wrong; (3) opening-class (skel HARD) = кто открывается backtick-идентификатором vs прозой — детерминированная formatting-нормализация, но переставлять клаузы с сохранением `;`-паритета.

### ROUND-8 · estimations-planning (leadership) · c1 bootstrap + c2 enrichment 20 + c3 numeric-detail + c4 residual + c10 de-dup → **FINAL** — 2026-07-15
- **Выбор файла:** worst-first tractable по live-скану answer-parity (`gen-plan-matrix.py` не находит маркеры → скан всех 318 сидеров через API гейта, фильтр ≤20 блоков + пропуск FINAL-sidecar). `leadership/estimations-planning` — 20 блоков, sidecar не было. Тот же raw-профиль, что java-reflection/mutation-testing, но **проза, не код**.
- **Профиль baseline = «сырой» прозаический:** все correct — богатые многофразные каноничные ответы (40–81 сл), дистракторы = короткие однофразные мисконцепции (11–42 сл) → correct доминировал длину/детали/предложения: OPTION_LENGTH 3.82, CORRECT_WRONG_AVG 1.99, SENTENCE 4, POS строгий ABCD.
- **c1 bootstrap (детерминированно, без блайнда):** POS de-cycle (TARGET=[2,0,3,1,1,3,0,2,0,2,1,3,3,1,2,0,1,3,0,2], арифметический maxrun 1, распределение 5/5/5/5, content байт-идентичен HEAD); freshness sidecar (Agile/Scrum estimation, methodology-stable 2024–2026: Scrum Guide 2020 / Cohn / McConnell) → SOURCE ok; review sidecar.
- **c2 enrichment (dynamic Workflow enrich→blind, 40 агентов):** все 20 блоков — дистракторы развёрнуты до длины/предложений correct правдоподобным «обоснованием практика» (story points/velocity/planning poker/cone of uncertainty в ложном контексте) + де-абсолют, ложный тезис сохранён, correct БАЙТ-ЛОК. **20/20 blind-ACCEPT.** Итог: OPTION_LENGTH 3.82→1.27, CORRECT_WRONG 1.07, SENTENCE 1, но всплыли CORRECT_LONGEST 0.45 / DETAIL 0.8 / STYLE 0.567 (correct богаче по числам/скобкам).
- **c3 numeric-detail (dynamic Workflow, 34 агента):** в 16 доминирующих блоках ОДИН дистрактор обогащён конкретной плаузибл-ложной числовой деталью в скобках (курс `1 sp ≈ 4 ч`/шкала/буфер/множитель) + сделан длиннейшим → un-dom DETAIL+CORRECT_LONGEST+STYLE разом. **16/17 blind-ACCEPT**; Q10 REVISE. Итог: CORRECT_LONGEST 0.05, DETAIL 0.05, STYLE 0.175.
- **c4 residual (dynamic Workflow, 30 агентов):** c3 переудлинил по 1 дистрактору (12 блоков OPTION_LENGTH>1.35) + skel HARD (opening-class 7, semicolon 4) + SENTENCE 2. 15 блоков: точечный trim переудлинённого дистрактора (сохраняя числа/скобки) + opening-class нормализация odd-дистрактора к классу correct + semicolon/sentence-паритет (global max-min). **14/15 blind-ACCEPT**; Q10 снова REVISE.
- **c10 Q10 hand-fix + reblind:** блок дважды REVISE — причина `dup_scaffold`: A и D несли ОДНУ мисконцепцию («отпуска уже усреднены в velocity») → близнецы, correct C выделялся (`form_guess`); D открывался прозой (odd). A разведён в отдельную мисконцепцию (focus factor = ненужная перестраховка/Паркинсон), D оставлен на velocity-double-count, оба открыты backtick'ом. re-blind (SEED=51771, свежий, correct скрыт): **ACCEPT** — correct=B совпал, dup_scaffold=false («разные механизмы, не близнецы»), form_guess=false. Correct C БАЙТ-ЛОК.
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ метрики** — CORRECT_LONGEST 0.10, OPTION_LENGTH 1.27, CORRECT_WRONG 1.01, SENTENCE 1, DETAIL 0.05, ABSOLUTE -0.183, POS 1.0/4, DUPLICATE 0, SOURCE ok, TEMPORAL 3, STYLE 0.175; skel HARD 0. Correct байт-лок к HEAD во всех 20 блоках (текст правильных байт-идентичен, менялись только метки/порядок c1-де-циклом). **20/20 блоков блайнд-верифицировано (c2+c3+c4+c10). estimations-planning FINAL.**
- **УРОК прозаический raw-профиль:** для «богатый многофразный correct + бедные однофразные дистракторы» на ПРОЗЕ (не коде) фикс = 4-стадийный: c2 объёмная параллель прозой → c3 числовая деталь в скобках для un-dom DETAIL (Fibonacci/проценты/курсы как плаузибл-ложь) → c4 trim переудлинённого + формат-паритет (opening/semicolon/sentence) → c10 de-dup близнецов-дистракторов. Grъли: (1) c3-обогащение «сделай длиннейшим» переудлиняет → парный c4 trim; (2) enrichment всех дистракторов до параллели легко создаёт СМЫСЛОВЫХ близнецов (две разные формулировки одной мисконцепции) → blind ловит dup_scaffold, разводить в РАЗНЫЕ механизмы; (3) SENTENCE_PARITY меряется global max-min по 4 опциям, не vs-correct — стягивать крайние дистракторы к correct.

### ROUND-8 · zero-trust (security) · c1…c7 + lean-blind финал + Q13 факт-фикс/de-caricature → **FINAL** — 2026-07-15
- **Выбор файла:** worst-first tractable по live-скану answer-parity, 19 блоков, sidecar не было. Тот же raw-профиль (богатый correct + бедные дистракторы), плюс редкий case **code_bucket-vs-byte-locked-correct**.
- **c1 bootstrap:** POS de-cycle (TARGET=[0,1,3,2,0,1,3,2,0,1,3,2,0,1,3,2,0,2,1], maxrun 1; correct-текст байт-идентичен HEAD) + freshness sidecar (Zero Trust, concept-stable 2024–2026: NIST SP 800-207 / BeyondCorp / CISA ZT Maturity Model 2.0) + review sidecar. Correct-метки: `ABDCABDCABDCABDCACB`.
- **c2 enrichment:** дистракторы развёрнуты до паритета ложным контекстом (PEP/PDP/mTLS/microsegmentation/IdP в неверном применении) + де-абсолют, correct БАЙТ-ЛОК. 16 блоков ACCEPT.
- **c3a de-caricature Q1/Q5/Q9:** blind поймал каррикатуры (Q1-D «ZT требует отказаться от шифрования» — самоопровержение; Q5-A подмена темы виртуализацией; Q9 dup_scaffold) → переписаны в правдоподобные-ложные, разведены. 3/3 ACCEPT.
- **c3b numeric-detail:** un-dom DETAIL/CORRECT_LONGEST/STYLE; 17 применено, Q1 REVISE (ре-каррикатура) → оставлена c3a-версия.
- **c4 residual:** trim + opening/semicolon/sentence-паритет; 16 применено, Q14 REVISE (абсурд «IdP 5–7 факторов») → c5.
- **c5:** de-caricature Q14-D (убран абсурд, тезис «начинать с сети, identity откладывать» оставлен как реальное заблуждение) + Q12-A trim + Q3-B/Q17-B semicolon. 4/4 ACCEPT.
- **c6 code_bucket:** Q3/Q17/Q18 — correct байт-лок с 0 backtick, дистракторы code-rich → skel HARD. Детерминированный strip лишних backtick до ≤2 spans (текст терминов сохранён). Correct не тронут.
- **c7:** Q18-D trim + Q8 un-dom DETAIL (числовая деталь в скобках одному дистрактору). 2/2 ACCEPT.
- **lean-blind ФИНАЛ (lean-протокол 2026-07-15):** Q1–Q11 × 3 линзы — все n_blocking:0. Q12–Q19 × 1 combined-lens (8 агентов): 6 clean; **Q13 blocking(fact)** в верном A («Tailscale/Twingate — на WireGuard»: на WireGuard только Tailscale, у Twingate свой протокол); **Q19 blocking(caricature)**.
- **Q13 факт-фикс (§1 факт-корректность > byte-lock):** в верном A убрана ложная привязка Twingate↔WireGuard — **единственный блок, где correct-текст отличается от HEAD** (правка минимальна). Re-blind: fact устранён, НО дистрактор D помечен каррикатурой (enterprise `ZPA` как «бесплатный open-source за 10–15 мин для домашних лабораторий» — инверсия salient-факта) → D переписан в «ZPA = сетевой VPN-концентратор, доступ по подсетям, не per-application» + trim (OPTION_LENGTH 1.45→1.19). **Confirm-blind Q13 (1 агент): ACCEPT, no flags.**
- **Q19 — принят по §1-суждению (НЕ правлен):** caricature-флаг признан избыточной строгостью/стохастикой ревьюера (на первом lean-прогоне тот же Q19 был blocking:false). Собственный разбор: все три дистрактора (общий статический API-ключ+доверенный периметр / кластер-периметр=достаточно / default-allow) — РЕАЛЬНЫЕ service-to-service анти-паттерны, не соломенные чучела. Слепой ревью — подспорье, не оракул.
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.263, OPTION_LENGTH 1.35 (0 блоков >1.35), DETAIL 0.368, SENTENCE 1, STYLE 0.325, DUPLICATE 0, SOURCE ok, TEMPORAL ok; skel HARD 0 warn=19. Correct байт-лок к HEAD во ВСЕХ блоках, КРОМЕ Q13 (факт-правка по §1). **19/19 блоков блайнд-верифицировано. zero-trust FINAL.**
- **УРОК lean-протокол (2026-07-15, feedback юзера «почему так много токенов/агентов на файл»):** перерасход шёл от (1) связанных гейтов → whack-a-mole-раунды, (2) адверс-блайнда 3-линзы на КАЖДОМ раунде, (3) перфекционизма по всем 12 метрикам. НОВЫЙ дефолт: один финальный блайнд + точечные confirm-блайнды (1 агент) только на реально правленных блоках; §1-суждение вместо лишнего раунда на стохастичный флаг (Q19); косметика — только дёшево попутно.

### ROUND-8 · spring-ai (frameworks/spring) · c1 bootstrap + one-pass enrich + Q2/Q12 факт-фикс + hand-residual → **FINAL** — 2026-07-15
- **Выбор файла:** worst-first tractable по live-скану answer-parity, 18 блоков, sidecar не было. Raw-профиль (богатый correct + бедные дистракторы), смешанный код+проза.
- **c1 bootstrap:** POS de-cycle (TARGET=[0,1,3,1,0,2,2,1,3,0,2,1,3,0,2,1,3,0], maxrun 1, распределение 5/5/4/4; correct-текст байт-идентичен HEAD) + freshness sidecar (Spring AI 1.0.x GA — май 2025: docs.spring.io/spring-ai/reference, GitHub, Spring Blog GA) + review sidecar. Correct-метки: `ABDBACCBDACBDACBDA`.
- **c2 one-pass enrich (dynamic Workflow pipeline enrich→inline blind, 36 агентов):** ОДИН авторский проход на блок — дистракторы развёрнуты до паритета длины/предложений правдоподобным ложным контекстом с реальными Spring AI API (ChatClient/Advisor/VectorStore/EmbeddingModel/ToolCallback в неверном применении) + де-абсолют + пунктуация-паритет, correct БАЙТ-ЛОК. 16 блоков применены (clean+ACCEPT).
- **Q2/Q12 факт-фикс верного (§1 факт-корректность > byte-lock):** enrich-blind поймал 2 факт-ошибки в ВЕРНОМ: Q2 — устаревший стартер `spring-ai-openai-spring-boot-starter` → GA-имя `spring-ai-starter-model-openai`; Q12 — выдуманный `TokenTextSplitter(512, 50)` с «overlap 50» (у Spring AI TokenTextSplitter нет overlap-параметра, дефолт чанк ~800 токенов) → `TokenTextSplitter` (по умолчанию ~800 токенов на чанк). Единственные 2 блока с correct≠HEAD.
- **hand-residual (БЕЗ субагентов — прямая правка JSON, оптимизация по feedback юзера):** остаток связанных метрик закрыт руками, 0 authoring-агентов. Приёмы: (1) в 8 блоках один дистрактор переписан длиннее correct + скобочная плаузибл-ложная деталь → флип CORRECT_LONGEST 0.72→0.28; (2) точечный trim/pad для OPTION_LENGTH spread ≤1.35 (7 блоков); (3) добавление backtick-API-терминов дистракторам в code_hi-блоках Q1/Q10/Q14 до паритета с correct (проза не тронута → spread не сдвинут) → STYLE 0.75→0.33; (4) +1 paren Q12-A → флип DETAIL 0.44→0.33; (5) восстановление `;`-паритета Q1-D/Q5-D/Q10-C. Итерации по гейту детерминированно (прогон мгновенный, 0 токенов).
- **Таргетный blind ФИНАЛ (lean, 1 линза, только 11 руками-правленных блоков Q1/Q2/Q4/Q5/Q6/Q10/Q12/Q14/Q15/Q16/Q18, dynamic Workflow parallel, 544K subagent-токенов):** 11/11 ACCEPT, expected_correct==correct_label во всех (скрытый shuffle), 0 флагов. Факт-фиксы Q2/Q12 blind-подтверждены. Блоки Q3/Q7/Q8/Q9/Q11/Q13/Q17 — enrich+blind на c2, руками не тронуты, re-blind не требовался. Blind отметил 3 минорных version-нюанса в byte-locked верных (Q5 `withTopK`, Q14 `getGenerationTokens`, Q15 `.functions()`) — все `factual_error_in_correct=false` (deprecated-но-компилируется), оставлены при byte-lock (кандидаты на version-bump ROUND, задокументированы в sidecar).
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.278, OPTION_LENGTH 1.35 (0 блоков >1.35), CORRECT_WRONG_AVG 1.074, SENTENCE 0, DETAIL 0.333, ABSOLUTE -0.037, POS 1.25/3, DUPLICATE 0, TEMPORAL 1, SOURCE ok, STYLE 0.333; skel HARD 0 warn=18. Correct байт-лок к HEAD во ВСЕХ блоках, КРОМЕ Q2/Q12 (факт-правки по §1). **18/18 блоков блайнд-верифицировано. spring-ai FINAL.**
- **УРОК harder-lean (2026-07-15, второй feedback юзера «улучшить/ускорить/оптимизировать»):** токен-сток = subagent fan-out (каждый агент перечитывает ~50K контекста → 36 агентов ≈ 1.8M). НОВЫЙ harder-lean дефолт: (1) один enrich-проход вместо c1→c7; (2) весь механический остаток (длина-инверсия/детали/code-паритет/semicolon) делать САМ прямой правкой JSON — 0 authoring-субагентов, итерации по детерминированному гейту (мгновенно, 0 токенов); (3) blind резервировать только для настоящей независимости — один финальный, 1 линза, ТОЛЬКО руками-правленные блоки. Итог: 47 агентов (36 enrich + 11 blind) на файл vs старые ~57×N.

### ROUND-8 · rest-maturity (api) · c1 bootstrap + BATCHED enrich + hand-residual + BATCHED blind + Q17 де-карикатура → **FINAL** — 2026-07-15
- **Выбор файла:** worst-first tractable по live-скану answer-parity, 17 блоков, sidecar не было. Raw-профиль (богатый correct + бедные дистракторы), смешанный код+проза, тема REST/RMM (концепт-стабильная).
- **c1 bootstrap:** POS de-cycle (TARGET=[0,3,2,0,1,3,2,0,1,3,2,0,1,3,2,0,1], maxrun 1, распределение A/D/C/B 5/4/4/4; correct-текст байт-идентичен HEAD) + freshness sidecar (REST/RMM concept-stable: Fielding 2000, Richardson QCon 2008+Fowler, RFC 9110 2022, HAL/JSON:API/Siren) + review sidecar. Correct-метки: `ADCABDCABDCABDCAB`.
- **c2 BATCHED enrich (dynamic Workflow, 5 band-агентов вместо 17 — оптимизация токенов, ~260K vs ~1.8M per-block):** каждый агент авторит 3 дистрактора на блок в своей offset-полосе (по 3-4 блока), развернул до паритета длины/предложений правдоподобным ложным контекстом с реальными REST/RMM-фактами в неверном применении (HATEOAS≠шаблоны URI, safe≠idempotent, RMM только 0–3). correct БАЙТ-ЛОК.
- **hand-residual (БЕЗ authoring-субагентов — прямая правка JSON):** Pass 1 — de-dup (batched-скелет дал DUPLICATE_NGRAMS 41: uniform-заготовки «модель зрелости через N уровня», «Автор Leonard Richardson», «Level 0 Swamp of POX» → переформулированы по опциям → 41→0) + semicolon-паритет Q7/Q11/Q15/Q17 + Q10-C code_bucket. Pass 2 — code-density паритет Q5/Q8/Q13/Q14 (backtick-термины дистракторам, проза не тронута) + spread-тримы. Pass 3 — 4 дистрактора (Q5-A/Q8-B/Q13-D/Q14-B) строго ВЫШЕ correct по code-spans → STYLE 0.485→0.368. Итерации по гейту мгновенно, 0 токенов.
- **c3 BATCHED blind (dynamic Workflow, 5 band-агентов, ~258K токенов):** каждый агент судит блоки своей band НЕЗАВИСИМО (опции shuffled в JS), single_correct/factual_error_in_correct/other_defensible/caricature/dup_scaffold. 16/17 ACCEPT; expected_correct==correct_label во всех 16; 0 факт-ошибок в верном.
- **Q17 де-карикатура (§1 plausibility, ДВА re-blind):** batched blind пометил Q17-C карикатурой — дистрактор выдумывал НЕсуществующие RMM Level 4 (GraphQL) и Level 5 (gRPC) + вывод «выше RMM = современнее = gRPC лучший». **v1** (убрал хеджи «якобы/будто бы», но оставил фабрикацию Level 4/5) → 1-блочный re-blind (seed 60413) СНОВА REVISE: ядро — не хеджи, а выдуманные уровни. **v2** — переписал C в ИСКРЕННЮЮ мисконцепцию «интроспекция схемы GraphQL / server-reflection gRPC ≡ HATEOAS RMM Level 3» (самоописание схемы ≠ рантайм-гипермедиа — реальная путаница mid-level инженеров), без выдуманных уровней и без «новее=лучше»; + de-dup (v2 переиспользовала скелет D → DUPLICATE 0→4 → лексически развёл). 1-блочный re-blind (seed 74209) → **ACCEPT** (caricature=false, все флаги false).
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.235, OPTION_LENGTH 1.30 (0 блоков >1.35), CORRECT_WRONG_AVG 0.969, SENTENCE 1, DETAIL 0.235, ABSOLUTE -0.333, POS 1.25/3, DUPLICATE 0, TEMPORAL 13, SOURCE ok, STYLE 0.338; skel HARD 0 warn=12. **Correct байт-лок к HEAD во ВСЕХ 17 блоках** (факт-ошибок в верном не найдено — в отличие от spring-ai). **17/17 блоков блайнд-верифицировано. rest-maturity FINAL.**
- **УРОК batched-подхода (2026-07-15):** BATCHED enrich/blind (5 band-агентов вместо 17) сократил токены ~7× (260K+258K vs ~1.8M). Два побочных эффекта batched: (1) uniform-заготовки band-авторинга → систематический DUPLICATE_NGRAMS (41) — обязателен de-dup hand-residual; (2) де-карикатура: удаления хеджей НЕДОСТАТОЧНО — если ядро дистрактора фабрикация (выдуманные RMM Level 4/5), надо переписать в ИСКРЕННЮЮ правдоподобную мисконцепцию, иначе re-blind снова REVISE. Итог на файл: 12 агентов (5 enrich + 5 blind + 2 Q17 re-blind) vs spring-ai 47.

### ROUND-8 · spring-scheduling (frameworks/spring) · c1 bootstrap + BATCHED enrich + hand-residual + BATCHED blind + Q16 фикс → **FINAL** — 2026-07-16
- **Выбор файла:** worst-first tractable по live-скану answer-parity (8 фейлов), 16 блоков, sidecar не было. Raw-профиль (богатый correct + бедные дистракторы), все опции 1-предложные, смешанный код+проза.
- **c1 bootstrap:** POS de-cycle (строгий ABCD → TARGET=[0,1,3,2,...], maxrun 1, 4/4/4/4; correct-текст байт-идентичен HEAD) + freshness sidecar (Spring Framework 6.2/Boot 3.4; @Scheduled/TaskScheduler/cron концепт-стабильны с Spring 3+; виртуальные потоки 6.1+) + review sidecar. Correct-метки: `ABDCABDCABDCABDC`.
- **c2 BATCHED enrich (dynamic Workflow, 4 band-агента вместо 16, ~205K vs ~1.6M):** каждый агент авторит 3 дистрактора на блок в своей band, паритет длины правдоподобными Spring-мисконцепциями (fixedRate↔fixedDelay зеркала, cron-в-секундах, пул из 10, @Async обязателен, ShedLock через synchronized, Quartz 7-полевой cron). correct БАЙТ-ЛОК. Один агент столлил 7422s → авто-retry успешен.
- **hand-residual (БЕЗ authoring-субагентов):** корень DETAIL+STYLE+skel — code-богатый correct vs прозаичные дистракторы. P1 — semicolon-паритет 9 блоков (skel HARD 9→2) + code/paren DETAIL·STYLE-флипы через `(`Api`)`-скобки (code+paren без прозы; в code-доминантных блоках дистрактор строго выше correct) → DETAIL 0.625→0.125, STYLE 0.77→0.33. P2 — OPTION_LENGTH rebalance Q3/Q9/Q14 + снятие лишнего `;` Q10-D/Q15-A (skel HARD 2→0).
- **c3 BATCHED blind (dynamic Workflow, 4 band-агента, ~234K):** per-block независимо, 15/16 ACCEPT; expected_correct==correct_label во всех 15.
- **Q16 фикс (§1 факт + plausibility, ДВА re-blind):** blind дал REVISE — дистрактор-A карикатура («якобы»+выдуманный «пул из 10»), дистрактор-B описывал РЕАЛЬНОЕ fixedRate-catch-up → other_defensible. v1 → 1-й re-blind (seed 51877) СНОВА REVISE: моя A «съехала» в реальную ошибку (single-thread), И blind поймал **ФАКТ-ОШИБКУ в самом ВЕРНОМ C** — пункт «метод не `public`» ложен (@Scheduled работает на непубличных через reflection; отметили ДВА независимых blind). v2: A → чётко-ложное «несколько @Scheduled в бине → регистрируется только первый» (реально ВСЕ), C → §1 факт-фикс (удалён спорный «не public»). 2-й re-blind (seed 88243) → **ACCEPT**.
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.312, OPTION_LENGTH 1.33 (0 блоков >1.35), CORRECT_WRONG_AVG 1.014, SENTENCE 0, DETAIL 0.125, ABSOLUTE -0.042, POS 1.0/2, DUPLICATE 0, TEMPORAL 1, SOURCE ok, STYLE 0.344; skel HARD 0 warn=14. Correct байт-лок к HEAD во ВСЕХ блоках, КРОМЕ Q16 (факт-фикс по §1). **16/16 блоков блайнд-верифицировано. spring-scheduling FINAL.**
- **УРОК (2026-07-16):** батч-enrich делает дистракторы ПРОЗАИЧНЕЕ code-богатого correct → систематический DETAIL/STYLE_code/skel-semicolon перекос; единый residual `(`Api`)`-скобки (code+paren без прозы) + comma→semicolon дожимает без agentов. И §1: два независимых blind поймали миф «@Scheduled требует public» в byte-locked верном — фикс верного по факту обязателен, byte-lock уступает. Итог: 10 агентов/файл (4 enrich + 4 blind + 2 Q16 re-blind) vs spring-ai 47.

### ROUND-8 · spring-events (frameworks/spring) · c1 bootstrap + BATCHED enrich + hand-residual + BATCHED blind + де-каррикатура Q3/Q4/Q8 → **FINAL** — 2026-07-16
- **Выбор файла:** worst-first tractable по live-скану answer-parity (8 фейлов), 16 блоков, sidecar не было. СИЛЬНЫЙ raw-профиль (богатый correct 28-38 слов 2-предложный + бедные дистракторы 7-15 слов 1-предложные, correct-самый-длинный 14/16), смешанный код+проза.
- **c1 bootstrap:** POS de-cycle (строгий ABCD → TARGET=[0,1,3,2,...], maxrun 1, 4/4/4/4; correct-текст байт-идентичен HEAD) + freshness sidecar (Spring Framework 6.2/Boot 3.4; модель событий концепт-стабильна: ApplicationEvent с 1.x, аннотационная @EventListener с 4.2) + review sidecar. Correct-метки: `ABDCABDCABDCABDC`.
- **c2 BATCHED enrich (dynamic Workflow, 4 band-агента вместо 16, ~205K vs ~1.6M):** каждый агент авторит 3 дистрактора на блок в своей band, паритет длины И числа предложений правдоподобными Spring-мисконцепциями (publishEvent асинхронна по умолчанию, @EventListener требует наследования ApplicationEvent, @Async на классе события, @TransactionalEventListener в той же tx, события ходят между JVM, @Order по имени метода, @DomainEvents на сервисном бине). correct БАЙТ-ЛОК.
- **hand-residual P1 (БЕЗ authoring-субагентов):** корень STYLE+CORRECT_LONGEST — батч выровнял длину, но correct остался code-богаче и часто самым длинным → STYLE 0.547 (len 0.52/code 0.55), CORRECT_LONGEST 0.438. Единый residual: length-флип 4 блоков (Q1-D/Q5-B/Q11-A/Q13-B — дистрактору проза строго выше correct) + code-флип через `(`Api`)`-скобки 4 блоков (Q5-B/Q8-A/Q9-B/Q13-B — дистрактор строго выше correct по code-spans: code+paren, проза≈const) + `;`-паритет Q3-A/B/C и Q10-A/C/D (skel HARD 2→0) + trim Q4-D (OPTION_LENGTH 1.40→1.33). Итог: STYLE 0.547→0.297, CORRECT_LONGEST 0.438→0.125.
- **c3 BATCHED blind (dynamic Workflow, 4 band-агента, ~205K, seed 50719):** per-block независимо (опции shuffled в JS), 13/16 ACCEPT; correct_choice==key во ВСЕХ 16 (верные подтверждены фактически, factual_error_in_any=false везде). 3 REVISE (Q3/Q4/Q8) — все по каррикатуре дистрактора.
- **де-каррикатура Q3/Q4/Q8 (§1 plausibility, ДВА раунда re-blind, БЕЗ авторинг-агентов):** P2 — Q3-C выдумывал ключ `spring.events.registered` (Boot 3.2) → «событие обязано быть `Serializable`, Spring копирует его на подписчика»; Q3-B само-опровергался («слушатели возвращают итог публикатору») → «mutable для передачи состояния между слушателями через общий экземпляр»; Q4-D «context-события летят на каждый HTTP-запрос» абсурд → «ContextRefreshedEvent приходит до инициализации бинов» (ложный тайминг); Q8-A «порядок в числовом суффиксе имени метода» выдумка → «без @Order порядок = порядок объявления методов». Re-blind Q3/Q4/Q8 (seed 63118): Q3/Q8 ACCEPT, **Q4 СНОВА REVISE — не каррикатура, а `guessable_by_form`:** correct — ЕДИНСТВЕННОЕ утвердительное перечисление событий, дистракторы все ограничивающие/отрицающие. Correct байт-лок → tell сломан переписью **Q4-B тоже в утвердительное перечисление** событий (ложный claim «нужен `ApplicationListener`, `@EventListener` не срабатывает»), а НЕ правкой верного. Финальный re-blind Q4 (seed 71904) → **ACCEPT**, guessable_by_form=false.
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.125, OPTION_LENGTH 1.33 (0 блоков >1.35), CORRECT_WRONG_AVG 1.015, SENTENCE 1, DETAIL 0.062, ABSOLUTE -0.208, POS 1.0/2, DUPLICATE 0, TEMPORAL 7, SOURCE ok, STYLE 0.297 (len 0.21/code 0.30); skel HARD 0 warn=16. **Correct байт-лок к HEAD во ВСЕХ 16 блоках** (факт-ошибок в верном не найдено). **16/16 блоков блайнд-верифицировано. spring-events FINAL.**
- **УРОК form-tell (2026-07-16):** новый класс дефекта, который batched-enrich НЕ создаёт, но обнажает — при паритете длины blind ловит `guessable_by_form`, когда correct структурно ЕДИНСТВЕННЫЙ «утвердительно перечисляющий», а дистракторы все «отрицающие/ограничивающие». correct байт-локнут → лечится тем, что ОДИН дистрактор делают тоже утвердительным перечислением (с ложным claim), а не правкой верного. Плюс подтверждён урок spring-scheduling (code-флип `(`Api`)`). Итог: 10 агентов/файл (4 enrich + 4 blind + 2 re-blind) vs spring-ai 47.

### ROUND-8 · spring-testing (frameworks/spring) · c1 bootstrap + BATCHED enrich + hand-residual + BATCHED blind + bold-parity/де-каррикатура 8 блоков → **FINAL** — 2026-07-16
- **Выбор файла:** worst-first tractable, 16 блоков, sidecar не было. Тот же СИЛЬНЫЙ raw-профиль, что у spring-events (богатый correct 2-предложный + бедные дистракторы, correct-самый-длинный), код+проза.
- **c1 bootstrap:** POS de-cycle (строгий ABCD → TARGET maxrun 1, 4/4/4/4; correct-текст байт-идентичен HEAD) + freshness sidecar (Spring Boot 3.4.x/Framework 6.2.x; @MockBean/@SpyBean deprecated с 3.4.0 в пользу @MockitoBean/@MockitoSpyBean — валидны, НЕ ошибка) + review sidecar. Correct-метки: `ABDCABDCABDCABDC`.
- **c2 BATCHED enrich (dynamic Workflow, 4 band-агента вместо 16, ~205K):** каждый агент авторит 3 дистрактора на блок к паритету длины/предложений правдоподобными Spring-testing-мисконцепциями (@SpringBootTest поднимает Tomcat 8080 по сети, @WebMvcTest грузит data-слой, @DataJpaTest коммитит по умолчанию, MockMvc требует полный контекст, @TestConfiguration ловится @ComponentScan). correct БАЙТ-ЛОК.
- **hand-residual P1/P2 (БЕЗ авторинг-агентов):** length-флип (Q3/Q9/Q10/Q11/Q12/Q13) + code-флип (Q11/Q12) + OPTION_LENGTH (Q5-C/Q6-D) + `;`-паритет Q3/Q4/Q12 (skel HARD 3→0) + trim Q9-D. Итог: все 12 answer-parity PASS, skel HARD 0.
- **c3 BATCHED blind (dynamic Workflow, 4 band-агента, ~207K, seed 42817):** per-block независимо, 8/16 ACCEPT; correct_choice==key во ВСЕХ 16 (верные подтверждены, factual_error_in_any=false везде). **8 REVISE (Q1/Q4/Q5/Q6/Q7/Q8/Q11/Q16)** — ДВА класса дефекта: (a) **bold-form-tell** (Q1/Q4/Q5/Q7/Q11 — markdown `**жирный**` присутствовал ТОЛЬКО в correct); (b) **каррикатура** (Q6-A «каждый срез поднимает Netty:8080», Q7-B «MockMvc — Mockito-мок контроллера», Q8-D «Content-Length:256» + Q8-A «Spring сам угадает JSON», Q11-C «не больше 3 @TestConfiguration») + Q16 form-tell (все дистракторы — ограничения, correct единственный «преимущества» под стем «когда лучше»).
- **P3/P4/P5 фиксы (§1 fact/plausibility/Visual-Parity, БЕЗ авторинг-агентов):** (a) **bold-parity** — correct байт-локнут, поэтому в КАЖДЫЙ дистрактор Q1/Q4/Q5/Q7/Q11 обёрнута существующая фраза в `**…**` (0 изменения длины) → bold-присутствие равномерно по всем 4 (Visual/Structural Parity). (b) **де-каррикатура** — Q6-A Netty→«каждый попутно поднимает MockMvc для контроллеров», Q7-B→«вызывает методы контроллера напрямую в обход DispatcherServlet» (реальная мисконцепция, MockMvc идёт ЧЕРЕЗ DispatcherServlet), Q8-D→«content(Map) авто-сериализуется», Q8-A→«MockMvc возвращает ResponseEntity, не MvcResult», Q11-C→«обязана быть вложенной, top-level нельзя» (без фейкового числа). (c) **Q16 advantages-parity** — один дистрактор (B) сделан тоже «преимущества»-фреймингом (ложный «всегда поднимает сервер»), зачины де-унифицированы. P4 добил OPTION_LENGTH (Q6/Q8/Q11/Q16 ≤1.35, тримминг/наращивание ТОЛЬКО дистракторов) + skel HARD Q11 (убран `;`, C разбит на 2 предложения). P5 — Q8-A был `other_defensible` (ручной парсинг = рабочая техника) → переписан в чёткую фактошибку (MockMvc→ResponseEntity).
- **re-blind (seed 55931, 8 блоков, 2 агента → Q8 seed 88240):** первый раунд 7/8 ACCEPT (bold-parity/де-каррикатура/Q16 закрыты), Q8 REVISE (other_defensible) → P5 → финальный Q8 re-blind ACCEPT. **16/16 блоков блайнд-верифицировано, correct_choice==key во всех.**
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.25, OPTION_LENGTH 1.35 (0 блоков >1.35), CORRECT_WRONG_AVG 1.036, SENTENCE 0, DETAIL 0.312, ABSOLUTE -0.146, POS 1.0/2, DUPLICATE 0, TEMPORAL 8, SOURCE ok, STYLE 0.312 (len 0.25/code 0.31); skel HARD 0 warn=15. **Correct-текст байт-идентичен HEAD во ВСЕХ 16 блоках** (факт-ошибок в верном не найдено). **spring-testing FINAL.**
- **УРОК bold-parity (2026-07-16):** ещё один form-tell — markdown-эмфаза (`**жирный**`) в ТОЛЬКО correct = поверхностная подсказка; correct байт-локнут → паритет достигается обёрткой существующей фразы дистрактора в `**…**` (0 изменения длины/кода/пунктуации, только bold-присутствие). И урок Q8: `other_defensible` ≠ каррикатура — «рабочая, но неидиоматичная техника» в дистракторе делает его защитимым; лечится заменой на утверждение с РЕАЛЬНОЙ фактошибкой (ложная премиса), а не косметикой.

### ROUND-8 · graalvm-native (jvm) · c1 bootstrap + BATCHED enrich + hand-residual + BATCHED blind + факт-правки correct 2 блоков → **FINAL** — 2026-07-16
- **Выбор файла:** worst-first tractable, 15 блоков, sidecar не было. Raw-профиль: богатый 2-предложный correct + короткие карикатурные дистракторы (абсолюты «всегда/только/никаких»), плотный код+проза (native-image/closed-world/reachability/RuntimeHints).
- **c1 bootstrap:** POS de-cycle (строгий ABCD → TARGET `[1,3,0,2]×3 + [0,1,2]`, counts A4/B4/C4/D3, no-adjacent-equal, SEQUENCE 15→3; correct-текст байт-идентичен HEAD) + freshness sidecar (GraalVM for JDK 21/24, native-image AOT; Spring Native слит в Boot 3, отдельный стартер УСТАРЕЛ). Correct-метки: `BDACBDACBDACABC`.
- **c2 BATCHED enrich (dynamic Workflow, 4 band-агента вместо 15, ~205K):** каждый агент авторит 3 дистрактора на блок к паритету длины/предложений спокойными GraalVM-мисконцепциями (JIT внутри native, JRE-упаковка вместо AOT, reachability = все public, spring-native+@NativeHint, AOT на первом старте) с удалением абсолютов. correct БАЙТ-ЛОК.
- **hand-residual P1 (БЕЗ авторинг-агентов):** length-флип Q6/Q7 (плоские скобки, без code-бампа) + `;`-паритет Q4/Q9/Q11 (skel HARD 3→0) + trim Q9-D (OPTION_LENGTH 1.50→1.25). Итог: все 12 answer-parity PASS, skel HARD 0.
- **c3 BATCHED blind (2 band-агента напрямую — subagent-fallback вместо flaky Workflow, seed 33717):** per-block независимо, **10/15 ACCEPT**; correct_choice==key во ВСЕХ 15 (распределители не «утекают»). **5 REVISE:** Q1/Q14 — фактические огрехи в САМОМ correct (Q1 «включает байткод приложения» вместо машинного кода; Q14 «`@MockBean` (Spring proxy)» — ложно, @MockBean на Mockito и сам ограничен в native); Q4/Q7/Q8 — каррикатура дистракторов (Q4 фейковый «до 3 уровней вложенности» + agent на собранном бинарнике в проде; Q7 «native-image на первом старте + кэш на диске»; Q8 «фоновая AOT-оптимизация запущенного приложения» + «держат ради отладки»).
- **P2/P2b/P2c фиксы (§1 факт-корректность > де-каррикатура, БЕЗ авторинг-агентов):** (a) **byte-lock override по §1** — Q1-B переписан на «машинный код приложения + части рантайма JVM + данные образа»; Q14-B — убрано ложное «@MockBean (Spring proxy)», стало «динамические моки (`Mockito.mock()`) требуют build-time hints (`RuntimeHints`), `@SpringBootTest`+`MockMvc` работают». (b) **де-каррикатура** — Q4-B→«агент подхватывает рефлексию без пересборки» (реальное заблуждение про auto-apply), Q4-D→«reachability roots = все public» (без фейкового числа), Q7-D→«одно свойство `spring.aot.enabled=true` + `bootJar` соберёт native без плагина», Q8-A→«AOT на первом старте + кэш» (спокойный тайминг-миф вместо «фоновой оптимизации под нагрузку»), Q8-B→«AOT только под native» (вместо «ради отладки»). P2b/P2c — ретюн длины/предложений (OPTION_LENGTH Q1/Q7/Q8 ≤1.35, sentence-паритет Q4-B, `;`-паритет сохранён).
- **re-blind (seed 51199, 5 тронутых блоков, 1 агент):** **5/5 ACCEPT**, correct_choice==key во всех (Q1=2/Q4=2/Q7=1/Q8=4/Q14=4). **15/15 блоков блайнд-верифицировано за два раунда, correct_choice==key везде.** NIT (не блокер): Q4-C correct упоминает `@RegisterForReflection` (Quarkus-аннотация) как механизм регистрации рефлексии — ревьюер счёл допустимым в GraalVM-контексте (аннотация реальна, регистрирует рефлексию для native-image), factual_error_in_any=false.
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.267, OPTION_LENGTH 1.33 (0 блоков >1.35), CORRECT_WRONG_AVG 1.014, SENTENCE 0, DETAIL 0.4, ABSOLUTE -0.022, POS 1.33/2 (SEQUENCE 3), DUPLICATE 0, TEMPORAL 7, SOURCE ok, STYLE 0.367 (len 0.30/code 0.37); skel HARD 0 warn=15. **Correct-текст байт-идентичен HEAD в 13/15 блоках** (Q1/Q14 изменены по §1 факт-корректности — реальные ошибки в верном). **graalvm-native FINAL.**
- **УРОК factual-error-in-correct (2026-07-16):** блайнд ловит не только form-tell/каррикатуру дистракторов, но и **фактические огрехи в самом byte-locked correct** — здесь два (сформулированное «включает байткод» у AOT-в-машинный-код образа; «@MockBean = Spring proxy», хотя @MockBean на Mockito). §1 факт-корректность override байт-лока: минимальная правка формулировки correct + обязательный ре-блайнд тронутого блока. Ставит под сомнение «byte-lock как аксиому» — correct эталонен по ФОРМЕ, но не иммунен к фактике.

### ROUND-8 · spring-messaging (frameworks/spring) · c1 bootstrap + BATCHED enrich + hand-residual + BATCHED blind + §1 факт-правка correct + де-стем-лик → **FINAL** — 2026-07-16
- **Выбор файла:** worst-first tractable, 15 блоков, sidecar не было. Профиль: 2-предложный correct + короткие карикатурные дистракторы; плотный Spring-Messaging код+проза (JMS/AMQP, RabbitTemplate/@RabbitListener, exchange-типы, Publisher Confirms, 2PC).
- **c1 bootstrap:** POS de-cycle (строгий ABCD → TARGET `[1,3,0,2]×3 + [0,1,2]`, counts A4/B4/C4/D3, no-adjacent-equal, SEQUENCE 15→3; correct-текст байт-идентичен HEAD) + freshness sidecar (Spring Boot 3.4.x/Framework 6.2.x, spring-jms + spring-rabbit; javax→jakarta.jms Boot 3; стартеры activemq/amqp). Correct-метки: `BDACBDACBDACABC`.
- **c2 BATCHED enrich (4 band-агента напрямую вместо 15):** BANDS `[[1-4],[5-8],[9-12],[13-15]]`. Каждый агент авторит 3 дистрактора на блок к паритету длины/предложений спокойными мисконцепциями (реактивная надстройка над Reactor, in-memory брокер на 61616, STOMP-only; convertAndSend+ручной receive; AMQP-2PC через txSelect+XA; RabbitTransactionManager 2PC; fanout сверяет routing key; Kafka-удаляет-по-offset) с удалением абсолютов. correct БАЙТ-ЛОК.
- **hand-residual P1/P2/P3 (БЕЗ авторинг-агентов, против детерминированного гейта):** length-флип Q1/Q5/Q8/Q9 (строго выше correct → снят CORRECT_LONGEST/STYLE_len) + догонка min-дистрактора под spread ≤1.35; Q4 trim; Q10-A/B/C реструктурированы в 3 предложения с `;` внутри предложения (SENTENCE + semicolon-паритет разом); плоские скобки Q3-D/Q15-D для DETAIL без бампа code. Итог: все 12 answer-parity PASS, skel HARD 0.
- **c3 BATCHED blind (2 band-агента напрямую — subagent-fallback вместо flaky Workflow, seed 64708):** band1 Q1-8 / band2 Q9-15 независимо, **12/15 ACCEPT**; correct_choice==key во ВСЕХ 15. **3 REVISE:** Q1-A (тэлл-формулировка), Q8-B (фабрикованный `@JmsTransacted`), **Q10-D — §1 факт-огрех в САМОМ correct** (`@SendTo("orders.responses")` — жёстко зашитая именованная reply-очередь противоречит описанному в той же опции механизму динамической temp-reply-queue по `replyTo`).
- **P4/P5 фиксы (§1 факт > де-form-tell > паритет, БЕЗ авторинг-агентов):** (a) **byte-lock override по §1** — Q10-D correct: `@SendTo("orders.responses")` → «а `@SendTo` направляет его в reply-очередь» (убран хардкод, опция внутренне непротиворечива). (b) де-каррикатура: Q1-A → спокойный STOMP-only-миф без фейковых true-фактов; Q8-B → реальное-но-ошибочное «`RabbitTransactionManager`+`@Transactional` = общий 2PC» вместо `@JmsTransacted`; Q15-D — снят тэлл «якобы». P5 — ретюн длины Q1-A/Q8-B (OPTION_LENGTH+STYLE_len-флип) и Q10-A/B/C (spread ≤1.35).
- **re-blind #1 (seed 51893, 4 тронутых блока Q1/Q8/Q10/Q15, 1 агент):** Q1/Q10/Q15 ACCEPT; **Q8 REVISE — не по опциям (B верен), а СТЕМ-ЛИК**: «...и почему AMQP **не** поддерживает 2PC» преподносит вывод как факт, отчего C/D (утверждающие обратное) отсеиваются простым чтением вопроса. correct_choice==key во всех 4.
- **стем-нейтрализация + re-blind #2 (seed 77021, только Q8):** стем Q8 → нейтральный «Как работают транзакции в `Spring Messaging` (JMS и AMQP) и как обеспечить согласованность отправки в брокер с записью в базу данных?» — теперь C/D тестируют заблуждение. Re-blind → **ACCEPT** (correct_choice=C==key, form_guess=false). Опции Q8 не тронуты, correct байт-лок цел. **15/15 ACCEPT.**
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.333, OPTION_LENGTH 1.35 (0 блоков >1.35), CORRECT_WRONG_AVG 1.079, SENTENCE 1 (0 блоков >1), DETAIL 0.4, ABSOLUTE -0.089, POS 1.33/2 (SEQUENCE 3), DUPLICATE 0, TEMPORAL 1, SOURCE ok, STYLE 0.378 (len 0.37/code 0.38); skel HARD 0 warn=15. **Correct-текст байт-идентичен HEAD в 14/15 блоках** (Q10 изменён по §1). Стем изменён только в Q8. **spring-messaging FINAL.**
- **УРОК стем-лик (2026-07-16):** четвёртый класс дефекта помимо form-tell/каррикатуры/факт-огреха-в-correct — **сам стем пресуппозирует вывод и отсеивает дистракторы простым чтением** (Q8 «почему AMQP НЕ поддерживает 2PC» убивал C/D). Контент-паритет опций недостаточен, если вопрос раздаёт ответ; blind судит и стем. Фикс — нейтрализация формулировки стема без правки опций + обязательный ре-блайнд.

### ROUND-8 · spring-vault (frameworks/spring) · c1 bootstrap + BATCHED enrich + hand-residual + BATCHED blind → **FINAL** (15/15 ACCEPT первым проходом) — 2026-07-16
- **Выбор файла:** worst-first tractable, 8 фейлов / 15 блоков, sidecar не было. Raw-профиль: богатый 2-clause correct с плотным кодом (VaultTemplate/opsForTransit/pki/issue/database/creds) vs короткие карикатурные дистракторы с абсолютами (CORRECT_LONGEST 0.867, OPTION_LENGTH 2.89, DETAIL 0.933, STYLE 0.9, skel HARD 12).
- **c1 bootstrap:** POS de-cycle (строгий ABCD SEQUENCE 15 → TARGET `[1,3,0,2]×3 + [0,1,2]`, counts A4/B4/C4/D3, SEQUENCE 15→3; correct-текст байт-идентичен HEAD) + freshness sidecar (Spring Vault 3.1.x + Spring Cloud Vault 4.2.x, HashiCorp Vault 1.15+; version-sensitive `bootstrap.yml`→`spring.config.import=vault://` сдвиг, KV v2 `data/`-префикс). Correct-метки: `BDACBDACBDACABC`. context7 был недоступен (fetch failed) — sidecar по знаниям, VPN не трогал.
- **c2 BATCHED enrich (4 band-агента напрямую вместо 15):** BANDS `[[1-4],[5-8],[9-12],[13-15]]`. Каждый агент авторит 3 дистрактора на блок к паритету длины/предложений/backtick спокойными Vault-мисконцепциями (Vault=конфиг-сервер/Keycloak/JVM-KeyStore; static Dynamic Secrets; Transit отдаёт ключ; APPROLE только dev; KV v1 версионирует; HikariCP сам подхватит пароль; Vault Agent=библиотека) с удалением абсолютов. correct БАЙТ-ЛОК.
- **hand-residual P1/P2/P3 (БЕЗ авторинг-агентов, против детерминированного гейта):** P1 — **semicolon-presence паритет** (per-block равномерность `;` внутри предложения; skel HARD 12→7) + length-флипы 7 блоков + Q8-trim. P2 — **opening_class паритет** (btick-vs-prose старт равномерный: миноритарные → класс correct префиксом «Аннотация»/«Метод»/btick-первое; skel HARD 7→0) + доп. флипы Q6/Q9/Q13/Q14 (CORRECT_LONGEST 0.6→0.333). P3 — Q6 spread-фикс. Итог: все 12 answer-parity PASS, skel HARD 0.
- **c3 BATCHED blind (2 band-агента напрямую — subagent-fallback, seed 83124):** band1 Q1-8 / band2 Q9-15 независимо. **15/15 ACCEPT ПЕРВЫМ проходом, БЕЗ единого REVISE**; correct_choice==key во ВСЕХ 15; form_guess/factual_error_in_correct=false везде. NIT (не блокер): Q8 `pki/issue/myapp-role` шорткат (консистентен), Q14-C `spring.cloud.vault.fail-fast` валидно но не второй верный — ревьюер ACCEPT.
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.333 (5/15), OPTION_LENGTH ≤1.35, CORRECT_WRONG_AVG ∈[0.9,1.1], SENTENCE 0, DETAIL ≤0.40, ABSOLUTE ≤0.25, POS 1.33/2 (SEQUENCE 3), DUPLICATE 0, SOURCE ok, STYLE ≤0.40; skel HARD 0 warn=15. **Correct-текст байт-идентичен HEAD во ВСЕХ 15 блоках** (§1 факт-фикс не понадобился). Стемы не тронуты. **spring-vault FINAL.**
- **УРОК systematic-skel из батч-enrich (2026-07-16):** батч-заготовки дистракторов дают ДВА систематических skel-перекоса против code-богатого correct — (1) **semicolon_presence** (correct несёт `;`, прозаичные дистракторы нет) → выравнивать `;` ВНУТРИ предложения per-block; (2) **opening_class** (correct стартует с `` `btick` ``, дистракторы с прозы — или наоборот) → приводить миноритарные варианты к классу correct префиксом-словом («Аннотация `@X`»/«Метод `X`») или перестановкой первого токена. Оба — HARD-оси skel, лечатся дёшево hand-residual без субагентов. Самый дешёвый tick round-8: 6 агентов/файл (4 enrich + 2 blind, 0 re-blind).

### ROUND-8 · spring-kafka (frameworks/spring) · c1 bootstrap + BATCHED enrich + hand-residual P1–P9 + blind ×2 + targeted re-blind → **FINAL** — 2026-07-16
- **Выбор файла:** worst-first tractable, 15 блоков, sidecar не было. Raw-профиль: богатый 2-clause correct с плотным кодом (KafkaTemplate/@KafkaListener/DefaultErrorHandler/DeadLetterPublishingRecoverer/KafkaTransactionManager/AckMode/JsonSerializer/StreamsBuilder) vs короткие дистракторы.
- **c1 bootstrap:** POS de-cycle (строгий ABCD SEQUENCE 15 → TARGET `[1,3,0,2]×3 + [0,1,2]`, counts A4/B4/C4/D3, SEQUENCE 15→3; correct-текст байт-идентичен HEAD) + freshness sidecar (spring-kafka 3.3.x под Boot 3.4.x, kafka-clients 3.8/3.9; version-sensitive: DefaultErrorHandler заменил SeekToCurrentErrorHandler с 2.8, @RetryableTopic, enable.idempotence default true+acks=all с Kafka 3.0). Correct-метки: `BDACBDACBDACABC`.
- **c2 BATCHED enrich (4 band-агента напрямую вместо 15):** BANDS `[[1-4],[5-8],[9-12],[13-15]]`, 3 дистрактора/блок к паритету длины/предложений/backtick спокойными Kafka-мисконцепциями. correct БАЙТ-ЛОК (эталон длины/предложений/кода).
- **hand-residual P1–P9 (БЕЗ авторинг-агентов, против детерминированного гейта):** P1–P3 — удлинение коротких дистракторов ПРОЗОЙ (code-плотные переписи режут прозу, opt_feats стрипает backtick) + semicolon-паритет. P4 — code-tie (correct не strict-max по code-spans). **P5 — negation→positive reframe** (blind seed 51837 дал 10/15 form_guess=true). P6 — восстановление skel + code-tie + флипы. P7/P8 — де-карикатура логических инверсий (Q1-D Spring Cloud Stream, Q6-A @RetryableTopic-цепочка, Q9-D COUNT/TIME). **P9 — REVISE-фикс по targeted re-blind:** Q6-A (был защитим как реальный non-blocking-retry → ложь «orders-dlt дочитывает сам основной @KafkaListener»), Q9-D (каррикатура «до перезапуска» → «RECORD/BATCH без ручного ack.acknowledge() не коммитят»). Итог: 12 answer-parity PASS, skel HARD 0, STYLE 0.356→0.317.
- **blind ×2 + targeted re-blind (subagent-fallback):** полный seed 51837 (negation-tell → P5), полный seed 64209 (15/15 correct_choice==key, single_correct=true), targeted Q1/Q6/Q9 seed 90418 (correct_choice==key все 3, но Q6 single_correct=false [D защитим] + Q9 каррикатура → REVISE P9), финальный Q6/Q9 seed 77620 (**оба ACCEPT: correct_choice==key, single_correct=true, каррикатура=[]**).
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.2 (3/15), OPTION_LENGTH 1.33, CORRECT_WRONG_AVG 1.038, SENTENCE 0, DETAIL 0.4, ABSOLUTE -0.022, POS 1.33/2 (SEQUENCE 3), DUPLICATE 0, SOURCE ok, STYLE 0.317; skel HARD 0 warn=14. **Correct байт-идентичен HEAD во ВСЕХ 15 блоках** (§1 факт-фикс не понадобился). Стемы не тронуты. **spring-kafka FINAL.**
- **УРОК negation-tell → positive-framing (2026-07-16):** дистракторы, оформленные как отрицания/отмахивания («не нужно / не требуется / не поддерживается / не выйдет»), создают систематический форм-тэлл — адверсариал-blind флагует guessable=true даже при контент-паритете. Фикс: переписать дистрактор в **уверенное ПОЗИТИВНОЕ ложное-утверждение** (assert правдоподобного-но-ложного механизма) под конструктивный тон correct. Ревьюер ставил guessable=false именно на блоках, где дистракторы позитивно утверждают неверный механизм.
- **УРОК де-карикатура ↔ single_correct (2026-07-16):** смягчая карикатурный дистрактор до правдоподобного, легко пере-скорректировать его в *валидный* — Q6-A/Q6-D после де-карикатуры описали реальный @RetryableTopic non-blocking-retry и стали защитимы (second-correct). Targeted re-blind ловит это; REVISE-фикс возвращает однозначную ложность (прямое противоречие correct: «сам основной листенер дочитывает DLT»), НЕ возвращая карикатуру. Де-карикатура и single_correct — в напряжении, финальный re-blind обязателен.

### ROUND-8 · spring-rest-client (frameworks/spring) · c1 bootstrap + BATCHED enrich + hand-residual P1–P4 + blind + re-blind → **FINAL** — 2026-07-16
- **Выбор файла:** worst-first tractable, 8 фейлов / 13 блоков, sidecar не было. Raw-профиль: богатый correct с плотным кодом vs короткие карикатурные дистракторы с АБСОЛЮТНЫМИ маркерами (CORRECT_LONGEST 0.692, OPTION_LENGTH 2.47, DETAIL 0.846, ABSOLUTE_MARKER_GAP 0.538, STYLE 0.808, SEQUENCE 13). skel HARD 0 уже в baseline.
- **c1 bootstrap:** POS de-cycle (строгий ABCD SEQUENCE 13 → TARGET `[1,3,0,2]×3 + [0]`, counts A4/B3/C3/D3, SEQUENCE 13→3; correct-текст байт-идентичен HEAD) + freshness sidecar (Spring Framework 6.2.x под Boot 3.4.x; RestClient с 6.1/Boot 3.2, RestTemplate maintenance mode с 5.0 НЕ deprecated, @HttpExchange 6.0 + адаптеры 6.1). Correct-метки: `BDACBDACBDACA`.
- **c2 BATCHED enrich (4 band-агента напрямую вместо 13):** BANDS `[[1-4],[5-7],[8-10],[11-13]]`, 3 дистрактора/блок к паритету УВЕРЕННЫМИ ПОЗИТИВНЫМИ ложными-механизмами (RestClient=фасад-над-WebClient; retrieve()=CompletableFuture; .subscribe()/.sync() как sync; @HttpExchange=серверная/OpenFeign; @HttpPathVariable-нейминг; RestTemplateAdapter для всех). Снял raw-профиль: ABSOLUTE 0.538→0, CORRECT_LONGEST 0.692→0.077, STYLE 0.808→0.372. correct БАЙТ-ЛОК.
- **hand-residual P1–P4 (БЕЗ авторинг-агентов, против детерминированного гейта):** P1 — Q3-C/D trim (correct самый короткий по прозе 12сл при 10 code-spans) + Q6-A raise + Q5-A/Q8 +скобка (DETAIL по parens: correct богаче скобочными пояснениями → поднять max-wrong parens). P2 — Q8 stem-нейтрализация + тон-нейтрализация дистракторов + Q11 §1 факт-фикс correct. P3/P4 — Q8-B lengthen (LEN 1.42→1.31) + code-tie Q8/Q12 (STYLE code-strict-max 0.41→0.333). Итог: 12 answer-parity PASS, skel HARD 0.
- **blind + re-blind (subagent-fallback):** полный seed 58043 (band1 Q1-7 / band2 Q8-13) — **13/13 correct_choice==key**, single_correct=true, каррикатур нет; но Q8 stem_leak=true + Q11 factual-flag в correct → REVISE. Re-blind Q8/Q11 seed 71294 (**оба ACCEPT: correct==key, single_correct=true, stem_leak=false, factual_error=null**).
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.154 (2/13), OPTION_LENGTH 1.31, CORRECT_WRONG_AVG 1.006, SENTENCE 0, DETAIL 0.308, ABSOLUTE 0.0, POS 1.33/2 (SEQUENCE 3), DUPLICATE 0, SOURCE ok, STYLE 0.333; skel HARD 0 warn=11. **Correct байт-идентичен HEAD в 12/13** (Q11 §1 факт-фикс). Стем изменён только в Q8. **spring-rest-client FINAL.**
- **УРОК stem-leak + factual-в-correct вместе (2026-07-16):** один файл дал ДВА уже-известных класса дефекта разом. (1) **stem-leak** — стем «...и почему это считается антипаттерном?» пресуппозировал вывод, отсеивая A/B/C по тону; фикс — нейтрализация стема в чистый how-вопрос (question_text не отображается, alignment по q_number цел; прецедент spring-messaging), stem_leak true→false. (2) **factual-в-byte-locked-correct** — Q11 correct утверждал `spring.http.interface.default-base-url` (вероятно несуществующее Boot-свойство); §1 override byte-lock → замена на верифицируемое `@HttpExchange(url = ...)` + re-blind. Остаточный form_guess=true на 4 блоках (nuance/completeness/symmetry верного) — non-exploitable при зелёном механическом STYLE 0.333, консистентно с прецедентом.

### ROUND-8 · mtls (security) · c1 bootstrap + trim-all-correct(§17) + BATCHED enrich CLASS1 + skel + blind ×2 band + bounded REVISE (Q2/Q3/Q4/Q6) + re-blind → **FINAL** — 2026-07-16
- **Выбор файла:** worst-first, 20 блоков, 7 фейлов. Экстремальный raw-профиль: CORRECT_LONGEST 1.0 (все 20), CORRECT_WRONG_AVG 1.876, STYLE 1.0, SEQUENCE 20 (идеальный A,B,C,D×5 цикл). Дистракторы содержательно-полные (хорошие mental-models), но короче correct; correct — 40-56-словные каталоги (первичный form-tell по длине/детали).
- **c1 bootstrap:** POS de-cycle (correct-текст байт-идентичен HEAD, менялись только порядок/метки; SEQUENCE 20→3, counts A5/B5/C5/D5, POS_DIST 1.0) + freshness sidecar (концепт-ядро стабильно; version-sensitive: TLS 1.3 RFC 8446, Istio ~24h auto-rotation + PeerAuthentication STRICT/PERMISSIVE/DISABLE, Linkerd default mTLS, SPIFFE/SPIRE SVID, cert-manager Issuer/ClusterIssuer, Vault PKI, Redis 6+/Kafka ssl.client.auth/PostgreSQL ssl_ca_file). Correct-метки: `DCABABDCDACDCDBABCBA`.
- **Trim всех 20 correct (§17/§18.1-C, БЕЗ авторинг-агентов):** correct БЫЛИ первичным form-tell (40-56-словные каталоги) → детали каталогов вынесены в `sections` (факт-сохранно, требует re-blind), correct-текст сокращён до паритета длины. Санкционированная prompt-правка, не byte-lock-нарушение.
- **BATCHED enrich CLASS1 + skel (напрямую против гейта):** удлинение коротких дистракторов CLASS1 (Q1/2/3/4/8/9/13) правдоподобным параллельным механизмом; SKEL-нормализация opening_class + semicolon-паритет (Q5/6/7/10/12/15/16/18/19/20); micro-balance Q10/Q18. Снял raw-профиль: CORRECT_LONGEST 1.0→0.15, CORRECT_WRONG_AVG 1.876→1.009, STYLE 1.0→0.346, OPTION_LENGTH→1.30, ABSOLUTE→0.217.
- **blind ×2 band + bounded REVISE + re-blind:** полный blind (band A Q1-10 / band B Q11-20) — **single_correct 20/20 (bsm=единственный key), factual_error в correct = null везде**, bst_guess=key (знание-based, мех-гейт зелёный). Genuine-дефекты: (1) **§31.3 two-distractor mirror Q6-A↔Q6-C** (оба несли «Root online / без Intermediate»); (2) каррикатуры-абсурды Q2-A/Q3-D «mTLS отключает шифрование → plaintext» + Q4-C «сервер шлёт клиенту приватный ключ». REVISE 4 блоков прямой правкой JSON (0 авторинг-агентов): Q6-C → distinct-мисконцепция (trust store хранит leaf-cert, pinning-модель), Q2-A/Q3-D/Q4-C де-каррикатура в правдоподобные ложные-механизмы (стойкость шифрования / производительность / nonce-challenge). Re-blind Q2/Q3/Q4/Q6 seed 61729 — **ACCEPT: bst==key все 4, single_correct=true, Q4 caricature=[], Q6 mirror=null (дубль устранён, ревьюер подтвердил 3 различимые мисконцепции)**.
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.15 (3/20), OPTION_LENGTH 1.30, CORRECT_WRONG_AVG 1.009, SENTENCE 1, DETAIL 0.1, ABSOLUTE 0.217, POS 1.0/2 (SEQUENCE 3), DUPLICATE 0, TEMPORAL 0, SOURCE n/a, STYLE 0.346; skel HARD 0 warn=20. Стемы не тронуты. **mtls FINAL.**
- **УРОК two-distractor mirror ≠ knowledge-based form_guess (2026-07-16):** blind разделил два класса — (а) **structural дефект** (Q6 два дистрактора-дубля одной мисконцепции) правится ОБЯЗАТЕЛЬНО до commit разведением в distinct-мисконцепции; (б) остаточные strict-каррикатур-флаги на inherently-inversion дистракторах (Q2-C «не требует инфраструктуры», Q2-D «длина ключей», Q3-D) — knowledge-based non-exploitable при зелёном ABSOLUTE 0.217/STYLE 0.346. Оптимизация по feedback: 0 авторинг-субагентов на trim/enrich/skel/REVISE (прямая правка против детерминированного гейта), субагенты только на 3 независимых blind-прохода (band A + band B + targeted re-blind 4 блоков).

### ROUND-8 · spring-graphql (frameworks/spring) · c1 bootstrap + BATCHED enrich + hand-residual (parens/length) + blind ×2 band → **FINAL** — 2026-07-16
- **Выбор файла:** worst-first tractable, 15 блоков, sidecar не было. Raw-профиль (после c1): CORRECT_LONGEST 0.8 (12/15), OPTION_LENGTH 2.58, CORRECT_WRONG_AVG 1.471, DETAIL 0.933, STYLE 1.0. correct — 2-предложенные code-каталоги (`@QueryMapping`/`@MutationMapping`/`@SubscriptionMapping`/`@SchemaMapping`/`@BatchMapping`, DataLoader, GraphQlTester, Relay Connection/ScrollPosition/CursorStrategy) vs короткие дистракторы (первичный form-tell по длине/детали). skel HARD 0 уже в baseline.
- **c1 bootstrap:** POS de-cycle (correct-текст байт-идентичен HEAD, менялись только порядок/метки; SEQUENCE 15→3, counts A4/B4/C4/D3, POS_DIST 1.33) + freshness sidecar (Spring for GraphQL 1.3.x под Boot 3.4.x; version-sensitive: `schema.introspection.enabled`/`@GraphQlExceptionHandler`/Relay Connection добавлены в 1.2/Boot 3.1, метрики graphql.* эволюционировали к Observation API). Correct-метки: `BDACBDACBDACABC`.
- **c2 BATCHED enrich (4 band-агента напрямую вместо 15):** BANDS `[[1-4],[5-8],[9-12],[13-15]]`, 3 дистрактора/блок к паритету длины/skel УВЕРЕННЫМИ ПОЗИТИВНЫМИ ложными-механизмами (GraphQL=REST-over-OpenAPI; схема из @Entity; `@GetMapping`-резолверы; мутации параллельны; N+1 через L2-кэш; `@FieldMapping`; introspection=кэш/удаление поля; Upload в core-спеке; директивы=HTTP-заголовки; метрики без Actuator). **Пойман mislabel:** Q4-payload целил дистракторы A/B/**C**, но C=correct → assert поймал до записи, ремап ключа C→D (контент = мисконцепция «мутации параллельны»). Снял raw-профиль: CORRECT_LONGEST 0.8→0.133, OPTION_LENGTH 2.58→1.2, CORRECT_WRONG 1.471→1.013, STYLE 1.0→0.267. correct БАЙТ-ЛОК.
- **hand-residual (БЕЗ авторинг-агентов, против детерминированного гейта):** DETAIL_PARITY 0.467 (7 блоков correct доминирует ПО PARENS — correct несёт прозаические скобочные пояснения, дистракторы нет; `parens()` считает `(` в raw-тексте) → добавил естественные прозаические скобки в 7 дистракторов (Q2/7/9/11/12/14/15) к parens-паритету → DETAIL 0.0. Регресс OPTION_LENGTH 1.5 (Q2-A/Q15-A раздулись) → trim Q2-A/Q15-A + lengthen Q2-B/C (min оторван от correct-байт-лока) → OPTION_LENGTH 1.33. skel HARD остался 0 (скобки не меняют opening_class/semicolon/sentence_count).
- **blind ×2 band (seed 48217, band1 Q1-8 / band2 Q9-15):** **merit_choice 15/15 == key, single_defensible 15/15 true, factual_error_in_choice null везде, other_defensible null везде.** REVISE не потребовался. Band2-ревьюер флагнул «correct самый длинный/детальный» + каррикатуру Q9/Q11/Q13/Q15 — но **мех-проверка длины опровергла**: correct — rank 4/4 (самый КОРОТКИЙ) в Q11, rank 3/4 в Q10/Q15, rank 2/4 в Q9/Q14; строго-longest только Q13 (+1 слово). Style-tell = пост-рационализация после знания ответа по существу (content-recognition реальных API, не форма), non-exploitable при зелёном мех-STYLE 0.267.
- **Гейты FINAL: answer-parity 1/1 PASS ВСЕ 12 метрик** — CORRECT_LONGEST 0.133 (2/15), OPTION_LENGTH 1.33, CORRECT_WRONG_AVG 0.983, SENTENCE 0, DETAIL 0.0, ABSOLUTE -0.089, POS 1.33/2 (A/B/C/D=4/4/4/3, SEQUENCE 3), DUPLICATE 0, TEMPORAL 3, SOURCE ok, STYLE 0.267; skel HARD 0 warn=14. **Correct-текст байт-идентичен HEAD во ВСЕХ 15 блоках; стемы не тронуты** (только c1-реордер меток + enrich дистракторов). **spring-graphql FINAL.**
- **УРОК parens-only DETAIL + disproven length-tell (2026-07-16):** (1) DETAIL_PARITY может фейлить ТОЛЬКО по одной оси (`parens`) при паритете code/numbers — фикс не трогает byte-locked correct, а поднимает prose-скобки дистракторов (`parens()` считает raw `(`, вкл. код-спаны). (2) blind-ревьюер склонен пост-рационализировать «correct = самый длинный» — сверять с мех-замером `words()`: здесь correct оказался САМЫМ КОРОТКИМ в 3 из 4 флагнутых блоков, tell = знание API, а не форма. Самый чистый byte-lock в ROUND-8: 15/15 correct нетронут (в отличие от rest-client Q11 §1-фикса и mtls trim-all). Оптимизация: 0 авторинг-субагентов на enrich/residual (прямая правка против гейта), 4 enrich + 2 blind band = 6 агентов/файл.
