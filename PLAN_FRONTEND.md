...
| `OVERLAYS` | Dialogs / popovers / dropdowns / confirmations | templates + `app.js` triggers | `overlay-state-atlas.html` | admin reset; confirmations; menus; popovers; tooltips; any discovered overlay | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ◐ | R0.106: атлас СОЗДАН + QA (file:// dark/light/narrow + живой демо scrim/focus-trap/Esc) — dialog/alertdialog/drawer/disclosure/tooltip (зеркалят прод `#kbd-help-overlay`/`#reset-confirm`/`#mobile-menu`) + menu/popover (кандидаты, не в проде). **R0.154 живьём:** reset-confirm (`.prompt-overlay`) прод-verified — role=alertdialog/aria-modal/scrim/focus-trap(→«Отмена»)/Esc+Cancel dismiss. **Находка OVL-1 (P3):** Esc-dismiss НЕ восстанавливает фокус на trigger (уходит на body), а Cancel-клик — восстанавливает (`danger-btn`); асимметрия focus-restore, WCAG 2.4.3. Чинить в выделенном OVERLAYS-тике (вместе с прод-верификацией kbd-help + mobile-menu). Final ◐ — прод-parity gated (OVL-1 + 2 оставшихся overlay) |
