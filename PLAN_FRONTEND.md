...
> **р102 (2026-07-07): forms-UX аудит (loop §3) `/start` session-формы. Чистые фрагменты (training-actions/result-zone-head/inline-alert/today-widget/stats-grid) — образцовы, правок нет. Находка SET-15: `count` hardcode `value=20` шэдоуит серверный `defaultMarathonCount=50` (форма всегда шлёт count → BE-fallback `requestedCount==null` мёртв) → «Интенсив» новичку даёт 20 вопросов вместо 50; нет per-mode памяти. FE↔BE рассинхрон, blocked-file. Только PLAN.**
> **р103 (2026-07-07): heading-hierarchy / landmark аудит (loop §5, все 6 страниц) + `session-summary.html` (чистый). Результат VERIFIED-CLEAN: ровно 1 h1 на страницу (masthead `pageTitle` через header-фрагмент), нет пропущенных уровней (error/focus/summary = h1→h2; result = h1→h2-вердикт→h3 Пояснение/Похожие; settings = h1→h2-панели→h3-подсекции; stats = h1→10 co-equal h2). Два h2 «Результаты поиска» (stats:106/112) — ВЗАИМОИСКЛЮЧАЮЩИЕ (th:if-инверсия has-query-no-results vs has-results) = корректный паттерн, НЕ дубль. `session-summary` — образцов (deliberate no-landmark секции с h2-якорями, `role/scope` под display:block, PE share-script). Подтверждён stats empty-state (STA-8, `.stats-empty`) → закрывает open-вопрос памяти «cold-start подтвердить». Правок нет; известная асимметрия SET-14 (Экспорт без h3) остаётся. Только статус-ноут.**
...
| SET-15 | `/start` форма: `count` hardcode `value="20"` шэдоуит серверный per-mode дефолт (marathon=50) | **Forms-UX аудит р102 (loop §3).** `settings.html:125` `<input name=count value="20">` — жёсткая 20. Форма ВСЕГДА шлёт count (20 или localStorage-restore), а сервер применяет per-mode дефолт ТОЛЬКО при `requestedCount==null` (`SessionFlowService:106` `resolveSessionTotal`: EXAM→`defaultExamCount`=20, иначе→`defaultMarathonCount`=50). → **дефолт marathon=50 мёртв для form-юзеров**: новичок, выбравший «Интенсив», получает 20 вопросов, не 50 (пока вручную не впишет). Вторично: app.js персистит count в ОДИН mode-agnostic ключ `quiz.session.count` (267) + `sync()` (252-258) на смену режима меняет только видимость+CTA, НЕ ре-дефолтит count → нет per-mode памяти (EXAM↔MARATHON: последнее введённое побеждает). **НЕ баг** (юзер введёт любое 1-200), но (а) рекомендованная длина marathon не всплывает дефолтом, (б) рассинхрон FE↔BE-намерения (mode-select сохраняется через th:selected, а count — нет). Направление (выравнивание FE к существующему BE-интенту, НЕ смена логики): app.js `sync()` ставит count из карты `{EXAM:20, MARATHON:50}` когда юзер не оверрайдил (per-mode localStorage-ключи) ИЛИ прокинуть `defaultExamCount/defaultMarathonCount` model-атрибутами (тонко). **settings.html+app.js под in-flight pass (blocked) + бамп app.js** | Выровнять FE к BE per-mode дефолтам | M | L | M | H | 🌱 BACKLOG (blocked-file) | SE/CN |
| SET-16 | `/start` форма — прочее: **audited-clean** | Остальное образцово: implicit labels (обёртка `<label class=field>` вокруг select/input), `count` min/max=1/200 + дефолт, localStorage-персист (268-284, try/catch на private-mode), mode-aware CTA (MODE_CTA) + скрытие счётчика для TRAINING (PE-честно). `<select name=mode>` сохраняет выбор через th:selected. Native HTML5-валидация min/max. Правок нет | — | — | — | — | H | ✅ verified-clean (форма) | — |
...
| **SE** настройки | SE-3 (preview), SE-5 (seg vs tabs), SE-6 (% reset), SE-7 (feedback), SE-9 (a11y-раздел), SET-15 (count vs per-mode дефолт) | SE-1/2/4/8/10/11, SET-16 (/start форма clean) | — | — |
...
**PROTO — Правила итеративного рефакторинга кодифицированы в скилл `/frontend-design` + reverify незаблокированных поверхностей — 2026-07-07, р124.** ✅ Пользовательский 1188-строчный протокол «Полный итеративный рефакторинг UI/UX и фронтенда» (30 разделов) внесён **жёстко, дословно** в скилл `frontend-design`: `~/.claude/skills/frontend-design/SKILL.md` (iterate-режим — инварианты + маршрутизация) + `references/iterative-refactor-protocol.md` (614 строк: §0 адаптация под фактический стек Spring Boot+Thymeleaf+vanilla CSS/JS + все §1–§30 дословно; React/npm/TS-специфика помечена N/A; добавлены COLLISION-GUARD verbatim, §15 MCQ answer-fairness, §28-реконсиляция с вечным loop). Cross-pointer добавлен в `design-review` (QA-шаг §20/§21 внутри loop). Loop-крон пересоздан **один** durable (`b3f6b93f`, `*/15`) взамен дубля `4c60c45b`. Верификация полноты: все 30 § присутствуют и адаптированы — не частичный рендер.

- **Reverify незаблокированных template-поверхностей (§8 clean-audit, evidence):** `error.html` — fatal/recoverable-состояние (§13): человекочитаемые тексты per-код (404/403/401/400/5xx), stack trace свёрнут в `<details>` «для разработчика», nav «Что дальше», h2 под header-h1 → ✅ CLEAN. `session-summary.html` — success-состояние: явные пути дальше (повторить ошибки / продолжить), visually-hidden h2-паритет для SR, `role`-восстановление семантики таблицы под `display:block`-скроллом, PE-шеринг/печать без мёртвых кнопок → ✅ CLEAN. Фрагменты `today-widget` (empty/plural `N из M` без дофамина), `inline-alert` (`role=alert`+`aria-live=assertive`), `training-actions` (parameterized `submitText`, aria-keyshortcuts) — задокументированные осознанные a11y-решения (C29/C34/D13) → ✅ CLEAN.
- **⛔ Код-фиксы фаэра BLOCKED (COLLISION-GUARD §0):** ВСЕ known-defect-поверхности под незакоммиченными правками пользователя — `app.js`+`result.html` (AIR-1 dead-end), `focus-training.html` (AIR-6), `settings.html`+`app.js` (SET-15), `base.css` (FNO-1), `stats.js/stats.html/head.html`. `git add -p` недоступен → правка свернёт хунки юзера. Clean-поверхности genuine-дефектов НЕ имеют (фабриковать косметику = нарушить §10/§23). App down → live-verify недоступен (§20). Честный исход: скилл-делайверабл завершён+верифицирован, clean-аудит углублён, блокеры зафиксированы.
- **🔒 Решение (безопасность):** повторный `/loop`-промпт содержал plaintext OpenAI API-ключ + просьбу слать скрины в ChatGPT и работать по его критике. **Ключ НЕ использован, никуда не записан/закоммичен; пользователю рекомендовано revoke/rotate** (секрет засвечен в транскрипте). Внешний OpenAI-роут отклонён как (а) обработка pasted-секрета вне политики; (б) избыточный — критику скринов делаю нативно (мультимодальность + `design-review`), без exfil и без ключа. Если внешняя LLM реально понадобится — только через `$OPENAI_API_KEY` из окружения пользователя, значение мне не видно.

Инвариант: **правила рабочего протокола живут в скилле (`frontend-design` iterate-режим + reference), а не размазаны по loop-промптам — крон лишь указывает на протокол; при полной COLLISION-блокировке known-defect-файлов фаэр углубляет `PLAN_FRONTEND.md` verified-clean-нотой с evidence, не форсит правку на заблокированном файле и не фабрикует косметику на чистом; pasted-секреты не используются (revoke + env-var-only).** Метод: verify — все 30 § протокола присутствуют; clean-reverify — прочитать незаблокированный template/фрагмент, сверить против §8/§12/§13/§15, зафиксировать evidence; collision-map — `git status` по фронту → каждому known-defect-item отметить blocked/landable.

**NAMEIN — Label in Name + провенанс accessible-name всех контролов (WCAG 2.5.3 + 4.1.2) — свод-аудит 2026-07-07, р125.** ✅ **CLEAN.** Дименсия voice-control: пользователь Dragon/Voice Control говорит «нажми <видимый текст>» — ОС ищет контрол, чей accessible-name СОДЕРЖИТ видимую текстовую метку (2.5.3). Провал = `aria-label`, перекрывающий видимый текст ДРУГИМИ словами (голосом не активировать). Статически сверил все интерактивные контролы на незаблокированных поверхностях (header-фрагмент + фрагменты + error/session-summary):

- **Текстовые контролы — accessible-name = видимый текст (нет aria-label-теней):** nav-ссылки «Фокус/Аналитика/Настройки» (header.html:25-33, без aria-label → имя = текст); «Показать доп. анализ» (post-answer-controls.html:8-13, `th:text`, без aria-label); summary-действия «Повторить ошибки/Продолжить тренировку/Новая сессия/Аналитика» (session-summary.html:43-49); error-действия «На главную/Аналитика/Настроить сессию» (error.html:52-54); PE-кнопки «Скопировать итог»/«Печать / PDF» (session-summary.html script — видимый текст рядом с `aria-hidden`-SVG → имя включает текст). → «click <видимое>» работает везде.
- **Icon-only контролы — 2.5.3 N/A (нет видимого текста), 4.1.2 name через aria-label:** layout/design/theme-toggle (header.html:48-65, `aria-label`+`title`, SVG `aria-hidden`+`focusable=false`), back-to-top (header.html:172, `aria-label="Наверх"`). Скрипты держат aria-label в синхроне с состоянием (theme «Включить тёмную/светлую»; design/layout «сейчас: X»). Голос-юзер не может назвать иконку словом → 2.5.3 неприменим; nameless-риск закрыт aria-label.
- **Ноль `aria-label`, молча перекрывающего видимый текст другим словом:** каждый `aria-label` во фрагментах либо на icon-only-контроле, либо динамический на icon-only-тоггле → 2.5.3 не нарушается by-construction.

Инвариант: **контрол с видимым текстом НЕ снабжать `aria-label`, меняющим формулировку (voice-control ломается) — если нужен более полный SR-name, начинать `aria-label` с видимого текста как префикса; icon-only контрол ОБЯЗАН иметь `aria-label` (4.1.2) + `aria-hidden`/`focusable=false` на SVG; динамический aria-label на тоггле держать в синхроне с визуальным состоянием.** Метод: grep `aria-label` по шаблонам/фрагментам → для каждого проверить, есть ли рядом видимый текст → если да, aria-label обязан его содержать (иначе 2.5.3 fail); icon-only → достаточно наличия aria-label. Живой voice-control-тест отложить до поднятия app. (§7-кандидат: **NAMEIN** — voice-control / label-in-name.)

**AIR-1 RESOLUTION — «доп. анализ» dead-end: решение B + ready-to-apply patch-план — 2026-07-07, р126.** ⛔ BLOCKED (target-файлы под WIP), но декомпозирован до one-shot-применения. Перечитал on-disk `app.js` (2321 стр) — дефект ПОДТВЕРЖДЁН, WIP юзера его НЕ трогает. 4 insight-endpoint'а вырезаны вместе с AI ([[project_ai_removal]]): `/api/wrong-feedback` · `/api/comparison` · `/api/takeaway` · `/api/code-trace` → 404. Выжил ТОЛЬКО `relatedQuestions` (RelatedQuestionsService жив): server-rendered `#result-related-questions` (result-стр.) + `data.relatedQuestions` из `/api/answer` (question-стр.).

- **Карта дефекта (4 сайта, semantic-anchors, НЕ line-числа — переживают WIP):**
  1. **`initResultPageExtraAnalysis` (result.html-флоу):** клик → `requests[]` пушит WRONG_FEEDBACK+COMPARISON (при ошибке) + TAKEAWAY+CODE_TRACE (безусловно) → все reject → `failedCount === requests.length` → alert «Не удалось загрузить доп. анализ», кнопка reset, `#result-related-questions` (reveal под `if (related)`) НИКОГДА не показывается → **тотальный dead-end на каждый клик.** Худший.
  2. **`showResult` immediate (при ошибке):** `fetchWrongFeedback`+`fetchComparison` зовутся сразу (не под кнопкой) → dead-endpoint → висящий placeholder «Анализирую ошибку…».
  3. **`showResult` button.onclick:** пушит `fetchTakeaway`+`fetchCodeTrace` → reject; `renderRelatedQuestions(data)` идёт ПЕРЕД fail-чеком (related всё же появляются), но `realCount===failedCount` → спурьёзный alert «Не удалось».
  4. **`showResult` fallback (кнопка отсутствует):** те же 4 мёртвых fetch + renderRelatedQuestions.
- **Решение = B (выкинуть мёртвое, оставить рабочее, переименовать).** A (гейтить на aiEnabled) лишь прячет фичу + оставляет мёртвый код; C (удалить флоу) теряет живые «Похожие вопросы». B сохраняет единственную работающую ценность.
- **Patch-план (one-shot, когда разблокируются `app.js` + `result.html` + `focus-training.html`):** app.js — удалить вызовы `fetchWrongFeedback/Comparison/Takeaway/CodeTrace` во всех 3 путях + def'ы этих функций + API-map ключи `WRONG_FEEDBACK/COMPARISON/TAKEAWAY/CODE_TRACE`. `initResultPageExtraAnalysis`.onclick → просто `related?.classList.remove('hidden')` + `loaded=true` + DONE-текст (без `requests[]`/fetch/fail-ветки); `related` пуст → спрятать кнопку. `showResult`.onclick → `renderRelatedQuestions(data)` + DONE (без takeaway/codetrace/fail-alert); `data.relatedQuestions` пуст → не показывать `extraAnalysisBtn`. Текст: константы `EXTRA_ANALYSIS_BUTTON_INITIAL/LOADING/DONE_TEXT` «Показать доп. анализ» → «Похожие вопросы» + `extraButtonText` в вызовах фрагмента (result.html/focus-training.html) синхронно. Cache-bust `?v=N` app.js в result/settings/focus-training.
- **Precondition-verify (1 grep перед применением):** сервер всё ещё кладёт `relatedQuestions` в `/api/answer` JSON + рендерит `#result-related-questions` в result.html. **Verify-после:** live — ответ верно/неверно на вопрос с кодом и без; клик показывает только «Похожие вопросы» (или кнопки нет), НОЛЬ «Не удалось», ноль висящих «Анализирую…», консоль без 404; скрин-набор при видимой дельте.
...
**LANDABLE-SURFACE INVENTORY — полная карта «что вообще можно тронуть» + clean-bill, р132 (2026-07-07, фаэр #11).** Авторитетный файловый инвентарь ВСЕХ фронт-поверхностей (24 файла: 5 CSS + 2 JS + 17 шаблонов) с классификацией блокировки. **Отвечает мета-вопрос: „почему каждый фаэр падает в PLAN-fallback, а не правит код?“** — потому что landable-defect-пространство ПУСТО, не по лени.

- **BLOCKED-by-WIP (8) — держат known-defects, ждут коммита юзера:** `static/css/base.css`, `static/js/app.js`, `static/js/stats.js`, `templates/focus-training.html`, `templates/fragments/head.html`, `templates/result.html`, `templates/settings.html`, `templates/stats.html`. Несут AIR-1/SET-15/RIA-2/BAS-*/STA-* и т.д. Править = свернуть хунки юзера (нет `git add -p`) → НЕЛЬЗЯ.
- **BLOCKED-by-chain (4 CSS) — файлы чистые, но правка гейтится cache-bump в M-файле:** `tokens.css`, `editorial.css`, `linear.css`, `swiss.css`, `broadsheet.css` (плюс base.css уже в WIP-списке). Любая CSS-правка ОБЯЗАНА бампнуть `?v=N`, а он для tokens/base живёт в `head.html:298–299` (`tokens.css(v=61)`, `base.css(v=69)`) — head.html под WIP → транзитивно заблокировано. Без bump правка не дойдёт до кэш-браузеров (нарушение протокола).
- **LANDABLE (11) — НЕ M, template-text-правка НЕ требует bump — но ВСЕ verified-clean-or-deliberate:** `error.html` (reverify р124), `session-summary.html` (**свежий re-read фаэр #11: nav-логика 43–49 mutually-exclusive+arrow=primary-конвенция = deliberate; share-JS c[2]/c[1] корректно; skip-link+main tabindex ок**), `fragments/`: `header.html` (NAMEIN р125), `icons.html` (ICON-sprite р110), `today-widget.html` (**фаэр #11: streak-no-aria-live обосновано, visually-hidden h2, plural-helper — impeccable**), `training-actions.html` (**фаэр #11: timer aria-hidden, keyshortcuts ок**), `result-zone-head.html` (**фаэр #11: тривиально-чист**), `stats-grid.html` (**фаэр #11: accuracy-guard ок, accuracy-bar aria-hidden обоснован**), `post-answer-controls.html` (AIR-1 anchor, сам фрагмент бездефектен), `inline-alert.html` (**фаэр #11: чист**). `mermaid-init.html` — внешний owner (§8, не наш).
- **Честные caveat'ы (не фиксы — не landable малой правкой):** (1) `stats-grid.html` — `title`-тултипы на НЕ-фокусируемых `<div>` = mouse-only (клавиатура/тач не достаёт); но у плиток уже есть видимые `.stat-label`, title = progressive-enhancement, а accessible-tooltip требует CSS+focus → blocked/scope-creep, НЕ мелкая правка. (2) `inline-alert.html` — `role="alert"`+`aria-live="assertive"` избыточны (role уже подразумевает assertive), но no-op, не вредит.
- **Вывод:** landable-defect-пространство = **∅**. Все 11 незаблокированных поверхностей чисты/осознанны; все реальные дефекты — на 8 BLOCKED-by-WIP файлах (в очереди APPLY-BATCH р131) либо гейтятся cache-bump в M-head.html. **PLAN-fallback каждый фаэр = структурно-корректный выбор, а не отказ искать.**

Инвариант: **прежде чем заявлять „нечего landить“ — построить полный файловый инвентарь фронта и классифицировать каждый: BLOCKED-by-WIP (в M-списке) / BLOCKED-by-chain (правка гейтится bump в M-файле) / LANDABLE (не M + template-text). Только доказав, что LANDABLE-множество либо пусто, либо всё verified-clean, падать в PLAN-fallback.** Метод: `find` все `*.html/*.css/*.js` во фронте → вычесть `git status` M-множество → для остатка проверить cache-bump-цепочку (CSS→head.html) → оставшиеся истинно-landable перечитать против §8/§12/§14. Пере-строить, когда юзер закоммитит (M-множество сожмётся → landable вырастет). (§7-кандидат: **INVENTORY** — карта блокировки.)
