...
| `RESULT` | Результат ответа / no-JS fallback | `result.html` | `result.html` | correct; wrong; explanations; related; extra controls; loading/error; no-JS | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ◐ | ◐ | ⏸ | **Parity ✅ (R0.151):** no-JS fallback live (native `form.submit()` → POST /answer → полная result.html SSR); wrong-вердикт, option-маркеры correct/wrong/other, per-option+main «Пояснение», SM-2 disclosure, next-btn, RES-16 related (proper links в wide sticky-рейке ≥1200px, инлайн-static на 375), favorite aria-pressed тоггл двунаправл. (A1); dark+light; 375 0-overflow. correct-вердикт = тот же th:classappend-бранч (template-verified). **Final ⏸:** DoD-свип (zoom/DPR/designs) → R0.152 |
