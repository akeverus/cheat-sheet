...
|---|---|---|---|---|
| 1 | Неверный правильный ответ (Spring `REQUIRES_NEW` self-invocation → A, не B) | MCQ-контент | **MCQ** | seed/mcq JSON — домен параллельной MCQ-сессии; я НЕ трогаю. Пользователь прав по сути (self-invocation минует прокси). **Решение пользователя нужно** (взять мне / оставить MCQ-сессии) |
...
| 8 | Широкие экраны 1920–2560: UI мелкий «остров» | FOCUS/layout | **DECISION-CONFLICT** | max-width 820–900 + масштаб заголовка/кода. **КОНФЛИКТ:** противоречит осознанному решению юзера «prose --measure full-width, 259ch на 2560» (см. память project_design_elevation_round1). §5-протокол |
...
**Порядок исполнения (по приоритету пользователя):** 1(MCQ→решение) → 2(состояния ответа) → 3(mobile-кнопка) → 4(обрезка кода) → 5/6/7(mobile прогресс/шапка/заголовок) → 8(широкие, §5-конфликт) → тексты 16/19/20 → a11y+состояния 11/14. Волна фиксов пойдёт по тикам, каждый фикс — live-verify + отдельный коммит (§2b). Быстрые текст-вины (19/16) можно раньше.
