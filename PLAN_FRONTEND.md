...
| 2 | Галочка на выбранном ДО проверки = ложное «верно» | FOCUS | **FE-FIX/VERIFY** | разделить состояния: selected(бирюза/radio) vs correct-после(зелёный/✓) vs wrong(красный/✗); CTA «Проверить»→«Следующий». Проверить текущее поведение опции |
...
**Порядок исполнения (по приоритету пользователя; #1 и #8 ИСКЛЮЧЕНЫ решением 2026-07-14):** 2(состояния ответа) → 3(mobile-кнопка) → 4(обрезка кода) → 5/6/7(mobile прогресс/шапка/заголовок) → 9/10(карточки) → 13(disclosure) → 14/15(клавиатура+CTA) → тексты 16/19/20 → a11y+состояния 11/12/17/18. Волна фиксов пойдёт по тикам, каждый фикс — live-verify + отдельный коммит (§2b). Быстрые текст-вины (19/16) можно раньше. **Первый фикс-тик: #2 (разделение состояний ответа selected/correct/wrong)** — самый высокий валидный приоритет.
