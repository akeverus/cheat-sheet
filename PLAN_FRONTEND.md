...
| 3 | Mobile: кнопка между кодом и вариантами (нарушен порядок) | FOCUS | **FE-FIX** | sticky-снизу после выбора + padding-bottom контенту + варианты выше в DOM/визуально |
| 4 | Mobile: код обрезается | FOCUS | **FE-FIX** | h-scroll без переноса + градиент-справа + «Развернуть код» + убрать фикс-высоту |
| 5 | Mobile: шапка слишком высокая | SHELL | **FE-FIX/DECISION** | 56–64px высота, 40–44px кнопки, лого меньше; «Завершить сессию» в session-bar (меню в проде нет) |
| 6 | Mobile: блок прогресса разваливается | FOCUS | **FE-FIX** | цельная структура «Вопрос N из M · серия» + прогресс-бар |
| 7 | Mobile: заголовок вопроса слишком большой | FOCUS | **FE-FIX** | `clamp()` 34–40 моб / 44–52 планшет / 52–64 деск, line-height 1.08–1.12 |
...
| 9 | Mobile: карточка ответа теряет ширину (padding/буква/radio) | FOCUS/COMPONENTS | **FE-FIX** | сжать h-padding, gap буква↔текст, резерв справа; вся карточка кликабельна ≥44px |
...
| 11 | Нужны hover/focus/pressed состояния | COMPONENTS | **VERIFY/FE-FIX** | состояния есть в атласе — проверить live + `:focus-visible` ≠ selected |
