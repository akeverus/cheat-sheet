...
| `HEAD-7` | self-host chart.js vs CDN (CSP sourcemap) | Архитектурный + download-gated выбор | — | ⛔ DEFERRED | ⛔ DEFERRED confirmed R0.132: chart.js грузится с jsdelivr CDN (head.html:316-317, SRI-пиннинг на точную версию + crossorigin). Self-host = **download-gated** (скачать файл chart.js — permission-действие) + архитектурное решение (CSP/sourcemap). Статус точен, остаётся user-decision — НЕ стале. |
